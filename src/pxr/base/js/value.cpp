...
struct Js_Null {
    bool operator==(const Js_Null& v) const { return true; }
    bool operator!=(const Js_Null& v) const { return false; }
};
...
public:
    ...
    bool operator==(const Js_Wrapper& other) const { return Get() == other.Get(); }
...
struct JsValue::_Holder {
    ...
    using Variant =
            std::variant<Js_ObjectWrapper, Js_ArrayWrapper, std::string, bool, int64_t, double, Js_Null, uint64_t>;
...
static bool _CheckType(const JsValue::Type& heldType, const JsValue::Type& requestedType, std::string* whyNot) {
    if (heldType != requestedType) {
        if (whyNot) {
            *whyNot = TfStringPrintf("Attempt to get %s from value holding %s", _GetTypeName(requestedType).c_str(),
...
int64_t JsValue::GetInt64() const {
    ...
    if (IsUInt64()) return static_cast<int64_t>(GetUInt64());
...
uint64_t JsValue::GetUInt64() const {
    ...
    if (!IsUInt64()) return static_cast<uint64_t>(GetInt64());
