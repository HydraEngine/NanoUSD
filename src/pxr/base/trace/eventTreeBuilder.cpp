...
void Trace_EventTreeBuilder::OnBeginThread(const TraceThreadId& threadId) {
    ...
    {
        _threadStacks[threadId].emplace_back(TfToken(threadId.ToString()), TraceCategory::Default, 0, 0, false, true);
    }
}
...
void Trace_EventTreeBuilder::OnEvent(const TraceThreadId& threadIndex, const TfToken& key, const TraceEvent& e) {
    switch (e.GetType()) {
...
void Trace_EventTreeBuilder::_OnBegin(const TraceThreadId& threadId, const TfToken& key, const TraceEvent& e) {
...
void Trace_EventTreeBuilder::_OnEnd(const TraceThreadId& threadId, const TfToken& key, const TraceEvent& e) {
    _PendingNodeStack& stack = _threadStacks[threadId];
    _PendingEventNode* prevNode = &stack.back();
...
void Trace_EventTreeBuilder::_OnTimespan(const TraceThreadId& threadId, const TfToken& key, const TraceEvent& e) {
...
void Trace_EventTreeBuilder::_OnMarker(const TraceThreadId& threadId, const TfToken& key, const TraceEvent& e) {
    _markersMap[key].push_back(std::make_pair(e.GetTimeStamp(), threadId));
}

void Trace_EventTreeBuilder::_OnData(const TraceThreadId& threadId, const TfToken& key, const TraceEvent& e) {
    _PendingNodeStack& stack = _threadStacks[threadId];
    if (!stack.empty()) {
        _PendingEventNode* prevNode = &stack.back();

        // if that data doesn't fall in this node's timespan, look to prevNode
        while ((e.GetTimeStamp() < prevNode->start || e.GetTimeStamp() > prevNode->end) && stack.size() > 1) {
            _PopAndClose(stack);
            prevNode = &stack.back();
        }

        // Add data to the real node in the stack
        prevNode->attributes.push_back(_PendingEventNode::AttributeData{e.GetTimeStamp(), key, e.GetData()});
    }
}
...
Trace_EventTreeBuilder::_PendingEventNode::_PendingEventNode(const TfToken& key,
                                                             ...
                                                             bool separateEvents,
                                                             bool isComplete)
    : key(key), category(category), start(start), end(end), separateEvents(separateEvents), isComplete(isComplete) {}
...
TraceEventNodeRefPtr Trace_EventTreeBuilder::_PendingEventNode::Close() {
    ...
    TraceEventNodeRefPtr node = TraceEventNode::New(key, category, start, end, std::move(children), separateEvents);
