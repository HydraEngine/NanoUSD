...
TraceEvent::TimeStamp TraceEvent::GetTimeStamp() const {
    return _time;
}
...
TraceEventData TraceEvent::GetData() const {
    ...
    static_assert(alignof(PayloadStorage) >= alignof(TimeStamp), "Payload Error");
    ...
    if (_type == _InternalEventType::ScopeData || _type == _InternalEventType::ScopeDataLarge) {
        const void* data =
                _type == _InternalEventType::ScopeData ? &_payload : *reinterpret_cast<const void* const*>(&_payload);
        switch (_dataType) {
            ...
            case DataType::String:
                return TraceEventData(std::string(reinterpret_cast<const char*>(data)));
...
TraceEvent::EventType TraceEvent::GetType() const {
    switch (_type) {
        ...
        case _InternalEventType::Timespan:
            return EventType::Timespan;
        case _InternalEventType::Marker:
            return EventType::Marker;
        case _InternalEventType::CounterDelta:
            return EventType::CounterDelta;
        case _InternalEventType::CounterValue:
            return EventType::CounterValue;
        case _InternalEventType::ScopeData:
            return EventType::ScopeData;
        case _InternalEventType::ScopeDataLarge:
            return EventType::ScopeData;
    }
