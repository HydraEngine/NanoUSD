...
static void _PrintLineTimes(ostream& s,
                            ...
                            bool recursive_node,
                            int iterationCount) {
    string inclusiveStr = TfStringPrintf("%9.3f ms ", ArchTicksToSeconds(uint64_t(inclusive * 1e3) / iterationCount));
    if (inclusive <= 0) inclusiveStr = string(inclusiveStr.size(), ' ');

    string exclusiveStr = TfStringPrintf("%9.3f ms ", ArchTicksToSeconds(uint64_t(exclusive * 1e3) / iterationCount));
    if (exclusive <= 0) exclusiveStr = string(exclusiveStr.size(), ' ');
...
static void _PrintNodeTimes(ostream& s, TraceAggregateNodeRefPtr node, int indent, int iterationCount) {
    ...
    if (node->GetId().IsValid()) {
        ...
        _PrintLineTimes(s, node->GetInclusiveTime(), node->GetExclusiveTime(r), node->GetCount(r),
                        _GetKeyName(node->GetKey()), indent, r, iterationCount);
    }
...
void TraceReporter::_PrintTimes(ostream& s) {
    ...
    for (const SortedTimes::value_type& it : sortedTimes) {
        s << TfStringPrintf("%9.3f ms ", ArchTicksToSeconds((uint64_t)(it.first * 1e3))) << _GetKeyName(it.second)
          << "\n";
    }
}
...
void TraceReporter::Report(std::ostream& s, int iterationCount) {
    ...
    if (ShouldAdjustForOverheadAndNoise()) {
        _aggregateTree->GetRoot()->AdjustForOverheadAndNoise(TraceCollector::GetInstance().GetScopeOverhead(),
                                                             ArchGetTickQuantum());
    }
...
/* static */ std::vector<TraceReporter::ParsedTree> TraceReporter::LoadReport(std::istream& stream) {
    ...
    static const auto traceRowRE = std::invoke([]() -> std::regex {
        ...
        const std::string msPattern = TfStringPrintf(R"(%s\s+%s?\s+)", msEntryPattern.c_str(), msEntryPattern.c_str());
        const std::string msPatternOptional = TfStringPrintf(R"((?:%s|(?:)\s+))", msPattern.c_str());
        ...
        return std::regex(R"(\s*)" + msPatternOptional + samplePattern + indentPattern + tagPattern);
    });
    ...
    for (std::string line; std::getline(stream, line);) {
        ...
        stack.push(
                parent->Append(TraceReporter::CreateValidEventId(),
                               /* key */ TfToken(match[5].str()),
                               /* timestamp */ ArchSecondsToTicks(currentIters * TfStringToDouble(match[1]) / 1000.0),
                               /* count */ samples,
...
void TraceReporter::_RebuildEventAndAggregateTrees() {
    ...
    if (root && !root->GetChildrenRef().empty() && TfMallocTag::IsInitialized()) {
        root->Append(TraceAggregateNode::Id(),
                     TfToken(TraceReporterTokens->warningString.GetString() + " MallocTags enabled"), 0, 1 /* count */,
                     1 /* exclusive count */);
    }
}
