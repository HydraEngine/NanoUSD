...
static void _OutputGlobalReport() {
...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
    const bool globalPyTracing = TfGetenvBool("PXR_ENABLE_GLOBAL_PY_TRACE", false);
...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
        if (globalPyTracing) SetPythonTracingEnabled(true);
#endif  // PXR_PYTHON_SUPPORT_ENABLED
    }
}
...
void TraceCollector::SetEnabled(bool isEnabled) {
    _isEnabled.store((int)isEnabled, std::memory_order_release);
}

void TraceCollector::Scope(const TraceKey& key, TimeStamp start, TimeStamp stop) noexcept {
    _PerThreadData* threadData = GetInstance()._GetThreadData();
    threadData->EmplaceEvent(TraceEvent::Timespan, key, start, stop, DefaultCategory::GetId());
}
...
TraceCollector::TimeStamp TraceCollector::GetScopeOverhead() const {
...
void TraceCollector::_EndScope(const TraceKey& key, TraceCategoryId cat) {
...
void TraceCollector::_MeasureScopeOverhead() {
    uint64_t* escape = &externallyVisibleValue;

    _measuredScopeOverhead = ArchMeasureExecutionTime([escape]() {
        TRACE_FUNCTION();
        (*escape)++;
    });
}
...
static inline TraceCollector::Key _MakePythonScopeKey(const TfPyTraceInfo& info) {
    string keyString = TfStringPrintf("%s() (py) in %s:%d (%s)", info.funcName, TfGetBaseName(info.fileName).c_str(),
                                      info.funcLine, info.fileName);
...
void TraceCollector::SetPythonTracingEnabled(bool enabled) {
...
TraceCollector::TimeStamp TraceCollector::_PerThreadData::BeginEvent(const Key& key, TraceCategoryId cat) {
    ...
    const TraceEvent& event = events->EmplaceBack(TraceEvent::Begin, events->CacheKey(key), cat);
...
TraceCollector::TimeStamp TraceCollector::_PerThreadData::EndEvent(const Key& key, TraceCategoryId cat) {
    ...
    AtomicRef lock(_writing);
    EventList* events = _events.load(std::memory_order_acquire);
    const TraceEvent& event = events->EmplaceBack(TraceEvent::End, events->CacheKey(key), cat);
    return event.GetTimeStamp();
}
...
TraceCollector::TimeStamp TraceCollector::_PerThreadData::MarkerEvent(const Key& key, TraceCategoryId cat) {
    ...
    AtomicRef lock(_writing);
    EventList* events = _events.load(std::memory_order_acquire);
    const TraceEvent& event = events->EmplaceBack(TraceEvent::Marker, events->CacheKey(key), cat);
    return event.GetTimeStamp();
}
...
void TraceCollector::_PerThreadData::BeginEventAtTime(const Key& key, double ms, TraceCategoryId cat) {
    ...
    const TimeStamp ts = uint64_t(ms * 1000 / double(ArchTicksToSeconds(1000000)));
    EventList* events = _events.load(std::memory_order_acquire);
    events->EmplaceBack(TraceEvent::Begin, events->CacheKey(key), ts, cat);
}

void TraceCollector::_PerThreadData::EndEventAtTime(const Key& key, double ms, TraceCategoryId cat) {
    AtomicRef lock(_writing);
    TfAutoMallocTag2 tag("Trace", "TraceCollector::_PerThreadData::EndEventAtTime");
    const TimeStamp ts = uint64_t(ms * 1000 / double(ArchTicksToSeconds(1000000)));
    EventList* events = _events.load(std::memory_order_acquire);
...
void TraceCollector::_PerThreadData::_EndScope(const TraceKey& key, TraceCategoryId cat) {
...
void TraceCollector::_PerThreadData::Clear() {
