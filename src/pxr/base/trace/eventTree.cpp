...
TraceEventTreeRefPtr TraceEventTree::New(const TraceCollection& collection, const CounterMap* initialCounterValues) {
    ...
    for (const TraceEventTree::CounterValuesMap::value_type& c : counters) {
        for (const TraceEventTree::CounterValues::value_type& v : c.second) {
            js.WriteObject("cat", "",
                           ...
                           "tid", 0, "pid", pid, "name", c.first.GetString(), "ph", "C", "ts",
                           _TimeStampToChromeTraceValue(v.first), "args", [&v](JsWriter& js) {
...
static void TraceEventTree_WriteMarkers(const int pid, const TraceEventTree::MarkerValuesMap& markers, JsWriter& js) {
    for (const TraceEventTree::MarkerValuesMap::value_type& m : markers) {
        for (const TraceEventTree::MarkerValues::value_type& v : m.second) {
            js.WriteObject("cat", "", "tid", v.second.ToString(), "pid", pid, "name", m.first.GetString(), "ph",
                           "I",       // Mark
                           "s", "t",  // Scope
