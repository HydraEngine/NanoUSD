...
public:
    ...
    TRACE_API static TraceCollector& GetInstance() { return TfSingleton<TraceCollector>::GetInstance(); }
    ...
    static bool IsEnabled() { return (_isEnabled.load(std::memory_order_acquire) == 1); }
    ...
    bool IsPythonTracingEnabled() const { return _isPythonTracingEnabled.load(std::memory_order_acquire) != 0; }
    ...
    void BeginScope(const TraceKey& _key) {
        if (ARCH_LIKELY(!Category::IsEnabled())) return;
    ...
    void BeginScope(const TraceKey& key, Args&&... args) {
        ...
        BeginScope<DefaultCategory>(key, std::forward<Args>(args)...);
    }
    ...
    void MarkerEventStatic(const TraceKey& key) {
        ...
        _PerThreadData* threadData = _GetThreadData();
        threadData->EmplaceEvent(TraceEvent::Marker, key, Category::GetId());
    }
...
private:
    ...
    void _StoreData(_PerThreadData* threadData, const TraceKey& key, TraceCategoryId cat, const std::string& value) {
    ...
    void _StoreDataRec(_PerThreadData* threadData, TraceCategoryId cat, K&& key, const T& value, Args&&... args) {
    ...
    public:
        ...
        // Debug Methods
        void BeginEventAtTime(const Key& key, double ms, TraceCategoryId cat);
        ...
        TRACE_API void CounterDelta(const Key&, double value, TraceCategoryId cat);
    ...
    private:
        ...
        class AtomicRef {
        public:
            AtomicRef(std::atomic<bool>& b) : _bool(b) { _bool.store(true, std::memory_order_release); }
