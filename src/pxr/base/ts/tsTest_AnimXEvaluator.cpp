...
TsTest_AnimXEvaluator::TsTest_AnimXEvaluator(const AutoTanType autoTanType) : _autoTanType(autoTanType) {}

namespace {
class _Curve : public adsk::ICurve {
public:
    _Curve(const SData& data, const Evaluator::AutoTanType autoTanType) : _autoTanType(autoTanType) {
        ...
        for (const SData::Knot& kf : data.GetKnots()) {
            ...
            axKf.linearInterpolation = (kf.nextSegInterpMethod == SData::InterpLinear);
        ...
        if (data.GetPreExtrapolation().method == SData::ExtrapLinear) {
            ...
            if (_kfs.size() == 1 || axKf.tanOut.type == adsk::TangentType::Step) {
                // Mirror a held (flat) segment.
                axKf.tanIn.x = 1.0;
                axKf.tanIn.y = 0.0;
            } else if (axKf.tanOut.type == adsk::TangentType::Linear) {
                // Mirror a linear segment.
                const adsk::Keyframe& axNextKf = (++_kfs.begin())->second;
        ...
        if (data.GetPostExtrapolation().method == SData::ExtrapLinear) {
            ...
            if (_kfs.size() == 1 || axKf.tanIn.type == adsk::TangentType::Step) {
                // Mirror a held (flat) segment.
                axKf.tanOut.x = 1.0;
                axKf.tanOut.y = 0.0;
            } else if (axKf.tanIn.type == adsk::TangentType::Linear) {
                // Mirror a linear segment.
                const adsk::Keyframe& axPrevKf = (++_kfs.rbegin())->second;
    ...
    bool keyframeAtIndex(const int index, adsk::Keyframe& keyOut) const override {
    ...
    bool keyframe(const double time, adsk::Keyframe& keyOut) const override {
    ...
    adsk::InfinityType preInfinityType() const override { return _preInfinity; }

    adsk::InfinityType postInfinityType() const override { return _postInfinity; }

    bool isWeighted() const override { return _isWeighted; }

    unsigned int keyframeCount() const override { return _kfs.size(); }
...
private:
    static adsk::InfinityType _GetInfinity(const SData::Extrapolation& extrap) {
        // Non-looped modes.
    ...
    static adsk::TangentType _GetBaseTanType(const SData::InterpMethod method) {
        switch (method) {
            case SData::InterpHeld:
                return adsk::TangentType::Step;
            case SData::InterpLinear:
                return adsk::TangentType::Linear;
    ...
    adsk::TangentType _GetTanType(const adsk::TangentType tanType, const bool isAuto) const {
        if (tanType != adsk::TangentType::Global || !isAuto) {
            return tanType;
        }

        switch (_autoTanType) {
            case Evaluator::AutoTanAuto:
                return adsk::TangentType::Auto;
            case Evaluator::AutoTanSmooth:
                return adsk::TangentType::Smooth;
        }
...
TsTest_SampleVec TsTest_AnimXEvaluator::Eval(const SData& data, const STimes& times) const {
    static const SData::Features supportedFeatures = SData::FeatureHeldSegments | SData::FeatureLinearSegments |
                                                     SData::FeatureBezierSegments | SData::FeatureHermiteSegments |
                                                     SData::FeatureAutoTangents | SData::FeatureExtrapolatingLoops;
