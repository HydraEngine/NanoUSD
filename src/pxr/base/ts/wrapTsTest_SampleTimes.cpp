...
static std::string _SampleTimeRepr(const This::SampleTime& st) {
    std::ostringstream result;

    result << "Ts.TsTest_SampleTimes.SampleTime(" << _HexFloatRepr(st.time) << ", " << st.pre << ")";

    return result.str();
}

static This* _ConstructSampleTimes(const object& times) {
...
static std::string _SampleTimesRepr(const This& times) {
    std::ostringstream result;

    std::vector<std::string> stStrs;
    for (const This::SampleTime& st : times.GetTimes()) stStrs.push_back(_SampleTimeRepr(st));
...
void wrapTsTest_SampleTimes() {
    ...
    classObj
            ...
            .def("__init__",
                 make_constructor(&_ConstructSampleTimes, default_call_policies(), (arg("times") = object())))

            .def("__repr__", &_SampleTimesRepr)

            .def("AddTimes", static_cast<void (This::*)(const std::vector<double>&)>(&This::AddTimes))

            .def("AddTimes", static_cast<void (This::*)(const std::vector<This::SampleTime>&)>(&This::AddTimes))

            .def(init<const TsTest_SplineData&>())

            .def("AddKnotTimes", &This::AddKnotTimes)

            .def("AddUniformInterpolationTimes", &This::AddUniformInterpolationTimes, (arg("numSamples")))

            .def("AddExtrapolationTimes", &This::AddExtrapolationTimes, (arg("extrapolationFactor")))

            .def("AddExtrapolatingLoopTimes", &This::AddExtrapolatingLoopTimes,
                 (arg("numIterations"), arg("numSamplesPerIteration")))

            .def("AddStandardTimes", &This::AddStandardTimes)

            .def("GetTimes", &This::GetTimes, return_value_policy<TfPySequenceToList>())
