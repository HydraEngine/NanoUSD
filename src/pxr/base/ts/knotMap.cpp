...
TsKnotMap::TsKnotMap(std::initializer_list<TsKnot> knots) {
    for (const TsKnot& knot : knots) insert(knot);
}
...
TsKnotMap::TsKnotMap(const Ts_SplineData* const data) {
    ...
    for (size_t i = 0; i < numKnots; i++) {
        if (haveCustom) {
            knotCustom = TfMapLookupByValue(data->customData, data->times[i], VtDictionary());
        }

        // This incurs a virtual method call per knot.  Could be improved, but
        // header dependencies make it not trivial.
        TsKnot knot(data->CloneKnotAtIndex(i), valueType, std::move(knotCustom));
...
TsKnotMap::iterator TsKnotMap::begin() {
    return _knots.begin();
}

TsKnotMap::const_iterator TsKnotMap::begin() const {
    return _knots.begin();
}

TsKnotMap::const_iterator TsKnotMap::cbegin() const {
    return _knots.cbegin();
}

TsKnotMap::iterator TsKnotMap::end() {
    return _knots.end();
}

TsKnotMap::const_iterator TsKnotMap::end() const {
    return _knots.end();
}

TsKnotMap::const_iterator TsKnotMap::cend() const {
    return _knots.cend();
}

TsKnotMap::reverse_iterator TsKnotMap::rbegin() {
    return _knots.rbegin();
}

TsKnotMap::const_reverse_iterator TsKnotMap::rbegin() const {
    return _knots.rbegin();
}

TsKnotMap::const_reverse_iterator TsKnotMap::crbegin() const {
    return _knots.crbegin();
}

TsKnotMap::reverse_iterator TsKnotMap::rend() {
    return _knots.rend();
}

TsKnotMap::const_reverse_iterator TsKnotMap::rend() const {
    return _knots.rend();
}

TsKnotMap::const_reverse_iterator TsKnotMap::crend() const {
    return _knots.crend();
}
...
std::pair<TsKnotMap::iterator, bool> TsKnotMap::insert(const TsKnot& knot) {
...
TsKnotMap::iterator TsKnotMap::erase(iterator i) {
    return _knots.erase(i);
}

TsKnotMap::iterator TsKnotMap::erase(iterator first, iterator last) {
    return _knots.erase(first, last);
}

TsKnotMap::iterator TsKnotMap::find(const TsTime time) {
    const iterator lbIt = lower_bound(time);
    if (lbIt != _knots.end() && lbIt->GetTime() == time) {
        return lbIt;
    }
...
TsKnotMap::const_iterator TsKnotMap::find(const TsTime time) const {
...
TsKnotMap::iterator TsKnotMap::lower_bound(const TsTime time) {
    return std::partition_point(_knots.begin(), _knots.end(), [=](const TsKnot& knot) {
        return knot.GetTime() < time;
    });
}

TsKnotMap::const_iterator TsKnotMap::lower_bound(const TsTime time) const {
    return std::partition_point(_knots.begin(), _knots.end(), [=](const TsKnot& knot) {
        return knot.GetTime() < time;
    });
}

TsKnotMap::iterator TsKnotMap::FindClosest(const TsTime time) {
    const const_iterator it = const_cast<const TsKnotMap*>(this)->FindClosest(time);

    return _knots.begin() + (it - _knots.cbegin());
}

TsKnotMap::const_iterator TsKnotMap::FindClosest(const TsTime time) const {
