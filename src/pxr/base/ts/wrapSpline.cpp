...
static TsSpline* _WrapInit(const std::string& typeName) {
...
static std::string _WrapStr(const TsSpline& spline) {
    return TfStringify(spline);
}

static std::string _WrapGetValueTypeName(const TsSpline& spline) {
    return Ts_GetTypeNameFromType(spline.GetValueType());
}

static GfInterval _WrapSetKnot(TsSpline& spline, const TsKnot& knot) {
    GfInterval affectedInterval;
    spline.SetKnot(knot, &affectedInterval);
    return affectedInterval;
}

static object _WrapGetKnot(const TsSpline& spline, const TsTime time) {
    TsKnot knot;
...
static void _WrapRemoveKnot(TsSpline& spline, const TsTime time) {
    spline.RemoveKnot(time);
}

#define WRAP_EVAL(method)                                                    \
    static object _Wrap##method(const TsSpline& spline, const TsTime time) { \
        double val = 0;                                                      \
...
void wrapSpline() {
    ...
    class_<This>("Spline", no_init)
            ...
            .def("__init__", make_constructor(&_WrapInit, default_call_policies(), (arg("typeName") = "double")))
            ...
            .def("GetAntiRegressionAuthoringMode", &This::GetAntiRegressionAuthoringMode)
