...
static PyObject* _WrapIter(const TsKnotMap& knotMap) {
    ...
    dict d;
    for (const TsKnot& knot : knotMap) d[knot.GetTime()] = knot;
...
// Iteration is implemented on a copy of the knots.
//
static std::vector<TsTime> _WrapKeys(const TsKnotMap& knotMap) {
    std::vector<TsTime> result;
    result.reserve(knotMap.size());

    for (const TsKnot& knot : knotMap) result.push_back(knot.GetTime());
...
// Iteration is implemented on a copy of the knots.
//
static std::vector<TsKnot> _WrapValues(const TsKnotMap& knotMap) {
    return std::vector<TsKnot>(knotMap.begin(), knotMap.end());
}

static object _WrapFindClosest(const TsKnotMap& knotMap, const TsTime time) {
...
//
static void _WrapSetItem(TsKnotMap& knotMap, const TsTime time, const TsKnot& knot) {
    if (knot.GetTime() != time) {
        TF_CODING_ERROR("Ts.KnotMap.__setitem__: key does not match value.GetTime");
        return;
    }
...
static void _WrapDelItem(TsKnotMap& knotMap, const TsTime time) {
    knotMap.erase(time);
}
...
void wrapKnotMap() {
    ...
    class_<This>("KnotMap", no_init)
            ...
            .def("keys", &_WrapKeys, return_value_policy<TfPySequenceToList>())
            .def("values", &_WrapValues, return_value_policy<TfPySequenceToList>())
