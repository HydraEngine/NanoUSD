...
void TsTest_SampleTimes::AddTimes(const std::vector<double>& times) {
    for (const double time : times) _times.insert(SampleTime(time));
}
...
void TsTest_SampleTimes::AddUniformInterpolationTimes(const int numSamples) {
    ...
    for (int i = 0; i < numSamples - 1; i++) _times.insert(SampleTime(firstTime + i * step));
}
...
void TsTest_SampleTimes::AddExtrapolationTimes(const double extrapolationFactor) {
    ...
    const bool preLoop = (_splineData.GetPreExtrapolation().method == SData::ExtrapLoop);
    const bool postLoop = (_splineData.GetPostExtrapolation().method == SData::ExtrapLoop);
    if (preLoop && postLoop) {
        return;
    }
...
void TsTest_SampleTimes::AddExtrapolatingLoopTimes(const int numIterations, const int numSamplesPerIteration) {
    if (!_haveSplineData) {
    ...
    if (preLoop) {
        ...
        for (int i = -numIterations; i < 0; i++) {
            for (const SampleTime& sample : knotRangeSet) {
                _times.insert(SampleTime(sample.time + knotRange * i, sample.pre));
            }
        }

        // Add a pre-time at each pre-loop join.
        for (int i = -numIterations; i <= 0; i++) {
            _times.insert(SampleTime(firstTime + knotRange * i,
                                     /* pre = */ true));
        }
    }
    ...
    if (postLoop) {
        ...
        for (int i = 0; i <= numIterations; i++) {
            _times.insert(SampleTime(lastTime + knotRange * i,
                                     /* pre = */ true));
        }
    }
}
