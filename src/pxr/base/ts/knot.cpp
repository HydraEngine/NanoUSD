...
TsKnot::TsKnot() : TsKnot(Ts_GetType<double>()) {}

TsKnot::TsKnot(TfType valueType, TsCurveType curveType)
    : _data(Ts_KnotData::Create(valueType)), _proxy(Ts_KnotDataProxy::Create(_data, valueType)) {
    SetCurveType(curveType);
}

TsKnot::TsKnot(Ts_KnotData* const data, const TfType valueType, VtDictionary&& customData)
    : _data(data), _proxy(Ts_KnotDataProxy::Create(_data, valueType)), _customData(std::move(customData)) {}
...
TsKnot::TsKnot(TsKnot&& other)
    : _data(other._data), _proxy(other._proxy.release()), _customData(std::move(other._customData)) {
...
bool TsKnot::operator==(const TsKnot& other) const {
    ...
    return _proxy->IsDataEqualTo(*other._data) && _customData == other._customData;
}
...
bool TsKnot::SetNextInterpolation(const TsInterpMode mode) {
...
bool TsKnot::SetCustomDataByKey(const std::string& keyPath, const VtValue value) {
    _customData.SetValueAtPath(keyPath, value);
    return true;
}

VtValue TsKnot::GetCustomDataByKey(const std::string& keyPath) const {
...
bool TsKnot::_CheckInParamVt(const VtValue value) const {
    if (value.GetType() != GetValueType()) {
        TF_CODING_ERROR("Cannot set '%s' VtValue into knot of type '%s'", value.GetType().GetTypeName().c_str(),
                        GetValueType().GetTypeName().c_str());
        return false;
    }

    bool ok = false;
    TsDispatchToValueTypeTemplate<_ValueChecker>(value.GetType(), value, &ok);
    if (!ok) {
        return false;
    }
...
std::ostream& operator<<(std::ostream& out, const TsKnot& knot) {
    out << "Knot:" << std::endl
        ...
        << "  curve type " << TfEnum::GetName(knot.GetCurveType()).substr(11) << std::endl
        << "  time " << TfStringify(knot.GetTime()) << std::endl
        << "  value " << GET_VT_VALUE(GetValue) << std::endl
        << "  next interp " << TfEnum::GetName(knot.GetNextInterpolation()).substr(8) << std::endl;
    ...
    if (knot.GetCurveType() == TsCurveTypeBezier) {
        out << "  pre-tan width " << TfStringify(knot.GetPreTanWidth()) << std::endl;
    }

    out << "  pre-tan slope " << GET_VT_VALUE(GetPreTanSlope) << std::endl;
    if (knot.GetCurveType() == TsCurveTypeBezier) {
        out << "  post-tan width " << TfStringify(knot.GetPostTanWidth()) << std::endl;
    }

    out << "  post-tan slope " << GET_VT_VALUE(GetPostTanSlope) << std::endl;
