...
static TsRegressionPreventer* _WrapConstructor1(TsSpline* spline, TsTime activeKnotTime, bool limit) {
    return new TsRegressionPreventer(spline, activeKnotTime, limit);
}

static TsRegressionPreventer* _WrapConstructor2(TsSpline* spline,
...
static object _WrapSet(TsRegressionPreventer& self, const TsKnot& proposedActiveKnot) {
...
void wrapRegressionPreventer() {
    ...
    class_<SetResult>("SetResult", no_init)
            ...
            .def("GetDebugDescription", &SetResult::GetDebugDescription, (arg("precision") = 6));

    // ...then the defs, which must occur after the nested type wrappings.
    classObj

            .def("__init__", make_constructor(&_WrapConstructor1, default_call_policies(),
                                              (arg("spline"), arg("activeKnotTime"), arg("limit") = true)))

            .def("__init__", make_constructor(&_WrapConstructor2, default_call_policies(),
                                              (arg("spline"), arg("activeKnotTime"), arg("mode"), arg("limit") = true)))

            .def("Set", &_WrapSet, (arg("proposedActiveKnot")))
