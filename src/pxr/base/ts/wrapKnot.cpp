...
template <typename T>
struct _Initter {
    void operator()(TsKnot* const knot,
                    const object& value,
...
static TsKnot* _WrapInit(const std::string& typeName,
    ...
    TsDispatchToValueTypeTemplate<_Initter>(valueType, knot, value, preValue, preTanSlope, postTanSlope);

    return knot;
}

static std::string _WrapGetValueTypeName(const TsKnot& knot) {
...
#define WRAP_SETTER(field)                                                        \
    +[](TsKnot& knot, const double value) {                                       \
        VtValue vt;                                                               \
        TsDispatchToValueTypeTemplate<_Bundler>(knot.GetValueType(), value, &vt); \
        knot.Set##field(vt);                                                      \
    }
...
void wrapKnot() {
    ...
    class_<This>("Knot", no_init)
            .def("__init__",
                 make_constructor(
                         &_WrapInit, default_call_policies(),
                         (arg("typeName") = "double", arg("curveType") = object(), arg("time") = object(),
                          arg("nextInterp") = object(), arg("value") = object(), arg("preValue") = object(),
                          arg("customData") = object(), arg("preTanWidth") = object(), arg("preTanSlope") = object(),
                          arg("postTanWidth") = object(), arg("postTanSlope") = object())))
