...
public:
    ...
    TS_API
    static void GetBinaryData(const TsSpline& spline,
                              std::vector<uint8_t>* buf,
                              const std::unordered_map<TsTime, VtDictionary>** customDataOut);

    // Read a spline out of binary data.
    TS_API
    static TsSpline CreateSplineFromBinaryData(const std::vector<uint8_t>& buf,
                                               std::unordered_map<TsTime, VtDictionary>&& customData);

private:
    static TsSpline _ParseV1(const std::vector<uint8_t>& buf, std::unordered_map<TsTime, VtDictionary>&& customData);
};
