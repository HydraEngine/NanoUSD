...
struct _Quadratic {
public:
    double Eval(const double t) const { return t * (t * a + b) + c; }
...
public:
    ...
    static _Cubic FromPoints(const double p0, const double p1, const double p2, const double p3) {
        _Cubic result;
    ...
    double Eval(const double t) const { return t * (t * (t * a + b) + c) + d; }
...
static double _RealCubeRoot(const double x) {
    static constexpr double oneThird = 1.0 / 3.0;
    return (x >= 0 ? std::pow(x, oneThird) : -std::pow(-x, oneThird));
}

static double _FilterZeroes(const std::vector<double>& candidates) {
    double result = 0;
    int numFound = 0;
...
//
static double _FindMonotonicZero(const _Quadratic& quad) {
...
//
static double _FindMonotonicZero(const double b, const double c, const double d) {
...
//
static double _FindMonotonicZero(const _Cubic& cubic) {
    ...
    return _FindMonotonicZero(cubic.b / cubic.a, cubic.c / cubic.a, cubic.d / cubic.a);
}

static double _EvalBezier(const Ts_TypedKnotData<double>& beginDataIn,
    ...
    Ts_RegressionPreventerBatchAccess::ProcessSegment(&beginData, &endData, TsAntiRegressionKeepRatio);

    // Find the coefficients for x = f(t).
    // Offset everything by the eval time, so that we can just find a zero.
    const _Cubic timeCubic =
            _Cubic::FromPoints(beginData.time - time, beginData.time + beginData.GetPostTanWidth() - time,
                               endData.time - endData.GetPreTanWidth() - time, endData.time - time);
    ...
    const _Cubic valueCubic =
            _Cubic::FromPoints(beginData.value, beginData.value + beginData.GetPostTanHeight(),
                               endData.GetPreValue() + endData.GetPreTanHeight(), endData.GetPreValue());
...
static double _EvalHermite(const Ts_TypedKnotData<double>& beginData,
...
//
static double _GetSegmentSlope(const Ts_TypedKnotData<double>& beginData, const Ts_TypedKnotData<double>& endData) {
    return (endData.GetPreValue() - beginData.value) / (endData.time - beginData.time);
}

// Find the slope in an extrapolation region.
//
static std::optional<double> _GetExtrapolationSlope(const TsExtrapolation& extrap,
...
//
static double _ExtrapolateLinear(const Ts_TypedKnotData<double>& knotData,
                                 const double slope,
                                 const TsTime time,
                                 const Ts_EvalLocation location) {
    if (location == Ts_EvalPre) {
...
public:
    ...
    _LoopResolver(const Ts_SplineData* data, TsTime time, Ts_EvalAspect aspect, Ts_EvalLocation location);

    // Output accessors.
    TsTime GetEvalTime() const { return _evalTime; }
    Ts_EvalLocation GetEvalLocation() const { return _location; }
    bool IsBetweenLastProtoAndEnd() const { return _betweenLastProtoAndEnd; }
    double GetValueOffset() const { return _valueOffset; }
    bool GetNegate() const { return _negate; }

    // Knot copiers for special cases.
    void ReplaceBoundaryKnots(Ts_TypedKnotData<double>* prevData, Ts_TypedKnotData<double>* nextData) const;
    void ReplacePreExtrapKnots(Ts_TypedKnotData<double>* nextData, Ts_TypedKnotData<double>* nextData2) const;
    void ReplacePostExtrapKnots(Ts_TypedKnotData<double>* prevData, Ts_TypedKnotData<double>* prevData2) const;

private:
    void _ResolveInner();
    void _ResolveExtrap();
    void _DoExtrap(const TsExtrapolation& extrapolation, TsTime offset, bool isPre);
    void _ComputeExtrapValueOffset();
    Ts_TypedKnotData<double> _CopyProtoKnotData(size_t index, int shiftIters) const;
...
_LoopResolver::_LoopResolver(const Ts_SplineData* const data,
                             const TsTime timeIn,
                             const Ts_EvalAspect aspect,
                             const Ts_EvalLocation location)
    : _data(data), _aspect(aspect), _evalTime(timeIn), _location(location) {
    ...
    const bool haveMultipleKnots = (_haveInnerLoops || _data->times.size() > 1);

    // Are any extrapolating loops enabled?
    _havePreExtrapLoops = haveMultipleKnots && _data->preExtrapolation.IsLooping();
    _havePostExtrapLoops = haveMultipleKnots && _data->postExtrapolation.IsLooping();
    ...
    TF_DEBUG_MSG(TS_DEBUG_LOOPS,
                 ...
                 "  firstTimeLooped: %d\n"
                 "  lastTimeLooped: %d\n",
                 _evalTime, _haveInnerLoops, _havePreExtrapLoops, _havePostExtrapLoops, _firstTimeLooped,
                 _lastTimeLooped);
...
void _LoopResolver::_ResolveInner() {
    TF_DEBUG_MSG(TS_DEBUG_LOOPS,
                 ...
                 "  numPostLoops: %d\n"
                 "  valueOffset: %g\n",
                 _firstInnerProtoIndex, _data->loopParams.protoStart, _data->loopParams.protoEnd,
                 _data->loopParams.numPreLoops, _data->loopParams.numPostLoops, _data->loopParams.valueOffset);
    ...
    if (loopedInterval.Contains(_evalTime) && !protoInterval.Contains(_evalTime)) {
    ...
    if (protoInterval.Contains(_evalTime)) {
        ...
        const auto lbIt = std::lower_bound(firstProtoIt, times.end(), lp.protoEnd);
    ...
    // looping.
        ...
        if (_firstTimeLooped) {
            ...
            _extrapKnot1 = _CopyProtoKnotData(_firstInnerProtoIndex, -lp.numPreLoops);

            if (_data->times.size() > _firstInnerProtoIndex + 1 &&
                protoInterval.Contains(_data->times[_firstInnerProtoIndex + 1])) {
                // Second knot is a copy of the second prototype knot.
                _extrapKnot2 = _CopyProtoKnotData(_firstInnerProtoIndex + 1, -lp.numPreLoops);
            } else {
                // There are no knots after the first prototype knot, so the
                // second is another copy of the first.
                _extrapKnot2 = _CopyProtoKnotData(_firstInnerProtoIndex, -lp.numPreLoops + 1);
            }
        }
    }
    ...
    // looping.
        ...
        if (_lastTimeLooped) {
            ...
            _extrapKnot1 = _CopyProtoKnotData(_firstInnerProtoIndex, lp.numPostLoops + 1);

            // Find last authored prototype knot, which may also be the first.
            // See comments in Case 1 above.
            const auto lastProtoIt = std::lower_bound(firstProtoIt, times.end(), lp.protoEnd) - 1;
            const size_t lastProtoIndex = lastProtoIt - _data->times.begin();

            // Second-to-last knot is a copy of the last prototype knot.
            _extrapKnot2 = _CopyProtoKnotData(lastProtoIndex, lp.numPostLoops);
        }
    }
    ...
    // region.
        ...
        const auto lbIt = std::lower_bound(times.begin(), firstProtoIt, loopedInterval.GetMin());
    ...
    // region.
        ...
        const auto ubIt = std::upper_bound(firstProtoIt + 1, times.end(), loopedInterval.GetMax());
    ...
    TF_DEBUG_MSG(TS_DEBUG_LOOPS,
                 ...
                 _evalTime, _valueOffset, _betweenLastProtoAndEnd, _betweenPreUnloopedAndLooped,
...
void _LoopResolver::_ResolveExtrap() {
    ...
    TF_DEBUG_MSG(TS_DEBUG_LOOPS,
                 ...
                 "  extrapValueOffset: %g\n"
                 "  negate: %d\n",
                 _evalTime, _valueOffset, _doPreExtrap, _doPostExtrap, _extrapValueOffset, _negate);
}
...
//
void _LoopResolver::_DoExtrap(const TsExtrapolation& extrapolation, const TsTime offset, const bool isPre) {
    ...
    const bool shortOffset = boundary && ((isPre && _location != Ts_EvalPre) || (!isPre && _location == Ts_EvalPre));
    ...
    if (_data->preExtrapolation.mode == TsExtrapLoopRepeat && _aspect != Ts_EvalDerivative) {
    ...
    // in time.
    else if (_data->preExtrapolation.mode == TsExtrapLoopOscillate && iterHop % 2 != 0) {
...
void _LoopResolver::_ComputeExtrapValueOffset() {
    ...
    if (!_firstTimeLooped) {
        ...
        firstValue = _data->GetKnotDataAsDouble(_firstInnerProtoIndex).GetPreValue() - lp.numPreLoops * lp.valueOffset;
    }
    ...
    if (!_lastTimeLooped) {
        ...
        lastValue = _data->GetKnotDataAsDouble(_firstInnerProtoIndex).value + (lp.numPostLoops + 1) * lp.valueOffset;
    }
...
//
void _LoopResolver::ReplaceBoundaryKnots(Ts_TypedKnotData<double>* prevData, Ts_TypedKnotData<double>* nextData) const {
    const TsLoopParams& lp = _data->loopParams;
    ...
    if (_betweenLastProtoAndEnd) {
        *nextData = _CopyProtoKnotData(_firstInnerProtoIndex, 1);
    }
    ...
    // looping region, and use that as nextData.
        ...
        *nextData = _CopyProtoKnotData(_firstInnerProtoIndex, -lp.numPreLoops);
    }
    ...
    // looping region, and use that as prevData.
        ...
        *prevData = _CopyProtoKnotData(_firstInnerProtoIndex, lp.numPostLoops + 1);
    }
}

void _LoopResolver::ReplacePreExtrapKnots(Ts_TypedKnotData<double>* const nextData,
                                          Ts_TypedKnotData<double>* const nextData2) const {
    if (!_firstTimeLooped) return;

    *nextData = _extrapKnot1;
    *nextData2 = _extrapKnot2;
}

void _LoopResolver::ReplacePostExtrapKnots(Ts_TypedKnotData<double>* const prevData,
                                           Ts_TypedKnotData<double>* const prevData2) const {
    if (!_lastTimeLooped) return;

    *prevData = _extrapKnot1;
    *prevData2 = _extrapKnot2;
}

Ts_TypedKnotData<double> _LoopResolver::_CopyProtoKnotData(const size_t index, const int shiftIters) const {
    const TsLoopParams& lp = _data->loopParams;
...
//
static std::optional<double> _Interpolate(const Ts_TypedKnotData<double>& beginData,
                                          const Ts_TypedKnotData<double>& endData,
                                          const TsTime time,
                                          const Ts_EvalAspect aspect) {
...
static std::optional<double> _EvalMain(const Ts_SplineData* const data,
    ...
    const bool afterEnd = (loopRes.IsBetweenLastProtoAndEnd() ? false : prevIt == times.end() - 1);
    ...
    if (atKnot) {
        // Handle values.
        if (aspect == Ts_EvalValue || aspect == Ts_EvalHeldValue) {
            // Pre-value after held segment = previous knot value.
            if (location == Ts_EvalPre && !atFirst && prevData.nextInterp == TsInterpHeld) {
                return prevData.value;
            }

            // Not a special case.  Return what's stored in the knot.
            return (location == Ts_EvalPre ? knotData.GetPreValue() : knotData.value);
        }
        ...
        // Handle derivatives.
            ...
            if (location == Ts_EvalPre) {
                ...
                if (atFirst) {
                    return _GetExtrapolationSlope(data->preExtrapolation, haveMultipleKnots, knotData, nextData,
                ...
                if (atLast) {
                    return _GetExtrapolationSlope(data->postExtrapolation, haveMultipleKnots, knotData, prevData,
    ...
    if (beforeStart) {
        ...
        const std::optional<double> slope = _GetExtrapolationSlope(data->preExtrapolation, haveMultipleKnots, nextData,
                                                                   nextData2, data->curveType, Ts_EvalPre);
    ...
    if (afterEnd) {
        ...
        // Find the extrapolation slope.
        const std::optional<double> slope = _GetExtrapolationSlope(data->postExtrapolation, haveMultipleKnots, prevData,
                                                                   prevData2, data->curveType, Ts_EvalPost);
...
std::optional<double> Ts_Eval(const Ts_SplineData* const data,
                              const TsTime timeIn,
                              const Ts_EvalAspect aspect,
                              const Ts_EvalLocation location) {
    ...
    return (*result + loopRes.GetValueOffset()) * (loopRes.GetNegate() ? -1 : 1);
}
