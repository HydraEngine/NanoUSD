...
static TsExtrapolation _MakeExtrap(const SData::Extrapolation extrapIn) {
...
TsSpline TsTest_TsEvaluator::SplineDataToSpline(const SData& data) const {
    const SData::Features features = data.GetRequiredFeatures();
    if ((features & SData::FeatureHermiteSegments) || (features & SData::FeatureAutoTangents)) {
    ...
    for (const SData::Knot& dataKnot : dataKnots) {
        ...
        switch (dataKnot.nextSegInterpMethod) {
            ...
            case SData::InterpCurve:
                ...
                break;
            default:
                TF_CODING_ERROR("Unexpected interpolation method");
        }
...
static SData::Extrapolation _MakeExtrap(const TsExtrapolation extrapIn) {
...
TsTest_SplineData TsTest_TsEvaluator::SplineToSplineData(const TsSpline& splineIn) const {
