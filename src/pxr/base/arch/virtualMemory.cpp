...
bool ArchSetMemoryProtection(void const* start, size_t numBytes, ArchMemoryProtection protection) {
    ...
    SIZE_T len = numBytes + (reinterpret_cast<char const*>(start) - reinterpret_cast<char const*>(pageStart));
...
void* ArchReserveVirtualMemory(size_t numBytes) {
    void* addr = mmap(NULL, numBytes, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (!addr || addr == MAP_FAILED) return nullptr;
    return addr;
}

bool ArchCommitVirtualMemoryRange(void* start, size_t numBytes) {
    void* pageStart = RoundToPageAddr(start);
    size_t len = numBytes + (reinterpret_cast<char*>(start) - reinterpret_cast<char*>(pageStart));
