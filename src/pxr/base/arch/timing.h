...
struct ArchIntervalTimer {
    ...
    uint64_t GetStartTicks() const { return (uint64_t(_startHigh) << 32) + _startLow; }
...
uint64_t ArchMeasureExecutionTime(Fn const& fn, uint64_t maxTicks = 1e7, bool* reachedConsensus = nullptr) {
    ...
    return Arch_MeasureExecutionTime(maxTicks, reachedConsensus, static_cast<void const*>(&measureN),
