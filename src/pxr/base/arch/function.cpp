...
static string _GetFunctionName(const string& function, string prettyFunction) {
    ...
    if (functionStart == string::npos || functionStart == 0) return function;
...
static std::map<string, string> _GetTemplateList(const std::string& templates) {
    ...
    while (i != string::npos) {
        ...
        result[templates.substr(nameStart, nameEnd + 1 - nameStart)] = templates.substr(typeStart, typeEnd - typeStart);
...
static string _GetNextIdentifier(const string& prettyFunction, string::size_type& pos) {
    ...
    if (last == string::npos) {
        ...
        if (last == string::npos) last = prettyFunction.size();
