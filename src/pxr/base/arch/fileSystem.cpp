...
FILE* ArchOpenFile(char const* fileName, char const* mode) {
    ...
    // Call CreateFileW.
    HANDLE hfile = CreateFileW(ArchWindowsUtf8ToUtf16(fileName).c_str(), desiredAccess, shareMode,
...
inline TokenType _GetTokenType(pair<Iter, Iter> t) {
    ...
    if (len == 1 && t.first[0] == '.') return Dot;
    if (len == 2 && t.first[0] == '.' && t.first[1] == '.') return DotDot;
    return Elem;
}
...
string _NormPath(string const& inPath) {
        ...
        switch (_GetTokenType(t)) {
            case Elem:
                ...
                if (inPath.begin() + writeIdx == t.first) {
                    ...
                    t.first = t.second;
                    if (writeIdx != path.size()) ++writeIdx;
                } else {
                    while (t.first != t.second) path[writeIdx++] = *t.first++;
                    if (writeIdx != path.size()) path[writeIdx++] = '/';
                }
                break;
            case Dot:
            ...
            case DotDot: {
                ...
                string::reverse_iterator rstart(path.begin() + firstWriteIdx), rwrite(path.begin() + writeIdx);
    ...
    if (writeIdx > firstWriteIdx && path.cbegin()[writeIdx - 1] == '/') --writeIdx;

    // Trim the string to length if necessary.
    if (writeIdx != path.size()) path.erase(writeIdx);
...
int64_t ArchGetFileLength(const char* fileName) {
    ...
    HANDLE handle = CreateFileW(ArchWindowsUtf8ToUtf16(fileName).c_str(), 0,
...
string ArchGetFileName(FILE* file) {
    ...
    if (GetFileInformationByHandleEx(hfile, FileNameInfo, static_cast<void*>(fileNameInfo), bufSize)) {
        size_t outSize = WideCharToMultiByte(CP_UTF8, 0, fileNameInfo->FileName,
                                             fileNameInfo->FileNameLength / sizeof(WCHAR), NULL, 0, NULL, NULL);
        result.resize(outSize);
        WideCharToMultiByte(CP_UTF8, 0, fileNameInfo->FileName, fileNameInfo->FileNameLength / sizeof(WCHAR),
...
namespace {
std::string MakeUnique(const std::string& sTemplate, std::function<bool(const char* name)> func, int maxRetry = 1000) {
...
int ArchMakeTmpFile(const std::string& tmpdir, const std::string& prefix, std::string* pathname) {
    ...
    auto cTemplate = MakeUnique(sTemplate, [&fd](const char* name) {
        _wsopen_s(&fd, ArchWindowsUtf8ToUtf16(name).c_str(), _O_CREAT | _O_EXCL | _O_RDWR | _O_BINARY, _SH_DENYNO,
                  _S_IREAD | _S_IWRITE);
        return fd != -1;
    });
#else
...
static inline Mapping Arch_MapFileImpl(FILE* file, std::string* errMsg) {
    ...
    HANDLE hFileMap =
            CreateFileMapping(_FileToWinHANDLE(file), NULL, PAGE_READONLY /* allow read-only or copy-on-write */,
                              maxSizeHigh, maxSizeLow, NULL);
    if (hFileMap == NULL) return Mapping();
    auto ptr = static_cast<PtrType>(MapViewOfFile(hFileMap, isConst ? FILE_MAP_READ : FILE_MAP_COPY,
...
void ArchMemAdvise(void const* addr, size_t len, ArchMemAdvice adv) {
    ...
    int adviceMap[] = {/* ArchMemAdviceNormal       = */ POSIX_MADV_NORMAL,
    ...
    int rval = posix_madvise(reinterpret_cast<void*>(alignedAddrInt), len + (addrInt - alignedAddrInt), adviceMap[adv]);
    if (rval != 0) {
        fprintf(stderr,
                "failed call to posix_madvise(%zd, %zd)"
                "ret=%d, errno=%d '%s'\n",
                alignedAddrInt, len + (addrInt - alignedAddrInt), rval, errno, ArchStrerror().c_str());
    }
#endif
}
...
bool ArchQueryMappedMemoryResidency(void const* addr, size_t len, unsigned char* pageMap) {
    ...
    int ret = mincore(reinterpret_cast<caddr_t>(const_cast<void*>(addr)), len, reinterpret_cast<char*>(pageMap));
    return ret == 0;
#endif
...
int64_t ArchPRead(FILE* file, void* buffer, size_t count, int64_t offset) {
    ...
    if (ARCH_LIKELY(nread == signedCount || nread == 0)) return nread;
    ...
    while (nread != -1 || (nread == -1 && errno == EINTR)) {
        ...
        if (ARCH_LIKELY(nread == signedCount || nread == 0)) return total + nread;
    }
...
int64_t ArchPWrite(FILE* file, void const* bytes, size_t count, int64_t offset) {
    ...
    if (ARCH_LIKELY(nwritten == signedCount)) return nwritten;
    ...
    while (nwritten != -1) {
        ...
        if (ARCH_LIKELY(nwritten == signedCount)) return total + nwritten;
    }
...
int ArchFileAccess(const char* path, int mode) {
    ...
    if (mode == F_OK) {
        return (GetFileAttributesW(wpath.c_str()) != INVALID_FILE_ATTRIBUTES) ? 0 : Arch_FileAccessError();
    }
    ...
    if (DuplicateToken(token, SecurityImpersonation, &duplicateToken)) {
        ...
        if (AccessCheck(security, duplicateToken, accessMask, &mapping, &privileges, &privilegesLength, &grantedAccess,
                        &accessStatus)) {
            if (accessStatus) {
                result = true;
            } else {
...
std::string ArchReadLink(const char* path) {
    HANDLE handle = ::CreateFileW(ArchWindowsUtf8ToUtf16(path).c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
    ...
    if (IsReparseTagMicrosoft(reparse->ReparseTag)) {
        if (reparse->ReparseTag == IO_REPARSE_TAG_SYMLINK) {
            ...
            if (str.length() >= 3 && str.substr(0, 3) == "\\??") return path;
...
void ArchFileAdvise(FILE* file, int64_t offset, size_t count, ArchFileAdvice adv) {
    ...
    if (rval != 0) {
        fprintf(stderr,
                ...
                "ret=%d, errno=%d '%s'\n",
                fileno(file), offset, static_cast<off_t>(count), rval, errno, ArchStrerror().c_str());
    }
#endif
}
