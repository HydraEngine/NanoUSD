...
#define ARCH_PRINTF_FUNCTION(_fmt, _firstArg) __attribute__((format(printf, _fmt, _firstArg)))
#define ARCH_SCANF_FUNCTION(_fmt, _firstArg) __attribute__((format(scanf, _fmt, _firstArg)))
...
struct ARCH_HIDDEN Arch_PerLibInit {
    Arch_PerLibInit() { /* "use" of init here forces instantiation */ (void)init; }
...
#define ARCH_CONSTRUCTOR(_name, _priority, ...)                                                                        \
    ...
    static const Arch_ConstructorEntry _ARCH_CAT_NOEXPAND(arch_ctor_, _name)                                           \
            __attribute__((used, section("__DATA,pxrctor"))) = {reinterpret_cast<Arch_ConstructorEntry::Type>(&_name), \
                                                                static_cast<unsigned>(PXR_VERSION), _priority};        \
    static void _name(__VA_ARGS__)
...
#define ARCH_DESTRUCTOR(_name, _priority, ...)                                                                         \
    ...
    static const Arch_ConstructorEntry _ARCH_CAT_NOEXPAND(arch_dtor_, _name)                                           \
            __attribute__((used, section("__DATA,pxrdtor"))) = {reinterpret_cast<Arch_ConstructorEntry::Type>(&_name), \
                                                                static_cast<unsigned>(PXR_VERSION), _priority};        \
    static void _name(__VA_ARGS__)
...
#define ARCH_CONSTRUCTOR(_name, _priority, ...) \
    __attribute__((used, section(".pxrctor"), constructor((_priority) + 100))) static void _name(__VA_ARGS__)
#define ARCH_DESTRUCTOR(_name, _priority, ...) \
    __attribute__((used, section(".pxrdtor"), destructor((_priority) + 100))) static void _name(__VA_ARGS__)
...
#define ARCH_CONSTRUCTOR(_name, _priority, ...)                                                                    \
    static void _name(__VA_ARGS__);                                                                                \
    namespace {                                                                                                    \
    __declspec(allocate(".pxrctor")) extern const Arch_ConstructorEntry _ARCH_CAT_NOEXPAND(arch_ctor_, _name) = {  \
            reinterpret_cast<Arch_ConstructorEntry::Type>(&_name), static_cast<unsigned>(PXR_VERSION), _priority}; \
    }                                                                                                              \
...
#define ARCH_DESTRUCTOR(_name, _priority, ...)                                                                     \
    static void _name(__VA_ARGS__);                                                                                \
    namespace {                                                                                                    \
    __declspec(allocate(".pxrdtor")) extern const Arch_ConstructorEntry _ARCH_CAT_NOEXPAND(arch_dtor_, _name) = {  \
            reinterpret_cast<Arch_ConstructorEntry::Type>(&_name), static_cast<unsigned>(PXR_VERSION), _priority}; \
    }                                                                                                              \
    _ARCH_ENSURE_PER_LIB_INIT(Arch_ConstructorInit, _archCtorInit);                                                \
    static void _name(__VA_ARGS__)
