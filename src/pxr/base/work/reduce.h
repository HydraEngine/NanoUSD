...
V WorkParallelReduceN(const V& identity, size_t n, Fn&& loopCallback, Rn&& reductionCallback, size_t grainSize) {
    ...
    if (WorkHasConcurrency()) {
        ...
        return tbb::parallel_reduce(tbb::blocked_range<size_t>(0, n, grainSize), identity, Work_Body_TBB(loopCallback),
                                    std::forward<Rn>(reductionCallback), tbb::auto_partitioner(), ctx);
    }
