...
GfRotation& GfRotation::SetRotateInto(const GfVec3d& rotateFrom, const GfVec3d& rotateTo) {
    ...
    // rotation
    if (cos > 0.9999999) return SetIdentity();
    ...
    if (cos < -0.9999999) {
        ...
        if (tmp.GetLength() < 0.00001) tmp = GfCross(from, GfVec3d(0.0, 1.0, 0.0));
...
GfVec3d GfRotation::Decompose(const GfVec3d& axis0, const GfVec3d& axis1, const GfVec3d& axis2) const {
    ...
    GfMatrix4d axes(nAxis0[0], nAxis1[0], nAxis2[0], 0, nAxis0[1], nAxis1[1], nAxis2[1], 0, nAxis0[2], nAxis1[2],
                    nAxis2[2], 0, 0, 0, 0, 1);
...
GfRotation GfRotation::RotateOntoProjected(const GfVec3d& v1, const GfVec3d& v2, const GfVec3d& axisParam) {
    ...
    double theta = 0;
    if (!(fabs(sinTheta) < _GetEpsilon() && fabs(cosTheta) < _GetEpsilon())) theta = atan2(sinTheta, cosTheta);
...
void GfRotation::MatchClosestEulerRotation(double targetTw,
    ...
    for (unsigned int i = 0; i < numVals; i++) {
        ...
        for (unsigned int j = 0; j < 4; j++) sum += fabs(targetDiff[j]);
        if ((i == 0) || (sum < min)) {
            min = sum;
            mini = i;
        }
...
void GfRotation::DecomposeRotation(const GfMatrix4d& rot,
    ...
    MatchClosestEulerRotation(hintTw, hintFB, hintLR, hintSw, zeroAngle == ZERO_TW ? nullptr : thetaTw,
                              zeroAngle == ZERO_FB ? nullptr : thetaFB, zeroAngle == ZERO_LR ? nullptr : thetaLR,
