...
public:
    ...
    float GetMidpoint() const { return static_cast<ScalarType>(0.5) * _min + static_cast<ScalarType>(0.5) * _max; }
    ...
    /// their extrema.
    bool Contains(const GfRange1f& range) const { return Contains(range._min) && Contains(range._max); }
    ...
    /// with all operations of this type, the ranges are assumed to include
    /// their extrema.
    bool IsOutside(const GfRange1f& range) const { return (range._max < _min || range._min > _max); }
    ...
    static GfRange1f Union(const GfRange1f& a, const GfRange1f& b) { return GetUnion(a, b); }
    ...
    static GfRange1f Intersection(const GfRange1f& a, const GfRange1f& b) { return GetIntersection(a, b); }
    ...
    /// binary sum.
    GfRange1f operator+(const GfRange1f& b) const { return GfRange1f(_min + b._min, _max + b._max); }

    /// binary difference.
    GfRange1f operator-(const GfRange1f& b) const { return GfRange1f(_min - b._max, _max - b._min); }
    ...
    /// scalar divide.
    friend GfRange1f operator/(const GfRange1f& r, double m) { return r * (1.0 / m); }

    /// hash.
    friend inline size_t hash_value(const GfRange1f& r) { return TfHash::Combine(r._min, r._max); }

    /// The min and max points must match exactly for equality.
    bool operator==(const GfRange1f& b) const { return (_min == b._min && _max == b._max); }
