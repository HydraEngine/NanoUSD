...
public:
    ...
    GfVec3f GetMidpoint() const { return static_cast<ScalarType>(0.5) * _min + static_cast<ScalarType>(0.5) * _max; }
    ...
    /// their extrema.
    bool Contains(const GfRange3f& range) const { return Contains(range._min) && Contains(range._max); }

    /// Returns true if the \p point is located inside the range. As with all
    /// operations of this type, the range is assumed to include its extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(const GfVec3f& point) const { return Contains(point); }

    /// Returns true if the \p range is located entirely inside the range. As
    /// with all operations of this type, the ranges are assumed to include
    /// their extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(const GfRange3f& range) const { return Contains(range); }
    ...
    static GfRange3f Union(const GfRange3f& a, const GfRange3f& b) { return GetUnion(a, b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange3f& Union(const GfRange3f& b) { return UnionWith(b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange3f& Union(const GfVec3f& b) { return UnionWith(b); }
    ...
    static GfRange3f Intersection(const GfRange3f& a, const GfRange3f& b) { return GetIntersection(a, b); }
    ...
    const GfRange3f& Intersection(const GfRange3f& b) { return IntersectWith(b); }
    ...
    /// unary division.
    GfRange3f& operator/=(double m) { return *this *= (1.0 / m); }

    /// binary sum.
    GfRange3f operator+(const GfRange3f& b) const { return GfRange3f(_min + b._min, _max + b._max); }

    /// binary difference.
    GfRange3f operator-(const GfRange3f& b) const { return GfRange3f(_min - b._max, _max - b._min); }

    /// scalar multiply.
    friend GfRange3f operator*(double m, const GfRange3f& r) {
        return (m > 0 ? GfRange3f(r._min * m, r._max * m) : GfRange3f(r._max * m, r._min * m));
    }
    ...
    /// scalar divide.
    friend GfRange3f operator/(const GfRange3f& r, double m) { return r * (1.0 / m); }

    /// hash.
    friend inline size_t hash_value(const GfRange3f& r) { return TfHash::Combine(r._min, r._max); }

    /// The min and max points must match exactly for equality.
    bool operator==(const GfRange3f& b) const { return (_min == b._min && _max == b._max); }

    bool operator!=(const GfRange3f& b) const { return !(*this == b); }
...
inline bool GfRange3f::operator==(const GfRange3d& other) const {
    return _min == GfVec3f(other.GetMin()) && _max == GfVec3f(other.GetMax());
}
