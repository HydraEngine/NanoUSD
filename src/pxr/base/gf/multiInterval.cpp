...
bool GfMultiInterval::Contains(double d) const {
    ...
    if (i != end() && i->Contains(d)) return true;

    // Case 2: the previous interval, (i-1), contains d.
    if (i != begin() && (--i)->Contains(d)) return true;
...
bool GfMultiInterval::Contains(const GfInterval& a) const {
    ...
    if (i != end() && i->Contains(a)) return true;

    // Case 2: the previous interval, (i-1), contains a.
    if (i != begin() && (--i)->Contains(a)) return true;
...
void GfMultiInterval::Add(const GfInterval& interval) {
    ...
    if (i != _set.begin()) {
        ...
        if (merged.Intersects(*i) || (merged.GetMin() == (*i).GetMax() && !(merged.IsMinOpen() && (*i).IsMaxOpen()))) {
            merged |= *i;
            _set.erase(i);
        }
    }
    ...
    if (TF_DEV_BUILD) _AssertInvariants();
}
...
// as necessary.
static void _RemoveInterval(const GfMultiInterval::Set::iterator i, const GfInterval& j, GfMultiInterval::Set* set) {
    if (!i->Intersects(j)) return;

    GfInterval lo(i->GetMin(), j.GetMin(), i->IsMinClosed(), !j.IsMinClosed());
    GfInterval hi(j.GetMax(), i->GetMax(), !j.IsMaxClosed(), i->IsMaxClosed());

    if (!lo.IsEmpty()) set->insert(/* hint */ i, lo);
    if (!hi.IsEmpty()) set->insert(/* hint */ i, hi);

    set->erase(i);
}

void GfMultiInterval::Remove(const GfInterval& intervalToRemove) {
    if (intervalToRemove.IsEmpty()) return;

    Set::iterator i;

    // Remove from subsequent intersecting intervals.
    i = _set.lower_bound(intervalToRemove);
    while (i != _set.end() && intervalToRemove.Intersects(*i)) _RemoveInterval(i++, intervalToRemove, &_set);

    // Remove from prior interval.  Need to handle at most one.
    i = _set.upper_bound(intervalToRemove);
    if (i != _set.begin()) _RemoveInterval(--i, intervalToRemove, &_set);
