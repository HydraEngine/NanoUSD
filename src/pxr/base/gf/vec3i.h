...
public:
    ...
    template <class Scl>
    constexpr explicit GfVec3i(Scl const* p) : _data{p[0], p[1], p[2]} {}
    ...
    GfVec3i& Set(int const* a) { return Set(a[0], a[1], a[2]); }
    ...
    /// Hash.
    friend inline size_t hash_value(GfVec3i const& vec) { return TfHash::Combine(vec[0], vec[1], vec[2]); }

    /// Equality comparison.
    bool operator==(GfVec3i const& other) const {
        return _data[0] == other[0] && _data[1] == other[1] && _data[2] == other[2];
    }
    bool operator!=(GfVec3i const& other) const { return !(*this == other); }
    ...
    GfVec3i operator-() const { return GfVec3i(-_data[0], -_data[1], -_data[2]); }
    ...
    friend GfVec3i operator+(GfVec3i const& l, GfVec3i const& r) { return GfVec3i(l) += r; }
    ...
    GfVec3i operator*(double s) const { return GfVec3i(*this) *= s; }
    friend GfVec3i operator*(double s, GfVec3i const& v) { return v * s; }
    ...
    GfVec3i operator/(int s) const { return GfVec3i(*this) /= s; }

    /// See GfDot().
    int operator*(GfVec3i const& v) const { return _data[0] * v[0] + _data[1] * v[1] + _data[2] * v[2]; }

    /// Returns the projection of \p this onto \p v. That is:
    /// \code
    /// v * (*this * v)
    /// \endcode
    GfVec3i GetProjection(GfVec3i const& v) const { return v * (*this * v); }
    ...
    /// \endcode
    GfVec3i GetComplement(GfVec3i const& b) const { return *this - this->GetProjection(b); }
