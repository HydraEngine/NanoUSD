...
public:
    ...
    friend inline size_t hash_value(const GfFrustum& f) {
        return TfHash::Combine(f._position, f._rotation, f._window, f._nearFar, f._viewDistance, f._projectionType);
    }
    ...
    GfFrustum::ProjectionType GetProjectionType() const { return _projectionType; }
...
private:
    ...
    GF_API GfRay _ComputePickRayOffsetToNearPlane(const GfVec3d& camSpaceFrom, const GfVec3d& camSpaceDir) const;
