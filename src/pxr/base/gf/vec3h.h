...
public:
    ...
    template <class Scl>
    constexpr explicit GfVec3h(Scl const* p) : _data{p[0], p[1], p[2]} {}
    ...
    GfVec3h& Set(GfHalf const* a) { return Set(a[0], a[1], a[2]); }
    ...
    /// Hash.
    friend inline size_t hash_value(GfVec3h const& vec) { return TfHash::Combine(vec[0], vec[1], vec[2]); }

    /// Equality comparison.
    bool operator==(GfVec3h const& other) const {
        return _data[0] == other[0] && _data[1] == other[1] && _data[2] == other[2];
    }
    bool operator!=(GfVec3h const& other) const { return !(*this == other); }
    ...
    GfVec3h operator-() const { return GfVec3h(-_data[0], -_data[1], -_data[2]); }
    ...
    friend GfVec3h operator+(GfVec3h const& l, GfVec3h const& r) { return GfVec3h(l) += r; }
    ...
    GfVec3h operator*(double s) const { return GfVec3h(*this) *= s; }
    friend GfVec3h operator*(double s, GfVec3h const& v) { return v * s; }
    ...
    GfVec3h operator/(double s) const { return *this * (1.0 / s); }

    /// See GfDot().
    GfHalf operator*(GfVec3h const& v) const { return _data[0] * v[0] + _data[1] * v[1] + _data[2] * v[2]; }

    /// Returns the projection of \p this onto \p v. That is:
    /// \code
    /// v * (*this * v)
    /// \endcode
    GfVec3h GetProjection(GfVec3h const& v) const { return v * (*this * v); }
    ...
    /// \endcode
    GfVec3h GetComplement(GfVec3h const& b) const { return *this - this->GetProjection(b); }
    ...
    static bool OrthogonalizeBasis(
            GfVec3h* tx, GfVec3h* ty, GfVec3h* tz, const bool normalize, double eps = GF_MIN_ORTHO_TOLERANCE);
...
inline GfVec3h::GfVec3h(class GfVec3d const& other) {
    _data[0] = other[0];
    _data[1] = other[1];
    _data[2] = other[2];
}
inline GfVec3h::GfVec3h(class GfVec3f const& other) {
    _data[0] = other[0];
    _data[1] = other[1];
    _data[2] = other[2];
}
inline GfVec3h::GfVec3h(class GfVec3i const& other) {
    _data[0] = other[0];
    _data[1] = other[1];
    _data[2] = other[2];
}
...
inline GfVec3h GfCompMult(GfVec3h const& v1, GfVec3h const& v2) {
    return GfVec3h(v1[0] * v2[0], v1[1] * v2[1], v1[2] * v2[2]);
}

/// Returns component-wise quotient of vectors \p v1 and \p v2.
inline GfVec3h GfCompDiv(GfVec3h const& v1, GfVec3h const& v2) {
    return GfVec3h(v1[0] / v2[0], v1[1] / v2[1], v1[2] / v2[2]);
}

/// Returns the dot (inner) product of two vectors.
inline GfHalf GfDot(GfVec3h const& v1, GfVec3h const& v2) {
    return v1 * v2;
}

/// Returns the geometric length of \c v.
inline GfHalf GfGetLength(GfVec3h const& v) {
    return v.GetLength();
}
...
inline GfHalf GfNormalize(GfVec3h* v, GfHalf eps = 0.001) {
...
/// \p eps is returned.
inline GfVec3h GfGetNormalized(GfVec3h const& v, GfHalf eps = 0.001) {
...
/// b * (a * b)
/// \endcode
inline GfVec3h GfGetProjection(GfVec3h const& a, GfVec3h const& b) {
...
///  a - a.GetProjection(b)
/// \endcode
inline GfVec3h GfGetComplement(GfVec3h const& a, GfVec3h const& b) {
...
inline bool GfIsClose(GfVec3h const& v1, GfVec3h const& v2, double tolerance) {
    GfVec3h delta = v1 - v2;
    return delta.GetLengthSq() <= tolerance * tolerance;
}

GF_API bool GfOrthogonalizeBasis(
        GfVec3h* tx, GfVec3h* ty, GfVec3h* tz, bool normalize, double eps = GF_MIN_ORTHO_TOLERANCE);
...
inline GfVec3h GfCross(GfVec3h const& v1, GfVec3h const& v2) {
    return GfVec3h(v1[1] * v2[2] - v1[2] * v2[1], v1[2] * v2[0] - v1[0] * v2[2], v1[0] * v2[1] - v1[1] * v2[0]);
}

/// Returns the cross product of \p v1 and \p v2.
/// \see GfCross()
inline GfVec3h operator^(GfVec3h const& v1, GfVec3h const& v2) {
