...
template <>
struct GfIsGfVec<class GfVec3d> {
...
public:
    ...
    template <class Scl>
    constexpr explicit GfVec3d(Scl const* p) : _data{p[0], p[1], p[2]} {}
    ...
    GfVec3d& Set(double const* a) { return Set(a[0], a[1], a[2]); }
    ...
    /// Hash.
    friend inline size_t hash_value(GfVec3d const& vec) { return TfHash::Combine(vec[0], vec[1], vec[2]); }

    /// Equality comparison.
    bool operator==(GfVec3d const& other) const {
        return _data[0] == other[0] && _data[1] == other[1] && _data[2] == other[2];
    }
    bool operator!=(GfVec3d const& other) const { return !(*this == other); }
    ...
    GfVec3d operator-() const { return GfVec3d(-_data[0], -_data[1], -_data[2]); }
    ...
    friend GfVec3d operator+(GfVec3d const& l, GfVec3d const& r) { return GfVec3d(l) += r; }
    ...
    GfVec3d operator*(double s) const { return GfVec3d(*this) *= s; }
    friend GfVec3d operator*(double s, GfVec3d const& v) { return v * s; }
    ...
    GfVec3d operator/(double s) const { return *this * (1.0 / s); }

    /// See GfDot().
    double operator*(GfVec3d const& v) const { return _data[0] * v[0] + _data[1] * v[1] + _data[2] * v[2]; }

    /// Returns the projection of \p this onto \p v. That is:
    /// \code
    /// v * (*this * v)
    /// \endcode
    GfVec3d GetProjection(GfVec3d const& v) const { return v * (*this * v); }
    ...
    /// \endcode
    GfVec3d GetComplement(GfVec3d const& b) const { return *this - this->GetProjection(b); }
    ...
    static bool OrthogonalizeBasis(
            GfVec3d* tx, GfVec3d* ty, GfVec3d* tz, const bool normalize, double eps = GF_MIN_ORTHO_TOLERANCE);
...
inline GfVec3d GfCross(GfVec3d const& v1, GfVec3d const& v2) {
    return GfVec3d(v1[1] * v2[2] - v1[2] * v2[1], v1[2] * v2[0] - v1[0] * v2[2], v1[0] * v2[1] - v1[1] * v2[0]);
}
