...
public:
    ...
    GfLineSeg2d(const GfVec2d& p0, const GfVec2d& p1) { _length = _line.Set(p0, p1 - p0); }
    ...
    bool operator==(const GfLineSeg2d& l) const { return (_line == l._line && _length == l._length); }

    /// Component-wise inequality test. The starting points,
    /// and directions must match exactly for lines to be
    /// considered equal.
    bool operator!=(const GfLineSeg2d& r) const { return !(*this == r); }

private:
    GF_API
    friend bool GfFindClosestPoints(const GfLine2d&, const GfLineSeg2d&, GfVec2d*, GfVec2d*, double* t1, double* t2);
    GF_API
    friend bool GfFindClosestPoints(const GfLineSeg2d&, const GfLineSeg2d&, GfVec2d*, GfVec2d*, double*, double*);
...
/// undefined.
GF_API
bool GfFindClosestPoints(const GfLine2d& line,
