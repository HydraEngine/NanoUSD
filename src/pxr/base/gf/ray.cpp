...
GfVec3d GfRay::FindClosestPoint(const GfVec3d& point, double* rayDistance) const {
    ...
    if (rayDistance) *rayDistance = lrd / len;
...
bool GfFindClosestPoints(const GfRay& ray,
    ...
    if (!GfFindClosestPoints(l, line, &rp, &lp, &rd, &ld)) return false;

    if (rd < 0.0) rd = 0.0;

    if (rayPoint) *rayPoint = l.GetPoint(rd);

    if (linePoint) *linePoint = lp;

    if (rayDistance) *rayDistance = rd / len;

    if (lineDistance) *lineDistance = ld;
...
bool GfFindClosestPoints(const GfRay& ray,
    ...
    if (!GfFindClosestPoints(l, seg, &rp, &sp, &rd, &sd)) return false;

    if (rd < 0.0) rd = 0.0;

    if (rayPoint) *rayPoint = l.GetPoint(rd);

    if (segPoint) *segPoint = sp;

    if (rayDistance) *rayDistance = rd / len;

    if (segDistance) *segDistance = sd;
...
bool GfRay::Intersect(const GfVec3d& p0,
    ...
    if (!Intersect(plane, &intersectionDist, frontFacing)) return false;

    if (intersectionDist > maxDist) return false;
    ...
    if (alpha < 0.0 || gamma < 0.0) return false;

    if (distance) *distance = intersectionDist;
    if (barycentricCoords) barycentricCoords->Set(gamma, alpha, beta);
...
bool GfRay::Intersect(const GfPlane& plane, double* distance, bool* frontFacing) const {
    ...
    if (d < GF_MIN_VECTOR_LENGTH && d > -GF_MIN_VECTOR_LENGTH) return false;
    ...
    if (frontFacing) *frontFacing = (d < 0.0);
...
bool GfRay::Intersect(const GfRange3d& box, double* enterDistance, double* exitDistance) const {
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        if (t1 > maxNearest) maxNearest = t1;
        if (t2 < minFarthest) minFarthest = t2;
    }
    ...
    // bounds.
    if (maxNearest > minFarthest || minFarthest < 0.0) return false;

    if (enterDistance) *enterDistance = maxNearest;
    if (exitDistance) *exitDistance = minFarthest;
    return true;
}
...
bool GfRay::Intersect(const GfVec3d& center, double radius, double* enterDistance, double* exitDistance) const {
