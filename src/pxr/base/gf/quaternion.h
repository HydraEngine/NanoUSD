...
public:
    ...
    static GfQuaternion GetZero() { return GfQuaternion(0.0, GfVec3d(0.0, 0.0, 0.0)); }

    /// Returns the identity quaternion, which has a real part of 1 and
    /// an imaginary part of (0,0,0).
    static GfQuaternion GetIdentity() { return GfQuaternion(1.0, GfVec3d(0.0, 0.0, 0.0)); }
    ...
    /// Hash.
    friend inline size_t hash_value(const GfQuaternion& q) { return TfHash::Combine(q.GetReal(), q.GetImaginary()); }
...
private:
    ...
    double _GetLengthSquared() const { return (_real * _real + GfDot(_imaginary, _imaginary)); }
};
