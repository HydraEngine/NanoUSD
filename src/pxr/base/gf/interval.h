...
public:
    ...
    /// Equality operator.
    bool operator==(const GfInterval& rhs) const { return _min == rhs._min && _max == rhs._max; }
    ...
    bool operator<(const GfInterval& rhs) const {
        ...
        // Compare max bound
        if (_max != rhs._max) return _max < rhs._max;
    ...
    friend inline size_t hash_value(GfInterval const& i) { return TfHash::Combine(i._min, i._max); }
    ...
    GfInterval operator-() const { return GfInterval(-_max.value, -_min.value, _max.closed, _min.closed); }
    ...
    static GfInterval GetFullInterval() {
        return GfInterval(-std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(), false,
                          false);
    }
...
private:
    ...
    struct _Bound {
        ...
        bool operator==(const _Bound& rhs) const { return value == rhs.value && closed == rhs.closed; }
        ...
        _Bound operator*(const _Bound& rhs) const { return _Bound(value * rhs.value, closed & rhs.closed); }
        friend inline size_t hash_value(const _Bound& b) { return TfHash::Combine(b.value, b.closed); }
    };
