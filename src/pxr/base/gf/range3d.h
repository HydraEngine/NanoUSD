...
public:
    ...
    GfVec3d GetMidpoint() const { return static_cast<ScalarType>(0.5) * _min + static_cast<ScalarType>(0.5) * _max; }
    ...
    bool IsEmpty() const { return _min[0] > _max[0] || _min[1] > _max[1] || _min[2] > _max[2]; }
    ...
    /// their extrema.
    bool Contains(const GfRange3d& range) const { return Contains(range._min) && Contains(range._max); }

    /// Returns true if the \p point is located inside the range. As with all
    /// operations of this type, the range is assumed to include its extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(const GfVec3d& point) const { return Contains(point); }

    /// Returns true if the \p range is located entirely inside the range. As
    /// with all operations of this type, the ranges are assumed to include
    /// their extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(const GfRange3d& range) const { return Contains(range); }
    ...
    static GfRange3d Union(const GfRange3d& a, const GfRange3d& b) { return GetUnion(a, b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange3d& Union(const GfRange3d& b) { return UnionWith(b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange3d& Union(const GfVec3d& b) { return UnionWith(b); }
    ...
    static GfRange3d Intersection(const GfRange3d& a, const GfRange3d& b) { return GetIntersection(a, b); }
    ...
    const GfRange3d& Intersection(const GfRange3d& b) { return IntersectWith(b); }
    ...
    /// unary division.
    GfRange3d& operator/=(double m) { return *this *= (1.0 / m); }

    /// binary sum.
    GfRange3d operator+(const GfRange3d& b) const { return GfRange3d(_min + b._min, _max + b._max); }

    /// binary difference.
    GfRange3d operator-(const GfRange3d& b) const { return GfRange3d(_min - b._max, _max - b._min); }

    /// scalar multiply.
    friend GfRange3d operator*(double m, const GfRange3d& r) {
        return (m > 0 ? GfRange3d(r._min * m, r._max * m) : GfRange3d(r._max * m, r._min * m));
    }
    ...
    /// scalar divide.
    friend GfRange3d operator/(const GfRange3d& r, double m) { return r * (1.0 / m); }

    /// hash.
    friend inline size_t hash_value(const GfRange3d& r) { return TfHash::Combine(r._min, r._max); }

    /// The min and max points must match exactly for equality.
    bool operator==(const GfRange3d& b) const { return (_min == b._min && _max == b._max); }

    bool operator!=(const GfRange3d& b) const { return !(*this == b); }
...
inline bool GfRange3d::operator==(const GfRange3f& other) const {
    return _min == GfVec3d(other.GetMin()) && _max == GfVec3d(other.GetMax());
}
