...
public:
    ...
    double GetMidpoint() const { return static_cast<ScalarType>(0.5) * _min + static_cast<ScalarType>(0.5) * _max; }
    ...
    bool Contains(double point) const { return (point >= _min && point <= _max); }
    ...
    /// their extrema.
    bool Contains(const GfRange1d& range) const { return Contains(range._min) && Contains(range._max); }

    /// Returns true if the \p point is located inside the range. As with all
    /// operations of this type, the range is assumed to include its extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(double point) const { return Contains(point); }

    /// Returns true if the \p range is located entirely inside the range. As
    /// with all operations of this type, the ranges are assumed to include
    /// their extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(const GfRange1d& range) const { return Contains(range); }

    /// Returns true if the \p range is located entirely outside the range. As
    /// with all operations of this type, the ranges are assumed to include
    /// their extrema.
    bool IsOutside(const GfRange1d& range) const { return (range._max < _min || range._min > _max); }
    ...
    static GfRange1d Union(const GfRange1d& a, const GfRange1d& b) { return GetUnion(a, b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange1d& Union(const GfRange1d& b) { return UnionWith(b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange1d& Union(double b) { return UnionWith(b); }
    ...
    static GfRange1d Intersection(const GfRange1d& a, const GfRange1d& b) { return GetIntersection(a, b); }
    ...
    const GfRange1d& Intersection(const GfRange1d& b) { return IntersectWith(b); }
    ...
    /// unary division.
    GfRange1d& operator/=(double m) { return *this *= (1.0 / m); }

    /// binary sum.
    GfRange1d operator+(const GfRange1d& b) const { return GfRange1d(_min + b._min, _max + b._max); }

    /// binary difference.
    GfRange1d operator-(const GfRange1d& b) const { return GfRange1d(_min - b._max, _max - b._min); }

    /// scalar multiply.
    friend GfRange1d operator*(double m, const GfRange1d& r) {
        return (m > 0 ? GfRange1d(r._min * m, r._max * m) : GfRange1d(r._max * m, r._min * m));
    }
    ...
    /// scalar divide.
    friend GfRange1d operator/(const GfRange1d& r, double m) { return r * (1.0 / m); }

    /// hash.
    friend inline size_t hash_value(const GfRange1d& r) { return TfHash::Combine(r._min, r._max); }

    /// The min and max points must match exactly for equality.
    bool operator==(const GfRange1d& b) const { return (_min == b._min && _max == b._max); }

    bool operator!=(const GfRange1d& b) const { return !(*this == b); }
...
inline bool GfRange1d::operator==(const GfRange1f& other) const {
    return _min == double(other.GetMin()) && _max == double(other.GetMax());
}
