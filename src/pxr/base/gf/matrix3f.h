...
template <>
struct GfIsGfMatrix<class GfMatrix3f> {
...
public:
    ...
    GfVec3f GetRow(int i) const { return GfVec3f(_mtx[i][0], _mtx[i][1], _mtx[i][2]); }

    /// Gets a column of the matrix as a Vec3.
    GfVec3f GetColumn(int i) const { return GfVec3f(_mtx[0][i], _mtx[1][i], _mtx[2][i]); }
    ...
    GfMatrix3f& SetIdentity() { return SetDiagonal(1); }

    /// Sets the matrix to zero.
    GfMatrix3f& SetZero() { return SetDiagonal(0); }
    ...
    friend inline size_t hash_value(GfMatrix3f const& m) {
        return TfHash::Combine(m._mtx[0][0], m._mtx[0][1], m._mtx[0][2], m._mtx[1][0], m._mtx[1][1], m._mtx[1][2],
    ...
    ///
    // Returns the product of a matrix and a float.
    friend GfMatrix3f operator*(double d, const GfMatrix3f& m) { return m * d; }
    ...
    friend GfMatrix3f operator/(const GfMatrix3f& m1, const GfMatrix3f& m2) { return (m1 * m2.GetInverse()); }
