...
public:
    ...
    GF_API
    GfRotation(const GfVec3d& rotateFrom, const GfVec3d& rotateTo) { SetRotateInto(rotateFrom, rotateTo); }

    /// Sets the rotation to be \p angle degrees about \p axis.
    GfRotation& SetAxisAngle(const GfVec3d& axis, double angle) {
        _axis = axis;
        _angle = angle;
        if (!GfIsClose(_axis * _axis, 1.0, 1e-10)) _axis.Normalize();
        return *this;
    }
    ...
    /// Hash.
    friend inline size_t hash_value(const GfRotation& r) { return TfHash::Combine(r._axis, r._angle); }
    ...
    bool operator==(const GfRotation& r) const { return (_axis == r._axis && _angle == r._angle); }
    ...
    /// Returns a rotation equivalent to \p r with its angle multiplied
    /// by \p scale.
    friend GfRotation operator*(double scale, const GfRotation& r) { return (r * scale); }
