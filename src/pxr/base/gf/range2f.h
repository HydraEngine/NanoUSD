...
template <>
struct GfIsGfRange<class GfRange2f> {
...
public:
    ...
    GfVec2f GetMidpoint() const { return static_cast<ScalarType>(0.5) * _min + static_cast<ScalarType>(0.5) * _max; }
    ...
    /// their extrema.
    bool Contains(const GfRange2f& range) const { return Contains(range._min) && Contains(range._max); }

    /// Returns true if the \p point is located inside the range. As with all
    /// operations of this type, the range is assumed to include its extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(const GfVec2f& point) const { return Contains(point); }

    /// Returns true if the \p range is located entirely inside the range. As
    /// with all operations of this type, the ranges are assumed to include
    /// their extrema.
    /// \deprecated Use Contains() instead.
    bool IsInside(const GfRange2f& range) const { return Contains(range); }
    ...
    static GfRange2f Union(const GfRange2f& a, const GfRange2f& b) { return GetUnion(a, b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange2f& Union(const GfRange2f& b) { return UnionWith(b); }

    /// Extend \p this to include \p b.
    /// \deprecated Use UnionWith() instead.
    const GfRange2f& Union(const GfVec2f& b) { return UnionWith(b); }
    ...
    static GfRange2f Intersection(const GfRange2f& a, const GfRange2f& b) { return GetIntersection(a, b); }
    ...
    const GfRange2f& Intersection(const GfRange2f& b) { return IntersectWith(b); }
    ...
    /// unary division.
    GfRange2f& operator/=(double m) { return *this *= (1.0 / m); }

    /// binary sum.
    GfRange2f operator+(const GfRange2f& b) const { return GfRange2f(_min + b._min, _max + b._max); }

    /// binary difference.
    GfRange2f operator-(const GfRange2f& b) const { return GfRange2f(_min - b._max, _max - b._min); }
    ...
    /// scalar divide.
    friend GfRange2f operator/(const GfRange2f& r, double m) { return r * (1.0 / m); }

    /// hash.
    friend inline size_t hash_value(const GfRange2f& r) { return TfHash::Combine(r._min, r._max); }

    /// The min and max points must match exactly for equality.
    bool operator==(const GfRange2f& b) const { return (_min == b._min && _max == b._max); }

    bool operator!=(const GfRange2f& b) const { return !(*this == b); }
