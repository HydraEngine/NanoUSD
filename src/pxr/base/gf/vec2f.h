...
public:
    ...
    template <class Scl>
    constexpr explicit GfVec2f(Scl const* p) : _data{p[0], p[1]} {}
    ...
    GfVec2f& Set(float const* a) { return Set(a[0], a[1]); }
    ...
    /// Hash.
    friend inline size_t hash_value(GfVec2f const& vec) { return TfHash::Combine(vec[0], vec[1]); }

    /// Equality comparison.
    bool operator==(GfVec2f const& other) const { return _data[0] == other[0] && _data[1] == other[1]; }
    bool operator!=(GfVec2f const& other) const { return !(*this == other); }
    ...
    GfVec2f operator-() const { return GfVec2f(-_data[0], -_data[1]); }
    ...
    friend GfVec2f operator+(GfVec2f const& l, GfVec2f const& r) { return GfVec2f(l) += r; }
    ...
    GfVec2f operator*(double s) const { return GfVec2f(*this) *= s; }
    friend GfVec2f operator*(double s, GfVec2f const& v) { return v * s; }
    ...
    GfVec2f operator/(double s) const { return *this * (1.0 / s); }

    /// See GfDot().
    float operator*(GfVec2f const& v) const { return _data[0] * v[0] + _data[1] * v[1]; }

    /// Returns the projection of \p this onto \p v. That is:
    /// \code
    /// v * (*this * v)
    /// \endcode
    GfVec2f GetProjection(GfVec2f const& v) const { return v * (*this * v); }
    ...
    /// \endcode
    GfVec2f GetComplement(GfVec2f const& b) const { return *this - this->GetProjection(b); }
