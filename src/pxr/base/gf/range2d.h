...
public:
    ...
    GfVec2d GetMidpoint() const { return static_cast<ScalarType>(0.5) * _min + static_cast<ScalarType>(0.5) * _max; }
    ...
    /// their extrema.
    bool Contains(const GfRange2d& range) const { return Contains(range._min) && Contains(range._max); }
    ...
    static GfRange2d Union(const GfRange2d& a, const GfRange2d& b) { return GetUnion(a, b); }
    ...
    static GfRange2d Intersection(const GfRange2d& a, const GfRange2d& b) { return GetIntersection(a, b); }
    ...
    /// binary sum.
    GfRange2d operator+(const GfRange2d& b) const { return GfRange2d(_min + b._min, _max + b._max); }

    /// binary difference.
    GfRange2d operator-(const GfRange2d& b) const { return GfRange2d(_min - b._max, _max - b._min); }

    /// scalar multiply.
    friend GfRange2d operator*(double m, const GfRange2d& r) {
        return (m > 0 ? GfRange2d(r._min * m, r._max * m) : GfRange2d(r._max * m, r._min * m));
    }
    ...
    /// scalar divide.
    friend GfRange2d operator/(const GfRange2d& r, double m) { return r * (1.0 / m); }

    /// hash.
    friend inline size_t hash_value(const GfRange2d& r) { return TfHash::Combine(r._min, r._max); }

    /// The min and max points must match exactly for equality.
    bool operator==(const GfRange2d& b) const { return (_min == b._min && _max == b._max); }
