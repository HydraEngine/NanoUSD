...
public:
    ...
    /// Move construction
    TfWeakPtr(TfWeakPtr&& p) noexcept : _rawPtr(p._rawPtr), _remnant(std::move(p._remnant)) { p._rawPtr = nullptr; }
    ...
    TfWeakPtr(TfRefPtr<U> const& p, typename std::enable_if<std::is_convertible<U*, T*>::value>::type* dummy = 0)
        ...
        TF_UNUSED(dummy);
        if (ARCH_LIKELY(_rawPtr)) _remnant = Tf_WeakBaseAccess::GetRemnant(_rawPtr->__GetTfWeakBase__());
    }

    /// Explicitly construct from a raw pointer \a p.
    template <class U>
    explicit TfWeakPtr(U* p, typename std::enable_if<std::is_convertible<U*, T*>::value>::type* dummy = nullptr)
        : _rawPtr(p) {
        TF_UNUSED(dummy);
        if (ARCH_LIKELY(_rawPtr)) _remnant = Tf_WeakBaseAccess::GetRemnant(_rawPtr->__GetTfWeakBase__());
    }
    ...
    TfWeakPtr(TfWeakPtr<U> const& p, typename std::enable_if<std::is_convertible<U*, T*>::value>::type* dummy = 0)
        : _rawPtr(p._rawPtr), _remnant(p._remnant) {}
    ...
    bool IsExpired() const { return this->IsInvalid(); }

private:
    T* _FetchPointer() const {
        if (ARCH_LIKELY(_remnant && _remnant->_IsAlive())) return _rawPtr;
        return 0;
    }

    bool _IsInvalid() const { return _remnant && !_remnant->_IsAlive(); }

    void const* _GetUniqueIdentifier() const { return _remnant ? _remnant->_GetUniqueIdentifier() : 0; }

    void _EnableExtraNotification() const { _remnant->EnableNotification(); }
...
public:
    ...
    template <class U>
    TfWeakPtr(TfWeakPtr<U> const& wp) : _remnant(wp._remnant) {}
    ...
    operator UnspecifiedBoolType() const { return (_remnant && _remnant->_IsAlive()) ? &TfWeakPtr::_remnant : nullptr; }

    bool operator!() const { return !bool(*this); }

    bool IsExpired() const { return _remnant && !_remnant->_IsAlive(); }
