...
///
/// \hideinitializer
#define TF_PY_REPR_PREFIX std::string(TF_PP_STRINGIZE(MFB_PACKAGE_MODULE) ".")
...
inline pxr_boost::python::object TfPyObject(PyObject* t, bool complainOnFailure = true) {
    TfPyLock pyLock;
...
std::string TfPyRepr(T const& t) {
    if (!TfPyIsInitialized()) return "<python not initialized>";
    TfPyLock lock;
...
TF_API
pxr_boost::python::object TfPyEvaluate(std::string const& expr,
...
TF_API
int64_t TfPyNormalizeIndex(int64_t index, uint64_t size, bool throwError = false);
...
TF_API
void Tf_PyWrapOnceImpl(pxr_boost::python::type_info const&, std::function<void()> const&, bool*);
...
pxr_boost::python::dict TfPyCopyMapToDictionary(Map const& map) {
    ...
    for (typename Map::const_iterator i = map.begin(); i != map.end(); ++i) d[i->first] = i->second;
    return d;
}
...
pxr_boost::python::list TfPyCopySequenceToList(Seq const& seq) {
    ...
    for (typename Seq::const_iterator i = seq.begin(); i != seq.end(); ++i) l.append(*i);
    return l;
}
...
bool TfPyEvaluateAndExtract(const std::string& expr, T* t) {
    ...
    if (!Tf_PyEvaluateWithErrorCheck(expr, &obj)) return false;
