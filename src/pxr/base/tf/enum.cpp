...
private:
    static Tf_EnumRegistry& _GetInstance() { return TfSingleton<Tf_EnumRegistry>::GetInstance(); }
    ...
    ~Tf_EnumRegistry() { TfRegistryManager::GetInstance().UnsubscribeFrom<TfEnum>(); }
    ...
    void _Remove(TfEnum val) {
        ...
        for (size_t i = 0; i < original.size(); i++)
            if (original[i] != name) v.push_back(original[i]);
...
string TfEnum::GetName(TfEnum val) {
    if (TfSafeTypeCompare(val.GetType(), typeid(int))) return TfIntToString(val.GetValueAsInt());
...
string TfEnum::GetFullName(TfEnum val) {
    if (TfSafeTypeCompare(val.GetType(), typeid(int))) return TfStringPrintf("int::%d", val.GetValueAsInt());
...
vector<string> TfEnum::GetAllNames(const type_info& ti) {
    if (TfSafeTypeCompare(ti, typeid(int))) return vector<string>();
...
void TfEnum::_FatalGetValueError(std::type_info const& typeInfo) const {
    TF_FATAL_ERROR(
            ...
            "a '%s'.",
            ArchGetDemangled(typeInfo).c_str(), _typeInfo->name());
}
