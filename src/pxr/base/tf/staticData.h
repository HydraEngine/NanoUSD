...
private:
    T* _TryToCreateData() const {
        ...
        T* n = nullptr;
        if (ARCH_LIKELY(_data.compare_exchange_strong(n, tmp))) return tmp;
...
#define TF_MAKE_STATIC_DATA(Type, Name)                                                                   \
    static void TF_PP_CAT(Name, _Tf_StaticDataFactoryImpl)(std::remove_const_t<TF_PP_EAT_PARENS(Type)>*); \
    namespace {                                                                                           \
    ...
    static void TF_PP_CAT(Name, _Tf_StaticDataFactoryImpl)(std::remove_const_t<TF_PP_EAT_PARENS(Type)> * Name)
