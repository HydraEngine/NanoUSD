...
class TfFunctionRef<Ret(Args...)> {
    ...
    using _IsFunctionRef = std::is_same<std::remove_cv_t<std::remove_reference_t<Fn>>, TfFunctionRef>;
...
public:
    ...
    inline Ret operator()(Args... args) const { return _invoke(_fn, std::forward<Args>(args)...); }
