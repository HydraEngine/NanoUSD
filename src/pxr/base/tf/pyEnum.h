...
private:
    ...
    struct _EnumFromPython {
        _EnumFromPython() {
            pxr_boost::python::converter::registry::insert(&convertible, &construct, pxr_boost::python::type_id<T>());
        }
        static void* convertible(PyObject* obj) {
            TfHashMap<PyObject*, TfEnum, _ObjectHash> const& o2e = Tf_PyEnumRegistry::GetInstance()._objectsToEnums;
            TfHashMap<PyObject*, TfEnum, _ObjectHash>::const_iterator i = o2e.find(obj);
            ...
            if (std::is_same<T, TfEnum>::value || (std::is_integral<T>::value && !std::is_enum<T>::value))
        ...
        static void construct(PyObject* src, pxr_boost::python::converter::rvalue_from_python_stage1_data* data) {
            void* storage = ((pxr_boost::python::converter::rvalue_from_python_storage<T>*)data)->storage.bytes;
    ...
    struct _EnumToPython {
        static PyObject* convert(T t) { return Tf_PyEnumRegistry ::GetInstance()._ConvertEnumToPython(TfEnum(t)); }
    };

    // Since our enum objects live as long as the registry does, we can use the
    // pointer values for a hash.
    struct _ObjectHash {
        size_t operator()(PyObject* o) const { return reinterpret_cast<size_t>(o); }
    };
...
struct Tf_PyEnumWrapper : public Tf_PyEnum {
    ...
    long GetValue() const { return value.GetValueAsInt(); }
    std::string GetName() const { return name; }
    std::string GetDisplayName() const { return TfEnum::GetDisplayName(value); }
    std::string GetFullName() const { return TfEnum::GetFullName(value); }
    friend bool operator==(Tf_PyEnumWrapper const& self, long other) { return self.value.GetValueAsInt() == other; }

    friend bool operator==(Tf_PyEnumWrapper const& lhs, Tf_PyEnumWrapper const& rhs) { return lhs.value == rhs.value; }

    friend bool operator!=(Tf_PyEnumWrapper const& lhs, Tf_PyEnumWrapper const& rhs) { return !(lhs == rhs); }

    friend bool operator<(Tf_PyEnumWrapper const& lhs, Tf_PyEnumWrapper const& rhs) {
        // If same, not less.
        if (lhs == rhs) return false;
        // If types don't match, string compare names.
        if (!lhs.value.IsA(rhs.value.GetType())) return TfEnum::GetFullName(lhs.value) < TfEnum::GetFullName(rhs.value);
        // If types do match, numerically compare values.
        return lhs.GetValue() < rhs.GetValue();
    }

    friend bool operator>(Tf_PyEnumWrapper const& lhs, Tf_PyEnumWrapper const& rhs) { return rhs < lhs; }
...
struct Tf_TypedPyEnumWrapper : Tf_PyEnumWrapper {
    ...
    static pxr_boost::python::object GetValueFromName(const std::string& name) {
        ...
        return found ? pxr_boost::python::object(value) : pxr_boost::python::object();
    }
};
...
TF_API
std::string Tf_PyCleanEnumName(std::string name, bool stripPackageName = false);
...
struct TfPyWrapEnum {
private:
    typedef pxr_boost::python::class_<Tf_TypedPyEnumWrapper<T>, pxr_boost::python::bases<Tf_PyEnumWrapper>>
            _EnumPyClassType;
...
public:
    ...
    explicit TfPyWrapEnum(std::string const& name = std::string()) {
        ...
        std::string enumName = explicitName ? name : TfStringReplace(ArchGetDemangled(typeid(T)), "::", ".");
        ...
        if (baseName == enumName) baseName = std::string();

        // If the name is dotted, take the last element as the enum name.
        if (!TfStringGetSuffix(enumName).empty()) enumName = TfStringGetSuffix(enumName);

        // If the name was not explicitly given, then clean it up by removing
        // the package name prefix if it exists.
        if (!explicitName) {
            if (!baseName.empty()) {
                baseName = Tf_PyCleanEnumName(baseName, /* stripPackageName = */ true);
            } else {
                enumName = Tf_PyCleanEnumName(enumName, /* stripPackageName = */ true);
            }
        }
        ...
        if (!type.IsUnknown()) type.DefinePythonClass(enumClass);
    }
...
private:
    ...
    void _ExportValues(bool stripPackageName, _EnumPyClassType& enumClass) {
        ...
        for (const std::string& name : TfEnum::GetAllNames<T>()) {
            ...
            const std::string cleanedName = Tf_PyCleanEnumName(name, stripPackageName);
