...
struct Tf_TokenRegistry {
    ...
    struct _Eq {
        ...
        inline bool operator()(const char* s1, const char* s2) const { return !strcmp(s1, s2); }
    };

    template <int Mul>
    struct _Hash {
        inline size_t operator()(TfToken::_Rep const& rep) const { return (*this)(rep._cstr); }
    ...
    struct alignas(ARCH_CACHE_LINE_SIZE) _Set {
        _RepSet reps;
    ...
    static Tf_TokenRegistry& _GetInstance() { return TfSingleton<Tf_TokenRegistry>::GetInstance(); }
    ...
    inline _RepPtrAndBits _GetPtrChar(char const* s, bool makeImmortal) { return _GetPtrImpl(s, makeImmortal); }
    ...
    inline _RepPtrAndBits _FindPtrChar(char const* s) const { return _FindPtrImpl(s); }
...
private:
    ...
    inline _RepPtrAndBits _GetPtrImpl(Str s, bool makeImmortal) {
        ...
        if (iter != set.reps.end()) {
            ...
            if (set.insertsUntilSweepCheck == 0) {
                ...
                if ((float(set.reps.size() + 1) / float(set.reps.bucket_count()) > set.reps.max_load_factor())) {
                    ...
                    for (auto iter = set.reps.begin(), end = set.reps.end(); iter != end;) {
                        if (iter->_refCount == 1) {
                            set.reps.erase(iter++);
                        } else {
                            ++iter;
                    ...
                    set.insertsUntilSweepCheck =
                            std::max(_MinInsertsUntilSweepCheck,
            ...
            _RepPtr rep = &(*set.reps.insert(TfToken::_Rep(s, setNum, compareCode)).first);
            // 3, because 1 for counted bit plus 2 for one refcount.  We can
            // store relaxed here since our lock on the set's mutex provides
            // synchronization.
            rep->_refCount.store(makeImmortal ? 0 : 3, std::memory_order_relaxed);
    ...
    _RepPtrAndBits _FindPtrImpl(Str s) const {
        ...
        unsigned oldVal = rep->_refCount.fetch_add(2, std::memory_order_relaxed);
...
TfToken::TfToken(const string& s) : _rep(Tf_TokenRegistry::_GetInstance()._GetPtrStr(s, /*makeImmortal*/ false)) {}

TfToken::TfToken(const char* s) : _rep(Tf_TokenRegistry::_GetInstance()._GetPtrChar(s, /*makeImmortal*/ false)) {}
...
std::vector<string> TfToStringVector(const std::vector<TfToken>& tv) {
    ...
    for (size_t i = 0; i != tv.size(); i++) sv[i] = tv[i].GetString();
