...
public:
    ...
    template <class T>
    TfEnum(T value, std::enable_if_t<std::is_enum<T>::value>* = 0) : _typeInfo(&typeid(T)), _value(int(value)) {}
    ...
    bool operator==(const TfEnum& t) const { return t._value == _value && TfSafeTypeCompare(*t._typeInfo, *_typeInfo); }
    ...
    bool IsA(const std::type_info& t) const { return TfSafeTypeCompare(*_typeInfo, t); }
    ...
    template <typename T>
    T GetValue() const {
        if (!IsA<T>()) _FatalGetValueError(typeid(T));
    ...
    static std::vector<std::string> GetAllNames(TfEnum val) { return GetAllNames(val.GetType()); }
    ...
    static char const* _NameIdent(char const* str = nullptr) { return str; }
...
/// \ingroup group_tf_RuntimeTyping
/// \hideinitializer
#define TF_ADD_ENUM_NAME(VAL, ...) TfEnum::_AddName(VAL, TF_PP_STRINGIZE(VAL), TfEnum::_NameIdent(__VA_ARGS__));
