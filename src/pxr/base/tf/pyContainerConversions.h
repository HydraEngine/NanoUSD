...
struct to_tuple<std::pair<First, Second>> {
    static PyObject* convert(std::pair<First, Second> const& a) {
        pxr_boost::python::tuple result = pxr_boost::python::make_tuple(a.first, a.second);
        return pxr_boost::python::incref(result.ptr());
    }
};
...
struct from_python_sequence {
    ...
    static void* convertible(PyObject* obj_ptr) {
        if (!(PyList_Check(obj_ptr) || PyTuple_Check(obj_ptr) || PySet_Check(obj_ptr) || PyFrozenSet_Check(obj_ptr) ||
              PyIter_Check(obj_ptr) || PyRange_Check(obj_ptr) ||
              (!PyBytes_Check(obj_ptr) && !PyUnicode_Check(obj_ptr) &&
               (Py_TYPE(obj_ptr) == 0 || Py_TYPE(Py_TYPE(obj_ptr)) == 0 || Py_TYPE(Py_TYPE(obj_ptr))->tp_name == 0 ||
            ...
            return 0;
        pxr_boost::python::handle<> obj_iter(pxr_boost::python::allow_null(PyObject_GetIter(obj_ptr)));
        ...
        if (ConversionPolicy::check_convertibility_per_element()) {
            ...
            if (!ConversionPolicy::check_size((ContainerType*)nullptr, obj_size)) return 0;
    ...
    static bool all_elements_convertible(pxr_boost::python::handle<>& obj_iter, bool is_range, std::size_t& i) {
        for (;; i++) {
            pxr_boost::python::handle<> py_elem_hdl(pxr_boost::python::allow_null(PyIter_Next(obj_iter.get())));
            if (PyErr_Occurred()) {
                PyErr_Clear();
                return false;
            }
    ...
    static void construct(PyObject* obj_ptr, pxr_boost::python::converter::rvalue_from_python_stage1_data* data) {
        ...
        void* storage = ((pxr_boost::python::converter::rvalue_from_python_storage<ContainerType>*)data)->storage.bytes;
...
struct from_python_tuple_pair {
    ...
    static void construct(PyObject* obj_ptr, pxr_boost::python::converter::rvalue_from_python_stage1_data* data) {
        void* storage = ((pxr_boost::python::converter::rvalue_from_python_storage<PairType>*)data)->storage.bytes;
        pxr_boost::python::extract<first_type> e1(PyTuple_GetItem(obj_ptr, 0));
        pxr_boost::python::extract<second_type> e2(PyTuple_GetItem(obj_ptr, 1));
...
struct to_tuple_mapping {
    to_tuple_mapping() { pxr_boost::python::to_python_converter<ContainerType, to_tuple<ContainerType>>(); }
};
...
struct tuple_mapping_fixed_size {
    tuple_mapping_fixed_size() { tuple_mapping<ContainerType, fixed_size_policy>(); }
};

template <typename ContainerType>
struct tuple_mapping_fixed_capacity {
    tuple_mapping_fixed_capacity() { tuple_mapping<ContainerType, fixed_capacity_policy>(); }
};

template <typename ContainerType>
struct tuple_mapping_variable_capacity {
    tuple_mapping_variable_capacity() { tuple_mapping<ContainerType, variable_capacity_policy>(); }
};
...
struct tuple_mapping_pair {
    tuple_mapping_pair() {
        pxr_boost::python::to_python_converter<ContainerType, to_tuple<ContainerType>>();
