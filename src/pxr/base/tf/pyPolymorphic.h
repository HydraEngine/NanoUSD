...
struct TfPyPolymorphic : public TfType::PyPolymorphicBase, public pxr_boost::python::wrapper<Derived> {
    ...
    Override GetOverride(char const* func) const {
        ...
        if (m_self) {
            ...
            if (handle<> m = handle<>(allow_null(PyObject_GetAttrString(m_self, const_cast<char*>(func))))) {
                ...
                if (func_object != ((PyMethodObject*)m.get())->im_func) return Override(m);
            }
        }
    ...
    Override GetPureOverride(char const* func) const {
        ...
        if (!ret) {
            ...
            PyErr_SetString(PyExc_AttributeError, TfStringPrintf("Pure virtual method '%s' called -- "
...
inline std::function<Ret(Args...)> TfPyPolymorphic<Derived>::CallVirtual(char const* fname,
    ...
    TfPyLock lock;
    if (Override o = GetOverride(fname)) return std::function<Ret(Args...)>(TfPyCall<Ret>(o));
...
namespace python {
template <typename T>
struct has_back_reference<PXR_NS::TfPyPolymorphic<T>> : std::true_type {};
}  // namespace python
}  // namespace PXR_BOOST_NAMESPACE
