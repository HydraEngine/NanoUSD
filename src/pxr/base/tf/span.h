...
public:
    ...
    TfSpan(pointer ptr, index_type count) : _data(ptr), _size(count) { TF_DEV_AXIOM(count == 0 || ptr); }

    /// Construct a span over the range [first, last).
    TfSpan(pointer first, pointer last) : TfSpan(first, index_type(last - first)) { TF_DEV_AXIOM(last >= first); }
    ...
    TfSpan(const Container& cont,
           typename std::enable_if<std::is_same<typename Container::value_type, value_type>::value, Container>::type* =
                   0)
        : _data(cont.data()), _size(cont.size()) {
        TF_DEV_AXIOM(_size == 0 || _data);
    }
