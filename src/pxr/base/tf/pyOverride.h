...
public:
    ...
    operator T&() const {
        TfPyLock lock;
        pxr_boost::python::converter::return_from_python<T&> converter;
        return converter(const_cast<pxr_boost::python::handle<>&>(m_obj).release());
    }
...
public:
    ...
    TfPyOverride(pxr_boost::python::handle<> callable) : TfPyObjWrapper(pxr_boost::python::object(callable)) {}
    ...
    TfPyMethodResult operator()(Args const&... args) const {
        ...
        TfPyMethodResult x(PyEval_CallFunction(this->ptr(), const_cast<char*>(pyCallFormat),
