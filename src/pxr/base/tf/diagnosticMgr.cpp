...
void TfDiagnosticMgr::PostError(TfEnum errorCode,
                                ...
                                bool quiet) {
    if (TfDebug::IsEnabled(TF_ATTACH_DEBUGGER_ON_ERROR)) ArchDebuggerTrap();
...
void TfDiagnosticMgr::PostError(const TfDiagnosticBase& diagnostic) {
    PostError(diagnostic.GetDiagnosticCode(), diagnostic.GetDiagnosticCodeAsString().c_str(), diagnostic.GetContext(),
              diagnostic.GetCommentary(), diagnostic._info, diagnostic.GetQuiet());
}
...
void TfDiagnosticMgr::PostWarning(TfEnum warningCode,
    ...
    if (TfDebug::IsEnabled(TF_ATTACH_DEBUGGER_ON_WARNING)) ArchDebuggerTrap();
...
void TfDiagnosticMgr::PostWarning(const TfDiagnosticBase& diagnostic) const {
    PostWarning(diagnostic.GetDiagnosticCode(), diagnostic.GetDiagnosticCodeAsString().c_str(), diagnostic.GetContext(),
                diagnostic.GetCommentary(), diagnostic._info, diagnostic.GetQuiet());
}
...
void TfDiagnosticMgr::PostStatus(const TfDiagnosticBase& diagnostic) const {
    PostStatus(diagnostic.GetDiagnosticCode(), diagnostic.GetDiagnosticCodeAsString().c_str(), diagnostic.GetContext(),
               diagnostic.GetCommentary(), diagnostic._info, diagnostic.GetQuiet());
}

void TfDiagnosticMgr::PostFatal(TfCallContext const& context, TfEnum statusCode, std::string const& msg) const {
    _ReentrancyGuard guard(&_reentrantGuard.local());
    if (guard.ScopeWasReentered()) {
        TfLogCrash("RECURSIVE FATAL ERROR", msg, std::string() /*additionalInfo*/, context, true /*logToDB*/);
    }

    if (TfDebug::IsEnabled(TF_ATTACH_DEBUGGER_ON_ERROR) || TfDebug::IsEnabled(TF_ATTACH_DEBUGGER_ON_FATAL_ERROR)) {
        ArchDebuggerTrap();
    }
...
void TfDiagnosticMgr::ErrorHelper::Post(const string& msg) const {
    TfDiagnosticMgr::GetInstance().PostError(_errorCode, _errorCodeString, _context, msg, TfDiagnosticInfo(), false);
}
...
void TfDiagnosticMgr::WarningHelper::Post(const string& msg) const {
    TfDiagnosticMgr::GetInstance().PostWarning(_warningCode, _warningCodeString, _context, msg, TfDiagnosticInfo(),
                                               false);
}
...
void TfDiagnosticMgr::StatusHelper::Post(const string& msg) const {
    TfDiagnosticMgr::GetInstance().PostStatus(_statusCode, _statusCodeString, _context, msg, TfDiagnosticInfo(), false);
}
...
void TfDiagnosticMgr::_LogText::_AppendAndPublishImpl(bool clear, ErrorIterator begin, ErrorIterator end) {
    ...
    if (parity) std::swap(first, second);

    // Update first.
    if (clear) first->clear();
...
std::string TfDiagnosticMgr::FormatDiagnostic(const TfEnum& code,
    ...
    if (context.IsHidden() || !strcmp(context.GetFunction(), "") || !strcmp(context.GetFile(), "")) {
        output = TfStringPrintf("%s%s: %s [%s]\n", codeName.c_str(), ArchIsMainThread() ? "" : " (secondary thread)",
                                msg.c_str(), ArchGetProgramNameForErrors());
    } else {
...
#ifdef PXR_PYTHON_SUPPORT_ENABLED
    if (const TfPyExceptionState* exc = std::any_cast<TfPyExceptionState>(&info)) {
