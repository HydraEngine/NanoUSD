...
struct TfPyAnnotatedBoolResult {
    ...
    std::string GetRepr() const { return GetValue() ? "True" : "(False, " + TfPyRepr(GetAnnotation()) + ")"; }

    /// Returns \c true if the result is the same as \p rhs.
    bool operator==(bool rhs) const { return _val == rhs; }

    friend bool operator==(bool lhs, const TfPyAnnotatedBoolResult& rhs) { return rhs == lhs; }

    friend bool operator!=(const TfPyAnnotatedBoolResult& lhs, bool rhs) { return !(lhs == rhs); }

    friend bool operator!=(bool lhs, const TfPyAnnotatedBoolResult& rhs) { return !(lhs == rhs); }
