...
size_t TfBits::_CountNumSet() const {
...
size_t TfBits::_FindNextSet(size_t index, size_t startBit) const {
...
size_t TfBits::_FindPrevSet(size_t index, size_t startBit) const {
...
size_t TfBits::_FindNextUnset(size_t index, size_t startBit) const {
    for (size_t w = index >> 6; w < _numWords; w++) {
...
bool TfBits::operator==(const TfBits& rhs) const {
    ...
    if (firstSet != rhsFirstSet || lastSet != rhsLastSet) return false;

    // Are any of the sets empty?
    if (firstSet == _num || rhsFirstSet == _num) return firstSet == rhsFirstSet;
...
TfBits& TfBits::operator|=(const TfBits& rhs) {
    if (TF_VERIFY(_num == rhs._num)) _Or(rhs);

    return *this;
}

void TfBits::_Or(const TfBits& rhs) {
    // Early bail out.
    if (this == &rhs) return;
    ...
    if (rhsFirstSet < rhs._num) {
        ...
        size_t lastSet = (lhsLastSet < _num) ? TfMax(lhsLastSet, rhsLastSet) : rhsLastSet;
        ...
        if (_numSet.Load() == lastSet - firstSet + 1 && firstSet == lhsFirstSet && lastSet == lhsLastSet) return;
...
void TfBits::OrSubset(const TfBits& rhs) {
    if (TF_VERIFY(_num >= rhs._num)) _Or(rhs);
}
...
TfBits& TfBits::operator^=(const TfBits& rhs) {
    ...
    if (i1 < _num) {
        ...
        size_t lastSet = (i0 < _num) ? TfMax(GetLastSet(), rhs.GetLastSet()) : rhs.GetLastSet();
...
TfBits& TfBits::Complement() {
    ...
    const size_t numSet = _numSet.Load();
    if (numSet != size_t(-1)) _numSet.Store(_num - numSet);
    _firstSet.Store(-1);
    _lastSet.Store(-1);
...
void TfBits::_ClearTrailingBits() {
...
std::string TfBits::GetAsStringLeftToRight() const {
    std::string res;

    for (size_t i = 0; i < _num; ++i) res.push_back('0' + IsSet(i));

    return res;
}

std::string TfBits::GetAsStringRightToLeft() const {
    std::string res;

    for (int64_t i = _num - 1; i >= 0; --i) res.push_back('0' + IsSet(i));

    return res;
}

std::ostream& operator<<(std::ostream& out, const TfBits& bits) {
