...
#define TF_PY_WRAP_PUBLIC_TOKENS(name, key, seq)                                            \
    pxr_boost::python::class_<_TF_TOKENS_STRUCT_NAME(key), pxr_boost::python::noncopyable>( \
            name, pxr_boost::python::no_init) _TF_PY_TOKENS_WRAP_SEQ(key, seq)
...
#define _TF_PY_TOKENS_WRAP_ATTR_MEMBER(r, key, name) \
    pxr_boost::python::scope().attr(TF_PP_STRINGIZE(name)) = key->name.GetString();
...
#define _TF_PY_TOKEN_GET_ELEM(elem) _TF_PP_IFF(TF_PP_IS_TUPLE(elem), TF_PP_TUPLE_ELEM(0, elem), elem)
