...
struct Tf_TypeCastFunctions<Derived, TfType::Bases<Bases...>> {
    using CastFunction = void* (*)(void*, bool);
    CastFunction castFunctions[sizeof...(Bases)] = {&Tf_CastToParent<Derived, Bases>...};
};
...
TfType const& TfType::Define() {
    ...
    return _DefineImpl(typeid(T), btis.baseTypeInfos, tcfs.castFunctions, btis.NumBases, TfSizeofType<T>::value,
                       std::is_pod_v<T>, std::is_enum_v<T>);
}
