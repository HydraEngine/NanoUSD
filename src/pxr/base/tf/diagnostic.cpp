...
void Tf_TerminateHandler() {
    ...
    if (!throwStack.empty()) {
        ...
        std::string throwContextMsg = throwContext ? TfStringPrintf("at %s (%s:%zu) ", throwContext.GetFunction(),
                                                                    throwContext.GetFile(), throwContext.GetLine())
                                                   : std::string();
        ArchSetExtraLogInfoForErrors(
                TfStringPrintf("Unhandled %s exception: %s; "
                               "thrown %sfrom ",
                               type.c_str(), reason.c_str(), throwContextMsg.empty() ? "" : throwContextMsg.c_str()),
                &throwStackMsg);
    }

    TF_FATAL_ERROR(throwStack.empty() ? "Unhandled exception %s - '%s'"
