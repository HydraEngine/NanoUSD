...
TfBigRWMutex::TfBigRWMutex() : _states(std::make_unique<_LockState[]>(NumStates)), _writerActive(false) {}

void TfBigRWMutex::_AcquireReadContended(int stateIndex) {
...
void TfBigRWMutex::_AcquireWrite() {
    ...
    TfSpinRWMutex::_StagedAcquireWriteState stageStates[NumStates]{TfSpinRWMutex::_StageNotAcquired};

    bool allAcquired;
    do {
        allAcquired = true;
        for (int i = 0; i != NumStates; ++i) {
            stageStates[i] = _states[i].mutex._StagedAcquireWriteStep(stageStates[i]);
            allAcquired &= (stageStates[i] == TfSpinRWMutex::_StageAcquired);
        }
    } while (!allAcquired);
}

void TfBigRWMutex::_ReleaseWrite() {
    _writerActive = false;

    // Release all the write locks.
    for (_LockState *lockState = _states.get(), *end = _states.get() + NumStates; lockState != end; ++lockState) {
