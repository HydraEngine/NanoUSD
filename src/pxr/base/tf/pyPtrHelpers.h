...
struct TfMakePyPtr {
    ...
    static Result Execute(Ptr const& p) {
        ...
        if (!p.GetUniqueIdentifier()) return Result(pxr_boost::python::detail::none(), false);
        ...
        if (PyObject* id = Tf_PyGetPythonIdentity(p)) return Result(id, false);

        // Just make a new python object holding this pointer.
        // TODO: use existing to-python conversion?
        PyObject* res = pxr_boost::python::objects::make_ptr_instance<Pointee, Holder>::execute(p);
...
struct _PtrFromPythonConversionPolicy<TfRefPtr<T>> {
    static void Apply(TfRefPtr<T> const& p, PyObject* obj) { Tf_PyRemovePythonOwnership(p, obj); }
};

template <class Ptr>
struct _PtrFromPython {
    typedef typename _PtrInterface<Ptr>::Pointee Pointee;
    _PtrFromPython() { converter::registry::insert(&convertible, &construct, type_id<Ptr>()); }
...
private:
    ...
    static void construct(PyObject* source, converter::rvalue_from_python_stage1_data* data) {
        void* const storage = ((converter::rvalue_from_python_storage<Ptr>*)data)->storage.bytes;
        ...
        if (data->convertible == source)
            ...
            if (ptr) Tf_PySetPythonIdentity(ptr, source);
        }
...
struct _AnyWeakPtrFromPython {
    _AnyWeakPtrFromPython() { converter::registry::insert(&convertible, &construct, type_id<TfAnyWeakPtr>()); }
...
struct _ConstPtrToPython {
    ...
    _ConstPtrToPython() { to_python_converter<ConstPtr, _ConstPtrToPython<Ptr>>(); }
    static PyObject* convert(ConstPtr const& p) { return incref(object(TfConst_cast<NonConstPtr>(p)).ptr()); }
};

template <typename Ptr>
struct _PtrToPython {
    _PtrToPython() { to_python_converter<Ptr, _PtrToPython<Ptr>>(); }
...
struct _ConvertPtrToPython {
    _ConvertPtrToPython() { to_python_converter<SrcPtr, _ConvertPtrToPython<SrcPtr, DstPtr>>(); }
...
struct WeakPtr : def_visitor<WeakPtr> {
    ...
    static void _RegisterConversionsHelper() {
        ...
        typedef typename _PtrInterface<WrapperPtrType>::template Rebind<T>::Type PtrType;
        ...
        implicitly_convertible<PtrType, typename _PtrInterface<PtrType>::ConstPtr>();
        ...
        converter::registration* r =
                const_cast<converter::registration*>(converter::registry::query(type_id<WrapperPtrType>()));
        if (r) {
