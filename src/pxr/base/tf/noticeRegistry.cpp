 ...
 */
void Tf_NoticeRegistry::_VerifyFailedCast(const type_info& toType, const TfNotice& notice, const TfNotice* castNotice) {
    ...
    TF_FATAL_ERROR(
            ...
            typeName.c_str(), ArchGetDemangled(toType).c_str(), typeName.c_str(), typeName.c_str());
}

void Tf_NoticeRegistry::_InsertProbe(const TfNotice::WeakProbePtr& probe) {
    _Lock lock(_probeMutex);
    if (probe) _probes.insert(probe);
...
void Tf_NoticeRegistry::_BeginSend(const TfNotice& notice,
    ...
    TF_FOR_ALL(i, probes)
    if (*i) (*i)->BeginSend(notice, sender, senderType);
}
...
void Tf_NoticeRegistry::_BeginDelivery(const TfNotice& notice,
                                       ...
                                       const std::vector<TfNotice::WeakProbePtr>& probes) {
    TF_FOR_ALL(i, probes)
    if (*i) (*i)->BeginDelivery(notice, sender, senderType, listener, listenerType);
}
...
TfNotice::Key Tf_NoticeRegistry::_Register(TfNotice::_DelivererBase* deliverer) {
    ...
    _DelivererContainer* container = _GetOrCreateDelivererContainer(noticeType);
