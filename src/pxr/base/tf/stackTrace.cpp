...
void TfLogCrash(const std::string& reason,
    ...
    std::string fullMessage = TfStringPrintf(
            ...
            ArchGetProgramNameForErrors(), reason.c_str(), message.c_str(), context.GetFunction(), context.GetLine(),
            context.GetFile());
...
time_t TfGetAppLaunchTime() {
    ...
    if (launchTime == 0) TF_RUNTIME_ERROR("Could not determine application launch time.");
    return launchTime;
}
