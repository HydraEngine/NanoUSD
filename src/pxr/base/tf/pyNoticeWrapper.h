...
public:
    ...
    static ClassType Wrap(std::string const& name = std::string()) {
        ...
        if (wrappedName.empty()) {
            ...
            if (!TfStringGetSuffix(wrappedName, ':').empty()) wrappedName = TfStringGetSuffix(wrappedName, ':');
        }
        ...
        return ClassType(wrappedName.c_str(), pxr_boost::python::no_init).def(TfTypePythonClass());
    }
