...
public:
    ...
    ///
    struct Hash {
        size_t operator()(TfBits const& bits) const { return bits.GetHash(); }
    };
    ...
    struct FastHash {
        size_t operator()(TfBits const& bits) const {
            return TfHash::Combine(bits.GetSize(), bits.GetFirstSet(), bits.GetLastSet(), bits.GetNumSet());
        }
    };
    ...
    TfBits(size_t num, size_t first, size_t last) {
        ...
        if (num == 0) {
            ...
            ClearAll();
            for (size_t i = first; i <= last; ++i) Set(i);
        }
    }
    ...
    TfBits(const TfBits& rhs) {
        ...
        for (size_t i = 0; i < _numWords; ++i) _bits[i] = rhs._bits[i];
    }

    /// Move constructor.
    ///
    TfBits(TfBits&& rhs) : TfBits(0) { Swap(rhs); }

    /// Destructor
    ///
    ~TfBits() { _Free(_bits, _numWords); }
    ...
    void Resize(size_t num) {
        ...
        if (_numWords) _bits[_numWords - 1] = 0;
    }
    ...
    void ResizeKeepContent(size_t num) {
        ...
        for (size_t i = 0; i < numWordsToCopy; ++i) temp._bits[i] = _bits[i];
    ...
    //
    friend void swap(TfBits& lhs, TfBits& rhs) { lhs.Swap(rhs); }
    ...
    void Clear(size_t index) {
        ...
        if (_bits[index >> 6] & mask) {
            ...
            if (numSet != size_t(-1)) _numSet.Decrement();
            if (index == _firstSet.Load()) _firstSet.Store(-1);
            if (index == _lastSet.Load()) _lastSet.Store(-1);
    ...
    void Set(size_t index) {
        ...
        if (!(_bits[index >> 6] & mask)) {
            ...
            if (numSet != size_t(-1)) _numSet.Increment();
            if (index < _firstSet.Load()) _firstSet.Store(index);
            const size_t lastSet = _lastSet.Load();
            if (index > lastSet || lastSet == _num) _lastSet.Store(index);
    ...
    size_t FindNextSet(size_t index) const {
        ...
        if (_bits[index >> 6] & (UINT64_C(1) << startBit)) return index;
    ...
    size_t GetAllocatedSize() const {
        ...
        if (_numWords > 1) memUsed += _numWords << 3;
    ...
    /// Returns true if this != \p rhs.
    ///
    bool operator!=(const TfBits& rhs) const { return !(*this == rhs); }
    ...
    bool HasNonEmptyDifference(const TfBits& rhs) const {
        ...
        if (firstSet < _num) {
            ...
            // set is not empty).
            if (firstSet < rhsFirstSet || lastSet > rhsLastSet || firstSet > rhsLastSet || lastSet < rhsFirstSet ||
    ...
    ///
    bool Contains(const TfBits& rhs) const { return !rhs.HasNonEmptyDifference(*this); }
    ...
    public:
        ...
        public:
            ...
            const_iterator() : _bits(NULL), _index(0) {}
        ...
        private:
            ...
            // Ctor.
            const_iterator(const TfBits* bits, size_t index) : _bits(bits), _index(index) {}

            bool equal(const const_iterator& rhs) const { return _bits == rhs._bits && _index == rhs._index; }
            ...
            const size_t& dereference() const { return _index; }
        ...
        const_iterator end() const { return const_iterator(_bits, _bits->GetSize()); }
    ...
    private:
        ...
        // Ctor.
        View(const TfBits* bits) : _bits(bits) {}
    ...
    ///
    AllView GetAllView() const { return AllView(this); }

    /// Returns an iteratable view for the bits that steps over all set bits.
    ///
    AllSetView GetAllSetView() const { return AllSetView(this); }

    /// Returns an iteratable view for the bits that steps over all unset bits.
    ///
    AllUnsetView GetAllUnsetView() const { return AllUnsetView(this); }
...
private:
    ...
    uint64_t* _Alloc(size_t numWords) {
        ...
        if (numWords == 1) return &_inlineData;
...
private:
    ...
    public:
        _RelaxedAtomicSize_t() : _n{} {}

        explicit _RelaxedAtomicSize_t(size_t n) : _n{n} {}

        void Increment() { _n.fetch_add(1, std::memory_order_relaxed); }
...
template <>
struct Tf_ShouldIterateOverCopy<TfBits::AllView> : std::true_type {};

template <>
struct Tf_ShouldIterateOverCopy<TfBits::AllSetView> : std::true_type {};

template <>
struct Tf_ShouldIterateOverCopy<TfBits::AllUnsetView> : std::true_type {};
