...
public:
    ...
    static_assert(std::is_same_v<void, decltype(TfDelegatedCountIncrement(std::declval<RawPtrType>()))>);
    static_assert(std::is_same_v<void, decltype(TfDelegatedCountDecrement(std::declval<RawPtrType>()))>);

    using IncrementIsNoExcept =
            std::integral_constant<bool, noexcept(TfDelegatedCountIncrement(std::declval<RawPtrType>()))>;
    using DecrementIsNoExcept =
            std::integral_constant<bool, noexcept(TfDelegatedCountDecrement(std::declval<RawPtrType>()))>;
    using IncrementAndDecrementAreNoExcept =
            std::integral_constant<bool, IncrementIsNoExcept() && DecrementIsNoExcept()>;
    using DereferenceIsNoExcept = std::integral_constant<bool, noexcept(*std::declval<RawPtrType>())>;

private:
    template <typename ConvertibleType>
    using _IsPtrConvertible = std::is_convertible<std::add_pointer_t<ConvertibleType>, RawPtrType>;
...
public:
    ...
    TfDelegatedCountPtr(TfDelegatedCountDoNotIncrementTagType, RawPtrType rawPointer) noexcept : _pointer{rawPointer} {}
    ...
    TfDelegatedCountPtr(const TfDelegatedCountPtr& ptr) noexcept(IncrementIsNoExcept()) : _pointer{ptr.get()} {
        _IncrementIfValid();
    }
    ...
    explicit TfDelegatedCountPtr(
            ...
            std::enable_if_t<_IsPtrConvertible<OtherType>::value, int> = 0) noexcept(IncrementIsNoExcept())
        : _pointer(ptr.get()) {
        _IncrementIfValid();
    }
    ...
    TfDelegatedCountPtr(TfDelegatedCountPtr&& ptr) noexcept : _pointer(ptr.get()) { ptr._pointer = nullptr; }
    ...
    /// is not `nullptr`.
    template <typename OtherType>
    TfDelegatedCountPtr& operator=(const TfDelegatedCountPtr<OtherType>& ptr) noexcept(
    ...
    ~TfDelegatedCountPtr() noexcept(DecrementIsNoExcept::value) { _DecrementIfValid(); }

    /// Dereference the underlying pointer.
    ReferenceType operator*() const noexcept(DereferenceIsNoExcept()) { return *get(); }

    /// Arrow operator dispatch for the underlying pointer.
    RawPtrType operator->() const noexcept { return get(); }
    ...
    void swap(TfDelegatedCountPtr& other) noexcept { std::swap(other._pointer, _pointer); }
...
TfDelegatedCountPtr<ValueType> TfMakeDelegatedCountPtr(Args&&... args) {
    return TfDelegatedCountPtr<ValueType>(TfDelegatedCountIncrementTag, new ValueType(std::forward<Args>(args)...));
}
