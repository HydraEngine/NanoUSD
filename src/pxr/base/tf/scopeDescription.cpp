...
struct _StackRegistry {
    ...
    StackLock LockThread(std::thread::id id) {
        _stacksMutex.lock();
        auto it = std::find_if(_stacks.begin(), _stacks.end(), [id](_StackEntry const& e) {
            return e.id == id;
        });
...
struct _Stack {
    ...
    _Stack() : head(nullptr) { GetRegistry().Add(std::this_thread::get_id(), this); }
...
static bool _TimedTryAcquire(tbb::spin_mutex::scoped_lock& lock, tbb::spin_mutex& mutex, int msecToTry = 10) {
    if (lock.try_acquire(mutex)) return true;
    ...
    do {
        ...
        if (lock.try_acquire(mutex)) return true;
        msec = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - start)
                       .count();
...
char const* _ComputeAndLockScopeDescriptionStackMsg() {
    ...
    std::sort(entries, entries + numEntries,
              [mainThreadId](_StackRegistry::_StackEntry* l, _StackRegistry::_StackEntry* r) {
                  ...
                  if (l->id == mainThreadId) return true;
                  if (r->id == mainThreadId) return false;
                  return l->id < r->id;
              });
    ...
    for (size_t i = 0; i != numEntries; ++i) {
        ...
        while (curScope) {
            ...
            if (TfCallContext const& ctx = Tf_GetScopeDescriptionContext(curScope)) {
                ...
                writer.Write(" in ");
                writer.Write(ctx.GetFile());
                writer.Write("#");
                writer.Write(ctx.GetLine());
                writer.Write(")");
            }
            writer.Write("\n");
...
TfScopeDescription::TfScopeDescription(char const* msg, TfCallContext const& ctx) : _description(msg), _context(ctx) {
    _Push();
}
