...
struct object_from_python {
    object_from_python() {
        pxr_boost::python::converter::registry::push_back(&TfromPy::convertible, &TfromPy::construct,
...
struct python_optional {
    ...
    struct optional_from_python {
        static void* convertible(PyObject* source) {
            ...
            if ((source == Py_None) || pxr_boost::python::extract<T>(source).check()) return source;
    ...
    explicit python_optional() {
        register_python_conversion<std::optional<T>, optional_to_python<std::optional<T>>,
