...
#define VTOPERATOR_WRAP_PYTYPE_BASE(op, method, pytype, isRightVer)                                \
    template <typename T>                                                                          \
    static VtArray<T> method##pytype(VtArray<T> vec, pytype obj) {                                 \
        size_t length = len(obj);                                                                  \
        if (length != vec.size()) {                                                                \
            TfPyThrowValueError("Non-conforming inputs for operator " #method);                    \
            return VtArray<T>();                                                                   \
        }                                                                                          \
        VtArray<T> ret(vec.size());                                                                \
        for (size_t i = 0; i < length; ++i) {                                                      \
            if (!extract<T>(obj[i]).check()) TfPyThrowValueError("Element is of incorrect type."); \
            if (isRightVer) {                                                                      \
                ret[i] = _ArrayPyOpHelp<T>::op((T)extract<T>(obj[i]), vec[i]);                     \
            } else {                                                                               \
                ret[i] = _ArrayPyOpHelp<T>::op(vec[i], (T)extract<T>(obj[i]));                     \
            }                                                                                      \
        }                                                                                          \
        return ret;                                                                                \
    }
...
#define VTOPERATOR_WRAP_PYTYPE_BOOL(func, pytype, op)                                \
    VTOPERATOR_WRAP_PYTYPE_BOOL_BASE(func, VtArray<T> const& vec, pytype const& obj, \
                                     (vec[i] op(T) extract<T>(obj[i])))              \
    VTOPERATOR_WRAP_PYTYPE_BOOL_BASE(func, pytype const& obj, VtArray<T> const& vec, ((T)extract<T>(obj[i]) op vec[i]))
