...
#define VT_INTEGRAL_BUILTIN_VALUE_TYPES                                                                         \
    ((bool, Bool))((char, Char))((unsigned char, UChar))((short, Short))((unsigned short, UShort))((int, Int))( \
            (unsigned int, UInt))((int64_t, Int64))((uint64_t, UInt64))

#define VT_VEC_INT_VALUE_TYPES ((GfVec4i, Vec4i))((GfVec3i, Vec3i))((GfVec2i, Vec2i))

#define VT_VEC_HALF_VALUE_TYPES ((GfVec4h, Vec4h))((GfVec3h, Vec3h))((GfVec2h, Vec2h))

#define VT_VEC_FLOAT_VALUE_TYPES ((GfVec4f, Vec4f))((GfVec3f, Vec3f))((GfVec2f, Vec2f))
...
#define VT_GFRANGE_VALUE_TYPES                                                                                      \
    ((GfRange3f, Range3f))((GfRange3d, Range3d))((GfRange2f, Range2f))((GfRange2d, Range2d))((GfRange1f, Range1f))( \
            (GfRange1d, Range1d))
...
#define VT_STRING_VALUE_TYPES ((std::string, String))((TfToken, Token))

#define VT_QUATERNION_VALUE_TYPES ((GfQuath, Quath))((GfQuatf, Quatf))((GfQuatd, Quatd))((GfQuaternion, Quaternion))

#define VT_DUALQUATERNION_VALUE_TYPES ((GfDualQuath, DualQuath))((GfDualQuatf, DualQuatf))((GfDualQuatd, DualQuatd))

#define VT_NONARRAY_VALUE_TYPES ((GfFrustum, Frustum))((GfMultiInterval, MultiInterval))
...
#define VT_ARRAY_TYPEDEF(unused, elem) \
    typedef VtArray<VT_TYPE(elem)> TF_PP_CAT(Vt, TF_PP_CAT(VT_TYPE_NAME(elem), Array));
...
#define VT_ARRAY_TYPE_TUPLE(unused, elem) \
    ((TF_PP_CAT(Vt, TF_PP_CAT(VT_TYPE_NAME(elem), Array)), TF_PP_CAT(VT_TYPE_NAME(elem), Array)))
...
using Vt_ValueTypeList =
        TfMetaApply<TfMetaTail, TfMetaList<void TF_PP_SEQ_FOR_EACH(_VT_MAP_TYPE_LIST, ~, VT_VALUE_TYPES)>>;
...
constexpr int GetIndexImpl(Typelist) {
    if (std::is_same_v<T, TfMetaApply<TfMetaHead, Typelist>>) {
        return 0;
    } else if (const int indexOfTail = GetIndexImpl<T>(TfMetaApply<TfMetaTail, Typelist>{}); indexOfTail >= 0) {
...
#define VT_SPECIALIZE_IS_VALUE_PROXY(unused, elem)                  \
    template <>                                                     \
    struct VtIsValueProxy<VT_TYPE(elem)> : std::false_type {};      \
    template <>                                                     \
    struct VtIsTypedValueProxy<VT_TYPE(elem)> : std::false_type {}; \
    template <>                                                     \
    struct VtIsErasedValueProxy<VT_TYPE(elem)> : std::false_type {};
...
struct Vt_ShapeData {
    unsigned int GetRank() const { return otherDims[0] == 0 ? 1 : otherDims[1] == 0 ? 2 : otherDims[2] == 0 ? 3 : 4; }
    bool operator==(Vt_ShapeData const& other) const {
        ...
        return std::equal(otherDims, otherDims + GetRank() - 1, other.otherDims);
    }
    bool operator!=(Vt_ShapeData const& other) const { return !(*this == other); }
