...
void _StreamArrayRecursive(std::ostream& out,
    ...
    if (dimension == shape.GetRank() - 1) {
        ...
        for (size_t j = 0; j < shape.otherDims[dimension]; ++j) {
            ...
            _StreamArrayRecursive(out, shape, streamNextElem, lastDimSize, dimension + 1);
        }
    }
    out << ']';
}
...
void VtStreamOutArray(std::ostream& out,
                      const Vt_ShapeData* shapeData,
                      TfFunctionRef<void(std::ostream&)> streamNextElem) {
    // Compute last dim size, and if size is not evenly divisible, output as
    // rank-1.
    size_t divisor = std::accumulate(shapeData->otherDims, shapeData->otherDims + shapeData->GetRank() - 1, 1,
                                     [](size_t x, size_t y) {
                                         return x * y;
                                     });
