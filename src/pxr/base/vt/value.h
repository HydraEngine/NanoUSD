...
struct Vt_DefaultValueHolder {
    ...
    std::type_info const& GetType() const { return *_type; }
    ...
    void const* GetPointer() const { return _ptr.Get(); }

private:
    Vt_DefaultValueHolder(TfAnyUniquePtr&& ptr, std::type_info const& type) : _ptr(std::move(ptr)), _type(&type) {}
...
class VtValue {
    ...
    template <class T>
    struct _Counted {
        explicit _Counted(T const& obj) : _obj(obj) { _refCount = 0; }
    ...
    template <class T>
    using _UsesLocalStore =
            std::integral_constant<bool,
                                   (sizeof(T) <= sizeof(_Storage)) && VtValueTypeHasCheapCopy<T>::value &&
    ...
    private:
        ...
        using _GetElementTypeidFunc = std::type_info const& (*)(_Storage const&);
        ...
        using _GetProxiedTypeidFunc = std::type_info const& (*)(_Storage const&);
    ...
    public:
        void CopyInit(_Storage const& src, _Storage& dst) const { _copyInit(src, dst); }
        void Destroy(_Storage& storage) const { _destroy(storage); }
        void Move(_Storage& src, _Storage& dst) const noexcept { _move(src, dst); }
        bool CanHash(_Storage const& storage) const { return _canHash(storage); }
        size_t Hash(_Storage const& storage) const { return _hash(storage); }
        bool Equal(_Storage const& lhs, _Storage const& rhs) const { return _equal(lhs, rhs); }
        bool EqualPtr(_Storage const& lhs, void const* rhs) const { return _equalPtr(lhs, rhs); }
        void MakeMutable(_Storage& storage) const { _makeMutable(storage); }
        TfPyObjWrapper GetPyObj(_Storage const& storage) const { return _getPyObj(storage); }
        std::ostream& StreamOut(_Storage const& storage, std::ostream& out) const { return _streamOut(storage, out); }
        bool IsArrayValued(_Storage const& storage) const { return _isArrayValued(storage); }
        std::type_info const& GetElementTypeid(_Storage const& storage) const { return _getElementTypeid(storage); }
        std::type_info const& GetTypeid(_Storage const& storage) const { return _getTypeid(storage); }
        const Vt_ShapeData* GetShapeData(_Storage const& storage) const { return _getShapeData(storage); }
        size_t GetNumElements(_Storage const& storage) const { return _getNumElements(storage); }
        bool ProxyHoldsType(_Storage const& storage, std::type_info const& t) const {
            return _proxyHoldsType(storage, t);
        }
        TfType GetProxiedType(_Storage const& storage) const { return _getProxiedType(storage); }
        std::type_info const& GetProxiedTypeid(_Storage const& storage) const { return _getProxiedTypeid(storage); }
        VtValue GetProxiedAsVtValue(_Storage const& storage) const { return _getProxiedAsVtValue(storage); }
        void const* GetProxiedObjPtr(_Storage const& storage) const { return _getProxiedObjPtr(storage); }
    ...
    struct _ArrayHelper<Array, typename std::enable_if<VtIsArray<Array>::value>::type> {
        static const Vt_ShapeData* GetShapeData(Array const& obj) { return obj._GetShapeData(); }
        static size_t GetNumElements(Array const& obj) { return obj.size(); }
        constexpr static std::type_info const& GetElementTypeid() { return typeid(typename Array::ElementType); }
    };

    // Function used in case T has equality comparison.
    template <class T>
    static inline auto _TypedProxyEqualityImpl(T const& a, T const& b, int) -> decltype(a == b) {
        return a == b;
    }
    // Function used in case T does not have equality comparison.
    template <class NoEqual>
    static inline bool _TypedProxyEqualityImpl(NoEqual const& a, NoEqual const& b, long) {
        return VtGetProxiedObject(a) == VtGetProxiedObject(b);
    }

    template <class T>
    static inline auto _ErasedProxyEqualityImpl(T const& a, T const& b, int) -> decltype(a == b) {
        return a == b;
    }
    // Function used in case T does not have equality comparison.
    template <class NoEqual>
    static inline bool _ErasedProxyEqualityImpl(NoEqual const& a, NoEqual const& b, long) {
    ...
    struct _ProxyHelper {
        ...
        static size_t Hash(T const& obj) { return VtHashValue(VtGetProxiedObject(obj)); }
        ...
        static Vt_ShapeData const* GetShapeData(T const& obj) {
            return _ArrayHelper<ProxiedType>::GetShapeData(VtGetProxiedObject(obj));
        }
        static size_t GetNumElements(T const& obj) {
            return _ArrayHelper<ProxiedType>::GetNumElements(VtGetProxiedObject(obj));
        }
        static bool IsArrayValued(T const&) { return VtIsArray<ProxiedType>::value; }
        static std::type_info const& GetTypeid(T const&) { return typeid(ProxiedType); }
        static std::type_info const& GetElementTypeid(T const&) {
            return _ArrayHelper<ProxiedType>::GetElementTypeid();
        }
        static VtValue GetProxiedAsVtValue(T const& obj) { return VtValue(VtGetProxiedObject(obj)); }
        static bool HoldsType(T const& tp, std::type_info const& query) {
            return TfSafeTypeCompare(typeid(ProxiedType), query);
        }
        static TfType GetTfType(T const& tp) { return TfType::Find<ProxiedType>(); }
        static void const* GetObjPtr(T const& tp) { return static_cast<void const*>(&VtGetProxiedObject(tp)); }
    };
    ...
    struct _ProxyHelper<ErasedProxy, typename std::enable_if<VtIsErasedValueProxy<ErasedProxy>::value>::type> {
        static bool CanHash(ErasedProxy const& proxy) { return VtGetErasedProxiedVtValue(proxy)->CanHash(); }
        static size_t Hash(ErasedProxy const& proxy) { return VtGetErasedProxiedVtValue(proxy)->GetHash(); }
        ...
        static std::ostream& StreamOut(ErasedProxy const& obj, std::ostream& out) { return VtStreamOut(obj, out); }
        ...
        static bool IsArrayValued(ErasedProxy const& obj) { return VtGetErasedProxiedVtValue(obj)->IsArrayValued(); }
        ...
        static VtValue GetProxiedAsVtValue(ErasedProxy const& ep) { return *VtGetErasedProxiedVtValue(ep); }
        static bool HoldsType(ErasedProxy const& ep, std::type_info const& query) {
            return VtErasedProxyHoldsType(ep, query);
        }
        static TfType GetTfType(ErasedProxy const& ep) { return VtGetErasedProxiedTfType(ep); }
    ...
    struct _TypeInfoImpl : public _TypeInfo {
        ...
        static T const& GetObj(_Storage const& storage) { return Derived::_GetObj(_Container(storage)); }

        static T& GetMutableObj(_Storage& storage) { return Derived::_GetMutableObj(_Container(storage)); }

        static void CopyInitObj(T const& objSrc, _Storage& dst) { Derived::_PlaceCopy(&_Container(dst), objSrc); }
    ...
    private:
        ...
        static void _CopyInit(_Storage const& src, _Storage& dst) { new (&_Container(dst)) Container(_Container(src)); }

        static void _Destroy(_Storage& storage) { _Container(storage).~Container(); }

        static bool _CanHash(_Storage const& storage) { return ProxyHelper::CanHash(GetObj(storage)); }

        static size_t _Hash(_Storage const& storage) { return ProxyHelper::Hash(GetObj(storage)); }
        ...
        static bool _EqualPtr(_Storage const& lhs, void const* rhs) {
            ...
            // That is, we only ever ask a proxy to compare to a proxy; we never
            // ask a proxy to compare to the proxied object.
            return ProxyHelper::Equal(GetObj(lhs), *static_cast<T const*>(rhs));
        }
        ...
        static void _MakeMutable(_Storage& storage) { GetMutableObj(storage); }

        static TfPyObjWrapper _GetPyObj(_Storage const& storage) { return ProxyHelper::GetPyObj(GetObj(storage)); }
        ...
        static bool _IsArrayValued(_Storage const& storage) { return ProxyHelper::IsArrayValued(GetObj(storage)); }

        static std::type_info const& _GetElementTypeid(_Storage const& storage) {
        ...
        static size_t _GetNumElements(_Storage const& storage) { return ProxyHelper::GetNumElements(GetObj(storage)); }

        static bool _ProxyHoldsType(_Storage const& storage, std::type_info const& t) {
            return ProxyHelper::HoldsType(GetObj(storage), t);
        }

        static TfType _GetProxiedType(_Storage const& storage) { return ProxyHelper::GetTfType(GetObj(storage)); }

        static std::type_info const& _GetProxiedTypeid(_Storage const& storage) {
            return ProxyHelper::GetTypeid(GetObj(storage));
        }

        static void const* _GetProxiedObjPtr(_Storage const& storage) {
            return ProxyHelper::GetObjPtr(GetObj(storage));
        }

        static VtValue _GetProxiedAsVtValue(_Storage const& storage) {
    ...
    template <class T>
    struct _LocalTypeInfo : _TypeInfoImpl<T,                 // type
                                          T,                 // container
    ...
    struct _RemoteTypeInfo : _TypeInfoImpl<T,                                 // type
                                           ...
                                           > {
        constexpr _RemoteTypeInfo() : _TypeInfoImpl<T, TfDelegatedCountPtr<_Counted<T>>, _RemoteTypeInfo<T>>() {}
    ...
    static TfPointerAndBits<const _TypeInfo> GetTypeInfo() {
        ...
        static const TI ti;
        static constexpr unsigned int flags = (TI::IsLocal ? _LocalFlag : 0) |
    ...
    struct _HoldAside {
        explicit _HoldAside(VtValue* val)
            : info((val->IsEmpty() || val->_IsLocalAndTriviallyCopyable()) ? static_cast<_TypeInfo const*>(NULL)
                                                                           : val->_info.Get()) {
            if (info) info->Move(val->_storage, storage);
        }
        ~_HoldAside() {
            if (info) info->Destroy(storage);
        }
        _Storage storage;
...
public:
    ...
    VtValue(VtValue const& other) { _Copy(other, *this); }

    /// Move construct with \p other.
    VtValue(VtValue&& other) noexcept { _Move(other, *this); }
    ...
    VtValue& operator=(VtValue const& other) {
        if (ARCH_LIKELY(this != &other)) _Copy(other, *this);
        return *this;
    }

    /// Move assignment from another \a VtValue.
    VtValue& operator=(VtValue&& other) noexcept {
        if (ARCH_LIKELY(this != &other)) _Move(other, *this);
        return *this;
    }
...
#else
    template <class T>
    std::enable_if_t<!_TypeInfoFor<T>::Type::IsLocal || !_TypeInfoFor<T>::Type::HasTrivialCopy, VtValue&> operator=(
            T const& obj) {
        _HoldAside tmp(this);
        _Init(obj);
        return *this;
    ...
    VtValue& operator=(char* cstr) { return *this = const_cast<char const*>(cstr); }
    ...
    int GetKnownValueTypeIndex() const {
        if (ARCH_UNLIKELY(_IsProxy())) {
            return _info->GetProxiedAsVtValue(_storage).GetKnownValueTypeIndex();
        }
    ...
    ///
    /// \sa \ref VtValue_Casting
    VtValue& CastToTypeOf(VtValue const& other) { return CastToTypeid(other.GetTypeid()); }
    ...
    ///
    /// \sa \ref VtValue_Casting
    bool CanCastToTypeOf(VtValue const& other) const { return _CanCast(GetTypeid(), other.GetTypeid()); }

    /// Return if \c this can be cast to \a type.
    ///
    /// \sa \ref VtValue_Casting
    bool CanCastToTypeid(std::type_info const& type) const { return _CanCast(GetTypeid(), type); }
    ...
    friend inline size_t hash_value(VtValue const& val) { return val.GetHash(); }
...
private:
    ...
    void const* _GetProxiedObjPtr() const { return _info->GetProxiedObjPtr(_storage); }
    ...
    inline void _Clear() {
        ...
        if (_info.GetLiteral() && !_IsLocalAndTriviallyCopyable()) _info.Get()->Destroy(_storage);
        ARCH_PRAGMA_POP
        _info.Set(nullptr, 0);
    }
    ...
    inline bool _IsLocalAndTriviallyCopyable() const {
        ...
        return (bits & (_LocalFlag | _TrivialCopyFlag)) == (_LocalFlag | _TrivialCopyFlag);
    }

    inline bool _IsProxy() const { return _info.BitsAs<unsigned int>() & _ProxyFlag; }
...
struct Vt_ValueShapeDataAccess {
    static const Vt_ShapeData* _GetShapeData(const VtValue& value) { return value._GetShapeData(); }

    static size_t _GetNumElements(const VtValue& value) { return value._GetNumElements(); }
};
...
template <class T>
inline Vt_DefaultValueHolder Vt_DefaultValueFactory<T>::Invoke() {
...
template <>
inline VtValue VtValue::Get<VtValue>() && {
...
template <>
inline VtValue VtValue::UncheckedGet<VtValue>() && {
    return std::move(*this);
}
