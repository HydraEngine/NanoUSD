...
struct Vt_GetSubElementType<
        T,
        typename std::enable_if<GfIsGfVec<T>::value || GfIsGfMatrix<T>::value || GfIsGfQuat<T>::value ||
                                GfIsGfDualQuat<T>::value || GfIsGfRange<T>::value>::type> {
...
Vt_ConvertFn<Dst> Vt_GetConvertFn(char srcFmt) {
    switch (srcFmt) {
        ...
        case 'b':
            return Vt_ConvertSingle<char, Dst>;
        case 'B':
            return Vt_ConvertSingle<unsigned char, Dst>;
        case 'h':
            return Vt_ConvertSingle<short, Dst>;
        case 'H':
            return Vt_ConvertSingle<unsigned short, Dst>;
        case 'i':
            return Vt_ConvertSingle<int, Dst>;
        case 'I':
            return Vt_ConvertSingle<unsigned int, Dst>;
        case 'l':
            return Vt_ConvertSingle<long, Dst>;
        case 'L':
            return Vt_ConvertSingle<unsigned long, Dst>;
        case 'q':
            return Vt_ConvertSingle<long long, Dst>;
        case 'Q':
            return Vt_ConvertSingle<unsigned long long, Dst>;
        case 'e':
            return Vt_ConvertSingle<GfHalf, Dst>;
        case 'f':
            return Vt_ConvertSingle<float, Dst>;
        case 'd':
            return Vt_ConvertSingle<double, Dst>;
    }
    return nullptr;
}
...
template <class T>
constexpr typename std::enable_if<GfIsGfDualQuat<T>::value, Vt_PyShape<2>>::type Vt_GetElementShapeImpl(T*) {
    return {2, 4};
}

template <class T>
constexpr typename std::enable_if<GfIsGfRange<T>::value && T::dimension == 1, Vt_PyShape<1>>::type
Vt_GetElementShapeImpl(T*) {
    return {2};
}

template <class T>
constexpr typename std::enable_if<GfIsGfRange<T>::value && T::dimension != 1, Vt_PyShape<2>>::type
Vt_GetElementShapeImpl(T*) {
    return {2, T::dimension};
}

constexpr Vt_PyShape<2> Vt_GetElementShapeImpl(GfRect2i*) {
    return {2, 2};
}

template <class T>
constexpr auto Vt_GetElementShape() -> decltype(Vt_GetElementShapeImpl(static_cast<T*>(nullptr))) {
...
// protocol.
template <class T>
bool Vt_ArrayFromBuffer(TfPyObjWrapper const& obj, VtArray<T>* out, string* errPtr = nullptr) {
    string localErr;
    ...
    auto multiply = [](Py_ssize_t x, Py_ssize_t y) {
        return x * y;
    };
    auto numItems = std::accumulate(view.shape, view.shape + view.ndim, (Py_ssize_t)1, multiply);

    // Compute the total # of items in one element.
    auto elemShape = Vt_GetElementShape<T>();
    auto elemSize = std::accumulate(elemShape.begin(), elemShape.end(), (Py_ssize_t)1, multiply);
    ...
    if (numItems % elemSize) {
        err = TfStringPrintf("Buffer size (%s items) must be a multiple of %s", TfStringify(numItems).c_str(),
        ...
        if (out) {
            ...
            auto increment = [&view](Py_ssize_t* index) {
                ...
                for (int i = 0; i != view.ndim; ++i) {
                    if (++index[ndim - i - 1] < view.shape[ndim - i - 1]) return;
                    index[ndim - i - 1] = 0;
                }
            };
            ...
            auto getPtrAt = [&view](Py_ssize_t* index) {
                ...
                while (i--) result += index[i] * view.strides[i];
                return result;
            };
...
static VtValue Vt_CastPyObjToArray(VtValue const& v) {
    VtValue ret;
    TfPyObjWrapper obj;
    if (v.IsHolding<TfPyObjWrapper>()) obj = v.UncheckedGet<TfPyObjWrapper>();
...
template <class T>
std::optional<VtArray<T>> VtArrayFromPyBuffer(TfPyObjWrapper const& obj, std::string* err) {
    VtArray<T> array;
    std::optional<VtArray<T>> result;
    if (Vt_ArrayFromBuffer(obj, &array, err)) result = array;
    return result;
}

#define INSTANTIATE(unused, elem)                                                                                \
    template std::optional<VtArray<VT_TYPE(elem)>> VtArrayFromPyBuffer<VT_TYPE(elem)>(TfPyObjWrapper const& obj, \
                                                                                      string* err);
...
#define VT_ADD_BUFFER_PROTOCOL(unused, elem)                                                             \
    ...
    pxr_boost::python::def(TF_PP_STRINGIZE(VT_TYPE_NAME(elem)) "ArrayFromBuffer",                        \
