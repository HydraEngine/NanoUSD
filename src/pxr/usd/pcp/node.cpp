...
bool PcpNodeRef::operator<(const PcpNodeRef& rhs) const {
    if (_nodeIdx < rhs._nodeIdx) return true;
    if (_nodeIdx > rhs._nodeIdx) return false;
...
PcpNodeRef PcpNodeRef::GetOriginRootNode() const {
    ...
    while (root.GetOriginNode() && root.GetOriginNode() != root.GetParentNode()) root = root.GetOriginNode();
...
#define PCP_DEFINE_GET_NODE_API(typeName, getter, varName)                                            \
    typeName PcpNodeRef::getter() const {                                                             \
        const PcpPrimIndex_Graph::_Node& graphNode = _graph->_GetNode(_nodeIdx);                      \
        if (graphNode.varName == PcpPrimIndex_Graph::_Node::_invalidNodeIndex) return (typeName) - 1; \
        return graphNode.varName;                                                                     \
    }
...
void PcpNodeRef::SetCulled(bool culled) {
    ...
    _RecordRestrictionDepth(culled ? _Restricted::Yes : _Restricted::Unknown);

    _graph->_unshared[_nodeIdx].culled = culled;
}

void PcpNodeRef::SetRestricted(bool restricted) {
    ...
    if (restricted != wasRestricted) {
        ...
        _RecordRestrictionDepth(restricted ? _Restricted::Yes : _Restricted::Unknown);
    }
}

void PcpNodeRef::SetInert(bool inert) {
    ...
    if (inert != wasInert) {
        ...
        // may be restricting contributions, so we don't know.
        _RecordRestrictionDepth(inert ? _Restricted::Yes : _Restricted::Unknown);
    }
}

void PcpNodeRef::_RecordRestrictionDepth(_Restricted isRestricted) {
    ...
    const bool contributionRestricted = isRestricted == _Restricted::Yes || !CanContributeSpecs();
    ...
    if (!contributionRestricted) {
        ...
        if (auto maxDepth = std::numeric_limits<std::decay_t<decltype(currDepth)>>::max(); newDepth > maxDepth) {
...
size_t PcpNodeRef::GetSpecContributionRestrictedDepth() const {
    return _graph->_unshared[_nodeIdx].restrictionDepth;
}

void PcpNodeRef::SetSpecContributionRestrictedDepth(size_t depth) {
...
static SdfPath _GetPathAtIntroDepth(const SdfPath& path, int depthBelowIntro) {
...
SdfPath PcpNodeRef::GetIntroPath() const {
    ...
    const PcpNodeRef parent = GetParentNode();
    if (!parent) return SdfPath::AbsoluteRootPath();

    return _GetPathAtIntroDepth(parent.GetPath(), GetDepthBelowIntroduction());
}

PCP_API
SdfPath PcpNodeRef::GetPathAtOriginRootIntroduction() const {
    return _GetPathAtIntroDepth(GetPath(), GetOriginRootNode().GetDepthBelowIntroduction());
}
...
PcpNodeRef::child_const_reverse_range PcpNodeRef::GetChildrenReverseRange() const {
    PcpNodeRef node(_graph, _nodeIdx);
    return child_const_reverse_range(child_const_reverse_iterator(node, /* end = */ false),
...
PcpNodeRef_ChildrenIterator::PcpNodeRef_ChildrenIterator() : _index(PcpPrimIndex_Graph::_Node::_invalidNodeIndex) {
    // Do nothing
}
...
std::ostream& operator<<(std::ostream& out, const PcpNodeRef& node) {
    out << "(" << node._GetNodeIndex() << ") " << TfEnum::GetDisplayName(node.GetArcType()) << " " << node.GetSite();
    return out;
}
