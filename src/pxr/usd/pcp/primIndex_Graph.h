...
private:
    ...
    const _Node& _GetNode(const PcpNodeRef& node) const { return _GetNode(node._GetNodeIndex()); }
...
private:
    ...
    struct _UnsharedData {
        ...
        explicit _UnsharedData(SdfPath&& p)
            : sitePath(std::move(p)), restrictionDepth(0), hasSpecs(false), culled(false), isDueToAncestor(false) {}
