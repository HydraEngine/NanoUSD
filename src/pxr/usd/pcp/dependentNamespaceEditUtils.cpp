...
struct _DebugScope {
    ...
    static void PrintDebug(const std::string& msg) {
        ...
        const std::string formattedMsg = indent + TfStringReplace(TfStringTrim(msg), "\n", "\n" + indent) + "\n";
...
static PcpNodeRef _GetUnpropagatedSpecializesNode(const PcpNodeRef& node) {
    ...
    if (originNode == node.GetParentNode() || originNode.GetSite() != node.GetSite()) {
        return node;
    }
...
// introduced).
static PcpNodeRef _GetPropagatedSpecializesNode(const PcpNodeRef& node) {
    if (!TF_VERIFY(node.GetArcType() == PcpArcTypeSpecialize)) {
        return PcpNodeRef();
    }
...
struct _NodeIntroductionInfo {
    ...
    _NodeIntroductionInfo(const PcpNodeRef& node_) : node(node_) {
        // Introducing info is populated from the unpropagated node which, for
        // all cases but specializes, is the node itself
        auto populateIntroducingInfo = [this](const PcpNodeRef& unpropagatedNode) {
            ...
            if (introducingNode && introducingNode.GetArcType() == PcpArcTypeSpecialize) {
                introducingNode = _GetPropagatedSpecializesNode(introducingNode);
            }
...
using _LayerSpecMovesScratch = std::unordered_map<SdfLayerHandle, _SpecMovesScratch, TfHash>;
...
public:
    _PrimIndexDependentNodeEditProcessor(const PcpPrimIndex* primIndex,
                                         PcpDependentNamespaceEdits* edits,
                                         _LayerSpecMovesScratch* layerSpecMovesScratch)
        : _primIndex(primIndex), _edits(edits), _layerSpecMovesScratch(layerSpecMovesScratch) {}

    // Adds a task for processing the spec move edit at the given node.
    void AddProcessEditsAtNodeTask(const PcpNodeRef& node,
...
private:
    ...
    friend std::ostream& operator<<(std::ostream& out, const _NodeTask& nodeTask) {
        out << nodeTask.node << "\n    move <" << nodeTask.oldPath << "> to <" << nodeTask.newPath << ">";
    ...
    void _ProcessNextImpliedClass(const _NodeTask& nodeTask);

    void _ProcessDependentNodePathAtIntroductionChange(const _NodeIntroductionInfo& nodeIntroInfo,
                                                       const SdfPath& oldPath,
                                                       const SdfPath& newPath);

    void _AddSpecMoveEdits(const _NodeTask& nodeTask);

    bool _HasUneditedUpstreamSpecConflicts(const PcpNodeRef& siteEditNode, const SdfPath& siteEditPath);

    bool _HasConflictingSpecsInUneditedNodes(const PcpNodeRef& siteEditNode, PcpNodeRef* firstConflictingNode);

    bool _HasConflictingChildSpecsInUneditedNodes(const PcpNodeRef& siteEditNode,
...
void _PrimIndexDependentNodeEditProcessor::_InsertNodeTask(_NodeTask&& nodeTask) {
    _PRINT_DEBUG("Inserting node task: %s", TfStringify(nodeTask).c_str());
    ...
    if (_nodeTasks.empty()) {
        ...
        auto insertIt =
                std::lower_bound(_nodeTasks.begin(), _nodeTasks.end(), nodeTask, [](const auto& lhs, const auto& rhs) {
...
static void _ProcessListOpCompositionFieldEditsAtSite(
    ...
    for (const auto& layer : siteLayerStack->GetLayers()) {
        ...
        if (listOp.ModifyOperations(modifyCallback)) {
            _PRINT_DEBUG(
                    ...
                    "  new %s value = %s",
                    fieldName.GetText(), layer->GetIdentifier().c_str(), sitePath.GetText(), fieldName.GetText(),
                    TfStringify(listOp).c_str());

            edits->push_back({layer, sitePath, fieldName, VtValue::Take(listOp)});
        }
    }
}
...
static void _ProcessRelocatesFieldEditsAtSite(const PcpLayerStackRefPtr& siteLayerStack,
                                              const SdfPath& oldPath,
                                              const SdfPath& newPath,
    ...
    for (const auto& layer : siteLayerStack->GetLayers()) {
        ...
        if (auto it = edits->find(layer); it != edits->end()) {
            if (Pcp_ModifyRelocates(&it->second, oldPath, newPath)) {
                _PRINT_DEBUG_SCOPE("Updating layer metadata relocates for layer @%s@ to:",
...
// introduction.
void _PrimIndexDependentNodeEditProcessor::_ProcessDependentNodePathAtIntroductionChange(
        const _NodeIntroductionInfo& nodeIntroInfo, const SdfPath& oldPath, const SdfPath& newPath) {
    ...
    const auto& introducingLayerStack = nodeIntroInfo.introducingNode.GetLayerStack();
    switch (nodeIntroInfo.node.GetArcType()) {
        case PcpArcTypeReference:
            _ProcessListOpCompositionFieldEditsAtSite<SdfReference>(
                    introducingLayerStack, nodeIntroInfo.introducingPath, SdfFieldKeys->References, oldPath, newPath,
                    &_edits->compositionFieldEdits);
            break;
        case PcpArcTypePayload:
            _ProcessListOpCompositionFieldEditsAtSite<SdfPayload>(introducingLayerStack, nodeIntroInfo.introducingPath,
                                                                  SdfFieldKeys->Payload, oldPath, newPath,
                                                                  &_edits->compositionFieldEdits);
            break;
        case PcpArcTypeInherit:
            _ProcessListOpCompositionFieldEditsAtSite<SdfPath>(introducingLayerStack, nodeIntroInfo.introducingPath,
        ...
        case PcpArcTypeRelocate:
            ...
            _ProcessRelocatesFieldEditsAtSite(introducingLayerStack, oldPath, newPath,
    ...
    _PRINT_DEBUG_SCOPE(
            "Deleted composition arc to <%s> introduced at path <%s> in "
            "node %s. Must remove specs in introducing node that compose "
            "over deleted child specs.",
            nodeIntroInfo.pathAtIntroduction.GetText(), nodeIntroInfo.introducingPath.GetText(),
            TfStringify(nodeIntroInfo.introducingNode).c_str());

    if (nodeIntroInfo.node.IsDueToAncestor()) {
        _PRINT_DEBUG(
                "Introduced node is ancestral; adding task to delete "
        ...
        // path)
        _InsertNodeTask({nodeIntroInfo.introducingNode, nodeIntroInfo.introducingNode.GetPath(), SdfPath()});
    } else {
        _PRINT_DEBUG(
                "Introduced node is direct; adding tasks to delete "
        ...
        _primIndex->ComputePrimChildNamesInSubtree(nodeIntroInfo.node, &children, &prohibitedChildren);
        // Add a task for deleting each corresponding child path in
        // the introducing node.
        for (const TfToken& childName : children) {
            _InsertNodeTask(
                    {nodeIntroInfo.introducingNode, nodeIntroInfo.introducingPath.AppendChild(childName), SdfPath()});
        }
    }
}
...
// edit task.
void _PrimIndexDependentNodeEditProcessor::_AddSpecMoveEdits(const _NodeTask& nodeTask) {
    ...
    if (!node.CanContributeSpecs()) {
        _PRINT_DEBUG("Skipping spec edits for %s node %s which cannot contribute specs",
                     TfStringify(node.GetArcType()).c_str(), TfStringify(node.GetSite()).c_str());
        return;
    }
    ...
    const bool isOptionalDelete = nodeTask.isImpliedClassTask && nodeTask.newPath.IsEmpty();

    // The old path may be an ancestor path of the node's path. If so the spec
    // we're moving for this node is the node path itself.
    const SdfPath& oldSpecPath = node.GetPath().HasPrefix(nodeTask.oldPath) ? node.GetPath() : nodeTask.oldPath;
    ...
    // them.
    if (!nodeTask.willBeRelocated && _HasUneditedUpstreamSpecConflicts(node, oldSpecPath)) {
        return;
    }

    // Map the old spec path to the new spec location.
    const SdfPath newSpecPath =
            isOptionalDelete ? SdfPath::EmptyPath() : oldSpecPath.ReplacePrefix(nodeTask.oldPath, nodeTask.newPath);
    ...
    SdfLayerHandleVector layersToEdit =
            PcpGatherLayersToEditForSpecMove(node.GetLayerStack(), oldSpecPath, newSpecPath, &_edits->errors);
    for (SdfLayerHandle& layer : layersToEdit) {
        // Print debug before adding as we're moving the layer.
        _PRINT_DEBUG("Added spec edit from <%s> to <%s> %s on layer @%s@", oldSpecPath.GetText(), newSpecPath.GetText(),
                     (isOptionalDelete ? "(edit is optional)" : ""), layer->GetIdentifier().c_str());

        // Add spec moves to the scratch space so that we can process optional
        // deletes when we finalize the edits.
        _SpecMovesScratch& specMovesScratch = (*_layerSpecMovesScratch)[std::move(layer)];
        if (isOptionalDelete) {
...
bool _PrimIndexDependentNodeEditProcessor::_HasUneditedUpstreamSpecConflicts(const PcpNodeRef& siteEditNode,
    ...
    if (siteEditNode.GetPath() != siteEditPath) {
        if (siteEditPath.GetParentPath() != siteEditNode.GetPath()) {
            TF_CODING_ERROR(
                    "Descendant site path <%s> is not a direct "
        ...
        if (!_HasConflictingChildSpecsInUneditedNodes(siteEditNode, siteEditPath.GetNameToken(),
                                                      &firstConflictingNode)) {
            return false;
        }
        // Otherwise, we just need to look for conflicts with the node path.
    } else if (!_HasConflictingSpecsInUneditedNodes(siteEditNode, &firstConflictingNode)) {
        return false;
    }
    ...
    std::string warning = TfStringPrintf(
            ...
            siteEditPath.GetText(), TfStringify(siteEditNode.GetSite()).c_str(),
...
// given node.
bool _PrimIndexDependentNodeEditProcessor::_HasConflictingSpecsInUneditedNodes(const PcpNodeRef& siteEditNode,
                                                                               PcpNodeRef* firstConflictingNode) {
    ...
    for (const auto& childNode : range) {
        ...
        for (const PcpNodeRef& subtreeNode : subtreeRange) {
            ...
            if (!_nodesVisitedByEditTasks.count(subtreeNode) && subtreeNode.HasSpecs() &&
...
bool _PrimIndexDependentNodeEditProcessor::_HasConflictingChildSpecsInUneditedNodes(const PcpNodeRef& siteEditNode,
                                                                                    const TfToken& childName,
                                                                                    PcpNodeRef* firstConflictingNode) {
    ...
    for (const auto& childNode : range) {
        ...
        for (const PcpNodeRef& subtreeNode : subtreeRange) {
            ...
            // contribute specs.
            if (_nodesVisitedByEditTasks.count(subtreeNode) || !subtreeNode.CanContributeSpecs()) {
                continue;
            }

            const PcpLayerStackRefPtr& layerStack = subtreeNode.GetLayerStack();

            // Map the child path into the subtree node.
            const SdfPath subtreeNodeChildPath = subtreeNode.GetPath().AppendChild(childName);
            ...
            if (subtreeNode.HasSpecs() && PcpComposeSiteHasPrimSpecs(layerStack, subtreeNodeChildPath)) {
                *firstConflictingNode = subtreeNode;
                return true;
            }
            ...
            if (layerStack->HasRelocates() &&
                layerStack->GetIncrementalRelocatesTargetToSource().count(subtreeNodeChildPath)) {
                *firstConflictingNode = subtreeNode;
                return true;
            }
        }
...
static SdfPath _MapNewPathToIntroducingNode(const _NodeIntroductionInfo& nodeIntroInfo, const SdfPath& newPath) {
    ...
    const SdfPath newPathInIntroducingNode =
            newPath.ReplacePrefix(nodeIntroInfo.pathAtIntroduction, nodeIntroInfo.introducingPath);

    // If the introducing node's layer has no relocates we're done.
    const PcpLayerStackRefPtr& introducingLayerStack = nodeIntroInfo.introducingNode.GetLayerStack();
    ...
    auto mapPath = [](const SdfRelocatesMap& reloMap, const SdfPath& path) {
        ...
        auto bestMatchIt = std::find_if(reloMap.rbegin(), reloMap.rend(), [&path](const auto& entry) {
            return path.HasPrefix(entry.first);
        });
        return bestMatchIt == reloMap.rend() ? path : path.ReplacePrefix(bestMatchIt->first, bestMatchIt->second);
    };
    const auto& sourceToTarget = introducingLayerStack->GetRelocatesSourceToTarget();
    const auto& targetToSource = introducingLayerStack->GetRelocatesTargetToSource();
    return mapPath(sourceToTarget, mapPath(targetToSource, newPathInIntroducingNode));
}

// Processes the next available node task. Returns true if a task was processed,
// false if there are no tasks to process.
bool _PrimIndexDependentNodeEditProcessor::_ProcessNextNodeTask() {
    ...
    _PRINT_DEBUG_SCOPE("Processing node task: %s", TfStringify(nodeTask).c_str());
    ...
    _PRINT_DEBUG(
            "Node was introduced as path <%s> by the path <%s> from "
            "introducing node %s",
            nodeIntroInfo.pathAtIntroduction.GetText(), nodeIntroInfo.introducingPath.GetText(),
            TfStringify(nodeIntroInfo.introducingNode).c_str());
    ...
    if (nodeIntroInfo.pathAtIntroduction.HasPrefix(oldPath)) {
        _PRINT_DEBUG("Spec move at <%s> affects this node's path at introduction <%s>", oldPath.GetText(),
                     nodeIntroInfo.pathAtIntroduction.GetText());
        _ProcessDependentNodePathAtIntroductionChange(nodeIntroInfo, oldPath, newPath);
    } else {
        ...
        SdfPath oldPathInIntroducingNode =
                oldPath.ReplacePrefix(nodeIntroInfo.pathAtIntroduction, nodeIntroInfo.introducingPath);

        // Determine the new path in the introducing node.
        SdfPath newPathInIntroducingNode = _MapNewPathToIntroducingNode(nodeIntroInfo, newPath);
        ...
        _PRINT_DEBUG(
                ...
                oldPath.GetText(), oldPathInIntroducingNode.GetText(), newPath.GetText(),
                newPathInIntroducingNode.GetText());

        _InsertNodeTask({nodeIntroInfo.introducingNode, oldPathInIntroducingNode, newPathInIntroducingNode});
    }
...
static PcpMapFunction _ComputeImpliedClassTransferFunction(const PcpNodeRef& sourceParent,
    ...
    PcpMapFunction transferFunction = PcpMapFunction::Create(
            {{sourceParent.GetPathAtIntroduction(), sourceParent.GetIntroPath()}}, SdfLayerOffset());
    ...
    // destination parent.
    for (PcpNodeRef transferNode = sourceParent.GetParentNode(); transferNode != destParent;
         transferNode = transferNode.GetParentNode()) {
        transferFunction = PcpMapFunction::Create({{transferNode.GetPathAtIntroduction(), transferNode.GetIntroPath()}},
                                                  SdfLayerOffset())
                                   .Compose(transferFunction);
    }
    ...
    return PcpMapExpression::Constant(transferFunction).AddRootIdentity().Evaluate();
}
...
// primIndex.cpp
static PcpNodeRef _GetImpliedClassTreeSourceParent(const PcpNodeRef originNode) {
    ...
    for (; !sourceParent.IsRootNode(); sourceParent = sourceParent.GetParentNode()) {
        ...
        if (originNode.GetArcType() == PcpArcTypeInherit && sourceParent.GetArcType() == PcpArcTypeSpecialize &&
            ...
            TF_WARN("Unable to fix specs for implied inherits for an inherit "
                    ...
                    TfStringify(originNode.GetSite()).c_str(), TfStringify(sourceParent.GetSite()).c_str());
            return PcpNodeRef();
        }
        ...
        if (originNode.GetDepthBelowIntroduction() < sourceParent.GetDepthBelowIntroduction()) {
            break;
        }
    }
...
// here instead.
static PcpNodeRef _GetClosestSharedAncestorNode(const PcpNodeRef& node1, const PcpNodeRef& node2) {
    if (node1 == node2) {
        return node1;
    }
    ...
    // ancestor.
    auto getSharedParent = [](const PcpNodeRef& strongerNode, const PcpNodeRef& weakerNode) {
        for (PcpNodeRef parent = weakerNode.GetParentNode(); parent; parent = parent.GetParentNode()) {
            if (parent < strongerNode) {
                return parent;
            }
        }
        TF_VERIFY(false, "Nodes %s and %s do not share an ancestor", TfStringify(strongerNode).c_str(),
                  TfStringify(weakerNode).c_str());
        return PcpNodeRef();
    };

    return node1 > node2 ? getSharedParent(node2, node1) : getSharedParent(node1, node2);
}
...
// specializes node.
static _ImpliedNodeAndTransferFunction _GetNextImpliedSpecializes(const PcpNodeRef& propagatedSpecializesOriginNode) {
    ...
    for (const auto& child : rootNode.GetChildrenReverseRange()) {
        ...
        const PcpNodeRef unpropagatedChild = _GetUnpropagatedSpecializesNode(child);
        const PcpNodeRef unpropagatedChildOrigin = unpropagatedChild.GetOriginNode();
        ...
        if (unpropagatedChildOrigin == unpropagatedSpecializesOriginNode) {
            ...
            const PcpNodeRef sourceParent = _GetImpliedClassTreeSourceParent(unpropagatedSpecializesOriginNode);
            const PcpNodeRef destParent = _GetClosestSharedAncestorNode(sourceParent, unpropagatedChild);
            // The implied node and origin nodes we return in this info need to
            // be the propagated versions of the specializes nodes.
            return {child, _ComputeImpliedClassTransferFunction(sourceParent, destParent)};
        }
    }
...
static PcpNodeRef _FindClassNodeWithOriginInSubtree(const PcpNodeRef& subtreeRoot, const PcpNodeRef& originNode) {
...
static _ImpliedNodeAndTransferFunction _GetNextImpliedInherit(const PcpNodeRef& originNode) {
    ...
    while (destParent) {
        ...
        for (const auto& child : range) {
            ...
            PcpNodeRef impliedNode = _FindClassNodeWithOriginInSubtree(child, curOriginNode);
            if (impliedNode) {
                ...
                return {impliedNode, _ComputeImpliedClassTransferFunction(sourceParent, destParent)};
            }
        }
...
void _PrimIndexDependentNodeEditProcessor::_ProcessNextImpliedClass(const _NodeTask& nodeTask) {
    const PcpNodeRef& node = nodeTask.node;
    const SdfPath& oldPath = nodeTask.oldPath;
    const SdfPath& newPath = nodeTask.newPath;
    ...
    const auto [impliedNode, transferFunction] =
            node.GetArcType() == PcpArcTypeSpecialize ? _GetNextImpliedSpecializes(node)
            : node.GetArcType() == PcpArcTypeInherit  ? _GetNextImpliedInherit(node)
    ...
    _PRINT_DEBUG_SCOPE(
            "Processing implied class "
            "\n  origin class node: %s"
            "\n  implied class node: %s"
            "\n  implied class transfer function: %s",
            TfStringify(node).c_str(), TfStringify(impliedNode).c_str(), transferFunction.GetString().c_str());
    ...
    // path arc's subtree.
    const SdfPath originPathAtClassIntroduction = node.GetPathAtOriginRootIntroduction();
    const SdfPath impliedPathAtClassIntroduction = impliedNode.GetPathAtOriginRootIntroduction();

    _PRINT_DEBUG(
            "Origin node path at class introduction: <%s>\n"
            "Implied node path at class introduction: <%s>",
            originPathAtClassIntroduction.GetText(), impliedPathAtClassIntroduction.GetText());
    ...
    if (originPathAtClassIntroduction.HasPrefix(oldPath)) {
        _PRINT_DEBUG(
                "Spec move at <%s> affects implied node's path at "
                "introduction",
                oldPath.GetText());
        oldPathInImpliedNode = transferFunction.MapSourceToTarget(oldPath);
        newPathInImpliedNode = transferFunction.MapSourceToTarget(newPath);
    } else {
        ...
        // node.
        _PRINT_DEBUG("Spec move at <%s> affects implied node's descendant", oldPath.GetText());

        // We can map the old path directly from the origin node to the implied
        // node.
        oldPathInImpliedNode = oldPath.ReplacePrefix(originPathAtClassIntroduction, impliedPathAtClassIntroduction);
        ...
        newPathInImpliedNode =
                newPath.HasPrefix(originPathAtClassIntroduction)
                        ? newPath.ReplacePrefix(originPathAtClassIntroduction, impliedPathAtClassIntroduction)
                        : SdfPath();
    }

    _PRINT_DEBUG("Mapped old path <%s> into implied node as <%s>", oldPath.GetText(), oldPathInImpliedNode.GetText());
    _PRINT_DEBUG("Mapped new path <%s> into implied node as <%s>", newPath.GetText(), newPathInImpliedNode.GetText());

    // Add the task and mark it as a implied class task so we handle deletes
    // properly.
    _InsertNodeTask({impliedNode, oldPathInImpliedNode, newPathInImpliedNode,
...
static void _FinalizeSpecMoveEdits(PcpDependentNamespaceEdits* edits, _LayerSpecMovesScratch&& layerSpecMovesScratch) {
    TRACE_FUNCTION();

    // Compare function for sorting spec move edit descriptions by old path
    // first and then by new path.
    auto specMoveDescLessThan = [](const PcpDependentNamespaceEdits::SpecMoveEditDescription& lhs,
                                   const PcpDependentNamespaceEdits::SpecMoveEditDescription& rhs) {
        return std::tie(lhs.oldPath, lhs.newPath) < std::tie(rhs.oldPath, rhs.newPath);
    };
    ...
    for (auto& [layer, specMovesScratch] : layerSpecMovesScratch) {
        ...
        if (specMoves.size() > 1) {
            ...
            // sorting.
            for (auto it = specMoves.begin(), next = it + 1; next != specMoves.end(); /*no advance*/) {
                ...
                if (next->oldPath == it->oldPath) {
                    if (next->newPath == it->newPath) {
                        ...
                        edits->errors.push_back(
                                TfStringPrintf("Dependent edit conflict: Trying to move spec at "
                                               "layer @%s@ and path <%s> to both new paths <%s> "
                                               "and <%s>",
                                               layer->GetIdentifier().c_str(), it->oldPath.GetText(),
                                               it->newPath.GetText(), next->newPath.GetText()));
                    }
                }
                ++it;
                ++next;
        ...
        for (const auto& deleteSpecPath : optionalSpecDeletes) {
            PcpDependentNamespaceEdits::SpecMoveEditDescription specDelete{deleteSpecPath, SdfPath()};
            auto insertIt = std::lower_bound(specMoves.begin(), specMoves.end(), specDelete, specMoveDescLessThan);
            if (insertIt == specMoves.end() || insertIt->oldPath != deleteSpecPath) {
        ...
        for (auto it = specMoves.begin(); it != specMoves.end();
            ...
            if (auto otherIt = std::find_if(std::make_reverse_iterator(it), specMoves.rend(),
                                            [&](const auto& other) {
                ...
                if ((otherIt->newPath.IsEmpty() && thisNewPath.IsEmpty()) ||
                    thisOldPath.ReplacePrefix(otherIt->oldPath, otherIt->newPath) == thisNewPath) {
...
PcpDependentNamespaceEdits PcpGatherDependentNamespaceEdits(const SdfPath& oldPrimPath,
    ...
    // start.)
    std::vector<std::pair<SdfLayerHandle, bool>> affectedLayersAndHasRelocatesEdits;
    ...
    if (affectedRelocatesLayerStack) {
        PcpLayerRelocatesEditBuilder builder(affectedRelocatesLayerStack, addRelocatesToLayerStackEditLayer);
        std::string error;
        if (!builder.Relocate(oldPrimPath, newPrimPath, &error)) {
            TF_CODING_ERROR("Cannot get relocates edits because: %s", error.c_str());
        }
        ...
        for (auto& relocatesEdit : builder.GetEdits()) {
            ...
            // is inserted by rvalue reference.
            {
                ...
                auto foundIt = std::find_if(affectedLayersAndHasRelocatesEdits.begin(),
                                            affectedLayersAndHasRelocatesEdits.end(), [&](const auto& entry) {
    ...
    for (const PcpCache* cache : dependentCaches) {
        _PRINT_DEBUG_SCOPE("Computing dependent namespace edits for PcpCache %s",
        ...
        for (const auto& entry : affectedLayersAndHasRelocatesEdits) {
            ...
            PcpDependencyVector deps = cache->FindSiteDependencies(layer, oldPrimPath, PcpDependencyTypeAnyNonVirtual,
                                                                   /* recurseOnSite */ true,
                                                                   /* recurseOnIndex */ false,
                                                                   /* filter */ true);

            _PRINT_DEBUG("Found %lu dependencies for spec edit at site @%s@<%s>.", deps.size(),
                         layer->GetIdentifier().c_str(), oldPrimPath.GetText());

            using _DependencySet = std::unordered_set<std::pair<SdfPath, SdfPath>, TfHash>;
            ...
            for (const PcpDependency& dep : deps) {
                ...
                if (!seenDependencies.emplace(dep.indexPath, dep.sitePath).second) {
                    _PRINT_DEBUG(
                ...
                // here.
                _PrimIndexDependentNodeEditProcessor dependentNodeProcessor(primIndex, &edits, &layerSpecMovesScratch);
                Pcp_ForEachDependentNode(dep.sitePath, layer, dep.indexPath, *cache,
...
SdfLayerHandleVector PcpGatherLayersToEditForSpecMove(const PcpLayerStackRefPtr& layerStack,
    ...
    for (const auto& layer : layersToEdit) {
        ...
        if (!layer->PermissionToEdit()) {
            errors->push_back(
                    TfStringPrintf("The spec @%s@<%s> "
                                   "cannot be edited because the layer is not editable",
                                   layer->GetIdentifier().c_str(), oldSpecPath.GetText()));
        }
        ...
        if (!newSpecPath.IsEmpty() && layer->HasSpec(newSpecPath)) {
            errors->push_back(
                    TfStringPrintf("The spec @%s@<%s> "
                                   ...
                                   "the new path",
                                   layer->GetIdentifier().c_str(), oldSpecPath.GetText(), newSpecPath.GetText()));
        }
    }
