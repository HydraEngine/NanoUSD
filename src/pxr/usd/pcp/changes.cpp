...
TF_DEFINE_ENV_SETTING(PCP_ENABLE_MINIMAL_CHANGES_FOR_LAYER_OPERATIONS,
                      false,
...
void Pcp_SubsumeDescendants(SdfPathSet* pathSet, const SdfPath& prefix) {
...
static bool Pcp_EntryRequiresLayerStackOffsetsChange(const SdfLayerHandle& layer,
    ...
    if (it != entry.infoChanged.end()) {
        ...
        auto _MatchesFallback = [&layer](const VtValue& val) {
            return layer->GetSchema().GetFallback(SdfFieldKeys->TimeCodesPerSecond) == val;
        };
...
static bool Pcp_EntryRequiresPrimIndexChange(const SdfChangeList::Entry& entry) {
...
enum {
    ...
    Pcp_EntryChangeSpecsAdd = Pcp_EntryChangeSpecsAddInert | Pcp_EntryChangeSpecsAddNonInert,
    Pcp_EntryChangeSpecsRemove = Pcp_EntryChangeSpecsRemoveInert | Pcp_EntryChangeSpecsRemoveNonInert,
    Pcp_EntryChangeSpecsInert = Pcp_EntryChangeSpecsAddInert | Pcp_EntryChangeSpecsRemoveInert,
    Pcp_EntryChangeSpecsNonInert = Pcp_EntryChangeSpecsAddNonInert | Pcp_EntryChangeSpecsRemoveNonInert
};

static int Pcp_EntryRequiresPrimSpecsChange(const SdfChangeList::Entry& entry) {
    int result = 0;

    result |= entry.flags.didAddInertPrim ? Pcp_EntryChangeSpecsAddInert : 0;
    result |= entry.flags.didRemoveInertPrim ? Pcp_EntryChangeSpecsRemoveInert : 0;
    result |= entry.flags.didAddNonInertPrim ? Pcp_EntryChangeSpecsAddNonInert : 0;
    result |= entry.flags.didRemoveNonInertPrim ? Pcp_EntryChangeSpecsRemoveNonInert : 0;

    return result;
}

static int Pcp_EntryRequiresPropertySpecsChange(const SdfChangeList::Entry& entry) {
    int result = 0;

    result |= entry.flags.didAddPropertyWithOnlyRequiredFields ? Pcp_EntryChangeSpecsAddInert : 0;
    result |= entry.flags.didRemovePropertyWithOnlyRequiredFields ? Pcp_EntryChangeSpecsRemoveInert : 0;
...
static bool Pcp_ChangeMayAffectDynamicFileFormatArguments(const PcpCache* cache,
                                                          const SdfChangeList::Entry& entry,
...
static bool Pcp_PrimSpecOrDescendantHasRelocates(const SdfLayerHandle& layer, const SdfPath& primPath) {
    TRACE_FUNCTION();
...
static bool Pcp_LayerMightHaveRelocates(const PcpCache* cache, const SdfLayerHandle& layer) {
    if (!layer) {
        return false;
    }
    ...
    if (cache->IsUsd()) {
        ...
        return Pcp_PrimSpecOrDescendantHasRelocates(layer, SdfPath::AbsoluteRootPath());
    }
}
...
static bool Pcp_DoesInfoChangeAffectFileFormatArguments(PcpCache const* cache,
                                                        const SdfPath& primIndexPath,
                                                        const SdfChangeList::Entry& changes,
                                                        std::string* debugSummary) {
    PCP_APPEND_DEBUG("Pcp_DoesInfoChangeAffectFileFormatArguments %s:%s?\n",
                     cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str(), primIndexPath.GetText());
    ...
    for (const auto& change : changes.infoChanged) {
        const bool isRelevantChange = depData.CanFieldChangeAffectFileFormatArguments(change.first, change.second.first,
                                                                                      change.second.second);
        PCP_APPEND_DEBUG(
                "  Field '%s' change: %s -> %s "
                "%s relevant for prim index path '%s'\n",
                change.first.GetText(), TfStringify(change.second.first).c_str(),
                TfStringify(change.second.second).c_str(), isRelevantChange ? "IS" : "is NOT", primIndexPath.GetText());
        if (isRelevantChange) {
            return true;
        }
    }
...
using AttributeMayAffectFileFormatArgumentsChangeVector = std::vector<AttributeMayAffectFileFormatArgumentsChange>;

static void Pcp_CollectAttributeMayAffectDynamicFileFormatArgumentsChanges(
    ...
    auto addChangeFn = [&](const SdfPath& propPath, const SdfChangeList::Entry::InfoChange* defaultFieldChange) {
    ...
    if (entry.flags.didRename) {
        ...
        if (cache->IsPossibleDynamicFileFormatArgumentAttribute(path.GetNameToken()) &&
            layer->HasField(path, SdfFieldKeys->Default)) {
            addChangeFn(path, nullptr);
            PCP_APPEND_DEBUG(
                    "Property renamed to @%s@<%s> may affect "
        ...
        if (cache->IsPossibleDynamicFileFormatArgumentAttribute(entry.oldPath.GetNameToken())) {
            addChangeFn(entry.oldPath, nullptr);
            PCP_APPEND_DEBUG(
                    "Property renamed from @%s@<%s> may affect "
                    "file format arguments in cache %s\n",
                    layer->GetIdentifier().c_str(), entry.oldPath.GetText(),
                    cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str());
        }
    ...
    if (entry.flags.didRemoveProperty) {
        PCP_APPEND_DEBUG(
                "Removed property @%s@<%s> may affect "
                "file format arguments in cache %s\n",
                layer->GetIdentifier().c_str(), path.GetText(),
                cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str());
        addChangeFn(path, nullptr);
    ...
    if (it != entry.infoChanged.end()) {
        PCP_APPEND_DEBUG(
                "Default value change for property @%s@<%s> may "
...
// format arguments of for a dynamic file format under the prim index at path.
static bool Pcp_DoesAttributeChangeAffectFileFormatArguments(PcpCache const* cache,
                                                             const SdfPath& primIndexPath,
                                                             const AttributeMayAffectFileFormatArgumentsChange& change,
                                                             std::string* debugSummary) {
    PCP_APPEND_DEBUG("Pcp_DoesAttributeChangeAffectFileFormatArguments %s:%s?\n",
                     cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str(), primIndexPath.GetText());
    ...
    // the file format args of any node in the prim index graph.
    const bool isRelevantChange = depData.CanAttributeDefaultValueChangeAffectFileFormatArguments(
            propName,
            /* oldValue = */ change.defaultFieldChange->first,
            /* newValue = */ change.defaultFieldChange->second);
    PCP_APPEND_DEBUG(
            "Field '%s' change: %s -> %s %s relevant for prim index "
            "path '%s'\n",
            propName.GetText(), TfStringify(change.defaultFieldChange->first).c_str(),
            TfStringify(change.defaultFieldChange->second).c_str(), isRelevantChange ? "IS" : "is NOT",
...
static void Pcp_DidChangeDependents(const PcpCache* cache,
                                    const SdfLayerHandle& layer,
                                    const SdfPath& path,
    ...
    const bool recurseOnSite =
            processPrimDescendants && (path == SdfPath::AbsoluteRootPath() || path.IsPrimOrPrimVariantSelectionPath());
    PcpDependencyVector deps =
            cache->FindSiteDependencies(layer, path, PcpDependencyTypeAnyIncludingVirtual, recurseOnSite,
    ...
    PCP_APPEND_DEBUG(
            ...
            cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str(),
            recurseOnSite ? "recurse on prim descendants" : "do not recurse on prim descendants",
            layer->GetIdentifier().c_str(), path.GetText(),
            onlyExistingDependentPaths ? " (restricted to existing caches)" : " (not restricted to existing caches)");
...
void PcpChanges::DidChange(const PcpCache* cache, const SdfLayerChangeListVec& changes) {
    // LayerStack changes
    ...
    typedef std::map<PcpLayerStackPtr, LayerStackChangeBitmask> LayerStackChangeMap;
    ...
    typedef std::map<SdfPath, PathChangeBitmask, SdfPath::FastLessThan> PathChangeMap;
    typedef PathChangeMap::value_type PathChangeValue;

    // Spec changes
    typedef int SpecChangeBitmask;
    typedef std::map<SdfPath, SpecChangeBitmask, SdfPath::FastLessThan> SpecChangesTypes;
    ...
    AttributeMayAffectFileFormatArgumentsChangeVector attributeMayAffectFileFormatArgumentsChanges;
    ...
    for (auto const& i : changes) {
        ...
        if (cacheHasDynamicFileFormatAttributeDependencies) {
            ...
            for (const auto& entry : entries) {
                Pcp_CollectAttributeMayAffectDynamicFileFormatArgumentsChanges(
                        cache, layer, entry.first, entry.second, &attributeMayAffectFileFormatArgumentsChanges,
                        debugSummary);
            }
        }
        ...
        if (cacheInUsdMode && attributeMayAffectFileFormatArgumentsChanges.empty()) {
            using _Entries = SdfChangeList::EntryList;

            const _Entries& entries = changeList.GetEntryList();
            const bool hasPrimChanges =
                    std::find_if(entries.begin(), entries.end(), [](const _Entries::value_type& entry) {
        ...
        const PcpLayerStackPtrVector layerStacks = cache->FindAllLayerStacksUsingLayer(layer);
        if (layerStacks.empty()) {
            PCP_APPEND_DEBUG("  Layer @%s@ changed:  unused\n", layer->GetIdentifier().c_str());
            continue;
        }

        PCP_APPEND_DEBUG("  Changes to layer %s:\n%s", layer->GetIdentifier().c_str(), TfStringify(changeList).c_str());
        ...
        for (auto const& j : changeList.GetEntryList()) {
            ...
            // ancestor path.
            const bool fallbackToParent = entry.flags.didAddInertPrim || entry.flags.didRemoveInertPrim ||
                                          entry.flags.didAddNonInertPrim || entry.flags.didRemoveNonInertPrim ||
                                          entry.flags.didAddProperty || entry.flags.didRemoveProperty ||
            ...
            if (path == SdfPath::AbsoluteRootPath()) {
                ...
                switch (Pcp_EntryRequiresLayerStackChange(entry)) {
                        ...
                        for (auto const& k : entry.subLayerChanges) {
                            if (k.second == SdfChangeList::SubLayerAdded ||
                                ...
                                const _SublayerChangeType sublayerChange =
                                        k.second == SdfChangeList::SubLayerAdded ? _SublayerAdded : _SublayerRemoved;

                                std::vector<bool> significant;
                                _DidAddOrRemoveSublayer(cache, layerStacks, layer, sublayerPath, sublayerChange,
                                                        debugSummary, &significant);

                                for (size_t i = 0, e = layerStacks.size(); i != e; ++i) {
                                    if (significant[i]) {
                                        layerStackChangesMap[layerStacks[i]] |= LayerStackSignificantChange;
                                    }
                                }
                            }
                        }
                    ...
                    case Pcp_ChangesLayerStackChangeSignificant:
                        ...
                        layerStackChangeMask |= LayerStackLayersChange | LayerStackSignificantChange;
                        pathsWithSignificantChanges.insert(path);
        ...
        for (const auto& path : pathsWithSignificantChanges) {
            ...
            Pcp_DidChangeDependents(
                    cache, layer, path, /*processPrimDescendants*/ true, onlyExistingDependentPaths,
        ...
        for (const SdfPath& changedPath : pathsWithFileFormatArgumentsChanges) {
            const bool onlyExistingDependentPaths = fallbackToAncestorPaths.count(changedPath) == 0;

            SdfChangeList::Entry const& changes = changeList.GetEntry(changedPath);
            ...
            Pcp_DidChangeDependents(
                    cache, layer, changedPath, /*processPrimDescendants*/ true, onlyExistingDependentPaths,
        ...
        for (const auto& p : attributeMayAffectFileFormatArgumentsChanges) {
            const bool onlyExistingDependentPaths = fallbackToAncestorPaths.count(p.propertyPath.GetPrimPath()) == 0;
            ...
            Pcp_DidChangeDependents(
                    ...
                    /*processPrimDescendants*/ true, onlyExistingDependentPaths,
                    [this, &cache, &p, &debugSummary](const PcpDependency& dep) {
                        if (Pcp_DoesAttributeChangeAffectFileFormatArguments(cache, dep.indexPath, p, debugSummary)) {
                            DidChangeSignificantly(cache, dep.indexPath);
                        }
                    },
                    debugSummary);
            ...
            Pcp_DidChangeDependents(
                    ...
                    [this, &changes, &changesType, &cache, &layer](const PcpDependency& dep) {
                        ...
                        if (changes & ~(PathChangeTargets | PathChangeConnections)) {
                            const ChangeSpecsType changeSpecType = changesType & Pcp_EntryChangeSpecsAdd
                                                                           ? ChangeSpecsTypeAdded
                                                                           : ChangeSpecsTypeRemoved;

                            DidChangeSpecs(cache, dep.indexPath, layer, dep.sitePath, changeSpecType);
                        }
        ...
        if (!oldPaths.empty()) {
            ...
            for (size_t i = 0, n = oldPaths.size(); i != n; ++i) {
                ...
                if (!newPath.IsEmpty()) {
                    PcpDependencyVector deps =
                            cache->FindSiteDependencies(layer, newPath, PcpDependencyTypeAnyNonVirtual,
                                                        /* recurseOnSite */ false,
                                                        /* recurseOnIndex */ false,
                                                        /* filter */ true);
                    for (const auto& dep : deps) {
                ...
                PcpDependencyVector deps = cache->FindSiteDependencies(layer, oldPath, PcpDependencyTypeAnyNonVirtual,
                                                                       /* recurseOnSite */ false,
                                                                       /* recurseOnIndex */ false,
                                                                       /* filter */ true);
                for (const auto& dep : deps) {
                    ...
                    PCP_APPEND_DEBUG("  renameChanges <%s> to <%s>\n", dep.indexPath.GetText(), newIndexPath.GetText());
                }
            }
        }
    ...
    for (const auto& entry : layerStackChangesMap) {
        ...
        if (layerStackChanges & LayerStackResolvedPathChange) {
            const bool needToRecompute = Pcp_NeedToRecomputeDueToAssetPathChange(layerStack);

            _DidChangeLayerStackResolvedPath(cache, layerStack, needToRecompute, debugSummary);
            if (needToRecompute) {
                layerStackChanges |= LayerStackLayersChange | LayerStackSignificantChange;
            }
        }
        ...
        if (layerStackChanges & LayerStackExpressionVarsChange) {
            _DidChangeLayerStackExpressionVariables(cache, layerStack, debugSummary);
        }

        _DidChangeLayerStack(cache, layerStack, layerStackChanges & LayerStackLayersChange,
...
void PcpChanges::DidMuteAndUnmuteLayers(const PcpCache* cache,
...
void PcpChanges::_MarkReferencingSitesAsSignificantlyChanged(const PcpCache* cache,
                                                             const PcpLayerStackPtrVector& layerStacks) {
    TRACE_FUNCTION();

    const PcpCacheChanges& cacheChanges = _GetCacheChanges(cache);

    const auto refOrPayloadChangeFunc = [this, cache](const SdfPath& depIndexPath, PcpArcType arcType) {
    ...
    for (const PcpLayerStackPtr& layerStack : layerStacks) {
        PcpDependencyVector deps = cache->FindSiteDependencies(layerStack, SdfPath::AbsoluteRootPath(),
        ...
        for (const PcpDependency& dep : deps) {
            ...
            if (PcpComposeSiteHasPrimSpecs(layerStack, dep.sitePath, cacheChanges.layersAffectedByMutingOrRemoval)) {
                continue;
            }
...
void PcpChanges::_ProcessLayerStackAndDependencyChanges(const PcpCache* cache,
                                                        const PcpLayerStackPtrVector& layerStacks) {
    TRACE_FUNCTION();

    for (const PcpLayerStackPtr& layerStack : layerStacks) {
        _DidChangeLayerStack(cache, layerStack,
...
void PcpChanges::_DidMuteLayer(const PcpCache* cache, const std::string& layerId) {
    // Change debugging.
    std::string summary;
    std::string* debugSummary = TfDebug::IsEnabled(PCP_CHANGES) ? &summary : 0;

    const SdfLayerRefPtr mutedLayer = _LoadSublayerForChange(cache, layerId, _SublayerRemoved);
    const PcpLayerStackPtrVector& layerStacks = cache->FindAllLayerStacksUsingLayer(mutedLayer);
    ...
    if (!TfGetEnvSetting(PCP_ENABLE_MINIMAL_CHANGES_FOR_LAYER_OPERATIONS) || !mutedLayer || mutedLayer->IsEmpty() ||
        Pcp_LayerMightHaveRelocates(cache, mutedLayer)) {
        _DidChangeSublayerAndLayerStacks(cache, layerStacks, layerId, mutedLayer, _SublayerRemoved, debugSummary);
    } else {
        SdfLayerRefPtr empty = SdfLayer::CreateAnonymous(mutedLayer->GetDisplayName(), mutedLayer->GetFileFormat(),
                                                         mutedLayer->GetFileFormatArguments());

        SdfLayerChangeListVec changes = {{mutedLayer, mutedLayer->CreateDiff(empty, /*compareFieldValues*/ false)}};

        DidChange(cache, changes);
        cacheChanges.layerChangeListVec.emplace_back(std::move(changes.front()));
...
void PcpChanges::_DidUnmuteLayer(const PcpCache* cache, const std::string& layerId) {
    // Change debugging.
    std::string summary;
    std::string* debugSummary = TfDebug::IsEnabled(PCP_CHANGES) ? &summary : 0;

    const SdfLayerRefPtr unmutedLayer = _LoadSublayerForChange(cache, layerId, _SublayerAdded);
    const PcpLayerStackPtrVector& layerStacks = cache->_layerStackCache->FindAllUsingMutedLayer(layerId);
    ...
    // been updated yet to reflect the change.  In this case we will use a less
    // fine grained approach.
    if (!TfGetEnvSetting(PCP_ENABLE_MINIMAL_CHANGES_FOR_LAYER_OPERATIONS) || !unmutedLayer || unmutedLayer->IsEmpty() ||
        Pcp_LayerMightHaveRelocates(cache, unmutedLayer)) {
        _DidChangeSublayerAndLayerStacks(cache, layerStacks, layerId, unmutedLayer, _SublayerAdded, debugSummary);
    } else {
        cache->_layerStackCache->SetLayerStackVectorOverride(unmutedLayer, layerStacks);

        SdfLayerRefPtr empty = SdfLayer::CreateAnonymous(unmutedLayer->GetDisplayName(), unmutedLayer->GetFileFormat(),
                                                         unmutedLayer->GetFileFormatArguments());

        SdfLayerChangeListVec changes = {{unmutedLayer, empty->CreateDiff(unmutedLayer, /*compareFieldValues*/ false)}};
...
void PcpChanges::DidMaybeFixSublayer(const PcpCache* cache,
                                     const SdfLayerHandle& layer,
    ...
    // layer stacks.
    const SdfLayerRefPtr sublayer = _LoadSublayerForChange(cache, layer, sublayerPath, _SublayerAdded);
    const PcpLayerStackPtrVector& layerStacks = cache->FindAllLayerStacksUsingLayer(layer);

    PCP_APPEND_DEBUG("  Layer @%s@ changed sublayer @%s@\n", layer ? layer->GetIdentifier().c_str() : "invalid",
                     sublayerPath.c_str());
...
void PcpChanges::_DidChangeSublayerAndLayerStacks(const PcpCache* cache,
                                                  const PcpLayerStackPtrVector& layerStacks,
                                                  const std::string& sublayerPath,
    ...
    _DidChangeSublayer(cache, layerStacks, sublayerPath, sublayer, sublayerChange, debugSummary,
                       &requiresSignificantChange);

    if (sublayer) {
        // Layer was loaded.  The layer stacks are changed.
        for (const PcpLayerStackPtr& layerStack : layerStacks) {
            _DidChangeLayerStack(cache, layerStack, requiresLayerStackChange, requiresLayerStackOffsetsChange,
                                 requiresSignificantChange);
        }
    }
}

void PcpChanges::DidMaybeFixAsset(const PcpCache* cache,
                                  const PcpSite& site,
    ...
    // Load the layer.
    TfErrorMark m;
    SdfLayerRefPtr layer = SdfLayer::FindOrOpenRelativeToLayer(srcLayer, assetPath);
    m.Clear();
    ...
    if (layer) {
        ...
        PCP_APPEND_DEBUG(
                ...
                cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str(),
                layerStack->GetIdentifier().rootLayer->GetIdentifier().c_str(), site.path.GetText());
        if (layerStack == cache->GetLayerStack()) {
            PCP_APPEND_DEBUG("    <%s>\n", site.path.GetText());
            DidChangeSignificantly(cache, site.path);
        }
        PcpDependencyVector deps =
                cache->FindSiteDependencies(layerStack, site.path, PcpDependencyTypeAnyIncludingVirtual,
                                            /* recurseOnSite */ true,
                                            /* recurseOnIndex */ true,
                                            /* filter */ true);
        for (const auto& dep : deps) {
...
void PcpChanges::DidChangeSignificantly(const PcpCache* cache, const SdfPath& path) {
    _GetCacheChanges(cache).didChangeSignificantly.insert(path);
}

static bool _NoLongerHasAnySpecs(const PcpCacheChanges& changes, const PcpPrimIndex& primIndex) {
    for (const PcpNodeRef& node : primIndex.GetNodeRange()) {
        if (PcpComposeSiteHasPrimSpecs(node.GetLayerStack(), node.GetPath(), changes.layersAffectedByMutingOrRemoval)) {
            return false;
        }
    }
    return true;
...
void PcpChanges::DidChangeSpecs(const PcpCache* cache,
                                const SdfPath& path,
    ...
    if (path.IsPrimPath()) {
        ...
        if (primIndex && primIndex->HasSpecs()) {
            ...
            // change.
            if (primWasRemoved && _NoLongerHasAnySpecs(_GetCacheChanges(cache), *primIndex)) {
...
void PcpChanges::DidChangeSpecStack(const PcpCache* cache, const SdfPath& path) {
    _GetCacheChanges(cache).didChangeSpecs.insert(path);
}

void PcpChanges::DidChangeTargets(const PcpCache* cache, const SdfPath& path, PcpCacheChanges::TargetType targetType) {
    _GetCacheChanges(cache).didChangeTargets[path] |= targetType;
}

void PcpChanges::DidChangePaths(const PcpCache* cache, const SdfPath& oldPath, const SdfPath& newPath) {
    TF_DEBUG(PCP_CHANGES)
            .Msg("PcpChanges::DidChangePaths: @%s@<%s> to <%s>\n",
                 cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str(), oldPath.GetText(),
                 newPath.GetText());
...
void PcpChanges::DidDestroyCache(const PcpCache* cache) {
...
void PcpChanges::DidChangeAssetResolver(const PcpCache* cache) {
    TF_DEBUG(PCP_CHANGES).Msg("PcpChanges::DidChangeAssetResolver\n");

    // Change debugging.
    std::string summary;
    std::string* debugSummary = TfDebug::IsEnabled(PCP_CHANGES) ? &summary : 0;

    cache->ForEachLayerStack([this, &cache, debugSummary](const PcpLayerStackPtr& layerStack) {
    ...
    if (debugSummary && !debugSummary->empty()) {
        TfDebug::Helper().Msg(
                ...
                cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str(), debugSummary->c_str());
    }
}

void PcpChanges::Swap(PcpChanges& other) {
...
bool PcpChanges::IsEmpty() const {
    return _layerStackChanges.empty() && _cacheChanges.empty() && _renameChanges.empty();
}

const PcpChanges::LayerStackChanges& PcpChanges::GetLayerStackChanges() const {
    return _layerStackChanges;
}

const PcpChanges::CacheChanges& PcpChanges::GetCacheChanges() const {
...
const PcpLifeboat& PcpChanges::GetLifeboat() const {
    return _lifeboat;
}

void PcpChanges::Apply() const {
    // NOTE: This is potentially expensive even if we've already done
    //       it.  In the expected use pattern we only call this method
    //       once, so it shouldn't be a problem.
    _Optimize();
...
PcpLayerStackChanges& PcpChanges::_GetLayerStackChanges(const PcpLayerStackPtr& layerStack) {
    return _layerStackChanges[layerStack];
}

PcpCacheChanges& PcpChanges::_GetCacheChanges(const PcpCache* cache) {
    return _cacheChanges[const_cast<PcpCache*>(cache)];
}

PcpChanges::_PathEditMap& PcpChanges::_GetRenameChanges(const PcpCache* cache) {
    return _renameChanges[const_cast<PcpCache*>(cache)];
}

void PcpChanges::_Optimize() const {
...
void PcpChanges::_Optimize(PcpCacheChanges* changes) {
...
void PcpChanges::_OptimizePathChanges(const PcpCache* cache,
...
SdfLayerRefPtr PcpChanges::_LoadSublayerForChange(const PcpCache* cache,
                                                  const std::string& sublayerPath,
                                                  _SublayerChangeType sublayerChange) const {
    // Bind the resolver context.
    const ArResolverContextBinder binder(cache->GetLayerStackIdentifier().pathResolverContext);

    // Load the layer.
    SdfLayerRefPtr sublayer;

    const SdfLayer::FileFormatArguments sublayerArgs =
            Pcp_GetArgumentsForFileFormatTarget(sublayerPath, cache->GetFileFormatTarget());
...
void PcpChanges::_DidAddOrRemoveSublayer(const PcpCache* cache,
                                         const PcpLayerStackPtrVector& layerStacks,
                                         const SdfLayerHandle& layer,
                                         const std::string& sublayerPath,
                                         _SublayerChangeType sublayerChange,
                                         std::string* debugSummary,
                                         std::vector<bool>* significant) {
    PCP_APPEND_DEBUG("  Layer @%s@ changed sublayers\n", layer ? layer->GetIdentifier().c_str() : "invalid");

    const auto& processChanges = [this, &cache, &sublayerPath, &debugSummary, &layer](
                                         const SdfLayerRefPtr sublayer, const PcpLayerStackPtrVector& layerStacks,
                                         _SublayerChangeType sublayerChange) {
        PcpCacheChanges& cacheChanges = _GetCacheChanges(cache);
        if (sublayer) {
            ...
            if (sublayerChange == _SublayerRemoved) {
                cacheChanges.layersAffectedByMutingOrRemoval.insert(sublayer);
            }
        }

        if (!TfGetEnvSetting(PCP_ENABLE_MINIMAL_CHANGES_FOR_LAYER_OPERATIONS) || !sublayer || sublayer->IsEmpty() ||
        ...
        SdfLayerRefPtr empty = SdfLayer::CreateAnonymous(sublayer->GetDisplayName(), sublayer->GetFileFormat(),
                                                         sublayer->GetFileFormatArguments());

        if (sublayerChange == _SublayerAdded) {
            cache->_layerStackCache->SetLayerStackVectorOverride(sublayer, layerStacks);
            changes.emplace_back(std::make_pair(sublayer, empty->CreateDiff(sublayer, /*compareFieldValues*/ false)));
        } else {
            changes.emplace_back(std::make_pair(sublayer, sublayer->CreateDiff(empty, /*compareFieldValues*/ false)));
        }
    ...
    if (!Pcp_IsVariableExpression(sublayerPath)) {
        const SdfLayerRefPtr sublayer = _LoadSublayerForChange(cache, layer, sublayerPath, sublayerChange);

        const bool sublayerIsSignificant = processChanges(sublayer, layerStacks, sublayerChange);
    ...
    PcpExpressionVariableCachingComposer expressionVarComposer(cache->GetLayerStackIdentifier());
    ...
    for (size_t i = 0, e = layerStacks.size(); i != e; ++i) {
        ...
        const PcpExpressionVariables& expressionVars =
                sublayerChange == _SublayerAdded
                        ? expressionVarComposer.ComputeExpressionVariables(layerStack->GetIdentifier())
                        : layerStack->GetExpressionVariables();

        const std::string evaluatedPath = Pcp_EvaluateVariableExpression(sublayerPath, expressionVars);
        ...
        if (d.sublayerPath.empty()) {
            d.sublayer = _LoadSublayerForChange(cache, layer, evaluatedPath, sublayerChange);
    ...
    for (const auto& entry : sublayerToLayerStack) {
        ...
        const bool sublayerIsSignificant = processChanges(d.sublayer, d.layerStacks, sublayerChange);
...
void PcpChanges::_DidChangeSublayer(const PcpCache* cache,
                                    const PcpLayerStackPtrVector& layerStacks,
                                    const std::string& sublayerPath,
                                    const SdfLayerHandle& sublayer,
                                    _SublayerChangeType sublayerChange,
    ...
    *significant = (sublayer && !sublayer->IsEmpty());
    ...
    PCP_APPEND_DEBUG("  %s sublayer @%s@ %s\n", sublayer ? (*significant ? "significant" : "insignificant") : "invalid",
                     sublayerPath.c_str(), sublayerChange == _SublayerAdded ? "added" : "removed");
            ...
            if (!anyFound) {
                PCP_APPEND_DEBUG(
                        ...
                        *significant ? "Resync" : "Spec changes",
                        cache->GetLayerStackIdentifier().rootLayer->GetIdentifier().c_str(),
                        *significant ? "significant" : "insignificant", sublayer->GetIdentifier().c_str());
                anyFound = true;
            }
            PCP_APPEND_DEBUG("    <%s>\n", dep.indexPath.GetText());
            if (*significant) {
...
void PcpChanges::_DidChangeLayerStack(const PcpCache* cache,
...
static void _DeterminePathsAffectedByRelocationChanges(const SdfRelocatesMap& oldMap,
...
void PcpChanges::_DidChangeLayerStackRelocations(const PcpCache* cache,
                                                 const PcpLayerStackPtr& layerStack,
                                                 std::string* debugSummary) {
    PcpLayerStackChanges& changes = _GetLayerStackChanges(layerStack);
    ...
    Pcp_ComputeRelocationsForLayerStack(
            *layerStack, &changes.newRelocatesSourceToTarget, &changes.newRelocatesTargetToSource,
            &changes.newIncrementalRelocatesSourceToTarget, &changes.newIncrementalRelocatesTargetToSource,
            &changes.newRelocatesPrimPaths, &changes.newRelocatesErrors);
    ...
    if (layerStack->IsUsd() && (layerStack->HasRelocates() != willHaveRelocates)) {
        ...
        _DeterminePathsAffectedByRelocationChanges(layerStack->GetIncrementalRelocatesSourceToTarget(),
    ...
    for (const SdfPath& path : changes.pathsAffectedByRelocationChanges) {
        ...
        PcpDependencyVector deps = cache->FindSiteDependencies(layerStack, path, PcpDependencyTypeAnyIncludingVirtual,
                                                               /* recurseOnSite */ true,
                                                               /* recurseOnIndex */ true,
...
void PcpChanges::_DidChangeLayerStackResolvedPath(const PcpCache* cache,
                                                  const PcpLayerStackPtr& layerStack,
                                                  bool requiresLayerStackChange,
                                                  std::string* debugSummary) {
    ...
    if (requiresLayerStackChange) {
        ...
        deps = cache->FindSiteDependencies(layerStack, SdfPath::AbsoluteRootPath(),
                                           PcpDependencyTypeAnyIncludingVirtual,
                                           /* recurseOnSite */ true,
                                           /* recurseOnIndex */ false,
        ...
        auto noResyncNeeded = [cache](const PcpDependency& dep) {
            ...
            return (TF_VERIFY(primIndex) && !Pcp_NeedToRecomputeDueToAssetPathChange(*primIndex));
        };
...
void PcpChanges::_DidChangeLayerStackExpressionVariables(const PcpCache* cache,
                                                         const PcpLayerStackPtr& layerStackIn,
                                                         std::string* debugSummary) {
    const auto resyncAllPrimsUsingLayerStack = [this, cache, debugSummary](const PcpLayerStackPtr& layerStack) {
        const PcpDependencyVector deps = cache->FindSiteDependencies(
                layerStack, SdfPath::AbsoluteRootPath(),
                PcpDependencyTypeRoot | PcpDependencyTypeDirect | PcpDependencyTypeNonVirtual,
                /* recurseOnSite = */ true,
                /* recurseOnIndex = */ false,
    ...
    for (; !layerStacks.empty(); layerStacks.pop_front()) {
        ...
        const PcpExpressionVariables& oldExprVars = layerStack->GetExpressionVariables();
        const PcpExpressionVariables& newExprVars =
                expressionVarComposer.ComputeExpressionVariables(layerStack->GetIdentifier());

        const bool expressionVarsChanged = oldExprVars.GetVariables() != newExprVars.GetVariables();
        const bool expressionVarSourceChanged = oldExprVars.GetSource() != newExprVars.GetSource();

        if (!expressionVarsChanged && !expressionVarSourceChanged) {
            PCP_APPEND_DEBUG("  Expression variables unchanged for layer stack @%s@\n",
                             layerStack->GetIdentifier().rootLayer->GetIdentifier().c_str());
            continue;
        }

        PCP_APPEND_DEBUG("  Expression variables changed for layer stack @%s@\n",
                         layerStack->GetIdentifier().rootLayer->GetIdentifier().c_str());
        ...
        if (expressionVarsChanged) {
            PCP_APPEND_DEBUG(
                    "    old: %s\n"
                    "    new: %s\n",
                    TfStringify(oldExprVars.GetVariables()).c_str(), TfStringify(newExprVars.GetVariables()).c_str());
        ...
        if (expressionVarSourceChanged) {
            PCP_APPEND_DEBUG(
                    "    old source: @%s@\n"
                    "    new source: @%s@\n",
                    oldExprVars.GetSource().ResolveLayerStackIdentifier(*cache).rootLayer->GetIdentifier().c_str(),

                    newExprVars.GetSource().ResolveLayerStackIdentifier(*cache).rootLayer->GetIdentifier().c_str());
        ...
        const auto expressionVarChanged = [&oldExprVars, &newExprVars](const std::string& usedVar) {
            const VtValue* oldVar = TfMapLookupPtr(oldExprVars.GetVariables(), usedVar);
            const VtValue* newVar = TfMapLookupPtr(newExprVars.GetVariables(), usedVar);
            return (oldVar && !newVar) || (!oldVar && newVar) || (oldVar && newVar && *oldVar != *newVar);
        };
        ...
        // recomputation.
        const std::unordered_set<std::string>& usedVars = layerStack->GetExpressionVariableDependencies();

        const bool requiresLayerStackChange = std::any_of(usedVars.begin(), usedVars.end(), expressionVarChanged);
        ...
        for (const SdfPath& primIndexPath : cache->GetPrimsUsingExpressionVariablesFromLayerStack(layerStack)) {
            for (const std::string& usedExprVar :
                 cache->GetExpressionVariablesFromLayerStackUsedByPrim(primIndexPath, layerStack)) {
        ...
        // recomputations.
        cache->ForEachLayerStack([&](const PcpLayerStackPtr& x) {
            if (x == layerStack) {
                return;
            }

            const PcpExpressionVariablesSource& overrideSource = x->GetIdentifier().expressionVariablesOverrideSource;

            if (overrideSource.ResolveLayerStackIdentifier(*cache) == layerStack->GetIdentifier()) {
                PCP_APPEND_DEBUG("    Checking dependent layer stack @%s@\n",
...
void PcpChanges::_DidChangeSpecStackInternal(const PcpCache* cache, const SdfPath& path) {
    _GetCacheChanges(cache)._didChangeSpecsInternal.insert(path);
}

void PcpChanges::_DidChangeSpecStackAndChildrenInternal(const PcpCache* cache, const SdfPath& path) {
