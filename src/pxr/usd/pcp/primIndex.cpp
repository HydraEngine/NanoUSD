...
static inline PcpPrimIndex const* _GetOriginatingIndex(PcpPrimIndex_StackFrame* previousFrame,
    ...
    return ARCH_UNLIKELY(previousFrame) ? previousFrame->originatingIndex : &outputs->primIndex;
}
...
PcpNodeRef PcpPrimIndex::GetRootNode() const {
    return _graph ? _graph->GetRootNode() : PcpNodeRef();
}

const SdfPath& PcpPrimIndex::GetPath() const {
    return _graph ? _graph->GetRootNode().GetPath() : SdfPath::EmptyPath();
}

bool PcpPrimIndex::HasSpecs() const {
...
bool PcpPrimIndex::HasAnyPayloads() const {
    return _graph && _graph->HasPayloads();
}

bool PcpPrimIndex::IsUsd() const {
    return _graph && _graph->IsUsd();
}

bool PcpPrimIndex::IsInstanceable() const {
...
void PcpPrimIndex::Swap(PcpPrimIndex& rhs) {
    _graph.swap(rhs._graph);
    _primStack.swap(rhs._primStack);
    _localErrors.swap(rhs._localErrors);
}

void PcpPrimIndex::PrintStatistics() const {
    Pcp_PrintPrimIndexStatistics(*this, std::cout);
}

std::string PcpPrimIndex::DumpToString(bool includeInheritOriginInfo, bool includeMaps) const {
    return PcpDump(*this, includeInheritOriginInfo, includeMaps);
}

void PcpPrimIndex::DumpToDotGraph(const std::string& filename, bool includeInheritOriginInfo, bool includeMaps) const {
    PcpDumpDotGraph(*this, filename.c_str(), includeInheritOriginInfo, includeMaps);
}

PcpNodeRange PcpPrimIndex::GetNodeRange(PcpRangeType rangeType) const {
    if (!_graph) {
        return PcpNodeRange();
    }

    const std::pair<size_t, size_t> range = _graph->GetNodeIndexesForRange(rangeType);
    return PcpNodeRange(PcpNodeIterator(get_pointer(_graph), range.first),
                        PcpNodeIterator(get_pointer(_graph), range.second));
}

PcpNodeIterator PcpPrimIndex::GetNodeIteratorAtNode(const PcpNodeRef& node) const {
    if (!_graph) {
        return PcpNodeIterator();
    }
    return PcpNodeIterator(get_pointer(_graph), _graph->GetNodeIndexForNode(node));
}

PcpNodeRange PcpPrimIndex::GetNodeSubtreeRange(const PcpNodeRef& node) const {
    if (!_graph) {
        return PcpNodeRange();
    }

    const std::pair<size_t, size_t> range = _graph->GetNodeIndexesForSubtreeRange(node);
    return PcpNodeRange(PcpNodeIterator(get_pointer(_graph), range.first),
                        PcpNodeIterator(get_pointer(_graph), range.second));
}

PcpPrimRange PcpPrimIndex::GetPrimRange(PcpRangeType rangeType) const {
    if (!_graph) {
        return PcpPrimRange();
    }
    ...
    if (rangeType == PcpRangeTypeAll) {
        return PcpPrimRange(PcpPrimIterator(this, 0), PcpPrimIterator(this, _primStack.size()));
    }
    ...
    for (size_t startPrimIdx = 0; startPrimIdx < _primStack.size(); ++startPrimIdx) {
        const Pcp_CompressedSdSite& startPrim = _primStack[startPrimIdx];
        if (startPrim.nodeIndex >= startNodeIdx && startPrim.nodeIndex < endNodeIdx) {
            ...
            return PcpPrimRange(PcpPrimIterator(this, startPrimIdx), PcpPrimIterator(this, endPrimIdx));
        }
    }

    return PcpPrimRange(PcpPrimIterator(this, _primStack.size()), PcpPrimIterator(this, _primStack.size()));
}

PcpPrimRange PcpPrimIndex::GetPrimRangeForNode(const PcpNodeRef& node) const {
...
PcpNodeRef PcpPrimIndex::GetNodeProvidingSpec(const SdfPrimSpecHandle& primSpec) const {
    return GetNodeProvidingSpec(primSpec->GetLayer(), primSpec->GetPath());
}

PcpNodeRef PcpPrimIndex::GetNodeProvidingSpec(const SdfLayerHandle& layer, const SdfPath& path) const {
    for (const PcpNodeRef& node : GetNodeRange()) {
        // If the site has the given path and contributes specs then
        // search for the layer.
        if (node.CanContributeSpecs() && node.GetPath() == path && node.GetLayerStack()->HasLayer(layer)) {
            return node;
        }
    }
    ...
    TF_FOR_ALL(i, GetPrimRange()) {
        ...
        for (auto it = vselMap.begin(); it != vselMap.end();) {
            ...
            if (Pcp_IsVariableExpression(vsel)) {
                const PcpLayerStackRefPtr& layerStack = i.base().GetNode().GetLayerStack();

                PcpErrorVector exprErrors;
                vsel = Pcp_EvaluateVariableExpression(vsel, layerStack->GetExpressionVariables(), "variant", site.layer,
                                                      site.path, nullptr, &exprErrors);
...
std::string PcpPrimIndex::GetSelectionAppliedForVariantSet(const std::string& variantSet) const {
    for (const PcpNodeRef& node : GetNodeRange()) {
        if (node.GetPath().IsPrimVariantSelectionPath()) {
            std::pair<std::string, std::string> vsel = node.GetPath().GetVariantSelection();
            if (vsel.first == variantSet) return vsel.second;
        }
    }
    return std::string();
}

PcpNodeRef PcpPrimIndex::AddChildPrimIndex(const PcpArc& arcToParent,
                                           PcpPrimIndex&& childPrimIndex,
                                           PcpErrorBasePtr* error) {
    PcpNodeRef parent = arcToParent.parent;
    PcpNodeRef newNode = parent.InsertChildSubgraph(childPrimIndex.GetGraph(), arcToParent, error);
    if (!newNode) {
        return newNode;
    }
    ...
    auto startLocalErrorsItr = std::make_move_iterator(childPrimIndex._localErrors->begin());
    auto endLocalErrorsItr = std::make_move_iterator(childPrimIndex._localErrors->end());
    // Inserting moved elements into _localErrors
    _localErrors->insert(_localErrors->end(), startLocalErrorsItr, endLocalErrorsItr);
...
template <class T>
static bool _CheckIfEquivalent(const T* lhsPtr, const T* rhsPtr) {
    if (lhsPtr == rhsPtr) {
        return true;
    }
...
bool PcpPrimIndexInputs::IsEquivalentTo(const PcpPrimIndexInputs& inputs) const {
    // Don't consider the PcpCache when determining equivalence, as
    // prim index computation is independent of the cache.
    return _CheckIfEquivalent(variantFallbacks, inputs.variantFallbacks) &&
           _CheckIfEquivalent(includedPayloads, inputs.includedPayloads) && cull == inputs.cull;
}
...
PcpNodeRef PcpPrimIndexOutputs::Append(PcpPrimIndexOutputs&& childOutputs,
                                       ...
                                       PcpErrorBasePtr* error) {
    PcpNodeRef newNode = primIndex.AddChildPrimIndex(arcToParent, std::move(childOutputs.primIndex), error);

    if (!newNode) {
        return newNode;
    }

    dynamicFileFormatDependency.AppendDependencyData(std::move(childOutputs.dynamicFileFormatDependency));

    expressionVariablesDependency.AppendDependencyData(std::move(childOutputs.expressionVariablesDependency));

    culledDependencies.insert(culledDependencies.end(),
                              std::make_move_iterator(childOutputs.culledDependencies.begin()),
                              std::make_move_iterator(childOutputs.culledDependencies.end()));

    allErrors.insert(allErrors.end(), childOutputs.allErrors.begin(), childOutputs.allErrors.end());
    ...
    if (childOutputs.payloadState == NoPayload) {
        ...
        TF_WARN("Inconsistent payload states for primIndex <%s> -- "
                ...
                primIndex.GetPath().GetText(), payloadState, childOutputs.payloadState, payloadState);
    }
...
static void Pcp_BuildPrimIndex(const PcpLayerStackSite& site,
...
static inline bool _NodeCanBeCulled(const PcpNodeRef& node, const PcpLayerStackSite& rootLayerStack);

static void _GatherNodesRecursively(const PcpNodeRef& node, std::vector<PcpNodeRef>* result);

static bool _HasSpecializesChild(const PcpNodeRef& parent) {
    TF_FOR_ALL(child, Pcp_GetChildrenRange(parent)) {
        if (PcpIsSpecializeArc((*child).GetArcType())) return true;
    }
    return false;
}
...
static PcpNodeRef _FindStartingNodeForImpliedSpecializes(const PcpNodeRef& node) {
    PcpNodeRef specializesNode;
    for (PcpNodeRef n = node, e = n.GetRootNode(); n != e; n = n.GetParentNode()) {
...
static bool _HasClassBasedChild(const PcpNodeRef& parent) {
    TF_FOR_ALL(child, Pcp_GetChildrenRange(parent)) {
        if (PcpIsClassBasedArc((*child).GetArcType())) return true;
    }
    return false;
}
...
static PcpNodeRef _FindStartingNodeForImpliedClasses(const PcpNodeRef& n) {
    TF_VERIFY(PcpIsClassBasedArc(n.GetArcType()));

    PcpNodeRef startNode = n;

    while (PcpIsClassBasedArc(startNode.GetArcType())) {
        const std::pair<PcpNodeRef, PcpNodeRef> instanceAndClass = Pcp_FindStartingNodeOfClassHierarchy(startNode);
        ...
        if (PcpIsClassBasedArc(instanceNode.GetArcType())) {
            const SdfPath ancestralClassPath = instanceNode.GetPathAtIntroduction();
            const bool classHierarchyIsChildOfAncestralHierarchy = classNode.GetPath().HasPrefix(ancestralClassPath);
...
static PcpMapExpression _CreateMapExpressionForArc(const SdfPath& sourcePath,
    ...
    PcpMapExpression arcExpr = PcpMapExpression::Constant(PcpMapFunction::Create(sourceToTargetMap, offset));

    // Apply relocations that affect namespace at and below this site if there
    // are relocations to map.
    if (PcpMapExpression reloMapExpr = targetNode.GetLayerStack()->GetExpressionForRelocatesAtPath(targetPath);
        !reloMapExpr.IsNull()) {
...
inline static size_t _ScanArcs(PcpNodeRef const& node) {
...
inline static size_t _ScanAncestralArcs(PcpNodeRef const& node) {
    ...
    if (const size_t restrictedDepth = node.GetSpecContributionRestrictedDepth(); restrictedDepth != 0) {
...
struct Task {
            ...
            switch (a.type) {
                ...
                case EvalNodeVariantFallback:
                    ...
                    if (a.node != b.node) {
                        ...
                        return std::tie(a.vsetPath, a.vsetNum) > std::tie(b.vsetPath, b.vsetNum);
                    }
                ...
                case EvalNodeVariantNoneFound:
                    ...
                    // be arbitrary.
                    return std::tie(a.node, a.vsetPath, a.vsetNum) > std::tie(b.node, b.vsetPath, b.vsetNum);
    ...
    explicit Task(Type type, const PcpNodeRef& node = PcpNodeRef()) : type(type), vsetNum(0), node(node) {}

    Task(Type type, const PcpNodeRef& node, const SdfPath& vsetPath, std::string&& vsetName, int vsetNum)
        : type(type), vsetNum(vsetNum), node(node), vsetName(std::move(vsetName)), vsetPath(vsetPath) {}

    Task(Type type, const PcpNodeRef& node, const SdfPath& vsetPath, std::string const& vsetName, int vsetNum)
        : Task(type, node, vsetPath, std::string(vsetName), vsetNum) {}

    // TfHash support.
    template <class HashState>
    friend void TfHashAppend(HashState& h, Task const& task) {
        h.Append(task.type, task.node, task.vsetNum, task.vsetName, task.vsetPath);
    }

    inline bool operator==(Task const& rhs) const {
        return type == rhs.type && node == rhs.node && vsetPath == rhs.vsetPath && vsetName == rhs.vsetName &&
               vsetNum == rhs.vsetNum;
    }
    ...
    friend std::ostream& operator<<(std::ostream& os, Task const& task) {
        ...
        os << TfStringPrintf("Task(type=%s, node=%s, nodePath=<%s>, nodeSite=<%s>", TfEnum::GetName(task.type).c_str(),
                             bytes.c_str(), task.node.GetPath().GetText(), TfStringify(task.node.GetSite()).c_str());
        if (!task.vsetName.empty()) {
            os << TfStringPrintf(", vsetPath=%s, vsetName=%s, vsetNum=%d", task.vsetPath.GetText(),
...
struct Pcp_PrimIndexer {
    ...
    using _VariantTraversalCaches = std::unordered_map<std::pair<PcpNodeRef, SdfPath>, _VariantTraversalCache, TfHash>;
    ...
    inline PcpPrimIndex const* GetOriginatingIndex() const { return _GetOriginatingIndex(previousFrame, outputs); }

    _VariantTraversalCache& GetVariantTraversalCache(PcpNodeRef const& node, SdfPath const& pathInNode) {
        if (!variantTraversalCache) {
            variantTraversalCache.emplace();
        }

        return variantTraversalCache->try_emplace({node, pathInNode}, node, pathInNode).first->second;
    }

    // Helper for mapping payload inclusion paths correctly to a node's parent
    static inline SdfPath _MapPathToNodeParentPayloadInclusionPath(PcpMapExpression const& mapToParentExpr,
                                                                   PcpArcType arcType,
                                                                   SdfPath const& path) {
        ...
        const bool isInternalReferenceOrPayload =
                mapToParent.HasRootIdentity() && (arcType == PcpArcTypeReference || arcType == PcpArcTypePayload);
        if (isInternalReferenceOrPayload) {
            ...
            PcpMapFunction::PathMap sourceToTargetMap = mapToParent.GetSourceToTargetMap();
            sourceToTargetMap.erase(SdfPath::AbsoluteRootPath());
            PcpMapFunction newMapFunction = PcpMapFunction::Create(sourceToTargetMap, mapToParent.GetTimeOffset());
    ...
    // prim index root.
    static inline SdfPath _MapPathToNodeRootPayloadInclusionPath(PcpNodeRef const& node, SdfPath const& path) {
        ...
        if (mappedPath == path && mapToRoot.HasRootIdentity()) {
            for (PcpNodeRef curNode = node; !mappedPath.IsEmpty() && !curNode.IsRootNode();
                 curNode = curNode.GetParentNode()) {
                mappedPath = _MapPathToNodeParentPayloadInclusionPath(curNode.GetMapToParent(), curNode.GetArcType(),
                                                                      mappedPath);
            }
        }
        return mappedPath;
    ...
    SdfPath MapNodePathToPayloadInclusionPath(PcpNodeRef const& node, SdfPath const& path) const {
        // First, map the node's path to the payload inclusion path for the root
        // of the prim index it's in.
        SdfPath p = _MapPathToNodeRootPayloadInclusionPath(node, path.StripAllVariantSelections());

        // If we're in a recursive prim indexing call, we need to map the
        // path across stack frames.
        for (PcpPrimIndex_StackFrameIterator it(node, previousFrame); !p.IsEmpty() && it.previousFrame;
             it.NextFrame()) {
            ...
            p = _MapPathToNodeParentPayloadInclusionPath(it.previousFrame->arcToParent->mapToParent,
                                                         it.previousFrame->arcToParent->type, p);
            ...
            // frame.
            p = _MapPathToNodeRootPayloadInclusionPath(it.previousFrame->parentNode, p);
        };
    ...
    static inline bool _IsImpliedTaskType(Task::Type taskType) {
        ...
        return (taskType == Task::Type::EvalImpliedClasses) | (taskType == Task::Type::EvalImpliedSpecializes);
    }
    ...
    inline void _AddTasksForNodeRecursively(const PcpNodeRef& n,
                                            ...
                                            bool isUsd) {
#ifdef PCP_DIAGNOSTIC_VALIDATION
        TF_VERIFY(seen.count(n) == 0, "Already processed <%s>", n.GetPath().GetText());
        seen.insert(n);
#endif  // PCP_DIAGNOSTIC_VALIDATION

        TF_FOR_ALL(child, Pcp_GetChildrenRange(n)) {
            _AddTasksForNodeRecursively(*child, skipTasksForExpressedArcs, skipCompletedNodesForImpliedSpecializes,
                                        evaluateUnresolvedPrimPathErrors, evaluateAncestralVariantsAndDynamicPayloads,
                                        isUsd);
        }
        ...
        if (evaluateUnresolvedPrimPathErrors &&
            (n.GetArcType() == PcpArcTypeReference || n.GetArcType() == PcpArcTypePayload)) {
    ...
    void AddTasksForRootNode(const PcpNodeRef& rootNode) {
        return _AddTasksForNodeRecursively(rootNode,
    ...
    void AddTasksForNode(const PcpNodeRef& n,
        ...
        _AddTasksForNodeRecursively(n, skipTasksForExpressedArcs, skipCompletedNodesForImpliedSpecializes,
                                    evaluateUnresolvedPrimPathErrors, evaluateAncestralVariantsAndDynamicPayloads,
                                    inputs.usd);
    ...
    void RetryVariantTasks() {
        ...
        for (auto i = tasks.begin(), e = tasks.end(); i != e; ++i) {
            Task& t = *i;
            if (t.type == Task::Type::EvalNodeVariantFallback || t.type == Task::Type::EvalNodeVariantNoneFound) {
    ...
    void RecordError(const PcpErrorBasePtr& err) { RecordError(err, &outputs->primIndex, &outputs->allErrors); }

    // Convenience function to record an error both in this primIndex's
    // local errors vector and the allErrors vector.
    static void RecordError(const PcpErrorBasePtr& err, PcpPrimIndex* primIndex, PcpErrorVector* allErrors) {
...
static void _InertSubtree(PcpNodeRef node) {
    node.SetInert(true);
...
inline static bool _HasAncestorCycle(const PcpLayerStackSite& parentNodeSite, const PcpLayerStackSite& childNodeSite) {
    ...
    return parentNodeSite.layerStack == childNodeSite.layerStack &&
           (parentNodeSite.path.HasPrefix(childNodeSite.path) || childNodeSite.path.HasPrefix(parentNodeSite.path));
}

inline static bool _FindAncestorCycleInParentGraph(const PcpNodeRef& parentNode,
...
static bool _IsImpliedClassBasedArc(PcpArcType arcType, const PcpNodeRef& parent, const PcpNodeRef& origin) {
    return PcpIsClassBasedArc(arcType) && parent != origin;
}

static bool _IsImpliedClassBasedArc(const PcpNodeRef& node) {
    return _IsImpliedClassBasedArc(node.GetArcType(), node.GetParentNode(), node.GetOriginNode());
}

// Check that no cycles are being introduced by adding this arc.
static PcpErrorArcCyclePtr _CheckForCycle(const PcpNodeRef& parent,
    ...
    if (_IsImpliedClassBasedArc(arcType, parent, origin)) {
        ...
        while (j.node && _IsImpliedClassBasedArc(j.GetArcType(), parent, origin)) {
            j.Next();
        }
    ...
    for (PcpPrimIndex_StackFrameIterator it(parent, previousFrame); it.node; it.NextFrame()) {
    ...
    if (foundCycle) {
        ...
        for (PcpPrimIndex_StackFrameIterator i(parent, previousFrame); i.node; i.Next()) {
...
static PcpNodeRef _AddArc(Pcp_PrimIndexer* indexer,
    ...
    PCP_INDEXING_PHASE(indexer, parent, "Adding new %s arc to %s from %s", TfEnum::GetDisplayName(arcType).c_str(),
                       Pcp_FormatSite(site).c_str(), Pcp_FormatSite(parent.GetSite()).c_str());
    ...
    PCP_INDEXING_MSG(indexer, parent,
                     ...
                     opts.includeAncestralOpinions ? "true" : "false", opts.skipDuplicateNodes ? "true" : "false",
                     indexer->previousFrame
                             ? TfStringPrintf(" (prev. frame: %s)",
    ...
    if (PcpErrorArcCyclePtr err = _CheckForCycle(parent, origin, arcType, site, indexer->previousFrame)) {
    ...
    if (opts.skipDuplicateNodes) {
        ...
        for (PcpPrimIndex_StackFrameIterator it(parent, indexer->previousFrame); it.node; it.NextFrame()) {
            ...
            if (it.previousFrame) {
                const SdfPath& requestedPathForCurrentGraph = it.previousFrame->requestedSite.path;
                const SdfPath& currentPathForCurrentGraph = currentGraph->GetRootNode().GetPath();

                siteToAddInCurrentGraph.path = requestedPathForCurrentGraph.ReplacePrefix(currentPathForCurrentGraph,
                                                                                          siteToAddInCurrentGraph.path);
            }
        }

        if (foundDuplicateNode) {
            PCP_INDEXING_MSG(indexer, parent, "Skipping because duplicate node exists.");
            return PcpNodeRef();
        }
    }
    ...
    if (!opts.includeAncestralOpinions) {
        ...
        if (newNode) {
            ...
            if (!newNode.IsInert() && newNode.HasSpecs()) {
                if (!indexer->inputs.usd) {
                    ...
                    newNode.SetPermission(PcpComposeSitePermission(site.layerStack, site.path));

                    // Determine whether this node has any symmetry information.
                    newNode.SetHasSymmetry(PcpComposeSiteHasSymmetry(site.layerStack, site.path));
                }
            }

            PCP_INDEXING_UPDATE(indexer, newNode, "Added new node for site %s to graph", TfStringify(site).c_str());
        }
        ...
        PCP_INDEXING_MSG(indexer, parent,
                         ...
                         TfEnum::GetDisplayName(arcType).c_str(), Pcp_FormatSite(site).c_str());
        ...
        PcpPrimIndex_StackFrame frame(site, parent, &newArc, indexer->previousFrame, indexer->GetOriginatingIndex(),
                                      opts.skipDuplicateNodes);

        PcpPrimIndexOutputs childOutputs;
        Pcp_BuildPrimIndex(site, indexer->rootSite, indexer->ancestorRecursionDepth, evaluateImpliedSpecializes,
                           evaluateVariantsAndDynamicPayloads, opts.directNodeShouldContributeSpecs, &frame,
                           indexer->inputs, &childOutputs);

        // Combine the child output with our current output.
        newNode = indexer->outputs->Append(std::move(childOutputs), newArc, &newNodeError);
        if (newNode) {
            PCP_INDEXING_UPDATE(indexer, newNode, "Added subtree for site %s to graph", TfStringify(site).c_str());
        }
    }
    ...
    if (!newNode) {
        TF_VERIFY(newNodeError,
                  "Failed to create a node, but did not "
                  "specify the error.");
        return PcpNodeRef();
    }
    ...
    const bool evaluateAncestralVariantsAndDynamicPayloads =
            indexer->evaluateVariantsAndDynamicPayloads && opts.includeAncestralOpinions;

    // Enqueue tasks to evaluate the new nodes.
    indexer->AddTasksForNode(newNode, opts.skipTasksForExpressedArcs, opts.skipImpliedSpecializesCompletedNodes,
...
static PcpNodeRef _AddArc(Pcp_PrimIndexer* indexer,
    ...
    const int namespaceDepth = PcpNode_GetNonVariantPathElementCount(parent.GetPath());

    return _AddArc(indexer, arcType, parent, origin, site, mapExpr, arcSiblingNum, namespaceDepth, options);
}

////////////////////////////////////////////////////////////////////////
// References

static SdfPath _GetDefaultPrimPath(SdfLayerHandle const& layer) {
...
PcpDynamicFileFormatContext Pcp_CreateDynamicFileFormatContext(
        const PcpNodeRef&, const SdfPath&, int arcNum, PcpPrimIndex_StackFrame*, TfToken::Set*, TfToken::Set*);

// Determine whether the current payload at assetPath is
// static or dynamic.
static PcpDynamicFileFormatInterface* _GetDynamicFileFormat(const SdfPayload& payload,
    ...
    if (!assetPath.empty()) {
        SdfFileFormatConstPtr fileFormat =
                SdfFileFormat::FindByExtension(SdfFileFormat::GetFileExtension(assetPath), fileFormatTarget);
        if (fileFormat) {
            dynamicFileFormat = const_cast<PcpDynamicFileFormatInterface*>(
                    dynamic_cast<const PcpDynamicFileFormatInterface*>(get_pointer(fileFormat)));
        }
    }
...
static void _ComposeFieldsForFileFormatArguments(const PcpNodeRef& node,
    ...
    PcpDynamicFileFormatContext context = Pcp_CreateDynamicFileFormatContext(
            node, nodePathAtIntroduction, arcNum, indexer.previousFrame, &composedFieldNames, &composedAttributeNames);
    // Ask the file format to generate dynamic file format arguments for
    // the asset in this context.
    VtValue dependencyContextData;
    dynamicFileFormat->ComposeFieldsForFileFormatArguments(payload.GetAssetPath(), context, args,
                                                           &dependencyContextData);
    ...
    indexer.outputs->dynamicFileFormatDependency.AddDependencyContext(
            dynamicFileFormat, std::move(dependencyContextData), std::move(composedFieldNames),
            std::move(composedAttributeNames));
}
...
static void _EvalRefOrPayloadArcs(PcpNodeRef node,
    ...
    for (size_t i = 0; i < arcs.size(); ++i) {
        ...
        PCP_INDEXING_MSG(indexer, node, "Found %s to @%s@<%s>", ARC_TYPE == PcpArcTypePayload ? "payload" : "reference",
                         info.authoredAssetPath.c_str(), refOrPayload.GetPrimPath().GetText());
        ...
        if (!refOrPayload.GetPrimPath().IsEmpty() &&
            !(refOrPayload.GetPrimPath().IsAbsolutePath() && refOrPayload.GetPrimPath().IsPrimPath() &&
        ...
        if (!layerOffset.IsValid() || !layerOffset.GetInverse().IsValid()) {
        ...
        if (isInternal) {
            ...
            if (indexer->inputs.cache->IsLayerMuted(srcLayer, info.authoredAssetPath, &canonicalMutedLayerId)) {
            ...
            _ComposeFieldsForFileFormatArguments(node, *indexer, refOrPayload, nodePathAtIntroduction, info.arcNum,
                                                 &args);
            Pcp_GetArgumentsForFileFormatTarget(refOrPayload.GetAssetPath(), indexer->inputs.fileFormatTarget, &args);
            ...
            if (!layer) {
                ...
                if (!m.IsClean()) {
                    ...
                    m.Clear();
                    err->messages = TfStringJoin(commentary.begin(), commentary.end(), "; ");
                }
                indexer->RecordError(err);
                continue;
            }

            const ArResolverContext& pathResolverContext = node.GetLayerStack()->GetIdentifier().pathResolverContext;
            ...
            layerStack = indexer->inputs.cache->ComputeLayerStack(layerStackIdentifier, &indexer->outputs->allErrors);
            ...
            if (!PcpIsTimeScalingForLayerTimeCodesPerSecondDisabled()) {
                ...
                const double srcTimeCodesPerSecond = srcLayer->GetTimeCodesPerSecond();
                const double destTimeCodesPerSecond = layerStack->GetTimeCodesPerSecond();
                if (srcTimeCodesPerSecond != destTimeCodesPerSecond) {
                    layerOffset.SetScale(layerOffset.GetScale() * srcTimeCodesPerSecond / destTimeCodesPerSecond);
                }
            }
        }
        ...
        if (refOrPayload.GetPrimPath().IsEmpty()) {
            ...
            if (defaultPrimPath.IsEmpty()) {
                ...
                err->unresolvedPath = SdfPath::ReflexiveRelativePath().AppendChild(SdfFieldKeys->DefaultPrim);
                err->sourceLayer = srcLayer;
                err->arcType = ARC_TYPE;
                indexer->RecordError(err);
        ...
        SdfPath primPath = defaultPrimPath.IsEmpty() ? refOrPayload.GetPrimPath() : defaultPrimPath;
        ...
        PcpMapExpression mapExpr = _CreateMapExpressionForArc(
                /* source */ primPath, /* targetNode */ node, indexer->inputs, layerOffset);
        if (isInternal) {
        ...
        const int namespaceDepth = PcpNode_GetNonVariantPathElementCount(nodePathAtIntroduction);

        _AddArc(indexer, ARC_TYPE,
                /* parent = */ node,
                /* origin = */ node, PcpLayerStackSite(layerStack, primPath), mapExpr, info.arcNum, namespaceDepth,
                opts);
    }
}

static void _EvalNodeReferences(PcpNodeRef node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating references at %s", Pcp_FormatSite(node.GetSite()).c_str());

    if (!node.CanContributeSpecs()) return;
    ...
    PcpComposeSiteReferences(node, &refArcs, &refInfo, &exprVarDependencies, &errors);

    if (!exprVarDependencies.empty()) {
        indexer->outputs->expressionVariablesDependency.AddDependencies(node.GetLayerStack(),
                                                                        std::move(exprVarDependencies));
    }
    ...
    _EvalRefOrPayloadArcs<SdfReference, PcpArcTypeReference>(node, indexer, refArcs, refInfo, node.GetPath());
}

////////////////////////////////////////////////////////////////////////
// Payload

static void _EvalNodePayloads(const PcpNodeRef& node,
                              Pcp_PrimIndexer* indexer,
                              Task::Type payloadType,
                              const SdfPath& nodePathAtIntroduction) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating payload for %s", Pcp_FormatSite(node.GetSite()).c_str());
    ...
    PcpComposeSitePayloads(node.GetLayerStack(), nodePathAtIntroduction, &payloadArcs, &payloadInfo,
    ...
    PCP_INDEXING_MSG(indexer, node, "Found payload for node %s", nodePathAtIntroduction.GetText());
    ...
    const PcpPrimIndexInputs::PayloadSet* includedPayloads = indexer->inputs.includedPayloads;
    ...
    const bool keepDynamicPayloads = (payloadType == Task::Type::EvalNodeDynamicPayloads);
    ...
    while (payloadIt != payloadArcs.end()) {
        const bool isDynamicPayload = _GetDynamicFileFormat(*payloadIt, indexer->inputs.fileFormatTarget);
    ...
    // computation.
    SdfPath const path = indexer->MapNodePathToPayloadInclusionPath(node, nodePathAtIntroduction);
    ...
    if (path.IsEmpty()) {
        ...
        _EvalRefOrPayloadArcs<SdfPayload, PcpArcTypePayload>(node, indexer, payloadArcs, payloadInfo, node.GetPath());
        ...
        indexer->outputs->payloadState =
                composePayload ? PcpPrimIndexOutputs::IncludedByPredicate : PcpPrimIndexOutputs::ExcludedByPredicate;
    } else {
        tbb::spin_rw_mutex::scoped_lock lock;
        auto* mutex = indexer->inputs.includedPayloadsMutex;
        if (mutex) {
            lock.acquire(*mutex, /*write=*/false);
        }
        composePayload = includedPayloads->count(path);
        indexer->outputs->payloadState =
                composePayload ? PcpPrimIndexOutputs::IncludedByIncludeSet : PcpPrimIndexOutputs::ExcludedByIncludeSet;
    }

    if (!composePayload) {
        PCP_INDEXING_MSG(indexer, node, "Payload <%s> was not included, skipping", path.GetText());
        return;
    }

    _EvalRefOrPayloadArcs<SdfPayload, PcpArcTypePayload>(node, indexer, payloadArcs, payloadInfo,
...
static bool _PrimSpecExistsUnderNode(const PcpNodeRef& node, const SpecExistsFn& specExists) {
    if (specExists(node)) {
        return true;
    }
...
static bool _PrimSpecExistsUnderNodeAtIntroduction(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    ...
    return node.GetDepthBelowIntroduction() == 0
                   ? _PrimSpecExistsUnderNode(node,
                                              [](const PcpNodeRef& node) {
                                                  return node.HasSpecs();
                                              })
                   : _PrimSpecExistsUnderNode(node, [](const PcpNodeRef& node) {
                         return PcpComposeSiteHasPrimSpecs(node.GetLayerStack(), node.GetPathAtIntroduction());
                     });
}

static void _EvalUnresolvedPrimPathError(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    ...
    if (!_PrimSpecExistsUnderNodeAtIntroduction(node, indexer)) {
        ...
        SdfPath parentNodePath = node.GetMapToParent().MapSourceToTarget(pathAtIntroduction);

        // Map the node path at introduction to the parent node, also applying
        // any variant selections from the parent path.
        if (const SdfPath parentPath = parentNode.GetPath(); parentPath.ContainsPrimVariantSelection()) {
            parentNodePath = parentNodePath.ReplacePrefix(parentPath.StripAllVariantSelections(), parentPath);
        }
        ...
        err->sourceLayer = [&]() {
            ...
            switch (node.GetArcType()) {
                case PcpArcTypeReference: {
                    ...
                    PcpComposeSiteReferences(parentNode.GetLayerStack(), parentNodePath, &unused, &srcInfo);
                    break;
                }

                case PcpArcTypePayload: {
                    SdfPayloadVector unused;
                    PcpComposeSitePayloads(parentNode.GetLayerStack(), parentNodePath, &unused, &srcInfo);
                    break;
                }
            ...
            const size_t arcNum = static_cast<size_t>(node.GetSiblingNumAtOrigin());
            return TF_VERIFY(arcNum < srcInfo.size()) ? srcInfo[arcNum].sourceLayer : SdfLayerHandle();
        }();
...
static void _ElideSubtree(PcpNodeRef node, bool cull) {
    if (cull) {
        node.SetCulled(true);
    } else {
        node.SetInert(true);
...
static void _ElideRelocatedSubtrees(const Pcp_PrimIndexer& indexer, PcpNodeRef node) {
    TF_FOR_ALL(it, Pcp_GetChildrenRange(node)) {
        ...
        if (childNode.CanContributeSpecs()) {
            ...
            if (relocatesSrcToTarget.find(childNode.GetPath()) != relocatesSrcToTarget.end()) {
...
// source via _AddArc().
static void _EvalNodeRelocations(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating relocations under %s", Pcp_FormatSite(node.GetSite()).c_str());
    ...
    const SdfRelocatesMap& relocatesTargetToSource = node.GetLayerStack()->GetIncrementalRelocatesTargetToSource();
    SdfRelocatesMap::const_iterator i = relocatesTargetToSource.find(node.GetPath());
    ...
    TF_FOR_ALL(childIt, Pcp_GetChildrenRange(node)) {
        ...
        PCP_INDEXING_UPDATE(indexer, child, "Elided subtree that will be superceded by relocation source <%s>",
                            relocSource.GetText());
    }
    ...
    PcpNodeRef newNode = _AddArc(indexer, PcpArcTypeRelocate,
                                 /* parent = */ node,
                                 /* origin = */ node, PcpLayerStackSite(node.GetLayerStack(), relocSource),
                                 identityMapExpr, arcSiblingNum, opts);
    ...
    if (newNode) {
        ...
        TF_FOR_ALL(site, sites) {
            PcpErrorOpinionAtRelocationSourcePtr err = PcpErrorOpinionAtRelocationSource::New();
            err->rootSite = PcpSite(node.GetRootNode().GetSite());
...
static void _EvalImpliedRelocations(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    if (node.GetArcType() != PcpArcTypeRelocate || node.IsDueToAncestor()) {
        return;
    }

    PCP_INDEXING_PHASE(indexer, node, "Evaluating relocations implied by %s", Pcp_FormatSite(node.GetSite()).c_str());
    ...
    if (const PcpNodeRef parent = node.GetParentNode()) {
        if (const PcpNodeRef gp = parent.GetParentNode()) {
            ...
            const SdfPath gpRelocSource = parent.GetMapToParent().MapSourceToTarget(node.GetPath());
            if (gpRelocSource.IsEmpty()) {
                PCP_INDEXING_PHASE(indexer, node, "No implied site for relocation source -- skipping");
                return;
            }

            PCP_INDEXING_PHASE(indexer, node, "Propagating relocate from %s to %s",
                               Pcp_FormatSite(node.GetSite()).c_str(), gpRelocSource.GetText());

            // Check if this has already been propagated.
            TF_FOR_ALL(gpChildIt, Pcp_GetChildrenRange(gp)) {
                const PcpNodeRef& gpChild = *gpChildIt;
                if (gpChild.GetPath() == gpRelocSource && gpChild.GetArcType() == PcpArcTypeRelocate) {
                    PCP_INDEXING_PHASE(indexer, node, "Relocate already exists -- skipping");
                    return;
                }
            }
            ...
            _AddArc(indexer, PcpArcTypeRelocate,
                    /* parent = */ gp,
                    /* origin = */ node, PcpLayerStackSite(gp.GetLayerStack(), gpRelocSource),
                    PcpMapExpression::Identity(),
                    /* arcSiblingNum = */ 0, opts);
        }
    }
}
...
// node.
static PcpNodeRef _FindMatchingChild(const PcpNodeRef& parent,
    ...
    TF_FOR_ALL(childIt, Pcp_GetChildrenRange(parent)) {
        ...
        if (parentArcType == PcpArcTypeRelocate) {
            if (child.GetArcType() == arcType && child.GetMapToParent().Evaluate() == mapToParent.Evaluate() &&
                child.GetOriginNode().GetDepthBelowIntroduction() == depthBelowIntroduction) {
                return child;
            }
        } else {
...
static SdfPath _FindContainingVariantSelection(SdfPath p) {
...
static SdfPath _DetermineInheritPath(const SdfPath& parentPath, const PcpMapExpression& inheritMap) {
    ...
    if (!parentPath.ContainsPrimVariantSelection()) {
        ...
        return inheritMap.MapTargetToSource(parentPath.StripAllVariantSelections())
...
static PcpNodeRef _AddClassBasedArc(PcpArcType arcType,
                                    PcpNodeRef parent,
                                    PcpNodeRef origin,
                                    ...
                                    Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, parent, "Preparing to add %s arc to %s", TfEnum::GetDisplayName(arcType).c_str(),
    ...
    PCP_INDEXING_MSG(indexer, parent,
                     ...
                     Pcp_FormatSite(origin.GetSite()).c_str(), inheritArcNum,
                     ignoreIfSameAsSite == PcpLayerStackSite() ? "<none>" : Pcp_FormatSite(ignoreIfSameAsSite).c_str());

    // Use the inherit map to figure out the site path to inherit.
    SdfPath inheritPath = _DetermineInheritPath(parent.GetPath(), inheritMap);
    ...
    // determine that.
    const PcpArcType parentArcType = PcpPrimIndex_StackFrameIterator(parent, indexer->previousFrame).GetArcType();

    if (!inheritPath.IsEmpty()) {
        PCP_INDEXING_MSG(indexer, parent, "Inheriting from path <%s>", inheritPath.GetText());
    } else {
        ...
        PCP_INDEXING_MSG(indexer, parent, "No appropriate site for inheriting opinions");
        return PcpNodeRef();
    }
    ...
    if (PcpNodeRef child = _FindMatchingChild(parent, parentArcType, inheritSite, arcType, inheritMap,
                                              origin.GetDepthBelowIntroduction())) {
        PCP_INDEXING_MSG(indexer, parent, child, TfEnum::GetDisplayName(arcType).c_str(),
                         "A %s arc to <%s> already exists. Skipping.", inheritPath.GetText());
    ...
    //
    opts.directNodeShouldContributeSpecs = (inheritPath != parent.GetPath()) && (inheritSite != ignoreIfSameAsSite);
    ...
    opts.includeAncestralOpinions = opts.directNodeShouldContributeSpecs && !inheritPath.IsRootPrimPath();

    return _AddArc(indexer, arcType, parent, origin, inheritSite, inheritMap, inheritArcNum, opts);
}

// Helper function for adding a list of class-based arcs under the given
// node in the given prim index.
static void _AddClassBasedArcs(const PcpNodeRef& node,
    ...
    for (size_t arcNum = 0; arcNum < classArcs.size(); ++arcNum) {
        ...
        PCP_INDEXING_MSG(indexer, node, "Found %s to <%s>", TfEnum::GetDisplayName(arcType).c_str(), arcPath.GetText());

        // Verify that the class-based arc (i.e., inherit or specialize)
        // targets a prim path, with no variant selection.
        if (!arcPath.IsEmpty() && !(arcPath.IsPrimPath() && !arcPath.ContainsPrimVariantSelection())) {
            PcpErrorInvalidPrimPathPtr err = PcpErrorInvalidPrimPath::New();
            err->rootSite = PcpSite(node.GetRootNode().GetSite());
            err->site = PcpSite(node.GetSite());
        ...
        PcpMapExpression mapExpr = _CreateMapExpressionForArc(
                                           /* source */ arcPath, /* targetNode */ node, indexer->inputs)
                                           .AddRootIdentity();
...
///
static PcpMapExpression _GetImpliedClass(const PcpMapExpression& transfer, const PcpMapExpression& classArc) {
...
static void _EvalImpliedClassTree(PcpNodeRef destNode,
                                  PcpNodeRef srcNode,
    ...
    if (destNode.GetArcType() == PcpArcTypeRelocate) {
        ...
        const PcpMapExpression newTransferFunc = destNode.GetMapToParent().AddRootIdentity().Compose(transferFunc);
        _EvalImpliedClassTree(destNode.GetParentNode(), srcNode, newTransferFunc, srcNodeIsStartOfTree, indexer);
    ...
    for (const PcpNodeRef& srcChild : Pcp_GetChildren(srcNode)) {
        ...
        if (!PcpIsClassBasedArc(srcChild.GetArcType())) continue;

        PCP_INDEXING_MSG(indexer, srcChild, destNode, "Attempting to propagate %s of %s to %s.",
                         TfEnum::GetDisplayName(srcChild.GetArcType()).c_str(),
                         Pcp_FormatSite(srcChild.GetSite()).c_str(), Pcp_FormatSite(destNode.GetSite()).c_str());
        ...
        //
        if (srcNodeIsStartOfTree && PcpIsClassBasedArc(srcNode.GetArcType()) &&
            srcNode.GetDepthBelowIntroduction() == srcChild.GetDepthBelowIntroduction()) {
            PCP_INDEXING_MSG(indexer, srcChild, destNode, "Skipping ancestral class");
            continue;
        }

        // Determine the equivalent class mapping under destNode.
        PcpMapExpression destClassFunc = _GetImpliedClass(transferFunc, srcChild.GetMapToParent());

        PCP_INDEXING_MSG(indexer, srcChild, destNode, "Transfer function:\n%s", transferFunc.GetString().c_str());
        PCP_INDEXING_MSG(indexer, srcChild, destNode, "Implied class:\n%s", destClassFunc.GetString().c_str());
        ...
        TF_FOR_ALL(destChildIt, Pcp_GetChildrenRange(destNode)) {
            if (destChildIt->GetOriginNode() == srcChild &&
                destChildIt->GetMapToParent().Evaluate() == destClassFunc.Evaluate()) {
                destChild = *destChildIt;

                PCP_INDEXING_MSG(indexer, srcChild, destChild, "Found previously added implied inherit node");
                break;
            }
        }
        ...
        //
        if (!destChild) {
            destChild = _AddClassBasedArc(srcChild.GetArcType(),
                                          /* parent = */ destNode,
                                          /* origin = */ srcChild, destClassFunc, srcChild.GetSiblingNumAtOrigin(),
                                          /* ignoreIfSameAsSite = */ srcChild.GetSite(), indexer);
        }
        ...
        if (destChild && _HasClassBasedChild(srcChild)) {
            ...
            PcpMapExpression childTransferFunc =
                    destClassFunc.Inverse().Compose(transferFunc.Compose(srcChild.GetMapToParent()));

            _EvalImpliedClassTree(destChild, srcChild, childTransferFunc,
                                  /* srcNodeIsStartOfTree = */ false, indexer);
        }
    }
}

static bool _IsPropagatedSpecializesNode(const PcpNodeRef& node);

static void _EvalImpliedClasses(PcpNodeRef node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating implied classes at %s", Pcp_FormatSite(node.GetSite()).c_str());
    ...
    _EvalImpliedClassTree(node.GetParentNode(), node, transferFunc,
...
static void _EvalNodeInherits(PcpNodeRef node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating inherits at %s", Pcp_FormatSite(node.GetSite()).c_str());
...
static void _EvalNodeSpecializes(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating specializes at %s", Pcp_FormatSite(node.GetSite()).c_str());
...
static bool _IsPropagatedSpecializesNode(const PcpNodeRef& node) {
    return (PcpIsSpecializeArc(node.GetArcType()) && node.GetParentNode() == node.GetRootNode() &&
            node.GetSite() == node.GetOriginNode().GetSite());
}

static bool _IsNodeInSubtree(const PcpNodeRef& node, const PcpNodeRef& subtreeRoot) {
...
static std::pair<PcpNodeRef, bool> _PropagateNodeToParent(PcpNodeRef parentNode,
                                                          PcpNodeRef srcNode,
    ...
    if (srcNode.GetParentNode() == parentNode) {
        ...
        newNode = _FindMatchingChild(parentNode, parentNode.GetArcType(), srcNode.GetSite(), srcNode.GetArcType(),
        ...
        if (!newNode) {
            ...
            if (!_IsImpliedClassBasedArc(srcNode) || !_IsNodeInSubtree(srcNode.GetOriginNode(), srcTreeRoot)) {
                const int namespaceDepth =
                        (srcNode == srcTreeRoot ? PcpNode_GetNonVariantPathElementCount(parentNode.GetPath())
                                                : srcNode.GetNamespaceDepth());

                const PcpNodeRef originNode =
                        (srcNode == srcTreeRoot || _IsImpliedClassBasedArc(srcNode) ? srcNode : parentNode);

                _ArcOptions opts;
                opts.directNodeShouldContributeSpecs = !srcNode.IsInert();
                opts.skipImpliedSpecializesCompletedNodes = skipImpliedSpecializes;
                opts.skipTasksForExpressedArcs = skipTasksForExpressedArcs;

                newNode = _AddArc(indexer, srcNode.GetArcType(),
                                  /* parent = */ parentNode,
                                  /* origin = */ originNode, srcNode.GetSite(), mapToParent,
                                  srcNode.GetSiblingNumAtOrigin(), namespaceDepth, opts);
        ...
        if (newNode) {
            const size_t newNodeRestrictedDepth = newNode.GetSpecContributionRestrictedDepth();
            ...
            if (newNodeRestrictedDepth != 0) {
                srcNode.SetSpecContributionRestrictedDepth(newNodeRestrictedDepth);
            }
        } else {
...
static void _PropagateSpecializesTreeToRoot(PcpNodeRef parentNode,
                                            PcpNodeRef srcNode,
    ...
    std::pair<PcpNodeRef, bool> newNode = _PropagateNodeToParent(
            parentNode, srcNode, skipImpliedSpecializes, skipTasksForExpressedArcs, mapToParent, srcTreeRoot, indexer);
    if (!newNode.first) {
        return;
    }
    ...
    for (PcpNodeRef childNode : Pcp_GetChildren(srcNode)) {
        if (!PcpIsSpecializeArc(childNode.GetArcType())) {
            _PropagateSpecializesTreeToRoot(newNode.first, childNode, newNode.first, childNode.GetMapToParent(),
                                            srcTreeRoot, indexer);
        }
    }
}

static void _FindSpecializesToPropagateToRoot(PcpNodeRef node, Pcp_PrimIndexer* indexer) {
    ...
    const PcpNodeRef parentNode = node.GetParentNode();
    const bool nodeIsRelocatesPlaceholder = parentNode != node.GetOriginNode() &&
    ...
    if (PcpIsSpecializeArc(node.GetArcType())) {
        PCP_INDEXING_MSG(indexer, node, node.GetRootNode(), "Propagating specializes arc %s to root",
        ...
        _PropagateSpecializesTreeToRoot(node.GetRootNode(), node, node, node.GetMapToRoot(), node, indexer);
...
static void _PropagateArcsToOrigin(PcpNodeRef parentNode,
                                   PcpNodeRef srcNode,
                                   const PcpMapExpression& mapToParent,
                                   const PcpNodeRef& srcTreeRoot,
                                   Pcp_PrimIndexer* indexer) {
    ...
    for (PcpNodeRef childNode : Pcp_GetChildren(srcNode)) {
        _PropagateArcsToOrigin(newNode.first, childNode, childNode.GetMapToParent(), srcTreeRoot, indexer);
    }
}

static void _FindArcsToPropagateToOrigin(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    TF_VERIFY(PcpIsSpecializeArc(node.GetArcType()));

    for (PcpNodeRef childNode : Pcp_GetChildren(node)) {
        PCP_INDEXING_MSG(indexer, childNode, node.GetOriginNode(), "Propagating arcs under %s to specializes origin %s",
                         Pcp_FormatSite(childNode.GetSite()).c_str(),
                         Pcp_FormatSite(node.GetOriginNode().GetSite()).c_str());

        _PropagateArcsToOrigin(node.GetOriginNode(), childNode, childNode.GetMapToParent(), node, indexer);
    }
}
...
//
static void _EvalImpliedSpecializes(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating implied specializes at %s", Pcp_FormatSite(node.GetSite()).c_str());
...
static bool _NodeCanContributeAncestralOpinions(const PcpNodeRef& node, const SdfPath& ancestralPath) {
    ...
    return restrictionDepth == 0 || restrictionDepth > ancestralPath.GetPathElementCount();
}

static bool _ComposeVariantSelectionForNode(const PcpNodeRef& node,
    ...
    const bool foundSelection =
            PcpComposeSiteVariantSelection(node.GetLayerStack(), pathInNode, vset, vsel, &exprVarDependencies, &errors);
...
static bool _FindPriorVariantSelection(const PcpNodeRef& startNode,
                                       const SdfPath& pathInStartNode,
                                       const std::string& vset,
                                       std::string* vsel,
                                       PcpNodeRef* nodeWithVsel,
                                       Pcp_PrimIndexer* indexer) {
    auto& traverser = indexer->GetVariantTraversalCache(startNode, pathInStartNode);
    ...
    for (auto it = traverser.begin(), e = traverser.end(); it != e; ++it) {
        ...
        if (node.GetArcType() == PcpArcTypeVariant) {
            ...
            const std::pair<std::string, std::string> nodeVsel = nodePathAtIntroduction.GetVariantSelection();
...
static bool _ComposeVariantSelectionAcrossNodes(const PcpNodeRef& startNode,
                                                const SdfPath& pathInStartNode,
                                                const std::string& vset,
                                                std::string* vsel,
                                                PcpNodeRef* nodeWithVsel,
    ...
    for (auto it = traverser.begin(), e = traverser.end(); it != e; ++it) {
        ...
        if (info.status == Info::Unknown) {
            info.sitePath = [&node = node, &pathInNode = pathInNode]() {
                ...
                if (node.GetArcType() == PcpArcTypeVariant) {
                    ...
                    return pathInNode.ReplacePrefix(variantPath.StripAllVariantSelections(), variantPath);
                }
                return pathInNode;
            }();

            info.status = PcpComposeSiteHasVariantSelections(node.GetLayerStack(), info.sitePath)
                                  ? Info::AuthoredSelections
                                  : Info::NoSelections;
        }
        ...
        if (_ComposeVariantSelectionForNode(node, info.sitePath, vset, vsel, indexer)) {
            *nodeWithVsel = node;
            return true;
        }
    }
...
static void _ComposeVariantSelection(const PcpNodeRef& node,
                                     const SdfPath& pathInNode,
                                     Pcp_PrimIndexer* indexer,
                                     const std::string& vset,
                                     std::string* vsel,
    ...
    TF_VERIFY(!pathInNode.ContainsPrimVariantSelection(), "%s", pathInNode.GetText());
    ...
    const auto [pathInStartNode, startNode] = Pcp_TranslatePathFromNodeToRootOrClosestNode(node, pathInNode);
    ...
    // prim index.
    if (_FindPriorVariantSelection(startNode, pathInStartNode, vset, vsel, nodeWithVsel, indexer)) {
        PCP_INDEXING_MSG(indexer, node, *nodeWithVsel, "Found prior variant selection {%s=%s} at %s", vset.c_str(),
                         vsel->c_str(), Pcp_FormatSite(nodeWithVsel->GetSite()).c_str());
        return;
    }

    // Otherwise, search all nodes to find the strongest variant selection.
    if (_ComposeVariantSelectionAcrossNodes(startNode, pathInStartNode, vset, vsel, nodeWithVsel, indexer)) {
        PCP_INDEXING_MSG(indexer, node, *nodeWithVsel, "Found authored variant selection {%s=%s} at %s", vset.c_str(),
                         vsel->c_str(), Pcp_FormatSite(nodeWithVsel->GetSite()).c_str());
    }
}

static bool _ShouldUseVariantFallback(const Pcp_PrimIndexer* indexer,
                                      const std::string& vset,
    ...
    if (nodeWithVsel.GetArcType() == PcpArcTypeVariant && nodeWithVsel.GetPath().IsPrimVariantSelectionPath() &&
    ...
    const SdfLayerHandle rootLayer = indexer->rootSite.layerStack->GetIdentifier().rootLayer;
    TF_FOR_ALL(layer, indexer->rootSite.layerStack->GetLayers()) {
        ...
        const VtValue& value = (*layer)->GetField(indexer->rootSite.path, field);
        if (value.IsHolding<SdfVariantSelectionMap>()) {
            const SdfVariantSelectionMap& vselMap = value.UncheckedGet<SdfVariantSelectionMap>();
...
static std::string _ChooseBestFallbackAmongOptions(const std::string& vset,
...
static void _AddVariantArc(Pcp_PrimIndexer* indexer,
                           const PcpNodeRef& node,
                           const std::string& vset,
                           int vsetNum,
                           const std::string& vsel) {
    ...
    if (_AddArc(indexer, PcpArcTypeVariant,
                /* parent = */ node,
                /* origin = */ node, PcpLayerStackSite(node.GetLayerStack(), varPath),
...
static void _AddAncestralVariantArc(Pcp_PrimIndexer* indexer,
                                    const PcpNodeRef& node,
                                    const SdfPath& vsetPath,
                                    const std::string& vset,
                                    int vsetNum,
                                    const std::string& vsel) {
    const SdfPath varPath = node.GetPath().ReplacePrefix(vsetPath, vsetPath.AppendVariantSelection(vset, vsel));
    const int namespaceDepth = PcpNode_GetNonVariantPathElementCount(vsetPath);
    ...
    opts.skipDuplicateNodes = [&]() {
        for (PcpNodeRef n = node; !n.IsRootNode(); n = n.GetParentNode()) {
            if (PcpIsClassBasedArc(n.GetArcType()) && n.GetDepthBelowIntroduction() == 0 && !n.IsInert()) {
                return true;
            }
        }
        return false;
    ...
    if (_AddArc(indexer, PcpArcTypeVariant,
                ...
                /* origin = */ node, PcpLayerStackSite(node.GetLayerStack(), varPath),
                /* mapExpression = */ PcpMapExpression::Identity(),
                /* arcSiblingNum = */ vsetNum, namespaceDepth, opts)) {
        // If we expanded a variant set, it may have introduced new
        // authored variant selections, so we must retry any pending
        // variant tasks as authored tasks.
        indexer->RetryVariantTasks();
...
static void _EvalVariantSetsAtSite(const PcpNodeRef& node,
    ...
    const Task::Type variantTaskType =
            (isAncestral ? Task::Type::EvalNodeAncestralVariantAuthored : Task::Type::EvalNodeVariantAuthored);

    for (int vsetNum = 0, numVsets = vsetNames.size(); vsetNum < numVsets; ++vsetNum) {
        std::string& vsetName = vsetNames[vsetNum];

        PCP_INDEXING_MSG(indexer, node, "Found variant set %s%s", vsetName.c_str(),
                         (node.GetPath() == sitePath ? "" : TfStringPrintf(" at <%s>", sitePath.GetText()).c_str()));

        indexer->AddTask(Task(variantTaskType, node, sitePath, std::move(vsetName), vsetNum));
    }
}

static void _EvalNodeVariantSets(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating variant sets at %s", Pcp_FormatSite(node.GetSite()).c_str());

    if (!node.CanContributeSpecs()) {
        return;
    }

    _EvalVariantSetsAtSite(node, node.GetPath(), indexer, /* isAncestral = */ false);
}

static void _EvalNodeAncestralDynamicPayloads(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating ancestral dynamic payloads at %s",
                       Pcp_FormatSite(node.GetSite()).c_str());

    for (SdfPath path = node.GetPath().GetParentPath(); !path.IsAbsoluteRootPath(); path = path.GetParentPath()) {
        ...
        _EvalNodePayloads(node, indexer, Task::Type::EvalNodeDynamicPayloads, path);
    }
}

static void _EvalNodeAncestralVariantSets(const PcpNodeRef& node, Pcp_PrimIndexer* indexer) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating ancestral variant sets at %s",
                       Pcp_FormatSite(node.GetSite()).c_str());
    ...
    for (SdfPath path = node.GetPath().GetParentPath(); !path.IsAbsoluteRootPath(); path = path.GetParentPath()) {
        ...
        TF_VERIFY(path.IsPrimOrPrimVariantSelectionPath());
        _EvalVariantSetsAtSite(node, path, indexer, /* isAncestral = */ true);
...
static void _EvalNodeAuthoredVariant(const PcpNodeRef& node,
                                     Pcp_PrimIndexer* indexer,
                                     const SdfPath& vsetPath,
                                     const std::string& vset,
                                     int vsetNum,
                                     bool isAncestral) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating authored selections for variant set %s at %s", vset.c_str(),
    ...
    PcpComposeSiteVariantSetOptions(node.GetLayerStack(), vsetPath, vset, &vsetOptions);
    ...
    const std::string vselFallback =
            _ChooseBestFallbackAmongOptions(vset, vsetOptions, *indexer->inputs.variantFallbacks);
    if (!vselFallback.empty()) {
        PCP_INDEXING_MSG(indexer, node, "Found fallback {%s=%s}", vset.c_str(), vselFallback.c_str());
    }

    // Determine the authored variant selection for this set, if any.
    std::string vsel;
    PcpNodeRef nodeWithVsel;
    _ComposeVariantSelection(node, vsetPath.StripAllVariantSelections(), indexer, vset, &vsel, &nodeWithVsel);

    // Check if we should use the fallback
    if (_ShouldUseVariantFallback(indexer, vset, vsel, vselFallback, nodeWithVsel)) {
        PCP_INDEXING_MSG(indexer, node, "Deferring to variant fallback");
        indexer->AddTask(
                Task((isAncestral ? Task::Type::EvalNodeAncestralVariantFallback : Task::Type::EvalNodeVariantFallback),
                     node, vsetPath, vset, vsetNum));
        return;
    }
    // If no variant was chosen, do not expand this variant set.
    if (vsel.empty()) {
        PCP_INDEXING_MSG(indexer, node, "No variant selection found for set '%s'", vset.c_str());
        indexer->AddTask(Task(
                (isAncestral ? Task::Type::EvalNodeAncestralVariantNoneFound : Task::Type::EvalNodeVariantNoneFound),
                node, vsetPath, vset, vsetNum));
        return;
    }

    isAncestral ? _AddAncestralVariantArc(indexer, node, vsetPath, vset, vsetNum, vsel)
                : _AddVariantArc(indexer, node, vset, vsetNum, vsel);
}

static void _EvalNodeFallbackVariant(const PcpNodeRef& node,
                                     Pcp_PrimIndexer* indexer,
                                     const SdfPath& vsetPath,
                                     const std::string& vset,
                                     int vsetNum,
                                     bool isAncestral) {
    PCP_INDEXING_PHASE(indexer, node, "Evaluating fallback selections for variant set %s s at %s", vset.c_str(),
                       Pcp_FormatSite(node.GetLayerStack(), vsetPath).c_str());
    ...
    // Determine what the fallback selection would be.
    const std::string vsel = _ChooseBestFallbackAmongOptions(vset, vsetOptions, *indexer->inputs.variantFallbacks);

    // If no variant was chosen, do not expand this variant set.
    if (vsel.empty()) {
        PCP_INDEXING_MSG(indexer, node, "No variant fallback found for set '%s'", vset.c_str());
        indexer->AddTask(Task(
                (isAncestral ? Task::Type::EvalNodeAncestralVariantNoneFound : Task::Type::EvalNodeVariantNoneFound),
                node, vsetPath, vset, vsetNum));
        return;
...
void _GatherNodesRecursively(const PcpNodeRef& node, std::vector<PcpNodeRef>* result) {
...
static void _EnforcePermissions(PcpPrimIndex* primIndex, PcpErrorVector* allErrors) {
    TRACE_FUNCTION();
    ...
    TF_REVERSE_FOR_ALL(nodeIter, allNodes) {
        ...
        if (privateNode) {
            ...
            if (curNode.HasSpecs()) {
                TF_REVERSE_FOR_ALL(layer, curNode.GetLayerStack()->GetLayers()) {
                    if ((*layer)->HasSpec(curNode.GetPath())) {
                        ...
                        // an error.
                        PcpErrorPrimPermissionDeniedPtr err = PcpErrorPrimPermissionDenied::New();
                        err->rootSite = PcpSite(curNode.GetRootNode().GetSite());
        ...
        // errors (see above).
        if (!privateNode && curNode.GetPermission() != SdfPermissionPublic) {
            privateNode = curNode;
        }
    }
}
    ...
    if (usd) {
        ...
        if (updateHasSpecs) {
            TF_FOR_ALL(nodeIt, index->GetNodeRange()) {
                auto node = *nodeIt;
                nodeIt->SetHasSpecs(PcpComposeSiteHasPrimSpecs(node.GetLayerStack(), node.GetPath(),
            ...
            if (!node.IsCulled() && node.CanContributeSpecs()) {
                ...
                for (size_t i = 0, n = layers.size(); i != n; ++i) {
                    if (layers[i]->HasSpec(path) &&
                        (!cacheChanges || cacheChanges->layersAffectedByMutingOrRemoval.count(layers[i]) == 0)) {
                        nodeHasSpecs = true;
...
static std::pair<PcpNodeRef_PrivateChildrenConstIterator, PcpNodeRef_PrivateChildrenConstIterator> _GetDirectChildRange(
        const PcpNodeRef& node, PcpArcType arcType) {
    auto range = std::make_pair(PcpNodeRef_PrivateChildrenConstIterator(node),
...
static bool _ComputedAssetPathWouldCreateDifferentNode(const PcpNodeRef& node, const std::string& newAssetPath) {
    // Get any file format arguments that were originally used to open the
    // layer so we can apply them to the new asset path.
    const SdfLayerRefPtr& nodeRootLayer = node.GetLayerStack()->GetIdentifier().rootLayer;

    std::string oldAssetPath;
    SdfLayer::FileFormatArguments oldArgs;
    if (!TF_VERIFY(SdfLayer::SplitIdentifier(nodeRootLayer->GetIdentifier(), &oldAssetPath, &oldArgs))) {
        return true;
    }
...
template <>
auto _GetSourceArcs<PcpArcTypeReference>(const PcpNodeRef& node, PcpArcInfoVector* info) {
...
template <>
auto _GetSourceArcs<PcpArcTypePayload>(const PcpNodeRef& node, PcpArcInfoVector* info) {
...
template <PcpArcType RefOrPayload>
static bool _NeedToRecomputeDueToAssetPathChange(const PcpNodeRef& node) {
    ...
    if (arcRange.first != arcRange.second) {
        ...
        for (size_t i = 0; i < sourceArcs.size(); ++i, ++arcRange.first) {
            ...
            if (_ComputedAssetPathWouldCreateDifferentNode(*arcRange.first, anchoredAssetPath)) {
                return true;
            }
        }
    }
...
static void _ConvertNodeForChild(PcpNodeRef node, const PcpPrimIndexInputs& inputs, bool isRoot = true) {
...
static inline bool _NodeCanBeCulled(const PcpNodeRef& node, const PcpLayerStackSite& rootSite) {
    ...
    if (node.GetArcType() == PcpArcTypeInherit && node.GetLayerStack() == rootSite.layerStack) {
        ...
        const PcpNodeRef& originNode = node.GetOriginNode() == node.GetParentNode() ? node : node.GetOriginRootNode();
    ...
    if (node.HasSpecs() && node.CanContributeSpecs()) return false;
...
static bool _CullMatchingChildrenInSubtree(PcpNodeRef node,
                                           const std::unordered_set<PcpLayerStackSite, TfHash>& culledSites) {
    bool allChildrenCulled = true;
    TF_FOR_ALL(child, Pcp_GetChildrenRange(node)) {
        allChildrenCulled &= _CullMatchingChildrenInSubtree(*child, culledSites);
    }
...
static void _CullSubtreesWithNoOpinionsHelper(PcpNodeRef node,
                                              const PcpLayerStackSite& rootSite,
    ...
    TF_FOR_ALL(child, Pcp_GetChildrenRange(node)) {
        ...
        _CullSubtreesWithNoOpinionsHelper(*child, rootSite, culledDeps, culledSites);
    }
...
static void _CullSubtreesWithNoOpinions(PcpPrimIndex* primIndex,
                                        const PcpLayerStackSite& rootSite,
                                        std::vector<PcpCulledDependency>* culledDeps) {
...
static void _BuildInitialPrimIndexFromAncestor(const PcpLayerStackSite& site,
                                               const PcpLayerStackSite& rootSite,
                                               int ancestorRecursionDepth,
                                               PcpPrimIndex_StackFrame* previousFrame,
                                               bool evaluateImpliedSpecializes,
    ...
    if (!previousFrame && evaluateImpliedSpecializes && inputs.cache->GetLayerStack() == site.layerStack &&
        ...
        const PcpPrimIndex& parentIndex =
                inputs.parentIndex ? *inputs.parentIndex
                                   : Pcp_ComputePrimIndexWithCompatibleInputs(*inputs.cache, site.path.GetParentPath(),
                                                                              inputs, &outputs->allErrors);

        // Clone the parent's graph..
        outputs->primIndex.SetGraph(PcpPrimIndex_Graph::New(parentIndex.GetGraph()));

        ancestorIsInstanceable = parentIndex.IsInstanceable();

        PCP_INDEXING_UPDATE(_GetOriginatingIndex(previousFrame, outputs), outputs->primIndex.GetRootNode(),
                            "Retrieved index for <%s> from cache", site.path.GetParentPath().GetText());
    } else {
        ...
        const PcpLayerStackSite parentSite(site.layerStack, site.path.GetParentPath());

        Pcp_BuildPrimIndex(parentSite, parentSite, ancestorRecursionDepth + 1, evaluateImpliedSpecializes,
                           evaluateVariantsAndDynamicPayloads,
                           /* rootNodeShouldContributeSpecs = */ true, previousFrame, inputs, outputs);
    ...
    PCP_INDEXING_UPDATE(_GetOriginatingIndex(previousFrame, outputs), rootNode, "Adjusted ancestral index for %s",
                        site.path.GetName().c_str());
}
...
static bool _ComposeIsProhibitedPrimChild(Pcp_PrimIndexer* indexer) {
    ...
    for (auto iter = range.begin(); iter != range.end(); ++iter) {
        ...
        const SdfRelocatesMap& relocatesSourceToTarget = layerStack->GetIncrementalRelocatesSourceToTarget();
        if (relocatesSourceToTarget.find(node.GetPath()) != relocatesSourceToTarget.end()) {
            // Report a composition error if this prohibited prim index was
            // meant to be the target of a composition arc.
            if (indexer->previousFrame) {
                PcpErrorArcToProhibitedChildPtr err = PcpErrorArcToProhibitedChild::New();
...
// policy.
static bool _ElidePrimIndexIfProhibited(Pcp_PrimIndexer* indexer) {
    TRACE_FUNCTION();
...
static void Pcp_BuildPrimIndex(const PcpLayerStackSite& site,
                               ...
                               const PcpPrimIndexInputs& inputs,
                               PcpPrimIndexOutputs* outputs) {
    Pcp_PrimIndexingDebug debug(&outputs->primIndex, _GetOriginatingIndex(previousFrame, outputs), site);
    ...
    if (!TF_VERIFY(site.path.IsAbsolutePath() &&
                           (site.path.IsAbsoluteRootOrPrimPath() || site.path.IsPrimVariantSelectionPath()),
                   "%s", site.path.GetText())) {
        return;
    }
    ...
    Pcp_PrimIndexer indexer(inputs, outputs, rootSite, ancestorRecursionDepth, previousFrame,
    ...
    if (site.path.IsPrimVariantSelectionPath()) {
        ...
        _BuildInitialPrimIndexFromAncestor(site, rootSite, ancestorRecursionDepth, previousFrame,
                                           evaluateImpliedSpecializes, evaluateVariantsAndDynamicPayloads,
                                           rootNodeShouldContributeSpecs, inputs, outputs);
    ...
    while (tasksAreLeft) {
        ...
        switch (task.type) {
            ...
            case Task::Type::EvalNodeDynamicPayloads:
                _EvalNodePayloads(task.node, &indexer, Task::Type::EvalNodeDynamicPayloads, task.node.GetPath());
                break;
            case Task::Type::EvalNodePayloads:
                _EvalNodePayloads(task.node, &indexer, Task::Type::EvalNodePayloads, task.node.GetPath());
                break;
            ...
            case Task::Type::EvalNodeAncestralVariantAuthored:
                _EvalNodeAuthoredVariant(task.node, &indexer, task.vsetPath, task.vsetName, task.vsetNum,
                                         /* ancestral = */ true);
                break;
            ...
            case Task::Type::EvalNodeAncestralVariantFallback:
                _EvalNodeFallbackVariant(task.node, &indexer, task.vsetPath, task.vsetName, task.vsetNum,
                                         /* ancestral = */ true);
                break;
...
void PcpComputePrimIndex(const SdfPath& primPath,
    ...
    if (!(primPath.IsAbsolutePath() &&
          (primPath.IsAbsoluteRootOrPrimPath() || primPath.IsPrimVariantSelectionPath()))) {
        TF_CODING_ERROR(
                "Path <%s> must be an absolute path to a prim, "
                "a prim variant-selection, or the pseudo-root.",
                primPath.GetText());
        return;
    }

    ArResolverContextBinder binder(resolver ? resolver : &ArGetResolver(),
    ...
    Pcp_BuildPrimIndex(site, site,
                       ...
                       /* previousFrame = */ NULL, inputs, outputs);

    // Mark subtrees in the graph that provide no opinions as culled.
    if (inputs.cull) {
        _CullSubtreesWithNoOpinions(&outputs->primIndex, site, &outputs->culledDependencies);
    }
    ...
    outputs->primIndex.GetGraph()->SetIsInstanceable(Pcp_PrimIndexIsInstanceable(outputs->primIndex));
...
// and relocations.
static void _ComposePrimChildNamesAtNode(const PcpPrimIndex& primIndex,
                                         const PcpNodeRef& node,
    ...
    if (node.GetLayerStack()->HasRelocates()) {
        ...
        const SdfRelocatesMap& relocatesSourceToTarget = node.GetLayerStack()->GetIncrementalRelocatesSourceToTarget();
        for (SdfRelocatesMap::const_iterator i = relocatesSourceToTarget.lower_bound(node.GetPath());
             i != relocatesSourceToTarget.end() && i->first.HasPrefix(node.GetPath()); ++i) {
            ...
            if (oldPath.GetParentPath() == node.GetPath()) {
                if (newPath.GetParentPath() == node.GetPath()) {
                    ...
                    namesToReplace[oldPath.GetNameToken()] = newPath.GetNameToken();
                } else {
        ...
        // See _EvalNodeRelocations for why we use the incremental relocates.
        const SdfRelocatesMap& relocatesTargetToSource = node.GetLayerStack()->GetIncrementalRelocatesTargetToSource();
        for (SdfRelocatesMap::const_iterator i = relocatesTargetToSource.lower_bound(node.GetPath());
             i != relocatesTargetToSource.end() && i->first.HasPrefix(node.GetPath()); ++i) {
            ...
            if (newPath.GetParentPath() == node.GetPath()) {
                if (oldPath.GetParentPath() == node.GetPath()) {
                    ...
                    if (nameSet->find(newPath.GetNameToken()) == nameSet->end()) {
        ...
        if (!namesToReplace.empty() || !namesToRemove.empty()) {
            ...
            TF_FOR_ALL(name, *nameOrder) {
                std::map<TfToken, TfToken>::const_iterator i = namesToReplace.find(*name);
        ...
        //
        nameOrder->insert(nameOrder->end(), namesToAdd.begin(), namesToAdd.end());
        nameSet->insert(namesToAdd.begin(), namesToAdd.end());
    }

    // Compose the site's local names over the current result.
    PcpComposeSiteChildNames(node.GetLayerStack()->GetLayers(), node.GetPath(), SdfChildrenKeys->PrimChildren,
                             nameOrder, nameSet, &SdfFieldKeys->PrimOrder);
...
static void _ComposePrimChildNames(const PcpPrimIndex& primIndex,
                                   const PcpNodeRef& node,
                                   TfTokenVector* nameOrder,
                                   PcpTokenSet* nameSet,
                                   PcpTokenSet* prohibitedNameSet) {
    ...
    TF_REVERSE_FOR_ALL(child, Pcp_GetChildrenRange(node)) {
        _ComposePrimChildNames(primIndex, *child, nameOrder, nameSet, prohibitedNameSet);
    }

    _ComposePrimChildNamesAtNode(primIndex, node, nameOrder, nameSet, prohibitedNameSet);
}
...
struct Pcp_PrimChildNameVisitor {
    Pcp_PrimChildNameVisitor(const PcpPrimIndex& primIndex,
                             ...
                             PcpTokenSet* nameSet,
                             PcpTokenSet* prohibitedNameSet)
        : _primIndex(primIndex), _nameOrder(nameOrder), _nameSet(nameSet), _prohibitedNameSet(prohibitedNameSet) {}

    void Visit(PcpNodeRef node, bool nodeIsInstanceable) {
        if (nodeIsInstanceable) {
            _ComposePrimChildNamesAtNode(_primIndex, node, _nameOrder, _nameSet, _prohibitedNameSet);
        }
    }
...
static void _ComposePrimChildNamesForInstance(const PcpPrimIndex& primIndex,
                                              const PcpNodeRef& subtreeStartNode,
                                              TfTokenVector* nameOrder,
                                              PcpTokenSet* nameSet,
                                              PcpTokenSet* prohibitedNameSet) {
    Pcp_PrimChildNameVisitor visitor(primIndex, nameOrder, nameSet, prohibitedNameSet);
    Pcp_TraverseInstanceableWeakToStrong(subtreeStartNode, &visitor);
}

static void _ComposePrimPropertyNames(const PcpPrimIndex& primIndex,
                                      const PcpNodeRef& node,
                                      bool isUsd,
                                      TfTokenVector* nameOrder,
                                      PcpTokenSet* nameSet) {
    ...
    // Reverse strength-order traversal (weak-to-strong).
    TF_REVERSE_FOR_ALL(child, Pcp_GetChildrenRange(node)) {
        _ComposePrimPropertyNames(primIndex, *child, isUsd, nameOrder, nameSet);
    }

    // Compose the site's local names over the current result.
    if (node.CanContributeSpecs()) {
        PcpComposeSiteChildNames(node.GetLayerStack()->GetLayers(), node.GetPath(), SdfChildrenKeys->PropertyChildren,
                                 nameOrder, nameSet, isUsd ? nullptr : &SdfFieldKeys->PropertyOrder);
    }
}

static void _ComputePrimChildNamesInSubtreeImpl(const PcpPrimIndex& primIndex,
    ...
    if (primIndex.IsInstanceable()) {
        _ComposePrimChildNamesForInstance(primIndex, subtreeRootNode, nameOrder, &nameSet, prohibitedNameSet);
    } else {
        _ComposePrimChildNames(primIndex, subtreeRootNode, nameOrder, &nameSet, prohibitedNameSet);
    }

    // Remove prohibited names from the composed prim child names.
    if (!prohibitedNameSet->empty()) {
        nameOrder->erase(std::remove_if(nameOrder->begin(), nameOrder->end(),
                                        [prohibitedNameSet](const TfToken& name) {
                                            return prohibitedNameSet->find(name) != prohibitedNameSet->end();
                                        }),
                         nameOrder->end());
    }
}

void PcpPrimIndex::ComputePrimChildNames(TfTokenVector* nameOrder, PcpTokenSet* prohibitedNameSet) const {
    if (!_graph) {
        return;
    }
    _ComputePrimChildNamesInSubtreeImpl(*this, GetRootNode(), nameOrder, prohibitedNameSet);
}

void PcpPrimIndex::ComputePrimChildNamesInSubtree(const PcpNodeRef& subtreeRootNode,
                                                  TfTokenVector* nameOrder,
    ...
    _ComputePrimChildNamesInSubtreeImpl(*this, subtreeRootNode, nameOrder, prohibitedNameSet);
}

void PcpPrimIndex::ComputePrimPropertyNames(TfTokenVector* nameOrder) const {
    if (!_graph) {
        return;
    }
    ...
    // Walk the graph to compose prim child names.
    _ComposePrimPropertyNames(*this, GetRootNode(), IsUsd(), nameOrder, &nameSet);
}
