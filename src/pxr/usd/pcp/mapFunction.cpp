...
struct _PathPairOrderSourceLowerBound {
    bool operator()(const PcpMapFunction::PathPair& lhs, const SdfPath& rhs) { return _PathLessThan(lhs.first, rhs); }

    bool operator()(const SdfPath& lhs, const PcpMapFunction::PathPair& rhs) { return _PathLessThan(lhs, rhs.first); }
...
PcpMapFunction::PcpMapFunction(PathPair const* begin, PathPair const* end, SdfLayerOffset offset, bool hasRootIdentity)
...
static PairIter _GetBestSourceMatch(const SdfPath& path,
    ...
    // we're matching.
    PairIter i = std::upper_bound(begin, end, path.GetPathElementCount(), [](size_t pathLen, const auto& pathPair) {
...
static bool _HasBetterTargetMatch(const SdfPath& targetPath,
    ...
    const size_t minElementCount =
            bestSourceMatch == end ? 0
                                   : (invert ? bestSourceMatch->first : bestSourceMatch->second).GetPathElementCount();
    PairIter bestTargetMatch = invert ? _GetBestSourceMatch(targetPath, begin, end, minElementCount)
...
static bool _IsRedundant(const PairIter& entry, const PairIter& begin, const PairIter& end) {
    ...
    if (isBlock) {
        ...
        // not blocked
        const SdfPath targetPath = entrySource.ReplacePrefix(bestSourceMatch->first, bestSourceMatch->second);
...
PcpMapFunction PcpMapFunction::Create(const PathMap& sourceToTarget, const SdfLayerOffset& offset) {
    ...
    for (const auto& [source, target] : sourceToTarget) {
        ...
        auto isValidMapPath = [](const SdfPath& path) {
            return path.IsAbsolutePath() && (path.IsAbsoluteRootOrPrimPath() || path.IsPrimVariantSelectionPath());
        };
...
static SdfPath _Map(const SdfPath& path,
    ...
    const PcpMapFunction::PathPair* bestMatch =
            invert ? _GetBestTargetMatch(path, begin, end) : _GetBestSourceMatch(path, begin, end);
...
PcpMapFunction PcpMapFunction::Compose(const PcpMapFunction& inner) const {
    ...
    int maxRequiredPairs =
            inner._data.numPairs + int(inner._data.hasRootIdentity) + _data.numPairs + int(_data.hasRootIdentity);
    ...
    for (const PathPair& pair : data_outer) {
        ...
        if (std::binary_search(scratchBegin, scratchSortedEnd, source, _PathPairOrderSourceLowerBound())) {
            continue;
        }
