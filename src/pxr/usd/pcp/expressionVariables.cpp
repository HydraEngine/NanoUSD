...
static VtDictionary Pcp_ComposeExpressionVariablesOver(const PcpLayerStackIdentifier& identifier,
                                                       const VtDictionary* expressionVariableOverrides) {
    VtDictionary expressionVars;

    VtDictionaryOver(identifier.rootLayer->template GetFieldAs<VtDictionary>(SdfPath::AbsoluteRootPath(),
                                                                             SdfFieldKeys->ExpressionVariables),
                     &expressionVars);

    if (identifier.sessionLayer) {
        VtDictionaryOver(identifier.sessionLayer->template GetFieldAs<VtDictionary>(SdfPath::AbsoluteRootPath(),
                                                                                    SdfFieldKeys->ExpressionVariables),
                         &expressionVars);
    }
...
struct NoCache {
public:
    PcpExpressionVariables* GetEntry(const PcpLayerStackIdentifier& id) { return nullptr; }

    PcpExpressionVariables* CacheEntry(const PcpLayerStackIdentifier& id, const PcpExpressionVariables&) {
...
public:
    ...
    PcpExpressionVariables* GetEntry(const PcpLayerStackIdentifier& id) { return TfMapLookupPtr(*_cache, id); }
    ...
    PcpExpressionVariables* CacheEntry(const PcpLayerStackIdentifier& id,
        ...
        auto mapResult = _cache->emplace(id, std::forward<decltype(expressionVarsAndSource)>(expressionVarsAndSource));
...
static const PcpExpressionVariables* Pcp_ComposeExpressionVariables(const PcpLayerStackIdentifier& sourceLayerStackId,
    ...
    for (const PcpLayerStackIdentifier* currId = &sourceLayerStackId;
         ...
         currId = &currId->expressionVariablesOverrideSource.ResolveLayerStackIdentifier(rootLayerStackId)) {
    ...
    for (size_t i = sources.size(); i-- > 0;) {
        VtDictionary overriddenVars = Pcp_ComposeExpressionVariablesOver(sources[i], &expressionVars->GetVariables());
        ...
        if (overriddenVars == expressionVars->GetVariables()) {
            ...
            PcpExpressionVariables newExpressionVars(PcpExpressionVariablesSource(sources[i], rootLayerStackId),
...
PcpExpressionVariables PcpExpressionVariables::Compute(const PcpLayerStackIdentifier& sourceLayerStackId,
                                                       const PcpLayerStackIdentifier& rootLayerStackId,
                                                       const PcpExpressionVariables* overrideVars) {
    if (overrideVars) {
        VtDictionary composedVars =
                Pcp_ComposeExpressionVariablesOver(sourceLayerStackId, &overrideVars->GetVariables());

        if (composedVars == overrideVars->GetVariables()) {
            return *overrideVars;
        }

        return PcpExpressionVariables(PcpExpressionVariablesSource(sourceLayerStackId, rootLayerStackId),
