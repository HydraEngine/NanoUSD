...
std::string Pcp_EvaluateVariableExpression(const std::string& expression,
                                           const PcpExpressionVariables& expressionVars) {
    return Pcp_EvaluateVariableExpression(expression, expressionVars, std::string(), SdfLayerHandle(), SdfPath(),
                                          nullptr, nullptr);
}
...
std::pair<PcpNodeRef, PcpNodeRef> Pcp_FindStartingNodeOfClassHierarchy(const PcpNodeRef& n) {
    ...
    while (PcpIsClassBasedArc(instanceNode.GetArcType()) && instanceNode.GetDepthBelowIntroduction() == depth) {
...
std::pair<SdfPath, PcpNodeRef> Pcp_TranslatePathFromNodeToRootOrClosestNode(const PcpNodeRef& node,
    ...
    if (SdfPath pathInRootNode = node.GetMapToRoot().MapSourceToTarget(curPath); !pathInRootNode.IsEmpty()) {
    ...
    if (const SdfPath pathAtIntro = curNode.GetPathAtIntroduction(); pathAtIntro.ContainsPrimVariantSelection()) {
