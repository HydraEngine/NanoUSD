...
private:
    ...
    bool _ComposeOpinionInSubtree(const PcpNodeRef& node,
        ...
        TF_FOR_ALL(childNode, Pcp_GetChildrenRange(node)) {
            ...
            SdfPath pathInChildNode =
                    childNode->GetMapToParent().MapTargetToSource(pathInNode.StripAllVariantSelections());
            ...
            if (const SdfPath childNodePathAtIntro = childNode->GetPathAtIntroduction();
                ...
                pathInChildNode = pathInChildNode.ReplacePrefix(childNodePathAtIntro.StripAllVariantSelections(),
    ...
    bool _ComposeOpinionFromAncestors(const TfToken& propName,
        ...
        return _ComposeOpinionFromAncestors(_iterator.node,
                                            _pathInNode.IsEmpty() ? _iterator.node.GetPath() : _pathInNode, propName,
                                            fieldName, composeFunc);
    }
    ...
    bool _ComposeOpinionFromAncestors(const PcpNodeRef& node,
        ...
        const auto [rootmostPath, rootmostNode] = Pcp_TranslatePathFromNodeToRootOrClosestNode(node, pathInNode);
        ...
        if (rootmostNode.IsRootNode() && _iterator.previousFrame) {
            ...
            if (_ComposeOpinionFromAncestors(parentNode, parentNodePath, propName, fieldName, composeFunc)) {
                return true;
            }
        }
...
bool PcpDynamicFileFormatContext::ComposeValue(const TfToken& field, VtValue* value) const {
    ...
    if (fieldIsDictValued) {
        ...
        return _ComposeValueHelper::ComposeFieldValue(this, field, /*findStrongestOnly = */ true,
                                                      [&value](VtValue&& val) {
...
bool PcpDynamicFileFormatContext::ComposeValueStack(const TfToken& field, VtValueVector* values) const {
    ...
    // in strength order.
    return _ComposeValueHelper::ComposeFieldValue(this, field, /*findStrongestOnly = */ false,
                                                  [&values](VtValue&& val) {
                                                      // Take advantage of VtValue's move assignment
                                                      // operator.
...
bool PcpDynamicFileFormatContext::ComposeAttributeDefaultValue(const TfToken& attributeName, VtValue* value) const {
    ...
    return _ComposeValueHelper::ComposeAttributeDefaultValue(this, attributeName, [&value](VtValue&& val) {
...
PcpDynamicFileFormatContext Pcp_CreateDynamicFileFormatContext(const PcpNodeRef& parentNode,
                                                               ...
                                                               TfToken::Set* composedFieldNames,
                                                               TfToken::Set* composedAttributeNames) {
    return PcpDynamicFileFormatContext(parentNode, ancestralPath, arcNum, previousFrame, composedFieldNames,
