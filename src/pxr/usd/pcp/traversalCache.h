...
public:
    ...
    public:
        ...
        /// computed.
        SdfPath const& PathInNode() { return *_owner->_GetEntry(*_iter, /* computePaths = */ true).path; }

        /// Return a reference to the data associated with the current node.
        Data& AssociatedData() { return _owner->_GetEntry(*_iter, /* computePaths = */ false).data; }
    ...
    private:
        ...
        iterator(Pcp_TraversalCache* owner, PcpNodeRef_PrivateSubtreeConstIterator iter) : _owner(owner), _iter(iter) {}
    ...
    /// namespace.
    Pcp_TraversalCache(PcpNodeRef const& startNode, SdfPath const& pathInNode) : _startNode(startNode) {
        _ResizeForGraph();
    ...
    iterator begin() {
        _ResizeForGraph();
        return iterator(this, PcpNodeRef_PrivateSubtreeConstIterator(_startNode, /* end = */ false));
    }

    iterator end() {
        _ResizeForGraph();
        return iterator(this, PcpNodeRef_PrivateSubtreeConstIterator(_startNode, /* end = */ true));
    }
...
private:
    ...
    SdfPath _TranslatePathsForNode(PcpNodeRef const& node) {
        ...
        if (!entry.path) {
            ...
            entry.path = pathInParent.IsEmpty() ? SdfPath() : node.GetMapToParent().MapTargetToSource(pathInParent);
        }
