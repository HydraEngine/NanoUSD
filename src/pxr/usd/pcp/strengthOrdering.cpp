...
int PcpCompareSiblingNodeStrength(const PcpNodeRef& a, const PcpNodeRef& b) {
    ...
    if (a.GetArcType() < b.GetArcType()) return -1;
    if (a.GetArcType() > b.GetArcType()) return 1;
    ...
    if (PcpIsSpecializeArc(a.GetArcType())) {
        ...
        if (!_OriginsAreNestedArcs(aOriginRoot.first, bOriginRoot.first)) {
            if (a.GetNamespaceDepth() > b.GetNamespaceDepth()) return -1;
            if (a.GetNamespaceDepth() < b.GetNamespaceDepth()) return 1;
        }
        ...
        if (aOrigin == bOrigin) {
            ...
            if (aOriginRoot.first != bOriginRoot.first) {
                const int result = _OriginIsStronger(a.GetRootNode(), aOriginRoot.first, bOriginRoot.first);
            ...
            if (a.GetLayerStack() == a.GetRootNode().GetLayerStack() &&
                b.GetLayerStack() == b.GetRootNode().GetLayerStack() && !aIsAuthoredArc && !bIsAuthoredArc) {
                const bool aIsImplied = (a.GetSite() != aOrigin.GetSite());
                const bool bIsImplied = (b.GetSite() != bOrigin.GetSite());
                if (aIsImplied && !bIsImplied) {
                    return -1;
            ...
            // origin is stronger.
            const int result = _OriginIsStronger(a.GetRootNode(), aOrigin, bOrigin);
    ...
    if (a.GetSiblingNumAtOrigin() < b.GetSiblingNumAtOrigin()) return -1;
    if (a.GetSiblingNumAtOrigin() > b.GetSiblingNumAtOrigin()) return 1;
...
int PcpCompareSiblingPayloadNodeStrength(const PcpNodeRef& payloadParent,
                                         int payloadArcNum,
    ...
    // ArcType.
    // We rely on the enum values being in strength order.
    if (PcpArcTypePayload < siblingNode.GetArcType()) return -1;
    if (PcpArcTypePayload > siblingNode.GetArcType()) return 1;

    // Origin namespace depth.
    // Higher values (deeper opinions) are stronger.
    if (payloadParent.GetNamespaceDepth() > siblingNode.GetNamespaceDepth()) return -1;
    if (payloadParent.GetNamespaceDepth() < siblingNode.GetNamespaceDepth()) return 1;

    // Origin sibling arc number.
    // Lower numbers are stronger.
    if (payloadArcNum < siblingNode.GetSiblingNumAtOrigin()) return -1;
    if (payloadArcNum > siblingNode.GetSiblingNumAtOrigin()) return 1;
