...
TF_DEFINE_ENV_SETTING(PCP_ENABLE_PARALLEL_LAYER_PREFETCH, true, "Enables parallel, threaded pre-fetch of sublayers.");
...
TF_DEFINE_ENV_SETTING(PCP_ENABLE_LEGACY_RELOCATES_BEHAVIOR,
                      true,
...
struct Pcp_SublayerInfo {
    ...
    Pcp_SublayerInfo(const SdfLayerRefPtr& layer_, const SdfLayerOffset& offset_, double timeCodesPerSecond_)
        : layer(layer_), offset(offset_), timeCodesPerSecond(timeCodesPerSecond_) {}
...
public:
    Pcp_SublayerOrdering(const std::string& sessionOwner) : _sessionOwner(sessionOwner) {
        // Do nothing
    }

    // Returns true if a's layer has an owner equal to _sessionOwner.
    bool IsOwned(const Pcp_SublayerInfo& a) const {
        return a.layer->HasOwner() && a.layer->GetOwner() == _sessionOwner;
    }
    ...
    bool operator()(const Pcp_SublayerInfo& a, const Pcp_SublayerInfo& b) const { return IsOwned(a) && !IsOwned(b); }

private:
    std::string _sessionOwner;
};

static void _ApplyOwnedSublayerOrder(const PcpLayerStackIdentifier& identifier,
    ...
    if (!sessionOwner.empty() && layer->GetHasOwnedSubLayers()) {
        ...
        if (!subtrees->empty() && ordering.IsOwned(subtrees->front())) {
            ...
            Pcp_SublayerInfoVector::iterator last = std::upper_bound(first, subtrees->end(), *first, ordering);

            // Report an error if more than one layer is owned.
            if (std::distance(first, last) > 1) {
                PcpErrorInvalidSublayerOwnershipPtr error = PcpErrorInvalidSublayerOwnership::New();
                error->rootSite = PcpSite(identifier, SdfPath::AbsoluteRootPath());
...
bool Pcp_IsValidRelocatesEntry(const SdfPath& source, const SdfPath& target, std::string* errorMessage) {
    ...
    if (source == target) {
        *errorMessage = "The target of a relocate cannot be the same as its source.";
        return false;
    }

    if (target.HasPrefix(source)) {
        *errorMessage = "The target of a relocate cannot be a descendant of its source.";
        return false;
    }

    if (source.HasPrefix(target)) {
        *errorMessage = "The target of a relocate cannot be an ancestor of its source.";
        return false;
    }
...
public:
    ...
    // info struct above.
    using ProcessedRelocatesMap = std::unordered_map<SdfPath, ProcessedRelocateInfo, TfHash>;
    ProcessedRelocatesMap processedRelocates;

    // Mapping of authored target path to the entries in the processed relocates
    // map.
    using PathToProcessedRelocateMap = std::unordered_map<SdfPath, ProcessedRelocatesMap::value_type*, TfHash>;
    ...
    // workspace.
    void Compute(const PcpLayerStack& layerStack) {
        TRACE_FUNCTION();
    ...
    void Compute(const std::vector<std::pair<SdfLayerHandle, SdfRelocates>>& layerRelocates) {
        TRACE_FUNCTION();
...
private:
    ...
    void _CollectRelocatesForLayer(const SdfLayerRefPtr& layer) {
        TRACE_FUNCTION();
        ...
        auto addChildrenToPathStack = [&](const SdfPath& primPath) {
            ...
            if (layer->HasField(primPath, SdfChildrenKeys->PrimChildren, &primChildrenNames)) {
    ...
    static bool _IsValidRelocatesEntryForComposing(const SdfPath& source,
        ...
        if (source.IsRootPrimPath()) {
            *errorMessage = "Root prims cannot be the source of a relocate.";
            return false;
        }
    ...
    void _CollectRelocates(const SdfLayerRefPtr& layer, const SdfPath& primPath, const RelocatesType& relocates) {
        for (const auto& [sourcePath, targetPath] : relocates) {
            ...
            std::string errorMessage;
            if (_IsValidRelocatesEntryForComposing(source, target, &errorMessage)) {
                // It's not an error for this to fail to be added; it just
                // means a stronger relocate for the source path as been
                // added already.
                processedRelocates.try_emplace(std::move(source), std::move(target), SdfSite(layer, primPath));
            } else {
                PcpErrorInvalidAuthoredRelocationPtr err = PcpErrorInvalidAuthoredRelocation::New();
    ...
    bool _CollectLayerRelocates(const SdfLayerRefPtr& layer) {
        // Check for layer metadata relocates.
        SdfRelocates relocates;
        if (!layer->HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->LayerRelocates, &relocates)) {
            return false;
        }
    ...
    bool _CollectPrimRelocates(const SdfLayerRefPtr& layer, const SdfPath& primPath) {
    ...
    void _ConformLegacyRelocates() {
        ...
        for (const auto& [source, reloInfo] : processedRelocates) {
            ...
            const SdfPath sourceOriginParent = reloInfo.computedSourceOrigin.GetParentPath();
            ...
            for (auto otherRelocateIt = processedRelocates.begin(); otherRelocateIt != processedRelocates.end();
                 ++otherRelocateIt) {
                const SdfPath& anotherSourceOriginPath = otherRelocateIt->second.computedSourceOrigin;
                const size_t elementCount = anotherSourceOriginPath.GetPathElementCount();
                if (elementCount > bestMatchElementCount && sourceOriginParent.HasPrefix(anotherSourceOriginPath)) {
            ...
            SdfPath mostRelocatedSource = reloInfo.computedSourceOrigin.ReplacePrefix(
                    bestMatchIt->second.computedSourceOrigin, bestMatchIt->second.targetPath);
            if (mostRelocatedSource != source) {
                relocateSourcesToMove.emplace_back(source, std::move(mostRelocatedSource));
            }
        }
        ...
        for (auto& [oldSource, newSource] : relocateSourcesToMove) {
            ...
            if (auto [it, success] = processedRelocates.emplace(newSource, std::move(oldIt->second));
                ...
                TF_WARN("Couldn't conform relocate from %s to %s to use the "
                        ...
                        oldIt->first.GetText(), oldIt->second.targetPath.GetText(), newSource.GetText(),
                        it->first.GetText(), it->second.targetPath.GetText());
            }
    ...
    void _ValidateAndRemoveConflictingRelocates() {
        using ConflictReason = PcpErrorInvalidConflictingRelocation::ConflictReason;
        ...
        for (auto& relocatesEntry : processedRelocates) {
            ...
            if (!targetPath.IsEmpty()) {
                ...
                if (auto [it, success] = targetPathToProcessedRelocateMap.emplace(targetPath, &relocatesEntry);
                    !success) {
                    ...
                    if (_LogInvalidSameTargetRelocates(relocatesEntry)) {
                        _LogInvalidSameTargetRelocates(*existingAuthoredRelocatesEntry);
                    }
                }
            }
            ...
            if (!targetPath.IsEmpty()) {
                ...
                if (auto it = processedRelocates.find(targetPath); it != processedRelocates.end()) {
                    _LogInvalidConflictingRelocate(relocatesEntry, *it, ConflictReason::TargetIsConflictSource);
                    _LogInvalidConflictingRelocate(*it, relocatesEntry, ConflictReason::SourceIsConflictTarget);
                }
                ...
                for (SdfPath pathToCheck = targetPath.GetParentPath(); !pathToCheck.IsAbsoluteRootPath();
                     pathToCheck = pathToCheck.GetParentPath()) {
                    if (auto it = processedRelocates.find(pathToCheck); it != processedRelocates.end()) {
            ...
            for (SdfPath pathToCheck = sourcePath.GetParentPath(); !pathToCheck.IsRootPrimPath();
                 pathToCheck = pathToCheck.GetParentPath()) {
                if (auto it = processedRelocates.find(pathToCheck); it != processedRelocates.end()) {
                    _LogInvalidConflictingRelocate(relocatesEntry, *it,
        ...
        for (auto& [targetPath, err] : _invalidSameTargetRelocates) {
            ...
            // across platforms.
            std::sort(err->sources.begin(), err->sources.end(), [](const auto& lhs, const auto& rhs) {
        ...
        // platforms.
        std::sort(_invalidConflictingRelocates.begin(), _invalidConflictingRelocates.end(),
                  [](const auto& lhs, const auto& rhs) {
                      return std::tie(lhs->sourcePath, lhs->conflictReason, lhs->conflictSourcePath) <
                             std::tie(rhs->sourcePath, rhs->conflictReason, rhs->conflictSourcePath);
                  });
    ...
    void _ComputeSourceOriginForTargetIfNeeded(ProcessedRelocatesMap::value_type* processedRelocationEntry) {
        ...
        ProcessedRelocatesMap::value_type* nearestAncestralSourceRelocation =
            ...
            for (SdfPath ancestorPath = sourcePath; !ancestorPath.IsAbsoluteRootPath();
                ...
                if (ancestorRelocateIt != targetPathToProcessedRelocateMap.end()) {
        ...
        if (nearestAncestralSourceRelocation) {
            _ComputeSourceOriginForTargetIfNeeded(nearestAncestralSourceRelocation);
            relocationInfo.computedSourceOrigin = relocationInfo.computedSourceOrigin.ReplacePrefix(
    ...
    void _LogInvalidConflictingRelocate(const ProcessedRelocatesMap::value_type& entry,
        ...
        PcpErrorInvalidConflictingRelocationPtr err = PcpErrorInvalidConflictingRelocation::New();
        err->rootSite = _GetErrorRootSite();
    ...
    bool _LogInvalidSameTargetRelocates(const ProcessedRelocatesMap::value_type& entry) {
        // See if we can add a new error
        const auto [it, createNewError] = _invalidSameTargetRelocates.try_emplace(entry.second.targetPath, nullptr);
        ...
        err->sources.push_back({entry.first, entry.second.owningSite.layer, entry.second.owningSite.path});

        return createNewError;
    }

    PcpSite _GetErrorRootSite() const {
        return PcpSite(_layerStack ? _layerStack->GetIdentifier() : PcpLayerStackIdentifier(),
    ...
    std::vector<PcpErrorInvalidConflictingRelocationPtr> _invalidConflictingRelocates;
    std::map<SdfPath, PcpErrorInvalidSameTargetRelocationsPtr> _invalidSameTargetRelocates;
};

};  // end anonymous namespace

void Pcp_BuildRelocateMap(const std::vector<std::pair<SdfLayerHandle, SdfRelocates>>& layerRelocates,
...
void Pcp_ComputeRelocationsForLayerStack(const PcpLayerStack& layerStack,
    ...
    if (errors && !ws.errors.empty()) {
        if (errors->empty()) {
            *errors = std::move(ws.errors);
            ...
            errors->insert(errors->end(), std::make_move_iterator(ws.errors.begin()),
    ...
    // the relocates maps.
    for (const auto& [source, reloInfo] : ws.processedRelocates) {
        incrementalRelocatesSourceToTarget->emplace(source, reloInfo.targetPath);
        incrementalRelocatesTargetToSource->emplace(reloInfo.targetPath, source);

        relocatesTargetToSource->emplace(reloInfo.targetPath, reloInfo.computedSourceOrigin);
        relocatesSourceToTarget->emplace(reloInfo.computedSourceOrigin, reloInfo.targetPath);
    }

    // Take the list of prim paths with relocates.
    relocatesPrimPaths->assign(std::make_move_iterator(ws.allPrimPathsWithAuthoredRelocates.begin()),
                               std::make_move_iterator(ws.allPrimPathsWithAuthoredRelocates.end()));
}

static PcpMapFunction _FilterRelocationsForPath(const PcpLayerStack& layerStack, const SdfPath& path) {
    ...
    for (SdfRelocatesMap::const_iterator i = relocates.lower_bound(path), n = relocates.end();
    ...
    const SdfRelocatesMap& incrementalRelocates = layerStack.GetIncrementalRelocatesSourceToTarget();
    for (SdfRelocatesMap::const_iterator i = incrementalRelocates.lower_bound(path), n = incrementalRelocates.end();
         (i != n) && (i->first.HasPrefix(path)); ++i) {
        // Skip relocates to empty targets. We don't want to prevent these paths
        // from mapping through the node.
...
bool Pcp_NeedToRecomputeDueToAssetPathChange(const PcpLayerStackPtr& layerStack) {
    ArResolverContextBinder binder(layerStack->GetIdentifier().pathResolverContext);
    ...
    for (const auto& sourceInfo : layerStack->_sublayerSourceInfo) {
        const std::string& assetPath =
                SdfComputeAssetPathRelativeToLayer(sourceInfo.layer, sourceInfo.authoredSublayerPath);
...
static bool _ShouldUseSessionTcps(const SdfLayerHandle& sessionLyr, const SdfLayerHandle& rootLyr) {
    return sessionLyr && (sessionLyr->HasTimeCodesPerSecond() ||
...
bool Pcp_NeedToRecomputeLayerStackTimeCodesPerSecond(const PcpLayerStackPtr& layerStack,
    ...
    const SdfLayerHandle& sessionLayer = layerStack->GetIdentifier().sessionLayer;
    const SdfLayerHandle& rootLayer = layerStack->GetIdentifier().rootLayer;
    ...
    const double newLayerStackTcps = _ShouldUseSessionTcps(sessionLayer, rootLayer)
...
PcpLayerStack::PcpLayerStack(const PcpLayerStackIdentifier& identifier, const Pcp_LayerStackRegistry& registry)
    : _identifier(identifier),
      _expressionVariables([&identifier, &registry]() {
          const PcpLayerStackIdentifier& selfId = identifier;
          const PcpLayerStackIdentifier& rootLayerStackId = registry._GetRootLayerStackIdentifier();
          ...
          const PcpLayerStackIdentifier& overrideLayerStackId =
                  selfId.expressionVariablesOverrideSource.ResolveLayerStackIdentifier(rootLayerStackId);

          const PcpLayerStackPtr overrideLayerStack =
                  overrideLayerStackId != selfId ? registry.Find(overrideLayerStackId) : TfNullPtr;

          PcpExpressionVariables composedExpressionVars;

          if (overrideLayerStack) {
              composedExpressionVars = PcpExpressionVariables::Compute(selfId, rootLayerStackId,
              ...
              if (composedExpressionVars == overrideLayerStack->GetExpressionVariables()) {
          ...
          return std::make_shared<PcpExpressionVariables>(std::move(composedExpressionVars));
      }()),
...
void PcpLayerStack::Apply(const PcpLayerStackChanges& changes, PcpLifeboat* lifeboat) {
    ...
    if (changes.didChangeSignificantly || changes.didChangeExpressionVariables ||
        changes._didChangeExpressionVariablesSource) {
        const auto updateExpressionVariables = [&](const VtDictionary& newExprVars,
                                                   const PcpExpressionVariablesSource& newSource) {
            const PcpLayerStackIdentifier& newSourceId =
                    newSource.ResolveLayerStackIdentifier(_registry->_GetRootLayerStackIdentifier());
            ...
            if (newSourceId == GetIdentifier()) {
                ...
                if (_expressionVariables->GetSource() == newSource) {
                    ...
                    _expressionVariables = std::make_shared<PcpExpressionVariables>(newSource, newExprVars);
                }
            } else {
                ...
                // in c'tor.
                const PcpLayerStackPtr overrideLayerStack = _registry->Find(newSourceId);
                if (overrideLayerStack) {
                    _expressionVariables = overrideLayerStack->_expressionVariables;
                    ...
                    if (newExprVars != _expressionVariables->GetVariables()) {
                        _expressionVariables->SetVariables(newExprVars);
                    }
                } else {
                    _expressionVariables = std::make_shared<PcpExpressionVariables>(newSource, newExprVars);
        ...
        if (changes.didChangeSignificantly) {
            const PcpExpressionVariables newExprVars =
                    PcpExpressionVariables::Compute(GetIdentifier(), _registry->_GetRootLayerStackIdentifier());
            updateExpressionVariables(newExprVars.GetVariables(), newExprVars.GetSource());
        } else {
            updateExpressionVariables(changes.didChangeExpressionVariables ? changes.newExpressionVariables
    ...
    if (changes.didChangeLayers || changes.didChangeLayerOffsets) {
        ...
        if (changes.didChangeSignificantly) {
            ...
            Pcp_ComputeRelocationsForLayerStack(*this, &_relocatesSourceToTarget, &_relocatesTargetToSource,
                                                &_incrementalRelocatesSourceToTarget,
                                                &_incrementalRelocatesTargetToSource, &_relocatesPrimPaths, &errors);
        } else {
            ...
            errors.insert(errors.end(), changes.newRelocatesErrors.begin(), changes.newRelocatesErrors.end());
        }
...
const PcpLayerStackIdentifier& PcpLayerStack::GetIdentifier() const {
    return _identifier;
}

const SdfLayerRefPtrVector& PcpLayerStack::GetLayers() const {
    return _layers;
}
...
const SdfLayerTreeHandle& PcpLayerStack::GetLayerTree() const {
    return _layerTree;
}

const SdfLayerTreeHandle& PcpLayerStack::GetSessionLayerTree() const {
...
static inline const SdfLayerOffset* _GetLayerOffsetForLayer(LayerPtr const& layer,
...
const SdfLayerOffset* PcpLayerStack::GetLayerOffsetForLayer(const SdfLayerHandle& layer) const {
    return _GetLayerOffsetForLayer(layer, _layers, _mapFunctions);
}

const SdfLayerOffset* PcpLayerStack::GetLayerOffsetForLayer(const SdfLayerRefPtr& layer) const {
    return _GetLayerOffsetForLayer(layer, _layers, _mapFunctions);
}

const SdfLayerOffset* PcpLayerStack::GetLayerOffsetForLayer(size_t layerIdx) const {
    // XXX: Optimization: store a flag if all offsets are identity
    //      and just return NULL if it's set.
...
const std::set<std::string>& PcpLayerStack::GetMutedLayers() const {
    return _mutedAssetPaths;
}

bool PcpLayerStack::HasLayer(const SdfLayerHandle& layer) const {
...
bool PcpLayerStack::HasLayer(const SdfLayerRefPtr& layer) const {
    return std::find(_layers.begin(), _layers.end(), layer) != _layers.end();
}

const SdfRelocatesMap& PcpLayerStack::GetRelocatesSourceToTarget() const {
    return _relocatesSourceToTarget;
}

const SdfRelocatesMap& PcpLayerStack::GetRelocatesTargetToSource() const {
    return _relocatesTargetToSource;
}

const SdfRelocatesMap& PcpLayerStack::GetIncrementalRelocatesSourceToTarget() const {
    return _incrementalRelocatesSourceToTarget;
}

const SdfRelocatesMap& PcpLayerStack::GetIncrementalRelocatesTargetToSource() const {
    return _incrementalRelocatesTargetToSource;
}

const SdfPathVector& PcpLayerStack::GetPathsToPrimsWithRelocates() const {
    return _relocatesPrimPaths;
}

PcpMapExpression PcpLayerStack::GetExpressionForRelocatesAtPath(const SdfPath& path) {
    ...
    PcpMapExpression::VariableUniquePtr newVar = PcpMapExpression::NewVariable(_FilterRelocationsForPath(*this, path));

    {
        // Retain the variable so that we can update it if relocations change.
        tbb::spin_mutex::scoped_lock lock{_relocatesVariablesMutex};
        _RelocatesVarMap::const_iterator i = _relocatesVariables.emplace(path, std::move(newVar)).first;
        var = i->second.get();
    }
...
void PcpLayerStack::_BlowLayers() {
...
void PcpLayerStack::_BlowRelocations() {
...
void PcpLayerStack::_Compute(const std::string& fileFormatTarget, const Pcp_MutedLayers& mutedLayers) {
    ...
    const SdfLayer::FileFormatArguments layerArgs = Pcp_GetArgumentsForFileFormatTarget(fileFormatTarget);
    ...
    const bool scaleLayerOffsetByTcps = !PcpIsTimeScalingForLayerTimeCodesPerSecondDisabled();
    ...
    if (_identifier.sessionLayer) {
        ...
        if (mutedLayers.IsLayerMuted(_identifier.sessionLayer, _identifier.sessionLayer->GetIdentifier(),
                                     &canonicalMutedPath)) {
            ...
            if (_ShouldUseSessionTcps(_identifier.sessionLayer, _identifier.rootLayer)) {
            ...
            _sessionLayerTree = _BuildLayerStack(_identifier.sessionLayer, sessionLayerOffset, sessionTcps,
                                                 _identifier.pathResolverContext, layerArgs, std::string(), mutedLayers,
                                                 &seenLayers, &errors);

            // Get the session owner.
            struct _Helper {
                static bool FindSessionOwner(const SdfLayerTreeHandle& tree, std::string* sessionOwner) {
                    if (tree->GetLayer()->HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SessionOwner,
                                                   sessionOwner)) {
                        return true;
                    }
    ...
    _layerTree = _BuildLayerStack(_identifier.rootLayer, rootLayerOffset, rootTcps, _identifier.pathResolverContext,
                                  layerArgs, sessionOwner, mutedLayers, &seenLayers, &errors);

    // Update layer-stack-to-layer maps in the registry, if we're installed in a
    // registry.
    if (_registry) _registry->_SetLayers(this);

    Pcp_ComputeRelocationsForLayerStack(*this, &_relocatesSourceToTarget, &_relocatesTargetToSource,
                                        &_incrementalRelocatesSourceToTarget, &_incrementalRelocatesTargetToSource,
                                        &_relocatesPrimPaths, &errors);
...
SdfLayerTreeHandle PcpLayerStack::_BuildLayerStack(const SdfLayerHandle& layer,
    ...
    for (size_t i = 0; i != numSublayers; ++i) {
        if (Pcp_IsVariableExpression(sublayers[i])) {
            sublayers[i] = Pcp_EvaluateVariableExpression(sublayers[i], *_expressionVariables, "sublayer", layer,
                                                          SdfPath::AbsoluteRootPath(), &_expressionVariableDependencies,
                                                          errors);
        ...
        if (mutedLayers.IsLayerMuted(layer, sublayers[i], &canonicalMutedPath)) {
            _mutedAssetPaths.insert(canonicalMutedPath);
            sublayers[i].clear();
        }
    }
    ...
    auto loadSublayer = [&](size_t i) {
        ...
        const SdfLayer::FileFormatArguments& layerArgs =
                Pcp_GetArgumentsForFileFormatTarget(sublayers[i], &defaultLayerArgs, &localArgs);
        ...
        string sublayerPath = SdfComputeAssetPathRelativeToLayer(layer, sublayers[i]);
        sublayerRefPtrs[i] = SdfLayer::FindOrOpen(sublayerPath, layerArgs);

        localSourceInfo[i] = _SublayerSourceInfo(layer, sublayers[i], sublayerPath);
        ...
        if (!m.IsClean()) {
            ...
            m.Clear();
            errCommentary[i] = TfStringJoin(commentary.begin(), commentary.end(), "; ");
        }
    };
    ...
    WorkWithScopedDispatcher([&](WorkDispatcher& wd) {
        ...
        const bool goParallel = _isUsd && numSublayers > 1 && TfGetEnvSetting(PCP_ENABLE_PARALLEL_LAYER_PREFETCH);
        ...
        for (size_t i = 0; i != numSublayers; ++i) {
            ...
            if (goParallel) {
                wd.Run([i, &loadSublayer, &pathResolverContext]() {
    ...
    for (size_t i = 0; i != numSublayers; ++i) {
        ...
        if (!sublayerRefPtrs[i]) {
            PcpErrorInvalidSublayerPathPtr err = PcpErrorInvalidSublayerPath::New();
        ...
        if (!sublayerOffset.IsValid() || !sublayerOffset.GetInverse().IsValid()) {
            // Report error, but continue with an identity layer offset.
            PcpErrorInvalidSublayerOffsetPtr err = PcpErrorInvalidSublayerOffset::New();
            err->rootSite = PcpSite(_identifier, SdfPath::AbsoluteRootPath());
            err->layer = layer;
            err->sublayer = sublayerRefPtrs[i];
        ...
        if (!PcpIsTimeScalingForLayerTimeCodesPerSecondDisabled() && layerTcps != sublayerTcps) {
        ...
        sublayerInfo.emplace_back(sublayerRefPtrs[i], sublayerOffset, sublayerTcps);
    }
    ...
    // muted).
    _sublayerSourceInfo.reserve(_sublayerSourceInfo.size() + localSourceInfo.size());
    ...
    _ApplyOwnedSublayerOrder(_identifier, layer, sessionOwner, &sublayerInfo, errors);
    ...
    TF_FOR_ALL(i, sublayerInfo) {
        if (SdfLayerTreeHandle subtree =
                    _BuildLayerStack(i->layer, i->offset, i->timeCodesPerSecond, pathResolverContext, defaultLayerArgs,
...
std::ostream& operator<<(std::ostream& s, const PcpLayerStackPtr& x) {
    if (x) {
...
std::ostream& operator<<(std::ostream& s, const PcpLayerStackRefPtr& x) {
    if (x) {
        return s << x->GetIdentifier();
    } else {
        return s << "@NULL@";
