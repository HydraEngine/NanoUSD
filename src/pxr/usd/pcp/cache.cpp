...
PcpCache::PcpCache(const PcpLayerStackIdentifier& layerStackIdentifier, const std::string& fileFormatTarget, bool usd)
    : _rootLayer(layerStackIdentifier.rootLayer),
      ...
      _layerStackCache(Pcp_LayerStackRegistry::New(_layerStackIdentifier, _fileFormatTarget, _usd)),
...
PcpCache::~PcpCache() {
    ...
    WorkWithScopedParallelism([this]() {
        ...
        wd.Run([this]() {
            TfReset(_includedPayloads);
        });
        wd.Run([this]() {
            TfReset(_variantFallbackMap);
        });
        wd.Run([this]() {
            _primIndexCache.ClearInParallel();
        });
        wd.Run([this]() {
            TfReset(_propertyIndexCache);
        });
...
const PcpLayerStackPtrVector& PcpCache::FindAllLayerStacksUsingLayer(const SdfLayerHandle& layer) const {
...
void PcpCache::RequestLayerMuting(const std::vector<std::string>& layersToMute,
    ...
    cacheChanges->DidMuteAndUnmuteLayers(this, finalLayersToMute, finalLayersToUnmute);
    ...
    if (!finalLayersToUnmute.empty()) {
        for (const auto& primIndexEntry : _primIndexCache) {
            ...
            for (const auto& error : primIndex.GetLocalErrors()) {
                PcpErrorMutedAssetPathPtr typedError = std::dynamic_pointer_cast<PcpErrorMutedAssetPath>(error);
                if (!typedError) {
                    continue;
                }

                const bool assetWasUnmuted = std::find(finalLayersToUnmute.begin(), finalLayersToUnmute.end(),
                                                       typedError->resolvedAssetPath) != finalLayersToUnmute.end();
                if (assetWasUnmuted) {
                    cacheChanges->DidMaybeFixAsset(this, typedError->site, typedError->sourceLayer,
...
PcpLayerStackRefPtr PcpCache::ComputeLayerStack(const PcpLayerStackIdentifier& id, PcpErrorVector* allErrors) {
...
void PcpCache::ComputeRelationshipTargetPaths(const SdfPath& relPath,
    ...
    if (!relPath.IsPropertyPath()) {
        TF_CODING_ERROR("Path <%s> must be a relationship path", relPath.GetText());
        return;
    }

    auto computeTargets = [&](const PcpPropertyIndex& propIndex) {
        PcpTargetIndex targetIndex;
        PcpBuildFilteredTargetIndex(PcpSite(GetLayerStackIdentifier(), relPath), propIndex, SdfSpecTypeRelationship,
                                    localOnly, stopProperty, includeStopProperty, this, &targetIndex, deletedPaths,
                                    allErrors);
...
void PcpCache::ComputeAttributeConnectionPaths(const SdfPath& attrPath,
    ...
    if (!attrPath.IsPropertyPath()) {
        TF_CODING_ERROR("Path <%s> must be an attribute path", attrPath.GetText());
        return;
    }

    auto computeTargets = [&](const PcpPropertyIndex& propIndex) {
        PcpTargetIndex targetIndex;
        PcpBuildFilteredTargetIndex(PcpSite(GetLayerStackIdentifier(), attrPath), propIndex, SdfSpecTypeAttribute,
                                    localOnly, stopProperty, includeStopProperty, this, &targetIndex, deletedPaths,
                                    allErrors);
        paths->swap(targetIndex.paths);
    };
...
PcpDependencyVector PcpCache::FindSiteDependencies(const SdfLayerHandle& layer,
    ...
    for (const auto& layerStack : FindAllLayerStacksUsingLayer(layer)) {
        PcpDependencyVector deps = FindSiteDependencies(layerStack, sitePath, depMask, recurseOnSite, recurseOnIndex,
...
static void _ProcessCulledDependency(const PcpCulledDependency& dep,
    ...
    if (!dep.unrelocatedSitePath.IsEmpty()) {
        ...
        depIndexPath = PcpTranslatePathFromNodeToRootUsingFunction(dep.mapToRoot, localSitePath, &valid);
    }
...
PcpDependencyVector PcpCache::FindSiteDependencies(const PcpLayerStackPtr& siteLayerStack,
    ...
    if (ARCH_UNLIKELY(!sitePath.IsPrimOrPrimVariantSelectionPath())) {
        tmpPath = (sitePath == SdfPath::AbsoluteRootPath()) ? sitePath : sitePath.GetPrimOrPrimVariantSelectionPath();
        sitePrimPath = &tmpPath;
    }

    // Handle the root dependency.
    // Sites containing variant selections are never root dependencies.
    if (depMask & PcpDependencyTypeRoot && siteLayerStack == _layerStack && !sitePath.ContainsPrimVariantSelection() &&
    ...
    auto visitSiteFn = [&](const SdfPath& depPrimIndexPath, const SdfPath& depPrimSitePath) {
        ...
        const PcpDependencyFlags localMask =
                (depPrimSitePath != *sitePrimPath && sitePrimPath->HasPrefix(depPrimSitePath))
        ...
        const SdfPath localSitePath = (depPrimSitePath != *sitePrimPath && depPrimSitePath.HasPrefix(*sitePrimPath))
                                              ? depPrimSitePath
                                              : sitePath;
        ...
        Pcp_ForEachDependentNode(depPrimSitePath, siteLayerStack, depPrimIndexPath, *this, visitNodeFn,
                                 visitCulledDepFn);
    };
    ...
    if (recurseOnIndex) {
        ...
        for (const PcpDependency& dep : deps) {
            ...
            if (indexPath.IsAbsoluteRootOrPrimPath()) {
                auto primRange = _primIndexCache.FindSubtreeRange(indexPath);
...
std::vector<std::string> PcpCache::GetInvalidSublayerIdentifiers() const {
    ...
    TF_FOR_ALL(layerStack, allLayerStacks) {
        ...
        TF_FOR_ALL(e, errs) {
            if (PcpErrorInvalidSublayerPathPtr typedErr = std::dynamic_pointer_cast<PcpErrorInvalidSublayerPath>(*e)) {
    ...
    TF_FOR_ALL(it, _primIndexCache) {
        ...
        if (primIndex.IsValid()) {
            ...
            for (const auto& e : errors) {
                if (PcpErrorInvalidAssetPathPtr typedErr = std::dynamic_pointer_cast<PcpErrorInvalidAssetPath>(e)) {
...
bool PcpCache::IsInvalidAssetPath(const std::string& resolvedAssetPath) const {
    ...
    std::map<SdfPath, std::vector<std::string>, SdfPath::FastLessThan> pathMap = GetInvalidAssetPaths();
...
const std::unordered_set<std::string>& PcpCache::GetExpressionVariablesFromLayerStackUsedByPrim(
    ...
    return _primDependencies->GetExpressionVariablesFromLayerStackUsedByPrim(primIndexPath, layerStack);
}
...
void PcpCache::Reload(PcpChanges* changes) {
    ...
    TF_FOR_ALL(it, _primIndexCache) {
        ...
        if (primIndex.IsValid()) {
            ...
            for (const auto& e : errors) {
                if (PcpErrorInvalidAssetPathPtr typedErr = std::dynamic_pointer_cast<PcpErrorInvalidAssetPath>(e)) {
                    changes->DidMaybeFixAsset(this, typedErr->site, typedErr->sourceLayer, typedErr->resolvedAssetPath);
                }
            }
        }
    }
...
void PcpCache::ReloadReferences(PcpChanges* changes, const SdfPath& primPath) {
    ...
    for (const PcpLayerStackPtr& layerStack : layerStacksAtOrUnderPrim) {
        ...
        for (const PcpErrorBasePtr& err : errs) {
            if (PcpErrorInvalidSublayerPathPtr typedErr = std::dynamic_pointer_cast<PcpErrorInvalidSublayerPath>(err)) {
                changes->DidMaybeFixSublayer(this, typedErr->layer, typedErr->sublayerPath);
            }
        }
    }
...
void PcpCache::_RemovePrimAndPropertyCaches(const SdfPath& root, PcpLifeboat* lifeboat) {
    std::pair<_PrimIndexCache::iterator, _PrimIndexCache::iterator> range = _primIndexCache.FindSubtreeRange(root);
...
private:
    ...
    void _ComputeIndex(const PcpPrimIndex* parentIndex, SdfPath path, bool checkCache) {
        ...
        if (!index) {
            ...
            // Run indexing.
            PcpComputePrimIndex(path, _layerStack, inputs, &outputs, &_resolver);
            ...
            // isn't already working on it.
                ...
                _toPublish.push({std::move(outputIndexNode), std::move(outputs)});
    ...
    PcpPrimIndex const* _PublishOneOutput(std::pair<_PrimIndexCache::NodeHandle, PcpPrimIndexOutputs>&& outputItem,
                                          bool allowInvalid) {
        ...
        if (!iresult.second) {
            TF_VERIFY(allowInvalid && !iresult.first->second.IsValid(), "PrimIndex <%s> already exists in cache",
                      iresult.first->first.GetAsString().c_str());
            // Replace the invalid index.
            iresult.first->second = std::move(outputItem.first.GetMutableMapped());
        }
        PcpPrimIndex* mutableIndex = &iresult.first->second;
        lock.release();
        _cache->_primDependencies->Add(*mutableIndex, std::move(outputItem.second.culledDependencies),
...
void PcpCache::_ComputePrimIndexesInParallel(const SdfPathVector& roots,
    ...
    if (!_layerStack) ComputeLayerStack(GetLayerStackIdentifier(), allErrors);
    ...
    for (const auto& rootPath : roots) {
        ...
        const PcpPrimIndex* parentIndex =
                rootPath == SdfPath::AbsoluteRootPath()
                        ? nullptr
                        : &_ComputePrimIndexWithCompatibleInputs(rootPath.GetParentPath(), inputs, allErrors);
...
const PcpPrimIndex& PcpCache::_ComputePrimIndexWithCompatibleInputs(const SdfPath& path,
    ...
    // Add dependencies.
    _primDependencies->Add(outputs.primIndex, std::move(outputs.culledDependencies),
...
const PcpPropertyIndex& PcpCache::ComputePropertyIndex(const SdfPath& path, PcpErrorVector* allErrors) {
    TRACE_FUNCTION();
