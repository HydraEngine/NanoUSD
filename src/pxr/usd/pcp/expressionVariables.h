...
public:
    ...
    PCP_API
    static PcpExpressionVariables Compute(const PcpLayerStackIdentifier& sourceLayerStackId,
    ...
    bool operator==(const PcpExpressionVariables& rhs) const {
        return (this == &rhs) ||
               (std::tie(_source, _expressionVariables) == std::tie(rhs._source, rhs._expressionVariables));
    }

    bool operator!=(const PcpExpressionVariables& rhs) const { return !(*this == rhs); }
    /// @}
    ...
    void SetVariables(const VtDictionary& variables) { _expressionVariables = variables; }
...
public:
    PCP_API
    explicit PcpExpressionVariableCachingComposer(const PcpLayerStackIdentifier& rootLayerStackIdentifier);
...
private:
    ...
    using _IdentifierToExpressionVarsMap = std::unordered_map<PcpLayerStackIdentifier, PcpExpressionVariables, TfHash>;
