...
PcpLayerRelocatesEditBuilder::PcpLayerRelocatesEditBuilder(const PcpLayerStackPtr& layerStack,
    ...
    const SdfLayerHandle newRelocatesLayer =
            addNewRelocatesLayer ? addNewRelocatesLayer : layerStack->GetIdentifier().rootLayer;

    if (!layerStack->HasLayer(newRelocatesLayer)) {
        TF_CODING_ERROR(
                "The layer for adding new relocates does not belong to "
                "the layer stack.");
        return;
    }
    ...
    for (const auto& layer : layerStack->GetLayers()) {
        ...
        const bool layerHasRelocates =
                layer->HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->LayerRelocates, &layerRelocates);
...
void PcpLayerRelocatesEditBuilder::_RemoveRelocatesWithErrors(const PcpErrorVector& errors) {
    ...
    for (const auto& error : errors) {
        if (error->errorType == PcpErrorType_InvalidAuthoredRelocation) {
            ...
            const auto* err = static_cast<const PcpErrorInvalidAuthoredRelocation*>(error.get());
            relocatesToDeletePerLayer[err->layer].emplace(err->sourcePath, err->targetPath);
            ...
            // relocates map.
            const auto* err = static_cast<const PcpErrorInvalidConflictingRelocation*>(error.get());
            ...
            const PcpErrorInvalidSameTargetRelocations* err =
                    static_cast<const PcpErrorInvalidSameTargetRelocations*>(error.get());
            for (const auto& source : err->sources) {
                relocateSourcePathsToDelete.insert(source.sourcePath);
            }
        } else {
            TF_CODING_ERROR("Unexpected error type: %s", TfStringify(TfEnum(error->errorType)).c_str());
        };
    }
    ...
    if (!relocatesToDeletePerLayer.empty()) {
        for (auto& [layer, relocates] : _layerRelocatesEdits) {
            ...
            const RelocateSet* relocatesToDelete = TfMapLookupPtr(relocatesToDeletePerLayer, layer);
            ...
            relocates.erase(std::remove_if(relocates.begin(), relocates.end(),
                                           [&](const auto& relocate) {
    ...
    if (!relocateSourcePathsToDelete.empty()) {
        for (auto& [layer, relocates] : _layerRelocatesEdits) {
            ...
            const auto eraseIt = std::remove_if(relocates.begin(), relocates.end(), [&](const auto& relocate) {
...
const SdfRelocatesMap& PcpLayerRelocatesEditBuilder::GetEditedRelocatesMap() const {
...
static void _PrintToWhyNot(
        std::string* whyNot, const SdfPath& source, const SdfPath& target, const char* format, Args... args) {
    if (!whyNot) {
        return;
    }
    *whyNot = TfStringPrintf("Cannot relocate <%s> to <%s>: ", source.GetText(), target.GetText()) +
              TfStringPrintf(format, args...);
}

static bool _ValidateAgainstExistingRelocate(const SdfPath& newSource,
                                             const SdfPath& newTarget,
                                             const SdfPath& existingSource,
                                             const SdfPath& existingTarget,
                                             std::string* whyNot) {
    ...
    if (newSource.HasPrefix(existingSource)) {
        _PrintToWhyNot(whyNot, newSource, newTarget,
                       ...
                       existingSource.GetText(), existingTarget.GetText(), existingSource.GetText());
        return false;
    }
    ...
    if (newTarget == existingTarget) {
        _PrintToWhyNot(whyNot, newSource, newTarget,
                       ...
                       existingSource.GetText(), existingTarget.GetText());
        return false;
    }
...
void PcpLayerRelocatesEditBuilder::_UpdateExistingRelocates(const SdfPath& source, const SdfPath& target) {
...
bool Pcp_ModifyRelocates(SdfRelocates* relocates, const SdfPath& oldPath, const SdfPath& newPath) {
    ...
    for (auto& [existingSource, existingTarget] : *relocates) {
        ...
        if (existingSource.HasPrefix(oldPath)) {
            existingSource = existingSource.ReplacePrefix(oldPath, newPath);
            modified = true;
        }
        // If the existing target source would be ancestrally relocated by
        // the new relocate, apply the relocate to it.
        if (existingTarget.HasPrefix(oldPath)) {
            existingTarget = existingTarget.ReplacePrefix(oldPath, newPath);
            modified = true;
        }
    }
    ...
    if (modified) {
        relocates->erase(std::remove_if(relocates->begin(), relocates->end(),
                                        [](const auto& relocate) {
                                            return relocate.first.IsEmpty() || relocate.first == relocate.second;
                                        }),
                         relocates->end());
    }
    return modified;
...
bool PcpLayerRelocatesEditBuilder::Relocate(const SdfPath& newSource, const SdfPath& newTarget, std::string* whyNot) {
    ...
    for (auto& existingRelocate : GetEditedRelocatesMap()) {
        ...
        if (!_ValidateAgainstExistingRelocate(newSource, newTarget, existingSource, existingTarget, whyNot)) {
            return false;
        }
    ...
    if (addNewRelocate) {
        ...
        // builder.
        LayerRelocatesEdit& layerRelocatesEdit = _layerRelocatesEdits[_editForNewRelocatesIndex];
...
bool PcpLayerRelocatesEditBuilder::RemoveRelocate(const SdfPath& sourcePath, std::string* whyNot) {
    ...
    if (it == relocatesMap.end()) {
        if (whyNot) {
            *whyNot = TfStringPrintf(
                    "Cannot remove relocate for source path "
    ...
    if (targetPath.IsEmpty()) {
        ...
        for (auto& [layer, relocates] : _layerRelocatesEdits) {
            auto removeIt = std::remove_if(relocates.begin(), relocates.end(), [&](const auto& relocate) {
...
PcpLayerRelocatesEditBuilder::LayerRelocatesEdits PcpLayerRelocatesEditBuilder::GetEdits() const {
