...
PcpErrorBase::PcpErrorBase(PcpErrorType errorType) : errorType(errorType) {}
...
PcpErrorArcCyclePtr PcpErrorArcCycle::New() {
...
// virtual
std::string PcpErrorArcCycle::ToString() const {
    if (cycle.empty()) return std::string();
    ...
    for (size_t i = 0; i < cycle.size(); i++) {
        ...
        if ((i > 0) && (i + 1 < cycle.size())) msg += "which ";
    }
    return msg;
}
...
PcpErrorArcPermissionDeniedPtr PcpErrorArcPermissionDenied::New() {
...
PcpErrorArcToProhibitedChild::PcpErrorArcToProhibitedChild() : PcpErrorBase(PcpErrorType_ArcToProhibitedChild) {}
...
std::string PcpErrorArcToProhibitedChild::ToString() const {
    ...
    msg += TfStringPrintf(
            ...
            "relocation at %s.",
            TfStringify(targetSite).c_str(), TfStringify(relocationSourceSite).c_str());
...
PcpErrorCapacityExceededPtr PcpErrorCapacityExceeded::New(PcpErrorType errorType) {
...
// virtual
std::string PcpErrorCapacityExceeded::ToString() const {
    return std::string("Composition graph capacity exceeded: ") + TfEnum::GetDisplayName(errorType);
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInconsistentPropertyBase::PcpErrorInconsistentPropertyBase(PcpErrorType errorType) : PcpErrorBase(errorType) {}
...
PcpErrorInconsistentPropertyTypePtr PcpErrorInconsistentPropertyType::New() {
    return PcpErrorInconsistentPropertyTypePtr(new PcpErrorInconsistentPropertyType);
}
...
std::string PcpErrorInconsistentPropertyType::ToString() const {
    return TfStringPrintf(
            ...
            rootSite.path.GetString().c_str(), definingLayerIdentifier.c_str(), definingSpecPath.GetString().c_str(),
            (definingSpecType == SdfSpecTypeAttribute ? "an attribute" : "a relationship"),
            conflictingLayerIdentifier.c_str(), conflictingSpecPath.GetString().c_str(),
            (conflictingSpecType == SdfSpecTypeAttribute ? "an attribute" : "a relationship"));
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInconsistentAttributeTypePtr PcpErrorInconsistentAttributeType::New() {
    return PcpErrorInconsistentAttributeTypePtr(new PcpErrorInconsistentAttributeType);
}
...
std::string PcpErrorInconsistentAttributeType::ToString() const {
    return TfStringPrintf(
            ...
            "The conflicting spec will be ignored.",
            rootSite.path.GetString().c_str(), definingLayerIdentifier.c_str(), definingSpecPath.GetString().c_str(),
            definingValueType.GetText(), conflictingLayerIdentifier.c_str(), conflictingSpecPath.GetString().c_str(),
            conflictingValueType.GetText());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInconsistentAttributeVariabilityPtr PcpErrorInconsistentAttributeVariability::New() {
    return PcpErrorInconsistentAttributeVariabilityPtr(new PcpErrorInconsistentAttributeVariability);
}
...
std::string PcpErrorInconsistentAttributeVariability::ToString() const {
    return TfStringPrintf(
            ...
            rootSite.path.GetString().c_str(), definingLayerIdentifier.c_str(), definingSpecPath.GetString().c_str(),
            TfEnum::GetName(definingVariability).c_str(), conflictingLayerIdentifier.c_str(),
            conflictingSpecPath.GetString().c_str(), TfEnum::GetName(conflictingVariability).c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidPrimPathPtr PcpErrorInvalidPrimPath::New() {
...
std::string PcpErrorInvalidPrimPath::ToString() const {
    return TfStringPrintf(
            "Invalid %s path <%s> introduced by %s"
            "-- must be an absolute prim path with no "
            "variant selections.",
            TfEnum::GetDisplayName(arcType).c_str(), primPath.GetText(),
            TfStringify(PcpSite(sourceLayer, site.path)).c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidAssetPathBase::PcpErrorInvalidAssetPathBase(PcpErrorType errorType) : PcpErrorBase(errorType) {}
...
PcpErrorInvalidAssetPathPtr PcpErrorInvalidAssetPath::New() {
    return PcpErrorInvalidAssetPathPtr(new PcpErrorInvalidAssetPath);
}

PcpErrorInvalidAssetPath::PcpErrorInvalidAssetPath() : PcpErrorInvalidAssetPathBase(PcpErrorType_InvalidAssetPath) {}

PcpErrorInvalidAssetPath::~PcpErrorInvalidAssetPath() {}

// virtual
std::string PcpErrorInvalidAssetPath::ToString() const {
    return TfStringPrintf(
            "Could not open asset @%s@ for "
            "%s introduced by %s%s%s.",
            resolvedAssetPath.c_str(), TfEnum::GetDisplayName(arcType).c_str(),
            TfStringify(PcpSite(sourceLayer, site.path)).c_str(), messages.empty() ? "" : " -- ", messages.c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorMutedAssetPathPtr PcpErrorMutedAssetPath::New() {
...
// virtual
std::string PcpErrorMutedAssetPath::ToString() const {
    return TfStringPrintf("Asset @%s@ was muted for %s introduced by %s.", resolvedAssetPath.c_str(),
...
PcpErrorTargetPathBase::PcpErrorTargetPathBase(PcpErrorType errorType) : PcpErrorBase(errorType) {}

PcpErrorTargetPathBase::~PcpErrorTargetPathBase() {}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidInstanceTargetPathPtr PcpErrorInvalidInstanceTargetPath::New() {
    return PcpErrorInvalidInstanceTargetPathPtr(new PcpErrorInvalidInstanceTargetPath);
}
...
// virtual
std::string PcpErrorInvalidInstanceTargetPath::ToString() const {
    ...
    return TfStringPrintf(
            ...
            targetPath.GetText(), owningPath.GetText(), layer->GetIdentifier().c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidExternalTargetPathPtr PcpErrorInvalidExternalTargetPath::New() {
    return PcpErrorInvalidExternalTargetPathPtr(new PcpErrorInvalidExternalTargetPath);
}
...
// virtual
std::string PcpErrorInvalidExternalTargetPath::ToString() const {
    TF_VERIFY(ownerSpecType == SdfSpecTypeAttribute || ownerSpecType == SdfSpecTypeRelationship);
    return TfStringPrintf(
            ...
            (ownerSpecType == SdfSpecTypeAttribute ? "attribute connection" : "relationship target"),
            targetPath.GetText(), owningPath.GetText(), layer->GetIdentifier().c_str(),
            TfEnum::GetDisplayName(TfEnum(ownerArcType)).c_str(), ownerIntroPath.GetText());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidTargetPathPtr PcpErrorInvalidTargetPath::New() {
    return PcpErrorInvalidTargetPathPtr(new PcpErrorInvalidTargetPath);
}

PcpErrorInvalidTargetPath::PcpErrorInvalidTargetPath() : PcpErrorTargetPathBase(PcpErrorType_InvalidTargetPath) {}

PcpErrorInvalidTargetPath::~PcpErrorInvalidTargetPath() {}

// virtual
std::string PcpErrorInvalidTargetPath::ToString() const {
    TF_VERIFY(ownerSpecType == SdfSpecTypeAttribute || ownerSpecType == SdfSpecTypeRelationship);
    return TfStringPrintf(
...
PcpErrorInvalidSublayerOffsetPtr PcpErrorInvalidSublayerOffset::New() {
    return PcpErrorInvalidSublayerOffsetPtr(new PcpErrorInvalidSublayerOffset);
}

PcpErrorInvalidSublayerOffset::PcpErrorInvalidSublayerOffset() : PcpErrorBase(PcpErrorType_InvalidSublayerOffset) {}
...
std::string PcpErrorInvalidSublayerOffset::ToString() const {
    return TfStringPrintf(
            ...
            TfStringify(offset).c_str(), sublayer->GetIdentifier().c_str(), layer->GetIdentifier().c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidReferenceOffsetPtr PcpErrorInvalidReferenceOffset::New() {
    return PcpErrorInvalidReferenceOffsetPtr(new PcpErrorInvalidReferenceOffset);
}

PcpErrorInvalidReferenceOffset::PcpErrorInvalidReferenceOffset() : PcpErrorBase(PcpErrorType_InvalidReferenceOffset) {}
...
std::string PcpErrorInvalidReferenceOffset::ToString() const {
    return TfStringPrintf(
            ...
            TfEnum::GetDisplayName(arcType).c_str(), TfStringify(offset).c_str(), assetPath.c_str(),
            targetPath.GetText(), TfStringify(PcpSite(sourceLayer, sourcePath)).c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidSublayerOwnershipPtr PcpErrorInvalidSublayerOwnership::New() {
    return PcpErrorInvalidSublayerOwnershipPtr(new PcpErrorInvalidSublayerOwnership);
}
...
std::string PcpErrorInvalidSublayerOwnership::ToString() const {
    ...
    return TfStringPrintf(
            ...
            "same owner '%s': %s",
            layer->GetIdentifier().c_str(), owner.c_str(), TfStringJoin(sublayerStrVec, ", ").c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorInvalidSublayerPathPtr PcpErrorInvalidSublayerPath::New() {
...
std::string PcpErrorInvalidSublayerPath::ToString() const {
    return TfStringPrintf(
            ...
            "skipping.",
            sublayerPath.c_str(), layer ? layer->GetIdentifier().c_str() : "<NULL>", messages.empty() ? "" : " -- ",
            messages.c_str());
}
...
std::string PcpErrorInvalidAuthoredRelocation::ToString() const {
    return TfStringPrintf(
            ...
            sourcePath.GetText(), targetPath.GetText(), layer->GetIdentifier().c_str(), owningPath.GetText(),
            messages.c_str());
}
...
std::string PcpErrorInvalidConflictingRelocation::ToString() const {
    ...
    return TfStringPrintf(
            ...
            sourcePath.GetText(), targetPath.GetText(), layer->GetIdentifier().c_str(), owningPath.GetText(),
            conflictSourcePath.GetText(), conflictTargetPath.GetText(), conflictLayer->GetIdentifier().c_str(),
            conflictOwningPath.GetText(), conflictReasonCstr(conflictReason));
}
...
std::string PcpErrorInvalidSameTargetRelocations::ToString() const {
    auto sourceToString = [](const RelocationSource& source) {
        return TfStringPrintf("relocation from <%s> authored at @%s@<%s>", source.sourcePath.GetText(),
                              source.layer->GetIdentifier().c_str(), source.owningPath.GetText());
    };
    ...
    return TfStringPrintf(
            ...
            targetPath.GetText(), sourcesString.c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorOpinionAtRelocationSourcePtr PcpErrorOpinionAtRelocationSource::New() {
    return PcpErrorOpinionAtRelocationSourcePtr(new PcpErrorOpinionAtRelocationSource);
}
...
PcpErrorPrimPermissionDeniedPtr PcpErrorPrimPermissionDenied::New() {
    return PcpErrorPrimPermissionDeniedPtr(new PcpErrorPrimPermissionDenied);
}

PcpErrorPrimPermissionDenied::PcpErrorPrimPermissionDenied() : PcpErrorBase(PcpErrorType_PrimPermissionDenied) {}
...
std::string PcpErrorPrimPermissionDenied::ToString() const {
    return TfStringPrintf(
            ...
            TfStringify(site).c_str(), TfStringify(privateSite).c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorPropertyPermissionDeniedPtr PcpErrorPropertyPermissionDenied::New() {
    return PcpErrorPropertyPermissionDeniedPtr(new PcpErrorPropertyPermissionDenied);
}
...
std::string PcpErrorPropertyPermissionDenied::ToString() const {
    return TfStringPrintf(
            ...
            layerPath.c_str(), propType == SdfSpecTypeAttribute ? "an attribute" : "a relationship",
            propPath.GetText());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorSublayerCyclePtr PcpErrorSublayerCycle::New() {
...
std::string PcpErrorSublayerCycle::ToString() const {
    return TfStringPrintf(
            ...
            layer->GetIdentifier().c_str(), sublayer->GetIdentifier().c_str());
}

///////////////////////////////////////////////////////////////////////////////

PcpErrorTargetPermissionDeniedPtr PcpErrorTargetPermissionDenied::New() {
    return PcpErrorTargetPermissionDeniedPtr(new PcpErrorTargetPermissionDenied);
}
...
// virtual
std::string PcpErrorTargetPermissionDenied::ToString() const {
    TF_VERIFY(ownerSpecType == SdfSpecTypeAttribute || ownerSpecType == SdfSpecTypeRelationship);
    return TfStringPrintf(
...
PcpErrorUnresolvedPrimPathPtr PcpErrorUnresolvedPrimPath::New() {
...
// virtual
std::string PcpErrorUnresolvedPrimPath::ToString() const {
    return TfStringPrintf("Unresolved %s prim path %s introduced by %s", TfEnum::GetDisplayName(arcType).c_str(),
...
PcpErrorVariableExpressionErrorPtr PcpErrorVariableExpressionError::New() {
    return PcpErrorVariableExpressionErrorPtr(new PcpErrorVariableExpressionError);
}
...
std::string PcpErrorVariableExpressionError::ToString() const {
    ...
    auto makeSourceStr = [this]() {
        ...
        if (!sourcePath.IsAbsoluteRootPath()) {
            result += TfStringPrintf("at %s ", sourcePath.GetAsString().c_str());
        }
        result += TfStringPrintf("in @%s@", sourceLayer ? sourceLayer->GetIdentifier().c_str() : "<expired>");
        return result;
    };

    return TfStringPrintf(R"(Error evaluating expression %s for %s %s: %s)", expression.substr(0, 32).c_str(),
                          context.c_str(), makeSourceStr().c_str(), expressionError.c_str());
}
