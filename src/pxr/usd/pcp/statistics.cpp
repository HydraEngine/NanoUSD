...
public:
    static void AccumulateGraphStats(const PcpPrimIndex& primIndex, Pcp_GraphStats* stats, bool culledNodesOnly) {
        for (const PcpNodeRef& node : primIndex.GetNodeRange()) {
            ...
            if (nodeIsImpliedInherit) {
                if (node.GetArcType() == PcpArcTypeInherit) ++(stats->numImpliedInherits);
            }
        }
    }
    ...
    struct _Helper {
        static std::string FormatNumber(size_t n) { return TfStringPrintf("%zd", n); }
    ...
    static void PrintGraphStats(const Pcp_GraphStats& totalStats,
        ...
        for (PcpArcType t = PcpArcTypeRoot; t != PcpNumArcTypes; t = (PcpArcType)(t + 1)) {
            ...
            out << "    " << nodeTypeName << ": "
                << TfStringPrintf("%*s%s / %s", (int)(31 - nodeTypeName.size()), "",
    ...
    static void PrintCacheStats(const PcpCache* cache, std::ostream& out) {
        ...
        out << "  Property indexes:                  " << _Helper::FormatNumber(stats.numPropertyIndexes) << endl;
        out << endl;

        out << "Prim graphs: " << endl;
        PrintGraphStats(stats.allGraphStats, stats.culledGraphStats, out);
        out << endl;
        ...
        out << "  sizeof(PcpPrimIndex_Graph):        " << _Helper::FormatSize(sizeof(PcpPrimIndex_Graph)) << endl;
