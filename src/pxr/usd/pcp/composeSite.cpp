...
static void _PcpComposeSiteReferencesOrPayloads(TfToken const& field,
    ...
    for (size_t i = layers.size(); i-- != 0;) {
        ...
        const SdfLayerOffset* layerOffset = layerStack->GetLayerOffsetForLayer(i);
        ...
        curListOp.ApplyOperations(
                result,
                [&](SdfListOpType opType, const RefOrPayloadType& refOrPayload) -> std::optional<RefOrPayloadType> {
                    ...
                    if (Pcp_IsVariableExpression(authoredAssetPath)) {
                        authoredAssetPath =
                                Pcp_EvaluateVariableExpression(authoredAssetPath, layerStack->GetExpressionVariables(),
                    ...
                    RefOrPayloadType result(assetPath, refOrPayload.GetPrimPath(), refOrPayload.GetLayerOffset());
...
void PcpComposeSiteReferences(PcpLayerStackRefPtr const& layerStack,
                              ...
                              PcpErrorVector* errors) {
    _PcpComposeSiteReferencesOrPayloads(SdfFieldKeys->References, layerStack, path, result, info, stageVarDependencies,
                                        errors);
}
...
void PcpComposeSitePayloads(PcpLayerStackRefPtr const& layerStack,
                            ...
                            std::unordered_set<std::string>* stageVarDependencies,
                            PcpErrorVector* errors) {
    _PcpComposeSiteReferencesOrPayloads(SdfFieldKeys->Payload, layerStack, path, result, info, stageVarDependencies,
                                        errors);
}
...
SdfPermission PcpComposeSitePermission(PcpLayerStackRefPtr const& layerStack, SdfPath const& path) {
    ...
    for (auto const& layer : layerStack->GetLayers()) {
        if (layer->HasField(path, SdfFieldKeys->Permission, &perm)) break;
    }
    return perm;
}
...
void PcpComposeSitePrimSites(PcpLayerStackRefPtr const& layerStack, SdfPath const& path, SdfSiteVector* result) {
    for (auto const& layer : layerStack->GetLayers()) {
        if (layer->HasSpec(path)) result->push_back(SdfSite(layer, path));
    }
}

void PcpComposeSiteRelocates(PcpLayerStackRefPtr const& layerStack, SdfPath const& path, SdfRelocatesMap* result) {
...
void PcpComposeSiteInherits(const PcpLayerStackRefPtr& layerStack, const SdfPath& path, SdfPathVector* result) {
    static const TfToken field = SdfFieldKeys->InheritPaths;
...
void PcpComposeSiteSpecializes(PcpLayerStackRefPtr const& layerStack, SdfPath const& path, SdfPathVector* result) {
    static const TfToken field = SdfFieldKeys->Specializes;
...
bool PcpComposeSiteVariantSelection(PcpLayerStackRefPtr const& layerStack,
    ...
    for (auto const& layer : layerStack->GetLayers()) {
        ...
        if (Pcp_IsVariableExpression(*vsel)) {
            ...
            *vsel = Pcp_EvaluateVariableExpression(*vsel, layerStack->GetExpressionVariables(), "variant", layer, path,
            ...
            if (!exprErrors.empty()) {
                if (errors) {
                    errors->insert(errors->end(), std::make_move_iterator(exprErrors.begin()),
...
void PcpComposeSiteVariantSelections(PcpLayerStackRefPtr const& layerStack,
    ...
    for (auto const& layer : layerStack->GetLayers()) {
        ...
        for (auto it = vselMap.begin(); it != vselMap.end();) {
            ...
            if (Pcp_IsVariableExpression(vsel)) {
                PcpErrorVector exprErrors;
                vsel = Pcp_EvaluateVariableExpression(vsel, layerStack->GetExpressionVariables(), "variant", layer,
                                                      path, exprVarDependencies, &exprErrors);
    ...
    TF_REVERSE_FOR_ALL(layer, layers) {
        ...
        if (orderField) {
            ...
            if (orderVal.IsHolding<TfTokenVector>()) {
                SdfApplyListOrdering(nameOrder, orderVal.UncheckedGet<TfTokenVector>());
            }
        }
    }
}
