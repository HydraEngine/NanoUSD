...
enum class UsdUtils_DependencyType { Reference, Sublayer, Payload, ClipTemplateAssetPath };
...
struct UsdUtils_LocalizationDelegate {
    using ProcessingFunc = std::function<UsdUtilsDependencyInfo(const SdfLayerRefPtr& layer,
                                                                const UsdUtilsDependencyInfo& dependencyInfo,
                                                                UsdUtils_DependencyType dependencyType)>;

    virtual std::vector<std::string> ProcessSublayers(const SdfLayerRefPtr& layer) { return {}; }

    virtual std::vector<std::string> ProcessPayloads(const SdfLayerRefPtr& layer, const SdfPrimSpecHandle& primSpec) {
        return {};
    }

    virtual std::vector<std::string> ProcessReferences(const SdfLayerRefPtr& layer, const SdfPrimSpecHandle& primSpec) {
        return {};
    }
    ...
    virtual void BeginProcessValue(const SdfLayerRefPtr& layer, const VtValue& val) {}

    virtual std::vector<std::string> ProcessValuePath(const SdfLayerRefPtr& layer,
    ...
    virtual std::vector<std::string> ProcessValuePathArrayElement(const SdfLayerRefPtr& layer,
                                                                  const std::string& keyPath,
                                                                  const std::string& authoredPath,
                                                                  const std::vector<std::string>& dependencies) {
        return {};
    ...
    virtual void EndProcessingValuePathArray(const SdfLayerRefPtr& layer, const std::string& keyPath) {}

    virtual void EndProcessTimeSampleValue(const SdfLayerRefPtr& layer,
                                           const SdfPath& path,
                                           double t,
                                           const VtValue& val) {}

    virtual void EndProcessValue(const SdfLayerRefPtr& layer,
                                 const SdfPath& path,
                                 const TfToken& key,
                                 const VtValue& val) {};

    virtual std::vector<std::string> ProcessClipTemplateAssetPath(const SdfLayerRefPtr& layer,
...
public:
    UsdUtils_ProcessedPathCache(const UsdUtils_LocalizationDelegate::ProcessingFunc& processingFun)
        : _processingFunc(processingFun) {}

    UsdUtilsDependencyInfo GetProcessedInfo(const SdfLayerRefPtr& layer,
                                            const UsdUtilsDependencyInfo& dependencyInfo,
                                            UsdUtils_DependencyType dependencyType);
...
private:
    ...
    struct ProcessedPathHash {
        size_t operator()(const PathKey& key) const { return TfHash::Combine(std::get<0>(key), std::get<1>(key)); }
    };
...
class UsdUtils_WritableLocalizationDelegate : public UsdUtils_LocalizationDelegate {
public:
    UsdUtils_WritableLocalizationDelegate(ProcessingFunc processingFunc) : _pathCache(processingFunc) {}
    ...
    virtual void BeginProcessValue(const SdfLayerRefPtr& layer, const VtValue& val) override;
    ...
    virtual void EndProcessingValuePathArray(const SdfLayerRefPtr& layer, const std::string& keyPath) override;
    ...
    inline void SetEditLayersInPlace(bool editLayersInPlace) { _editLayersInPlace = editLayersInPlace; }
    ...
    // processing.
    inline void SetKeepEmptyPathsInArrays(bool keep) { _keepEmptyPathsInArrays = keep; }
...
private:
    ...
    std::vector<std::string> _ProcessReferencesOrPayloads(const SdfLayerRefPtr& layer,
                                                          const SdfPrimSpecHandle& primSpec,
                                                          const TfToken& listOpToken);

    template <class RefOrPayloadType, UsdUtils_DependencyType DEP_TYPE>
    std::optional<RefOrPayloadType> _ProcessRefOrPayload(const SdfLayerRefPtr& layer,
...
class UsdUtils_ReadOnlyLocalizationDelegate : public UsdUtils_LocalizationDelegate {
public:
    UsdUtils_ReadOnlyLocalizationDelegate(ProcessingFunc processingFunc) : _pathCache(processingFunc) {}
...
private:
    ...
    std::vector<std::string> ProcessReferencesOrPayloads(const SdfLayerRefPtr& layer,
