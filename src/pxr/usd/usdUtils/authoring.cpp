...
bool UsdUtilsCopyLayerMetadata(const SdfLayerHandle& source,
    ...
    if (!TF_VERIFY(source && destination)) return false;
    ...
    if (bakeUnauthoredFallbacks) {
        bool bakeColorConfiguration =
                std::find(infoKeys.begin(), infoKeys.end(), SdfFieldKeys->ColorConfiguration) == infoKeys.end();
        bool bakeColorManagementSystem =
                std::find(infoKeys.begin(), infoKeys.end(), SdfFieldKeys->ColorManagementSystem) == infoKeys.end();
...
static SdfPathVector _GetPathsToExcludeBelowCommonAncestor(const UsdUtilsPathHashSet& includedRootPaths,
    ...
    UsdPrimRange commonAncestorRange(commonAncestor, UsdTraverseInstanceProxies(UsdPrimAllPrimsPredicate));
...
static bool _ComputePathsToIncludeAndExclude(const UsdUtilsPathHashSet& includedRootPaths,
    ...
    SdfPathVector pathsToExcludeBelowCommonAncestor =
            _GetPathsToExcludeBelowCommonAncestor(includedRootPaths, commonAncestor, pathsToIgnore);
        ...
        if (inclPathCount > 0) {
            ...
            if (inclusionRatio >= minInclusionRatio && exclPathCount <= maxNumExcludesBelowInclude) {
                ...
                if (exclPathCount > 0) {
                    ...
                    pathsToExclude->insert(pathsToExclude->end(), excludedPaths.begin(), excludedPaths.end());
                }
...
bool UsdUtilsComputeCollectionIncludesAndExcludes(const SdfPathSet& includedRootPaths,
    ...
    if (includedRootPaths.size() < minIncludeExcludeCollectionSize) {
        pathsToInclude->insert(pathsToInclude->end(), includedRootPaths.begin(), includedRootPaths.end());
        return true;
    }
    ...
    return _ComputePathsToIncludeAndExclude(includedRootPathsHashSet, commonAncestor, minInclusionRatio,
                                            maxNumExcludesBelowInclude, pathsToInclude, pathsToExclude, pathsToIgnore);
}
...
std::vector<UsdCollectionAPI> UsdUtilsCreateCollections(const std::vector<std::pair<TfToken, SdfPathSet>>& assignments,
                                                        const UsdPrim& usdPrim,
                                                        double minInclusionRatio /* 0.75 */,
    ...
    WorkParallelForN(assignments.size(), [&](size_t start, size_t end) {
        for (size_t i = start; i < end; ++i) {
            ...
            UsdUtilsComputeCollectionIncludesAndExcludes(includedRootPaths, usdStage, &pathsToInclude, &pathsToExclude,
    ...
    for (size_t i = 0; i < assignments.size(); ++i) {
        ...
        UsdCollectionAPI coll = UsdUtilsAuthorCollection(collectionName, usdPrim, includesAndExcludes.first,
