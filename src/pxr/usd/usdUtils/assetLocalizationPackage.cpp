...
std::string UsdUtils_DirectoryRemapper::Remap(const std::string& filePath) {
    ...
    if (insertStatus.second) {
        insertStatus.first->second = TfStringPrintf("%zu", _nextDirectoryNum++);
    }

    return TfStringCatPaths(insertStatus.first->second, baseName);
}

bool UsdUtils_AssetLocalizationPackage::Build(const SdfAssetPath& assetPath, const std::string& firstLayerName) {
    ...
    if (!_rootLayer) {
        TF_WARN("Failed to find or open root asset layer: %s", assetPathStr.c_str());
        return false;
    }

    // Change the first layer's name if requested.
    _firstLayerName = firstLayerName.empty() ? TfGetBaseName(_rootLayer->GetRealPath()) : firstLayerName;
...
bool UsdUtils_AssetLocalizationPackage::Write(const std::string& packagePath) {
...
UsdUtilsDependencyInfo UsdUtils_AssetLocalizationPackage::_ProcessDependency(const SdfLayerRefPtr& layer,
                                                                             const UsdUtilsDependencyInfo& depInfo,
                                                                             UsdUtils_DependencyType dependencyType) {
    if (_userProcessingFunc) {
        UsdUtilsDependencyInfo processedInfo = _userProcessingFunc(layer, depInfo);
...
UsdUtilsDependencyInfo UsdUtils_AssetLocalizationPackage::_AddDependenciesToPackage(
        const SdfLayerRefPtr& layer, const UsdUtilsDependencyInfo& depInfo) {
    ...
    // the final output.
    if (layer->GetFileFormat()->IsPackage() || ArIsPackageRelativePath(layer->GetRealPath())) {
        return depInfo;
    }

    bool isRelativeOutput;
    const std::string remappedPath = _ProcessAssetPath(layer, depInfo.GetAssetPath(), &isRelativeOutput);
    ...
    if (isRelativeOutput) {
        ...
        const auto containingLayerInfo = _layersToCopy.find(layer->GetIdentifier());

        if (containingLayerInfo != _layersToCopy.end()) {
            packagePath =
                    TfNormPath(TfStringCatPaths(TfGetPathName(containingLayerInfo->second), depInfo.GetAssetPath()));
        }
    }
...
void UsdUtils_AssetLocalizationPackage::_AddDependencyToPackage(const SdfLayerRefPtr& layer,
                                                                const std::string& dependency,
                                                                const std::string& destDirectory) {
    const std::string dependencyAnchored = SdfComputeAssetPathRelativeToLayer(layer, dependency);
    const std::string dependencyPackage = TfNormPath(TfStringCatPaths(destDirectory, TfGetBaseName(dependency)));

    if (UsdStage::IsSupportedFile(dependencyAnchored)) {
        _layersToCopy[dependencyAnchored] = dependencyPackage;
    } else {
        _filesToCopy.emplace_back(std::make_pair(dependencyAnchored, dependencyPackage));
    }
}
...
static bool _PathIsURIResolvable(const std::string& path) {
    ...
    return std::binary_search(registeredSchemes.begin(), registeredSchemes.end(), scheme);
}

std::string UsdUtils_AssetLocalizationPackage::_ProcessAssetPath(const SdfLayerRefPtr& layer,
    ...
    if (!isContextDependentPath && !_PathIsURIResolvable(refPath)) {
        ...
        // it's relative.
        const std::string anchored = resolver.CreateIdentifier(refPath, layer->GetResolvedPath());
        const std::string unanchored = resolver.CreateIdentifier(refPath);
        ...
        if (isRelativePath) {
            ...
            const bool refPathIsOutsideAssetLocation =
                    !resolvedRefPath.empty() && !TfStringStartsWith(TfNormPath(TfGetPathName(resolvedRefPath)),
    ...
    if (result == layerPath) {
        ...
        // renamed root layer.
        return resultPointsToRoot && !_firstLayerName.empty() ? _firstLayerName : TfGetBaseName(result);
    }
    ...
    if (resultPointsToRoot && layerPath == rootFilePath) {
        return !_firstLayerName.empty() ? _firstLayerName : TfGetBaseName(result);
    }
...
bool UsdUtils_AssetLocalizationPackage::_AddLayerToPackage(SdfLayerRefPtr sourceLayer, const std::string& destPath) {
    SdfLayerConstHandle layer = _delegate.GetLayerUsedForWriting(sourceLayer);
    TF_DEBUG(USDUTILS_CREATE_PACKAGE)
    ...
    const auto extensionsMatch = [](const std::string& src, const std::string& dest) {
        auto& resolver = ArGetResolver();
    ...
    if (layer->GetFileFormat()->IsPackage() || ArIsPackageRelativePath(layer->GetIdentifier())) {
    ...
    } else if (!layer->IsDirty() && extensionsMatch(layer->GetRealPath(), destPath)) {
        ...
        const SdfFileFormatConstPtr fileFormat =
                SdfFileFormat::FindByExtension(SdfFileFormat::GetFileExtension(destPath));

        if (TfDynamic_cast<UsdUsdFileFormatConstPtr>(fileFormat)) {
            args[UsdUsdFileFormatTokens->FormatArg] =
                    UsdUsdFileFormat::GetUnderlyingFormatForLayer(*get_pointer(layer));
        }
        ...
        if (!_WriteToPackage(tmpLayerExportPath, destPath)) {
            ...
            TF_WARN("Failed to add temporary layer at '%s' to the package "
                    "at path '%s'.",
                    tmpLayerExportPath.c_str(), _packagePath.c_str());
            return false;
        } else {
...
bool UsdUtils_AssetLocalizationPackage::_AddAssetToPackage(const std::string& srcPath, const std::string& destPath) {
    TF_DEBUG(USDUTILS_CREATE_PACKAGE)
    ...
    if (ArIsPackageRelativePath(destPath)) {
        ...
        if (!_WriteToPackage(srcPath, destPath)) {
            // XXX: Should we abort package creation and return early here?
            TF_WARN("Failed to add file '%s' to the package at path '%s'.", srcPath.c_str(), _packagePath.c_str());
            return false;
        }
    }
...
std::string UsdUtils_AssetLocalizationPackage::_RemapAssetPath(const SdfLayerRefPtr& layer,
                                                               const std::string& refPath,
                                                               bool isRelativePath) {
    if (isRelativePath) {
        return refPath;
