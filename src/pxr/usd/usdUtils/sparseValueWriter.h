...
public:
    ...
    USDUTILS_API
    bool SetAttribute(const UsdAttribute& attr, const VtValue& value, const UsdTimeCode time = UsdTimeCode::Default());
    ...
    /// destroyed.
    USDUTILS_API
    bool SetAttribute(const UsdAttribute& attr, VtValue* value, const UsdTimeCode time = UsdTimeCode::Default());
    ...
    USDUTILS_API
    void Clear() { _attrValueWriterMap.clear(); }
...
private:
    ...
    struct _AttrHash {
        inline size_t operator()(const UsdAttribute& attr) const { return hash_value(attr); }
    };
