...
size_t UsdUtilsComputeUsdStageStats(const UsdStageWeakPtr& stage, VtDictionary* stats) {
    ...
    size_t primaryPrimCount = 0, primaryActivePrimCount = 0, primaryInactivePrimCount = 0, primaryPureOverCount = 0,
    ...
    for (UsdPrim prim : stage->TraverseAll()) {
        _UpdateCountsHelper(prim, &seenAssetNames, &totalPrimCount, &primaryPrimCount, &modelCount,
                            &instancedModelCount, &assetCount, &primaryActivePrimCount, &primaryInactivePrimCount,
                            &primaryPureOverCount, &primaryInstanceCount, &primCountsByType);
    }
    ...
    if (prototypeCount > 0) {
        ...
        for (const UsdPrim& prototypePrim : prototypes) {
            for (UsdPrim prim : UsdPrimRange(prototypePrim)) {
                _UpdateCountsHelper(prim, &seenAssetNames, &totalPrimCount, &prototypesPrimCount, &modelCount,
                                    &instancedModelCount, &assetCount, &prototypesActivePrimCount,
                                    &prototypesInactivePrimCount, &prototypesPureOverCount, &prototypesInstanceCount,
