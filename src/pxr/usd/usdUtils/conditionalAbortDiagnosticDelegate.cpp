...
UsdUtilsConditionalAbortDiagnosticDelegateErrorFilters::UsdUtilsConditionalAbortDiagnosticDelegateErrorFilters(
        const std::vector<std::string>& stringFilters, const std::vector<std::string>& codePathFilters)
...
void UsdUtilsConditionalAbortDiagnosticDelegate::IssueError(const TfError& err) {
    ...
    if (_RuleMatcher(err, _includePatternStringFilters, _includePatternCodePathFilters) &&
        ...
        TfLogCrash(
                ...
                "Error",
                err.GetCommentary(), std::string(), err.GetContext(), true);
        ArchAbort(false);
...
void UsdUtilsConditionalAbortDiagnosticDelegate::IssueFatalError(const TfCallContext& ctx, const std::string& msg) {
...
void UsdUtilsConditionalAbortDiagnosticDelegate::IssueWarning(const TfWarning& warning) {
    ...
    if (_RuleMatcher(warning, _includePatternStringFilters, _includePatternCodePathFilters) &&
        ...
        TfLogCrash(
                "Aborted by UsdUtilsConditionalAbortDiagnosticDelegate On "
                "Warning",
                warning.GetCommentary(), std::string(), warning.GetContext(), true);
        ArchAbort(false);
