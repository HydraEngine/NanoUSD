...
template <typename T>
T _GetUnboxedValue(const SdfLayerRefPtr& resultLayer,
...
template <typename T>
void _SetValue(const SdfLayerRefPtr& resultLayer,
               const SdfPath& primPath,
               const TfToken& key,
               const T& value,
               const TfToken& clipSet) {
    resultLayer->SetFieldDictValueByKey(primPath, UsdTokens->clips,
                                        TfToken(clipSet.GetString() + ":" + key.GetString()), value);
}
...
template <typename C>
void _AppendCollection(const SdfLayerRefPtr& lhs,
...
void _RetimeClipActive(const SdfLayerHandle& layer, const SdfPath& path, const TfToken& clipSet) {
    size_t timer = 0;
...
std::string _GetRelativePathIfPossible(const std::string& referencedIdentifier,
    ...
    return resultingIdentifier.empty() ? referencedIdentifier : resultingIdentifier;
}
...
void _NormalizeClipAssetPaths(const SdfLayerHandle& resultLayer,
    ...
    const auto currentAssetPaths =
            _GetUnboxedValue<_SdfAssetArray>(resultLayer, clipPath, UsdClipsAPIInfoKeys->assetPaths, clipSet);
    ...
    for (size_t i = 0; i < currentAssetPaths.size() - diff; ++i) {
        const auto path =
                _GetRelativePathIfPossible(clipLayers[i]->GetIdentifier(), clipLayers[i]->GetRealPath(), resultPath);
        result.push_back(SdfAssetPath(path));
    }

    _SetValue(resultLayer, clipPath, UsdClipsAPIInfoKeys->assetPaths, result, clipSet);
}
...
void _MergeRootLayerMetadata(const SdfLayerRefPtr& lhs,
                             ...
                             const SdfPath& clipPath,
                             const TfToken& clipSet) {
    _AppendCollection<_SdfAssetArray>(lhs, rhs, clipPath, UsdClipsAPIInfoKeys->assetPaths, clipSet);
    _AppendCollection<VtVec2dArray>(lhs, rhs, clipPath, UsdClipsAPIInfoKeys->times, clipSet);
    _AppendCollection<VtVec2dArray>(lhs, rhs, clipPath, UsdClipsAPIInfoKeys->active, clipSet);
}
...
void _StitchClipPrimPath(const SdfLayerRefPtr& resultLayer, const SdfPath& stitchPath, const TfToken& clipSet) {
    _SetValue(resultLayer, stitchPath, UsdClipsAPIInfoKeys->primPath, stitchPath.GetString(), clipSet);
}
...
// layer.
static double _GetStartTimeCode(const SdfLayerConstHandle& layer) {
    return layer->HasStartTimeCode() ? layer->GetStartTimeCode()
                                     : (_HasStartFrame(layer) ? _GetStartFrame(layer) : 0.0);
}

// Backwards compatible helper function for getting the endTimeCode of a
// layer.
static double _GetEndTimeCode(const SdfLayerConstHandle& layer) {
    return layer->HasEndTimeCode() ? layer->GetEndTimeCode() : (_HasEndFrame(layer) ? _GetEndFrame(layer) : 0.0);
}

////////////////////////////////////////////////////////////////////////////

void _StitchClipActive(const SdfLayerRefPtr& resultLayer,
                       const SdfLayerRefPtr& clipLayer,
                       const SdfPath& stitchPath,
                       const TfToken& clipSet) {
    VtVec2dArray currentClipActive =
            _GetUnboxedValue<VtVec2dArray>(resultLayer, stitchPath, UsdClipsAPIInfoKeys->active, clipSet);
    ...
    const double clipIndex =
            static_cast<double>(_GetUnboxedValue<VtArray<SdfAssetPath>>(resultLayer, stitchPath,
...
void _StitchClipTime(const SdfLayerRefPtr& resultLayer,
                     const SdfLayerRefPtr& clipLayer,
                     const SdfPath& stitchPath,
                     const TfToken& clipSet) {
    VtVec2dArray currentClipTimes =
            _GetUnboxedValue<VtVec2dArray>(resultLayer, stitchPath, UsdClipsAPIInfoKeys->times, clipSet);
    ...
    if (resultLayer->GetPrimAtPath(stitchPath)) {
        ...
        _SetValue(resultLayer, stitchPath, UsdClipsAPIInfoKeys->times, currentClipTimes, clipSet);
    }
}

void _StitchClipsTopologySubLayerPath(const SdfLayerRefPtr& resultLayer, const std::string& topIdentifier) {
...
void _StitchClipAssetPath(const SdfLayerRefPtr& resultLayer,
                          const SdfLayerRefPtr& clipLayer,
                          const SdfPath& stitchPath,
                          const TfToken& clipSet) {
    _SdfAssetArray currentAssets =
            _GetUnboxedValue<_SdfAssetArray>(resultLayer, stitchPath, UsdClipsAPIInfoKeys->assetPaths, clipSet);

    if (resultLayer->GetPrimAtPath(stitchPath)) {
        std::string clipId = _GetRelativePathIfPossible(clipLayer->GetIdentifier(), clipLayer->GetRealPath(),
                                                        resultLayer->GetRealPath());

        currentAssets.push_back(SdfAssetPath(clipId));
        _SetValue(resultLayer, stitchPath, UsdClipsAPIInfoKeys->assetPaths, currentAssets, clipSet);
    }
}

// Add the clipManifestAssetPath metadata at the specified \p stitchPath
// within the \p resultLayer.
void _StitchClipManifest(const SdfLayerRefPtr& resultLayer,
                         const SdfLayerRefPtr& manifestLayer,
                         const SdfPath& stitchPath,
                         const TfToken& clipSet) {
    const std::string manifestAssetPath = _GetRelativePathIfPossible(
            manifestLayer->GetIdentifier(), manifestLayer->GetRealPath(), resultLayer->GetRealPath());

    _SetValue(resultLayer, stitchPath, UsdClipsAPIInfoKeys->manifestAssetPath, SdfAssetPath(manifestAssetPath),
              clipSet);
}
...
void _StitchClipMetadata(const SdfLayerRefPtr& resultLayer,
                         const SdfLayerRefPtr& clipLayer,
                         const SdfPath& stitchPath,
...
void _SetTimeCodeRange(const SdfLayerHandle& resultLayer,
    ...
    if (!resultLayer->GetPrimAtPath(clipDataPath)) {
        TF_CODING_ERROR("Invalid prim in path: @%s@<%s>", resultLayer->GetIdentifier().c_str(),
    ...
    VtVec2dArray currentClipTimes =
            _GetUnboxedValue<VtVec2dArray>(resultLayer, clipDataPath, UsdClipsAPIInfoKeys->times, clipSet);

    // sort based on stage frame number
    std::sort(currentClipTimes.begin(), currentClipTimes.end(), [](const GfVec2d& v1, const GfVec2d& v2) {
        return v1[0] < v2[0];
    });
...
void _StitchLayersIgnoringTimeSamples(const SdfLayerHandle& strongLayer, const SdfLayerHandle& weakLayer) {
...
struct _StitchLayersResult {
    ...
    _StitchLayersResult(const SdfPath& _clipPath, const TfToken& _clipSet)
        : clipPath(_clipPath),
...
_StitchLayersResult _AggregateDataFromClips(const SdfLayerRefPtr& topologyLayer,
                                            const SdfLayerRefPtrVector& clipLayers,
    ...
    tbb::blocked_range<SdfLayerRefPtrVector::const_iterator> clipRange(clipLayers.begin(), clipLayers.end());
    tbb::parallel_reduce(clipRange, result);

    return result;
}

void _GenerateClipManifest(const SdfLayerRefPtr& manifestLayer,
                           const SdfLayerRefPtr& topologyLayer,
                           const SdfLayerRefPtrVector& clipLayers,
                           const SdfPath& clipPath) {
    SdfLayerRefPtr generatedManifest = UsdClipsAPI::GenerateClipManifestFromLayers(
            SdfLayerHandleVector(clipLayers.begin(), clipLayers.end()), clipPath);

    generatedManifest->Traverse(clipPath, [&generatedManifest, &manifestLayer, &topologyLayer](const SdfPath& path) {
        ...
        SdfJustCreatePrimAttributeInLayer(manifestLayer, path, generatedAttr->GetTypeName(),
                                          generatedAttr->GetVariability(), generatedAttr->IsCustom());
...
void _StitchLayers(const SdfLayerHandle& resultLayer,
                   const SdfLayerRefPtr& topologyLayer,
                   const SdfLayerRefPtr& manifestLayer,
                   const SdfLayerRefPtrVector& clipLayers,
                   const SdfPath& clipPath,
                   const TfToken& clipSet) {
    auto result = _AggregateDataFromClips(topologyLayer, clipLayers, clipPath, clipSet);
    ...
    _GenerateClipManifest(manifestLayer, topologyLayer, clipLayers, clipPath);
...
bool _UsdUtilsStitchClipsTopologyImpl(const SdfLayerRefPtr& topologyLayer, const SdfLayerRefPtrVector& clipLayers) {
...
bool _UsdUtilsStitchClipsManifestImpl(const SdfLayerRefPtr& manifestLayer,
                                      const SdfLayerRefPtr& topologyLayer,
                                      const SdfLayerRefPtrVector& clipLayers,
                                      const SdfPath& clipPath) {
    TfErrorMark errorMark;
...
bool _UsdUtilsStitchClipsImpl(const SdfLayerHandle& resultLayer,
    ...
    if (interpolateMissingClipValues) {
        _SetValue(resultLayer, clipPath, UsdClipsAPIInfoKeys->interpolateMissingClipValues,
...
bool _OpenClipLayers(SdfLayerRefPtrVector* clipLayers, const _ClipFileVector& clipLayerFiles, const SdfPath& clipPath) {
    TfErrorMark errorMark;
    ...
    return errorMark.IsClean() && _ClipLayersAreValid(*clipLayers, clipLayerFiles, clipPath);
}
}  // namespace

// public facing API
// ----------------------------------------------------------------------------
bool UsdUtilsStitchClipsTopology(const SdfLayerHandle& topologyLayer, const _ClipFileVector& clipLayerFiles) {
...
bool UsdUtilsStitchClipsManifest(const SdfLayerHandle& manifestLayer,
    ...
    if (!clipLayersAreValid || !_UsdUtilsStitchClipsManifestImpl(manifestLayer, topologyLayer, clipLayers, clipPath)) {
        return false;
    }
...
bool UsdUtilsStitchClips(const SdfLayerHandle& resultLayer,
    ...
    // Open all clip layers and validate clipPath
    SdfLayerRefPtrVector clipLayers;
    const bool clipLayersAreValid = _OpenClipLayers(&clipLayers, clipLayerFiles, clipPath);

    if (!clipLayersAreValid ||
        !_UsdUtilsStitchClipsImpl(resultLayer, topologyLayer, manifestLayer, clipLayers, clipPath, startTimeCode,
                                  endTimeCode, interpolateMissingClipValues, clipSet)) {
...
std::string UsdUtilsGenerateClipTopologyName(const std::string& baseFileName) {
    ...
    return std::string(baseFileName).insert(delimiterPos, delimiter + topologyFileBaseName);
}

std::string UsdUtilsGenerateClipManifestName(const std::string& baseFileName) {
    const std::string delimiter = ".";
    const std::size_t delimiterPos = baseFileName.rfind(".");
    ...
    return std::string(baseFileName).insert(delimiterPos, delimiter + manifestFileBaseName);
}

bool UsdUtilsStitchClipsTemplate(const SdfLayerHandle& resultLayer,
                                 const SdfLayerHandle& topologyLayer,
    ...
    const std::string topologyId = _GetRelativePathIfPossible(topologyLayer->GetIdentifier(),
                                                              topologyLayer->GetRealPath(), resultLayer->GetRealPath());
    const std::string manifestId = _GetRelativePathIfPossible(manifestLayer->GetIdentifier(),
                                                              manifestLayer->GetRealPath(), resultLayer->GetRealPath());
