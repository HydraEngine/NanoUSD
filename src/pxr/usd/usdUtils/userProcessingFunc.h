...
public:
    ...
    USDUTILS_API explicit UsdUtilsDependencyInfo(const std::string& assetPath) : _assetPath(assetPath) {}
    ...
    USDUTILS_API const std::string& GetAssetPath() const { return _assetPath; }
    ...
    USDUTILS_API const std::vector<std::string>& GetDependencies() const { return _dependencies; }

    /// Equality: Asset path and dependencies are the same
    bool operator==(const UsdUtilsDependencyInfo& rhs) const {
        return _assetPath == rhs._assetPath && _dependencies == rhs._dependencies;
    }

    /// Inequality operator
    /// \sa UsdUtilsDependencyInfo::operator==(const UsdUtilsDependencyInfo&)
    bool operator!=(const UsdUtilsDependencyInfo& rhs) const { return !(*this == rhs); }
...
using UsdUtilsProcessingFunc = UsdUtilsDependencyInfo(const SdfLayerHandle& layer,
