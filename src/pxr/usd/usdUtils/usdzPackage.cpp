...
public:
    ...
    bool Write(const std::string& usdzFilePath) override {
        _writer = UsdZipFileWriter::CreateNew(usdzFilePath);

        const bool success = UsdUtils_AssetLocalizationPackage::Write(usdzFilePath);

        return success && _writer.Save();
    }

protected:
    bool _WriteToPackage(const std::string& source, const std::string& dest) override {
...
static bool _CreateNewUsdzPackage(const SdfAssetPath& assetPath,
...
bool UsdUtilsCreateNewUsdzPackage(const SdfAssetPath& assetPath,
    ...
    return _CreateNewUsdzPackage(assetPath, usdzFilePath, firstLayerName,
                                 /*origRootFilePath*/ "",
                                 /*origRootFilePath*/ {}, editLayersInPlace);
}
...
bool UsdUtilsCreateNewARKitUsdzPackage(const SdfAssetPath& assetPath,
    ...
    UsdUtils_ExtractExternalReferences(resolvedPath, UsdUtils_LocalizationContext::ReferenceType::CompositionOnly,
                                       &sublayers, &references, &payloads);

    // Ensure that the root layer has the ".usdc" extension.
    std::string targetBaseName = firstLayerName.empty() ? TfGetBaseName(assetPath.GetAssetPath()) : firstLayerName;
    ...
    if (fileExt != UsdUsdcFileFormatTokens->Id) {
        ...
        targetBaseName =
                targetBaseName.substr(0, targetBaseName.rfind(".") + 1) + UsdUsdcFileFormatTokens->Id.GetString();
    }
    ...
    if (sublayers.empty() && references.empty() && payloads.empty()) {
        if (renamingRootLayer) {
            return _CreateNewUsdzPackage(assetPath, usdzFilePath,
                                         ...
                                         /*dependenciesToSkip*/ {resolvedPath}, editLayersInPlace);
        } else {
            return _CreateNewUsdzPackage(assetPath, usdzFilePath,
                                         /*firstLayerName*/ targetBaseName,
                                         /*origRootFilePath*/ resolvedPath,
                                         /*dependenciesToSkip*/ {}, editLayersInPlace);
        }
    }
    ...
    TF_DEBUG(USDUTILS_CREATE_PACKAGE)
            .Msg("Flattening asset @%s@ located at '%s' to temporary layer at "
                 "path '%s'.\n",
                 assetPath.GetAssetPath().c_str(), resolvedPath.c_str(), tmpFileName.c_str());
    ...
    bool success = _CreateNewUsdzPackage(
            /*assetPath*/ SdfAssetPath(tmpFileName), usdzFilePath,
            /* firstLayerName */ targetBaseName,
            /* origRootFilePath*/ resolvedPath,
            /*dependenciesToSkip*/ {resolvedPath}, editLayersInPlace);
