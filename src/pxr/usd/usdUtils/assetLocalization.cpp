...
bool UsdUtils_LocalizationContext::Process(const SdfLayerRefPtr& layer) {
    if (!layer) {
...
void UsdUtils_LocalizationContext::_EnqueueDependencies(const SdfLayerRefPtr layer,
...
void UsdUtils_LocalizationContext::_EnqueueDependency(const SdfLayerRefPtr layer, const std::string& assetPath) {
    if (!_recurseLayerDependencies || assetPath.empty()) {
        return;
    }

    const std::string anchoredPath = SdfComputeAssetPathRelativeToLayer(layer, assetPath);

    if (_encounteredPaths.count(anchoredPath) > 0 || _dependenciesToSkip.count(anchoredPath)) {
        return;
    }
    ...
    if (resolvedPath.empty()) {
        TF_WARN("Failed to resolve reference @%s@ with computed asset path "
                "@%s@ found in layer @%s@.",
                assetPath.c_str(), anchoredPath.c_str(), layer->GetRealPath().c_str());
        return;
    }
...
void UsdUtils_LocalizationContext::_ProcessLayer(const SdfLayerRefPtr& layer) {
    ...
    while (!dfs.empty()) {
        ...
        // variants "children"
        for (const SdfVariantSetsProxy::value_type& p : curr->GetVariantSets()) {
            for (const SdfVariantSpecHandle& variantSpec : p.second->GetVariantList()) {
...
void UsdUtils_LocalizationContext::_ProcessSublayers(const SdfLayerRefPtr& layer) {
    ...
    const std::vector<std::string> processedDeps = _delegate->ProcessSublayers(layer);
    _EnqueueDependencies(layer, processedDeps);
}

static std::vector<std::string> _GetClipSets(const SdfPrimSpecHandle& primSpec) {
...
static std::string _GetTemplateAssetPathForClipSet(const SdfPrimSpecHandle& primSpec, const std::string& clipSetName) {
    const VtDictionary clipsDict = primSpec->GetInfo(UsdTokens->clips).UncheckedGet<VtDictionary>();
...
void UsdUtils_LocalizationContext::_ProcessMetadata(const SdfLayerRefPtr& layer, const SdfPrimSpecHandle& primSpec) {
    ...
    for (const auto& clipSet : clipSets) {
        std::string templatePath = _GetTemplateAssetPathForClipSet(primSpec, clipSet);

        if (templatePath.empty()) {
            continue;
        }

        const std::vector<std::string> clipFiles = _GetTemplatedClips(layer, templatePath);

        const std::vector<std::string> dependencies =
                _delegate->ProcessClipTemplateAssetPath(layer, primSpec, clipSet, templatePath, clipFiles);
...
std::vector<std::string> UsdUtils_LocalizationContext::_GetTemplatedClips(const SdfLayerRefPtr& layer,
    ...
    const std::string clipsDirAssetPath = SdfComputeAssetPathRelativeToLayer(layer, clipsDir);
    ...
    std::string globPattern = TfStringCatPaths(clipsDirAssetPath, TfStringReplace(clipsBaseName, "#", "*"));
    ...
    const auto fixupRefFunc = [&clipsDirAssetPath, &clipsDir](const std::string& clipAsset) {
    ...
    std::transform(clipAssetRefs.begin(), clipAssetRefs.end(), clipAssetRefs.begin(), fixupRefFunc);

    return clipAssetRefs;
}

void UsdUtils_LocalizationContext::_ProcessPayloads(const SdfLayerRefPtr& layer, const SdfPrimSpecHandle& primSpec) {
    ...
    const std::vector<std::string> processedDeps = _delegate->ProcessPayloads(layer, primSpec);
    _EnqueueDependencies(layer, processedDeps);
}

void UsdUtils_LocalizationContext::_ProcessReferences(const SdfLayerRefPtr& layer, const SdfPrimSpecHandle& primSpec) {
    ...
    const std::vector<std::string> processedDeps = _delegate->ProcessReferences(layer, primSpec);
    _EnqueueDependencies(layer, processedDeps);
}

void UsdUtils_LocalizationContext::_ProcessProperties(const SdfLayerRefPtr& layer, const SdfPrimSpecHandle& primSpec) {
    ...
    if (_refTypesToInclude == ReferenceType::CompositionOnly) return;
    ...
    //
    const VtValue propertyNames = primSpec->GetField(SdfChildrenKeys->PropertyChildren);
    ...
    for (const auto& name : propertyNames.UncheckedGet<std::vector<TfToken>>()) {
        ...
        if (typeName == SdfValueTypeNames->Asset || typeName == SdfValueTypeNames->AssetArray) {
            ...
            if (_ValueTypeIsRelevant(defValue)) {
                ...
                _delegate->EndProcessValue(layer, path, SdfFieldKeys->Default, defValue);
            }
            ...
            for (double t : layer->ListTimeSamplesForPath(path)) {
                ...
                if (layer->QueryTimeSample(path, t, &timeSampleVal)) {
                    ...
                    _delegate->EndProcessTimeSampleValue(layer, path, t, timeSampleVal);
                }
            }
        }
    }
...
void UsdUtils_LocalizationContext::_ProcessAssetValue(const SdfLayerRefPtr& layer,
                                                      const VtValue& val,
...
void UsdUtils_LocalizationContext::_ProcessAssetValue(const SdfLayerRefPtr& layer,
    ...
    if (val.IsHolding<SdfAssetPath>()) {
        ...
        const std::vector<std::string> dependencies = _GetDependencies(layer, rawAssetPath);

        const std::vector<std::string> processedDeps =
                _delegate->ProcessValuePath(layer, keyPath, rawAssetPath, dependencies);
        ...
        for (const SdfAssetPath& assetPath : originalArray) {
            ...
            const std::vector<std::string> processedDeps =
                    _delegate->ProcessValuePathArrayElement(layer, keyPath, rawAssetPath, dependencies);
        ...
        for (const auto& p : originalDict) {
            const std::string dictKey = keyPath.empty() ? p.first : keyPath + ':' + p.first;
...
std::vector<std::string> UsdUtils_LocalizationContext::_GetDependencies(const SdfLayerRefPtr& layer,
                                                                        const std::string& assetPath) {
    return _GetUdimTiles(layer, assetPath);
}

std::vector<std::string> UsdUtils_LocalizationContext::_GetUdimTiles(const SdfLayerRefPtr& layer,
                                                                     const std::string& assetPath) {
    ...
    const std::string resolvedUdimPath = UsdShadeUdimUtils::ResolveUdimPath(assetPath, layer);
    ...
    for (const auto& udim : resolvedPaths) {
        additionalPaths.emplace_back(UsdShadeUdimUtils::ReplaceUdimPattern(assetPath, udim.second));
    }

    return additionalPaths;
}

bool UsdUtils_LocalizationContext::_ShouldFilterAssetPath(const std::string& key, bool processingMetadata) {
...
bool UsdUtils_LocalizationContext::_ValueTypeIsRelevant(const VtValue& val) {
    return val.IsHolding<SdfAssetPath>() || val.IsHolding<VtArray<SdfAssetPath>>() || val.IsHolding<VtDictionary>();
}

struct UsdUtils_ExtractExternalReferencesClient {
    UsdUtilsDependencyInfo Process(const SdfLayerRefPtr&,
    ...
    void PlaceAsset(const std::string& dependency, UsdUtils_DependencyType dependencyType) {
    ...
    void SortAndRemoveDuplicates() {
        ...
        sublayers.erase(std::unique(sublayers.begin(), sublayers.end()), sublayers.end());

        std::sort(references.begin(), references.end());
        references.erase(std::unique(references.begin(), references.end()), references.end());

        std::sort(payloads.begin(), payloads.end());
        payloads.erase(std::unique(payloads.begin(), payloads.end()), payloads.end());
    }
...
void UsdUtils_ExtractExternalReferences(const std::string& filePath,
    ...
    UsdUtils_ReadOnlyLocalizationDelegate delegate(std::bind(&UsdUtils_ExtractExternalReferencesClient::Process,
