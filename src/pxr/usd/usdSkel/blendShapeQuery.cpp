    ...
    for (size_t i = 0; i < targets.size(); ++i) {
        ...
        for (const auto& inbetween : targetShape.GetInbetweens()) {
            ...
            if (GfIsClose(weight, 0.0, EPS) || GfIsClose(weight, 1.0, EPS)) {
                TF_WARN("%s -- skipping inbetween with invalid weight (%f)", inbetween.GetAttr().GetPath().GetText(),
                        weight);
                continue;
            }
...
UsdSkelBlendShape UsdSkelBlendShapeQuery::GetBlendShape(size_t blendShapeIndex) const {
    return blendShapeIndex < _blendShapes.size() ? _blendShapes[blendShapeIndex].shape : UsdSkelBlendShape();
}
            ...
            if (blendShape.shape) {
                ...
                if (blendShape.shape.GetPointIndicesAttr().Get(&val)) {
                    if (val.IsHolding<VtIntArray>()) {
                        ...
                        const VtUIntArray& uindices = val.UncheckedGet<VtUIntArray>();
...
bool UsdSkelBlendShapeQuery::ComputeSubShapeWeights(const TfSpan<const float>& weights,
    ...
    for (size_t i = 0; i < weights.size(); ++i) {
        ...
        const _SubShape* it = std::upper_bound(start, end, w, _SubShapeCompareByWeight());
...
bool UsdSkelBlendShapeQuery::ComputeFlattenedSubShapeWeights(const TfSpan<const float>& weights,
    ...
    if (ComputeSubShapeWeights(weights, &sparseSubShapeWeights, &sparseBlendShapeIndices, &sparseSubShapeIndices)) {
        if (!TF_VERIFY((sparseSubShapeIndices.size() == sparseSubShapeWeights.size()),
                       "numIndices (%zu) != numWeights (%zu)", sparseSubShapeIndices.size(),
        ...
        for (size_t i = 0; i < sparseSubShapeWeights.size(); ++i) {
            if (ARCH_UNLIKELY(!TF_VERIFY((sparseSubShapeIndices[i] < numSubShapes),
                                         "index (%d) out of range %zu - skipping.", (int)sparseSubShapeIndices[i],
                                         numSubShapes))) {
                continue;
            }
...
bool UsdSkelBlendShapeQuery::ComputeDeformedPoints(const TfSpan<const float> subShapeWeights,
    ...
    if (blendShapeIndices.size() != subShapeWeights.size()) {
        TF_WARN("blendShapeIndices size [%td] != subShapeWeights size [%td]", blendShapeIndices.size(),
                subShapeWeights.size());
        return false;
    }
    if (subShapeIndices.size() != subShapeWeights.size()) {
        TF_WARN("subShapeIndices size [%td] != subShapeWeights size [%td]", subShapeIndices.size(),
                subShapeWeights.size());
        return false;
    }
        ...
        if (blendShapeIndex < blendShapePointIndices.size()) {
            ...
            if (subShapeIndex < subShapePointOffsets.size()) {
                if (!subShapePointOffsets[subShapeIndex].empty() &&
                    !UsdSkelApplyBlendShape(subShapeWeights[i], subShapePointOffsets[subShapeIndex],
                                            blendShapePointIndices[blendShapeIndex], points)) {
                    return false;
                }
            } else {
...
bool UsdSkelBlendShapeQuery::ComputeDeformedNormals(const TfSpan<const float> subShapeWeights,
                                                    const TfSpan<const unsigned> blendShapeIndices,
                                                    const TfSpan<const unsigned> subShapeIndices,
                                                    const std::vector<VtIntArray>& blendShapePointIndices,
    ...
    if (ComputeDeformedPoints(subShapeWeights, blendShapeIndices, subShapeIndices, blendShapePointIndices,
...
unsigned _ComputeRangesFromCounts(const TfSpan<const unsigned>& counts, TfSpan<GfVec2i> ranges) {
...
size_t _ComputeApproximateNumPointsForShapes(const std::vector<VtIntArray>& indicesPerBlendShape,
                                             const std::vector<VtVec3fArray>& offsetsPerSubShape) {
    // Get the max index across all of the shapes.
    int maxIndex = WorkParallelReduceN(
...
bool UsdSkelBlendShapeQuery::ComputePackedShapeTable(VtVec4fArray* offsets, VtVec2iArray* ranges) const {
    ...
    const std::vector<VtIntArray> indicesPerBlendShape = ComputeBlendShapePointIndices();

    const std::vector<VtVec3fArray> offsetsPerSubShape = ComputeSubShapePointOffsets();
    ...
    const unsigned numOffsets = _ComputeRangesFromCounts(numOffsetsPerPoint, *ranges);
    ...
    for (size_t i = 0; i < _subShapes.size(); ++i) {
        ...
        const auto& indices = indicesPerBlendShape[subShape.GetBlendShapeIndex()];
        ...
        if (indices.empty()) {
            ...
            for (size_t pi = 0; pi < offsets.size(); ++pi) {
                ...
                dst[offsetIndex] = GfVec4f(offset[0], offset[1], offset[2], subShapeIndexAsFloat);
            ...
            for (size_t j = 0; j < indices.size(); ++j) {
                ...
                const unsigned offsetIndex = nextOffsetIndexPerPoint[pointIndex];
                dst[offsetIndex] = GfVec4f(offset[0], offset[1], offset[2], subShapeIndexAsFloat);
