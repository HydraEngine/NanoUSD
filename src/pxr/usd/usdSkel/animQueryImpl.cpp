...
public:
    ...
    bool ComputeJointLocalTransforms(VtMatrix4dArray* xforms, UsdTimeCode time) const override {
        return _ComputeJointLocalTransforms(xforms, time);
    }

    bool ComputeJointLocalTransforms(VtMatrix4fArray* xforms, UsdTimeCode time) const override {
        return _ComputeJointLocalTransforms(xforms, time);
    }
...
private:
    ...
    bool _ComputeJointLocalTransforms(VtArray<Matrix4>* xforms, UsdTimeCode time) const;
...
template <typename Matrix4>
bool UsdSkel_SkelAnimationQueryImpl::_ComputeJointLocalTransforms(VtArray<Matrix4>* xforms, UsdTimeCode time) const {
    TRACE_FUNCTION();
    ...
    if (ComputeJointLocalTransformComponents(&translations, &rotations, &scales, time)) {
        ...
        if (UsdSkelMakeTransforms(translations, rotations, scales, *xforms)) {
            ...
            TF_WARN("%s -- size of transform component arrays [%zu] "
                    "!= joint order size [%zu].",
                    _anim.GetPrim().GetPath().GetText(), xforms->size(), _jointOrder.size());
        } else {
...
bool UsdSkel_SkelAnimationQueryImpl::ComputeJointLocalTransformComponents(VtVec3fArray* translations,
    ...
    return _translations.Get(translations, time) && _rotations.Get(rotations, time) && _scales.Get(scales, time);
}
...
bool UsdSkel_SkelAnimationQueryImpl::BlendShapeWeightsMightBeTimeVarying() const {
