...
UsdSkelAnimMapper::UsdSkelAnimMapper(const VtTokenArray& sourceOrder, const VtTokenArray& targetOrder)
    : UsdSkelAnimMapper(sourceOrder.cdata(), sourceOrder.size(), targetOrder.cdata(), targetOrder.size()) {}
...
UsdSkelAnimMapper::UsdSkelAnimMapper(const TfToken* sourceOrder,
    ...
    if (std::all_of(targetMapped.begin(), targetMapped.end(), [](bool val) {
            return val;
        })) {
        _flags |= _SourceOverridesAllTargetValues;
    }
...
bool UsdSkelAnimMapper::IsSparse() const {
    ...
    if (target->IsEmpty()) {
        *target = VtArray<T>();
        ...
        TF_CODING_ERROR(
                ...
                "'source' [%s].",
                target->GetTypeName().c_str(), source.GetTypeName().c_str());
        return false;
    }
    ...
    if (!defaultValue.IsEmpty()) {
        if (defaultValue.IsHolding<T>()) {
            ...
            TF_CODING_ERROR(
                    ...
                    "'%s'.",
                    defaultValue.GetTypeName().c_str(), TfType::Find<T>().GetTypeName().c_str());
            return false;
        }
    }
...
template USDSKEL_API bool UsdSkelAnimMapper::RemapTransforms(const VtMatrix4dArray&, VtMatrix4dArray*, int) const;

template USDSKEL_API bool UsdSkelAnimMapper::RemapTransforms(const VtMatrix4fArray&, VtMatrix4fArray*, int) const;

bool UsdSkelAnimMapper::operator==(const UsdSkelAnimMapper& o) const {
    return _targetSize == o._targetSize && _offset == o._offset && _flags == o._flags && _indexMap == o._indexMap;
}
