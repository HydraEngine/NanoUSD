...
TF_DEFINE_PRIVATE_TOKENS(_tokens, ((inbetweensPrefix, "inbetweens:"))((normalOffsetsSuffix, ":normalOffsets")));
...
UsdSkelInbetweenShape UsdSkelInbetweenShape::_Create(const UsdPrim& prim, const TfToken& name) {
    if (TF_VERIFY(prim)) {
        ...
        if (!attrName.IsEmpty()) {
            return UsdSkelInbetweenShape(prim.CreateAttribute(attrName, SdfValueTypeNames->Point3fArray,
...
bool UsdSkelInbetweenShape::_IsValidInbetweenName(const std::string& name, bool quiet) {
    ...
    return TfStringStartsWith(name, _tokens->inbetweensPrefix) && !TfStringEndsWith(name, _tokens->normalOffsetsSuffix);
}
...
UsdAttribute UsdSkelInbetweenShape::_GetNormalOffsetsAttr(bool create) const {
    ...
    if (!create) {
        ...
        return _attr.GetPrim().CreateAttribute(normalOffsetsName, SdfValueTypeNames->Vector3fArray,
