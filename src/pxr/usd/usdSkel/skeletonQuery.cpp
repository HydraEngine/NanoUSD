...
bool UsdSkelSkeletonQuery::ComputeJointLocalTransforms(VtArray<Matrix4>* xforms, UsdTimeCode time, bool atRest) const {
    TRACE_FUNCTION();
...
template <typename Matrix4>
bool UsdSkelSkeletonQuery::_ComputeJointLocalTransforms(VtArray<Matrix4>* xforms, UsdTimeCode time, bool atRest) const {
    if (atRest) {
    ...
    if (_animToSkelMapper.IsSparse()) {
        ...
        if (!_definition->GetJointLocalRestTransforms(xforms)) {
            TF_WARN("%s -- Failed computing local space transforms: "
                    ...
                    GetSkeleton().GetPrim().GetPath().GetText(), GetAnimQuery().GetPrim().GetPath().GetText());
            return false;
        }
    }
...
template <typename Matrix4>
bool UsdSkelSkeletonQuery::ComputeJointSkelTransforms(VtArray<Matrix4>* xforms, UsdTimeCode time, bool atRest) const {
    TRACE_FUNCTION();
...
template <typename Matrix4>
bool UsdSkelSkeletonQuery::_ComputeJointSkelTransforms(VtArray<Matrix4>* xforms, UsdTimeCode time, bool atRest) const {
    if (atRest) {
...
template <typename Matrix4>
bool UsdSkelSkeletonQuery::ComputeJointRestRelativeTransforms(VtArray<Matrix4>* xforms, UsdTimeCode time) const {
    TRACE_FUNCTION();
...
template USDSKEL_API bool UsdSkelSkeletonQuery::ComputeSkinningTransforms(VtArray<GfMatrix4d>*, UsdTimeCode) const;

template USDSKEL_API bool UsdSkelSkeletonQuery::ComputeSkinningTransforms(VtArray<GfMatrix4f>*, UsdTimeCode) const;
    ...
    if (ComputeJointSkelTransforms(xforms, time)) {
        ...
        if (xforms->size() == inverseBindXforms.size()) {
            ...
            TF_WARN("%s -- Size of computed joints transforms [%zu] does not "
                    ...
                    "attr [%zu].",
                    GetSkeleton().GetPrim().GetPath().GetText(), xforms->size(), inverseBindXforms.size());
        }
    }
    return false;
}
...
template USDSKEL_API bool UsdSkelSkeletonQuery::GetJointWorldBindTransforms(VtArray<GfMatrix4d>* xforms) const;

template USDSKEL_API bool UsdSkelSkeletonQuery::GetJointWorldBindTransforms(VtArray<GfMatrix4f>* xforms) const;
...
std::string UsdSkelSkeletonQuery::GetDescription() const {
    if (IsValid()) {
        return TfStringPrintf("UsdSkelSkeletonQuery (skel = <%s>, anim = <%s>)", GetPrim().GetPath().GetText(),
