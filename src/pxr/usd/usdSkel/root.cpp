...
static bool _ComputeExtent(const UsdGeomBoundable& boundable,
    ...
    auto processSkeleton = [&](UsdSkelSkeleton const& skeleton, UsdSkelBinding const& binding = UsdSkelBinding()) {
        ...
        if (skelQuery.ComputeJointSkelTransforms(&skelRestXforms, time, /*atRest*/ true)) {
            for (const auto& skinningQuery : binding.GetSkinningTargets()) {
                ...
                float skelPadding = skinningQuery.ComputeExtentsPadding(skelRestXforms, UsdGeomBoundable(skinnedPrim));
        ...
        GfMatrix4d skelRootXform =
                xfCache.ComputeRelativeTransform(skeleton.GetPrim(), skelRoot.GetPrim(), &resetXformStack);
    ...
    if (!skelCache.ComputeSkelBindings(skelRoot, &bindings, UsdTraverseInstanceProxies()) || bindings.size() == 0) {
