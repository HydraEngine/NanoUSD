...
bool UsdSkelIsSkinnablePrim(const UsdPrim& prim) {
    ...
    return prim.IsA<UsdGeomBoundable>() && !prim.IsA<UsdSkelSkeleton>() && !prim.IsA<UsdSkelRoot>();
}
...
void _InvertTransforms(TfSpan<const Matrix4> xforms, TfSpan<Matrix4> inverseXforms) {
...
template <typename Matrix4>
bool UsdSkel_ConcatJointTransforms(const UsdSkelTopology& topology,
    ...
    if (ARCH_UNLIKELY(jointLocalXforms.size() != topology.size())) {
        TF_WARN("Size of jointLocalXforms [%zu] != number of joints [%zu]", jointLocalXforms.size(), topology.size());
        return false;
    }
...
bool UsdSkelConcatJointTransforms(const UsdSkelTopology& topology,
    ...
    return UsdSkel_ConcatJointTransforms(topology, jointLocalXforms, xforms, rootXform);
}
...
bool UsdSkelConcatJointTransforms(const UsdSkelTopology& topology,
                                  ...
                                  GfMatrix4d* xforms,
                                  const GfMatrix4d* rootXform) {
    return UsdSkelConcatJointTransforms(topology, TfSpan<const GfMatrix4d>(jointLocalXforms, topology.size()),
                                        TfSpan<GfMatrix4d>(xforms, topology.size()), rootXform);
}

namespace {

template <typename Matrix4>
bool UsdSkel_ComputeJointLocalTransforms(const UsdSkelTopology& topology,
    ...
    if (ARCH_UNLIKELY(inverseXforms.size() != topology.size())) {
        TF_WARN("Size of inverseXforms [%zu] != number of joints [%zu]", inverseXforms.size(), topology.size());
        return false;
    }
    if (ARCH_UNLIKELY(jointLocalXforms.size() != topology.size())) {
        TF_WARN("Size of jointLocalXforms [%zu] != number of joints [%zu]", jointLocalXforms.size(), topology.size());
...
bool UsdSkel_ComputeJointLocalTransforms(const UsdSkelTopology& topology,
    ...
    return UsdSkel_ComputeJointLocalTransforms<Matrix4>(topology, xforms, inverseXforms, jointLocalXforms,
                                                        rootInverseXform);
}

}  // namespace

bool UsdSkelComputeJointLocalTransforms(const UsdSkelTopology& topology,
    ...
    return UsdSkel_ComputeJointLocalTransforms(topology, xforms, inverseXforms, jointLocalXforms, rootInverseXform);
}
...
bool UsdSkelComputeJointLocalTransforms(const UsdSkelTopology& topology,
                                        ...
                                        TfSpan<GfMatrix4d> jointLocalXforms,
                                        const GfMatrix4d* rootInverseXform) {
    return UsdSkel_ComputeJointLocalTransforms(topology, xforms, jointLocalXforms, rootInverseXform);
}
...
bool UsdSkelComputeJointLocalTransforms(const UsdSkelTopology& topology,
    ...
    return UsdSkelComputeJointLocalTransforms(topology, xforms, inverseXforms, *jointLocalXforms, rootInverseXform);
}
...
bool UsdSkelComputeJointLocalTransforms(const UsdSkelTopology& topology,
    ...
    jointLocalXforms->resize(topology.size());
    return UsdSkelComputeJointLocalTransforms(topology, xforms, *jointLocalXforms, rootInverseXform);
}
...
bool UsdSkelComputeJointLocalTransforms(const UsdSkelTopology& topology,
                                        ...
                                        const GfMatrix4d* rootInverseXform) {
    return UsdSkelComputeJointLocalTransforms(topology, TfSpan<const GfMatrix4d>(xforms, topology.size()),
                                              TfSpan<const GfMatrix4d>(inverseXforms, topology.size()),
                                              TfSpan<GfMatrix4d>(jointLocalXforms, topology.size()), rootInverseXform);
}
...
template <typename Matrix4>
bool UsdSkel_DecomposeTransforms(TfSpan<const Matrix4> xforms,
...
bool UsdSkelDecomposeTransforms(TfSpan<const GfMatrix4d> xforms,
                                TfSpan<GfVec3f> translations,
                                TfSpan<GfQuatf> rotations,
                                TfSpan<GfVec3h> scales) {
...
bool UsdSkelDecomposeTransforms(TfSpan<const GfMatrix4f> xforms,
                                TfSpan<GfVec3f> translations,
                                TfSpan<GfQuatf> rotations,
                                TfSpan<GfVec3h> scales) {
    return UsdSkel_DecomposeTransforms(xforms, translations, rotations, scales);
...
// deprecated
bool UsdSkelDecomposeTransforms(
        const GfMatrix4d* xforms, GfVec3f* translations, GfQuatf* rotations, GfVec3h* scales, size_t count) {
    return UsdSkelDecomposeTransforms(TfSpan<const GfMatrix4d>(xforms, count), TfSpan<GfVec3f>(translations, count),
                                      TfSpan<GfQuatf>(rotations, count), TfSpan<GfVec3h>(scales, count));
}

// deprecated
bool UsdSkelDecomposeTransforms(const VtMatrix4dArray& xforms,
    ...
    return UsdSkelDecomposeTransforms(xforms, *translations, *rotations, *scales);
}

template <typename Matrix4>
void UsdSkelMakeTransform(const GfVec3f& translate, const GfMatrix3f& rotate, const GfVec3h& scale, Matrix4* xform) {
    if (xform) {
        // Order is scale*rotate*translate
        *xform = Matrix4(rotate[0][0] * scale[0], rotate[0][1] * scale[0], rotate[0][2] * scale[0], 0,
...
template USDSKEL_API void UsdSkelMakeTransform(const GfVec3f&, const GfMatrix3f&, const GfVec3h&, GfMatrix4d*);

template USDSKEL_API void UsdSkelMakeTransform(const GfVec3f&, const GfMatrix3f&, const GfVec3h&, GfMatrix4f*);

template <typename Matrix4>
void UsdSkelMakeTransform(const GfVec3f& translate, const GfQuatf& rotate, const GfVec3h& scale, Matrix4* xform) {
    UsdSkelMakeTransform(translate, GfMatrix3f(rotate), scale, xform);
}

template USDSKEL_API void UsdSkelMakeTransform(const GfVec3f&, const GfQuatf&, const GfVec3h&, GfMatrix4d*);

template USDSKEL_API void UsdSkelMakeTransform(const GfVec3f&, const GfQuatf&, const GfVec3h&, GfMatrix4f*);

namespace {

template <typename Matrix4>
bool UsdSkel_MakeTransforms(TfSpan<const GfVec3f> translations,
...
// deprecated
bool UsdSkelMakeTransforms(const GfVec3f* translations,
                           const GfQuatf* rotations,
                           const GfVec3h* scales,
                           GfMatrix4d* xforms,
                           size_t count) {
    return UsdSkelMakeTransforms(TfSpan<const GfVec3f>(translations, count), TfSpan<const GfQuatf>(rotations, count),
                                 TfSpan<const GfVec3h>(scales, count), TfSpan<GfMatrix4d>(xforms, count));
}

// deprecated
bool UsdSkelMakeTransforms(const VtVec3fArray& translations,
...
template <typename Matrix4>
bool UsdSkelComputeJointsExtent(TfSpan<const Matrix4> xforms, GfRange3f* extent, float pad, const Matrix4* rootXform) {
    TRACE_FUNCTION();
    ...
    for (size_t i = 0; i < xforms.size(); ++i) {
        ...
        extent->UnionWith(rootXform ? GfVec3f(rootXform->TransformAffine(pivot)) : pivot);
    }
...
template USDSKEL_API bool UsdSkelComputeJointsExtent(TfSpan<const GfMatrix4d>, GfRange3f*, float, const GfMatrix4d*);

template USDSKEL_API bool UsdSkelComputeJointsExtent(TfSpan<const GfMatrix4f>, GfRange3f*, float, const GfMatrix4f*);
...
// deprecated
bool UsdSkelComputeJointsExtent(
        const GfMatrix4d* xforms, size_t count, VtVec3fArray* extent, float pad, const GfMatrix4d* rootXform) {
    GfRange3f range;
    if (UsdSkelComputeJointsExtent<GfMatrix4d>(TfSpan<const GfMatrix4d>(xforms, count), &range, pad, rootXform)) {
        extent->resize(2);
        (*extent)[0] = range.GetMin();
        (*extent)[1] = range.GetMax();
        return true;
...
bool UsdSkelNormalizeWeights(TfSpan<float> weights, int numInfluencesPerComponent, float eps) {
    TRACE_FUNCTION();
...
bool UsdSkelExpandConstantInfluencesToVarying(VtIntArray* indices, size_t size) {
    return _ExpandConstantArray(indices, size);
}

bool UsdSkelExpandConstantInfluencesToVarying(VtFloatArray* weights, size_t size) {
...
template <typename T>
bool _ResizeInfluences(VtArray<T>* array,
                       int srcNumInfluencesPerComponent,
                       int newNumInfluencesPerComponent,
                       T defaultVal) {
    if (srcNumInfluencesPerComponent == newNumInfluencesPerComponent) return true;
    ...
    if (!_ValidateArrayShape(array->size(), srcNumInfluencesPerComponent)) return false;

    size_t numComponents = array->size() / srcNumInfluencesPerComponent;
    if (numComponents == 0) return true;
    ...
    if (newNumInfluencesPerComponent < srcNumInfluencesPerComponent) {
        ...
        for (size_t i = 0; i < numComponents; ++i) {
            ...
            std::fill(data + idx * newNumInfluencesPerComponent + srcNumInfluencesPerComponent,
...
bool UsdSkelResizeInfluences(VtIntArray* indices, int srcNumInfluencesPerComponent, int newNumInfluencesPerComponent) {
    TRACE_FUNCTION();
    return _ResizeInfluences(indices, srcNumInfluencesPerComponent, newNumInfluencesPerComponent, 0);
}

bool UsdSkelResizeInfluences(VtFloatArray* weights,
                             int srcNumInfluencesPerComponent,
                             int newNumInfluencesPerComponent) {
    TRACE_FUNCTION();
...
bool UsdSkelInterleaveInfluences(const TfSpan<const int>& indices,
    ...
    if (interleavedInfluences.size() != indices.size()) {
        TF_WARN("Size of interleavedInfluences [%zu] != size of indices [%zu]", interleavedInfluences.size(),
                indices.size());
        return false;
    }
...
struct _InterleavedInfluencesFn {
    ...
    int GetIndex(size_t index) const { return static_cast<int>(influences[index][0]); }
...
struct _FaceVaryingPointIndexFn {
    ...
    size_t GetPointIndex(size_t index) const {
        ...
        if (pointIndex < 0 || pointIndex >= numPoints) {
            TF_WARN("faceVertexIndices is out of range [%d] at index [%zu]", pointIndex, index);
            return 0;
        }
...
void _ConvertToDualQuaternions(TfSpan<const Matrix4> jointXforms,
    ...
    for (size_t ji = 0; ji < jointXforms.size(); ++ji) {
        ...
        if (!matrix.Factor(&scaleOrientMat, &scale, &factoredRotMat, &translation, &perspMat)) {
        ...
        const GfQuaternion rotationQ = factoredRotMat.ExtractRotationMatrix().ExtractRotationQuaternion();

        // Construct dual quaternion from rotation & translation
        jointDualQuats[ji] = GfDualQuatd(GfQuatd(rotationQ.GetReal(), rotationQ.GetImaginary()), translation);

        // Calculate jointScales by removing jointDualQuats from jointXforms
        const GfMatrix4d tmpNonScaleXform = factoredRotMat * GfMatrix4d(1.0).SetTranslate(translation);
        // Extract the upper-left 3x3 matrix;
        jointScales[ji] = Matrix3((matrix * tmpNonScaleXform.GetInverse()).ExtractRotationMatrix());

        // If jointScales[ji] is not an identity matrix, need to set the flag
        if (hasJointScale && !*hasJointScale && !GfIsClose(jointScales[ji], Matrix3(1), 1e-6)) {
            *hasJointScale = true;
        }
    }
}
...
int _GetPivotJointIndex(const size_t pointIdx,
...
bool _SkinPointsLBS(const Matrix4& geomBindTransform,
            ...
            for (int wi = 0; wi < numInfluencesPerPoint; ++wi) {
                ...
                if (jointIdx >= 0 && static_cast<size_t>(jointIdx) < jointXforms.size()) {
                    ...
                    if (w != 0.0f) {
                        ...
                        p += GfVec3f(jointXforms[jointIdx].TransformAffine(initialP)) * w;
                    }
...
template <typename Matrix4, typename InfluenceFn>
bool _SkinPointsDQS(const Matrix4& geomBindTransform,
                    TfSpan<const Matrix4> jointXforms,
                    const InfluenceFn& influenceFn,
                    const int numInfluencesPerPoint,
                    TfSpan<GfVec3f> points,
    ...
    _ConvertToDualQuaternions(jointXforms, TfSpan<GfDualQuatd>(jointDualQuats), TfSpan<GfMatrix3f>(jointScales),
                              &hasJointScale);
    ...
    _ParallelForN(points.size(), /* inSerial = */ inSerial, [&](size_t start, size_t end) {
        for (size_t pi = start; pi < end; ++pi) {
            ...
            const int pivotIdx = _GetPivotJointIndex(pi, jointDualQuats.size(), influenceFn, numInfluencesPerPoint);
            if (pivotIdx >= 0) pivotQuat = jointDualQuats[pivotIdx].GetReal();
            ...
            for (int wi = 0; wi < numInfluencesPerPoint; ++wi) {
                ...
                if (jointIdx >= 0 && static_cast<size_t>(jointIdx) < jointDualQuats.size()) {
                    ...
                    if (w != 0.0f) {
                        ...
                        if (GfDot(jointDQ.GetReal(), pivotQuat) < 0.0) w = -w;
...
template <typename Matrix4>
bool _InterleavedSkinPoints(const TfToken& skinningMethod,
    ...
    if (skinningMethod == UsdSkelTokens->classicLinear) {
        return _SkinPointsLBS(geomBindTransform, jointXforms, influenceFn, numInfluencesPerPoint, points, inSerial);
    } else if (skinningMethod == UsdSkelTokens->dualQuaternion) {
        return _SkinPointsDQS(geomBindTransform, jointXforms, influenceFn, numInfluencesPerPoint, points, inSerial);
    } else {
...
template <typename Matrix4>
bool _NonInterleavedSkinPoints(const TfToken& skinningMethod,
                               const Matrix4& geomBindTransform,
                               TfSpan<const Matrix4> jointXforms,
    ...
    if (jointIndices.size() != jointWeights.size()) {
        TF_WARN("Size of jointIndices [%zu] != size of jointWeights [%zu]", jointIndices.size(), jointWeights.size());
        return false;
    }
...
bool UsdSkelSkinPoints(const TfToken& skinningMethod,
                       ...
                       TfSpan<GfVec3f> points,
                       const bool inSerial) {
    return _NonInterleavedSkinPoints(skinningMethod, geomBindTransform, jointXforms, jointIndices, jointWeights,
...
bool UsdSkelSkinPoints(const TfToken& skinningMethod,
                       ...
                       TfSpan<GfVec3f> points,
                       const bool inSerial) {
    return _InterleavedSkinPoints(skinningMethod, geomBindTransform, jointXforms, influences, numInfluencesPerPoint,
                                  points, inSerial);
}
...
bool UsdSkelSkinPoints(const TfToken& skinningMethod,
                       ...
                       const bool inSerial) {
    return UsdSkelSkinPoints(skinningMethod, geomBindTransform, TfSpan<const GfMatrix4d>(jointXforms, numJoints),
                             TfSpan<const int>(jointIndices, numInfluences),
                             TfSpan<const float>(jointWeights, numInfluences), numInfluencesPerPoint,
                             TfSpan<GfVec3f>(points, numPoints), inSerial);
}
...
bool UsdSkelSkinPoints(const TfToken& skinningMethod,
    ...
    return UsdSkelSkinPoints(skinningMethod, geomBindTransform, jointXforms, jointIndices, jointWeights,
...
bool UsdSkelSkinPointsLBS(const GfMatrix4d& geomBindTransform,
                          ...
                          TfSpan<GfVec3f> points,
                          const bool inSerial) {
    return UsdSkelSkinPoints(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, jointIndices, jointWeights,
                             numInfluencesPerPoint, points, inSerial);
}

bool UsdSkelSkinPointsLBS(const GfMatrix4f& geomBindTransform,
                          TfSpan<const GfMatrix4f> jointXforms,
                          TfSpan<const int> jointIndices,
                          TfSpan<const float> jointWeights,
                          const int numInfluencesPerPoint,
...
bool UsdSkelSkinPointsLBS(const GfMatrix4d& geomBindTransform,
                          ...
                          TfSpan<GfVec3f> points,
                          const bool inSerial) {
    return UsdSkelSkinPoints(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, influences,
                             numInfluencesPerPoint, points, inSerial);
}
...
bool UsdSkelSkinPointsLBS(const GfMatrix4d& geomBindTransform,
                          ...
                          const size_t numPoints,
                          const bool inSerial) {
    return UsdSkelSkinPoints(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, numJoints, jointIndices,
                             jointWeights, numInfluences, numInfluencesPerPoint, points, numPoints, inSerial);
}
...
void _ConvertToQuaternions(TfSpan<const Matrix3> jointXforms,
    ...
    for (size_t ji = 0; ji < jointXforms.size(); ++ji) {
        ...
        jointQuats[ji] = GfQuatd(rotationQ.GetReal(), rotationQ.GetImaginary());
...
template <typename Matrix3, typename InfluenceFn, typename PointIndexFn>
bool _SkinNormalsLBS(const Matrix3& geomBindTransform,
    ...
    _ParallelForN(normals.size(), inSerial, [&](size_t start, size_t end) {
...
template <typename Matrix3, typename InfluenceFn, typename PointIndexFn>
bool _SkinNormalsDQS(const Matrix3& geomBindTransform,
                     TfSpan<const Matrix3> jointXforms,
                     const InfluenceFn& influenceFn,
                     const PointIndexFn& pointIndexFn,
                     const int numInfluencesPerPoint,
    ...
    bool hasJointScale(false);
    _ConvertToQuaternions(jointXforms, TfSpan<GfQuatd>(jointQuats), TfSpan<Matrix3>(jointScales), &hasJointScale);
    ...
    _ParallelForN(normals.size(), inSerial, [&](size_t start, size_t end) {
        ...
        for (size_t ni = start; ni < end; ++ni) {
            ...
            GfQuatd pivotQuat(0.0);
            const int pivotIdx = _GetPivotJointIndex(pi, jointQuats.size(), influenceFn, numInfluencesPerPoint);
            if (pivotIdx >= 0) pivotQuat = jointQuats[pivotIdx];
            ...
            for (int wi = 0; wi < numInfluencesPerPoint; ++wi) {
                ...
                if (jointIdx >= 0 && static_cast<size_t>(jointIdx) < jointQuats.size()) {
                    float w = influenceFn.GetWeight(influenceIdx);
                    if (w != 0.0f) {
                        // Apply scale using LBS, if any of jointScales is not identity
                        if (hasJointScale) {
                            scaledN += GfVec3f(initialN * jointScales[jointIdx]) * w;
                        }
                        ...
                        // on the same hemisphere as the pivotQuat.
                        if (GfDot(jointQuat, pivotQuat) < 0.0) w = -w;
...
template <typename Matrix3>
bool _InterleavedSkinNormals(const TfToken& skinningMethod,
    ...
    const _InterleavedInfluencesFn influenceFn{influences};
    if (skinningMethod == UsdSkelTokens->classicLinear) {
        return _SkinNormalsLBS(geomBindTransform, jointXforms, influenceFn, _IdentityPointIndexFn(),
                               numInfluencesPerPoint, normals, inSerial);
    } else if (skinningMethod == UsdSkelTokens->dualQuaternion) {
        return _SkinNormalsDQS(geomBindTransform, jointXforms, influenceFn, _IdentityPointIndexFn(),
                               numInfluencesPerPoint, normals, inSerial);
    } else {
        TF_WARN("Unknown skinning method: '%s' ", skinningMethod.GetText());
        return false;
...
template <typename Matrix3>
bool _NonInterleavedSkinNormals(const TfToken& skinningMethod,
                                const Matrix3& geomBindTransform,
                                TfSpan<const Matrix3> jointXforms,
                                TfSpan<const int> jointIndices,
                                TfSpan<const float> jointWeights,
...
template <typename Matrix3>
bool _SkinFaceVaryingNormals(const TfToken& skinningMethod,
                             const Matrix3& geomBindTransform,
                             TfSpan<const Matrix3> jointXforms,
                             TfSpan<const int> jointIndices,
                             TfSpan<const float> jointWeights,
    ...
    if (faceVertexIndices.size() != normals.size()) {
        TF_WARN("Size of faceVertexIndices [%zu] != size of normals [%zu]", faceVertexIndices.size(), normals.size());
        return false;
    }
    ...
    if (skinningMethod == UsdSkelTokens->classicLinear) {
        return _SkinNormalsLBS(geomBindTransform, jointXforms, influenceFn, indexFn, numInfluencesPerPoint, normals,
                               inSerial);
    } else if (skinningMethod == UsdSkelTokens->dualQuaternion) {
        return _SkinNormalsDQS(geomBindTransform, jointXforms, influenceFn, indexFn, numInfluencesPerPoint, normals,
                               inSerial);
    } else {
        TF_WARN("Unknown skinning method: '%s' ", skinningMethod.GetText());
        return false;
...
bool UsdSkelSkinNormals(const TfToken& skinningMethod,
                        ...
                        TfSpan<GfVec3f> normals,
                        const bool inSerial) {
    return _NonInterleavedSkinNormals(skinningMethod, geomBindTransform, jointXforms, jointIndices, jointWeights,
                                      numInfluencesPerPoint, normals, inSerial);
}
...
bool UsdSkelSkinNormals(const TfToken& skinningMethod,
                        ...
                        TfSpan<GfVec3f> points,
                        const bool inSerial) {
    return _InterleavedSkinNormals(skinningMethod, geomBindTransform, jointXforms, influences, numInfluencesPerPoint,
                                   points, inSerial);
}
...
bool UsdSkelSkinFaceVaryingNormals(const TfToken& skinningMethod,
                                   const GfMatrix3d& geomBindTransform,
                                   TfSpan<const GfMatrix3d> jointXforms,
                                   TfSpan<const int> jointIndices,
                                   TfSpan<const float> jointWeights,
                                   ...
                                   TfSpan<GfVec3f> normals,
                                   const bool inSerial) {
    return _SkinFaceVaryingNormals(skinningMethod, geomBindTransform, jointXforms, jointIndices, jointWeights,
...
bool UsdSkelSkinNormalsLBS(const GfMatrix3d& geomBindTransform,
                           ...
                           TfSpan<GfVec3f> normals,
                           const bool inSerial) {
    return UsdSkelSkinNormals(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, jointIndices, jointWeights,
                              numInfluencesPerPoint, normals, inSerial);
}
...
bool UsdSkelSkinNormalsLBS(const GfMatrix3d& geomBindTransform,
                           ...
                           TfSpan<GfVec3f> normals,
                           const bool inSerial) {
    return UsdSkelSkinNormals(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, influences,
                              numInfluencesPerPoint, normals, inSerial);
}
...
bool UsdSkelSkinFaceVaryingNormalsLBS(const GfMatrix3d& geomBindTransform,
                                      ...
                                      TfSpan<GfVec3f> normals,
                                      const bool inSerial) {
    return _SkinFaceVaryingNormals(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, jointIndices,
                                   jointWeights, numInfluencesPerPoint, faceVertexIndices, normals, inSerial);
}
...
bool UsdSkel_SkinTransformLBS(const Matrix4& geomBindTransform,
                              TfSpan<const Matrix4> jointXforms,
...
template <typename Matrix4, typename InfluencesFn>
bool UsdSkel_SkinTransformDQS(const Matrix4& geomBindTransform,
                              TfSpan<const Matrix4> jointXforms,
                              const InfluencesFn& influencesFn,
                              Matrix4* xform) {
    TRACE_FUNCTION();
    ...
    GfQuatd pivotQuat(0.0);
    const int pivotIdx = _GetPivotJointIndex(0, jointDualQuats.size(), influencesFn, influencesFn.size());
    if (pivotIdx >= 0) pivotQuat = jointDualQuats[pivotIdx].GetReal();
...
template <typename Matrix4>
bool UsdSkel_NonInterleavedSkinTransform(const TfToken& skinningMethod,
                                         const Matrix4& geomBindTransform,
                                         TfSpan<const Matrix4> jointXforms,
                                         TfSpan<const int> jointIndices,
                                         TfSpan<const float> jointWeights,
    ...
    if (skinningMethod == UsdSkelTokens->classicLinear) {
        return UsdSkel_SkinTransformLBS(geomBindTransform, jointXforms, influencesFn, xform);
    } else if (skinningMethod == UsdSkelTokens->dualQuaternion) {
        return UsdSkel_SkinTransformDQS(geomBindTransform, jointXforms, influencesFn, xform);
    } else {
        TF_WARN("Unknown skinning method: '%s' ", skinningMethod.GetText());
        return false;
    }
...
bool UsdSkelSkinTransform(const TfToken& skinningMethod,
                          const GfMatrix4d& geomBindTransform,
                          TfSpan<const GfMatrix4d> jointXforms,
                          TfSpan<const int> jointIndices,
                          TfSpan<const float> jointWeights,
                          GfMatrix4d* xform) {
    return UsdSkel_NonInterleavedSkinTransform(skinningMethod, geomBindTransform, jointXforms, jointIndices,
                                               jointWeights, xform);
}
...
bool UsdSkelSkinTransform(const TfToken& skinningMethod,
    ...
    return UsdSkel_NonInterleavedSkinTransform(
            skinningMethod, geomBindTransform, TfSpan<const GfMatrix4d>(jointXforms, numJoints),
            TfSpan<const int>(jointIndices, numInfluences), TfSpan<const float>(jointWeights, numInfluences), xform);
}

bool UsdSkelSkinTransformLBS(const GfMatrix4d& geomBindTransform,
                             TfSpan<const GfMatrix4d> jointXforms,
                             TfSpan<const int> jointIndices,
                             TfSpan<const float> jointWeights,
                             GfMatrix4d* xform) {
    return UsdSkelSkinTransform(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, jointIndices,
                                jointWeights, xform);
}

bool UsdSkelSkinTransformLBS(const GfMatrix4f& geomBindTransform,
                             TfSpan<const GfMatrix4f> jointXforms,
                             TfSpan<const int> jointIndices,
                             TfSpan<const float> jointWeights,
                             GfMatrix4f* xform) {
...
bool UsdSkelSkinTransformLBS(const GfMatrix4d& geomBindTransform,
                             ...
                             TfSpan<const GfVec2f> influences,
                             GfMatrix4d* xform) {
    return UsdSkelSkinTransform(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, influences, xform);
}
...
bool UsdSkelSkinTransformLBS(const GfMatrix4d& geomBindTransform,
                             ...
                             const size_t numInfluences,
                             GfMatrix4d* xform) {
    return UsdSkelSkinTransform(UsdSkelTokens->classicLinear, geomBindTransform, jointXforms, numJoints, jointIndices,
                                jointWeights, numInfluences, xform);
}

namespace {

/// Apply indexed offsets to \p points.
bool UsdSkel_ApplyIndexedBlendShape(const float weight,
    ...
    _ParallelForN(offsets.size(), /*inSerial*/ false, [&](size_t start, size_t end) {
        for (size_t i = start; i < end; ++i) {
            ...
            if (index >= 0 && static_cast<size_t>(index) < points.size()) {
                ...
                TF_WARN("Out of range point index %d (num points = %zu).", index, points.size());
                errors = true;
                return;
            }
        }
...
void UsdSkel_ApplyNonIndexedBlendShape(const float weight,
                                       const TfSpan<const GfVec3f> offsets,
                                       TfSpan<GfVec3f> points) {
    TRACE_FUNCTION();
...
bool UsdSkelApplyBlendShape(const float weight,
                            const TfSpan<const GfVec3f> offsets,
                            const TfSpan<const int> indices,
                            TfSpan<GfVec3f> points) {
