...
/* static */
UsdShadeNodeDefAPI UsdShadeNodeDefAPI::Get(const UsdStagePtr& stage, const SdfPath& path) {
    if (!stage) {
...
/* static */
UsdShadeNodeDefAPI UsdShadeNodeDefAPI::Apply(const UsdPrim& prim) {
...
UsdAttribute UsdShadeNodeDefAPI::CreateImplementationSourceAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdShadeTokens->infoImplementationSource, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, defaultValue, writeSparsely);
}

UsdAttribute UsdShadeNodeDefAPI::GetIdAttr() const {
    return GetPrim().GetAttribute(UsdShadeTokens->infoId);
}

UsdAttribute UsdShadeNodeDefAPI::CreateIdAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdShadeTokens->infoId, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, defaultValue, writeSparsely);
}
...
/*static*/
const TfTokenVector& UsdShadeNodeDefAPI::GetSchemaAttributeNames(bool includeInherited) {
...
TfToken UsdShadeNodeDefAPI::GetImplementationSource() const {
    ...
    if (implSource == UsdShadeTokens->id || implSource == UsdShadeTokens->sourceAsset ||
        ...
        TF_WARN("Found invalid info:implementationSource value '%s' on shader "
                ...
                implSource.GetText(), GetPath().GetText());
        return UsdShadeTokens->id;
    }
}

bool UsdShadeNodeDefAPI::SetShaderId(const TfToken& id) const {
    return CreateImplementationSourceAttr(VtValue(UsdShadeTokens->id),
                                          /*writeSparsely*/ true) &&
           GetIdAttr().Set(id);
}

bool UsdShadeNodeDefAPI::GetShaderId(TfToken* id) const {
...
bool UsdShadeNodeDefAPI::SetSourceAsset(const SdfAssetPath& sourceAsset, const TfToken& sourceType) const {
    ...
    return CreateImplementationSourceAttr(VtValue(UsdShadeTokens->sourceAsset)) &&
           UsdSchemaBase::_CreateAttr(sourceAssetAttrName, SdfValueTypeNames->Asset,
                                      /* custom = */ false, SdfVariabilityUniform, VtValue(sourceAsset),
                                      /* writeSparsely */ false);
}

bool UsdShadeNodeDefAPI::GetSourceAsset(SdfAssetPath* sourceAsset, const TfToken& sourceType) const {
    TfToken implSource = GetImplementationSource();
    ...
    if (sourceType != UsdShadeTokens->universalSourceType) {
        UsdAttribute univSourceAssetAttr =
                GetPrim().GetAttribute(_GetSourceAssetAttrName(UsdShadeTokens->universalSourceType));
...
bool UsdShadeNodeDefAPI::SetSourceAssetSubIdentifier(const TfToken& subIdentifier, const TfToken& sourceType) const {
    TfToken subIdentifierAttrName = _GetSourceAssetSubIdentifierAttrName(sourceType);
    return CreateImplementationSourceAttr(VtValue(UsdShadeTokens->sourceAsset)) &&
           UsdSchemaBase::_CreateAttr(subIdentifierAttrName, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, VtValue(subIdentifier),
                                      /* writeSparsely */ false);
}

bool UsdShadeNodeDefAPI::GetSourceAssetSubIdentifier(TfToken* subIdentifier, const TfToken& sourceType) const {
    TfToken implSource = GetImplementationSource();
    if (implSource != UsdShadeTokens->sourceAsset) {
        return false;
    }
    ...
    if (sourceType != UsdShadeTokens->universalSourceType) {
        UsdAttribute univSubIdentifierAttr =
...
bool UsdShadeNodeDefAPI::SetSourceCode(const std::string& sourceCode, const TfToken& sourceType) const {
    ...
    return CreateImplementationSourceAttr(VtValue(UsdShadeTokens->sourceCode)) &&
           UsdSchemaBase::_CreateAttr(sourceCodeAttrName, SdfValueTypeNames->String,
                                      /* custom = */ false, SdfVariabilityUniform, VtValue(sourceCode),
                                      /* writeSparsely */ false);
}

bool UsdShadeNodeDefAPI::GetSourceCode(std::string* sourceCode, const TfToken& sourceType) const {
    TfToken implSource = GetImplementationSource();
    ...
    if (sourceType != UsdShadeTokens->universalSourceType) {
        UsdAttribute univSourceCodeAttr =
                GetPrim().GetAttribute(_GetSourceCodeAttrName(UsdShadeTokens->universalSourceType));
...
static NdrTokenMap _GetSdrMetadata(UsdPrim const& prim) {
    NdrTokenMap result;
...
std::vector<std::string> UsdShadeNodeDefAPI::GetSourceTypes() const {
    // Early exit if the implementation source is not "sourceAsset" or "sourceCode"
    const TfToken implSource = GetImplementationSource();
    if ((implSource != UsdShadeTokens->sourceAsset) && (implSource != UsdShadeTokens->sourceCode)) {
        return {};
    }
    ...
    const std::vector<UsdProperty> props = GetPrim().GetPropertiesInNamespace("info");
    ...
    for (auto it = props.begin(); it != props.end(); it++) {
        ...
        if (TfStringEndsWith(name, implSource.GetString())) {
            const std::vector<std::string> tokenizedName =
                    TfStringTokenize(name, SdfPathTokens->namespaceDelimiter.GetText());
...
SdrShaderNodeConstPtr UsdShadeNodeDefAPI::GetShaderNodeForSourceType(const TfToken& sourceType) const {
    TfToken implSource = GetImplementationSource();
    if (implSource == UsdShadeTokens->id) {
        TfToken shaderId;
        if (GetShaderId(&shaderId)) {
            return SdrRegistry::GetInstance().GetShaderNodeByIdentifierAndType(shaderId, sourceType);
        }
        ...
        if (GetSourceAsset(&sourceAsset, sourceType)) {
            ...
            return SdrRegistry::GetInstance().GetShaderNodeFromAsset(sourceAsset, _GetSdrMetadata(GetPrim()),
                                                                     subIdentifier, sourceType);
        }
    } else if (implSource == UsdShadeTokens->sourceCode) {
        std::string sourceCode;
        if (GetSourceCode(&sourceCode, sourceType)) {
            return SdrRegistry::GetInstance().GetShaderNodeFromSourceCode(sourceCode, sourceType,
                                                                          _GetSdrMetadata(GetPrim()));
        }
    }
