...
static UsdValidationErrorVector _EncapsulationValidator(const UsdPrim& usdPrim) {
    ...
    if (parentConnectable && !parentConnectable.IsContainer()) {
        ...
        // connectable prim.
        errors.emplace_back(UsdShadeValidationErrorNameTokens->connectableInNonContainer, UsdValidationErrorType::Error,
        ...
        std::function<void(const UsdPrim&)> _VerifyValidAncestor = [&](const UsdPrim& currentAncestor) -> void {
            ...
            if (ancestorConnectable) {
                ...
                errors.emplace_back(
                        UsdShadeValidationErrorNameTokens->invalidConnectableHierarchy, UsdValidationErrorType::Error,
                        UsdValidationErrorSites{UsdValidationErrorSite(usdPrim.GetStage(), usdPrim.GetPath())},
                        TfStringPrintf("Connectable %s <%s> can only have "
                                       "Connectable Container ancestors up to %s "
                                       "ancestor <%s>, but its parent %s is a %s.",
                                       usdPrim.GetTypeName().GetText(), usdPrim.GetPath().GetText(),
                                       currentAncestor.GetTypeName().GetText(), currentAncestor.GetPath().GetText(),
                                       parentPrim.GetName().GetText(), parentPrim.GetTypeName().GetText()));
                return;
            }
...
static UsdValidationErrorVector _MaterialBindingApiAppliedValidator(const UsdPrim& usdPrim) {
    UsdValidationErrorVector errors;
    ...
    auto hasMaterialBindingRelationship = [](const UsdPrim& usdPrim) {
        ...
        return std::any_of(relationships.begin(), relationships.end(), [&](const UsdRelationship& rel) {
    ...
    if (!usdPrim.HasAPI<UsdShadeMaterialBindingAPI>() && hasMaterialBindingRelationship(usdPrim)) {
        errors.emplace_back(UsdShadeValidationErrorNameTokens->missingMaterialBindingAPI, UsdValidationErrorType::Error,
                            UsdValidationErrorSites{UsdValidationErrorSite(usdPrim.GetStage(), usdPrim.GetPath())},
...
static UsdValidationErrorVector _MaterialBindingRelationships(const UsdPrim& usdPrim) {
    if (!usdPrim) {
        return {};
    }
    ...
    for (const UsdProperty& matBindingProperty : matBindingProperties) {
        ...
        const UsdValidationErrorSites propertyErrorSites = {
                UsdValidationErrorSite(usdPrim.GetStage(), matBindingProperty.GetPath())};

        errors.emplace_back(UsdShadeValidationErrorNameTokens->materialBindingPropNotARel,
                            UsdValidationErrorType::Error, propertyErrorSites,
                            TfStringPrintf("Prim <%s> has material binding property '%s' that is not "
                                           "a relationship.",
                                           usdPrim.GetPath().GetText(), matBindingProperty.GetName().GetText()));
    }

    return errors;
}

void _MaterialBindingCheckCollection(const UsdPrim& prim,
    ...
    if (targets.size() == 1) {
        if (UsdShadeMaterialBindingAPI::CollectionBinding ::IsCollectionBindingRel(rel)) {
            outErrors.emplace_back(UsdShadeValidationErrorNameTokens->invalidMaterialCollection,
                                   UsdValidationErrorType::Error,
                                   UsdValidationErrorSites{UsdValidationErrorSite(prim.GetStage(), rel.GetPath())},
            ...
            UsdShadeMaterialBindingAPI::DirectBinding directBinding = UsdShadeMaterialBindingAPI::DirectBinding(rel);
            if (!directBinding.GetMaterial()) {
                outErrors.emplace_back(
                        UsdShadeValidationErrorNameTokens->invalidResourcePath, UsdValidationErrorType::Error,
                        UsdValidationErrorSites{UsdValidationErrorSite(prim.GetStage(), rel.GetPath())},
                        TfStringPrintf("Direct material binding <%s> targets "
                                       "an invalid material <%s>.",
                                       rel.GetPath().GetText(), directBinding.GetMaterialPath().GetText()));
            }
        }
    } else if (targets.size() == 2) {
        UsdShadeMaterialBindingAPI::CollectionBinding collBinding = UsdShadeMaterialBindingAPI::CollectionBinding(rel);
        if (!collBinding.GetMaterial()) {
            outErrors.emplace_back(UsdShadeValidationErrorNameTokens->invalidResourcePath,
                                   ...
                                   TfStringPrintf("Collection-based material binding "
                                                  ...
                                                  rel.GetPath().GetText(), collBinding.GetMaterialPath().GetText()));
        }
        if (!collBinding.GetCollection()) {
            outErrors.emplace_back(UsdShadeValidationErrorNameTokens->invalidResourcePath,
                                   ...
                                   TfStringPrintf("Collection-based material binding "
                                                  ...
                                                  rel.GetPath().GetText(), collBinding.GetCollectionPath().GetText()));
        }
    } else {
        outErrors.emplace_back(UsdShadeValidationErrorNameTokens->invalidMaterialCollection,
                               UsdValidationErrorType::Error,
...
static UsdValidationErrorVector _MaterialBindingCollectionValidator(const UsdPrim& usdPrim) {
    ...
    for (const UsdProperty& matBindingProperty : matBindingProperties) {
        if (const UsdRelationship& matBindingRel = matBindingProperty.As<UsdRelationship>()) {
...
static UsdValidationErrorVector _ShaderPropertyTypeConformance(const UsdPrim& usdPrim) {
    ...
    const TfTokenVector expectedImplSource = {UsdShadeTokens->id, UsdShadeTokens->sourceAsset,
                                              UsdShadeTokens->sourceCode};

    const TfToken implSource = shader.GetImplementationSource();
    if (std::find(expectedImplSource.begin(), expectedImplSource.end(), implSource) == expectedImplSource.end()) {
        const UsdValidationErrorSites implSourceErrorSite = {
                UsdValidationErrorSite(usdPrim.GetStage(), shader.GetImplementationSourceAttr().GetPath())};
        return {UsdValidationError(UsdShadeValidationErrorNameTokens->invalidImplSource, UsdValidationErrorType::Error,
                                   implSourceErrorSite,
                                   TfStringPrintf("Shader <%s> has invalid implementation source "
                                                  "'%s'.",
                                                  usdPrim.GetPath().GetText(), implSource.GetText()))};
    }

    const std::vector<std::string> sourceTypes = shader.GetSourceTypes();
    if (sourceTypes.empty() && implSource != UsdShadeTokens->id) {
        const UsdValidationErrorSites primErrorSite = {UsdValidationErrorSite(usdPrim.GetStage(), usdPrim.GetPath())};
        return {UsdValidationError(UsdShadeValidationErrorNameTokens->missingSourceType, UsdValidationErrorType::Error,
                                   primErrorSite,
                                   TfStringPrintf("Shader <%s> has no sourceType.", usdPrim.GetPath().GetText()))};
    }

    UsdValidationErrorVector errors;
    std::unordered_map<TfToken, SdrShaderPropertyConstPtr, TfToken::HashFunctor> propNameToPropertyMap;
    if (sourceTypes.empty() && implSource == UsdShadeTokens->id) {
        TfToken shaderId;
        if (shader.GetShaderId(&shaderId)) {
            // Single shaderNode, just emplace all properties, no need to find
            // anything.
            if (SdrShaderNodeConstPtr sdrShaderNode = SdrRegistry::GetInstance().GetShaderNodeByIdentifier(shaderId)) {
                ...
                return {UsdValidationError(UsdShadeValidationErrorNameTokens->missingShaderIdInRegistry,
                                           ...
                                           TfStringPrintf("shaderId '%s' specified on shader prim "
                                                          ...
                                                          shaderId.GetText(), usdPrim.GetPath().GetText()))};
            }
        }
    } else {
        ...
        for (const auto& sourceType : sourceTypes) {
            if (SdrShaderNodeConstPtr sdrShaderNode = shader.GetShaderNodeForSourceType(TfToken(sourceType))) {
                ...
                errors.emplace_back(UsdShadeValidationErrorNameTokens->missingSourceTypeInRegistry,
                                    UsdValidationErrorType::Error, sourceTypeSites,
                                    TfStringPrintf("sourceType '%s' specified on shader prim "
                                                   "<%s> not found in sdrRegistry.",
                                                   sourceType.c_str(), usdPrim.GetPath().GetText()));
            }
        }
        ...
        const UsdValidationErrorSites sdrWarnSite = {UsdValidationErrorSite(usdPrim.GetStage(), usdPrim.GetPath())};
        for (const auto& entry : sdrShaderComplianceResults) {
            ...
            errors.emplace_back(UsdShadeValidationErrorNameTokens->incompatShaderPropertyWarning,
                                UsdValidationErrorType::Warn, sdrWarnSite,
                                TfStringPrintf("Shader nodes '%s' have incompatible property "
                                               "'%s'.",
                                               TfStringJoin(shaderNames).c_str(), entry.first.GetText()));
        }
    }
    ...
    for (const UsdShadeInput& input : shader.GetInputs(false)) {
        ...
        if (sdrPropnameToSdfType.find(baseName) != sdrPropnameToSdfType.end()) {
            ...
            if (usdInputType != expectedSdrInputType) {
                ...
                errors.emplace_back(
                        UsdShadeValidationErrorNameTokens->mismatchPropertyType, UsdValidationErrorType::Error,
                        inputErrorSite,
                        TfStringPrintf("Incorrect type for %s. "
                                       "Expected '%s'; got '%s'.",
                                       input.GetAttr().GetPath().GetText(), expectedSdrInputType.GetAsToken().GetText(),
...
static UsdValidationErrorVector _SubsetMaterialBindFamilyName(const UsdPrim& usdPrim) {
    if (!(usdPrim && usdPrim.IsInFamily<UsdGeomSubset>(UsdSchemaRegistry::VersionPolicy::All))) {
        return {};
    }
    ...
    const UsdValidationErrorSites primErrorSites = {UsdValidationErrorSite(usdPrim.GetStage(), usdPrim.GetPath())};

    return {UsdValidationError(UsdShadeValidationErrorNameTokens->missingFamilyNameOnGeomSubset,
                               UsdValidationErrorType::Error, primErrorSites,
                               TfStringPrintf("GeomSubset prim <%s> with material bindings applied but "
                                              "no authored family name should set familyName to '%s'.",
                                              usdPrim.GetPath().GetText(), UsdShadeTokens->materialBind.GetText()))};
}

static UsdValidationErrorVector _SubsetsMaterialBindFamily(const UsdPrim& usdPrim) {
    if (!(usdPrim && usdPrim.IsInFamily<UsdGeomImageable>(UsdSchemaRegistry::VersionPolicy::All))) {
        return {};
    }
    ...
    const std::vector<UsdGeomSubset> materialBindSubsets =
            UsdGeomSubset::GetGeomSubsets(imageable,
                                          /* elementType = */ TfToken(),
    ...
    // multiple materials.
    const TfToken materialBindFamilyType = UsdGeomSubset::GetFamilyType(imageable, UsdShadeTokens->materialBind);
    if (materialBindFamilyType == UsdGeomTokens->unrestricted) {
        const UsdValidationErrorSites primErrorSites = {UsdValidationErrorSite(usdPrim.GetStage(), usdPrim.GetPath())};

        errors.emplace_back(UsdShadeValidationErrorNameTokens->invalidFamilyType, UsdValidationErrorType::Error,
                            primErrorSites,
                            TfStringPrintf("Imageable prim <%s> has '%s' subset family with invalid "
                                           "family type '%s'. Family type should be '%s' or '%s' "
                                           "instead.",
                                           usdPrim.GetPath().GetText(), UsdShadeTokens->materialBind.GetText(),
                                           materialBindFamilyType.GetText(), UsdGeomTokens->nonOverlapping.GetText(),
...
TF_REGISTRY_FUNCTION(UsdValidationRegistry) {
    ...
    registry.RegisterPluginValidator(UsdShadeValidatorNameTokens->materialBindingApiAppliedValidator,
                                     _MaterialBindingApiAppliedValidator);

    registry.RegisterPluginValidator(UsdShadeValidatorNameTokens->materialBindingRelationships,
                                     _MaterialBindingRelationships);

    registry.RegisterPluginValidator(UsdShadeValidatorNameTokens->materialBindingCollectionValidator,
                                     _MaterialBindingCollectionValidator);

    registry.RegisterPluginValidator(UsdShadeValidatorNameTokens->shaderSdrCompliance, _ShaderPropertyTypeConformance);

    registry.RegisterPluginValidator(UsdShadeValidatorNameTokens->subsetMaterialBindFamilyName,
                                     _SubsetMaterialBindFamilyName);

    registry.RegisterPluginValidator(UsdShadeValidatorNameTokens->subsetsMaterialBindFamily,
                                     _SubsetsMaterialBindFamily);

    registry.RegisterPluginValidator(UsdShadeValidatorNameTokens->encapsulationValidator, _EncapsulationValidator);
}
