...
static std::vector<UsdShadeUdimUtils::ResolvedPathAndTile> _ResolveUdimPaths(const std::string& udimPath,
...
/* static */
std::string UsdShadeUdimUtils::ResolveUdimPath(const std::string& udimPath, const SdfLayerHandle& layer) {
    // Return empty if passed path is a non-UDIM path or just doesn't
    // resolve as a UDIM
    std::vector<ResolvedPathAndTile> udimPaths = _ResolveUdimPaths(udimPath, layer, /* stopAtFirst = */ true);
    ...
    if (!TfStringEndsWith(firstTilePath, suffix)) {
        TF_WARN("Resolution of first udim tile gave ambigious result. "
    ...
    return firstTilePackage.empty() ? firstTilePath : ArJoinPackageRelativePath(firstTilePackage, firstTilePath);
}
