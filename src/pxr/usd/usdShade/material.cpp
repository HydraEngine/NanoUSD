...
TF_REGISTRY_FUNCTION(TfType) {
    TfType::Define<UsdShadeMaterial, TfType::Bases<UsdShadeNodeGraph>>();
...
UsdShadeMaterial UsdShadeMaterial::Define(const UsdStagePtr& stage, const SdfPath& path) {
    ...
    return UsdShadeMaterial(stage->DefinePrim(path, usdPrimTypeName));
}
...
UsdAttribute UsdShadeMaterial::CreateSurfaceAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdShadeTokens->outputsSurface, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}

UsdAttribute UsdShadeMaterial::GetDisplacementAttr() const {
    return GetPrim().GetAttribute(UsdShadeTokens->outputsDisplacement);
}

UsdAttribute UsdShadeMaterial::CreateDisplacementAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdShadeTokens->outputsDisplacement, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}

UsdAttribute UsdShadeMaterial::GetVolumeAttr() const {
    return GetPrim().GetAttribute(UsdShadeTokens->outputsVolume);
}

UsdAttribute UsdShadeMaterial::CreateVolumeAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdShadeTokens->outputsVolume, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}
...
/*static*/
const TfTokenVector& UsdShadeMaterial::GetSchemaAttributeNames(bool includeInherited) {
    ...
    static TfTokenVector allNames =
            _ConcatenateAttributeNames(UsdShadeNodeGraph::GetSchemaAttributeNames(true), localNames);
...
std::pair<UsdStagePtr, UsdEditTarget> UsdShadeMaterial::GetEditContextForVariant(const TfToken& materialVariation,
    ...
    if (materialVariant.AddVariant(materialVariation) && materialVariant.SetVariantSelection(materialVariation)) {
...
static SdfPath _GetRootPath(const UsdPrim& prim) {
    SdfPath path = prim.GetPrimPath();

    // special-case pseudo-root
    if (path == SdfPath::AbsoluteRootPath()) return path;

    while (!path.IsRootPrimPath()) path = path.GetParentPath();

    return path;
}

UsdVariantSet UsdShadeMaterial::GetMaterialVariant() const {
    return GetPrim().GetVariantSet(UsdShadeTokens->materialVariant);
}

/* static */
bool UsdShadeMaterial::CreateMasterMaterialVariant(const UsdPrim& masterPrim,
    ...
    TfToken masterSetName = masterVariantSetName.IsEmpty() ? UsdShadeTokens->materialVariant : masterVariantSetName;
    ...
    TF_FOR_ALL(material, materials) {
        ...
        if (stage != material->GetStage()) {
            TF_CODING_ERROR(
                    ...
                    masterPrim.GetPath().GetText(), material->GetPrimPath().GetText());
            return false;
        }
    ...
    TF_FOR_ALL(varName, allMaterialVariants) {
        if (!masterSet.AddVariant(*varName)) {
            TF_RUNTIME_ERROR(
                    ...
                    varName->c_str(), masterPrim.GetPath().GetText());
            return false;
        }
...

        {
            ...
            TF_FOR_ALL(material, materials) {
                if (!*material) {
                    ...
                    // to expire.
                    TF_RUNTIME_ERROR(
                            "Switching master variant %s to %s "
                            "caused one or more material prims to "
                            "expire.  First such: %s.",
                            masterSetName.GetText(), varName->c_str(), material->GetDescription().c_str());
                    return false;
                }
                ...
                if (material->GetPath().HasPrefix(masterPrim.GetPath())) {
                    ...
                    SdfPath derivedPath =
                            material->GetPrimPath().ReplacePrefix(_GetRootPath(*material), masterPrim.GetPath());
...
SdfPath UsdShadeMaterial::GetBaseMaterialPath() const {
    SdfPath parentMaterialPath = FindBaseMaterialPathInPrimIndex(GetPrim().GetPrimIndex(), [this](const SdfPath& p) {
...
void UsdShadeMaterial::ClearBaseMaterial() const {
    SetBaseMaterialPath(SdfPath());
}
...
UsdShadeAttributeVector UsdShadeMaterial::_ComputeNamedOutputSources(const TfToken& baseName,
    ...
    for (TfToken const& renderContext : contextVector) {
        ...
        if (output) {
            ...
            UsdShadeAttributeVector valueAttrs =
                    UsdShadeUtils::GetValueProducingAttributes(output, /*shaderOutputsOnly*/ true);
            ...
            if (valueAttrs.size() > 1) {
                TF_WARN("Multiple connected sources for output %s:%s on material"
                        " %s. Only the first will be consider as a terminal.",
                        renderContext.GetText(), baseName.GetText(), GetPath().GetText());
            }
    ...
    if (!universalRenderContextComputed) {
        ...
        if (TF_VERIFY(universalOutput)) {
            return UsdShadeUtils::GetValueProducingAttributes(universalOutput, /*shaderOutputsOnly*/ true);
        }
    }
...
std::vector<UsdShadeOutput> UsdShadeMaterial::_GetOutputsForTerminalName(const TfToken& terminalName) const {
    std::vector<UsdShadeOutput> outputs;

    UsdShadeOutput universalOutput = GetOutput(_GetOutputName(terminalName, UsdShadeTokens->universalRenderContext));
    if (universalOutput) {
...
UsdShadeOutput UsdShadeMaterial::CreateSurfaceOutput(const TfToken& renderContext) const {
    return CreateOutput(_GetOutputName(UsdShadeTokens->surface, renderContext), SdfValueTypeNames->Token);
}

UsdShadeOutput UsdShadeMaterial::GetSurfaceOutput(const TfToken& renderContext) const {
...
UsdShadeShader UsdShadeMaterial::ComputeSurfaceSource(const TfToken& renderContext,
                                                      TfToken* sourceName,
                                                      UsdShadeAttributeType* sourceType) const {
    TRACE_FUNCTION();
...
UsdShadeOutput UsdShadeMaterial::GetDisplacementOutput(const TfToken& renderContext) const {
...
UsdShadeShader UsdShadeMaterial::ComputeDisplacementSource(const TfToken& renderContext,
                                                           TfToken* sourceName,
                                                           UsdShadeAttributeType* sourceType) const {
    TRACE_FUNCTION();
    return _ComputeNamedOutputShader(UsdShadeTokens->displacement, {renderContext}, sourceName, sourceType);
}
...
UsdShadeOutput UsdShadeMaterial::GetVolumeOutput(const TfToken& renderContext) const {
...
UsdShadeShader UsdShadeMaterial::ComputeVolumeSource(const TfToken& renderContext,
                                                     TfToken* sourceName,
                                                     UsdShadeAttributeType* sourceType) const {
    TRACE_FUNCTION();
...
public:
    ...
    bool CanConnectOutputToSource(const UsdShadeOutput& output,
                                  const UsdAttribute& source,
                                  std::string* reason) const override {
        return _CanConnectOutputToSource(output, source, reason, ConnectableNodeTypes::DerivedContainerNodes);
    }
};

TF_REGISTRY_FUNCTION(UsdShadeConnectableAPI) {
    UsdShadeRegisterConnectableAPIBehavior<UsdShadeMaterial, UsdShadeMaterial_ConnectableAPIBehavior>();
}
