...
struct UsdHydraTokensType {
    ...
    /// \brief "black"
    ///
    /// Possible value for "wrapT" and "wrapS" inputs on a "UvTexture" shader prim. Causes black to be returned when
    /// sampling outside the bounds of the texture.
    const TfToken black;
    /// \brief "clamp"
    ///
    /// Possible value for "wrapT" and "wrapS" inputs on a "UvTexture" shader prim. Causes the the texture coordinate to
    /// be clamped to [0,1].
    const TfToken clamp;
    /// \brief "displayLook:bxdf"
    ///
    /// deprecated This has been deprecated in favor of the glslfx:surface output.  Relationship on a material that
    /// targets the "bxdf" or the surface shader prim.
    ...
    /// \brief "linear"
    ///
    /// A weighted linear blend of nearest adjacent samples. Possible value for "minFilter" and "magFilter" inputs on a
    /// UvTextureshader.
    ...
    /// \brief "mirror"
    ///
    /// Possible value for "wrapT" and "wrapS" inputs on a "UvTexture" shader prim. Causes the texture coordinate to
    /// wrap around like a mirror. -0.2 becomes 0.2, -1.2 becomes 0.8, etc. ,
    const TfToken mirror;
    /// \brief "nearest"
    ///
    /// Selects the nearest sample for the given coordinate  Possible value for "minFilter" and "magFilter" inputs on a
    /// UvTexture shader.
    const TfToken nearest;
    /// \brief "nearestMipmapLinear"
    ///
    /// See https://www.opengl.org/wiki/Sampler_Object Possible value for "minFilter" and "magFilter" inputs on a
    /// UvTexture shader.
    ...
    ///
    /// Possible value for "wrapT" and "wrapS" inputs on a "UvTexture" shader prim.  Causes the texture coordinate to
    /// wrap around the texture. So a texture coordinate of -0.2 becomes the equivalent of 0.8.
    ...
    /// \brief "useMetadata"
    ///
    /// Possible value for "wrapT" and "wrapS" inputs on a "UvTexture" shader prim. Causes the wrap value to be loaded
    /// from the texture file instead of being specified in the prim.  If the texture file doesn't support metadata or
    /// the metadata doesn't contain a wrap mode,  the "black" wrap mode is used.
    ...
    /// \brief "wrapS"
    ///
    /// A shader input on a hydra UvTexture shader which defines the behavior of texture coordinates that are outside
