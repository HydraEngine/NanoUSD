...
TF_DEFINE_ENV_SETTING(PXR_AR_DISABLE_PLUGIN_URI_RESOLVERS, false, "Disables plugin URI/IRI resolver implementations.");
...
std::pair<bool, std::string> _ValidateResourceIdentifierScheme(const std::string_view& caseFoldedScheme) {
    ...
    const auto it = std::find_if(std::next(caseFoldedScheme.begin()), caseFoldedScheme.end(), [](const char c) {
        return !((c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') || (c == '-') || (c == '.') || (c == '+'));
    });
    if (it != caseFoldedScheme.end()) {
        TfUtf8CodePointIterator codePointIt(it, caseFoldedScheme.end());
        return std::make_pair(false, TfStringPrintf("'%s' not allowed in scheme. "
...
JsOptionalValue _FindMetadataValueOnTypeOrBase(const TfToken& metadata, const TfType& t) {
    ...
    JsOptionalValue value = JsFindValue(plugin->GetMetadataForType(t), metadata);

    if (value) {
        if (value->Is<T>()) {
            return value;
        } else {
            TF_CODING_ERROR("'%s' metadata for %s must be a %s.", metadata.GetText(), t.GetTypeName().c_str(),
...
std::vector<_ResolverInfo> _GetAvailableResolvers() {
    ...
    for (const TfType& resolverType : sortedResolverTypes) {
        ...
        if (const JsOptionalValue uriSchemesVal =
                    JsFindValue(plugin->GetMetadataForType(resolverType), _tokens->uriSchemes.GetString())) {
            if (uriSchemesVal->IsArrayOf<std::string>()) {
                uriSchemes = uriSchemesVal->GetArrayOf<std::string>();
            } else {
                TF_CODING_ERROR("'%s' metadata for %s must be a list of strings.", _tokens->uriSchemes.GetText(),
        ...
        const JsOptionalValue implementsContextsVal =
                _FindMetadataValueOnTypeOrBase<bool>(_tokens->implementsContexts, resolverType);

        const JsOptionalValue implementsScopedCachesVal =
                _FindMetadataValueOnTypeOrBase<bool>(_tokens->implementsScopedCaches, resolverType);
...
std::vector<_ResolverInfo> _GetAvailablePrimaryResolvers(const std::vector<_ResolverInfo>& availableResolvers) {
    ...
    const std::vector<_ResolverInfo>* allAvailableResolvers =
            TfGetEnvSetting(PXR_AR_DISABLE_PLUGIN_RESOLVER) ? &emptyResolverList : &availableResolvers;
    ...
    for (const _ResolverInfo& resolver : *allAvailableResolvers) {
        ...
        if (std::find(_resolverStack->begin(), _resolverStack->end(), resolver.type) != _resolverStack->end()) {
            continue;
        }
...
public:
    ...
    std::unique_ptr<ResolverType> Create() {
        ...
        if (!_plugin->Load()) {
            TF_CODING_ERROR("Failed to load plugin %s for %s", _plugin->GetName().c_str(),
        ...
        if (!resolver) {
            TF_CODING_ERROR("Failed to manufacture asset resolver %s from plugin %s",
                            _resolverType.GetTypeName().c_str(), _plugin->GetName().c_str());
        }
...
std::unique_ptr<ArResolver> _CreateResolver(const TfType& resolverType, std::string* debugMsg = nullptr) {
    ...
    TfScoped<> popResolverStack([]() {
        _resolverStack->pop_back();
    });
    ...
    if (!resolverType) {
        ...
        const PlugPluginPtr plugin = _GetPluginForType(resolverType);
        if (plugin) {
            tmpResolver = _PluginResolver<ArResolver, Ar_ResolverFactoryBase>(plugin, resolverType).Create();

            if (tmpResolver && debugMsg) {
                *debugMsg = TfStringPrintf("Using asset resolver %s from plugin %s", resolverType.GetTypeName().c_str(),
    ...
    if (!tmpResolver) {
        if (debugMsg) {
            *debugMsg = TfStringPrintf("Using default asset resolver %s", defaultResolverType.GetTypeName().c_str());
        }
...
public:
    ...
    ArResolverContext CreateContextFromString(const std::string& uriScheme, const std::string& contextStr) const {
        ArResolver* resolver = uriScheme.empty() ? _resolver->Get() : _GetURIResolverForScheme(uriScheme);
        return resolver ? resolver->CreateContextFromString(contextStr) : ArResolverContext();
    }

    ArResolverContext CreateContextFromStrings(const std::vector<std::pair<std::string, std::string>>& strs) const {
    ...
    std::string _CreateIdentifier(const std::string& assetPath, const ArResolvedPath& anchorAssetPath) const final {
        return _CreateIdentifierHelper(
                ...
                [](ArResolver& resolver, const std::string& assetPath, const ArResolvedPath& anchorAssetPath) {
    ...
    std::string _CreateIdentifierForNewAsset(const std::string& assetPath,
        ...
        return _CreateIdentifierHelper(
                assetPath, anchorAssetPath,
                [](ArResolver& resolver, const std::string& assetPath, const ArResolvedPath& anchorAssetPath) {
                    return resolver.CreateIdentifierForNewAsset(assetPath, anchorAssetPath);
                });
    }
    ...
    std::string _CreateIdentifierHelper(const std::string& assetPath,
        ...
        const ArResolvedPath anchorResolvedPath(ArSplitPackageRelativePathOuter(anchorAssetPath).first);

        if (ArIsPackageRelativePath(assetPath)) {
            std::pair<std::string, std::string> packageAssetPath = ArSplitPackageRelativePathOuter(assetPath);
            packageAssetPath.first = createIdentifierFn(*resolver, packageAssetPath.first, anchorResolvedPath);
    ...
    ArResolverContext _GetCurrentContext() const final {
        ...
        if (const ArResolverContext* ctx = GetInternallyManagedCurrentContext()) {
        ...
        auto resolveFn = [this](const std::string& path) {
            ...
            if (!info->implementsScopedCaches) {
                if (_CachePtr currentCache = _threadCache.GetCurrentCache()) {
                    ...
                    if (currentCache->_pathToResolvedPathMap.insert(accessor, std::make_pair(path, ArResolvedPath()))) {
    ...
    ArAssetInfo _GetAssetInfo(const std::string& assetPath, const ArResolvedPath& resolvedPath) const final {
        ArAssetInfo assetInfo;

        ArResolver& resolver = _GetResolver(assetPath);
        if (ArIsPackageRelativePath(assetPath)) {
            std::pair<std::string, std::string> packageAssetPath = ArSplitPackageRelativePathOuter(assetPath);
            std::pair<std::string, std::string> packageResolvedPath = ArSplitPackageRelativePathOuter(resolvedPath);

            assetInfo = resolver.GetAssetInfo(packageAssetPath.first, ArResolvedPath(packageResolvedPath.first));
            ...
            if (!assetInfo.repoPath.empty()) {
                assetInfo.repoPath = ArJoinPackageRelativePath(assetInfo.repoPath, packageResolvedPath.second);
            }
    ...
    ArTimestamp _GetModificationTimestamp(const std::string& path, const ArResolvedPath& resolvedPath) const final {
        ArResolver& resolver = _GetResolver(path);
        if (ArIsPackageRelativePath(path)) {
            return resolver.GetModificationTimestamp(
                    ArSplitPackageRelativePathOuter(path).first,
                    ArResolvedPath(ArSplitPackageRelativePathOuter(resolvedPath).first));
        }
...
private:
    void _InitializePrimaryResolver(const std::vector<_ResolverInfo>& availableResolvers) {
        ...
        const std::vector<_ResolverInfo> primaryResolvers = _GetAvailablePrimaryResolvers(availableResolvers);
        ...
        auto createResolver = [&, this](const TfType& resolverType) {
            for (const _ResolverInfo& info : primaryResolvers) {
                ...
                if (resolver) {
                    _resolver = std::make_shared<_Resolver>(info, std::shared_ptr<ArResolver>(resolver.release()));
                    return true;
                }
            }
            return false;
    ...
    void _InitializeURIResolvers(const std::vector<_ResolverInfo>& availableResolvers) {
        ...
        for (const _ResolverInfo& resolverInfo : availableResolvers) {
            TF_DEBUG(AR_RESOLVER_INIT)
                    .Msg("ArGetResolver(): Found URI resolver %s\n", resolverInfo.type.GetTypeName().c_str());
            ...
            for (std::string uriScheme : resolverInfo.uriSchemes) {
                ...
                } else if (const auto validation = _ValidateResourceIdentifierScheme(uriScheme); !validation.first) {
                    TF_WARN("ArGetResolver(): '%s' for '%s' is not a valid "
                            "resource identifier scheme: %s. Paths with this "
                            "prefix will be handled by other resolvers.",
                            uriScheme.c_str(), resolverInfo.type.GetTypeName().c_str(), validation.second.c_str());
                } else {
            ...
            TF_DEBUG(AR_RESOLVER_INIT)
                    .Msg("ArGetResolver(): Using %s for URI scheme(s) [\"%s\"]\n",
                         resolverInfo.type.GetTypeName().c_str(), TfStringJoin(uriSchemes, "\", \"").c_str());
            ...
            _ResolverSharedPtr uriResolver =
                    resolverInfo.type == _resolver->GetType() ? _resolver : std::make_shared<_Resolver>(resolverInfo);
    ...
    void _InitializePackageResolvers() {
        ...
        for (const TfType& packageResolverType : packageResolverTypes) {
            ...
            const JsOptionalValue extensionsVal =
                    JsFindValue(plugin->GetMetadataForType(packageResolverType), _tokens->extensions.GetString());
            if (!extensionsVal) {
                TF_CODING_ERROR("No package formats specified in '%s' metadata for '%s'", _tokens->extensions.GetText(),
                                packageResolverType.GetTypeName().c_str());
                continue;
            }
            ...
            if (extensionsVal->IsArrayOf<std::string>()) {
                ...
                TF_CODING_ERROR("'%s' metadata for %s must be a list of strings.", _tokens->extensions.GetText(),
                                packageResolverType.GetTypeName().c_str());
                continue;
            }
            ...
            for (const std::string& extension : extensions) {
                ...
                _packageResolvers.push_back(std::make_shared<_PackageResolver>(extension, plugin, packageResolverType));

                TF_DEBUG(AR_RESOLVER_INIT)
                        .Msg("ArGetResolver(): Using package resolver %s for %s "
                             "from plugin %s\n",
                             packageResolverType.GetTypeName().c_str(), extension.c_str(), plugin->GetName().c_str());
            }
        }
    }

    ArResolver& _GetResolver(const std::string& assetPath, const _ResolverInfo** info = nullptr) const {
    ...
    ArResolver* _GetURIResolver(const std::string& assetPath, const _ResolverInfo** info = nullptr) const {
    ...
    ArResolver* _GetURIResolverForScheme(const std::string& scheme, const _ResolverInfo** info = nullptr) const {
        ...
        // as well.
        const _ResolverSharedPtr* uriResolver = TfMapLookupPtr(_uriResolvers, TfStringToLowerAscii(scheme));
        if (uriResolver) {
            if (info) {
    ...
    class _PackageResolver : public _PluginResolver<ArPackageResolver, Ar_PackageResolverFactoryBase> {
        using Base = _PluginResolver<ArPackageResolver, Ar_PackageResolverFactoryBase>;

    public:
        _PackageResolver(const std::string& packageFormat, const PlugPluginPtr& plugin, const TfType& resolverType)
            : Base(plugin, resolverType), _packageFormat(packageFormat) {}

        bool HandlesFormat(const std::string& extension) const { return _packageFormat == extension; }
...
std::string ArResolver::CreateIdentifier(const std::string& assetPath, const ArResolvedPath& anchorAssetPath) const {
...
ArAssetInfo ArResolver::GetAssetInfo(const std::string& assetPath, const ArResolvedPath& resolvedPath) const {
...
ArAssetInfo ArResolver::_GetAssetInfo(const std::string& assetPath, const ArResolvedPath& resolvedPath) const {
    return ArAssetInfo();
}
...
bool ArResolver::_CanWriteAssetToPath(const ArResolvedPath& resolvedPath, std::string* whyNot) const {
    return true;
}
...
const std::vector<std::string>& ArGetRegisteredURISchemes() {
    static const std::vector<std::string> uriSchemes = _GetResolver().GetURISchemes();
    return uriSchemes;
}
