...
std::shared_ptr<ArFilesystemWritableAsset> ArFilesystemWritableAsset::Create(const ArResolvedPath& resolvedPath,
    ...
    if (!dir.empty() && !TfIsDir(dir) && !TfMakeDirs(dir, -1, true)) {
        TF_RUNTIME_ERROR("Could not create directory '%s' for asset '%s'", dir.c_str(),
