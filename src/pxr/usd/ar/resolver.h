...
public:
    ...
    /// ArResolvedPath.
    AR_API
    ArResolvedPath Resolve(const std::string& assetPath) const;
    ...
    /// resolved path.
    AR_API
    ArResolvedPath ResolveForNewAsset(const std::string& assetPath) const;
    ...
    AR_API
    void BindContext(const ArResolverContext& context, VtValue* bindingData);
    ...
    /// \see ArResolverContextBinder
    AR_API
    void UnbindContext(const ArResolverContext& context, VtValue* bindingData);
    ...
    AR_API
    bool IsContextDependentPath(const std::string& assetPath) const;
    ...
    AR_API
    std::string GetExtension(const std::string& assetPath) const;
    ...
    AR_API
    ArTimestamp GetModificationTimestamp(const std::string& assetPath, const ArResolvedPath& resolvedPath) const;
    ...
    AR_API
    std::shared_ptr<ArWritableAsset> OpenAssetForWrite(const ArResolvedPath& resolvedPath, WriteMode writeMode) const;
    ...
    AR_API
    bool CanWriteAssetToPath(const ArResolvedPath& resolvedPath, std::string* whyNot = nullptr) const;
...
protected:
    ...
    ///   identifier.
    virtual std::string _CreateIdentifierForNewAsset(const std::string& assetPath,
                                                     const ArResolvedPath& anchorAssetPath) const = 0;
    ...
    /// The default implementation does nothing.
    AR_API
    virtual void _RefreshContext(const ArResolverContext& context);
    ...
    ///   identifier.
    AR_API
    virtual bool _IsContextDependentPath(const std::string& assetPath) const;
    ...
    AR_API
    virtual std::string _GetExtension(const std::string& assetPath) const;
    ...
    AR_API
    virtual ArAssetInfo _GetAssetInfo(const std::string& assetPath, const ArResolvedPath& resolvedPath) const;
    ...
    AR_API
    virtual void _BeginCacheScope(VtValue* cacheScopeData);
    ...
    /// for more implementation details.
    AR_API
    virtual void _EndCacheScope(VtValue* cacheScopeData);
    ...
    AR_API
    virtual bool _IsRepositoryPath(const std::string& path) const;
