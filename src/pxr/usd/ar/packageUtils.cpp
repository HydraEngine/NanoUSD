...
bool ArIsPackageRelativePath(const std::string& path) {
    return !path.empty() && path.back() == ']' && _FindMatchingOpeningDelimiter(path, path.end() - 1) != path.end();
}
...
std::string _JoinPackageRelativePath(Iter begin, Iter end) {
    ...
    if (packageRelativePath.back() == ']') {
        auto innermostCloseRevIt = std::find_if(packageRelativePath.rbegin(), packageRelativePath.rend(), [](char c) {
            return c != ']';
        });
...
std::pair<std::string, std::string> ArSplitPackageRelativePathInner(const std::string& path) {
    ...
    packagePath.erase(std::distance(path.begin(), innermostOpenIt),
