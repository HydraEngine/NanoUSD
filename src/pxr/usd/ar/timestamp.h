...
public:
    ...
    friend bool operator==(const ArTimestamp& lhs, const ArTimestamp& rhs) {
        return (!lhs.IsValid() && !rhs.IsValid()) || (lhs.IsValid() && rhs.IsValid() && lhs._time == rhs._time);
    }

    friend bool operator!=(const ArTimestamp& lhs, const ArTimestamp& rhs) { return !(lhs == rhs); }
