...
ArResolvedPath ArDefaultResolver::_Resolve(const std::string& path) const {
    ...
    if (_IsRelativePath(path)) {
        ...
        if (_IsSearchPath(path)) {
            const ArDefaultResolverContext* contexts[2] = {_GetCurrentContextPtr(), &_DefaultPath->context};
...
ArResolverContext ArDefaultResolver::_CreateDefaultContextForAsset(const std::string& assetPath) const {
    if (assetPath.empty()) {
