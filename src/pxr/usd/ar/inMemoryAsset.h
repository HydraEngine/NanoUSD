...
public:
    ...
    AR_API
    static std::shared_ptr<ArInMemoryAsset> FromAsset(const ArAsset& srcAsset);

    /// Constructs a new instance sharing ownership of the given \p buffer
    /// containing \p bufferSize bytes.
    AR_API
    static std::shared_ptr<ArInMemoryAsset> FromBuffer(const std::shared_ptr<const char>& buffer, size_t bufferSize);

    /// Constructs a new instance taking ownership of the given \p buffer
    /// containing \p bufferSize bytes.
    AR_API
    static std::shared_ptr<ArInMemoryAsset> FromBuffer(std::shared_ptr<const char>&& buffer, size_t bufferSize);
