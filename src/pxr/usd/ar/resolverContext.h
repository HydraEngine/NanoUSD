...
struct Ar_AllValidForContext<Object, Other...> {
    static const bool value = (std::is_same<Object, ArResolverContext>::value || ArIsContextObject<Object>::value) &&
...
public:
    ...
    bool operator!=(const ArResolverContext& rhs) const { return !(*this == rhs); }
    ...
    friend size_t hash_value(const ArResolverContext& context) { return TfHash()(context._contexts); }
...
private:
    ...
    struct _Untyped {
        ...
        bool IsHolding(const std::type_info& ti) const { return TfSafeTypeCompare(ti, GetTypeid()); }
    ...
    struct _Typed : public _Untyped {
        ...
        virtual bool LessThan(const _Untyped& rhs) const { return _context < _GetTyped<Context>(rhs)._context; }

        virtual bool Equals(const _Untyped& rhs) const { return _context == _GetTyped<Context>(rhs)._context; }

        virtual size_t Hash() const { return hash_value(_context); }

        virtual std::string GetDebugString() const { return ArGetDebugString(_context); }
