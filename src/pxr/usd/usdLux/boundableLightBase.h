...
public:
    ...
    /// an invalid \p prim
    explicit UsdLuxBoundableLightBase(const UsdPrim& prim = UsdPrim()) : UsdGeomBoundable(prim) {}
    ...
    explicit UsdLuxBoundableLightBase(const UsdSchemaBase& schemaObj) : UsdGeomBoundable(schemaObj) {}
    ...
    ///
    USDLUX_API
    static UsdLuxBoundableLightBase Get(const UsdStagePtr& stage, const SdfPath& path);
