...
TF_DEFINE_PRIVATE_TOKENS(
        ...
        /* discoveryTypes */
        ((sourceType, "USD"))((discoveryType, "usd-schema-gen"))

                (MeshLight)(MeshLightAPI)(LightAPI)(ShadowAPI)(ShapingAPI)(VolumeLight)(VolumeLightAPI));

/*static*/
const TfToken& UsdLux_LightDefParserPlugin::_GetSourceType() {
    return _tokens->sourceType;
}

/*static*/
const TfToken& UsdLux_LightDefParserPlugin::_GetDiscoveryType() {
...
UsdLux_LightDefParserPlugin::_GetShaderIdToAPITypeNameMap() {
    static const UsdLux_LightDefParserPlugin::ShaderIdToAPITypeNameMap shaderIdToAPITypeNameMap = {
            {_tokens->MeshLight, _tokens->MeshLightAPI}, {_tokens->VolumeLight, _tokens->VolumeLightAPI}};
    return shaderIdToAPITypeNameMap;
}

static NdrTokenMap _GetSdrMetadata(const UsdShadeConnectableAPI& connectable,
    ...
    metadata[SdrNodeMetadata->Primvars] = UsdShadeShaderDefUtils::GetPrimvarNamesMetadataString(metadata, connectable);

    return metadata;
}

static SdfLayerRefPtr _GetGeneratedSchema() {
    // Get the generateSchema file for this plugin and open it as a layer.
    const std::string fname = PLUG_THIS_PLUGIN->FindPluginResource("generatedSchema.usda", false);
    SdfLayerRefPtr layer = SdfLayer::OpenAsAnonymous(fname);
    return layer;
}

static bool _CopyPropertiesFromSchema(const SdfLayerRefPtr& schemaLayer,
    ...
    const SdfPath schemaPath = SdfPath::AbsoluteRootPath().AppendChild(schemaName);
    SdfPrimSpecHandle schemaSpec = schemaLayer->GetPrimAtPath(schemaPath);
    if (!schemaSpec) {
        TF_CODING_ERROR(
                "The generatedSchema for UsdLux does not have a prim "
    ...
    for (const SdfPropertySpecHandle& propSpec : schemaSpec->GetProperties()) {
        if (!SdfCopySpec(schemaLayer, propSpec->GetPath(), destLayer,
                         destPath.AppendProperty(propSpec->GetNameToken()))) {
            TF_CODING_ERROR(
                    "Could not copy property spec '%s' from "
                    "generatedSchema for UsdLux schema '%s' to "
                    "destination layer.",
                    propSpec->GetPath().GetText(), schemaName.GetText());
            return false;
        }
    }
    return true;
...
NdrNodeUniquePtr UsdLux_LightDefParserPlugin::Parse(const NdrNodeDiscoveryResult& discoveryResult) {
    TRACE_FUNCTION();

    const UsdLux_LightDefParserPlugin::ShaderIdToAPITypeNameMap& shaderIdToAPITypeNameMap =
            _GetShaderIdToAPITypeNameMap();
    ...
    const TfToken& primTypeName =
            (shaderIdToAPITypeNameMap.find(discoveryResult.identifier) == shaderIdToAPITypeNameMap.end())
    ...
    SdfPrimSpecHandle primSpec = SdfPrimSpec::New(layer, primTypeName, SdfSpecifierDef);
    ...
    const TfTokenVector schemas({_tokens->LightAPI, primTypeName, _tokens->ShadowAPI, _tokens->ShapingAPI});
    ...
    return NdrNodeUniquePtr(new SdrShaderNode(
            discoveryResult.identifier, discoveryResult.version, discoveryResult.name, discoveryResult.family,
            SdrNodeContext->Light, discoveryResult.sourceType,
            /*nodeUriAssetPath=*/std::string(),
            /*resolvedImplementationUri=*/std::string(), UsdShadeShaderDefUtils::GetShaderProperties(connectable),
            _GetSdrMetadata(connectable, discoveryResult.metadata), discoveryResult.sourceCode));
}
...
const TfToken& UsdLux_LightDefParserPlugin::GetSourceType() const {
