...
//         search path
static bool _FsHelpersExamineFiles(NdrNodeDiscoveryResultVec* foundNodes,
    ...
    for (const std::string& fileName : dirFileNames) {
        ...
        NdrStringVec::const_iterator extIter = std::find(allowedExtensions.begin(), allowedExtensions.end(), extension);
        ...
        if (extIter != allowedExtensions.end()) {
            ...
            std::string identifierAndType = identifier.GetString() + "-" + extension;
            ...
            if (!foundNodesWithTypes->insert(identifierAndType).second) {
                TF_DEBUG(NDR_DISCOVERY)
                        .Msg("Found a duplicate node with identifier [%s] "
            ...
            const bool parsed = parseIdentifierFn
                                        ...
                                        : NdrFsHelpersSplitShaderIdentifier(identifier, &family, &name, &version);
            if (!parsed) {
...
bool NdrFsHelpersSplitShaderIdentifier(const TfToken& identifier, TfToken* family, TfToken* name, NdrVersion* version) {
    const std::vector<std::string> tokens = TfStringTokenize(identifier.GetString(), "_");
    ...
    *family = TfToken(tokens[0]);
    ...
    if (penultimateTokenIsNumber) {
        ...
        *version = NdrVersion(std::stoi(*(tokens.end() - 2)), std::stoi(*(tokens.end() - 1)));
...
static void _WalkDirs(const NdrStringVec& searchPaths, const TfWalkFunction& fn, bool followSymlinks) {
    for (const std::string& searchPath : searchPaths) {
        if (!TfIsDir(searchPath)) {
            continue;
        }

        TfWalkDirs(searchPath, fn,
                   /* topDown = */ true, TfWalkIgnoreErrorHandler, followSymlinks);
    }
}
...
NdrNodeDiscoveryResultVec NdrFsHelpersDiscoverNodes(const NdrStringVec& searchPaths,
    ...
    auto discoverNodesFn = [&](const std::string& dirPath, NdrStringVec* unused, const NdrStringVec& dirFileNames) {
        return _FsHelpersExamineFiles(&foundNodes, &foundNodesWithTypes, allowedExtensions, context, dirPath,
                                      dirFileNames, parseIdentifierFn);
    };
...
NdrDiscoveryUriVec NdrFsHelpersDiscoverFiles(const NdrStringVec& searchPaths,
    ...
    auto findUrisFn = [&](const std::string& dirPath, NdrStringVec* unused, const NdrStringVec& dirFileNames) {
        for (const std::string& fileName : dirFileNames) {
            const std::string extension = TfStringToLowerAscii(TfGetExtension(fileName));
            ...
            if (std::find(allowedExtensions.begin(), allowedExtensions.end(), extension) != allowedExtensions.end()) {
                ...
                discoveryUri.resolvedUri = ArGetResolver().Resolve(discoveryUri.uri);
