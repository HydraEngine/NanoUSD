...
std::string NdrProperty::GetInfoString() const {
    return TfStringPrintf("%s (type: '%s'); %s", _name.GetText(), _type.GetText(), _isOutput ? "output" : "input");
}
...
NdrSdfTypeIndicator NdrProperty::GetTypeAsSdfType() const {
    return NdrSdfTypeIndicator(SdfValueTypeNames->Token, _type, false);
}
