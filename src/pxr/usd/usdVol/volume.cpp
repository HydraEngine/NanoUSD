...
TF_REGISTRY_FUNCTION(TfType) {
    TfType::Define<UsdVolVolume, TfType::Bases<UsdGeomGprim>>();
...
UsdVolVolume UsdVolVolume::Define(const UsdStagePtr& stage, const SdfPath& path) {
    ...
    return UsdVolVolume(stage->DefinePrim(path, usdPrimTypeName));
}
...
/*static*/
const TfTokenVector& UsdVolVolume::GetSchemaAttributeNames(bool includeInherited) {
    static TfTokenVector localNames;
    static TfTokenVector allNames = UsdGeomGprim::GetSchemaAttributeNames(true);
...
TF_DEFINE_PRIVATE_TOKENS(_tokens, ((fieldPrefix, "field:")));

TfToken UsdVolVolume::_MakeNamespaced(const TfToken& name) {
    TfToken result;
...
UsdVolVolume::FieldMap UsdVolVolume::GetFieldPaths() const {
...
bool UsdVolVolume::HasFieldRelationship(const TfToken& name) const {
    return GetPrim().HasRelationship(_MakeNamespaced(name));
}

SdfPath UsdVolVolume::GetFieldPath(const TfToken& name) const {
...
bool UsdVolVolume::BlockFieldRelationship(const TfToken& name) const {
    UsdRelationship fieldRel = GetPrim().GetRelationship(_MakeNamespaced(name));
