...
bool CreateExampleSRT(UsdGeomXformable const& gprim) {
    ...
    return (t.Set(GfVec3d(0, 100, 0), UsdTimeCode::Default()) && r.Set(GfVec3f(30, 60, 90), UsdTimeCode::Default()) &&
...
bool CreateSRTWithDefaults(UsdGeomXformable const& gprim,
    ...
    if (UsdGeomXformCommonAPI xform = UsdGeomXformCommonAPI(gprim)) {
        return xform.SetXformVectors(defTranslate, defRotateXYZ, defScale, defPivot,
...
bool CreateAnimatedTransform(UsdGeomXformable const& gprim,
    ...
    UsdGeomXformOp pivotInv = gprim.AddTranslateOp(UsdGeomXformOp::PrecisionFloat, pivSuffix,
                                                   /* isInverseOp = */ true);
