...
inline void _Transform(GfBBox3d* const bbox, const _IdentityTransform& m) {}

inline void _Transform(GfBBox3d* const bbox, const GfMatrix4d& m) {
    bbox->Transform(m);
}
...
public:
    ...
    _BBoxTask(const _PrimContext& primContext,
        ...
        : _primContext(primContext), _inverseComponentCtm(inverseComponentCtm), _owner(owner), _xfCaches(xfCaches) {}
    explicit operator bool() const { return _owner; }
    void operator()() const {
...
private:
    ...
    struct _PrototypeTask {
        ...
        _PrototypeTask(_PrototypeTask&& other) : dependentPrototypes(std::move(other.dependentPrototypes)) {
            numDependencies.store(other.numDependencies.load());
        }
    ...
    typedef TfHashMap<_PrimContext, _PrototypeTask, _PrimContextHash> _PrototypeTaskMap;

public:
    _PrototypeBBoxResolver(UsdGeomBBoxCache* bboxCache) : _owner(bboxCache) {}
    ...
    void Resolve(const std::vector<_PrimContext>& prototypePrimContexts) {
        ...
        for (const auto& t : prototypeTasks) {
            if (t.second.numDependencies == 0) {
                _owner->_dispatcher.Run(&_PrototypeBBoxResolver::_ExecuteTaskForPrototype, this, t.first,
                                        &prototypeTasks, &xfCache, &_owner->_dispatcher);
            }
        }
...
private:
    void _PopulateTasksForPrototype(const _PrimContext& prototypePrim, _PrototypeTaskMap* prototypeTasks) {
        std::pair<_PrototypeTaskMap::iterator, bool> prototypeTaskStatus =
                prototypeTasks->insert(std::make_pair(prototypePrim, _PrototypeTask()));
        ...
        for (const auto& reqPrototype : requiredPrototypes) {
            ...
            (*prototypeTasks)[reqPrototype].dependentPrototypes.push_back(prototypePrim);
        }
    }
    ...
    void _ExecuteTaskForPrototype(const _PrimContext& prototype,
        ...
        for (const auto& dependentPrototype : prototypeData.dependentPrototypes) {
            ...
            if (dependentPrototypeData.numDependencies.fetch_sub(1) == 1) {
                dispatcher->Run(&_PrototypeBBoxResolver::_ExecuteTaskForPrototype, this, dependentPrototype,
                                prototypeTasks, xfCaches, dispatcher);
            }
        }
    }
};
...
static const UsdAttributeQuery& _GetOrCreateExtentsHintQuery(UsdGeomModelAPI& geomModel, UsdAttributeQuery* q) {
    if (!*q) {
...
UsdGeomBBoxCache::UsdGeomBBoxCache(UsdTimeCode time,
...
GfBBox3d UsdGeomBBoxCache::ComputeWorldBound(const UsdPrim& prim) {
    ...
    if (!_Resolve(prim, &bboxes)) return bbox;
...
GfBBox3d UsdGeomBBoxCache::ComputeLocalBound(const UsdPrim& prim) {
    GfBBox3d bbox;
...
GfBBox3d UsdGeomBBoxCache::ComputeUntransformedBound(const UsdPrim& prim) {
    GfBBox3d empty;
    ...
    _PurposeToBBoxMap bboxes;
    if (!_Resolve(prim, &bboxes)) return empty;

    return _GetCombinedBBoxForIncludedPurposes(bboxes);
}

template <typename TransformType>
GfBBox3d UsdGeomBBoxCache::_ComputeBoundWithOverridesHelper(
        const UsdPrim& prim,
    ...
    for (auto it = range.begin(); it != range.end(); ++it) {
        ...
        // included.
        if (ancestorsOfPathsToSkip.find(primPath) != ancestorsOfPathsToSkip.end()) {
            continue;
        }
        ...
        if (!foundAncestorWithOverride) {
            ...
            bbox = ComputeRelativeBound(p, prim.GetStage()->GetPrimAtPath(overrideIter->first));
...
GfBBox3d UsdGeomBBoxCache::ComputeUntransformedBound(
        ...
        const SdfPathSet& pathsToSkip,
        const TfHashMap<SdfPath, GfMatrix4d, SdfPath::Hash>& ctmOverrides) {
    return _ComputeBoundWithOverridesHelper(prim, pathsToSkip, _IdentityTransform(), ctmOverrides);
}

GfBBox3d UsdGeomBBoxCache::ComputeWorldBoundWithOverrides(
        const UsdPrim& prim,
        const SdfPathSet& pathsToSkip,
        const GfMatrix4d& primOverride,
        const TfHashMap<SdfPath, GfMatrix4d, SdfPath::Hash>& ctmOverrides) {
    return _ComputeBoundWithOverridesHelper(prim, pathsToSkip, primOverride, ctmOverrides);
}
...
bool UsdGeomBBoxCache::_ComputePointInstanceBoundsHelper(const UsdGeomPointInstancer& instancer,
    ...
    if (!instancer.GetProtoIndicesAttr().Get(&protoIndices, time)) {
        TF_WARN("%s -- no prototype indices", instancer.GetPrim().GetPath().GetText());
        return false;
    }
    ...
    for (auto protoIndex : cprotoIndices) {
        if (protoIndex < 0 || static_cast<size_t>(protoIndex) >= protoPaths.size()) {
            TF_WARN("%s -- invalid prototype index: %d. Should be in [0, %zu)", instancer.GetPrim().GetPath().GetText(),
    ...
    if (!instancer.ComputeInstanceTransformsAtTime(&instanceTransforms, time, baseTime,
...
bool UsdGeomBBoxCache::ComputePointInstanceWorldBounds(UsdGeomPointInstancer const& instancer,
                                                       ...
                                                       size_t numIds,
                                                       GfBBox3d* result) {
    return _ComputePointInstanceBoundsHelper(instancer, instanceIdBegin, numIds,
...
void UsdGeomBBoxCache::SetIncludedPurposes(const TfTokenVector& includedPurposes) {
    _includedPurposes = includedPurposes;
}

GfBBox3d UsdGeomBBoxCache::_GetCombinedBBoxForIncludedPurposes(const _PurposeToBBoxMap& bboxes) {
    ...
    for (const TfToken& purpose : _includedPurposes) {
        ...
        if (it != bboxes.end()) {
            ...
            if (!bboxForPurpose.GetRange().IsEmpty()) combinedBound = GfBBox3d::Combine(combinedBound, bboxForPurpose);
        }
    }
    return combinedBound;
}

void UsdGeomBBoxCache::SetTime(UsdTimeCode time) {
    ...
    if (_time == UsdTimeCode::Default() || time == UsdTimeCode::Default()) clearUnvarying = true;
    TF_DEBUG(USDGEOM_BBOX)
            .Msg("[BBox Cache] Setting time: %f "
                 " clearUnvarying: %s\n",
                 time.GetValue(), clearUnvarying ? "true" : "false");
...
bool UsdGeomBBoxCache::_ShouldIncludePrim(const UsdPrim& prim) {
    TRACE_FUNCTION();
    ...
    if (!prim.IsA<UsdGeomImageable>()) {
        TF_DEBUG(USDGEOM_BBOX)
                .Msg("[BBox Cache] excluded, not IMAGEABLE type. "
                     "prim: %s, primType: %s\n",
    ...
    if (!_ignoreVisibility) {
        ...
        if (img.GetVisibilityAttr().Get(&vis, _time) && vis == UsdGeomTokens->invisible) {
            TF_DEBUG(USDGEOM_BBOX)
                    .Msg("[BBox Cache] excluded for VISIBILITY. "
                         "prim: %s visibility at time %s: %s\n",
                         prim.GetPath().GetText(), TfStringify(_time).c_str(), vis.GetText());
            return false;
        }
    }
...
static bool _IsVaryingImpl(const UsdTimeCode time, const AttributeOrQuery& attr) {
    ...
    if (attr.GetBracketingTimeSamples(queryTime, &lower, &upper, &hasSamples) && hasSamples) {
        ...
        attr.GetBracketingTimeSamples(lower + .000001, &lower, &upper, &hasSamples);
        if (lower != upper) {
            return true;
        }
...
bool UsdGeomBBoxCache::_IsVarying(const UsdAttribute& attr) {
    return _IsVaryingImpl(_time, attr);
}

bool UsdGeomBBoxCache::_IsVarying(const UsdAttributeQuery& query) {
    return _IsVaryingImpl(_time, query);
}

// Returns true if the given prim is a component or a subcomponent.
static bool _IsComponentOrSubComponent(const UsdPrim& prim) {
    UsdModelAPI model(prim);
    TfToken kind;
    if (!model.GetKind(&kind)) return false;
...
// a subcomponent.
//
static UsdPrim _GetNearestComponent(const UsdPrim& prim) {
    UsdPrim modelPrim = prim;
    while (modelPrim) {
        if (_IsComponentOrSubComponent(modelPrim)) return modelPrim;
...
void UsdGeomBBoxCache::_ComputePurposeInfo(_Entry* entry, const _PrimContext& primContext) {
    ...
    if (prim.IsPrototype()) {
        ...
        if (parentPrim && parentPrim.GetPath() != SdfPath::AbsoluteRootPath()) {
            ...
            _PrimContext parentPrimContext(parentPrim, primContext.instanceInheritablePurpose);
            ...
            if (parentEntry) {
                if (IsRecursive) {
                    ...
                    TF_DEBUG(USDGEOM_BBOX)
                            .Msg("[BBox Cache] Computing purpose for <%s> before purpose"
                                 "of parent <%s> is cached\n",
                                 primContext.ToString().c_str(), parentPrimContext.ToString().c_str());
                }
            }
        }
        TF_DEBUG(USDGEOM_BBOX)
                .Msg("[BBox Cache] Computing purpose without "
                     "cached parent for <%s>\n",
...
bool UsdGeomBBoxCache::_UseExtentsHintForPrim(UsdPrim const& prim) const {
    return _useExtentsHint && prim.IsModel() && prim.GetPath() != SdfPath::AbsoluteRootPath();
}

bool UsdGeomBBoxCache::_ShouldPruneChildren(const UsdPrim& prim, UsdGeomBBoxCache::_Entry* entry) {
    ...
    return (extentsHintAttr && extentsHintAttr.Get(&extentsHint, _time) && extentsHint.size() >= 2);
}

UsdGeomBBoxCache::_Entry* UsdGeomBBoxCache::_FindOrCreateEntriesForPrim(
        const _PrimContext& primContext, std::vector<_PrimContext>* prototypePrimContexts) {
...
bool UsdGeomBBoxCache::_Resolve(const UsdPrim& prim, UsdGeomBBoxCache::_PurposeToBBoxMap* bboxes) {
    TRACE_FUNCTION();
    ...
    _Entry* entry = _FindOrCreateEntriesForPrim(primContext, &prototypePrimContexts);
    if (entry && entry->isComplete) {
        *bboxes = entry->bboxes;
        return (!bboxes->empty());
    }
...
bool UsdGeomBBoxCache::_GetBBoxFromExtentsHint(const UsdGeomModelAPI& geomModel,
    ...
    if (!extentsHintQuery || !extentsHintQuery.Get(&extents, _time)) {
        if (TfDebug::IsEnabled(USDGEOM_BBOX) && !geomModel.GetPrim().IsLoaded()) {
            TF_DEBUG(USDGEOM_BBOX)
                    .Msg("[BBox Cache] MISSING extentsHint for "
                         "UNLOADED model %s.\n",
    ...
    TF_DEBUG(USDGEOM_BBOX)
            .Msg("[BBox Cache] Found cached extentsHint for "
                 "model %s.\n",
                 geomModel.GetPrim().GetPath().GetString().c_str());
    ...
    for (size_t i = 0; i < purposeTokens.size(); ++i) {
        ...
        // Hence, we can break.
        if ((idx + 2) > extents.size()) break;

        (*bboxes)[purposeTokens[i]] = GfBBox3d(GfRange3d(extents[idx], extents[idx + 1]));
    }
...
void UsdGeomBBoxCache::_ResolvePrim(const _BBoxTask* task,
    ...
    if (!TF_VERIFY(entry != NULL)) return;

    _PurposeToBBoxMap* bboxes = &entry->bboxes;
    if (entry->isComplete) {
        TF_DEBUG(USDGEOM_BBOX)
                .Msg("[BBox Cache] Dependent cache hit: "
                     "%s %s\n",
                     primContext.ToString().c_str(), TfStringify(_GetCombinedBBoxForIncludedPurposes(*bboxes)).c_str());
        return;
    }
    ...
    if (!queries) {
        ...
        // corresponding query.
        const size_t numQueries = (useExtentsHintForPrim ? NumQueries : NumQueries - 1);
        queries.reset(new UsdAttributeQuery[numQueries]);
    }

    if (useExtentsHintForPrim) {
        UsdGeomModelAPI geomModel(prim);
        const UsdAttributeQuery& extentsHintQuery = _GetOrCreateExtentsHintQuery(geomModel, &queries[ExtentsHint]);
    ...
    if (UsdGeomBoundable boundableObj = UsdGeomBoundable(prim)) {
        ...
        if (primIsInstance) {
            ...
            // future.
            childInheritableInstancePurpose = primContext.instanceInheritablePurpose;
        }

        for (const UsdPrim& childPrim : children) {
            _PrimContext childPrimContext(childPrim, childInheritableInstancePurpose);
            ...
            if (!TF_VERIFY(childEntry,
                           "Could not find prim <%s>"
                           "in the bboxCache.",
                           childPrimContext.ToString().c_str())) {
                continue;
            }

            // If we're about to process the child for the first time, we must
            // populate isIncluded.
            if (!childEntry->isComplete) childEntry->isIncluded = _ShouldIncludePrim(childPrim);
            ...
            if (!_ignoreVisibility && !childEntry->isIncluded) {
                ...
                if (img) entry->isVarying |= _IsVarying(img.GetVisibilityAttr());
                continue;
            }
            ...
            if (primIsInstance) {
                ...
                included.emplace_back(childPrimContext, _BBoxTask(childPrimContext, inverseEnclosingComponentCtm, this,
                                                                  task->GetXformCaches()));
            }
        }
        ...
        for (const auto& childAndTask : included) {
            ...
            if (!TF_VERIFY(childEntry->isComplete)) continue;
            ...
            if (!childEntry->bboxes.empty()) {
                ...
                if (primIsInstance) {
                    ...
                    childLocalToComponentXform =
                            xfCache.GetLocalTransformation(childPrim, &resetsXf) * localToComponentXform;
                } else {
                    childLocalToComponentXform =
                            xfCache.GetLocalToWorldTransform(childPrim) * inverseEnclosingComponentCtm;
                }
                ...
                for (auto& purposeAndBBox : childBBoxes) {
                    ...
                    bbox.SetRange(GfRange3d(bbox.GetRange()).UnionWith(childBBox.ComputeAlignedRange()));
                }
            }
        }
    ...
    entry->isComplete = true;
    TF_DEBUG(USDGEOM_BBOX)
            .Msg("[BBox Cache] resolved value: %s %s "
                 "(varying: %s)\n",
                 primContext.ToString().c_str(), TfStringify(_GetCombinedBBoxForIncludedPurposes(*bboxes)).c_str(),
