...
TF_DEFINE_PRIVATE_TOKENS(_tokens, ((primvarsPrefix, "primvars:"))((idFrom, ":idFrom"))((indicesSuffix, ":indices")));

UsdGeomPrimvar::UsdGeomPrimvar(const UsdAttribute& attr) : _attr(attr), _idTargetStatus(IdTargetUninitialized) {}
...
bool UsdGeomPrimvar::IsValidPrimvarName(const TfToken& name) {
    ...
    return (TfStringStartsWith(name, _tokens->primvarsPrefix) && !TfStringEndsWith(name, _tokens->indicesSuffix));
}
...
bool UsdGeomPrimvar::SetInterpolation(const TfToken& interpolation) {
    if (!IsValidInterpolation(interpolation)) {
        TF_CODING_ERROR(
                ...
                "\"%s\" for attribute %s",
                interpolation.GetText(), _attr.GetPath().GetString().c_str());
        return false;
    }
...
bool UsdGeomPrimvar::SetElementSize(int eltSize) {
    if (eltSize < 1) {
        TF_CODING_ERROR(
                ...
                eltSize, _attr.GetPath().GetString().c_str());
        return false;
    }
...
UsdAttribute UsdGeomPrimvar::_GetIndicesAttr(bool create) const {
    ...
    if (create) {
        return _attr.GetPrim().CreateAttribute(indicesAttrName, SdfValueTypeNames->IntArray, /*custom*/ false,
...
bool UsdGeomPrimvar::GetIndices(VtIntArray* indices, UsdTimeCode time) const {
    ...
    if (indicesAttr) return indicesAttr.Get(indices, time);
...
bool UsdGeomPrimvar::_ComputeFlattenedArray(
        const VtValue& attrVal, const VtIntArray& indices, int elementSize, VtValue* value, std::string* errorString) {
    if (!attrVal.IsHolding<ArrayType>()) return false;

    ArrayType result;
    if (_ComputeFlattenedHelper(attrVal.UncheckedGet<ArrayType>(), indices, elementSize, &result, errorString)) {
...
bool UsdGeomPrimvar::ComputeFlattened(
        VtValue* value, const VtValue& attrVal, const VtIntArray& indices, int elementSize, std::string* errStr) {
    ...
    bool foundSupportedType = _ComputeFlattenedArray<VtVec2fArray>(attrVal, indices, elementSize, value, errStr) ||
    ...
    if (!foundSupportedType && errStr) {
        std::string thisErr =
                TfStringPrintf("Unsupported indexed primvar value type %s.", attrVal.GetTypeName().c_str());
...
UsdGeomPrimvar::UsdGeomPrimvar(const UsdPrim& prim, const TfToken& primvarName, const SdfValueTypeName& typeName)
    : _idTargetStatus(IdTargetUninitialized) {
    TF_VERIFY(prim);
