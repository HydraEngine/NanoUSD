...
public:
    ...
    USDGEOM_API
    static const SdfValueTypeName& GetValueTypeName(const Type opType, const Precision precision);
    ...
    Type GetOpType() const { return _opType; }
    ...
    bool IsInverseOp() const { return _isInverseOp; }
    ...
    bool GetAs(T* value, UsdTimeCode time) const {
        ...
        if (v.IsEmpty()) {
            ...
            TF_CODING_ERROR(
                    ...
                    "requested type %s.",
                    GetAttr().GetPath().GetText(), thisType.GetTypeName().c_str(),
    ...
    bool MightBeTimeVarying() const { return std::visit(_GetMightBeTimeVarying(), _attr); }
    ...
    UsdAttribute const& GetAttr() const { return std::visit(_GetAttr(), _attr); }
...
public:
    ...
    friend bool operator!=(const UsdGeomXformOp& lhs, const UsdGeomXformOp& rhs) { return !(lhs == rhs); }
    ...
    /// is authored.
    bool GetTimeSamples(std::vector<double>* times) const { return std::visit(_GetTimeSamples(times), _attr); }
    ...
    size_t GetNumTimeSamples() const { return std::visit(_GetNumTimeSamples(), _attr); }
...
public:
    ...
    USDGEOM_API
    UsdGeomXformOp(const UsdAttribute& attr, bool isInverseOp, _ValidAttributeTagType);
    USDGEOM_API
...
private:
    ...
    // this xformOp.
    void _CreateAttributeQuery() const { _attr = UsdAttributeQuery(GetAttr()); }
    ...
    UsdGeomXformOp(UsdPrim const& prim,
                   Type const opType,
    ...
    struct _Get {
        ...
        bool operator()(const UsdAttribute& attr) const { return attr.Get(value, time); }

        bool operator()(const UsdAttributeQuery& attrQuery) const { return attrQuery.Get(value, time); }
    ...
    struct _GetAttr {
        ...
        const UsdAttribute& operator()(const UsdAttribute& attr) const { return attr; }

        const UsdAttribute& operator()(const UsdAttributeQuery& attrQuery) const { return attrQuery.GetAttribute(); }
    };
    ...
    struct _GetTimeSamples {
        ...
        bool operator()(const UsdAttribute& attr) const { return attr.GetTimeSamples(times); }

        bool operator()(const UsdAttributeQuery& attrQuery) const { return attrQuery.GetTimeSamples(times); }
    ...
    struct _GetTimeSamplesInInterval {
        ...
        bool operator()(const UsdAttribute& attr) const { return attr.GetTimeSamplesInInterval(interval, times); }
    ...
    struct _GetNumTimeSamples {
        ...
        size_t operator()(const UsdAttribute& attr) const { return attr.GetNumTimeSamples(); }

        size_t operator()(const UsdAttributeQuery& attrQuery) const { return attrQuery.GetNumTimeSamples(); }
    };
    ...
    struct _GetMightBeTimeVarying {
        ...
        bool operator()(const UsdAttribute& attr) const { return attr.ValueMightBeTimeVarying(); }

        bool operator()(const UsdAttributeQuery& attrQuery) const { return attrQuery.ValueMightBeTimeVarying(); }
    };
};
