...
/// <A HREF="https://renderman.pixar.com/resources/RenderMan_20/appnote.22.html#classSpecifiers">interpolation rules for
...
public:
    ...
    ///
    USDGEOM_API
    static UsdGeomPointInstancer Get(const UsdStagePtr& stage, const SdfPath& path);
...
public:
    ...
    USDGEOM_API
    std::vector<bool> ComputeMaskAtTime(UsdTimeCode time, VtInt64Array const* ids = nullptr) const;
    ...
    USDGEOM_API
    bool ComputeInstanceTransformsAtTime(VtArray<GfMatrix4d>* xforms,
...
private:
    ...
    bool _GetPrototypePathsForInstanceTransforms(const VtIntArray& protoIndices, SdfPathVector* protoPaths) const;

    // Get the authored prototype indices for instance transform computation.
    // Fail if prototype indices are not authored.
    bool _GetProtoIndicesForInstanceTransforms(UsdTimeCode baseTime, VtIntArray* protoIndices) const;
    ...
    bool _ComputePointInstancerAttributesPreamble(const UsdTimeCode baseTime,
...
public:
    ...
    ///                sample.
    USDGEOM_API
    bool ComputeExtentAtTimes(std::vector<VtVec3fArray>* extents,
                              const std::vector<UsdTimeCode>& times,
...
private:
    ...
    bool _ComputeExtentFromTransforms(VtVec3fArray* extent,
                                      const VtIntArray& protoIndices,
                                      const std::vector<bool>& mask,
    ...
    bool _ComputeExtentAtTimes(std::vector<VtVec3fArray>* extent,
                               const std::vector<UsdTimeCode>& times,
                               const UsdTimeCode baseTime,
                               const GfMatrix4d* transform) const;
};
...
template <class T>
bool UsdGeomPointInstancer::ApplyMaskToArray(std::vector<bool> const& mask,
                                             VtArray<T>* dataArray,
