...
TF_REGISTRY_FUNCTION(TfType) {
    TfType::Define<UsdGeomXformable, TfType::Bases<UsdGeomImageable>>();
}
...
UsdAttribute UsdGeomXformable::CreateXformOpOrderAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->xformOpOrder, SdfValueTypeNames->TokenArray,
...
const TfTokenVector& UsdGeomXformable::GetSchemaAttributeNames(bool includeInherited) {
    ...
    static TfTokenVector allNames =
            _ConcatenateAttributeNames(UsdGeomImageable::GetSchemaAttributeNames(true), localNames);
...
bool UsdGeomXformable::_GetXformOpOrderValue(VtTokenArray* xformOpOrder) const {
    UsdAttribute xformOpOrderAttr = GetXformOpOrderAttr();
    if (!xformOpOrderAttr) return false;

    xformOpOrderAttr.Get(xformOpOrder, UsdTimeCode::Default());
    return true;
}

UsdGeomXformOp UsdGeomXformable::AddXformOp(UsdGeomXformOp::Type const opType,
    ...
    if (it != xformOpOrder.end()) {
        TF_CODING_ERROR("The xformOp '%s' already exists in xformOpOrder [%s].", opName.GetText(),
    ...
    if (UsdAttribute xformOpAttr = GetPrim().GetAttribute(xformOpAttrName)) {
        ...
        UsdGeomXformOp::Precision existingPrecision =
                UsdGeomXformOp::GetPrecisionFromValueTypeName(xformOpAttr.GetTypeName());
        ...
        if (existingPrecision != precision) {
            TF_CODING_ERROR(
                    ...
                    xformOpAttr.GetPath().GetText(), xformOpAttr.GetTypeName().GetAsToken().GetText(),
    ...
    if (result) {
        ...
        TF_CODING_ERROR(
                ...
                TfEnum::GetName(opType).c_str(), TfEnum::GetName(precision).c_str(), GetPath().GetText(),
                opSuffix.GetText(), isInverseOp);
        return UsdGeomXformOp();
    }
...
UsdGeomXformOp UsdGeomXformable::GetXformOp(UsdGeomXformOp::Type const opType,
                                            TfToken const& opSuffix,
                                            bool isInverseOp) const {
    VtTokenArray xformOpOrder;
    _GetXformOpOrderValue(&xformOpOrder);
...
UsdGeomXformOp UsdGeomXformable::AddTranslateOp(UsdGeomXformOp::Precision const precision,
                                                TfToken const& opSuffix,
                                                bool isInverseOp) const {
...
static bool _XformOpOrderHasResetXformStack(const VtTokenArray& opOrderVec) {
    return std::find(opOrderVec.begin(), opOrderVec.end(), UsdGeomXformOpTypes->resetXformStack) != opOrderVec.end();
}

bool UsdGeomXformable::SetResetXformStack(bool resetXformStack) const {
    ...
    if (resetXformStack) {
        ...
        if (_XformOpOrderHasResetXformStack(opOrderVec)) return true;

        VtTokenArray newOpOrderVec(opOrderVec.size() + 1);

        newOpOrderVec[0] = UsdGeomXformOpTypes->resetXformStack;
        for (size_t i = 0; i < opOrderVec.size(); i++) newOpOrderVec[i + 1] = opOrderVec[i];
...
bool UsdGeomXformable::GetResetXformStack() const {
    VtTokenArray opOrderVec;
    if (!_GetXformOpOrderValue(&opOrderVec)) return false;

    return _XformOpOrderHasResetXformStack(opOrderVec);
}

bool UsdGeomXformable::SetXformOpOrder(vector<UsdGeomXformOp> const& orderedXformOps, bool resetXformStack) const {
    VtTokenArray ops;
    ops.reserve(orderedXformOps.size() + (resetXformStack ? 1 : 0));

    if (resetXformStack) ops.push_back(UsdGeomXformOpTypes->resetXformStack);
    ...
    TF_FOR_ALL(it, orderedXformOps) {
        ...
        if (it->GetAttr().GetPrim() == GetPrim()) {
            ...
            TF_CODING_ERROR(
                    ...
                    "prim <%s>.",
                    it->GetAttr().GetPath().GetText(), GetPath().GetText());
            return false;
        }
    }
...
bool UsdGeomXformable::ClearXformOpOrder() const {
...
vector<UsdGeomXformOp> UsdGeomXformable::GetOrderedXformOps(bool* resetXformStack) const {
    return _GetOrderedXformOps(resetXformStack, /*withAttributeQueries=*/false);
}

vector<UsdGeomXformOp> UsdGeomXformable::_GetOrderedXformOps(bool* resetsXformStack, bool withAttributeQueries) const {
...
bool UsdGeomXformable::XformQuery::GetLocalTransformation(GfMatrix4d* transform, const UsdTimeCode time) const {
...
static bool _TransformMightBeTimeVarying(vector<UsdGeomXformOp> const& xformOps) {
    ...
    TF_FOR_ALL(it, xformOps) {
        if (it->MightBeTimeVarying()) return true;
    }

    return false;
}

bool UsdGeomXformable::XformQuery::TransformMightBeTimeVarying() const {
...
bool UsdGeomXformable::TransformMightBeTimeVarying(const vector<UsdGeomXformOp>& ops) const {
    if (!ops.empty()) return _TransformMightBeTimeVarying(ops);
...
/* static */
bool UsdGeomXformable::GetTimeSamplesInInterval(std::vector<UsdGeomXformOp> const& orderedXformOps,
...
bool UsdGeomXformable::XformQuery::GetTimeSamples(vector<double>* times) const {
    return GetTimeSamplesInInterval(GfInterval::GetFullInterval(), times);
}

bool UsdGeomXformable::XformQuery::GetTimeSamplesInInterval(const GfInterval& interval, vector<double>* times) const {
    if (_xformOps.size() == 1) {
    ...
    for (auto& xformOp : _xformOps) {
        ...
        const UsdAttributeQuery& attrQuery = std::get<UsdAttributeQuery>(xformOp._attr);
        xformOpAttrQueries.push_back(attrQuery);
    }

    return UsdAttributeQuery::GetUnionedTimeSamplesInInterval(xformOpAttrQueries, interval, times);
}
...
static bool _AreInverseXformOps(const UsdGeomXformOp& a, const UsdGeomXformOp& b) {
...
bool UsdGeomXformable::GetLocalTransformation(GfMatrix4d* transform,
    ...
    for (vector<UsdGeomXformOp>::const_reverse_iterator it = orderedXformOps.rbegin(); it != orderedXformOps.rend();
         ++it) {
...
/* static */
bool UsdGeomXformable::IsTransformationAffectedByAttrNamed(const TfToken& attrName) {
    return attrName == UsdGeomTokens->xformOpOrder || UsdGeomXformOp::IsXformOp(attrName);
}
