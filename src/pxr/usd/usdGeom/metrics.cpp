...
bool UsdGeomSetStageUpAxis(const UsdStageWeakPtr& stage, const TfToken& axis) {
    ...
    if (axis != UsdGeomTokens->y && axis != UsdGeomTokens->z) {
        TF_CODING_ERROR(
                ...
                axis.GetText(), stage->GetRootLayer()->GetIdentifier().c_str());
        return false;
    }
    ...
    TF_FOR_ALL(plugIter, plugs) {
        ...
        if (TfMapLookup(metadata, _tokens->UsdGeomMetrics, &metricsDictValue)) {
            if (!metricsDictValue.Is<JsObject>()) {
                TF_CODING_ERROR("%s[%s] was not a dictionary in plugInfo.json file.", plug->GetName().c_str(),
            ...
            if (TfMapLookup(metricsDict, UsdGeomTokens->upAxis, &upAxisValue)) {
                if (!upAxisValue.Is<std::string>()) {
                    TF_CODING_ERROR("%s[%s][%s] was not a string.", plug->GetName().c_str(),
                                    _tokens->UsdGeomMetrics.GetText(), UsdGeomTokens->upAxis.GetText());
                    continue;
                }
                ...
                if (axisStr == "Y") {
                    ...
                    TF_CODING_ERROR(
                            ...
                            " \"Z\" are allowed.",
                            plug->GetName().c_str(), _tokens->UsdGeomMetrics.GetText(), UsdGeomTokens->upAxis.GetText(),
                            axisStr.c_str());
                    continue;
                }
                ...
                if (!upAxis.IsEmpty() && upAxis != axisToken) {
                    TF_CODING_ERROR(
                            ...
                            " \"%s\"",
                            definingPluginName.c_str(), plug->GetName().c_str(), UsdGeomTokens->upAxis.GetText(),
