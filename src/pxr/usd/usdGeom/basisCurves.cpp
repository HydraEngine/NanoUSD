...
TF_REGISTRY_FUNCTION(TfType) {
    TfType::Define<UsdGeomBasisCurves, TfType::Bases<UsdGeomCurves>>();
...
UsdGeomBasisCurves UsdGeomBasisCurves::Define(const UsdStagePtr& stage, const SdfPath& path) {
    ...
    return UsdGeomBasisCurves(stage->DefinePrim(path, usdPrimTypeName));
}
...
UsdAttribute UsdGeomBasisCurves::CreateTypeAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->type, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, defaultValue, writeSparsely);
}

UsdAttribute UsdGeomBasisCurves::GetBasisAttr() const {
...
UsdAttribute UsdGeomBasisCurves::CreateBasisAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->basis, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, defaultValue, writeSparsely);
}

UsdAttribute UsdGeomBasisCurves::GetWrapAttr() const {
...
UsdAttribute UsdGeomBasisCurves::CreateWrapAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->wrap, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, defaultValue, writeSparsely);
}
...
static size_t _ComputeVaryingDataSize(const UsdGeomBasisCurves& basisCurves,
    ...
    if (curvesType == UsdGeomTokens->linear) {
        if (wrap == UsdGeomTokens->nonperiodic || wrap == UsdGeomTokens->pinned) {
...
static size_t _ComputeVertexDataSize(const VtIntArray& curveVertexCounts) {
    size_t result = 0;
...
TfToken UsdGeomBasisCurves::ComputeInterpolationForSize(size_t n,
...
size_t UsdGeomBasisCurves::ComputeUniformDataSize(const UsdTimeCode& timeCode) const {
    VtIntArray curveVertexCounts;
    GetCurveVertexCountsAttr().Get(&curveVertexCounts, timeCode);
    return curveVertexCounts.size();
}

size_t UsdGeomBasisCurves::ComputeVaryingDataSize(const UsdTimeCode& timeCode) const {
    VtIntArray curveVertexCounts;
    GetCurveVertexCountsAttr().Get(&curveVertexCounts, timeCode);
    return _ComputeVaryingDataSize(*this, curveVertexCounts, timeCode);
}

size_t UsdGeomBasisCurves::ComputeVertexDataSize(const UsdTimeCode& timeCode) const {
    VtIntArray curveVertexCounts;
    GetCurveVertexCountsAttr().Get(&curveVertexCounts, timeCode);
    return _ComputeVertexDataSize(curveVertexCounts);
}

VtIntArray UsdGeomBasisCurves::ComputeSegmentCounts(const UsdTimeCode& timeCode) const {
    VtIntArray curveVertexCounts;
    ...
    if (type == UsdGeomTokens->linear) {
        if (wrap == UsdGeomTokens->periodic) {
            ...
            isValid = true;
        } else if (wrap == UsdGeomTokens->nonperiodic || wrap == UsdGeomTokens->pinned) {
            // Linear and nonperiodic/pinned
            std::transform(curveVertexCounts.cbegin(), curveVertexCounts.cend(), segmentCounts.begin(), [](int n) {
                return n - 1;
            });
            isValid = true;
        }
        ...
        if (basis == UsdGeomTokens->bezier) {
            ...
            if (wrap == UsdGeomTokens->periodic) {
                ...
                std::transform(curveVertexCounts.cbegin(), curveVertexCounts.cend(), segmentCounts.begin(),
                               [vstep](int n) {
                                   return n / vstep;
                               });
                isValid = true;
        ...
        } else if (basis == UsdGeomTokens->bspline || basis == UsdGeomTokens->catmullRom) {
            if (wrap == UsdGeomTokens->periodic) {
