...
UsdAttribute UsdGeomTetMesh::CreateTetVertexIndicesAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->tetVertexIndices, SdfValueTypeNames->Int4Array,
...
UsdAttribute UsdGeomTetMesh::CreateSurfaceFaceVertexIndicesAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->surfaceFaceVertexIndices, SdfValueTypeNames->Int3Array,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}
...
struct _Vec3iHash {
    size_t operator()(const GfVec3i& v) const {
        return static_cast<size_t>(v[0]) << 42 ^ static_cast<size_t>(v[1]) << 21 ^ static_cast<size_t>(v[2]);
    }
};
...
VtVec3iArray _ComputeSurfaceFaces(const VtVec4iArray& tetVertexIndices) {
    ...
    for (size_t t = 0; t < tetVertexIndices.size(); t++) {
        ...
        for (int tFace = 0; tFace < 4; tFace++) {
            ...
            const GfVec3i triangle(tet[tetFaceIndices[tFace][0]], tet[tetFaceIndices[tFace][1]],
...
VtIntArray _FindInvertedElements(const VtVec3fArray& tetMeshPoints,
    ...
    for (size_t t = 0; t < tetVertexIndices.size(); t++) {
        ...
        const float tripleProduct = sign * GfDot(elemPoints[3] - elemPoints[0],
                                                 GfCross(elemPoints[1] - elemPoints[0], elemPoints[2] - elemPoints[0]));
