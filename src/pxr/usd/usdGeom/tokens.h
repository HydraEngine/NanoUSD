...
struct UsdGeomTokensType {
    ...
    /// \brief "catmullClark"
    ///
    /// Fallback value for UsdGeomMesh::GetSubdivisionSchemeAttr(), Fallback value for
    /// UsdGeomMesh::GetTriangleSubdivisionRuleAttr()
    ...
    /// \brief "constant"
    ///
    /// Possible value for UsdGeomPrimvar::SetInterpolation. Default value for UsdGeomPrimvar::GetInterpolation. One
    /// value remains constant over the entire surface primitive.
    ...
    /// \brief "default"
    ///
    /// Fallback value for UsdGeomImageable::GetPurposeAttr(), Possible value for
    /// UsdGeomModelAPI::GetModelDrawModeAttr()
    ...
    /// \brief "elementSize"
    ///
    /// UsdGeomPrimvar - The number of values in the value array that must be aggregated for each element on the
    /// primitive.
    ...
    /// \brief "extent"
    ///
    /// UsdGeomBoundable, UsdGeomCube, UsdGeomSphere, UsdGeomCylinder, UsdGeomCapsule, UsdGeomCone, UsdGeomCylinder_1,
    /// UsdGeomCapsule_1, UsdGeomPlane
    const TfToken extent;
    /// \brief "extentsHint"
    ///
    /// Name of the attribute used to author extents hints at the root of leaf models. Extents hints are stored by
    /// purpose as a vector of GfVec3f values. They are ordered based on the order of purpose tokens returned by
    ...
    /// \brief "faceVarying"
    ///
    /// Possible value for UsdGeomPrimvar::SetInterpolation. For polygons and subdivision surfaces, four values are
    /// interpolated over each face of the mesh. Bilinear interpolation  is used for interpolation between the four
    /// values.
    ...
    /// \brief "hermite"
    ///
    /// A deprecated basis token for UsdGeomBasisCurves. Consumers of USD should transition to using the
    /// UsdGeomHermiteCurves schema.
    ...
    /// \brief "inactiveIds"
    ///
    /// int64listop prim metadata that specifies the PointInstancer ids that should be masked (unrenderable) over all
    /// time.
    ...
    /// \brief "inherited"
    ///
    /// Fallback value for UsdGeomImageable::GetVisibilityAttr(), Possible value for
    /// UsdGeomVisibilityAPI::GetGuideVisibilityAttr(), Fallback value for
    /// UsdGeomVisibilityAPI::GetProxyVisibilityAttr(), Fallback value for
    /// UsdGeomVisibilityAPI::GetRenderVisibilityAttr(), Fallback value for UsdGeomModelAPI::GetModelDrawModeAttr()
    ...
    /// \brief "interpolation"
    ///
    /// UsdGeomPrimvar - How a Primvar interpolates across a primitive; equivalent to RenderMan's \ref
    /// Usd_InterpolationVals "class specifier"
    const TfToken interpolation;
    /// \brief "invisible"
    ///
    /// Possible value for UsdGeomImageable::GetVisibilityAttr(), Fallback value for
    ...
    /// \brief "none"
    ///
    /// Possible value for UsdGeomMesh::GetFaceVaryingLinearInterpolationAttr(), Possible value for
    /// UsdGeomMesh::GetInterpolateBoundaryAttr(), Possible value for UsdGeomMesh::GetSubdivisionSchemeAttr()
    const TfToken none;
    /// \brief "nonOverlapping"
    ///
    /// A type of family of GeomSubsets. It implies that  the elements in the various subsets belonging to the family
    /// are  mutually exclusive, i.e., an element that appears in one  subset may not belong to any other subset in the
    /// family.
    ...
    /// \brief "partition"
    ///
    /// A type of family of GeomSubsets. It implies  that every element appears exacly once in only one of the  subsets
    /// in the family.
    ...
    /// \brief "uniform"
    ///
    /// Possible value for UsdGeomPrimvar::SetInterpolation. One value remains constant for each uv patch segment of the
    /// surface primitive (which is a \em face for meshes).
    const TfToken uniform;
    /// \brief "unrestricted"
    ///
    /// A type of family of GeomSubsets. It implies that there are no restrictions w.r.t. the membership of elements in
    /// the subsets. There could be overlapping members in subsets  belonging to the family and the union of all subsets
    /// in the  family may not contain all the elements.
    ...
    /// \brief "varying"
    ///
    /// Possible value for UsdGeomPrimvar::SetInterpolation. Four values are interpolated over each uv patch segment of
    /// the  surface. Bilinear interpolation is used for interpolation  between the four values.
    ...
    /// \brief "vertex"
    ///
    /// Possible value for UsdGeomPrimvar::SetInterpolation. Values are interpolated between each vertex in the surface
    /// primitive. The basis function of the surface is used for  interpolation between vertices.
    ...
    /// \brief "X"
    ///
    /// Possible value for UsdGeomCylinder::GetAxisAttr(), Possible value for UsdGeomCapsule::GetAxisAttr(), Possible
    /// value for UsdGeomCone::GetAxisAttr(), Possible value for UsdGeomCylinder_1::GetAxisAttr(), Possible value for
    /// UsdGeomCapsule_1::GetAxisAttr(), Possible value for UsdGeomPlane::GetAxisAttr()
    const TfToken x;
    /// \brief "xformOpOrder"
    ///
    /// UsdGeomXformable
    ...
    /// \brief "Z"
    ///
    /// Fallback value for UsdGeomCylinder::GetAxisAttr(), Fallback value for UsdGeomCapsule::GetAxisAttr(), Fallback
    /// value for UsdGeomCone::GetAxisAttr(), Fallback value for UsdGeomCylinder_1::GetAxisAttr(), Fallback value for
    /// UsdGeomCapsule_1::GetAxisAttr(), Fallback value for UsdGeomPlane::GetAxisAttr()
    const TfToken z;
    /// \brief "BasisCurves"
    ///
