...
TF_REGISTRY_FUNCTION(TfType) {
    TfType::Define<UsdGeomSubset, TfType::Bases<UsdTyped>>();
...
UsdGeomSubset UsdGeomSubset::Define(const UsdStagePtr& stage, const SdfPath& path) {
    ...
    return UsdGeomSubset(stage->DefinePrim(path, usdPrimTypeName));
}
...
UsdAttribute UsdGeomSubset::CreateElementTypeAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->elementType, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, defaultValue, writeSparsely);
}

UsdAttribute UsdGeomSubset::GetIndicesAttr() const {
...
UsdAttribute UsdGeomSubset::CreateIndicesAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->indices, SdfValueTypeNames->IntArray,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}

UsdAttribute UsdGeomSubset::GetFamilyNameAttr() const {
...
UsdAttribute UsdGeomSubset::CreateFamilyNameAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdGeomTokens->familyName, SdfValueTypeNames->Token,
                                      /* custom = */ false, SdfVariabilityUniform, defaultValue, writeSparsely);
}
...
static UsdGeomSubset _CreateUniqueGeomSubset(UsdStagePtr stage,
...
/* static */
UsdGeomSubset UsdGeomSubset::CreateUniqueGeomSubset(const UsdGeomImageable& geom,
                                                    const TfToken& subsetName,
                                                    const TfToken& elementType,
                                                    const VtIntArray& indices,
                                                    const TfToken& familyName,
                                                    const TfToken& familyType) {
    UsdGeomSubset subset = _CreateUniqueGeomSubset(geom.GetPrim().GetStage(), geom.GetPath(), /*baseName*/ subsetName);
...
static Usd_PrimFlagsConjunction _GetGeomSubsetPredicate() {
    return UsdPrimIsActive && UsdPrimHasDefiningSpecifier && UsdPrimIsLoaded && !UsdPrimIsAbstract;
}

/* static */
std::vector<UsdGeomSubset> UsdGeomSubset::GetAllGeomSubsets(const UsdGeomImageable& geom) {
    std::vector<UsdGeomSubset> result;

    for (const auto& childPrim : geom.GetPrim().GetFilteredChildren(_GetGeomSubsetPredicate())) {
...
/* static */
std::vector<UsdGeomSubset> UsdGeomSubset::GetGeomSubsets(const UsdGeomImageable& geom,
                                                         const TfToken& elementType,
                                                         const TfToken& familyName) {
    std::vector<UsdGeomSubset> result;
...
/* static */
TfToken::Set UsdGeomSubset::GetAllGeomSubsetFamilyNames(const UsdGeomImageable& geom) {
...
static TfToken _GetFamilyTypeAttrName(const TfToken& familyName) {
    return TfToken(TfStringJoin(std::vector<std::string>{_tokens->subsetFamily.GetString(), familyName.GetString(),
                                                         _tokens->familyType.GetString()},
                                ":"));
}

/* static */
bool UsdGeomSubset::SetFamilyType(const UsdGeomImageable& geom, const TfToken& familyName, const TfToken& familyType) {
    UsdAttribute familyTypeAttr =
            geom.GetPrim().CreateAttribute(_GetFamilyTypeAttrName(familyName), SdfValueTypeNames->Token,
                                           /* custom */ false, SdfVariabilityUniform);
    return familyTypeAttr.Set(familyType);
}

/* static */
TfToken UsdGeomSubset::GetFamilyType(const UsdGeomImageable& geom, const TfToken& familyName) {
    UsdAttribute familyTypeAttr = geom.GetPrim().GetAttribute(_GetFamilyTypeAttrName(familyName));
    TfToken familyType;
...
static bool _GetEdgesFromPrim(const UsdGeomImageable& geom,
                              const UsdTimeCode& t,
                              std::set<GfVec2i, cmpPair>& edgesOnPrim) {
    const UsdAttribute fvcAttr = geom.GetPrim().GetAttribute(UsdGeomTokens->faceVertexCounts);
    const UsdAttribute fviAttr = geom.GetPrim().GetAttribute(UsdGeomTokens->faceVertexIndices);
    if (!fvcAttr || !fviAttr) {
        return false;
    }
    ...
    if (!fvcAttr.Get(&faceVertexCounts, t) || !fviAttr.Get(&faceVertexIndices, t)) {
        return false;
    }
...
static bool _GetAllPossibleSegments(const UsdGeomImageable& geom,
                                    const UsdTimeCode& t,
                                    std::set<GfVec2i, cmpPair>& allSegments) {
    const UsdAttribute curvesAttr = geom.GetPrim().GetAttribute(UsdGeomTokens->curveVertexCounts);
    if (!curvesAttr) {
        return false;
    }
...
static size_t _GetElementCountAtTime(const UsdGeomImageable& geom,
                                     const TfToken& elementType,
                                     UsdTimeCode time,
    ...
    if (elementType == UsdGeomTokens->face) {
        ...
        if (prim.IsA<UsdGeomMesh>()) {
            const UsdAttribute fvcAttr = prim.GetAttribute(UsdGeomTokens->faceVertexCounts);
            if (fvcAttr) {
                VtIntArray faceVertexCounts;
            ...
            const UsdAttribute sfviAttr = prim.GetAttribute(UsdGeomTokens->surfaceFaceVertexIndices);
        ...
        const UsdAttribute ptAttr = geom.GetPrim().GetAttribute(UsdGeomTokens->points);
        if (ptAttr) {
        ...
        if (_GetEdgesFromPrim(geom, time, edgesOnPrim)) {
            ...
            if (fvcAttr && fviAttr && isCountTimeVarying) {
                *isCountTimeVarying = fvcAttr.ValueMightBeTimeVarying() || fviAttr.ValueMightBeTimeVarying();
            }
        }
        ...
        const UsdAttribute tviAttr = geom.GetPrim().GetAttribute(UsdGeomTokens->tetVertexIndices);
        if (tviAttr) {
...
static bool _ValidateGeomType(const UsdGeomImageable& geom, const TfToken& elementType) {
    ...
    if (prim.IsA<UsdGeomMesh>()) {
        if (elementType != UsdGeomTokens->face && elementType != UsdGeomTokens->point &&
            ...
            TF_CODING_ERROR("Unsupported element type '%s' for prim type Mesh.", elementType.GetText());
            return false;
        }
    } else if (prim.IsA<UsdGeomTetMesh>()) {
        if (elementType != UsdGeomTokens->face && elementType != UsdGeomTokens->tetrahedron) {
            TF_CODING_ERROR("Unsupported element type '%s' for prim type TetMesh.", elementType.GetText());
            return false;
        }
    } else if (prim.IsA<UsdGeomBasisCurves>()) {
        if (elementType != UsdGeomTokens->segment) {
            TF_CODING_ERROR("Unsupported element type '%s' for prim type BasisCurves.", elementType.GetText());
            return false;
        }
    } else {
        TF_CODING_ERROR("Unsupported prim type '%s'.", elementType.GetText());
        return false;
    }
    return true;
}

VtVec2iArray UsdGeomSubset::_GetIndexPairs(const UsdTimeCode t, bool preserveOrder = false) const {
...
/* static */
VtIntArray UsdGeomSubset::GetUnassignedIndices(const UsdGeomImageable& geom,
                                               const TfToken& elementType,
                                               const TfToken& familyName,
    ...
    std::vector<UsdGeomSubset> subsets = UsdGeomSubset::GetGeomSubsets(geom, elementType, familyName);
    ...
    if (elementType != UsdGeomTokens->edge && elementType != UsdGeomTokens->segment) {
        ...
        if (assignedIndices.empty()) {
            ...
            for (size_t idx = 0; idx < elementCount; ++idx) result.push_back(idx);
        } else {
            std::vector<int> allIndices;
            allIndices.reserve(elementCount);
            for (size_t idx = 0; idx < elementCount; ++idx) allIndices.push_back(idx);
            ...
            std::set_difference(allIndices.begin(), allIndices.end(), assignedIndices.begin(), assignedIndices.end(),
        ...
        for (const auto& subset : subsets) {
            ...
            std::move(subsetPairs.begin(), subsetPairs.end(), std::back_inserter(pairsInFamily));
        }

        struct cmpPair e;
        std::vector<GfVec2i> unassignedIndexPairs;
        std::set_difference(possiblePairs.begin(), possiblePairs.end(), pairsInFamily.begin(), pairsInFamily.end(),
...
/* static */
bool UsdGeomSubset::ValidateSubsets(const std::vector<UsdGeomSubset>& subsets,
                                    const size_t elementCount,
                                    const TfToken& familyType,
                                    std::string* const reason) {
    if (subsets.empty()) return true;
        ...
        if (subsetElementType != elementType) {
            if (reason) {
                *reason = TfStringPrintf(
                        ...
                        subset.GetPath().GetText(), subsetElementType.GetText(), elementType.GetText());
            }
                ...
                if (!indicesInFamily.insert(index).second && familyType != UsdGeomTokens->unrestricted) {
                    ...
                    if (reason) {
                        *reason += TfStringPrintf(
                                ...
                                index, subset.GetPath().GetText(), TfStringify(t).c_str());
                    }
                }
            }
        }
...
/* static */
bool UsdGeomSubset::ValidateFamily(const UsdGeomImageable& geom,
                                   const TfToken& elementType,
                                   const TfToken& familyName,
                                   std::string* const reason) {
    if (!_ValidateGeomType(geom, elementType)) {
        *reason += TfStringPrintf("Invalid geom type for elementType %s.", elementType.GetText());
        return false;
    }
    ...
    const std::vector<UsdGeomSubset> familySubsets =
            UsdGeomSubset::GetGeomSubsets(geom, /* elementType = */ TfToken(), familyName);
    ...
    const size_t earliestTimeElementCount =
            _GetElementCountAtTime(geom, elementType, UsdTimeCode::EarliestTime(), &isElementCountTimeVarying);
    if (!isElementCountTimeVarying && earliestTimeElementCount == 0u) {
        valid = false;
        if (reason) {
            *reason += TfStringPrintf(
                    "Unable to determine element count "
    ...
    for (const UsdTimeCode& t : allTimeCodes) {
        ...
        for (const UsdGeomSubset& subset : familySubsets) {
            ...
            if (elementType == UsdGeomTokens->edge || elementType == UsdGeomTokens->segment) {
                if (subsetIndices.size() % 2 != 0) {
                    ...
                    if (reason) {
                        *reason += TfStringPrintf(
                                "Indices attribute has an "
                                "odd number of elements in GeomSubset at path <%s> "
                                "at time %s with elementType %s.",
                                subset.GetPath().GetText(), TfStringify(t).c_str(), elementType.GetText());
                    }
                }
            }
            ...
            if (!familyIsRestricted || elementType == UsdGeomTokens->edge || elementType == UsdGeomTokens->segment) {
        ...
        const size_t elementCount =
                isElementCountTimeVarying ? _GetElementCountAtTime(geom, elementType, t) : earliestTimeElementCount;
        if (!indicesInFamily.empty() && isElementCountTimeVarying && elementCount == 0u) {
            valid = false;
            if (reason) {
                *reason += TfStringPrintf(
                        "Geometry <%s> has no elements at "
                        "time %s, but the \"%s\" GeomSubset family contains "
                        "indices.",
                        geom.GetPath().GetText(), TfStringify(t).c_str(), familyName.GetText());
            }
        }
        ...
        if (elementType != UsdGeomTokens->edge && elementType != UsdGeomTokens->segment) {
                ...
                if (!familyIsRestricted) {
                    ...
                    for (const GfVec2i& edge : subsetIndices) {
                        if (!pairsInFamily.insert(edge).second) {
                            ...
                            if (reason) {
                                *reason += TfStringPrintf(
                                        "Found duplicate %s (%d, %d) "
                                        "in GeomSubset at path <%s> at time %s.",
                                        elementType.GetText(), edge[0], edge[1], subset.GetPath().GetText(),
                                        TfStringify(t).c_str());
                            }
                        }
                    }
            ...
            if (elementType == UsdGeomTokens->edge) {
                ...
                if (_GetEdgesFromPrim(geom, t, possiblePairs)) {
                    ...
                    if (!hasValidEdges) {
                        ...
                        if (reason) {
                            *reason += TfStringPrintf(
                                    "At least one edge in family %s at time %s "
            ...
            if (familyType == UsdGeomTokens->partition) {
                ...
                std::set_difference(possiblePairs.begin(), possiblePairs.end(), pairsInFamily.begin(),
                                    pairsInFamily.end(), std::inserter(res, res.begin()), e);
        ...
        if (elementType != UsdGeomTokens->segment) {
            ...
            if (elementCount > 0 && lastIndex >= 0 && static_cast<size_t>(lastIndex) >= elementCount) {
                valid = false;
                if (reason) {
                    *reason += TfStringPrintf(
                            "Found one or more indices that are "
