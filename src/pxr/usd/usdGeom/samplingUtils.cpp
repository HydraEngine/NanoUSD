...
static bool _GetAttrForTransforms(const UsdAttribute& attr,
    ...
    if (baseTime.IsNumeric()) {
        ...
        bool hasSamples;
        if (!attr.GetBracketingTimeSamples(baseTime.GetValue(), &sampleTimeValue, &sampleUpperTimeValue, &hasSamples)) {
            return false;
        }
        ...
        if (GfIsClose(sampleTimeValue, sampleUpperTimeValue, std::numeric_limits<double>::epsilon())) {
            ...
            if (!attr.GetBracketingTimeSamples(baseTimeEpsilon.GetValue(), &sampleTimeValue, &sampleUpperTimeValue,
                                               &hasSamples)) {
                return false;
            }
        }
...
static bool _CheckSampleAlignment(bool attribute1HasSamples,
    ...
    *attrCorrectLength = true;
    ...
    if (!bracketingTimeSamplesAligned || !GfIsClose(attribute1SampleTime.GetValue(), attribute2SampleTime.GetValue(),
...
bool UsdGeom_GetPositionsVelocitiesAndAccelerations(const UsdAttribute& positionsAttr,
    ...
    if (!_GetAttrForTransforms<VtVec3fArray>(positionsAttr, baseTime, &positionsSampleTime, &positionsLowerTimeValue,
                                             &positionsUpperTimeValue, &positionsHasSamples, positions)) {
    ...
    if (expectedNumPositions != 0) {
        if (positions->size() != expectedNumPositions) {
            TF_WARN("%s -- found [%zu] positions, but expected [%zu]", prim.GetPath().GetText(), positions->size(),
    ...
    if (!positionsHasSamples ||
        !_GetAttrForTransforms<VtVec3fArray>(velocitiesAttr, baseTime, velocitiesSampleTime, &velocitiesLowerTimeValue,
                                             &velocitiesUpperTimeValue, &velocitiesHasSamples, velocities)) {
        velocities->clear();
    }
    if (!_CheckSampleAlignment(velocitiesHasSamples, positionsLowerTimeValue, positionsUpperTimeValue,
                               positionsSampleTime, velocitiesLowerTimeValue, velocitiesUpperTimeValue,
                               *velocitiesSampleTime, VtValue(*velocities), correctAttrDataLength,
                               &velocitiesAlignmentValid, &velocitiesCorrectLength)) {
        ...
        if (!velocities->empty() && velocitiesAlignmentValid && !velocitiesCorrectLength) {
            TF_WARN("%s -- found [%zu] velocities, but expected [%zu]", prim.GetPath().GetText(), velocities->size(),
    ...
    if (!velocitiesHasSamples || (velocities->size() == 0) ||
        !_GetAttrForTransforms<VtVec3fArray>(accelerationsAttr, baseTime, &accelerationsSampleTime,
                                             &accelerationsLowerTimeValue, &accelerationsUpperTimeValue,
                                             &accelerationsHasSamples, accelerations)) {
        accelerations->clear();
    }
    if (!_CheckSampleAlignment(accelerationsHasSamples, velocitiesLowerTimeValue, velocitiesUpperTimeValue,
                               *velocitiesSampleTime, accelerationsLowerTimeValue, accelerationsUpperTimeValue,
                               accelerationsSampleTime, VtValue(*accelerations), correctAttrDataLength,
                               &accelerationsAlignmentValid, &accelerationsCorrectLength)) {
        ...
        if (!accelerations->empty() && accelerationsAlignmentValid && !accelerationsCorrectLength) {
            TF_WARN("%s -- found [%zu] accelerations, but expected [%zu]", prim.GetPath().GetText(),
                    accelerations->size(), correctAttrDataLength);
        }
        accelerations->clear();
    }
...
bool _UsdGeom_GetOrientationsAndAngularVelocities(const UsdAttribute& orientationsAttr,
    ...
    if (!_GetAttrForTransforms<VtArray<QuatType>>(orientationsAttr, baseTime, &orientationsSampleTime,
                                                  &orientationsLowerTimeValue, &orientationsUpperTimeValue,
                                                  &orientationsHasSamples, orientations)) {
        return false;
    }
    ...
    if (expectedNumOrientations != 0) {
        if (orientations->size() != expectedNumOrientations) {
            TF_WARN("%s -- found [%zu] orientations, but expected [%zu]", prim.GetPath().GetText(),
                    orientations->size(), expectedNumOrientations);
            return false;
        }
    }
    ...
    if (!orientationsHasSamples ||
        !_GetAttrForTransforms<VtVec3fArray>(angularVelocitiesAttr, baseTime, angularVelocitiesSampleTime,
                                             &angularVelocitiesLowerTimeValue, &angularVelocitiesUpperTimeValue,
                                             &angularVelocitiesHasSamples, angularVelocities)) {
        angularVelocities->clear();
    }
    if (!_CheckSampleAlignment(angularVelocitiesHasSamples, orientationsLowerTimeValue, orientationsUpperTimeValue,
                               orientationsSampleTime, angularVelocitiesLowerTimeValue, angularVelocitiesUpperTimeValue,
                               *angularVelocitiesSampleTime, VtValue(*angularVelocities), correctAttrDataLength,
                               &angularVelocitiesAlignmentValid, &angularVelocitiesCorrectLength)) {
        ...
        if (!angularVelocities->empty() && angularVelocitiesAlignmentValid && !angularVelocitiesCorrectLength) {
            TF_WARN("%s -- found [%zu] angular velocities, but expected [%zu]", prim.GetPath().GetText(),
                    angularVelocities->size(), correctAttrDataLength);
        }
        angularVelocities->clear();
    }
...
bool UsdGeom_GetOrientationsAndAngularVelocities(const UsdAttribute& orientationsAttr,
                                                 ...
                                                 UsdTimeCode* angularVelocitiesSampleTime,
                                                 UsdPrim const& prim) {
    return _UsdGeom_GetOrientationsAndAngularVelocities<GfQuatf>(orientationsAttr, angularVelocitiesAttr, baseTime,
                                                                 expectedNumOrientations, orientations,
                                                                 angularVelocities, angularVelocitiesSampleTime, prim);
}
...
bool UsdGeom_GetOrientationsAndAngularVelocities(const UsdAttribute& orientationsAttr,
                                                 ...
                                                 UsdTimeCode* angularVelocitiesSampleTime,
                                                 UsdPrim const& prim) {
    return _UsdGeom_GetOrientationsAndAngularVelocities<GfQuath>(orientationsAttr, angularVelocitiesAttr, baseTime,
                                                                 expectedNumOrientations, orientations,
                                                                 angularVelocities, angularVelocitiesSampleTime, prim);
}
...
bool UsdGeom_GetScales(const UsdAttribute& scalesAttr,
    ...
    if (!_GetAttrForTransforms<VtVec3fArray>(scalesAttr, baseTime, &scalesSampleTime, &dummyLowerTimeValue,
                                             &dummyUpperTimeValue, &scalesHasSamples, scales)) {
        return false;
    }

    if (scales->size() != expectedScales) {
        TF_WARN("%s -- found [%zu] scales, but expected [%zu]", prim.GetPath().GetText(), scales->size(),
                expectedScales);
    }
