...
std::string StringVal(const TfToken& key, const NdrTokenMap& metadata, const std::string& defaultValue) {
    const NdrTokenMap::const_iterator search = metadata.find(key);
...
bool IsPropertyAnAssetIdentifier(const NdrTokenMap& metadata) {
    ...
    if (widgetSearch != metadata.end()) {
        ...
        if ((widget == _tokens->assetIdInput) || (widget == _tokens->filename) || (widget == _tokens->fileInput)) {
            return true;
        }
    }
