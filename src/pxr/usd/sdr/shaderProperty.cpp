...
const TokenToSdfTypeMap& _GetTokenTypeToSdfType() {
    static const TokenToSdfTypeMap tokenTypeToSdfType = {{SdrPropertyTypes->Int, SdfValueTypeNames->Int},
...
const TokenToSdfTypeMap& _GetAliasesTokensToSdfValueTypeNames() {
    ...
    static const TokenToSdfTypeMap& aliasesTokensToSdfValueTypeNames = MapBuilder();
...
const TokenToMapTable& _GetConvertedSdrTypes() {
    static const TokenToMapTable convertedSdrTypes = {
            ...
            {SdrPropertyTypes->Color4, {{SdrPropertyRole->None, {SdrPropertyTypes->Float, 4}}}},
...
SdfValueTypeName _GetSdrUsdDefinitionType(const NdrTokenMap& metadata) {
    const TfToken& sdrUsdDefinitionType = TfToken(StringVal(SdrPropertyMetadata->SdrUsdDefinitionType, metadata));

    if (sdrUsdDefinitionType.IsEmpty()) {
        return SdfValueTypeName();
    }

    const TokenToSdfTypeMap& aliasesTokensToSdfValueTypeNames = _GetAliasesTokensToSdfValueTypeNames();

    if (aliasesTokensToSdfValueTypeNames.find(sdrUsdDefinitionType) == aliasesTokensToSdfValueTypeNames.end()) {
...
// has an array type
bool _IsArray(size_t arraySize, const NdrTokenMap& metadata) {
...
NdrSdfTypeIndicator _GetTypeIndicatorFromDefaultMapping(const TfToken& type, bool isArray) {
    const TokenToSdfTypeMap& tokenTypeToSdfType = isArray ? _GetTokenTypeToSdfArrayType() : _GetTokenTypeToSdfType();
...
NdrSdfTypeIndicator GetTypeAsSdfType(const TfToken& type, size_t arraySize, const NdrTokenMap& metadata) {
    ...
    if (_IsAssetIdentifier(metadata)) {
        auto sdfType = isArray ? SdfValueTypeNames->StringArray : SdfValueTypeNames->String;
    ...
    if (type == SdrPropertyTypes->Vstruct) {
        auto sdfType = isArray ? SdfValueTypeNames->FloatArray : SdfValueTypeNames->Float;
        return NdrSdfTypeIndicator(sdfType, type);
    }
...
NdrSdfTypeIndicator GetTypeAsSdfType(const TfToken& type, size_t arraySize, const NdrTokenMap& metadata) {
    const SdfValueTypeName& sdfValueTypeName = _GetSdrUsdDefinitionType(metadata);
    if (sdfValueTypeName) {
    ...
    // mapping because it is determined dynamically
    if (_IsAssetIdentifier(metadata)) {
        auto sdfType = isArray ? SdfValueTypeNames->AssetArray : SdfValueTypeNames->Asset;
        return NdrSdfTypeIndicator(sdfType, type);
    }
    ...
    if (type == SdrPropertyTypes->Terminal || type == SdrPropertyTypes->Struct || type == SdrPropertyTypes->Vstruct) {
        SdfValueTypeName sdfType = isArray ? SdfValueTypeNames->TokenArray : SdfValueTypeNames->Token;
        return NdrSdfTypeIndicator(sdfType, type);
    }
...
NdrSdfTypeIndicator _GetTypeAsSdfType(const TfToken& type,
                                      size_t arraySize,
    ...
    switch (usdEncodingVersion) {
        ...
        default:
            TF_DEBUG(NDR_PARSING)
                    .Msg("Invalid/unsupported usdEncodingVersion %d. "
...
VtValue _ConformSdrDefaultValue(const VtValue& sdrDefaultValue,
    ...
    if (sdrType == SdrPropertyTypes->Int) {
        ...
        if (!isArray) {
            ...
            isSdrValueConformed = sdrDefaultValue.IsHolding<VtStringArray>();
        }
        ...
        if (!isArray) {
            ...
            if (arrayVal.size() != arraySize) {
                ...
                TF_DEBUG(SDR_TYPE_CONFORMANCE)
                        .Msg("Default value for fixed size float array type does not "
            ...
            if (isSdrValueConformed) {
                switch (arraySize) {
                    case 2:
                        defaultValue = VtValue(GfVec2f(arrayVal[0], arrayVal[1]));
                        break;
                    case 3:
                        defaultValue = VtValue(GfVec3f(arrayVal[0], arrayVal[1], arrayVal[2]));
                        break;
                    case 4:
                        defaultValue = VtValue(GfVec4f(arrayVal[0], arrayVal[1], arrayVal[2], arrayVal[3]));
                        break;
                    default:
                        TF_DEBUG(SDR_TYPE_CONFORMANCE)
                                .Msg("Invalid "
    ...
    } else if (sdrType == SdrPropertyTypes->Color || sdrType == SdrPropertyTypes->Point ||
               sdrType == SdrPropertyTypes->Normal || sdrType == SdrPropertyTypes->Vector) {
        if (!isArray) {
        ...
        if (!isArray) {
            ...
            isSdrValueConformed = sdrDefaultValue.IsHolding<VtArray<GfMatrix4d>>();
        }
    } else {
        // malformed sdrType
        isSdrValueConformed = false;
    ...
    if (!isSdrValueConformed) {
        TF_DEBUG(SDR_TYPE_CONFORMANCE)
                .Msg("Expected type for defaultValue for property: %s is %s, "
                     "but %s was provided.",
                     name.GetText(), sdrType.GetText(), defaultValue.GetTypeName().c_str());
    }
    return defaultValue;
}
...
// instantiated.
VtValue _ConformSdfTypeDefaultValue(const VtValue& sdrDefaultValue,
                                    const TfToken& sdrType,
                                    size_t arraySize,
                                    const NdrTokenMap& metadata,
                                    int usdEncodingVersion) {
    ...
    NdrSdfTypeIndicator sdfTypeIndicator = _GetTypeAsSdfType(sdrType, arraySize, metadata, usdEncodingVersion);
    ...
    if (metadata.find(SdrPropertyMetadata->SdrUsdDefinitionType) != metadata.end()) {
        ...
        VtValue sdfTypeValue = VtValue::CastToTypeid(sdrDefaultValue, sdfType.GetType().GetTypeid());
    ...
    // ---------------------------------------------------------------------
    if (sdrType == SdrPropertyTypes->String && _IsAssetIdentifier(metadata)) {
        if (isArray) {
...
std::string SdrShaderProperty::GetImplementationName() const {
    return StringVal(SdrPropertyMetadata->ImplementationName, _metadata, GetName().GetString());
}
...
bool SdrShaderProperty::CanConnectTo(const NdrProperty& other) const {
    ...
    NdrSdfTypeIndicator sdfInputTypeInd =
            _GetTypeAsSdfType(inputType, inputArraySize, inputMetadata, _usdEncodingVersion);
    NdrSdfTypeIndicator sdfOutputTypeInd =
            _GetTypeAsSdfType(outputType, outputArraySize, outputMetadata, _usdEncodingVersion);
    ...
    bool outputIsFloat3 = (outputType == SdrPropertyTypes->Color) || (outputType == SdrPropertyTypes->Point) ||
                          (outputType == SdrPropertyTypes->Normal) || (outputType == SdrPropertyTypes->Vector) ||
    ...
    bool inputIsFloat4 = (inputType == SdrPropertyTypes->Color4) || (sdfInputType == SdfValueTypeNames->Float4);

    bool outputIsFloat4 = (outputType == SdrPropertyTypes->Color4) || (sdfOutputType == SdfValueTypeNames->Float4);
...
bool SdrShaderProperty::IsVStructMember() const {
    return _metadata.count(SdrPropertyMetadata->VstructMemberName);
}

bool SdrShaderProperty::IsVStruct() const {
    return _type == SdrPropertyTypes->Vstruct;
}

NdrSdfTypeIndicator SdrShaderProperty::GetTypeAsSdfType() const {
    return _GetTypeAsSdfType(_type, _arraySize, _metadata, _usdEncodingVersion);
}

bool SdrShaderProperty::IsAssetIdentifier() const {
    return _IsAssetIdentifier(_metadata);
}

bool SdrShaderProperty::IsDefaultInput() const {
...
void SdrShaderProperty::_FinalizeProperty() {
    _sdfTypeDefaultValue =
            _ConformSdfTypeDefaultValue(_defaultValue, _type, _arraySize, _metadata, _usdEncodingVersion);

    _defaultValue = _ConformSdrDefaultValue(_defaultValue, _type, _arraySize, _metadata, _name);
}
