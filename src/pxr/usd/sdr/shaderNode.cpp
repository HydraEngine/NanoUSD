...
void SdrShaderNode::_PostProcessProperties() {
    ...
    int usdEncodingVersion = IntVal(SdrNodeMetadata->SdrUsdEncodingVersion, _metadata, DEFAULT_ENCODING);

    const NdrTokenVec vsNames = GetAllVstructNames();

    for (const NdrPropertyUniquePtr& property : _properties) {
        SdrShaderPropertyConstPtr constShaderProperty = dynamic_cast<SdrShaderPropertyConstPtr>(property.get());
        ...
        bool isVStruct = std::find(vsNames.begin(), vsNames.end(), shaderProperty->GetName()) != vsNames.end();
        if (isVStruct) {
...
NdrTokenVec SdrShaderNode::GetPropertyNamesForPage(const std::string& pageName) const {
    ...
    for (const NdrPropertyUniquePtr& property : _properties) {
        const SdrShaderPropertyConstPtr shaderProperty = dynamic_cast<const SdrShaderPropertyConstPtr>(property.get());
        ...
        if (TfStringStartsWith(primvar, "$")) {
            ...
            if (input && (input->GetType() == SdrPropertyTypes->String)) {
                ...
                TF_DEBUG(NDR_PARSING)
                        .Msg("Found a node [%s] whose metadata "
