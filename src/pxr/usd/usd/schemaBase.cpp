...
UsdSchemaBase::UsdSchemaBase(const UsdPrim& prim) : _primData(prim._Prim()), _proxyPrimPath(prim._ProxyPrimPath()) {
    /* NOTHING */
}
...
UsdAttribute UsdSchemaBase::_CreateAttr(TfToken const& attrName,
    ...
    if (writeSparsely && !custom) {
        ...
        VtValue fallback;
        if (defaultValue.IsEmpty() || (!attr.HasAuthoredValue() && attr.Get(&fallback) && fallback == defaultValue)) {
            return attr;
        }
    }
