...
bool UsdUsdcFileFormat::_CanReadFromAsset(const string& filePath, const std::shared_ptr<ArAsset>& asset) const {
...
bool UsdUsdcFileFormat::_ReadDetached(SdfLayer* layer, const std::string& resolvedPath, bool metadataOnly) const {
    TRACE_FUNCTION();
    return _ReadHelper(layer, resolvedPath, metadataOnly,
                       /* detached = */ true);
}
...
bool UsdUsdcFileFormat::WriteToString(const SdfLayer& layer, std::string* str, const std::string& comment) const {
