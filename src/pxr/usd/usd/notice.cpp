...
TfTokenVector UsdNotice::ObjectsChanged::PathRange::const_iterator::GetChangedFields() const {
    ...
    for (const SdfChangeList::Entry* entry : _underlyingIterator->second) {
        ...
        std::transform(entry->infoChanged.begin(), entry->infoChanged.end(), std::back_inserter(fields), TfGet<0>());
    }
...
const UsdNotice::ObjectsChanged::_PathsToChangesMap& UsdNotice::ObjectsChanged::_GetEmptyChangesMap() {
    static const _PathsToChangesMap empty;
    return empty;
}

UsdNotice::ObjectsChanged::ObjectsChanged(const UsdStageWeakPtr& stage, const _PathsToChangesMap* resyncChanges)
...
UsdNotice::ObjectsChanged::PathRange UsdNotice::ObjectsChanged::GetResolvedAssetPathsResyncedPaths() const {
