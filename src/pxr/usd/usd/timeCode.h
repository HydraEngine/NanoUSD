...
public:
    ...
    static constexpr UsdTimeCode EarliestTime() { return UsdTimeCode(std::numeric_limits<double>::lowest()); }
    ...
    static constexpr UsdTimeCode Default() { return UsdTimeCode(std::numeric_limits<double>::quiet_NaN()); }
    ...
    bool IsEarliestTime() const { return IsNumeric() && (_value == std::numeric_limits<double>::lowest()); }
    ...
    double GetValue() const {
        if (ARCH_UNLIKELY(IsDefault())) _IssueGetValueOnDefaultError();
        return _value;
    }
    ...
    friend bool operator!=(const UsdTimeCode& lhs, const UsdTimeCode& rhs) { return !(lhs == rhs); }
    ...
    /// Hash function.
    friend size_t hash_value(const UsdTimeCode& time) { return TfHash()(time._value); }
