...
struct ValueRep {
    ...
    inline TypeEnum GetType() const { return static_cast<TypeEnum>((data >> 48) & 0xFF); }
    ...
    friend inline size_t hash_value(ValueRep v) { return static_cast<size_t>(v.data); }
...
struct TimeSamples {
    ...
    bool IsInMemory() const { return valueRep.GetData() == 0; }
    ...
    bool operator==(TimeSamples const& other) const {
        return valueRep == other.valueRep && times == other.times && values == other.values &&
    ...
    friend size_t hash_value(TimeSamples const& ts) {
        return TfHash::Combine(ts.valueRep, ts.times, ts.values, ts.valuesFileOffset);
    }

    friend std::ostream& operator<<(std::ostream& os, TimeSamples const& samples);
...
inline size_t hash_value(const Index& i) {
    return i.value;
}
...
struct _SectionName {
    ...
    constexpr bool operator==(_SectionName other) const { return _p == other._p; }
    constexpr bool operator!=(_SectionName other) const { return _p != other._p; }
    bool operator==(char const* otherStr) const { return !strcmp(_p, otherStr); }
...
private:
    ...
    struct _FileRange {
        ...
        _FileRange(_FileRange&& other)
            : file(other.file), startOffset(other.startOffset), length(other.length), hasOwnership(other.hasOwnership) {
            other.file = nullptr;
        }
    ...
    class _FileMapping {
        ...
        class _Impl {
            ...
            struct ZeroCopySource : public Vt_ArrayForeignDataSource {
                ...
                bool operator!=(ZeroCopySource const& other) const { return !(*this == other); }
                ...
                bool NewRef() { return _refCount.fetch_add(1, std::memory_order_relaxed) == 0; }
            ...
            explicit _Impl(ArchConstFileMapping&& mapping, int64_t offset, int64_t length)
                : _refCount(0),
            ...
            Vt_ArrayForeignDataSource* _AddRangeReference(void const* addr, size_t numBytes);
    ...
    public:
        ...
        explicit _FileMapping(ArchConstFileMapping&& mapping, int64_t offset = 0, int64_t length = -1) noexcept
            : _impl(TfDelegatedCountIncrementTag, new _Impl(std::move(mapping), offset, length)) {}

        _FileMapping(_FileMapping&& other) noexcept : _impl(std::move(other._impl)) { other._impl = nullptr; }
        ...
        // Add an an externally referenced page range.
        Vt_ArrayForeignDataSource* AddRangeReference(void const* addr, size_t numBytes) {
...
public:
    ...
    struct Version {
        ...
        // forward-compatible.
        bool CanRead(Version const& fileVer) const { return fileVer.majver == majver && fileVer.minver <= minver; }
        ...
        bool CanWrite(Version const& fileVer) const {
            return fileVer.majver == majver &&
                   (fileVer.minver < minver || (fileVer.minver == minver && fileVer.patchver <= patchver));
        }

#define LOGIC_OP(op) \
    constexpr bool operator op(Version const& other) const { return AsInt() op other.AsInt(); }
        LOGIC_OP(==);
        LOGIC_OP(!=);
        LOGIC_OP(<);
        LOGIC_OP(>);
...
#undef LOGIC_OP
    ...
    struct Field {
        ...
        bool operator==(const Field& other) const {
            return tokenIndex == other.tokenIndex && valueRep == other.valueRep;
        }
        friend size_t hash_value(const Field& f) {
            _Hasher h;
            return TfHash::Combine(h(f.tokenIndex), f.valueRep);
        }
    ...
    struct Spec {
        Spec() {}
        Spec(PathIndex pi, SdfSpecType type, FieldSetIndex fsi) : pathIndex(pi), fieldSetIndex(fsi), specType(type) {}
        Spec(Spec_0_0_1 const&);
        bool operator==(const Spec& other) const {
            return pathIndex == other.pathIndex && fieldSetIndex == other.fieldSetIndex && specType == other.specType;
        }
        bool operator!=(const Spec& other) const { return !(*this == other); }
        friend size_t hash_value(Spec const& s) {
            _Hasher h;
            return TfHash::Combine(h(s.pathIndex), s.fieldSetIndex, s.specType);
        }
        PathIndex pathIndex;
    ...
    struct Spec_0_0_1 {
        ...
        bool operator!=(const Spec_0_0_1& other) const { return !(*this == other); }
    ...
    static std::unique_ptr<CrateFile> Open(string const& assetPath, bool detached = false);
    ...
    inline vector<FieldIndex> const& GetFieldSets() const { return _fieldSets; }
    ...
    inline void RemoveStructuralData(vector<Spec>& outSpecs,
        ...
        {
            vector<FieldIndex> tmp;
            tmp.swap(_fieldSets);
            outFieldSets.swap(tmp);
        }
    }
...
private:
    enum class Options { Default, UseMmap, Detached };

    explicit CrateFile(Options opt);
    CrateFile(string const& assetPath, string const& fileName, _FileMapping&& mapping, ArAssetSharedPtr const& asset);
    CrateFile(string const& assetPath, string const& fileName, _FileRange&& inputFile, ArAssetSharedPtr const& asset);
    CrateFile(string const& assetPath, ArAssetSharedPtr const& asset, bool detached);
    ...
    static _FileMapping _MmapFile(char const* fileName, FILE* file);
    static _FileMapping _MmapAsset(char const* fileName, ArAssetSharedPtr const& asset);
    ...
    void _AddSpec(const SdfPath& path, SdfSpecType type, const std::vector<FieldValuePair>& fields);
    ...
    mutable unordered_map<ValueRep, TimeSamples::SharedTimes, _Hasher> _sharedTimes;
    mutable tbb::spin_rw_mutex _sharedTimesMutex;

    // functions to write VtValues to file by type.
    pxr_tsl::robin_map<std::type_index, std::function<ValueRep(VtValue const&)>> _packValueFunctions;
