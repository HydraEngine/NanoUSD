...
UsdStagePopulationMask UsdStagePopulationMask::Union(UsdStagePopulationMask const& l, UsdStagePopulationMask const& r) {
    ...
    if (lcur != lend) result._paths.insert(result._paths.end(), lcur, lend);
    if (rcur != rend) result._paths.insert(result._paths.end(), rcur, rend);
...
std::pair<bool, std::vector<SdfPath>::const_iterator> _IncludesSubtree(std::vector<SdfPath> const& paths,
                                                                       SdfPath const& path) {
    if (paths.empty()) return {false, paths.end()};
...
bool UsdStagePopulationMask::GetIncludedChildNames(SdfPath const& path, std::vector<TfToken>* names) const {
    ...
    if (includesSubtree.first) return true;
