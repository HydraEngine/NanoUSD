...
public:
    ...
    const UsdPrimTypeInfo& GetPrimTypeInfo() const { return _Prim()->GetPrimTypeInfo(); }
    ...
    /// not.
    const UsdPrimDefinition& GetPrimDefinition() const { return _Prim()->GetPrimDefinition(); }
    ...
    /// opinion wins" semantics.
    USD_API
    std::vector<std::pair<SdfPrimSpecHandle, SdfLayerOffset>> GetPrimStackWithLayerOffsets() const;

    /// Author an opinion for this Prim's specifier at the current edit
    /// target.
    bool SetSpecifier(SdfSpecifier specifier) const { return SetMetadata(SdfFieldKeys->Specifier, specifier); }
    ...
    bool SetTypeName(const TfToken& typeName) const { return SetMetadata(SdfFieldKeys->TypeName, typeName); }

    /// Clear the opinion for this Prim's typeName at the current edit
    /// target.
    bool ClearTypeName() const { return ClearMetadata(SdfFieldKeys->TypeName); }

    /// Return true if a typeName has been authored.
    bool HasAuthoredTypeName() const { return HasAuthoredMetadata(SdfFieldKeys->TypeName); }
    ...
    /// a prim.
    bool SetActive(bool active) const { return SetMetadata(SdfFieldKeys->Active, active); }
    ...
    /// See \ref Usd_ActiveInactive for the effects of activating or deactivating
    /// a prim.
    bool ClearActive() const { return ClearMetadata(SdfFieldKeys->Active); }

    /// Return true if this prim has an authored opinion for 'active', false
    /// otherwise.
    ///
    /// See \ref Usd_ActiveInactive for what it means for a prim to be active.
    bool HasAuthoredActive() const { return HasAuthoredMetadata(SdfFieldKeys->Active); }
    ...
    bool HasDefiningSpecifier() const { return _Prim()->HasDefiningSpecifier(); }
    ...
    USD_API
    TfTokenVector GetPropertyNames(const PropertyPredicateFunc& predicate = {}) const;
    ...
    /// \sa UsdProperty::IsAuthored()
    USD_API
    TfTokenVector GetAuthoredPropertyNames(const PropertyPredicateFunc& predicate = {}) const;
    ...
    /// \sa UsdProperty::IsAuthored()
    USD_API
    std::vector<UsdProperty> GetProperties(const PropertyPredicateFunc& predicate = {}) const;
    ...
    USD_API
    std::vector<UsdProperty> GetPropertiesInNamespace(const std::string& namespaces) const;
    ...
    /// For details of namespaced properties, see \ref Usd_Ordering
    USD_API
    std::vector<UsdProperty> GetAuthoredPropertiesInNamespace(const std::vector<std::string>& namespaces) const;
    ...
    USD_API
    std::vector<UsdProperty> GetAuthoredPropertiesInNamespace(const std::string& namespaces) const;
    ...
    /// EditTarget.
    void SetPropertyOrder(const TfTokenVector& order) const { SetMetadata(SdfFieldKeys->PropertyOrder, order); }

    /// Remove the opinion for propertyOrder metadata on this prim at the current
    /// EditTarget.
    void ClearPropertyOrder() const { ClearMetadata(SdfFieldKeys->PropertyOrder); }
...
private:
    ...
    USD_API
    bool _HasAPIInstance(const UsdSchemaRegistry::SchemaInfo* schemaInfo, const TfToken& instanceName) const;

    USD_API
    bool _CanApplySingleApplyAPI(const UsdSchemaRegistry::SchemaInfo& schemaInfo, std::string* whyNot) const;

    USD_API
    bool _CanApplyMultipleApplyAPI(const UsdSchemaRegistry::SchemaInfo& schemaInfo,
                                   const TfToken& instanceName,
                                   std::string* whyNot) const;

    USD_API
    bool _ApplySingleApplyAPI(const UsdSchemaRegistry::SchemaInfo& schemaInfo) const;

    USD_API
    bool _ApplyMultipleApplyAPI(const UsdSchemaRegistry::SchemaInfo& schemaInfo, const TfToken& instanceName) const;

    USD_API
    bool _RemoveSingleApplyAPI(const UsdSchemaRegistry::SchemaInfo& schemaInfo) const;

    USD_API
    bool _RemoveMultipleApplyAPI(const UsdSchemaRegistry::SchemaInfo& schemaInfo, const TfToken& instanceName) const;
...
public:
    ...
    USD_API
    bool IsA(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion) const;
    ...
    USD_API
    bool IsInFamily(const TfToken& schemaFamily,
    ...
    bool IsInFamily(UsdSchemaRegistry::VersionPolicy versionPolicy) const {
        ...
        if (!schemaInfo) {
            TF_CODING_ERROR(
                    "Class '%s' is not correctly registered with the "
                    "UsdSchemaRegistry as a schema type. The schema may need to be "
                    "regenerated.",
                    TfType::Find<SchemaType>().GetTypeName().c_str());
            return false;
        }
        return IsInFamily(schemaInfo->family, schemaInfo->version, versionPolicy);
    };
    ...
    USD_API
    bool IsInFamily(const TfType& schemaType, UsdSchemaRegistry::VersionPolicy versionPolicy) const;
    ...
    USD_API
    bool IsInFamily(const TfToken& schemaIdentifier, UsdSchemaRegistry::VersionPolicy versionPolicy) const;
    ...
    /// prim \ref IsA.
    USD_API
    bool GetVersionIfIsInFamily(const TfToken& schemaFamily, UsdSchemaVersion* schemaVersion) const;
    ...
    USD_API
    bool HasAPI(const TfType& schemaType, const TfToken& instanceName) const;
    ...
    /// type.
    USD_API
    bool HasAPI(const TfToken& schemaIdentifier, const TfToken& instanceName) const;

    /// This is an overload of \ref HasAPI that takes a \p schemaFamily and
    /// \p schemaVersion to determine the schema type.
    USD_API
    bool HasAPI(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion) const;

    /// This is an overload of \ref HasAPI(const TfToken &) const "HasAPI" with
    /// \p instanceName that takes a \p schemaFamily and \p schemaVersion to
    /// determine the schema type.
    USD_API
    bool HasAPI(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion, const TfToken& instanceName) const;
    ...
    USD_API
    bool HasAPIInFamily(const TfToken& schemaFamily) const;
    ...
    /// \p instanceName must be non-empty, otherwise it is a coding error.
    USD_API
    bool HasAPIInFamily(const TfToken& schemaFamily, const TfToken& instanceName) const;
    ...
    /// has been applied.
    USD_API
    bool HasAPIInFamily(const TfToken& schemaFamily,
                        UsdSchemaVersion schemaVersion,
                        UsdSchemaRegistry::VersionPolicy versionPolicy) const;
    ...
    bool HasAPIInFamily(UsdSchemaRegistry::VersionPolicy versionPolicy) const {
        ...
        return HasAPIInFamily(schemaInfo->family, schemaInfo->version, versionPolicy);
    };

    /// Overload for convenience of
    /// \ref HasAPIInFamily(const TfToken&, UsdSchemaVersion, UsdSchemaRegistry::VersionPolicy, const TfToken &) const
    /// "HasAPIInFamily" that finds a registered schema for the C++ schema class \p SchemaType and uses that schema's
    /// family and version.
    template <typename SchemaType>
    bool HasAPIInFamily(UsdSchemaRegistry::VersionPolicy versionPolicy, const TfToken& instanceName) const {
        ...
        return HasAPIInFamily(schemaInfo->family, schemaInfo->version, versionPolicy, instanceName);
    };
    ...
    /// schema's family and version.
    USD_API
    bool HasAPIInFamily(const TfType& schemaType, UsdSchemaRegistry::VersionPolicy versionPolicy) const;

    /// Overload for convenience of
    /// \ref HasAPIInFamily(const TfToken&, UsdSchemaVersion, UsdSchemaRegistry::VersionPolicy, const TfToken &) const
    /// "HasAPIInFamily" that finds a registered schema for the given \p schemaType and uses that schema's family and
    /// version.
    USD_API
    bool HasAPIInFamily(const TfType& schemaType,
                        UsdSchemaRegistry::VersionPolicy versionPolicy,
                        const TfToken& instanceName) const;
    ...
    /// See UsdSchemaRegistry::ParseSchemaFamilyAndVersionFromIdentifier.
    USD_API
    bool HasAPIInFamily(const TfToken& schemaIdentifier, UsdSchemaRegistry::VersionPolicy versionPolicy) const;

    /// Overload for convenience of
    /// \ref HasAPIInFamily(const TfToken&, UsdSchemaVersion, UsdSchemaRegistry::VersionPolicy, const TfToken &) const
    /// "HasAPIInFamily" that parses the schema family and version to use from the given
    /// \p schemaIdentifier.
    ///
    /// Note that the schema identifier is not required to be a registered
    /// schema as it only parsed to get what its family and version would be
    /// See UsdSchemaRegistry::ParseSchemaFamilyAndVersionFromIdentifier.
    USD_API
    bool HasAPIInFamily(const TfToken& schemaIdentifier,
                        UsdSchemaRegistry::VersionPolicy versionPolicy,
                        const TfToken& instanceName) const;
    ...
    USD_API
    bool GetVersionIfHasAPIInFamily(const TfToken& schemaFamily, UsdSchemaVersion* schemaVersion) const;
    ...
    USD_API
    bool GetVersionIfHasAPIInFamily(const TfToken& schemaFamily,
                                    const TfToken& instanceName,
    ...
    /// \p schemaType .
    USD_API
    bool CanApplyAPI(const TfType& schemaType, std::string* whyNot = nullptr) const;
    ...
    USD_API
    bool CanApplyAPI(const TfType& schemaType, const TfToken& instanceName, std::string* whyNot = nullptr) const;

    /// This is an overload of \ref CanApplyAPI that takes a \p schemaIdentifier
    /// to determine the schema type.
    USD_API
    bool CanApplyAPI(const TfToken& schemaIdentifier, std::string* whyNot = nullptr) const;
    ...
    /// schema type.
    USD_API
    bool CanApplyAPI(const TfToken& schemaIdentifier, const TfToken& instanceName, std::string* whyNot = nullptr) const;

    /// This is an overload of \ref CanApplyAPI that takes a \p schemaFamily and
    /// \p schemaVersion to determine the schema type.
    USD_API
    bool CanApplyAPI(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion, std::string* whyNot = nullptr) const;
    ...
    /// schema type.
    USD_API
    bool ApplyAPI(const TfToken& schemaIdentifier, const TfToken& instanceName) const;

    /// This is an overload of \ref ApplyAPI that takes a \p schemaFamily and
    /// \p schemaVersion to determine the schema type.
    USD_API
    bool ApplyAPI(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion) const;

    /// This is an overload of \ref ApplyAPI(const TfToken &) const "ApplyAPI"
    /// with \p instanceName that takes a \p schemaFamily and \p schemaVersion
    /// to determine the schema type.
    USD_API
    bool ApplyAPI(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion, const TfToken& instanceName) const;
    ...
    /// with \p instanceName that takes a TfType \p schemaType .
    USD_API
    bool RemoveAPI(const TfType& schemaType, const TfToken& instanceName) const;
    ...
    /// schema type.
    USD_API
    bool RemoveAPI(const TfToken& schemaIdentifier, const TfToken& instanceName) const;

    /// This is an overload of \ref RemoveAPI that takes a \p schemaFamily and
    /// \p schemaVersion to determine the schema type.
    USD_API
    bool RemoveAPI(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion) const;

    /// This is an overload of \ref RemoveAPI(const TfToken &) const "RemoveAPI"
    /// with \p instanceName that takes a \p schemaFamily and \p schemaVersion
    /// to determine the schema type.
    USD_API
    bool RemoveAPI(const TfToken& schemaFamily, UsdSchemaVersion schemaVersion, const TfToken& instanceName) const;
    ...
    /// See \ref Usd_PrimFlags "Prim predicate flags"
    /// and #UsdPrimDefaultPredicate for more information.
    inline SiblingRange GetFilteredChildren(const Usd_PrimFlagsPredicate& predicate) const;
    ...
    USD_API
    TfTokenVector GetFilteredChildrenNames(const Usd_PrimFlagsPredicate& predicate) const;
    ...
    /// #UsdPrimDefaultPredicate for more information, UsdStage::Traverse(),
    /// and \c UsdPrimRange for more general Stage traversal behaviors.
    inline SubtreeRange GetFilteredDescendants(const Usd_PrimFlagsPredicate& predicate) const;
    ...
    void SetChildrenReorder(const TfTokenVector& order) const { SetMetadata(SdfFieldKeys->PrimOrder, order); }

    /// Remove the opinion for the metadata used to reorder children of this
    /// prim at the current EditTarget.
    void ClearChildrenReorder() const { ClearMetadata(SdfFieldKeys->PrimOrder); }
...
public:
    ...
    /// and #UsdPrimDefaultPredicate for more information.
    USD_API
    UsdPrim GetFilteredNextSibling(const Usd_PrimFlagsPredicate& predicate) const;
    ...
    USD_API
    UsdAttribute CreateAttribute(const TfToken& name,
    ...
    /// \ref Usd_Ordering
    USD_API
    UsdAttribute CreateAttribute(const std::vector<std::string>& nameElts,
                                 const SdfValueTypeName& typeName,
                                 bool custom,
                                 SdfVariability variability = SdfVariabilityVarying) const;
    /// \overload
    ...
    USD_API
    SdfPathVector FindAllAttributeConnectionPaths(Usd_PrimFlagsPredicate const& traversalPredicate,
    ...
    USD_API
    SdfPathVector FindAllAttributeConnectionPaths(std::function<bool(UsdAttribute const&)> const& pred = nullptr,
                                                  bool recurseOnSources = false) const;
    ...
    USD_API
    SdfPathVector FindAllRelationshipTargetPaths(Usd_PrimFlagsPredicate const& traversalPredicate,
    ...
    USD_API
    SdfPathVector FindAllRelationshipTargetPaths(std::function<bool(UsdRelationship const&)> const& pred = nullptr,
                                                 bool recurseOnTargets = false) const;
    ...
    USD_API
    bool SetPayload(const std::string& assetPath, const SdfPath& primPath) const;
    ...
    bool IsInstanceable() const {
        ...
        return GetMetadata(SdfFieldKeys->Instanceable, &instanceable) && instanceable;
    }

    /// Author 'instanceable' metadata for this prim at the current
    /// EditTarget.
    bool SetInstanceable(bool instanceable) const { return SetMetadata(SdfFieldKeys->Instanceable, instanceable); }

    /// Remove the authored 'instanceable' opinion at the current EditTarget.
    /// Do nothing if there is no authored opinion.
    bool ClearInstanceable() const { return ClearMetadata(SdfFieldKeys->Instanceable); }

    /// Return true if this prim has an authored opinion for 'instanceable',
    /// false otherwise.
    bool HasAuthoredInstanceable() const { return HasAuthoredMetadata(SdfFieldKeys->Instanceable); }
    ...
    /// prim.
    bool IsInstanceProxy() const { return Usd_IsInstanceProxy(_Prim(), _ProxyPrimPath()); }
    ...
    bool IsInPrototype() const {
        return (IsInstanceProxy() ? IsPathInPrototype(GetPrimPath()) : _Prim()->IsInPrototype());
    }
    ...
    USD_API
    UsdResolveTarget MakeResolveTargetUpToEditTarget(const UsdEditTarget& editTarget) const;
    ...
    /// this prim, a null resolve target is returned.
    USD_API
    UsdResolveTarget MakeResolveTargetStrongerThanEditTarget(const UsdEditTarget& editTarget) const;
...
private:
    ...
    // Prim constructor.
    UsdPrim(const Usd_PrimDataHandle& primData, const SdfPath& proxyPrimPath) : UsdObject(primData, proxyPrimPath) {}

    // General constructor.
    UsdPrim(UsdObjType objType, const Usd_PrimDataHandle& prim, const SdfPath& proxyPrimPath, const TfToken& propName)
        : UsdObject(objType, prim, proxyPrimPath, propName) {}

    // Helper to make a sibling range.
    inline SiblingRange _MakeSiblingRange(const Usd_PrimFlagsPredicate& pred) const;

    // Helper to make a range of descendants.
    inline SubtreeRange _MakeDescendantsRange(const Usd_PrimFlagsPredicate& pred) const;

    // Helper to make a vector of properties from names.
    std::vector<UsdProperty> _MakeProperties(const TfTokenVector& names) const;
    ...
    std::vector<UsdRelationship> _GetRelationships(bool onlyAuthored, bool applyOrder = false) const;
    ...
    const PcpPrimIndex& _GetSourcePrimIndex() const { return _Prim()->GetSourcePrimIndex(); }

    // Helper function for MakeResolveTargetUpToEditTarget and
    // MakeResolveTargetStrongerThanEditTarget.
    UsdResolveTarget _MakeResolveTargetFromEditTarget(const UsdEditTarget& editTarget, bool makeAsStrongerThan) const;
...
public:
    ...
    bool operator==(const UsdPrimSiblingIterator& other) const { return equal(other); }
...
private:
    ...
    UsdPrimSiblingIterator(const _UnderlyingIterator& i,
        ...
        : _underlyingIterator(i), _proxyPrimPath(proxyPrimPath), _predicate(predicate) {
        // Need to advance iterator to first matching element.
        if (_underlyingIterator && !Usd_EvalPredicate(_predicate, _underlyingIterator, _proxyPrimPath)) increment();
    }

    bool equal(const UsdPrimSiblingIterator& other) const {
        return _underlyingIterator == other._underlyingIterator && _proxyPrimPath == other._proxyPrimPath &&
               _predicate == other._predicate;
    }

    void increment() {
        if (Usd_MoveToNextSiblingOrParent(_underlyingIterator, _proxyPrimPath, _predicate)) {
            _underlyingIterator = nullptr;
            _proxyPrimPath = SdfPath();
        }
    }

    reference dereference() const { return UsdPrim(_underlyingIterator, _proxyPrimPath); }
...
public:
    ...
    UsdPrimSiblingRange(UsdPrimSiblingIterator begin, UsdPrimSiblingIterator end) : _begin(begin), _end(end) {}
    ...
    /// Equality compare.
    bool equal(const UsdPrimSiblingRange& other) const { return _begin == other._begin && _end == other._end; }
...
private:
    /// Equality comparison.
    friend bool operator==(const UsdPrimSiblingRange& lhs, const UsdPrimSiblingRange& rhs) { return lhs.equal(rhs); }

    /// Equality comparison.
    template <class ForwardRange>
    friend bool operator==(const UsdPrimSiblingRange& lhs, const ForwardRange& rhs) {
        static_assert(
                std::is_same<typename decltype(std::cbegin(rhs))::iterator_category, std::forward_iterator_tag>::value,
                "rhs must be a forward iterator.");
        return (std::distance(std::cbegin(lhs), std::cend(lhs)) == std::distance(std::cbegin(rhs), std::cend(rhs))) &&
...
template <>
struct Tf_ShouldIterateOverCopy<UsdPrimSiblingRange> : std::true_type {};
template <>
struct Tf_ShouldIterateOverCopy<const UsdPrimSiblingRange> : std::true_type {};
...
UsdPrimSiblingRange UsdPrim::GetAllChildren() const {
    return GetFilteredChildren(UsdPrimAllPrimsPredicate);
}

UsdPrimSiblingRange UsdPrim::GetChildren() const {
    return GetFilteredChildren(UsdPrimDefaultPredicate);
}

// Helper to make a sibling range.
UsdPrim::SiblingRange UsdPrim::_MakeSiblingRange(const Usd_PrimFlagsPredicate& pred) const {
    ...
    return SiblingRange(SiblingIterator(firstChild, firstChildPath, pred), SiblingIterator(nullptr, SdfPath(), pred));
}
...
public:
    ...
    /// Equality.
    bool operator==(const UsdPrimSubtreeIterator& other) const { return equal(other); }
...
private:
    ...
    UsdPrimSubtreeIterator(const _UnderlyingIterator& i,
        ...
        : _underlyingIterator(i), _proxyPrimPath(proxyPrimPath), _predicate(predicate) {
        // Need to advance iterator to first matching element.
        if (_underlyingIterator && !Usd_EvalPredicate(_predicate, _underlyingIterator, _proxyPrimPath)) {
            if (Usd_MoveToNextSiblingOrParent(_underlyingIterator, _proxyPrimPath, _predicate)) {
                _underlyingIterator = nullptr;
                _proxyPrimPath = SdfPath();
            }
    ...
    void increment() {
        if (!Usd_MoveToChild(_underlyingIterator, _proxyPrimPath, _predicate)) {
            while (Usd_MoveToNextSiblingOrParent(_underlyingIterator, _proxyPrimPath, _predicate)) {
            }
        }
    }
...
public:
    ...
    /// Construct with a pair of iterators.
    UsdPrimSubtreeRange(UsdPrimSubtreeIterator begin, UsdPrimSubtreeIterator end) : _begin(begin), _end(end) {}
    ...
    /// Equality compare.
    bool equal(const UsdPrimSubtreeRange& other) const { return _begin == other._begin && _end == other._end; }
...
private:
    /// Equality comparison.
    friend bool operator==(const UsdPrimSubtreeRange& lhs, const UsdPrimSubtreeRange& rhs) { return lhs.equal(rhs); }

    /// Equality comparison.
    template <class ForwardRange>
    friend bool operator==(const UsdPrimSubtreeRange& lhs, const ForwardRange& rhs) {
        static_assert(std::is_convertible<typename decltype(std::cbegin(rhs))::iterator_category,
...
// Inform TfIterator it should feel free to make copies of the range type.
template <>
struct Tf_ShouldIterateOverCopy<UsdPrimSubtreeRange> : std::true_type {};
template <>
struct Tf_ShouldIterateOverCopy<const UsdPrimSubtreeRange> : std::true_type {};

UsdPrimSubtreeRange UsdPrim::GetFilteredDescendants(const Usd_PrimFlagsPredicate& pred) const {
    return _MakeDescendantsRange(Usd_CreatePredicateForTraversal(_Prim(), _ProxyPrimPath(), pred));
}

UsdPrimSubtreeRange UsdPrim::GetAllDescendants() const {
    return GetFilteredDescendants(UsdPrimAllPrimsPredicate);
}

UsdPrimSubtreeRange UsdPrim::GetDescendants() const {
    return GetFilteredDescendants(UsdPrimDefaultPredicate);
}

// Helper to make a sibling range.
UsdPrim::SubtreeRange UsdPrim::_MakeDescendantsRange(const Usd_PrimFlagsPredicate& pred) const {
    Usd_PrimDataConstPtr firstChild = get_pointer(_Prim());
    SdfPath firstChildPath = _ProxyPrimPath();
...
inline UsdPrim UsdObject::GetPrim() const {
