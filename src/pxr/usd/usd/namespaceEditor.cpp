...
struct _PropertySpecWithAuthoredTargetsInfo {
    ...
    SdfPathListOp GetTargetListOp() const {
        ...
        if (!layer->HasField(path, fieldName, &listOp)) {
            TF_CODING_ERROR(
                    "Spec at site @%s@<%s> is expected to have a "
                    "path list op for field %s",
                    layer->GetIdentifier().c_str(), path.GetText(), fieldName.GetText());
        }
        return listOp;
    }
};

using _PropertySpecWithAuthoredTargetsVector = std::vector<_PropertySpecWithAuthoredTargetsInfo>;
...
public:
    ...
    // the given stage
    static _TargetingPropertyDependencies GetDependencies(const UsdStageRefPtr& stage) {
...
private:
    ...
    explicit _TargetingPropertyDependencyCollector()
        : _consumerTask(_dispatcher, [this]() {
              _ConsumerTask();
          }) {}
    ...
    void _Run(const UsdStageRefPtr& stage) {
        WorkWithScopedParallelism([this, &stage]() {
            ...
            WorkParallelForEach(range.begin(), range.end(), [this](UsdPrim const& prim) {
                _VisitPrim(prim);
            });
            _dispatcher.Wait();
        });
    ...
    void _VisitPrim(UsdPrim const& prim) {
        std::unordered_map<SdfPath, _WorkQueueEntry, TfHash> workEntriesPerProperty;
        ...
        // order.
        for (Usd_Resolver res(&(prim.GetPrimIndex())); res.IsValid(); res.NextLayer()) {
            ...
            if (!layer->HasField(primSpecPath, SdfChildrenKeys->PropertyChildren, &primSpecPropertyNames)) {
                continue;
            }
            ...
            for (const TfToken& propName : primSpecPropertyNames) {
                ...
                if (specType == SdfSpecTypeAttribute) {
                    ...
                    TF_CODING_ERROR(
                            "Spec type for property child of at site "
                            "@%s@<%s> is not an attribute or relationship",
                            layer->GetIdentifier().c_str(), localPropPath.GetText());
                    continue;
                }
                ...
                if (!layer->HasField<SdfPathListOp>(localPropPath, targetPathListOpField, &targetPathsListOp)) {
                    continue;
                }

                // Add or get the work entry for the composed property path so
                // we can add this spec's info to it.
                _WorkQueueEntry& workEntry =
                        workEntriesPerProperty.emplace(prim.GetPrimPath().AppendProperty(propName), _WorkQueueEntry())
                                .first->second;
                ...
                // necessary.
                auto collectMappedPathsFn = [&](const SdfPathListOp::ItemVector& items) {
                    if (node.IsRootNode() || node.GetMapToRoot().IsIdentity()) {
                        workEntry.targetedPaths.insert(items.begin(), items.end());
                    } else {
                        for (const auto& item : items) {
                            SdfPath mappedItem = node.GetMapToRoot().MapSourceToTarget(item);
                            if (!mappedItem.IsEmpty()) {
                                workEntry.targetedPaths.insert(std::move(mappedItem));
                            }
                        }
                    }
                };
                ...
                workEntry.propSpecsWithAuthoredTargets.push_back(
                        {layer, std::move(localPropPath), std::move(targetPathListOpField), node});
            }
        }
    ...
    void _ConsumerTask() {
        ...
        while (_workQueue.try_pop(queueEntry)) {
            ...
            _result.composedPropertyToSpecsWithAuthoredTargetsMap.emplace(
                    queueEntry.composedPropertyPath, std::move(queueEntry.propSpecsWithAuthoredTargets));
            ...
            for (const auto& targetedPath : queueEntry.targetedPaths) {
                _result.targetedPathToTargetingPropertiesPathTable[targetedPath].push_back(
...
static std::string _GetErrorString(const std::vector<std::string>& errors) {
    return TfStringJoin(errors, "; ");
}

UsdNamespaceEditor::UsdNamespaceEditor(const UsdStageRefPtr& stage) : _stage(stage) {}

UsdNamespaceEditor::UsdNamespaceEditor(const UsdStageRefPtr& stage, EditOptions&& editOptions)
    : _stage(stage), _editOptions(std::move(editOptions)) {}

UsdNamespaceEditor::UsdNamespaceEditor(const UsdStageRefPtr& stage, const EditOptions& editOptions)
    : _stage(stage), _editOptions(editOptions) {}

void UsdNamespaceEditor::AddDependentStage(const UsdStageRefPtr& stage) {
...
void UsdNamespaceEditor::RemoveDependentStage(const UsdStageRefPtr& stage) {
    _ClearProcessedEdits();
    _dependentStages.erase(stage);
}

void UsdNamespaceEditor::SetDependentStages(const UsdStageRefPtrVector& stages) {
    for (const auto& stage : stages) {
        AddDependentStage(stage);
    }
}

bool UsdNamespaceEditor::DeletePrimAtPath(const SdfPath& path) {
    return _AddPrimDelete(path);
}

bool UsdNamespaceEditor::MovePrimAtPath(const SdfPath& path, const SdfPath& newPath) {
    return _AddPrimMove(path, newPath);
}

bool UsdNamespaceEditor::DeletePrim(const UsdPrim& prim) {
    return _AddPrimDelete(prim.GetPrimPath());
}

bool UsdNamespaceEditor::RenamePrim(const UsdPrim& prim, const TfToken& newName) {
    return _AddPrimMove(prim.GetPrimPath(), prim.GetPrimPath().ReplaceName(newName));
}

bool UsdNamespaceEditor::ReparentPrim(const UsdPrim& prim, const UsdPrim& newParent) {
    return _AddPrimMove(prim.GetPrimPath(), newParent.GetPrimPath().AppendChild(prim.GetName()));
}

bool UsdNamespaceEditor::ReparentPrim(const UsdPrim& prim, const UsdPrim& newParent, const TfToken& newName) {
    return _AddPrimMove(prim.GetPrimPath(), newParent.GetPrimPath().AppendChild(newName));
}

bool UsdNamespaceEditor::DeletePropertyAtPath(const SdfPath& path) {
    return _AddPropertyDelete(path);
}

bool UsdNamespaceEditor::MovePropertyAtPath(const SdfPath& path, const SdfPath& newPath) {
    return _AddPropertyMove(path, newPath);
}

bool UsdNamespaceEditor::DeleteProperty(const UsdProperty& property) {
    return _AddPropertyDelete(property.GetPath());
}

bool UsdNamespaceEditor::RenameProperty(const UsdProperty& property, const TfToken& newName) {
    return _AddPropertyMove(property.GetPath(), property.GetPath().ReplaceName(newName));
}

bool UsdNamespaceEditor::ReparentProperty(const UsdProperty& property, const UsdPrim& newParent) {
    return _AddPropertyMove(property.GetPath(), newParent.GetPrimPath().AppendProperty(property.GetName()));
}

bool UsdNamespaceEditor::ReparentProperty(const UsdProperty& property,
                                          const UsdPrim& newParent,
                                          const TfToken& newName) {
    return _AddPropertyMove(property.GetPath(), newParent.GetPrimPath().AppendProperty(newName));
}

bool UsdNamespaceEditor::ApplyEdits() {
...
bool UsdNamespaceEditor::CanApplyEdits(std::string* whyNot) const {
    _ProcessEditsIfNeeded();
    if (!_processedEdit) {
        TF_CODING_ERROR("Failed to process edits");
        return false;
...
static bool _IsValidPrimEditPath(const SdfPath& path) {
    return path.IsPrimPath() && path.IsAbsolutePath() && !path.ContainsPrimVariantSelection();
}

static bool _IsValidPropertyEditPath(const SdfPath& path) {
    return path.IsPrimPropertyPath() && path.IsAbsolutePath() && !path.ContainsPrimVariantSelection();
}

bool UsdNamespaceEditor::_AddPrimDelete(const SdfPath& oldPath) {
    ...
    if (!_IsValidPrimEditPath(oldPath)) {
        TF_CODING_ERROR(
                "Invalid path '%s' provided as the source for "
                "a prim namespace edit.",
                oldPath.GetText());
...
bool UsdNamespaceEditor::_AddPrimMove(const SdfPath& oldPath, const SdfPath& newPath) {
    // We always clear the processed edits when a new edit is added.
    _ClearProcessedEdits();
    ...
    if (!_IsValidPrimEditPath(newPath)) {
        TF_CODING_ERROR(
                "Invalid path '%s' provided as the destination for a "
                "prim namespace edit.",
                newPath.GetText());
        _editDescription.editType = _EditType::Invalid;
        return false;
...
bool UsdNamespaceEditor::_AddPropertyDelete(const SdfPath& oldPath) {
    // We always clear the processed edits when a new edit is added.
    _ClearProcessedEdits();
...
bool UsdNamespaceEditor::_AddPropertyMove(const SdfPath& oldPath, const SdfPath& newPath) {
    // We always clear the processed edits when a new edit is added.
    _ClearProcessedEdits();
...
void UsdNamespaceEditor::_ClearProcessedEdits() {
...
public:
    ...
    static UsdNamespaceEditor::_ProcessedEdit ProcessEdit(const UsdStageRefPtr& stage,
...
private:
    _EditProcessor(const UsdStageRefPtr& stage,
    ...
    void _ProcessPrimEditRequiresRelocates(const PcpPrimIndex& primIndex);

    void _ProcessPropEditRequiresRelocates(const PcpPrimIndex& primIndex);
...
void UsdNamespaceEditor::_ProcessEditsIfNeeded() const {
    ...
    _processedEdit =
            UsdNamespaceEditor::_EditProcessor::ProcessEdit(_stage, _dependentStages, _editDescription, _editOptions);
}

static bool _IsValidPrimToEdit(const UsdPrim& prim, std::string* whyNot = nullptr) {
    ...
    if (prim.IsInstanceProxy()) {
        if (whyNot) {
            *whyNot =
                    "The prim to edit is a prototype proxy descendant of an "
                    "instance prim";
        }
        return false;
    }
...
static bool _IsValidPropertyToEdit(const UsdPrim& prim, const TfToken& propertyName, std::string* whyNot = nullptr) {
    ...
    if (prim.IsInstanceProxy()) {
        if (whyNot) {
            *whyNot =
                    "The property to edit belongs to an instance prototype "
                    "proxy";
        }
        return false;
    }
...
static bool _IsValidNewParentPath(const UsdStageRefPtr& stage,
    ...
    if (newParentPrim.IsInstanceProxy()) {
        if (whyNot) {
            *whyNot =
                    "The new parent prim is a prototype proxy descendant "
                    "of an instance prim";
        }
        return false;
    }
    ...
    if (pathToEdit.IsPrimPropertyPath()) {
        ...
        if (newParentPrim.IsPseudoRoot()) {
            if (whyNot) {
                *whyNot =
                        "The new parent prim for a property cannot be the "
                        "pseudo-root";
            }
            return false;
        }
        ...
        if (newParentPrim.IsInstance()) {
            if (whyNot) {
                *whyNot =
                        "The new parent prim is an instance prim whose "
        ...
        if (newParentPath.HasPrefix(pathToEdit)) {
            if (whyNot) {
                *whyNot =
                        "The new parent prim is a descendant of the prim to "
                        "move";
            }
            return false;
        }
...
UsdNamespaceEditor::_ProcessedEdit UsdNamespaceEditor::_EditProcessor::ProcessEdit(const UsdStageRefPtr& stage,
                                                                                   const _StageSet& dependentStages,
...
UsdNamespaceEditor::_EditProcessor::_EditProcessor(const UsdStageRefPtr& stage,
                                                   const _StageSet& dependentStages,
                                                   const UsdNamespaceEditor::_EditDescription& editDesc,
                                                   const UsdNamespaceEditor::EditOptions& editOptions,
                                                   _ProcessedEdit* processedEdit)
...
bool UsdNamespaceEditor::_EditProcessor::_ProcessNewPath() {
    ...
    if (_stage->GetObjectAtPath(_editDesc.newPath)) {
        _processedEdit->errors.push_back("An object already exists at the new path");
        return false;
    }
    ...
    if (_editDesc.editType == _EditType::Reparent) {
        ...
        // reparent to.
        std::string whyNot;
        if (!_IsValidNewParentPath(_stage, _editDesc.oldPath, _editDesc.newPath.GetParentPath(), &whyNot)) {
    ...
    UsdPrim newParentPrim = _stage->GetPrimAtPath(_editDesc.newPath.GetParentPath());
    if (!newParentPrim) {
        TF_CODING_ERROR("Parent prim at path %s does not exist", _editDesc.newPath.GetParentPath().GetText());
        return false;
    }
    ...
    newParentPrimIndex.ComputePrimChildNames(&childNames, &prohibitedChildren);
    ...
    const SdfRelocatesMap& localRelocates = _nodeForEditTarget.GetLayerStack()->GetIncrementalRelocatesSourceToTarget();
    ...
    _processedEdit->errors.push_back(
            "The new path is a prohibited child of "
            "its parent path because of existing relocates.");
    return false;
}

void UsdNamespaceEditor::_EditProcessor::_ProcessPrimEditRequiresRelocates(const PcpPrimIndex& primIndex) {
    const bool requiresRelocatesAuthoring = [&]() {
        ...
        const SdfRelocatesMap& targetToSourceRelocates =
                _nodeForEditTarget.GetLayerStack()->GetIncrementalRelocatesTargetToSource();
...
void UsdNamespaceEditor::_EditProcessor::_ProcessPropEditRequiresRelocates(const PcpPrimIndex& primIndex) {
    ...
    PcpNodeRange subtreeRange = primIndex.GetNodeSubtreeRange(_nodeForEditTarget);
    ...
    for (const PcpNodeRef& subtreeNode : subtreeRange) {
        ...
        SdfPath mappedPropertyPath = subtreeNode.GetPath().AppendProperty(propName);
        ...
        const SdfLayerRefPtrVector& layers = subtreeNode.GetLayerStack()->GetLayers();
        const bool hasPropertySpecs = std::any_of(layers.begin(), layers.end(), [&](const auto& layer) {
...
void UsdNamespaceEditor::_EditProcessor::_GatherLayersToEdit() {
    // Get all the layers in the layer stack where the edits will be performed.
    const SdfLayerRefPtrVector& layers = _nodeForEditTarget.GetLayerStack()->GetLayers();
    ...
    if (!_editTarget.GetMapFunction().IsIdentityPathMapping()) {
        _processedEdit->errors.push_back(
                "Edit targets that map paths across "
                "composition arcs are not currently supported for namespace "
                "editing");
        return;
    }
    if (std::find(layers.begin(), layers.end(), _editTarget.GetLayer()) == layers.end()) {
        _processedEdit->errors.push_back(
                "Edit targets with layers outside of "
    ...
    _processedEdit->layersToEdit = PcpGatherLayersToEditForSpecMove(
            _nodeForEditTarget.GetLayerStack(), _editDesc.oldPath, _editDesc.newPath, &_processedEdit->errors);
}

void UsdNamespaceEditor::_EditProcessor::_GatherTargetListOpEdits() {
    ...
    // namespace path.
    _TargetingPropertyDependencies deps = _TargetingPropertyDependencyCollector::GetDependencies(_stage);
    ...
    const auto range = deps.targetedPathToTargetingPropertiesPathTable.FindSubtreeRange(_editDesc.oldPath);
    ...
    for (const SdfPath& propertyPath : propPathsWithAffectedTargets) {
        ...
        const _PropertySpecWithAuthoredTargetsVector* propertySpecs =
                TfMapLookupPtr(deps.composedPropertyToSpecsWithAuthoredTargetsMap, propertyPath);
        ...
        for (const auto& specInfo : *propertySpecs) {
            ...
            if (targetListOp.ModifyOperations([&](const SdfPath& path) {
                    ...
                    // new path.
                    return std::optional<SdfPath>(path.ReplacePrefix(_editDesc.oldPath, _editDesc.newPath));
                })) {
                // If the target list op was modified, add the edit we need
                // to perform for this spec in the processed edit.
                _processedEdit->targetPathListOpEdits.push_back({specInfo.layer->GetPropertyAtPath(specInfo.path),
                                                                 specInfo.fieldName, std::move(targetListOp)});
            }
        }
        ...
        for (auto rIt = propertySpecs->rbegin(); rIt != propertySpecs->rend(); ++rIt) {
            ...
            rIt->GetTargetListOp().ApplyOperations(
                    &targetsRequireRelocates, [&](SdfListOpType opType, const SdfPath& inPath) {
                        const SdfPath translatedPath = rIt->originatingNode.GetMapToRoot().MapSourceToTarget(inPath);
                        // Skip paths that don't map. Also skip paths that aren't
                        // affected by the namespace edit; we don't care about these
                        // either.
                        if (translatedPath.IsEmpty() || !translatedPath.HasPrefix(_editDesc.oldPath)) {
                            return std::optional<SdfPath>();
                        }
        ...
        if (!targetsRequireRelocates.empty()) {
            const bool isAttribute = _stage->GetObjectAtPath(propertyPath).Is<UsdAttribute>();
            _processedEdit->targetPathListOpErrors.push_back(TfStringPrintf(
                    ...
                    isAttribute ? "connection" : "relationship", TfStringify(targetsRequireRelocates).c_str(),
                    isAttribute ? "attribute" : "relationship", propertyPath.GetText(), _editDesc.oldPath.GetText(),
                    _editDesc.IsPropertyEdit() ? "properties ever"
                                               : "prims that do not have opinions across composition arcs"));
        }
    }
}

bool UsdNamespaceEditor::_ProcessedEdit::CanApply(std::string* whyNot) const {
...
static bool _ApplyLayerSpecMove(const SdfLayerHandle& layer, const SdfPath& oldPath, const SdfPath& newPath) {
    ...
    auto applyEditsToLayersFn = [&](const SdfPath& createParentSpecIfNeededPath) {
        ...
        if (!createParentSpecIfNeededPath.IsEmpty() && !SdfJustCreatePrimInLayer(layer, createParentSpecIfNeededPath)) {
            TF_CODING_ERROR(
                    "Failed to find or create new parent spec "
                    "at path '%s' on layer '%s' which is necessary to "
                    "apply edits. The edit will be incomplete.",
                    createParentSpecIfNeededPath.GetText(), layer->GetIdentifier().c_str());
            return false;
        }
        ...
        if (!layer->Apply(batchEdit)) {
            TF_CODING_ERROR(
                    "Failed to apply batch edit '%s' on layer '%s' "
                    "which is necessary to apply edits. The edit will be "
                    "incomplete.",
                    TfStringify(batchEdit.GetEdits()).c_str(), layer->GetIdentifier().c_str());
            return false;
        }
    ...
    const bool isReparent = !newPath.IsEmpty() && newPath.GetParentPath() != oldPath.GetParentPath();
    if (isReparent) {
...
bool UsdNamespaceEditor::_ProcessedEdit::Apply() {
    ...
    if (std::string errorMsg; !CanApply(&errorMsg)) {
        TF_CODING_ERROR(
                TfStringPrintf("Failed to apply edits to the stage "
    ...
    if (editDescription.IsPropertyEdit()) {
        ...
        for (const auto& layer : layersToEdit) {
            _ApplyLayerSpecMove(layer, editDescription.oldPath, editDescription.newPath);
        }
    } else {
        ...
        for (const auto& [layer, editVec] : dependentStageNamespaceEdits.layerSpecMoves) {
            for (const auto& edit : editVec) {
                _ApplyLayerSpecMove(layer, edit.oldPath, edit.newPath);
            }
        }

        for (const auto& edit : dependentStageNamespaceEdits.compositionFieldEdits) {
            edit.layer->SetField(edit.path, edit.fieldName, edit.newFieldValue);
        }

        for (const auto& [layer, relocates] : dependentStageNamespaceEdits.dependentRelocatesEdits) {
...
void UsdNamespaceEditor::_EditProcessor::_GatherDependentStageEdits() {
    ...
    const PcpLayerStackRefPtr& addRelocatesToLayerStack =
            _processedEdit->willAuthorRelocates ? _nodeForEditTarget.GetLayerStack() : PcpLayerStackRefPtr();

    // Gather all the dependent edits for all stage PcpCaches.
    _processedEdit->dependentStageNamespaceEdits =
            PcpGatherDependentNamespaceEdits(_editDesc.oldPath, _editDesc.newPath, _processedEdit->layersToEdit,
                                             addRelocatesToLayerStack, _editTarget.GetLayer(), dependentCaches);
