...
using StagesByRootLayer = std::unordered_multimap<SdfLayerHandle, UsdStageRefPtr, TfHash>;
...
public:
    ...
    size_t EraseAll(const SdfLayerHandle& rootLayer, vector<Entry>* erased) {
        return _EraseAllIf(
                rootLayer,
                [](const auto& stage) {
                    return true;
    ...
    size_t EraseAll(const SdfLayerHandle& rootLayer, const SdfLayerHandle& sessionLayer, vector<Entry>* erased) {
        return _EraseAllIf(
                rootLayer,
...
private:
    ...
    void _EraseRootLayerEntry(const UsdStageRefPtr& stage) {
        const auto range = _byRootLayer.equal_range(stage->GetRootLayer());
        const auto it = std::find_if(range.first, range.second, [&stage](const auto& element) {
    ...
    size_t _EraseAllIf(const SdfLayerHandle& rootLayer, ConditionFn&& conditionFn, vector<Entry>* erased) {
        ...
        for (auto it = range.first; it != range.second;) {
            if (conditionFn(it->second)) {
                ...
                if (byStageIt == _byStage.cend()) {
                    ...
                    if (erased) {
                        erased->emplace_back(byStageIt->first, byStageIt->second);
                    }
...
struct DebugHelper {
    ...
    void IssueMessage() const {
        if (_entries.size() == 1) {
            DBG("%s %s %s (id=%s)\n", UsdDescribe(_cache).c_str(), _prefix, UsdDescribe(_entries.front().stage).c_str(),
...
vector<UsdStageRefPtr> UsdStageCache::GetAllStages() const {
    ...
    std::transform(byStage.cbegin(), byStage.cend(), result.begin(), [](const auto& entry) {
        return entry.first;
    });
    return result;
}
...
std::pair<UsdStageRefPtr, bool> UsdStageCache::RequestStage(UsdStageCacheRequest&& request) {
...

    {
        ...
        if (!mailbox.Subscribed()) _impl->pendingRequests.push_back(&request);
    ...
    if (mailbox.Subscribed()) return std::make_pair(mailbox.Wait(), false);
...
UsdStageRefPtr UsdStageCache::FindOneMatching(const SdfLayerHandle& rootLayer) const {
    ...
    DBG("%s by rootLayer%s in %s\n", (result ? FMT("found %s", UsdDescribe(result).c_str()) : "failed to find stage"),
        (result ? "" : FMT(" @%s@", rootLayer->GetIdentifier().c_str())), UsdDescribe(*this).c_str());
...
UsdStageRefPtr UsdStageCache::FindOneMatching(const SdfLayerHandle& rootLayer,
    ...
    DBG("%s by rootLayer%s, sessionLayer%s in %s\n",
        ...
        (result ? "" : (!sessionLayer ? " <null>" : FMT(" @%s@", sessionLayer->GetIdentifier().c_str()))),
...
std::vector<UsdStageRefPtr> UsdStageCache::FindAllMatching(const SdfLayerHandle& rootLayer,
                                                           const SdfLayerHandle& sessionLayer) const {
    LockGuard lock(_mutex);
    const StagesByRootLayer& byRootLayer = _impl->stages.ByRootLayer();
    vector<UsdStageRefPtr> result;
    ...
    for (auto entryIt = range.first; entryIt != range.second; ++entryIt) {
        const auto& entry = *entryIt;
        if (entry.second->GetSessionLayer() == sessionLayer) result.push_back(entry.second);
    }
    return result;
}
...
std::vector<UsdStageRefPtr> UsdStageCache::FindAllMatching(const SdfLayerHandle& rootLayer,
    ...
    for (auto entryIt = range.first; entryIt != range.second; ++entryIt) {
        const auto& entry = *entryIt;
        if (entry.second->GetPathResolverContext() == pathResolverContext) result.push_back(entry.second);
    }
    return result;
}
...
size_t UsdStageCache::EraseAll(const SdfLayerHandle& rootLayer, const SdfLayerHandle& sessionLayer) {
    ...
    {
        LockGuard lock(_mutex);
        numErased = _impl->stages.EraseAll(rootLayer, sessionLayer, debug.GetEntryVec());
    }
    return numErased;
}
...
size_t UsdStageCache::EraseAll(const SdfLayerHandle& rootLayer,
    ...
    {
        LockGuard lock(_mutex);
        numErased = _impl->stages.EraseAll(rootLayer, sessionLayer, pathResolverContext, debug.GetEntryVec());
    }
    return numErased;
}
...
string UsdDescribe(const UsdStageCache& cache) {
    return TfStringPrintf(
            ...
            (cache.GetDebugName().empty() ? FMT("%p", &cache) : FMT("\"%s\"", cache.GetDebugName().c_str())),
            cache.Size());
}
...
void UsdStageCacheRequest::_Subscribe(_Mailbox* mailbox) {
    if (!_data) _data.reset(new _Data);
