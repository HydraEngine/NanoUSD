...
UsdValidationErrorSite::UsdValidationErrorSite(const SdfLayerHandle& layer, const SdfPath& objectPath)
...
UsdValidationError::UsdValidationError() : _errorType(UsdValidationErrorType::None) {
    _validator = nullptr;
}

UsdValidationError::UsdValidationError(const TfToken& name,
                                       const UsdValidationErrorType& type,
                                       const UsdValidationErrorSites& errorSites,
                                       const std::string& errorMsg)
    : _name(name), _errorType(type), _errorSites(errorSites), _errorMsg(errorMsg) {
    _validator = nullptr;
}

TfToken UsdValidationError::GetIdentifier() const {
    ...
    if (!_validator) {
        TF_CODING_ERROR(
                "Validator not set on ValidationError. Possibly this "
    ...
    return TfToken(_validator->GetMetadata().name.GetString() + _tokens->ValidationErrorNameDelimiter.GetString() +
                   _name.GetString());
}

std::string UsdValidationError::GetErrorAsString() const {
    return _errorType == UsdValidationErrorType::None
                   ? _errorMsg
                   : TfStringPrintf("%s: %s", TfEnum::GetDisplayName(_errorType).c_str(), _errorMsg.c_str());
}

void UsdValidationError::_SetValidator(const UsdValidator* validator) {
