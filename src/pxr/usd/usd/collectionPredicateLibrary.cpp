...
static inline bool _IsStrict(FnArgs const& args, bool defaultStrict = false) {
        ...
        if (arg.argName == "strict") {
            ...
            if (arg.value.IsHolding<std::string>()) {
                ...
                if (!str.empty()) {
                    ...
                    return firstChar == '1' || firstChar == 'y' || firstChar == 'Y';
                }
            }
            return false;
        }
...
static UsdObjectPredicateLibrary* _MakeCollectionPredicateLibrary() {
    ...
    lib.Define("abstract",
               [](UsdObject const& obj, bool abstract) {
            ...
            .Define("defined",
                    [](UsdObject const& obj, bool defined) {
            ...
            .Define("model",
                    [](UsdObject const& obj, bool model) {
            ...
            .Define("group",
                    [](UsdObject const& obj, bool group) {
                        if (UsdPrim const& prim = obj.As<UsdPrim>()) {
                              ...
                              return [queryKinds, checkSubKinds](UsdObject const& obj) {
                                  if (UsdPrim const& prim = obj.As<UsdPrim>()) {
                                      ...
                                      for (TfToken const& queryKind : queryKinds) {
                                          if (checkSubKinds ? KindRegistry::IsA(primKind, queryKind)
                                                            : primKind == queryKind) {
            ...
            .DefineBinder("specifier",
                          [](FnArgs const& args) -> PredicateFunction {
                              ...
                              for (FnArg const& arg : args) {
                                  ...
                                  std::string const& argVal = arg.value.UncheckedGet<std::string>();
                                  if (argVal == "over") {
            ...
            .DefineBinder("isa",
                          [](FnArgs const& args) -> PredicateFunction {
                              ...
                              for (FnArg const& arg : args) {
                                  if (arg.argName.empty() && arg.value.IsHolding<std::string>()) {
                                      TfType schemaType = UsdSchemaRegistry::GetTypeFromSchemaTypeName(
                              ...
                              return [queryTypes, exactMatch](UsdObject const& obj) {
                                  if (UsdPrim const& prim = obj.As<UsdPrim>()) {
                                      TfType const& primType = prim.GetPrimTypeInfo().GetSchemaType();
                                      for (TfType const& queryType : queryTypes) {
                                          if (exactMatch ? primType == queryType : primType.IsA(queryType)) {
                                              return PredResult::MakeVarying(true);
                                          }
                                      }
                                      return PredResult::MakeVarying(false);
                          ...
                          [](FnArgs const& args) -> PredicateFunction {
                              ...
                              for (FnArg const& arg : args) {
                                  if (arg.argName == "instanceName") {
                                      if (arg.value.IsHolding<std::string>()) {
                                          instanceName = TfToken(arg.value.UncheckedGet<std::string>());
                                          break;
                                      }
                                      return nullptr;
                                  }
            ...
            .DefineBinder("variant", [](FnArgs const& args) -> PredicateFunction {
                ...
                for (FnArg const& arg : args) {
                    ...
                    std::string const& selStr = arg.value.UncheckedGet<std::string>();
                ...
                return [exactSels, globSels](UsdObject const& obj) {
                    ...
                    if (UsdPrim const& prim = obj.As<UsdPrim>()) {
                        ...
                        for (auto const& setNsel : exactSels) {
                            if (vsets.GetVariantSelection(setNsel.first) != setNsel.second) {
                                return PredResult::MakeVarying(false);
                            }
                        }
                        for (auto const& setNglob : globSels) {
                            if (!setNglob.second.Match(vsets.GetVariantSelection(setNglob.first))) {
                                return PredResult::MakeVarying(false);
                            }
                        }
                        return PredResult::MakeVarying(true);
...
UsdObjectPredicateLibrary const& UsdGetCollectionPredicateLibrary() {
    static UsdObjectPredicateLibrary* theLibrary = _MakeCollectionPredicateLibrary();
    return *theLibrary;
}
