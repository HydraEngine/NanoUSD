...
bool Usd_UntypedInterpolator::Interpolate(
        const SdfLayerRefPtr& layer, const SdfPath& path, double time, double lower, double upper) {
...
template <class Src>
bool Usd_UntypedInterpolator::_Interpolate(
        const Src& src, const SdfPath& path, double time, double lower, double upper) {
    ...
    if (!attrValueType) {
        TF_RUNTIME_ERROR("Unknown value type '%s' for attribute '%s'", _attr.GetTypeName().GetAsToken().GetText(),
