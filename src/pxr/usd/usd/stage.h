...
public:
    ...
    /// information.
    USD_API
    UsdPrim Load(const SdfPath& path = SdfPath::AbsoluteRootPath(), UsdLoadPolicy policy = UsdLoadWithDescendants);
...
public:
    ...
    USD_API
    bool HasMetadataDictKey(const TfToken& key, const TfToken& keyPath) const;
    ...
    /// \sa \ref Usd_Dictionary_Type
    USD_API
    bool ClearMetadataByDictKey(const TfToken& key, const TfToken& keyPath) const;
    ...
    /// \ref Usd_ColorConfigurationAPI "Color Configuration API"
    USD_API
    static void GetColorConfigFallbacks(SdfAssetPath* colorConfiguration, TfToken* colorManagementSystem);
    ...
    /// \ref Usd_ColorConfigurationAPI "Color Configuration API"
    USD_API
    static void SetColorConfigFallbacks(const SdfAssetPath& colorConfiguration, const TfToken& colorManagementSystem);
...
private:
    ...
    static std::vector<std::pair<SdfPrimSpecHandle, SdfLayerOffset>> _GetPrimStackWithLayerOffsets(const UsdPrim& prim);
    ...
    SdfAttributeSpecHandle _CreateNewSpecFromSchemaAttribute(const UsdPrim& prim,
                                                             const UsdPrimDefinition::Attribute& attrDef);

    SdfRelationshipSpecHandle _CreateNewSpecFromSchemaRelationship(const UsdPrim& prim,
    ...
    UsdProperty _FlattenProperty(const UsdProperty& srcProp, const UsdPrim& dstParent, const TfToken& dstName);
    ...
    struct _IsEditTargetMappable {
        static const bool value = std::is_same<T, SdfTimeCode>::value || std::is_same<T, VtArray<SdfTimeCode>>::value ||
                                  std::is_same<T, SdfPathExpression>::value ||
                                  std::is_same<T, VtArray<SdfPathExpression>>::value ||
                                  std::is_same<T, SdfTimeSampleMap>::value || std::is_same<T, TsSpline>::value ||
                                  std::is_same<T, VtDictionary>::value;
    };

    // Set value for types that don't need to be mapped for edit targets.
    template <class T>
    typename std::enable_if<!_IsEditTargetMappable<T>::value, bool>::type _SetValue(UsdTimeCode time,
    ...
    template <class T>
    typename std::enable_if<!_IsEditTargetMappable<T>::value, bool>::type _SetMetadata(const UsdObject& object,
                                                                                       const TfToken& key,
    ...
    USD_API
    bool _SetMetadata(const UsdObject& object, const TfToken& key, const TfToken& keyPath, const VtValue& value);
    ...
    template <class T>
    bool _SetMetadataImpl(const UsdObject& obj, const TfToken& fieldName, const TfToken& keyPath, const T& value);

    bool _ClearMetadata(const UsdObject& obj, const TfToken& fieldName, const TfToken& keyPath = TfToken());
    ...
    void _ComposeChildSubtree(Usd_PrimDataPtr prim, Usd_PrimDataConstPtr parent, UsdStagePopulationMask const* mask);
    ...
    template <class Iter>
    void _ComputeSubtreesToRecompose(Iter start, Iter finish, std::vector<Usd_PrimDataPtr>* recompose);
    ...
    void _MakeResolvedAttributeValue(UsdTimeCode time, const UsdAttribute& attr, VtValue* value) const;
...
public:
    ...
    struct _HasTypeSpecificResolution {
        static const bool value =
                std::is_same<T, SdfAssetPath>::value || std::is_same<T, VtArray<SdfAssetPath>>::value ||
                std::is_same<T, SdfTimeCode>::value || std::is_same<T, VtArray<SdfTimeCode>>::value ||
                std::is_same<T, SdfPathExpression>::value || std::is_same<T, VtArray<SdfPathExpression>>::value ||
                std::is_same<T, SdfTimeSampleMap>::value || std::is_same<T, TsSpline>::value ||
                std::is_same<T, VtDictionary>::value;
    };
...
private:
    ...
    template <class T>
    USD_API bool _GetTypeSpecificResolvedMetadata(
            const UsdObject& obj, const TfToken& fieldName, const TfToken& keyPath, bool useFallbacks, T* result) const;
    ...
    template <class Composer>
    void _GetAttrVariabilityImpl(const UsdAttribute& attr, bool useFallbacks, Composer* composer) const;
    ...
    bool _HasMetadata(const UsdObject& obj, const TfToken& fieldName, const TfToken& keyPath, bool useFallbacks) const;
    ...
    template <class T>
    bool _GetValueImpl(UsdTimeCode time, const UsdAttribute& attr, Usd_InterpolatorBase* interpolator, T* value) const;
    ...
    template <class T>
    bool _GetDefaultValueFromResolveInfoImpl(const UsdResolveInfo& info, const UsdAttribute& attr, T* value) const;
    ...
    size_t _GetNumTimeSamplesFromResolveInfo(const UsdResolveInfo& info, const UsdAttribute& attr) const;
    ...
    bool _ValueMightBeTimeVaryingFromResolveInfo(const UsdResolveInfo& info, const UsdAttribute& attr) const;
...
private:
    ...
    struct _TbbHashEq {
        inline bool equal(SdfPath const& l, SdfPath const& r) const { return l == r; }
    ...
    if (result.IsHolding<T>()) {
        *value = result.UncheckedGet<T>();
        ...
        TF_CODING_ERROR(
                ...
                ArchGetDemangled<T>().c_str(), key.GetText(), result.GetTypeName().c_str());
        return false;
    }
}
    ...
    if (result.IsHolding<T>()) {
        *value = result.UncheckedGet<T>();
        ...
        TF_CODING_ERROR(
                ...
                " match retrieved type %s",
                ArchGetDemangled<T>().c_str(), key.GetText(), keyPath.GetText(), result.GetTypeName().c_str());
        return false;
    }
}

template <typename T>
bool UsdStage::SetMetadataByDictKey(const TfToken& key, const TfToken& keyPath, const T& value) const {
    VtValue in(value);
...
// Get metadata for types that do not have type specific value resolution.
template <class T>
typename std::enable_if<!UsdStage::_HasTypeSpecificResolution<T>::value, bool>::type UsdStage::_GetMetadata(
        const UsdObject& obj, const TfToken& fieldName, const TfToken& keyPath, bool useFallbacks, T* result) const {
...
// Set metadata for types that don't need to be mapped across edit targets.
template <class T>
typename std::enable_if<!UsdStage::_IsEditTargetMappable<T>::value, bool>::type UsdStage::_SetMetadata(
    ...
    return _SetMetadataImpl<SdfAbstractDataConstValue>(object, key, keyPath, in);
}
