...
// are collected.
static void _AppendValidatorsFromMetadata(const UsdValidatorMetadataVector& metadata,
                                          std::unordered_set<const UsdValidator*>* outUniqueValidators) {
    UsdValidationRegistry& validationRegistry = UsdValidationRegistry::GetInstance();

    for (const UsdValidatorMetadata& m : metadata) {
        if (!m.isSuite) {
            if (const UsdValidator* const validator = validationRegistry.GetOrLoadValidatorByName(m.name)) {
                outUniqueValidators->insert(validator);
            }
        } else {
            if (const UsdValidatorSuite* const suite = validationRegistry.GetOrLoadValidatorSuiteByName(m.name)) {
                const std::vector<const UsdValidator*> suiteValidators = suite->GetContainedValidators();
                outUniqueValidators->insert(suiteValidators.begin(), suiteValidators.end());
            }
        }
    }
}
...
static void _CollectAncestorTypeValidators(const TfTokenVector& schemaTypeNames,
                                           std::unordered_set<const UsdValidator*>* outUniqueValidators) {
    std::unordered_set<TfToken, TfToken::HashFunctor> allTypes(schemaTypeNames.begin(), schemaTypeNames.end());
...
static bool _ShouldRunSchemaTypeValidator(const UsdPrim& prim, const TfToken& schema) {
    ...
    const TfToken schemaTypeName = UsdSchemaRegistry::GetInstance().GetSchemaTypeName(type);
    ...
    return std::find(apiSchemas.begin(), apiSchemas.end(), schemaTypeName) != apiSchemas.end();
}

UsdValidationContext::UsdValidationContext(const TfTokenVector& keywords, bool includeAllAncestors) {
    UsdValidationRegistry& validationRegistry = UsdValidationRegistry::GetInstance();

    const UsdValidatorMetadataVector validatorsMetadata = validationRegistry.GetValidatorMetadataForKeywords(keywords);
    _InitializeFromValidatorMetadata(validatorsMetadata, includeAllAncestors);
}

UsdValidationContext::UsdValidationContext(const PlugPluginPtrVector& plugins, bool includeAllAncestors) {
    UsdValidationRegistry& validationRegistry = UsdValidationRegistry::GetInstance();
...
UsdValidationContext::UsdValidationContext(const UsdValidatorMetadataVector& metadata, bool includeAllAncestors) {
    _InitializeFromValidatorMetadata(metadata, includeAllAncestors);
}

void UsdValidationContext::_InitializeFromValidatorMetadata(const UsdValidatorMetadataVector& metadata,
    ...
    if (includeAllAncestors) {
        const TfTokenVector schemaTypesFromMetadata = [&metadata]() -> TfTokenVector {
        ...
        _CollectAncestorTypeValidators(schemaTypesFromMetadata, &uniqueValidators);
    }
...
UsdValidationContext::UsdValidationContext(const std::vector<TfType>& schemaTypes) {
...
UsdValidationContext::UsdValidationContext(const std::vector<const UsdValidator*>& validators) {
    // distribute the given validators into different sets based on the type of
    // validation to be performed.
    _DistributeValidators(validators);
}

UsdValidationContext::UsdValidationContext(const std::vector<const UsdValidatorSuite*>& suites) {
    std::unordered_set<const UsdValidator*> uniqueValidators;
...
void UsdValidationContext::_DistributeValidators(const std::vector<const UsdValidator*>& validators) {
        ...
        if (validator->_GetValidateLayerTask()) {
            ...
            if (validator->GetMetadata().schemaTypes.empty()) {
                ...
                for (const TfToken& schemaType : validator->GetMetadata().schemaTypes) {
                    const auto it = std::find_if(_schemaTypeValidators.begin(), _schemaTypeValidators.end(),
                    ...
                    if (it != _schemaTypeValidators.end()) {
                        ...
                        _schemaTypeValidators.emplace_back(schemaType, std::vector<const UsdValidator*>{validator});
                    }
                }
            }
        }
...
UsdValidationErrorVector UsdValidationContext::Validate(const SdfLayerHandle& layer) const {
    if (!layer) {
    ...
    WorkWithScopedDispatcher([this, &layer, &errors, &errorsMutex](WorkDispatcher& dispatcher) {
        _ValidateLayer(dispatcher, layer, &errors, &errorsMutex);
    });
    return errors;
}

UsdValidationErrorVector UsdValidationContext::Validate(const UsdStagePtr& stage) const {
    if (!stage) {
    ...
    UsdValidationErrorVector errors;
    std::mutex errorsMutex;
    WorkWithScopedDispatcher([this, &stage, &errors, &errorsMutex](WorkDispatcher& dispatcher) {
        _ValidateStage(dispatcher, stage, &errors, &errorsMutex);
    });
    return errors;
}

UsdValidationErrorVector UsdValidationContext::Validate(const std::vector<UsdPrim>& prims) const {
    UsdValidationErrorVector errors;
    std::mutex errorsMutex;
    WorkWithScopedDispatcher([this, &prims, &errors, &errorsMutex](WorkDispatcher& dispatcher) {
        _ValidatePrims(dispatcher, prims, &errors, &errorsMutex);
    });
    return errors;
}

UsdValidationErrorVector UsdValidationContext::Validate(const UsdPrimRange& prims) const {
    UsdValidationErrorVector errors;
    std::mutex errorsMutex;
    WorkWithScopedDispatcher([this, &prims, &errors, &errorsMutex](WorkDispatcher& dispatcher) {
        _ValidatePrims(dispatcher, prims, &errors, &errorsMutex);
...
void _AddErrors(const UsdValidationErrorVector& errors, UsdValidationErrorVector* outErrors, std::mutex* errorsMutex) {
    if (errors.empty()) {
        return;
    }
...
void UsdValidationContext::_ValidateLayer(WorkDispatcher& dispatcher,
...
void UsdValidationContext::_ValidateStage(WorkDispatcher& dispatcher,
...
template <typename T>
void UsdValidationContext::_ValidatePrims(WorkDispatcher& dispatcher,
                                          const T& prims,
                                          UsdValidationErrorVector* errors,
                                          std::mutex* errorsMutex) const {
    for (const UsdValidator* validator : _primValidators) {
        for (const UsdPrim& prim : prims) {
            if (!prim) {
                TF_CODING_ERROR(
                        "Invalid prim found in the vector of prims to "
                        "validate.");
                return;
            }
...
// Prims
template void UsdValidationContext::_ValidatePrims<UsdPrimRange>(WorkDispatcher& dispatcher,
                                                                 const UsdPrimRange& prims,
                                                                 UsdValidationErrorVector* errors,
                                                                 std::mutex* errorsMutex) const;

template void UsdValidationContext::_ValidatePrims<std::vector<UsdPrim>>(WorkDispatcher& dispatcher,
