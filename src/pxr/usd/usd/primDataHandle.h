...
private:
    // Equality comparison.
    friend bool operator==(const Usd_PrimDataHandle& lhs, const Usd_PrimDataHandle& rhs) { return lhs._p == rhs._p; }

    // Inequality comparison.
    friend bool operator!=(const Usd_PrimDataHandle& lhs, const Usd_PrimDataHandle& rhs) { return !(lhs == rhs); }

    // Swap \p lhs and \p rhs.
    friend void swap(Usd_PrimDataHandle& lhs, Usd_PrimDataHandle& rhs) { lhs.swap(rhs); }

    // Provide hash_value.
    friend size_t hash_value(const Usd_PrimDataHandle& h) { return TfHash()(h._p.get()); }

    friend element_type* get_pointer(const Usd_PrimDataHandle& h) { return h._p.get(); }
