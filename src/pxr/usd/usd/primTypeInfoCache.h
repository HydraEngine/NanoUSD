...
public:
    ...
    const UsdPrimTypeInfo* FindOrCreatePrimTypeInfo(TypeId&& primTypeId) {
        ...
        std::unique_ptr<UsdPrimTypeInfo> newPrimTypeInfo(new UsdPrimTypeInfo(std::move(primTypeId)));
        return _primTypeInfoMap.Insert(std::move(newPrimTypeInfo));
    }

    // Return the single empty prim type info
    const UsdPrimTypeInfo* GetEmptyPrimTypeInfo() const { return _emptyPrimTypeInfo; }
...
private:
    ...
    private:
        struct _TbbHashFunc {
            inline bool equal(const KeyType& l, const KeyType& r) const { return l == r; }
            inline size_t hash(const KeyType& t) const { return t.Hash(); }
        };

        using _HashMap = tbb::concurrent_hash_map<KeyType, ValueTypePtr, _TbbHashFunc>;
        _HashMap _hashMap;
    };
