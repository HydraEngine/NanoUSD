...
/* static */
TfTokenVector UsdAPISchemaBase::_GetMultipleApplyInstanceNames(const UsdPrim& prim, const TfType& schemaType) {
    ...
    for (const auto& appliedSchema : appliedSchemas) {
        std::pair<TfToken, TfToken> typeNameAndInstance = UsdSchemaRegistry::GetTypeNameAndInstance(appliedSchema);
