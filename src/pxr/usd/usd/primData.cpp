...
Usd_PrimData::Usd_PrimData(UsdStage* stage, const SdfPath& path)
    : _stage(stage),
      ...
      _firstChild(nullptr),
      _refCount(0) {
    if (!stage) TF_FATAL_ERROR("Attempted to construct with null stage");
...
Usd_PrimDataConstPtr Usd_PrimData::GetParent() const {
    if (Usd_PrimDataPtr parentLink = GetParentLink()) return parentLink;
...
void Usd_PrimData::_ComposeAndCacheFlags(Usd_PrimDataConstPtr parent, bool isPrototypePrim) {
    ...
    if (ARCH_UNLIKELY(!parent || isPrototypePrim)) {
        ...
        _flags[Usd_PrimLoadedFlag] =
                active &&
                (hasPayload ? _stage->_GetPcpCache()->IsPayloadIncluded(_primIndex->GetPath()) : parent->IsLoaded());
...
bool Usd_PrimData::IsSubComponent() const {
    const TfToken kind = UsdStage::_GetKind(this);
...
std::string Usd_DescribePrimData(const Usd_PrimData* p, SdfPath const& proxyPrimPath) {
    ...
    const bool isInPrototype =
            isInstanceProxy ? Usd_InstanceCache::IsPathInPrototype(proxyPrimPath) : p->IsInPrototype();
    const bool isPrototype = p->IsPrototype();
    const Usd_PrimDataConstPtr prototypeForInstance = isInstance && p->_stage ? p->GetPrototype() : nullptr;

    std::string desc = TfStringPrintf(
            "%s%s%sprim %s<%s> ", isDead ? "expired " : (p->_flags[Usd_PrimActiveFlag] ? "" : "inactive "),
            ...
            : isInstanceProxy ? "instance proxy "
                              : "",
            isInPrototype ? "in prototype " : "", isInstanceProxy ? proxyPrimPath.GetText() : p->_path.GetText());
    ...
    if (!isDead) {
        if (isInstanceProxy || isInstance) {
            if (isInstance && !prototypeForInstance) {
                ...
                desc += TfStringPrintf("with prototype <%s> ",
                                       isInstance ? prototypeForInstance->GetPath().GetText() : p->_path.GetText());
            }
        }

        if (isInstanceProxy || isPrototype || isInPrototype) {
            desc += TfStringPrintf("using prim index <%s> ", p->GetSourcePrimIndex().GetPath().GetText());
        }

        desc += TfStringPrintf("on %s", UsdDescribe(p->_stage).c_str()).c_str();
    }
...
void Usd_ThrowExpiredPrimAccessError(const Usd_PrimData* p) {
    TF_THROW(UsdExpiredPrimAccessError, TfStringPrintf("Used %s", Usd_DescribePrimData(p, SdfPath()).c_str()));
}
