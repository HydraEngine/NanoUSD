...
std::vector<std::pair<SdfPropertySpecHandle, SdfLayerOffset>> UsdProperty::GetPropertyStackWithLayerOffsets(
...
TfToken UsdProperty::GetBaseName() const {
    ...
    if (!TF_VERIFY(delim != fullName.size() - 1)) return TfToken();
...
bool UsdProperty::_GetTargets(SdfSpecType specType, SdfPathVector* out, bool* foundErrors) const {
    ...
    if (!targetIndex.paths.empty() && _Prim()->IsInPrototype()) {
        UsdPrim::_ProtoToInstancePathMap pathMap = GetPrim()._GetProtoToInstancePathMap();
        // Now map the targets.
    ...
    if (!isClean) {
        stage->_ReportPcpErrors(
                pcpErrors, TfStringPrintf(specType == SdfSpecTypeAttribute ? "getting connections for attribute <%s>"
