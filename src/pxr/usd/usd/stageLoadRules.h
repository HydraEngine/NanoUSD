...
public:
    ...
    /// Set literal rules, must be sorted by SdfPath::operator<.
    inline void SetRules(std::vector<std::pair<SdfPath, Rule>>&& rules) { _rules = std::move(rules); }
    ...
    inline std::vector<std::pair<SdfPath, Rule>> const& GetRules() const { return _rules; }
    ...
    /// operator==().
    inline bool operator!=(UsdStageLoadRules const& other) const { return !(*this == other); }
