...
TF_DEFINE_PRIVATE_TOKENS(_tokens,
                         ((PluginValidatorsKey, "Validators"))((Keywords, "keywords"))((Doc, "doc"))(
                                 (SchemaTypes, "schemaTypes"))((IsSuite, "isSuite"))((PluginValidatorNameDelimiter,
                                                                                      ":")));
...
static TfTokenVector _FillTokenVector(const JsValue& value) {
...
static TfTokenVector _ParseStringArrayValue(const JsObject& object,
                                            const TfToken& token,
                                            const TfToken& validatorName = TfToken()) {
    const std::string& errorStrTemplate = "Expected array of strings for " + token.GetString() + " metadata";
...
void UsdValidationRegistry::_PopulateMetadataFromPlugInfo() {
    const PlugPluginPtrVector& plugins = PlugRegistry::GetInstance().GetAllPlugins();

    auto _GetKey = [](const JsObject& dict, const std::string& key, JsObject* value) {
    ...
    for (const PlugPluginPtr& plugin : plugins) {
        ...
        const TfTokenVector keywords = _ParseStringArrayValue(validators, _tokens->Keywords);
        ...
        for (const auto& validatorEntry : validators) {
            ...
            if (!_GetKey(validators, validatorName, &validatorDict)) {
                TF_RUNTIME_ERROR("Expected dict for validator '%s'", validatorName.GetText());
                continue;
            }

            const JsValue* const doc = TfMapLookupPtr(validatorDict, _tokens->Doc);
            if (!doc || !doc->IsString() || doc->GetString().empty()) {
                TF_RUNTIME_ERROR(
                        "Missing or invalid or empty doc string "
            ...
            metadata.name = TfToken(pluginName.GetString() + _tokens->PluginValidatorNameDelimiter.GetString() +
                                    validatorName.GetString());
            validatorNames.push_back(metadata.name);

            metadata.doc = doc->GetString();

            metadata.schemaTypes = _ParseStringArrayValue(validatorDict, _tokens->SchemaTypes, metadata.name);

            metadata.keywords = keywords;
            const TfTokenVector& localKeywords =
                    _ParseStringArrayValue(validatorDict, _tokens->Keywords, metadata.name);
            metadata.keywords.reserve(metadata.keywords.size() + localKeywords.size());
            metadata.keywords.insert(metadata.keywords.end(), localKeywords.begin(), localKeywords.end());

            if (const JsValue* const isSuite = TfMapLookupPtr(validatorDict, _tokens->IsSuite)) {
                if (!isSuite->IsBool()) {
                    TF_RUNTIME_ERROR(
                            "Expected bool for isSuite for validator "
                            "'%s'",
        ...
        if (!validatorNames.empty()) {
            _pluginNameToValidatorNames.emplace(plugin->GetName(), validatorNames);
        }
    }
}

void UsdValidationRegistry::RegisterPluginValidator(const TfToken& validatorName,
                                                    const UsdValidateLayerTaskFn& layerTaskFn) {
    _RegisterPluginValidator<UsdValidateLayerTaskFn>(validatorName, layerTaskFn);
}

void UsdValidationRegistry::RegisterPluginValidator(const TfToken& validatorName,
                                                    const UsdValidateStageTaskFn& stageTaskFn) {
    _RegisterPluginValidator<UsdValidateStageTaskFn>(validatorName, stageTaskFn);
}
...
void UsdValidationRegistry::_RegisterPluginValidator(const TfToken& validatorName, const ValidateTaskFn& taskFn) {
...
void UsdValidationRegistry::RegisterValidator(const UsdValidatorMetadata& metadata,
                                              const UsdValidateLayerTaskFn& layerTaskFn) {
...
template <typename ValidateTaskFn>
void UsdValidationRegistry::_RegisterValidator(const UsdValidatorMetadata& metadata,
    ...
    static constexpr bool isPrimTaskFn = std::is_same<ValidateTaskFn, UsdValidatePrimTaskFn>::value;
...

    {
        ...
        std::unique_ptr<UsdValidator> validator = std::make_unique<UsdValidator>(metadata, taskFn);
...
bool UsdValidationRegistry::HasValidator(const TfToken& validatorName) const {
    std::shared_lock lock(_mutex);
    return _validators.find(validatorName) != _validators.end();
}

std::vector<const UsdValidator*> UsdValidationRegistry::GetOrLoadAllValidators() {
...
const UsdValidator* UsdValidationRegistry::GetOrLoadValidatorByName(const TfToken& validatorName) {
...
std::vector<const UsdValidator*> UsdValidationRegistry::GetOrLoadValidatorsByName(const TfTokenVector& validatorNames) {
...
void UsdValidationRegistry::RegisterPluginValidatorSuite(const TfToken& suiteName,
...
void UsdValidationRegistry::RegisterValidatorSuite(const UsdValidatorMetadata& metadata,
                                                   const std::vector<const UsdValidator*>& containedValidators) {
    _RegisterValidatorSuite(metadata, containedValidators);
}

void UsdValidationRegistry::_RegisterValidatorSuite(const UsdValidatorMetadata& metadata,
                                                    const std::vector<const UsdValidator*>& containedValidators,
                                                    bool addMetadata) {
        ...
        for (const UsdValidator* const validator : containedValidators) {
            ...
            if (!validator->_GetValidatePrimTask()) {
                TF_CODING_ERROR(
                        ...
                        metadata.name.GetText(), validator->GetMetadata().name.GetText());
                return;
            }
            ...
            for (const TfToken& schemaType : validator->GetMetadata().schemaTypes) {
                if (std::find(metadata.schemaTypes.begin(), metadata.schemaTypes.end(), schemaType) ==
                    metadata.schemaTypes.end()) {
                    TF_CODING_ERROR(
                            "schemaType '%s' provided by a contained validator "
                            "'%s' is not in schemaTypes for '%s' validator suite",
                            schemaType.GetText(), validator->GetMetadata().name.GetText(), metadata.name.GetText());
                    return;
                }
            }
        }
...

    {
        ...
        std::unique_lock lock(_mutex);
        if (_validatorSuites.find(metadata.name) != _validatorSuites.end()) {
            TF_CODING_ERROR(
        ...
        if (!_validatorSuites.emplace(metadata.name, std::move(validatorSuite)).second) {
            TF_CODING_ERROR(
...
bool UsdValidationRegistry::HasValidatorSuite(const TfToken& suiteName) const {
    std::shared_lock lock(_mutex);
    return _validatorSuites.find(suiteName) != _validatorSuites.end();
}

std::vector<const UsdValidatorSuite*> UsdValidationRegistry::GetOrLoadAllValidatorSuites() {
...
const UsdValidatorSuite* UsdValidationRegistry::GetOrLoadValidatorSuiteByName(const TfToken& suiteName) {
    auto _GetValidatorSuite = [&](const TfToken& name) -> const UsdValidatorSuite* {
        std::shared_lock lock(_mutex);
...
std::vector<const UsdValidatorSuite*> UsdValidationRegistry::GetOrLoadValidatorSuitesByName(
        const TfTokenVector& suiteNames) {
    std::vector<const UsdValidatorSuite*> suites;
    suites.reserve(suiteNames.size());
...
bool UsdValidationRegistry::GetValidatorMetadata(const TfToken& name, UsdValidatorMetadata* metadata) const {
    std::shared_lock lock(_mutex);
    const auto& validatorNameToMetadataItr = _validatorNameToMetadata.find(name);
...
UsdValidatorMetadataVector UsdValidationRegistry::GetAllValidatorMetadata() const {
...
UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForPlugin(const TfToken& pluginName) const {
    return GetValidatorMetadataForPlugins({pluginName});
}

UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForKeyword(const TfToken& keyword) const {
    return GetValidatorMetadataForKeywords({keyword});
}

UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForSchemaType(const TfToken& schemaType) const {
    return GetValidatorMetadataForSchemaTypes({schemaType});
}

UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForPlugins(
    ...
    return _GetValidatorMetadataForToken(_pluginNameToValidatorNames, pluginNames);
}

UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForKeywords(const TfTokenVector& keywords) const {
    std::shared_lock lock(_mutex);
    return _GetValidatorMetadataForToken(_keywordToValidatorNames, keywords);
}

UsdValidatorMetadataVector UsdValidationRegistry::GetValidatorMetadataForSchemaTypes(
        const TfTokenVector& schemaTypes) const {
    std::shared_lock lock(_mutex);
    return _GetValidatorMetadataForToken(_schemaTypeToValidatorNames, schemaTypes);
}

UsdValidatorMetadataVector UsdValidationRegistry::_GetValidatorMetadataForToken(
        const _TokenToValidatorNamesMap& tokenToValidatorNames, const TfTokenVector& tokens) const {
    UsdValidatorMetadataVector result;
    for (const TfToken& token : tokens) {
        ...
        for (const TfToken& validatorName : itr->second) {
            ...
            const auto& nameToMetadataItr = _validatorNameToMetadata.find(validatorName);
...
bool UsdValidationRegistry::_AddValidatorMetadata(const UsdValidatorMetadata& metadata) {
    const bool didAddValidatorMetadata = [&]() {
        return _validatorNameToMetadata.emplace(metadata.name, metadata).second;
    }();
    ...
    if (didAddValidatorMetadata) {
        ...
        // protected here.
        _UpdateValidatorNamesMappings(_schemaTypeToValidatorNames, metadata.name, metadata.schemaTypes);
        _UpdateValidatorNamesMappings(_keywordToValidatorNames, metadata.name, metadata.keywords);
    }
    return didAddValidatorMetadata;
}

/* static */
bool UsdValidationRegistry::_CheckMetadata(const UsdValidatorMetadata& metadata,
...
/* static */
void UsdValidationRegistry::_UpdateValidatorNamesMappings(_TokenToValidatorNamesMap& tokenMap,
