...
public:
    ...
    static UsdStagePopulationMask All() { return UsdStagePopulationMask().Add(SdfPath::AbsoluteRootPath()); }
    ...
    USD_API
    static UsdStagePopulationMask Intersection(UsdStagePopulationMask const& l, UsdStagePopulationMask const& r);
    ...
    bool operator==(UsdStagePopulationMask const& other) const { return _paths == other._paths; }

    /// Return true if this mask is not equivalent to \p other.
    bool operator!=(UsdStagePopulationMask const& other) const { return !(*this == other); }
