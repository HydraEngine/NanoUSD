...
UsdEditTarget UsdEditTarget::ForLocalDirectVariant(const SdfLayerHandle& layer, const SdfPath& varSelPath) {
    ...
    PcpMapFunction mapping = PcpMapFunction::Create(pathMap, SdfLayerOffset());
...
SdfPath UsdEditTarget::MapToSpecPath(const SdfPath& scenePath) const {
    ...
    if (result.ContainsTargetPath()) {
        ...
        for (const SdfPath& targetPath : targetPaths) {
            const SdfPath translatedTargetPath = _mapping.MapTargetToSource(targetPath).StripAllVariantSelections();
...
SdfPrimSpecHandle UsdEditTarget::GetPrimSpecForScenePath(const SdfPath& scenePath) const {
    if (const SdfLayerHandle& layer = GetLayer()) return layer->GetPrimAtPath(MapToSpecPath(scenePath));
    return TfNullPtr;
}

SdfPropertySpecHandle UsdEditTarget::GetPropertySpecForScenePath(const SdfPath& scenePath) const {
    if (const SdfLayerHandle& layer = GetLayer()) return layer->GetPropertyAtPath(MapToSpecPath(scenePath));
    return TfNullPtr;
}

SdfAttributeSpecHandle UsdEditTarget::GetAttributeSpecForScenePath(const SdfPath& scenePath) const {
    if (const SdfLayerHandle& layer = GetLayer()) return layer->GetAttributeAtPath(MapToSpecPath(scenePath));
    return TfNullPtr;
}

SdfRelationshipSpecHandle UsdEditTarget::GetRelationshipSpecForScenePath(const SdfPath& scenePath) const {
    if (const SdfLayerHandle& layer = GetLayer()) return layer->GetRelationshipAtPath(MapToSpecPath(scenePath));
    return TfNullPtr;
}

SdfSpecHandle UsdEditTarget::GetSpecForScenePath(const SdfPath& scenePath) const {
    if (const SdfLayerHandle& layer = GetLayer()) return layer->GetObjectAtPath(MapToSpecPath(scenePath));
    return TfNullPtr;
}
