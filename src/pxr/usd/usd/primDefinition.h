...
public:
    ...
    const TfTokenVector& GetAppliedAPISchemas() const { return _appliedAPISchemas; }
...
public:
    ...
    protected:
        ...
        Property(const TfToken& name, const _LayerAndPath* layerAndPath) : _name(name), _layerAndPath(layerAndPath) {}
...
private:
    ...
    template <class T>
    bool _HasFieldDictKey(const TfToken& propName, const TfToken& fieldName, const TfToken& keyPath, T* value) const {
        if (const _LayerAndPath* layerAndPath = _GetPropertyLayerAndPath(propName)) {
    ...
    USD_API
    void _IntializeForTypedSchema(const SdfLayerHandle& schematicsLayer,
                                  const SdfPath& schematicsPrimPath,
                                  const VtTokenArray& propertiesToIgnore);

    USD_API
    void _IntializeForAPISchema(const TfToken& apiSchemaName,
    ...
    void _ComposePropertiesFromPrimDefInstance(const UsdPrimDefinition& weakerPrimDef, const std::string& instanceName);
    ...
    USD_API
    bool _ComposeWeakerAPIPrimDefinition(const UsdPrimDefinition& apiPrimDef,
    ...
    // property name.
    using _PrimTypePropNameToPathMap = std::unordered_map<TfToken, _LayerAndPath, TfToken::HashFunctor>;
...
template <class T>
bool UsdPrimDefinition::Property::GetMetadata(const TfToken& key, T* value) const {
    if (UsdSchemaRegistry::IsDisallowedField(key)) {
        return false;
    }
...
template <class T>
bool UsdPrimDefinition::Property::GetMetadataByDictKey(const TfToken& key, const TfToken& keyPath, T* value) const {
    if (UsdSchemaRegistry::IsDisallowedField(key)) {
        return false;
    }
