...
struct _IsBitwiseReadWrite {
    static const bool value = std::is_enum<T>::value || std::is_arithmetic<T>::value ||
                              std::is_same<T, GfHalf>::value || std::is_trivial<T>::value || GfIsGfVec<T>::value ||
                              GfIsGfMatrix<T>::value || GfIsGfQuat<T>::value || std::is_base_of<Index, T>::value;
};
}  // namespace Usd_CrateFile
...
constexpr _SectionName _KnownSections[] = {_TokensSectionName,    _StringsSectionName, _FieldsSectionName,
                                           _FieldSetsSectionName, _PathsSectionName,   _SpecsSectionName};

template <class T>
struct _IsAlwaysInlined : std::integral_constant<bool, sizeof(T) <= sizeof(uint32_t) && _IsBitwiseReadWrite<T>::value> {
};

template <>
struct _IsAlwaysInlined<string> : std::true_type {};
template <>
struct _IsAlwaysInlined<TfToken> : std::true_type {};
template <>
struct _IsAlwaysInlined<SdfPath> : std::true_type {};
template <>
struct _IsAlwaysInlined<SdfAssetPath> : std::true_type {};
...
#define xx(ENUMNAME, _unused1, CPPTYPE, SUPPORTSARRAY)    \
    template <>                                           \
    struct _TypeEnumFor<CPPTYPE> {                        \
        static const TypeEnum value = TypeEnum::ENUMNAME; \
    };                                                    \
    template <>                                           \
    struct _SupportsArray<CPPTYPE> {                      \
...
static constexpr ValueRep ValueRepFor(uint64_t payload = 0) {
    return ValueRep(_TypeEnumFor<T>::value, _IsAlwaysInlined<T>::value, /*isArray=*/false, payload);
}
...
T* RoundToPageAddr(T* addr) {
    return reinterpret_cast<T*>(reinterpret_cast<uintptr_t>(addr) & CRATE_PAGEMASK);
}
...
static CrateFile::Version _GetVersionForNewlyCreatedFiles() {
...
static CrateFile::Version GetVersionForNewlyCreatedFiles() {
...
template <>
struct _IsBitwiseReadWrite<_ListOpHeader> : std::true_type {};
...
Vt_ArrayForeignDataSource* CrateFile::_FileMapping::_Impl ::_AddRangeReference(void const* addr, size_t numBytes) {
...
static void TouchPages(char volatile* start, size_t numPages) {
    while (numPages--) {
...
void CrateFile::_FileMapping::_Impl::_DetachReferencedRanges() {
    ...
    for (auto const& zeroCopy : _outstandingRanges) {
        ...
        if (zeroCopy.IsInUse()) {
            ...
            char* startAddr = reinterpret_cast<char*>(pageStart * CRATE_PAGESIZE);
            if (ArchSetMemoryProtection(startAddr, (pageEnd - pageStart) * CRATE_PAGESIZE, ArchProtectReadWriteCopy)) {
...
CrateFile::_FileMapping::_Impl::ZeroCopySource::ZeroCopySource(CrateFile::_FileMapping::_Impl* m,
                                                               void const* addr,
                                                               size_t numBytes)
    : Vt_ArrayForeignDataSource(_Detached), _mapping(m), _addr(addr), _numBytes(numBytes) {}

bool CrateFile::_FileMapping::_Impl::ZeroCopySource::operator==(ZeroCopySource const& other) const {
    return _mapping == other._mapping && _addr == other._addr && _numBytes == other._numBytes;
}

void CrateFile::_FileMapping::_Impl::ZeroCopySource::_Detached(Vt_ArrayForeignDataSource* selfBase) {
...
struct _MmapStream {
    ...
    inline void Read(void* dest, size_t nBytes) {
        ...
        if (doRangeChecks) {
            ...
            if (ARCH_UNLIKELY(!inRange)) {
                ...
                TF_THROW(UsdReadOutOfBoundsError, TfStringPrintf("Read out-of-bounds: %zd bytes at offset %td in "
        ...
        if (_prefetchKB) {
            ...
            ArchMemAdvise(reinterpret_cast<void*>(const_cast<char*>(beginAddr)), endAddr - beginAddr,
    ...
    inline int64_t Tell() const { return _cur - _mapping->GetMapStart(); }
    inline void Seek(int64_t offset) { _cur = _mapping->GetMapStart() + offset; }
    inline void Prefetch(int64_t offset, int64_t size) {
        ArchMemAdvise(_mapping->GetMapStart() + offset, size, ArchMemAdviceWillNeed);
    }

    Vt_ArrayForeignDataSource* CreateZeroCopyDataSource(void const* addr, size_t numBytes) {
        char const* mapStart = _mapping->GetMapStart();
    ...
    inline void const* TellMemoryAddress() const { return _cur; }
...
struct _PreadStream {
    ...
    explicit _PreadStream(FileRange const& fr) : _start(fr.startOffset), _cur(0), _file(fr.file) {}
    inline void Read(void* dest, size_t nBytes) { _cur += ArchPRead(_file, dest, nBytes, _start + _cur); }
...
struct _AssetStream {
    ...
    explicit _AssetStream(ArAssetSharedPtr const& asset) : _asset(asset), _cur(0) {}
    inline void Read(void* dest, size_t nBytes) { _cur += _asset->Read(dest, nBytes, _cur); }
    inline int64_t Tell() const { return _cur; }
    inline void Seek(int64_t offset) { _cur = offset; }
...
////////////////////////////////////////////////////////////////////////
// _TableOfContents
CrateFile::_Section const* CrateFile::_TableOfContents::GetSection(_SectionName name) const {
...
public:
    ...
    explicit _BufferedOutput(OutputType file)
        : _filePos(0), _file(file), _bufferPos(0), _writeTask(_dispatcher, [this]() {
              _DoWrites();
          }) {
    ...
    inline void Write(void const* bytes, int64_t nBytes) {
        ...
        while (nBytes) {
            ...
            if (numToWrite == available) _FlushBuffer();
        }
    }
...
private:
    inline void _FlushBuffer() {
        if (_buffer.size) {
            ...
            while (!_freeBuffers.try_pop(_buffer)) _dispatcher.Wait();
        }
...
struct CrateFile::_PackingContext {
    ...
    _PackingContext(CrateFile* crate, OutputType&& outAsset, std::string const& fileName)
        ...
        wd.Run([this, crate]() {
            for (size_t i = 0; i != crate->_paths.size(); ++i) pathToPathIndex[crate->_paths[i]] = PathIndex(i);
        });

        // Ensure that fieldToFieldIndex is correctly populated.
        wd.Run([this, crate]() {
            for (size_t i = 0; i != crate->_fields.size(); ++i) fieldToFieldIndex[crate->_fields[i]] = FieldIndex(i);
        });
        ...
        wd.Run([this, &fsets]() {
            ...
            for (auto fsBegin = fsets.begin(), fsEnd = find(fsBegin, fsets.end(), FieldIndex()); fsBegin != fsets.end();
        ...
        wd.Run([this, crate]() {
            for (size_t i = 0; i != crate->_tokens.size(); ++i) tokenToTokenIndex[crate->_tokens[i]] = TokenIndex(i);
        });
    ...
    bool CloseOutputAsset() { return outputAsset->Close(); }
    ...
    bool RequestWriteVersionUpgrade(Version ver, std::string reason) {
        if (!writeVersion.CanRead(ver)) {
            TF_WARN("Upgrading crate file <%s> from version %s to %s: %s", fileName.c_str(),
                    writeVersion.AsString().c_str(), ver.AsString().c_str(), reason.c_str());
            writeVersion = ver;
        }
    ...
    RawDataPtr _ReadSectionBytes(_Section const& sec, CrateFile* crate) const {
    ...
    unordered_map<vector<FieldIndex>, FieldSetIndex, _Hasher> fieldsToFieldSetIndex;
...
class CrateFile::_Reader {
    ...
    SdfListOp<T> _Read(SdfListOp<T>*) {
        ...
        if (h.HasAddedItems()) {
            listOp.SetAddedItems(Read<vector<T>>());
        }
        ...
        if (h.HasDeletedItems()) {
            listOp.SetDeletedItems(Read<vector<T>>());
        }
        if (h.HasOrderedItems()) {
            listOp.SetOrderedItems(Read<vector<T>>());
        }
        return listOp;
    }
...
public:
    ...
    _Reader(CrateFile const* crate, ByteStream& src) : crate(crate), src(src) {}
    ...
    inline T Read() {
        if constexpr (_IsBitwiseReadWrite<T>::value) {
        ...
        } else if constexpr (std::is_same_v<T, VtDictionary> || std::is_same_v<T, SdfVariantSelectionMap>) {
            return ReadMap<T>();
        } else if constexpr (std::is_same_v<T, SdfAssetPath> || std::is_same_v<T, SdfPathExpression>) {
            ...
            if (val.IsHolding<string>()) return SdfUnregisteredValue(val.UncheckedGet<string>());
            if (val.IsHolding<VtDictionary>()) return SdfUnregisteredValue(val.UncheckedGet<VtDictionary>());
            if (val.IsHolding<SdfUnregisteredValueListOp>())
                return SdfUnregisteredValue(val.UncheckedGet<SdfUnregisteredValueListOp>());
            TF_CODING_ERROR(
                    "SdfUnregisteredValue in crate file contains "
            ...
            return SdfReference(std::move(assetPath), std::move(primPath), std::move(layerOffset),
                                std::move(customData));
            ...
            const bool canReadLayerOffset = (Version(crate->_boot) >= Version(0, 8, 0));
            ...
            tbb::spin_rw_mutex::scoped_lock lock(crate->_sharedTimesMutex, /*write=*/false);
            ...
            if (sharedTimesIter != crate->_sharedTimes.end()) {
                ...
                auto iresult = crate->_sharedTimes.emplace(timesRep, Usd_EmptySharedTag);
                if (iresult.second) {
            ...
            std::unordered_map<double, VtDictionary> customData = ReadMap<std::unordered_map<double, VtDictionary>>();
            return Ts_BinaryDataAccess::CreateSplineFromBinaryData(splineData, std::move(customData));
        } else {
    ...
    void ReadContiguous(T* values, size_t sz) {
        if constexpr (_IsBitwiseReadWrite<T>::value) {
            ...
            std::for_each(values, values + sz, [this](T& v) {
                v = Read<T>();
            });
        }
    }
...
public:
    explicit _Writer(CrateFile* crate) : crate(crate), sink(&crate->_packCtx->bufferedOutput) {}
...
public:
    ...
    uint32_t GetInlinedValue(string const& s) { return crate->_AddString(s).value; }

    uint32_t GetInlinedValue(TfToken const& t) { return crate->_AddToken(t).value; }

    uint32_t GetInlinedValue(SdfPath const& p) { return crate->_AddPath(p).value; }

    uint32_t GetInlinedValue(SdfAssetPath const& p) { return crate->_AddToken(TfToken(p.GetAssetPath())).value; }
    ...
    void WriteAs(T const& obj) {
        return Write(static_cast<U>(obj));
    }
    ...
    void Write(SdfTimeCode const& tc) {
        crate->_packCtx->RequestWriteVersionUpgrade(Version(0, 9, 0),
                                                    "A timecode or timecode[] value type was detected which requires "
                                                    "crate version 0.9.0.");
        Write(tc.GetValue());
    }
    void Write(SdfPathExpression const& pathExpr) {
        crate->_packCtx->RequestWriteVersionUpgrade(Version(0, 10, 0),
    ...
    void Write(SdfRelocate const& relocate) {
        crate->_packCtx->RequestWriteVersionUpgrade(Version(0, 11, 0),
    ...
    void Write(VtValue const& val) {
        ValueRep rep;
        _RecursiveWrite([this, &val, &rep]() {
            rep = crate->_PackValue(val);
        });
        Write(rep);
    }
    ...
    void Write(const TsSpline& spline) {
        ...
        crate->_packCtx->RequestWriteVersionUpgrade(Version(0, 12, 0),
    ...
    void WriteContiguous(T const* values, size_t sz) {
        if constexpr (_IsBitwiseReadWrite<T>::value) {
            ...
            std::for_each(values, values + sz, [this](T const& v) {
                Write(v);
            });
        }
    }
    ...
    ValueRep PackArray(_Writer w, VtArray<T> const& array) {
        ...
        if (iresult.second) {
            ...
            if (w.crate->_packCtx->writeVersion < Version(0, 5, 0)) {
                ...
                // compress this array.
                target = _WritePossiblyCompressedArray(w, array, w.crate->_packCtx->writeVersion, 0);
            }
        }
        return target;
    }
...
static inline ValueRep _WriteUncompressedArray(Writer w, VtArray<T> const& array, CrateFile::Version ver) {
    ...
    (ver < CrateFile::Version(0, 7, 0)) ? w.template WriteAs<uint32_t>(array.size())
...
template <class Writer, class T>
static inline ValueRep _WritePossiblyCompressedArray(Writer w, VtArray<T> const& array, CrateFile::Version ver, ...) {
...
_ReadUncompressedArray(Reader reader, ValueRep rep, VtArray<T>* out, CrateFile::Version ver) {
    ...
    // into memory.
    out->resize(ver < CrateFile::Version(0, 7, 0) ? reader.template Read<uint32_t>()
...
_ReadUncompressedArray(Reader reader, ValueRep rep, VtArray<T>* out, CrateFile::Version ver) {
    ...
    uint64_t size =
            (ver < CrateFile::Version(0, 7, 0)) ? reader.template Read<uint32_t>() : reader.template Read<uint64_t>();
    ...
    if (zeroCopyEnabled &&
        ...
        /*    alignment ok? */
        (reinterpret_cast<uintptr_t>(reader.src.TellMemoryAddress()) & (alignof(T) - 1)) == 0) {
        ...
        if (Vt_ArrayForeignDataSource* foreignSrc = reader.src.CreateZeroCopyDataSource(addr, numBytes)) {
            *out = VtArray<T>(foreignSrc, static_cast<T*>(const_cast<void*>(addr)), size, /*addRef=*/false);
        } else {
...
template <class Reader, class T>
static inline void _ReadPossiblyCompressedArray(
...
template <class Reader, class Int>
static inline void _ReadCompressedInts(Reader& reader, Int* out, size_t size) {
...
bool CrateFile::CanRead(string const& assetPath, ArAssetSharedPtr const& asset) {
    ...
    if (file) {
        ArchFileAdvise(file, offset, asset->GetSize(), ArchFileAdviceRandomAccess);
    }
...
/* static */
std::unique_ptr<CrateFile> CrateFile::CreateNew(bool detached) {
    const bool useMmap = !TfGetEnvSetting(USDC_USE_ASSET) && !TfGetenvBool("USDC_USE_PREAD", false);

    const Options opt = detached ? Options::Detached : useMmap ? Options::UseMmap : Options::Default;

    return std::unique_ptr<CrateFile>(new CrateFile(opt));
}

/* static */
CrateFile::_FileMapping CrateFile::_MmapAsset(char const* assetPath, ArAssetSharedPtr const& asset) {
    FILE* file;
    size_t offset;
    std::tie(file, offset) = asset->GetFileUnsafe();
    std::string errMsg;
    auto mapping = _FileMapping(ArchMapFileReadOnly(file, &errMsg), offset, asset->GetSize());
...
/* static */
CrateFile::_FileMapping CrateFile::_MmapFile(char const* fileName, FILE* file) {
    std::string errMsg;
...
/* static */
std::unique_ptr<CrateFile> CrateFile::Open(string const& assetPath, bool detached) {
    TfAutoMallocTag tag("Usd_CrateFile::CrateFile::Open");
    return Open(assetPath, ArGetResolver().OpenAsset(ArResolvedPath(assetPath)), detached);
}

std::unique_ptr<CrateFile> CrateFile::Open(string const& assetPath, ArAssetSharedPtr const& srcAsset, bool detached) {
    TfAutoMallocTag tag("Usd_CrateFile::CrateFile::Open");
    ...
    if (!TfGetEnvSetting(USDC_USE_ASSET)) {
        ...
        if (file) {
            ...
            if (!TfGetenvBool("USDC_USE_PREAD", false)) {
                ...
                result.reset(new CrateFile(assetPath, ArchGetFileName(file), std::move(mapping), asset));
            } else {
                // Use pread with the asset's file.
                result.reset(new CrateFile(assetPath, ArchGetFileName(file),
                                           _FileRange(file, offset, asset->GetSize(),
                                                      /*hasOwnership=*/false),
                                           asset));
            }
        }
    ...
    if (result->GetAssetPath().empty()) result.reset();
...
CrateFile::CrateFile(Options opt) : _detached(opt == Options::Detached), _useMmap(opt == Options::UseMmap) {
    _DoAllTypeRegistrations();
}

CrateFile::CrateFile(string const& assetPath,
                     string const& fileName,
                     _FileMapping&& mapping,
                     ArAssetSharedPtr const& asset)
    : _mmapSrc(std::move(mapping)), _detached(false), _assetPath(assetPath), _useMmap(true) {
...
void CrateFile::_InitMMap() {
    if (_mmapSrc) {
        ...
        ArchMemAdvise(_mmapSrc.GetMapStart(), mapSize, ArchMemAdviceRandomAccess);
        ...
        if (!debugPageMapPattern.empty() && (debugPageMapPattern == "*" || debugPageMapPattern == "1" ||
                                             ArchRegex(debugPageMapPattern, ArchRegex::GLOB).Match(_assetPath))) {
            auto pageAlignedMapSize = (_mmapSrc.GetMapStart() + mapSize) - RoundToPageAddr(_mmapSrc.GetMapStart());
            int64_t npages = (pageAlignedMapSize + CRATE_PAGESIZE - 1) / CRATE_PAGESIZE;
        ...
        auto reader = _MakeReader(_MakeMmapStream(&_mmapSrc, _debugPageMap.get()).DisablePrefetch());
        TfErrorMark m;
        ...
        if (!_GetMMapPrefetchKB()) {
            ArchMemAdvise(_mmapSrc.GetMapStart(), mapSize, ArchMemAdviceNormal);
        }
    } else {
        _assetPath.clear();
    }
...
CrateFile::CrateFile(string const& assetPath,
                     ...
                     ArAssetSharedPtr const& asset)
    : _preadSrc(std::move(inputFile)), _assetSrc(asset), _detached(false), _assetPath(assetPath), _useMmap(false) {
...
void CrateFile::_InitPread() {
    // Mark the whole file range as random access to start to avoid large NFS
    // prefetch.  We explicitly prefetch the structural sections later.
    int64_t rangeLength = _preadSrc.GetLength();
    ArchFileAdvise(_preadSrc.file, _preadSrc.startOffset, rangeLength, ArchFileAdviceRandomAccess);
    ...
    ArchFileAdvise(_preadSrc.file, _preadSrc.startOffset, rangeLength, ArchFileAdviceNormal);
}

CrateFile::CrateFile(string const& assetPath, ArAssetSharedPtr const& asset, bool detached)
    : _assetSrc(asset), _detached(detached), _assetPath(assetPath), _useMmap(false) {
    _DoAllTypeRegistrations();
    _InitAsset();
}

void CrateFile::_InitAsset() {
    auto reader = _MakeReader(_AssetStream(_assetSrc));
    TfErrorMark m;
    _ReadStructuralSections(reader, _assetSrc->GetSize());
    if (!m.IsClean()) _assetPath.clear();
}
...
CrateFile::~CrateFile() {
    ...
    if (_useMmap && _mmapSrc && _debugPageMap) {
        ...
        printf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
               ...
               ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
               ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n",
               _assetPath.c_str(), npages, pagesAccessed, 100.0 * pagesAccessed / (double)npages, pagesInCore,
               100.0 * pagesInCore / (double)npages, 100.0 * pagesAccessed / (double)pagesInCore);
...
CrateFile::Packer CrateFile::StartPacking(string const& fileName) {
    auto out = ArGetResolver().OpenAssetForWrite(ArResolvedPath(fileName), _assetPath.empty()
...
bool CrateFile::Packer::Close() {
    if (!TF_VERIFY(_crate && _crate->_packCtx)) return false;
    ...
    if (!TfGetEnvSetting(USDC_USE_ASSET)) {
        ...
        if (file) {
            if (_crate->_useMmap) {
                ...
                _crate->_preadSrc = _FileRange(file, offset, asset->GetSize(), /*hasOwnership=*/false);
...
vector<tuple<string, int64_t, int64_t>> CrateFile::GetSectionsNameStartSize() const {
...
void CrateFile::_AddDeferredSpecs() {
        ...
        for (auto& tsf : spec.timeSampleFields) {
            for (size_t i = 0; i != tsf.second.values.size(); ++i) {
                if (!tsf.second.values[i].IsHolding<ValueRep>()) {
                    allValuesAtAllTimes[tsf.second.times.Get()[i]].push_back(&tsf.second.values[i]);
                }
            }
        }
    }
    ...
    std::transform(std::cbegin(allValuesAtAllTimes), std::cend(allValuesAtAllTimes), std::begin(orderedTimes),
                   [](const auto& element) {
    ...
    for (auto const& t : orderedTimes) {
        ...
        for (VtValue* val : it->second) *val = _PackValue(*val);
    }
    ...
    for (auto& spec : _deferredSpecs) {
        ...
        for (auto& p : spec.timeSampleFields) {
            spec.ordinaryFields.push_back(_AddField(make_pair(p.first, VtValue::Take(p.second))));
        }
        _specs.emplace_back(spec.path, spec.specType, _AddFieldSet(spec.ordinaryFields));
    }

    TfReset(_deferredSpecs);
}

bool CrateFile::_Write() {
    ...
    _WriteSection(w, _TokensSectionName, toc, [this, &w]() {
        _WriteTokens(w);
    });
    _WriteSection(w, _StringsSectionName, toc, [this, &w]() {
        w.Write(_strings);
    });
    _WriteSection(w, _FieldsSectionName, toc, [this, &w]() {
        _WriteFields(w);
    });
    _WriteSection(w, _FieldSetsSectionName, toc, [this, &w]() {
        _WriteFieldSets(w);
    });
    _WriteSection(w, _PathsSectionName, toc, [this, &w]() {
        _WritePaths(w);
    });
    _WriteSection(w, _SpecsSectionName, toc, [this, &w]() {
        _WriteSpecs(w);
    });
...
void CrateFile::_AddSpec(const SdfPath& path, SdfSpecType type, const std::vector<FieldValuePair>& fields) {
    ...
    for (auto const& p : fields) {
        if (p.second.IsHolding<TimeSamples>() && p.second.UncheckedGet<TimeSamples>().IsInMemory()) {
        ...
        } else if (p.second.IsHolding<TsSpline>() && p.second.UncheckedGet<TsSpline>().IsEmpty()) {
    ...
    if (timeSampleFields.empty() && versionUpgradePendingFields.empty()) {
        ...
        _deferredSpecs.emplace_back(_AddPath(path), type, std::move(ordinaryFields),
                                    std::move(versionUpgradePendingFields), std::move(timeSampleFields));
    }
}

VtValue CrateFile::_GetTimeSampleValueImpl(TimeSamples const& ts, size_t i) const {
    // Need to read the rep from the file for index i.
    auto offset = ts.valuesFileOffset + i * sizeof(ValueRep);
    if (_useMmap) {
        auto reader = _MakeReader(_MakeMmapStream(&_mmapSrc, _debugPageMap.get()));
        reader.Seek(offset);
...
void CrateFile::_MakeTimeSampleValuesMutableImpl(TimeSamples& ts) const {
    // Read out the reps into the vector.
    ts.values.resize(ts.times.Get().size());
    if (_useMmap) {
        auto reader = _MakeReader(_MakeMmapStream(&_mmapSrc, _debugPageMap.get()));
        reader.Seek(ts.valuesFileOffset);
        for (size_t i = 0, n = ts.times.Get().size(); i != n; ++i) ts.values[i] = reader.Read<ValueRep>();
    } else if (_preadSrc) {
        auto reader = _MakeReader(_PreadStream(_preadSrc));
        reader.Seek(ts.valuesFileOffset);
        for (size_t i = 0, n = ts.times.Get().size(); i != n; ++i) ts.values[i] = reader.Read<ValueRep>();
...
void CrateFile::_WriteFields(_Writer& w) {
    if (_packCtx->writeVersion < Version(0, 4, 0)) {
        ...
        std::transform(_fields.begin(), _fields.end(), tokenIndexVals.begin(), [](Field const& f) {
        ...
        size_t tokenIndexesSize = Usd_IntegerCompression::CompressToBuffer(tokenIndexVals.data(), tokenIndexVals.size(),
                                                                           compBuffer.get());
        ...
        uint64_t repsSize = TfFastCompression::CompressToBuffer(reinterpret_cast<char*>(reps.data()), compBuffer2.get(),
                                                                reps.size() * sizeof(reps[0]));
        w.WriteAs<uint64_t>(repsSize);
        w.WriteContiguous(compBuffer2.get(), repsSize);
    }
}

void CrateFile::_WriteFieldSets(_Writer& w) {
    if (_packCtx->writeVersion < Version(0, 4, 0)) {
        ...
        size_t fsetsSize =
                Usd_IntegerCompression::CompressToBuffer(fieldSetsVals.data(), fieldSetsVals.size(), compBuffer.get());
        w.WriteAs<uint64_t>(fsetsSize);
        w.WriteContiguous(compBuffer.get(), fsetsSize);
    }
}

void CrateFile::_WritePaths(_Writer& w) {
    ...
    if (_packCtx->writeVersion < Version(0, 4, 0)) {
        ...
        for (auto const& item : _packCtx->pathToPathIndex) pathToIndexTable[item.first] = item.second;
        ...
        std::sort(ppaths.begin(), ppaths.end(),
                  [](pair<SdfPath, PathIndex> const& l, pair<SdfPath, PathIndex> const& r) {
...
void CrateFile::_WriteSpecs(_Writer& w) {
    ...
    if (_packCtx->writeVersion == Version(0, 0, 1)) {
        ...
        std::unique_ptr<char[]> compBuffer(new char[Usd_IntegerCompression::GetCompressedBufferSize(_specs.size())]);
        ...
        size_t pathIndexesSize = Usd_IntegerCompression::CompressToBuffer(tmp.data(), tmp.size(), compBuffer.get());
        ...
        size_t fsetIndexesSize = Usd_IntegerCompression::CompressToBuffer(tmp.data(), tmp.size(), compBuffer.get());
        ...
        size_t specTypesSize = Usd_IntegerCompression::CompressToBuffer(tmp.data(), tmp.size(), compBuffer.get());
...
template <class Iter>
Iter CrateFile::_WritePathTree(_Writer& w, Iter cur, Iter end) {
    ...
    for (Iter next = cur; cur != end; cur = next) {
        ...
        if (hasChild) next = _WritePathTree(w, next, end);
        ...
        if (!hasSibling) return next;
    }
    return end;
}

template <class Iter>
Iter CrateFile::_BuildCompressedPathDataRecursive(size_t& curIndex,
                                                  Iter cur,
                                                  Iter end,
...
void CrateFile::_WriteCompressedPathData(_Writer& w, Container const& pathVec) {
    ...
    size_t index = 0;
    _BuildCompressedPathDataRecursive(index, pathVec.begin(), pathVec.end(), pathIndexes, elementTokenIndexes, jumps);

    // Compress and store the arrays.
    std::unique_ptr<char[]> compBuffer(new char[Usd_IntegerCompression::GetCompressedBufferSize(pathVec.size())]);

    // pathIndexes.
    uint64_t pathIndexesSize =
            Usd_IntegerCompression::CompressToBuffer(pathIndexes.data(), pathIndexes.size(), compBuffer.get());
    w.WriteAs<uint64_t>(pathIndexesSize);
    w.WriteContiguous(compBuffer.get(), pathIndexesSize);

    // elementTokenIndexes.
    uint64_t elemToksSize = Usd_IntegerCompression::CompressToBuffer(elementTokenIndexes.data(),
                                                                     elementTokenIndexes.size(), compBuffer.get());
    w.WriteAs<uint64_t>(elemToksSize);
    w.WriteContiguous(compBuffer.get(), elemToksSize);

    // jumps.
    uint64_t jumpsSize = Usd_IntegerCompression::CompressToBuffer(jumps.data(), jumps.size(), compBuffer.get());
    w.WriteAs<uint64_t>(jumpsSize);
    w.WriteContiguous(compBuffer.get(), jumpsSize);
}

void CrateFile::_WriteTokens(_Writer& w) {
    // # of strings.
    w.WriteAs<uint64_t>(_tokens.size());
    if (_packCtx->writeVersion < Version(0, 4, 0)) {
        // Count total bytes.
        uint64_t totalBytes = 0;
        for (auto const& t : _tokens) totalBytes += t.GetString().size() + 1;
        ...
        std::unique_ptr<char[]> compressed(new char[TfFastCompression::GetCompressedBufferSize(tokenData.size())]);
        uint64_t compressedSize =
                TfFastCompression::CompressToBuffer(tokenData.data(), compressed.get(), tokenData.size());
...
template <class Reader>
void CrateFile::_ReadStructuralSections(Reader reader, int64_t fileSize) {
    TfErrorMark m;
    try {
        ...
        TF_RUNTIME_ERROR("Encountered: %s, while reading @%s@", e.what(), _assetPath.c_str());
        _specs.clear();
        _fieldSets.clear();
        _fields.clear();
    }
    ...
    if (m.IsClean()) {
        ...
        for (Spec const& spec : _specs) {
            ...
            if (spec.pathIndex.value >= _paths.size() || spec.fieldSetIndex.value >= _fieldSets.size() ||
                (spec.fieldSetIndex.value && _fieldSets[spec.fieldSetIndex.value - 1] != FieldIndex()) ||
                spec.specType == SdfSpecTypeUnknown || spec.specType >= SdfNumSpecTypes) {
                errorAndClear();
                return;
            }
        }
...
template <class ByteStream>
/*static*/
CrateFile::_BootStrap CrateFile::_ReadBootStrap(ByteStream src, int64_t fileSize) {
    _BootStrap b;
    ...
    // Check version.
        ...
        TF_RUNTIME_ERROR(
                ...
                "software supports %s",
                Version(b).AsString().c_str(), _SoftwareVersion.AsString().c_str());
    }
...
template <class Reader>
void CrateFile::_PrefetchStructuralSections(Reader reader) const {
    // Go through the _toc and find its maximal range, then ask the reader to
    // prefetch that range.
    int64_t min = -1, max = -1;
    for (_Section const& sec : _toc.sections) {
        if (min == -1 || (sec.start < min)) min = sec.start;
        int64_t end = sec.start + sec.size;
        if (max == -1 || (end > max)) max = end;
    }
    if (min != -1 && max != -1) reader.Prefetch(min, max - min);
}

template <class Reader>
CrateFile::_TableOfContents CrateFile::_ReadTOC(Reader reader, _BootStrap const& b) const {
    reader.Seek(b.tocOffset);
    return reader.template Read<_TableOfContents>();
}

template <class Reader>
void CrateFile::_ReadFieldSets(Reader reader) {
...
template <class Reader>
void CrateFile::_ReadFields(Reader reader) {
    ...
    if (auto fieldsSection = _toc.GetSection(_FieldsSectionName)) {
        ...
        if (Version(_boot) < Version(0, 4, 0)) {
            ...
            TfFastCompression::DecompressFromBuffer(compBuffer.get(), reinterpret_cast<char*>(repsData.data()),
...
template <class Reader>
void CrateFile::_ReadSpecs(Reader reader) {
    ...
    for (Spec& spec : _specs) {
        ...
        if (specPath.IsEmpty()) {
            messages.push_back(
                    TfStringPrintf("spec at index %zu has empty path", std::distance(&_specs.front(), &spec)));
            // Mark for removal.
            spec.specType = SdfSpecTypeUnknown;
            continue;
        }
        if (spec.specType == SdfSpecTypeUnknown || spec.specType >= SdfNumSpecTypes) {
            messages.push_back(
                    TfStringPrintf("spec <%s> has %s", specPath.GetAsString().c_str(),
                                   spec.specType == SdfSpecTypeUnknown
                                           ? "unknown spec type"
                                           : TfStringPrintf("invalid spec type value %d", spec.specType).c_str()));
            // Mark for removal.
            spec.specType = SdfSpecTypeUnknown;
            continue;
        }
        if (!seenPaths.insert(specPath).second) {
            messages.push_back(TfStringPrintf("spec <%s> repeated", specPath.GetAsString().c_str()));
            // Mark for removal.
            spec.specType = SdfSpecTypeUnknown;
            continue;
        }
    ...
    if (!messages.empty()) {
        ...
        _specs.erase(std::remove_if(_specs.begin(), _specs.end(),
                                    [](Spec const& s) {
        ...
        TF_RUNTIME_ERROR("Corrupt asset @%s@ - ignoring invalid specs: %s.", _assetPath.c_str(),
                         TfStringJoin(messages, ", ").c_str());
    }
#endif  // PXR_PREFER_SAFETY_OVER_SPEED
}

template <class Reader>
void CrateFile::_ReadStrings(Reader reader) {
...
template <class Reader>
void CrateFile::_ReadTokens(Reader reader) {
...
template <class Reader>
void CrateFile::_ReadPaths(Reader reader) {
...
void CrateFile::_ReadPathsImpl(Reader reader, WorkDispatcher& dispatcher, SdfPath parentPath) {
    ...
    do {
        ...
        if (hasChild) {
            if (hasSibling) {
                ...
                dispatcher.Run([this, reader, siblingOffset, &dispatcher, parentPath]() {
                    // XXX Remove these tags when bug #132031 is addressed
                    TfAutoMallocTag tag("Usd", "Usd_CrateDataImpl::Open", "Usd_CrateFile::CrateFile::Open",
                                        "_ReadPaths");
...
void CrateFile::_ReadCompressedPaths(Reader reader, WorkDispatcher& dispatcher) {
    ...
    // Now build the paths.
    _BuildDecompressedPathsImpl(pathIndexes, elementTokenIndexes, jumps, 0, SdfPath(), dispatcher);

    dispatcher.Wait();
}

void CrateFile::_BuildDecompressedPathsImpl(vector<uint32_t> const& pathIndexes,
    ...
    do {
        ...
        if (thisIndex >= pathIndexes.size()) {
            TF_RUNTIME_ERROR(
                    "Corrupt paths encoding in crate file "
                    "(index:%zu >= %zu)",
...
#endif
                dispatcher.Run(
                        [this, &pathIndexes, &elementTokenIndexes, &jumps, siblingIndex, &dispatcher, parentPath]() {
                            // XXX Remove these tags when bug #132031 is addressed
                            TfAutoMallocTag tag("Usd", "Usd_CrateDataImpl::Open", "Usd_CrateFile::CrateFile::Open",
                                                "_ReadPaths");
                            _BuildDecompressedPathsImpl(pathIndexes, elementTokenIndexes, jumps, siblingIndex,
                                                        parentPath, dispatcher);
                        });
            }
            // Have a child (may have also had a sibling). Reset parent path.
...
void CrateFile::_ReadRawBytes(int64_t start, int64_t size, char* buf) const {
    if (_useMmap) {
        auto reader = _MakeReader(_MakeMmapStream(&_mmapSrc, _debugPageMap.get()));
        reader.Seek(start);
...
PathIndex CrateFile::_AddPath(const SdfPath& path) {
    ...
    if (iresult.second) {
        ...
        if (path.IsTargetPath()) _AddPath(path.GetTargetPath());

        // Not present -- ensure parent is added.
        if (path != SdfPath::AbsoluteRootPath()) _AddPath(path.GetParentPath());
        ...
        _AddToken(path.IsPrimPropertyPath() ? path.GetNameToken() : path.GetElementToken());
...
FieldSetIndex CrateFile::_AddFieldSet(const std::vector<FieldIndex>& fieldIndexes) {
    auto iresult = _packCtx->fieldsToFieldSetIndex.emplace(fieldIndexes, FieldSetIndex());
    if (iresult.second) {
        // Not yet present.  Copy the fields to _fieldSets, terminate, and store
        // the start index.
        iresult.first->second = FieldSetIndex(_fieldSets.size());
        _fieldSets.insert(_fieldSets.end(), fieldIndexes.begin(), fieldIndexes.end());
        _fieldSets.push_back(FieldIndex());
    }
    return iresult.first->second;
}

FieldIndex CrateFile::_AddField(const FieldValuePair& fv) {
...
TokenIndex CrateFile::_AddToken(const TfToken& token) {
...
TokenIndex CrateFile::_GetIndexForToken(const TfToken& token) const {
    auto iter = _packCtx->tokenToTokenIndex.find(token);
    if (!TF_VERIFY(iter != _packCtx->tokenToTokenIndex.end())) return TokenIndex();
    return iter->second;
}

StringIndex CrateFile::_AddString(const string& str) {
...
template <class T>
CrateFile::_ValueHandler<T>& CrateFile::_GetValueHandler() {
    return *static_cast<_ValueHandler<T>*>(_valueHandlers[static_cast<int>(_TypeEnumFor<T>::value)]);
}

template <class T>
CrateFile::_ValueHandler<T> const& CrateFile::_GetValueHandler() const {
    return *static_cast<_ValueHandler<T> const*>(_valueHandlers[static_cast<int>(_TypeEnumFor<T>::value)]);
}

template <class T>
ValueRep CrateFile::_PackValue(T const& v) {
    return _GetValueHandler<T>().Pack(_Writer(this), v);
}

template <class T>
ValueRep CrateFile::_PackValue(VtArray<T> const& v) {
    return _GetValueHandler<T>().PackArray(_Writer(this), v);
}

ValueRep CrateFile::_PackValue(VtValue const& v) {
    ...
    if (v.IsHolding<ValueRep>()) {
        ...
        // the new file.
        if (valueRep.GetType() == TypeEnum::Payload && Version(_boot) < Version(0, 8, 0) &&
    ...
    if (v.IsHolding<TimeSamples>()) {
        ...
        if (!ts.IsInMemory()) return ts.valueRep;
    }

    std::type_index ti = v.IsArrayValued() ? v.GetElementTypeid() : v.GetTypeid();

    auto it = _packValueFunctions.find(ti);
    if (it != _packValueFunctions.end()) return it->second(v);

    TF_CODING_ERROR(
            "Attempted to pack unsupported type '%s' "
            "(%s)",
            ArchGetDemangled(ti).c_str(), TfStringify(v).c_str());

    return ValueRep(0);
}

template <class T>
void CrateFile::_UnpackValue(ValueRep rep, T* out) const {
    try {
        auto const& h = _GetValueHandler<T>();
        if (_useMmap) {
            h.Unpack(_MakeReader(_MakeMmapStream(&_mmapSrc, _debugPageMap.get())), rep, out);
        } else if (_preadSrc) {
        ...
        TF_RUNTIME_ERROR(
                "Corrupt asset <%s>: exception thrown unpacking a "
                "%s, returning a value-initialized object",
                GetAssetPath().c_str(), ArchGetDemangled<T>().c_str());
        *out = T();
    }
}

template <class T>
void CrateFile::_UnpackValue(ValueRep rep, VtArray<T>* out) const {
    try {
        auto const& h = _GetValueHandler<T>();
        if (_useMmap) {
            h.UnpackArray(_MakeReader(_MakeMmapStream(&_mmapSrc, _debugPageMap.get())), rep, out);
        } else if (_preadSrc) {
...
void CrateFile::_UnpackValue(ValueRep rep, VtValue* result) const {
...
void CrateFile::_DoTypeRegistration() {
    ...
    _packValueFunctions[std::type_index(typeid(T))] = [this, valueHandler](VtValue const& val) {
        return valueHandler->PackVtValue(_Writer(this), val);
    };

    _unpackValueFunctionsPread[typeEnumIndex] = [this, valueHandler](ValueRep rep, VtValue* out) {
        valueHandler->UnpackVtValue(_MakeReader(_PreadStream(_preadSrc)), rep, out);
    };

    _unpackValueFunctionsMmap[typeEnumIndex] = [this, valueHandler](ValueRep rep, VtValue* out) {
        valueHandler->UnpackVtValue(_MakeReader(_MakeMmapStream(&_mmapSrc, _debugPageMap.get())), rep, out);
    };

    _unpackValueFunctionsAsset[typeEnumIndex] = [this, valueHandler](ValueRep rep, VtValue* out) {
        valueHandler->UnpackVtValue(_MakeReader(_AssetStream(_assetSrc)), rep, out);
    };
}

// Functions that populate the value read/write functions.
void CrateFile::_DoAllTypeRegistrations() {
    TfAutoMallocTag tag("Usd_CrateFile::CrateFile::_DoAllTypeRegistrations");
#define xx(_unused1, _unused2, CPPTYPE, _unused3) _DoTypeRegistration<CPPTYPE>();
...
void CrateFile::_DeleteValueHandlers() {
#define xx(_unused1, _unused2, T, _unused3) \
    delete static_cast<_ValueHandler<T>*>(_valueHandlers[static_cast<int>(_TypeEnumFor<T>::value)]);
...
void CrateFile::_ClearValueHandlerDedupTables() {
#define xx(_unused1, _unused2, T, _unused3) \
    static_cast<_ValueHandler<T>*>(_valueHandlers[static_cast<int>(_TypeEnumFor<T>::value)])->Clear();
...
std::string const& CrateFile::_GetEmptyString() const {
    static std::string empty;
    return empty;
}

TfToken const& CrateFile::_GetEmptyToken() const {
...
CrateFile::_Section::_Section(char const* inName, int64_t start, int64_t size) : start(start), size(size) {
    ...
    if (TF_VERIFY(strlen(inName) <= _SectionNameMaxLength)) strcpy(name, inName);
}

std::ostream& operator<<(std::ostream& o, ValueRep rep) {
    o << "ValueRep enum=" << int(rep.GetType());
    if (rep.IsArray()) o << " (array)";
    return o << " payload=" << rep.GetPayload();
}

std::ostream& operator<<(std::ostream& os, TimeSamples const& samples) {
    return os << "TimeSamples with " << samples.times.Get().size() << " samples";
}
