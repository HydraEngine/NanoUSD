...
static TfToken _GetFormatArgumentForFileFormat(const SdfFileFormatConstPtr& fileFormat) {
    ...
    TF_VERIFY(formatArg == UsdUsdaFileFormatTokens->Id || formatArg == UsdUsdcFileFormatTokens->Id,
              "Unhandled file format '%s'", fileFormat ? formatArg.GetText() : "<null>");
    return formatArg;
}
...
static SdfFileFormatConstPtr _GetFileFormatForArguments(const SdfFileFormat::FileFormatArguments& args) {
    ...
    if (it != args.end()) {
        ...
        TF_CODING_ERROR(
                ...
                "Defaulting to '%s'.",
                UsdUsdFileFormatTokens->FormatArg.GetText(), format.c_str(), UsdUsdaFileFormatTokens->Id.GetText(),
...
bool UsdUsdFileFormat::_ReadDetached(SdfLayer* layer, const std::string& resolvedPath, bool metadataOnly) const {
    TRACE_FUNCTION();
...
bool UsdUsdFileFormat::_ReadHelper(SdfLayer* layer, const string& resolvedPath, bool metadataOnly) const {
...
TfToken UsdUsdFileFormat::GetUnderlyingFormatForLayer(const SdfLayer& layer) {
    if (layer.GetFileFormat()->GetFormatId() != UsdUsdFileFormatTokens->Id) return TfToken();
...
bool UsdUsdFileFormat::WriteToString(const SdfLayer& layer, std::string* str, const std::string& comment) const {
...
bool UsdUsdFileFormat::WriteToStream(const SdfSpecHandle& spec, std::ostream& out, size_t indent) const {
    return _GetUnderlyingFileFormatForLayer(*get_pointer(spec->GetLayer()))->WriteToStream(spec, out, indent);
}
