...
bool UsdAttribute::GetBracketingTimeSamples(double desiredTime,
    ...
    return _GetStage()->_GetBracketingTimeSamples(*this, desiredTime, /*requireAuthored*/ false, lower, upper,
                                                  hasTimeSamples);
}
...
bool UsdAttribute::HasSpline() const {
    return _GetStage()->_HasMetadata(*this,                 // find a field in our attribute spec
                                     SdfFieldKeys->Spline,  // find the Spline field
                                     TfToken(),             // not a dict field, so no dict key
                                     false);                // want authored opinions only
}

TsSpline UsdAttribute::GetSpline() const {
    TsSpline spline;
    _GetStage()->_GetMetadata(*this,                 // read a field in our attribute spec
...
bool UsdAttribute::SetSpline(const TsSpline& spline) {
    return _GetStage()->_SetMetadata(*this,                 // write a field in our attribute spec
...
SdfAttributeSpecHandle UsdAttribute::_CreateSpec(const SdfValueTypeName& typeName,
    ...
    TfErrorMark m;
    if (SdfAttributeSpecHandle attrSpec = stage->_CreateAttributeSpecForEditing(*this)) return attrSpec;
    ...
    if (m.IsClean()) {
        ...
        return SdfAttributeSpec::New(stage->_CreatePrimSpecForEditing(GetPrim()), _PropName(), typeName, variability,
                                     custom);
    }
    return TfNullPtr;
}
...
bool UsdAttribute::_Create(const SdfValueTypeName& typeName, bool custom, const SdfVariability& variability) const {
...
SdfPath UsdAttribute::_GetPathForAuthoring(const SdfPath& path, std::string* whyNot) const {
    ...
    if (result.IsEmpty()) {
        if (whyNot) {
            *whyNot = TfStringPrintf("Cannot map <%s> to layer @%s@ via stage's EditTarget", path.GetText(),
...
bool UsdAttribute::AddConnection(const SdfPath& source, UsdListPosition position) const {
    ...
    if (pathToAuthor.IsEmpty()) {
        TF_CODING_ERROR("Cannot append connection <%s> to attribute <%s>: %s", source.GetText(), GetPath().GetText(),
                        errMsg.c_str());
        return false;
    }
    ...
    Usd_InsertListItem(attrSpec->GetConnectionPathList(), pathToAuthor, position);
    return true;
}
...
bool UsdAttribute::RemoveConnection(const SdfPath& source) const {
    ...
    const SdfPath pathToAuthor = _GetPathForAuthoring(source, &errMsg);
    if (pathToAuthor.IsEmpty()) {
        TF_CODING_ERROR("Cannot remove connection <%s> from attribute <%s>: %s", source.GetText(), GetPath().GetText(),
                        errMsg.c_str());
        return false;
    }
...
bool UsdAttribute::SetConnections(const SdfPathVector& sources) const {
    ...
    for (const SdfPath& path : sources) {
        ...
        if (mappedPaths.back().IsEmpty()) {
            TF_CODING_ERROR("Cannot set connection <%s> on attribute <%s>: %s", path.GetText(), GetPath().GetText(),
                            errMsg.c_str());
            return false;
        }
    }
