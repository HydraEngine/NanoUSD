...
void UsdPrimRange::iterator::increment() {
    ...
    if (ARCH_UNLIKELY(_isPost)) {
        _isPost = false;
        if (Usd_MoveToNextSiblingOrParent(_underlyingIterator, _proxyPrimPath, end, _range->_predicate)) {
            if (_depth) {
                --_depth;
                _isPost = true;
            } else {
    ...
    } else if (!_pruneChildrenFlag && Usd_MoveToChild(_underlyingIterator, _proxyPrimPath, end, _range->_predicate)) {
        ++_depth;
    } else {
        if (_range->_postOrder) {
            _isPost = true;
        } else {
            while (Usd_MoveToNextSiblingOrParent(_underlyingIterator, _proxyPrimPath, end, _range->_predicate)) {
                if (_depth) {
                    --_depth;
                } else {
                    _underlyingIterator = end;
