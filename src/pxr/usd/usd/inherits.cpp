...
SdfPathVector UsdInherits::GetAllDirectInherits() const {
    ...
    auto addIfDirectInheritFn = [&](const PcpNodeRef& node) {
        if (node.GetArcType() == PcpArcTypeInherit && node.GetLayerStack() == node.GetRootNode().GetLayerStack() &&
            !node.GetOriginRootNode().IsDueToAncestor() && seen.insert(node.GetPath()).second) {
    ...
    for (const PcpNodeRef& node : fullPrimIndex.GetNodeRange(PcpRangeTypeInherit)) {
        addIfDirectInheritFn(node);
    }
    for (const PcpNodeRef& node : fullPrimIndex.GetNodeRange(PcpRangeTypeSpecialize)) {
        addIfDirectInheritFn(node);
    }
    return ret;
}
