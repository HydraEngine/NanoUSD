...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (generated_manifest));

bool Usd_IsAutoGeneratedClipManifest(const SdfLayerHandle& manifestLayer) {
    return manifestLayer->IsAnonymous() &&
           TfStringContains(manifestLayer->GetIdentifier(), _tokens->generated_manifest.GetString());
}
...
SdfLayerRefPtr Usd_GenerateClipManifest(const Usd_ClipRefPtrVector& clips,
    ...
    return Usd_GenerateClipManifest(clipLayers, clipPrimPath, tag,
...
SdfLayerRefPtr Usd_GenerateClipManifest(const SdfLayerHandleVector& clipLayers,
    ...
    if (std::any_of(clipLayers.begin(), clipLayers.end(), [](const SdfLayerHandle& l) {
            return !l;
        })) {
    ...
    SdfLayerRefPtr manifestLayer =
            SdfLayer::CreateAnonymous(TfStringPrintf("%s.%s", tag.c_str(), UsdUsdaFileFormatTokens->Id.GetText()));

    SdfChangeBlock changeBlock;

    for (const SdfLayerHandle& clipLayer : clipLayers) {
        clipLayer->Traverse(clipPrimPath, [&manifestLayer, &clipLayer](const SdfPath& path) {
            ...
            if (!manifestLayer->HasSpec(path) && layerPtr->GetSpecType(path) == SdfSpecTypeAttribute &&
                layerPtr->HasField(path, SdfFieldKeys->TypeName, &typeName) &&
                layerPtr->HasField(path, SdfFieldKeys->Variability, &variability) &&
                clipLayer->GetNumTimeSamplesForPath(path) != 0) {
                SdfJustCreatePrimAttributeInLayer(manifestLayer, path, layerPtr->GetSchema().FindType(typeName),
                                                  variability);
            }
        });
    }
    ...
    if (clipActive) {
        ...
        for (auto pathAndActiveTime : attrToActiveTimeOfClipsWithoutSamples) {
            for (double time : pathAndActiveTime.second) {
                manifestLayer->SetTimeSample(pathAndActiveTime.first, time, block);
            }
        }
    }
...
static bool _ValidateClipFields(const VtArray<SdfAssetPath>& clipAssetPaths,
    ...
    if (!(path.IsAbsolutePath() && path.IsPrimPath())) {
        *errMsg = TfStringPrintf("Path '%s' in '%s' must be an absolute path to a prim", clipPrimPath.c_str(),
    ...
    for (const auto& startFrameAndClipIndex : clipActive) {
        if (startFrameAndClipIndex[1] < 0 || startFrameAndClipIndex[1] >= numClips) {
            *errMsg = TfStringPrintf("Invalid clip index %d in '%s'", (int)startFrameAndClipIndex[1],
    ...
    for (const auto& startFrameAndClipIndex : clipActive) {
        std::pair<_ActiveClipMap::iterator, bool> status =
                activeClipMap.insert(std::make_pair(startFrameAndClipIndex[0], startFrameAndClipIndex[1]));

        if (!status.second) {
            *errMsg = TfStringPrintf(
                    "Clip %d cannot be active at time %.3f in '%s' because "
                    "clip %d was already specified as active at this time.",
                    (int)startFrameAndClipIndex[1], startFrameAndClipIndex[0], UsdClipsAPIInfoKeys->active.GetText(),
                    status.first->second);
            return false;
        }
    }
    ...
    if (clipTimes) {
        ...
        for (const auto& stageTimeAndClipTime : *clipTimes) {
            int& numSeen = stageTimesMap.emplace(stageTimeAndClipTime[0], 0).first->second;
            numSeen += 1;
            ...
            if (numSeen > 2) {
                *errMsg = TfStringPrintf(
                        ...
                        "stage time (%.3f).",
                        UsdClipsAPIInfoKeys->times.GetText(), stageTimeAndClipTime[0]);
                return false;
            }
        }
    }
...
Usd_ClipSetRefPtr Usd_ClipSet::New(const std::string& name, const Usd_ClipSetDefinition& clipDef, std::string* status) {
    ...
    // required.
    if (!clipDef.clipAssetPaths || !clipDef.clipPrimPath || !clipDef.clipActive) {
        return nullptr;
    }

    // XXX: Possibly want a better way to inform consumers of the error
    //      message..
    if (!_ValidateClipFields(*clipDef.clipAssetPaths, *clipDef.clipPrimPath, *clipDef.clipActive,
                             clipDef.clipTimes ? &(clipDef.clipTimes.value()) : nullptr, status)) {
        return nullptr;
    }
...
Usd_ClipSet::Usd_ClipSet(const std::string& name_, const Usd_ClipSetDefinition& clipDef)
    ...
    for (const auto& startFrameAndClipIndex : *clipDef.clipActive) {
        ...
        TF_VERIFY(startTimeToClip.insert(std::make_pair(entry.startTime, entry)).second);
    }
    ...
    if (!times->empty()) {
        ...
        std::stable_sort(times->begin(), times->end(), Usd_Clip::Usd_SortByExternalTime());
        ...
        for (size_t i = 0; i < times->size() - 1; ++i) {
            if ((*times)[i].externalTime == (*times)[i + 1].externalTime) {
                (*times)[i].externalTime = (*times)[i].externalTime - UsdTimeCode::SafeStep();
    ...
    if (clipDef.clipManifestAssetPath) {
        ...
        generatedManifest = Usd_GenerateClipManifest(valueClips, clipPrimPath, _tokens->generated_manifest.GetString());
...
bool Usd_ClipSet::_ClipContributesValue(const Usd_ClipRefPtr& clip, const SdfPath& path) const {
    ...
    const bool hasAuthoredSamples =
            !manifestClip->IsBlocked(path, clip->authoredStartTime) && clip->HasAuthoredTimeSamples(path);
    if (hasAuthoredSamples) {
        return true;
    }

    // Use the clip if a default value specified in the manifest.
    if (Usd_HasDefault<int>(manifestClip, path, nullptr) != Usd_DefaultValueResult::None) {
        return true;
    }
...
bool Usd_ClipSet::GetBracketingTimeSamplesForPath(const SdfPath& path,
                                                  double time,
                                                  double* lower,
                                                  double* upper) const {
    ...
    if (_ClipContributesValue(activeClip, path)) {
        if (!TF_VERIFY(activeClip->GetBracketingTimeSamplesForPath(path, time, lower, upper))) {
            return false;
        }
    ...
    for (size_t i = clipIndex; !foundLower && i-- != 0;) {
        ...
        if (!TF_VERIFY(clip->GetBracketingTimeSamplesForPath(path, time, &tmpLower, &tmpUpper))) {
            return false;
        }
...
std::set<double> Usd_ClipSet::ListTimeSamplesForPath(const SdfPath& path) const {
    ...
    for (const Usd_ClipRefPtr& clip : valueClips) {
        ...
        const std::set<Usd_Clip::ExternalTime> clipSamples = clip->ListTimeSamplesForPath(path);
...
std::vector<double> Usd_ClipSet::GetTimeSamplesInInterval(const SdfPath& path, const GfInterval& interval) const {
    std::vector<double> timeSamples;

    for (const Usd_ClipRefPtr& clip : valueClips) {
        if (interval.IsMaxOpen() ? clip->startTime >= interval.GetMax() : clip->startTime > interval.GetMax()) {
        ...
        if (!interval.Intersects(GfInterval(clip->startTime, clip->endTime,
        ...
        Usd_CopyTimeSamplesInInterval(clip->ListTimeSamplesForPath(path), interval, &timeSamples);
    }
    ...
    if (timeSamples.empty() &&
        std::none_of(valueClips.begin(), valueClips.end(), [this, &path](const Usd_ClipRefPtr& clip) {
...
size_t Usd_ClipSet::_FindClipIndexForTime(double time) const {
    size_t clipIndex = 0;
    ...
    if (valueClips.size() > 1) {
        ...
        // time.
        auto it = std::upper_bound(valueClips.begin(), valueClips.end(), time,
