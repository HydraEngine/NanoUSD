...
TF_DEFINE_ENV_SETTING(USD_ABC_READ_ARCHIVE_USE_MMAP, true, "Use mmap when reading from an Ogawa archive.");
#endif
...
static std::string _GetAlembicPath(const IScalarProperty& p) {
    std::vector<std::string> names;
    names.push_back(p.getName());

    for (ICompoundProperty prop = p.getParent(); prop.valid(); prop = prop.getParent()) {
...
static std::string _GetSampleSelectorDescription(const ISampleSelector& iss) {
...
static const char* _WarningNames[] = {"visibility", "subdivision scheme", "interpolate boundary",
                                      "face varying interpolate boundary"};

static void _PostUnsupportedValueWarning(const IScalarProperty& property,
    ...
    if (TfGetEnvSetting(USD_ABC_WARN_ALL_UNSUPPORTED_VALUES)) {
        TF_WARN("Unsupported %s '%s' for <%s> at %s in archive '%s'. "
                "Using '%s' instead.",
                _WarningNames[warning], authoredValue.c_str(), _GetAlembicPath(property).c_str(),
                _GetSampleSelectorDescription(iss).c_str(), archiveName.c_str(), replacementValue.c_str());
        return;
    }
    ...
    if (issueWarning) {
        TF_WARN("Unsupported %s detected in archive '%s'. Using '%s' instead.", _WarningNames[warning],
                archiveName.c_str(), replacementValue.c_str());
    }
}
...
struct _AlembicFixName {
    std::string operator()(std::string const& x) const { return TfMakeValidIdentifier(x); }
};
struct _AlembicFixNamespacedName {
    std::string operator()(std::string const& x) const {
        auto elems = TfStringSplit(x, ":");
        std::transform(elems.begin(), elems.end(), elems.begin(), TfMakeValidIdentifier);
        return TfStringJoin(elems, ":");
    }
};

template <class T>
static std::string _CleanName(const std::string& inName,
...
static void _GetStringMetadata(const MetaData& alembicMetadata, MetadataMap& usdMetadata, const TfToken& field) {
    std::string value = alembicMetadata.get(_AmdName(field));
    if (!value.empty()) {
...
static void _GetTokenMetadata(const MetaData& alembicMetadata, MetadataMap& usdMetadata, const TfToken& field) {
    std::string value = alembicMetadata.get(_AmdName(field));
    if (!value.empty()) {
...
static void _GetDoubleMetadata(const MetaData& alembicMetadata, MetadataMap& usdMetadata, const TfToken& field) {
    std::string value = alembicMetadata.get(_AmdName(field));
    if (!value.empty()) {
        char* end;
...
struct _AlembicPropertyHelper<ICompoundProperty> {
    ICompoundProperty operator()(const ICompoundProperty& parent,
...
struct _AlembicPropertyHelper<IScalarProperty> {
    IScalarProperty operator()(const ICompoundProperty& parent,
                               const std::string& name,
                               SchemaInterpMatching matching = kStrictMatching) const {
        if (const PropertyHeader* header = parent.getPropertyHeader(name)) {
            if (header->isScalar()) {
...
struct _AlembicPropertyHelper<ITypedScalarProperty<T>> {
    ITypedScalarProperty<T> operator()(const ICompoundProperty& parent,
                                       const std::string& name,
                                       SchemaInterpMatching matching = kStrictMatching) const {
        if (const PropertyHeader* header = parent.getPropertyHeader(name)) {
...
struct _AlembicPropertyHelper<IArrayProperty> {
    IArrayProperty operator()(const ICompoundProperty& parent,
                              const std::string& name,
                              SchemaInterpMatching matching = kStrictMatching) const {
        if (const PropertyHeader* header = parent.getPropertyHeader(name)) {
            if (header->isArray()) {
...
struct _AlembicPropertyHelper<ITypedArrayProperty<T>> {
    ITypedArrayProperty<T> operator()(const ICompoundProperty& parent,
                                      const std::string& name,
                                      SchemaInterpMatching matching = kStrictMatching) const {
        if (const PropertyHeader* header = parent.getPropertyHeader(name)) {
...
struct _AlembicPropertyHelper<ITypedGeomParam<T>> {
    ITypedGeomParam<T> operator()(const ICompoundProperty& parent,
                                  const std::string& name,
                                  SchemaInterpMatching matching = kStrictMatching) const {
        if (const PropertyHeader* header = parent.getPropertyHeader(name)) {
...
public:
    ...
    AlembicProperty(const SdfPath& path, const std::string& name, const IObject& parent);
    AlembicProperty(const SdfPath& path, const std::string& name, const ICompoundProperty& parent);
...
AlembicProperty::AlembicProperty(const SdfPath& path, const std::string& name) : _path(path), _parent(), _name(name) {
    // Do nothing
}
...
AlembicProperty::AlembicProperty(const SdfPath& path, const std::string& name, const ICompoundProperty& parent)
    : _path(path), _parent(parent), _name(name) {
    // Do nothing
}

ICompoundProperty AlembicProperty::GetParent() const {
    return _parent;
}

const std::string& AlembicProperty::GetName() const {
    return _name;
}

const PropertyHeader* AlembicProperty::GetHeader() const {
...
public:
    ...
    TypeRef AddFallbackType() { return AddType(std::string()); }

    /// Returns the object holding the conversions registry.
    const UsdAbc_AlembicDataConversion& GetConversions() const { return _conversions.data; }
...
public:
    ...
    /// Open an archive.
    bool Open(const std::string& filePath, std::string* errorLog, const SdfFileFormat::FileFormatArguments& args);
    ...
    bool HasValue(const SdfPath& path, Index index, const UsdAbc_AlembicDataAny& value) const;
...
private:
    ...
    bool _HasField(const Property* property, const TfToken& fieldName, const UsdAbc_AlembicDataAny& value) const;
    bool _HasValue(const Property* property, const ISampleSelector& selector, const UsdAbc_AlembicDataAny& value) const;
...
static void _ReadPrimChildren(_ReaderContext& context,
...
bool _ReaderContext::Open(const std::string& filePath,
                          std::string* errorLog,
    ...
    switch (abcType) {
        case IFactory::kHDF5:
            format = "HDF5";
            break;
        case IFactory::kOgawa:
            format = "Ogawa";
            break;
        case IFactory::kLayer:
            format = "Layer";
            break;
        default:
        case IFactory::kUnknown:
            format = "Unknown";
            break;
    }
    if (!archive.valid()) {
    ...
    if (IsFlagSet(UsdAbc_AlembicContextFlagNames->verbose)) {
        TF_STATUS("Opened %s file written by Alembic %s", format.c_str(),
    ...
    for (size_t i = 0, n = root.getNumChildren(); i != n; ++i) {
        ...
        std::string name =
                _CleanName(child.getName(), " _", usedRootNames, _AlembicFixName(), &SdfPath::IsValidIdentifier);
        ...
        if (reRoot != args.end()) {
            if (!TfIsValidIdentifier(reRoot->second)) {
                TF_WARN("[usdAbc] Ignoring re-root because identifer '%s' is"
                        " not valid (%s).",
                        reRoot->second.c_str(), filePath.c_str());
            } else
    ...
    if (!_instanceSources.empty()) {
        const bool instanceable = !IsFlagSet(UsdAbc_AlembicContextFlagNames->disableInstancing);
    ...
    if (const PropertyHeader* property = root.getProperties().getPropertyHeader("Usd")) {
        const MetaData& metadata = property->getMetaData();
        _GetDoubleMetadata(metadata, _pseudoRoot->metadata, SdfFieldKeys->StartTimeCode);
        _GetDoubleMetadata(metadata, _pseudoRoot->metadata, SdfFieldKeys->EndTimeCode);

        _GetDoubleMetadata(metadata, _pseudoRoot->metadata, SdfFieldKeys->FramesPerSecond);

        _GetTokenMetadata(metadata, _pseudoRoot->metadata, UsdGeomTokens->upAxis);

        // Read the default prim name.
        _GetTokenMetadata(metadata, _pseudoRoot->metadata, SdfFieldKeys->DefaultPrim);
    }
    ...
    if (!_pseudoRoot->children.empty()) {
        ...
        _pseudoRoot->metadata.emplace(std::piecewise_construct, std::forward_as_tuple(SdfFieldKeys->DefaultPrim),
...
bool _ReaderContext::IsFlagSet(const TfToken& flagName) const {
    return _flags.count(flagName);
}

_ReaderContext::Prim& _ReaderContext::AddPrim(const SdfPath& path) {
    return _prims[path];
}

bool _ReaderContext::IsInstance(const IObject& object) const {
    // object is an instance if it's a key in _instances.
    return _instances.find(object.getFullName()) != _instances.end();
}

_ReaderContext::Prim& _ReaderContext::AddInstance(const SdfPath& path, const IObject& object) {
...
_ReaderContext::Property& _ReaderContext::FindOrCreateProperty(const SdfPath& path) {
    return AddPrim(path.GetPrimPath()).propertiesCache[path.GetNameToken()];
}

const _ReaderContext::Property* _ReaderContext::FindProperty(const SdfPath& path) {
    _PrimMap::const_iterator i = _prims.find(path.GetPrimPath());
    if (i != _prims.end()) {
        PropertyMap::const_iterator j = i->second.propertiesCache.find(path.GetNameToken());
...
void _ReaderContext::AddSampleTimes(const TimeSamples& sampleTimes) {
    sampleTimes.AddTo(&_allTimeSamples);
}

bool _ReaderContext::HasSpec(const SdfPath& path) const {
...
SdfSpecType _ReaderContext::GetSpecType(const SdfPath& path) const {
    if (const Prim* prim = _GetPrim(path)) {
...
bool _ReaderContext::HasField(const SdfPath& path, const TfToken& fieldName, const UsdAbc_AlembicDataAny& value) const {
    TRACE_FUNCTION();
    ...
    for (const auto& v : _prims) {
        ...
        if (&prim != _pseudoRoot) {
            for (const auto& w : prim.propertiesCache) {
                if (!visitor->VisitSpec(owner, primPath.AppendProperty(w.first))) {
                    return;
                }
            }
        }
...
TfTokenVector _ReaderContext::List(const SdfPath& path) const {
    TRACE_FUNCTION();
...
const std::set<double>& _ReaderContext::ListAllTimeSamples() const {
...
void _ReaderContext::_FindInstances(const IObject& parent, _SourceToInstancesMap* instances) {
...
void _ReaderContext::_SetupInstancing(const _SourceToInstancesMap& instances,
                                      const _ObjectReaderSet& promotable,
                                      std::set<std::string>* usedNames) {
    ...
    for (const auto& value : instances) {
        ...
        const std::string& sourceFullName = promoted ? source->getParent()->getFullName() : source->getFullName();
        if (promoted) {
            for (const auto& instance : value.second) {
                _instances[instance->getParent()->getFullName()] = sourceFullName;
            }
        } else {
            // Not promotable.
            for (const auto& instance : value.second) {
        ...
        const std::string prototypeName = (j != std::string::npos) ? sourceFullName.substr(j + 1) : sourceFullName;
...
const _ReaderContext::Prim* _ReaderContext::_GetPrim(const SdfPath& path) const {
...
const _ReaderContext::Property* _ReaderContext::_GetProperty(const Prim& prim, const SdfPath& path) const {
    ...
    PropertyMap::const_iterator i = prim.propertiesCache.find(path.GetNameToken());
    return i == prim.propertiesCache.end() ? NULL : &i->second;
}

bool _ReaderContext::_HasField(const Prim* prim, const TfToken& fieldName, const UsdAbc_AlembicDataAny& value) const {
...
bool _ReaderContext::_HasField(const Property* property,
    ...
    if (fieldName == SdfFieldKeys->Default) {
        ...
        return value.Set(property->uniform ? SdfVariabilityUniform : SdfVariabilityVarying);
    }
...
bool _ReaderContext::_HasValue(const Property* property,
...
template <typename T>
size_t _GetNumSamples(const T& object) {
    size_t nSamples = object.getNumSamples();
    if (!object.isConstant()) return nSamples;
...
template <class T>
static _AlembicTimeSamples _GetSampleTimes(const T& object) {
...
template <class T>
static _AlembicTimeSamples _GetSampleTimes(const ISchemaObject<T>& object) {
    return _GetSampleTimes(object.getSchema());
}

static TfToken _GetRole(const std::string& role) {
    if (role.empty()) {
        return TfToken();
    }
    if (role == "point") {
...
static SdfValueTypeName _GetInterpretation(const SdfValueTypeName& typeName, const TfToken& role) {
    if (role == "quat") {
    ...
    SdfValueTypeName result = SdfSchema::GetInstance().FindType(typeName.GetType(), role);
    return result ? result : typeName;
}

static TfToken _GetInterpolation(GeometryScope geometryScope) {
...
public:
    ...
    _PrimReaderContext(_ReaderContext&, const IObject& prim, const SdfPath& path);
    ...
    template <class T>
    void AddProperty(const TfToken& name, const SdfValueTypeName& typeName, const T& converter);
    ...
    void AddOutOfSchemaProperty(const std::string& name, const AlembicProperty& property);
    ...
    template <typename T>
    bool GetSample(typename T::schema_type::Sample& sample, const ISampleSelector& iss) const {
...
private:
    ...
    Property& _AddProperty(const TfToken& name,
                           ...
                           bool isOutOfSchemaProperty);
    void _GetPropertyMetadata(const MetaData& alembicMetadata, Property* property, bool isOutOfSchemaProperty) const;
...
_PrimReaderContext::_PrimReaderContext(_ReaderContext& context, const IObject& prim, const SdfPath& path)
    : _context(context), _prim(prim), _path(path) {
...
IObject _PrimReaderContext::GetObject() const {
    return _prim;
}

const SdfPath& _PrimReaderContext::GetPath() const {
    return _path;
}

std::string _PrimReaderContext::GetUsdName(const std::string& name) const {
    return _CleanName(name, " .", _usedNames, _AlembicFixNamespacedName(), &SdfPath::IsValidNamespacedIdentifier);
}
...
void _PrimReaderContext::SetSchema(const std::string& schemaName) {
    _unextractedSchema.clear();
    _schema = ICompoundProperty(_prim.getProperties(), schemaName, ErrorHandler::kQuietNoopPolicy);
    if (_schema.valid()) {
...
AlembicProperty _PrimReaderContext::Extract(const std::string& name) {
    std::vector<std::string>::iterator i = std::find(_unextracted.begin(), _unextracted.end(), name);
...
AlembicProperty _PrimReaderContext::ExtractSchema(const std::string& name) {
    std::vector<std::string>::iterator i = std::find(_unextractedSchema.begin(), _unextractedSchema.end(), name);
...
std::vector<std::string> _PrimReaderContext::GetUnextractedNames() const {
    return _unextracted;
}

template <class T>
void _PrimReaderContext::AddProperty(const TfToken& name, const SdfValueTypeName& typeName, const T& converter) {
...
template <class T>
void _PrimReaderContext::AddUniformProperty(const TfToken& name, const SdfValueTypeName& typeName, const T& converter) {
    if (converter(IsValidTag())) {
    ...
    if (header->isCompound()) {
        ...
        for (size_t i = 0, n = compound.getNumProperties(); i != n; ++i) {
            ...
            const std::string cleanName =
                    _CleanName(rawName, " .", usedNames, _AlembicFixName(), &SdfPath::IsValidIdentifier);
    ...
    if (usdTypeName) {
        _PrimReaderContext::Property& prop =
                (TfGetEnvSetting(USD_ABC_WRITE_UV_AS_ST_TEXCOORD2FARRAY) && name == UsdAbcPropertyNames->uvIndices)
                        ? (_AddProperty(UsdAbcPropertyNames->stIndices, usdTypeName, header->getMetaData(), sampleTimes,
                                        isOutOfSchema))
                        : (_AddProperty(TfToken(name), usdTypeName, header->getMetaData(), sampleTimes, isOutOfSchema));
        prop.converter =
                std::bind(_context.GetSchema().GetConversions().GetToUsdConverter(alembicType, prop.typeName),
                          property.GetParent(), property.GetName(), std::placeholders::_2, std::placeholders::_1);
    } else {
        TF_WARN("No conversion for \"%s\" of type \"%s\" at <%s>", name.c_str(), alembicType.Stringify().c_str(),
                GetPath().GetText());
    }
}
...
_PrimReaderContext::Property& _PrimReaderContext::_AddProperty(const TfToken& name,
                                                               ...
                                                               const GetSampleTimes& sampleTimes,
                                                               bool isOutOfSchemaProperty) {
    return _AddProperty(name, typeName, metadata(MetaDataTag()), sampleTimes(SampleTimesTag()), isOutOfSchemaProperty);
}
...
_PrimReaderContext::Property& _PrimReaderContext::_AddProperty(const TfToken& name,
    ...
    fprintf(stdout, "%*s%s%s %s\n", 2 * ((int)GetPath().GetPathElementCount() + 1), "",
            property.metadata[SdfFieldKeys->Custom].UncheckedGet<bool>() ? "custom " : "",
...
void _PrimReaderContext::_GetPropertyMetadata(const MetaData& alembicMetadata,
                                              Property* property,
    ...
    if (!property->typeName) {
        property->typeName = SdfSchema::GetInstance().FindType(alembicMetadata.get(_AmdName(SdfFieldKeys->TypeName)));
    }
    ...
    property->typeName = _GetInterpretation(property->typeName, _GetRole(alembicMetadata.get("interpretation")));

    // Set the interpolation if there is one.
    if (!alembicMetadata.get("geoScope").empty()) {
        const TfToken interpolation = _GetInterpolation(GetGeometryScope(alembicMetadata));
    ...
    // Custom metadata.
    _GetStringMetadata(alembicMetadata, usdMetadata, UsdAbcCustomMetadata->riName);
    _GetStringMetadata(alembicMetadata, usdMetadata, UsdAbcCustomMetadata->riType);
    _GetBoolMetadata(alembicMetadata, usdMetadata, UsdAbcCustomMetadata->gprimDataRender);
}

_PrimReaderContext _PrimReaderContext::GetParentContext() const {
    _PrimReaderContext parentContext(_context, _prim.getParent(), _path.GetParentPath());
...
static VtValue _CopyGenericValue(const AlembicType& src) {
    typedef typename PODTraitsFromEnum<AlembicTraits::pod_enum>::value_type SrcType;
...
static VtValue _CopyGenericValue(const shared_ptr<TypedArraySample<AlembicTraits>>& src) {
    typedef typename PODTraitsFromEnum<AlembicTraits::pod_enum>::value_type SrcType;
    static const int SrcExtent = AlembicTraits::extent;
    ...
    _ConvertPODToUsdArray<UsdType, SrcType, SrcExtent>()(result.data(), srcData, src->size());
...
struct _CopySynthetic {
    ...
    bool operator()(_IsValidTag) const { return true; }

    const MetaData& operator()(_MetaDataTag) const { return metadata; }

    _AlembicTimeSamples operator()(_SampleTimesTag) const { return _AlembicTimeSamples(1, 0.0); }

    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const { return dst.Set(value); }
};
...
struct _CopyGeneric {
    ...
    bool operator()(_IsValidTag) const { return object.valid(); }

    const MetaData& operator()(_MetaDataTag) const { return object.getMetaData(); }

    _AlembicTimeSamples operator()(_SampleTimesTag) const { return _GetSampleTimes(object); }

    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        return dst.Set(_CopyGenericValue<AlembicTraits, UsdValueType>(object.getValue(iss)));
    }
};
...
struct _CopyGeneric<ITypedGeomParam<T>, UsdValueType, expand> {
    ...
    _CopyGeneric(const AlembicProperty& object_) : object(object_.Cast<GeomParamType>()) {}
    ...
    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        ...
        return dst.Set(_CopyGenericValue<AlembicTraits, UsdValueType>(sample.getVals()));
    }
};
...
struct _CopyIndices {
    ...
    GeomParamType object;
    _CopyIndices(const AlembicProperty& object_) : object(object_.Cast<GeomParamType>()) {}
    ...
    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        if (object.isIndexed()) {
            typename GeomParamType::sample_type sample;
            object.getIndexed(sample, iss);
            return dst.Set(_CopyGenericValue<Uint32TPTraits, int>(sample.getIndices()));
        }
        return false;
    }
};
...
struct _CopyBoundingBox : _CopyGeneric<IBox3dProperty> {
    _CopyBoundingBox(const AlembicProperty& object_) : _CopyGeneric<IBox3dProperty>(object_) {}
...
struct _CopyOrientation : _CopyGeneric<IStringProperty> {
    _CopyOrientation(const AlembicProperty& object_) : _CopyGeneric<IStringProperty>(object_) {}
...
struct _CopyVisibility : _CopyGeneric<ICharProperty> {
    _CopyVisibility(const AlembicProperty& object_) : _CopyGeneric<ICharProperty>(object_) {}

    using _CopyGeneric<ICharProperty>::operator();

    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        const ObjectVisibility vis = static_cast<ObjectVisibility>(object.getValue(iss));
        switch (vis) {
            ...
            case kVisibilityVisible: {
                const std::string authoredValue = (vis == kVisibilityVisible ? "kVisibilityVisible" : TfStringify(vis));
                // Usd doesn't support this.
                _PostUnsupportedValueWarning(object, iss, WarningVisibility, authoredValue.c_str(),
                                             "kVisibilityDeferred");
                // Fall through
            }
...
struct _CopyAdskColor : _CopyGeneric<IC4fProperty> {
    _CopyAdskColor(const AlembicProperty& object_) : _CopyGeneric<IC4fProperty>(object_) {}
...
struct _CopyCameraFocalLength : _CopyCameraBase {
    ...
    _CopyCameraFocalLength(const ICamera& object_) : _CopyCameraBase(object_) {};
...
struct _CopyCameraHorizontalAperture : _CopyCameraBase {
    ...
    _CopyCameraHorizontalAperture(const ICamera& object_) : _CopyCameraBase(object_) {};
...
struct _CopyCameraVerticalAperture : _CopyCameraBase {
    ...
    _CopyCameraVerticalAperture(const ICamera& object_) : _CopyCameraBase(object_) {};
...
struct _CopyCameraHorizontalApertureOffset : _CopyCameraBase {
    ...
    _CopyCameraHorizontalApertureOffset(const ICamera& object_) : _CopyCameraBase(object_) {};
    ...
    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        ...
        const float horizontalApertureOffset = sample.getHorizontalFilmOffset() * sample.getLensSqueezeRatio() * 10.0;
...
struct _CopyCameraVerticalApertureOffset : _CopyCameraBase {
    ...
    _CopyCameraVerticalApertureOffset(const ICamera& object_) : _CopyCameraBase(object_) {};
    ...
    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        ...
        const float verticalApertureOffset = sample.getVerticalFilmOffset() * sample.getLensSqueezeRatio() * 10.0;
...
struct _CopyCameraClippingRange : _CopyCameraBase {
    ...
    _CopyCameraClippingRange(const ICamera& object_) : _CopyCameraBase(object_) {};

    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        const CameraSample sample = object.getSchema().getValue(iss);

        return dst.Set(GfVec2f(sample.getNearClippingPlane(), sample.getFarClippingPlane()));
    }
};

/// Copy a subdivision scheme from an IStringProperty.
struct _CopySubdivisionScheme : _CopyGeneric<IStringProperty> {
    _CopySubdivisionScheme(const AlembicProperty& object_) : _CopyGeneric<IStringProperty>(object_) {}
    ...
    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        ...
        _PostUnsupportedValueWarning(object, iss, WarningSubdivisionScheme, value, "catmull-clark");
        return dst.Set(UsdGeomTokens->catmullClark);
    }
};
...
struct _CopyInterpolateBoundary : _CopyGeneric<IInt32Property> {
    _CopyInterpolateBoundary(const AlembicProperty& object_) : _CopyGeneric<IInt32Property>(object_) {}
    ...
    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        switch (object.getValue(iss)) {
            ...
            default:
                _PostUnsupportedValueWarning(object, iss, WarningInterpolateBoundary,
...
struct _CopyFaceVaryingInterpolateBoundary : _CopyGeneric<IInt32Property> {
    ...
    bool operator()(const UsdAbc_AlembicDataAny& dst, const ISampleSelector& iss) const {
        switch (object.getValue(iss)) {
            ...
            default:
                _PostUnsupportedValueWarning(object, iss, WarningFaceVaryingInterpolateBoundary,
                                             TfStringify(object.getValue(iss)).c_str(), "0");
                // Fall-through
            case 0:
...
struct _CopyFaceSetFamilyType : _CopyFaceSetBase {
    ...
    _CopyFaceSetFamilyType(const IFaceSet& object_) : _CopyFaceSetBase(object_) {};
...
static TfToken _ConvertCurveBasis(BasisType value) {
    switch (value) {
        default:
        case kNoBasis:
        case kBezierBasis:
        ...
        case kHermiteBasis:
            TF_WARN("'hermite' basis is no longer an allowed token for "
                    "UsdGeomBasisCurves.");
            return UsdGeomTokens->hermite;
        case kPowerBasis:
            TF_WARN("'power' basis is no longer an allowed token for "
                    "UsdGeomBasisCurves.");
            return UsdGeomTokens->power;
    }
}

static TfToken _ConvertCurveType(CurveType value) {
    switch (value) {
        default:
        case kCubic:
...
static TfToken _ConvertCurveWrap(CurvePeriodicity value) {
    switch (value) {
        default:
        case kNonPeriodic:
...
static void _ReadGprim(_PrimReaderContext* context) {
    // Add properties.
    context->AddProperty(UsdGeomTokens->extent, SdfValueTypeNames->Float3Array,
...
static void _ReadUserProperties(_PrimReaderContext* context) {
    // Add userProperties.
    context->AddOutOfSchemaProperty(UsdAbcPropertyNames->userProperties, context->ExtractSchema(".userProperties"));
}

static void _ReadImageable(_PrimReaderContext* context) {
    // Add properties.
    context->AddProperty(UsdGeomTokens->visibility, SdfValueTypeNames->Token,
                         _CopyVisibility(context->Extract(kVisibilityPropertyName)));
}

static void _ReadMayaColor(_PrimReaderContext* context) {
    static const TfToken displayColor("primvars:displayColor");

    // Add properties.
    context->AddProperty(displayColor, SdfValueTypeNames->Color3fArray,
                         _CopyAdskColor(context->ExtractSchema("adskDiffuseColor")));
}

static void _ReadOther(_PrimReaderContext* context) {
    ...
    for (const auto& name : context->GetUnextractedNames()) {
        context->AddOutOfSchemaProperty(context->GetUsdName(name), context->Extract(name));
    }
}

template <class T, class UsdValueType>
void _ReadProperty(_PrimReaderContext* context, const char* name, TfToken propName, SdfValueTypeName typeName) {
    ...
    if (prop.Cast<T>().isIndexed()) {
        context->AddProperty(propName, typeName, _CopyGeneric<T, UsdValueType, false>(prop));
        context->AddProperty(TfToken(SdfPath::JoinIdentifier(propName, UsdGeomTokens->indices)),
                             SdfValueTypeNames->IntArray, _CopyIndices<T>(prop));
    } else {
        context->AddProperty(propName, typeName, _CopyGeneric<T, UsdValueType>(prop));
    }
}

static void _ReadOrientation(_PrimReaderContext* context) {
    AlembicProperty orientation = context->Extract(_AmdName(UsdGeomTokens->orientation));
    if (orientation.Cast<IStringProperty>().valid()) {
        context->AddProperty(UsdGeomTokens->orientation, SdfValueTypeNames->Token, _CopyOrientation(orientation));
    } else {
        ...
        context->AddUniformProperty(UsdGeomTokens->orientation, SdfValueTypeNames->Token,
...
static void _ReadUnknown(_PrimReaderContext* context) {
    // Set prim type.
...
static void _ReadGeomBase(_PrimReaderContext* context) {
...
static void _ReadXform(_PrimReaderContext* context) {
    typedef IXform Type;
    ...
    for (index_t i = 0; i < nSamples; ++i) {
        if (!schema.getInheritsXforms(ISampleSelector(i))) {
            TF_WARN("Ignoring transform that doesn't inherit at "
                    ...
                    schema.getTimeSampling()->getSampleTime(i), context->GetPath().GetText());
            return;
        }
    }
    ...
    if (nSamples > 0) {
        ...
        context->AddUniformProperty(UsdGeomTokens->xformOpOrder, SdfValueTypeNames->TokenArray,
...
static void _ReadPolyMesh(_PrimReaderContext* context) {
    ...
    // Add properties.
    context->AddProperty(UsdGeomTokens->points, SdfValueTypeNames->Point3fArray,
                         _CopyGeneric<IP3fArrayProperty, GfVec3f>(context->ExtractSchema("P"), kNoMatching));
    context->AddProperty(UsdGeomTokens->velocities, SdfValueTypeNames->Vector3fArray,
                         _CopyGeneric<IV3fArrayProperty, GfVec3f>(context->ExtractSchema(".velocities")));
    context->AddProperty(UsdGeomTokens->normals, SdfValueTypeNames->Normal3fArray,
                         _CopyGeneric<IN3fGeomParam, GfVec3f>(context->ExtractSchema("N")));
    context->AddProperty(UsdGeomTokens->faceVertexIndices, SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".faceIndices")));
    context->AddProperty(UsdGeomTokens->faceVertexCounts, SdfValueTypeNames->IntArray,
    ...
    context->AddUniformProperty(UsdGeomTokens->subdivisionScheme, SdfValueTypeNames->Token,
...
static void _ReadSubD(_PrimReaderContext* context) {
    ...
    context->AddProperty(UsdGeomTokens->interpolateBoundary, SdfValueTypeNames->Token,
                         _CopyInterpolateBoundary(context->ExtractSchema(".interpolateBoundary")));
    context->AddProperty(
            UsdGeomTokens->faceVaryingLinearInterpolation, SdfValueTypeNames->Token,
            _CopyFaceVaryingInterpolateBoundary(context->ExtractSchema(".faceVaryingInterpolateBoundary")));
    context->AddProperty(UsdGeomTokens->holeIndices, SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".holes")));
    context->AddProperty(UsdGeomTokens->cornerIndices, SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".cornerIndices")));
    context->AddProperty(UsdGeomTokens->cornerSharpnesses, SdfValueTypeNames->FloatArray,
                         _CopyGeneric<IFloatArrayProperty, float>(context->ExtractSchema(".cornerSharpnesses")));
    context->AddProperty(UsdGeomTokens->creaseIndices, SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".creaseIndices")));
    context->AddProperty(UsdGeomTokens->creaseLengths, SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".creaseLengths")));
    context->AddProperty(UsdGeomTokens->creaseSharpnesses, SdfValueTypeNames->FloatArray,
...
static void _ReadFaceSet(_PrimReaderContext* context) {
    ...
    context->AddProperty(UsdGeomTokens->indices, SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".faces")));
    context->AddUniformProperty(UsdGeomTokens->elementType, SdfValueTypeNames->Token,
                                _CopySynthetic(UsdGeomTokens->face));

    context->AddUniformProperty(UsdGeomTokens->familyName, SdfValueTypeNames->Token,
                                _CopySynthetic(UsdAbcPropertyNames->defaultFamilyName));

    _PrimReaderContext parentPrimContext = context->GetParentContext();

    parentPrimContext.AddUniformProperty(UsdAbcPropertyNames->defaultFamilyTypeAttributeName, SdfValueTypeNames->Token,
...
static void _ReadCurves(_PrimReaderContext* context) {
    ...
    if (sample.getType() == kVariableOrder) {
        ...
        context->AddProperty(UsdGeomTokens->order, SdfValueTypeNames->IntArray,
                             _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".orders")));
        context->AddProperty(UsdGeomTokens->knots, SdfValueTypeNames->DoubleArray,
        ...
        context->AddProperty(UsdGeomTokens->tangents, SdfValueTypeNames->Vector3fArray,
                             _CopyHermiteTangents(context->ExtractSchema("P")));
    } else {
        context->GetPrim().typeName = UsdAbcPrimTypeNames->BasisCurves;
        context->AddUniformProperty(UsdGeomTokens->type, SdfValueTypeNames->Token,
                                    _CopySynthetic(_ConvertCurveType(sample.getType())));
        context->AddUniformProperty(UsdGeomTokens->basis, SdfValueTypeNames->Token,
                                    _CopySynthetic(_ConvertCurveBasis(sample.getBasis())));
        context->AddUniformProperty(UsdGeomTokens->wrap, SdfValueTypeNames->Token,
    ...
    context->AddProperty(UsdGeomTokens->normals, SdfValueTypeNames->Normal3fArray,
                         _CopyGeneric<IN3fGeomParam, GfVec3f>(context->ExtractSchema("N")));
    context->AddProperty(UsdGeomTokens->curveVertexCounts, SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema("nVertices")));
    context->AddProperty(UsdGeomTokens->widths, SdfValueTypeNames->FloatArray,
                         _CopyGeneric<IFloatGeomParam, float>(context->ExtractSchema("width")));
}

static void _ReadPoints(_PrimReaderContext* context) {
    ...
    context->AddProperty(UsdGeomTokens->ids, SdfValueTypeNames->Int64Array,
...
static void _ReadCameraParameters(_PrimReaderContext* context) {
    ...
    context->AddProperty(UsdGeomTokens->horizontalAperture, SdfValueTypeNames->Float,
                         _CopyCameraHorizontalAperture(object));
    context->AddProperty(UsdGeomTokens->verticalAperture, SdfValueTypeNames->Float,
                         _CopyCameraVerticalAperture(object));
    context->AddProperty(UsdGeomTokens->horizontalApertureOffset, SdfValueTypeNames->Float,
                         _CopyCameraHorizontalApertureOffset(object));
    context->AddProperty(UsdGeomTokens->verticalApertureOffset, SdfValueTypeNames->Float,
                         _CopyCameraVerticalApertureOffset(object));
    context->AddProperty(UsdGeomTokens->clippingRange, SdfValueTypeNames->Float2, _CopyCameraClippingRange(object));
...
static _ReaderContext::Ordering _GetOrderingMetadata(const MetaData& alembicMetadata, const TfToken& field) {
    ...
    if (!value.empty()) {
        ...
        if (!names.empty()) {
            return _ReaderContext::Ordering(TfTokenVector(names.begin(), names.end()));
        }
    }
    return _ReaderContext::Ordering();
}

static void _GetPrimMetadata(const MetaData& metadata, _ReaderContext::Prim& prim) {
    // def/over.
    ...
    prim.primOrdering = _GetOrderingMetadata(metadata, SdfFieldKeys->PrimOrder);

    // Add property ordering.
    prim.propertyOrdering = _GetOrderingMetadata(metadata, SdfFieldKeys->PropertyOrder);
}

static std::string _ComputeSchemaName(const _ReaderContext& context, const IObject& object) {
    ...
    std::string value = object.getMetaData().get(_AmdName(SdfFieldKeys->TypeName));
    if (!value.empty()) {
        return value;
    }
...
static ICompoundProperty _GetSchemaProperty(const IObject& object) {
    return ICompoundProperty(object.getProperties(), TYPE::schema_type::info_type::defaultName(),
...
static std::string _ReadPrim(_ReaderContext& context,
    ...
    // Choose the name.
    std::string name =
            _CleanName(object.getName(), " _", *usedSiblingNames, _AlembicFixName(), &SdfPath::IsValidIdentifier);
    ...
    if (!context.IsInstance(object)) {
        ...
        do {
            ...
            ICompoundProperty parentProperties = _GetSchemaProperty<IXform>(parent);
            ...
            } else if (ICamera::matches(object.getMetaData(), kSchemaTitleMatching)) {
    ...
    // prim with nothing but a reference at the old path.
        ...
        if (!instance->prototype.IsEmpty()) {
            ...
            const std::string prototypePath =
                    object.isInstanceRoot() ? IObject(object).instanceSourcePath() : object.getFullName();
            TF_CODING_ERROR("Instance %s has no prototype at %s.", object.getFullName().c_str(), prototypePath.c_str());
    ...
    if (!instance || prim.typeName.IsEmpty()) {
        ...
        fprintf(stdout, "%*s%s%s%s \"%s\" { # %s, %s\n", 2 * (int)(path.GetPathElementCount() - 1), "",
                prim.specifier == SdfSpecifierOver ? "over" : "def", prim.typeName.IsEmpty() ? "" : " ",
                prim.typeName.GetText(), name.empty() ? "<merge-with-parent>" : name.c_str(), schemaName.c_str(),
...
static const _ReaderSchema& _GetSchema() {
...
void UsdAbc_AlembicDataReader::TimeSamples::Swap(TimeSamples& other) {
    _times.swap(other._times);
}

bool UsdAbc_AlembicDataReader::TimeSamples::IsEmpty() const {
    return _times.empty();
}

size_t UsdAbc_AlembicDataReader::TimeSamples::GetSize() const {
    return _times.size();
}

UsdAbc_TimeSamples UsdAbc_AlembicDataReader::TimeSamples::GetTimes() const {
    return UsdAbc_TimeSamples(_times.begin(), _times.end());
}

double UsdAbc_AlembicDataReader::TimeSamples::operator[](size_t index) const {
    return _times[index];
}

void UsdAbc_AlembicDataReader::TimeSamples::AddTo(UsdAbc_TimeSamples* samples) const {
    samples->insert(_times.begin(), _times.end());
}

bool UsdAbc_AlembicDataReader::TimeSamples::FindIndex(double usdTime, Index* index) const {
    _UsdTimeCodes::const_iterator i = std::lower_bound(_times.begin(), _times.end(), usdTime);
...
template <class T>
static typename std::vector<T>::const_iterator UsdAbc_lower_bound(const std::vector<T>& c, T x) {
...
template <class T>
bool UsdAbc_AlembicDataReader::TimeSamples::Bracket(const T& samples, double usdTime, double* tLower, double* tUpper) {
    if (samples.empty()) {
        return false;
    }
...
template bool UsdAbc_AlembicDataReader::TimeSamples::Bracket(const UsdAbc_TimeSamples& samples,
                                                             double usdTime,
                                                             double* tLower,
                                                             double* tUpper);
...
UsdAbc_AlembicDataReader::UsdAbc_AlembicDataReader() : _impl(new UsdAbc_AlembicDataReaderImpl) {
...
bool UsdAbc_AlembicDataReader::Open(const std::string& filePath, const SdfFileFormat::FileFormatArguments& args) {
    TRACE_FUNCTION();
...
void UsdAbc_AlembicDataReader::Close() {
    _impl->Close();
}

std::string UsdAbc_AlembicDataReader::GetErrors() const {
    return _errorLog;
}

void UsdAbc_AlembicDataReader::SetFlag(const TfToken& flagName, bool set) {
    _impl->SetFlag(flagName, set);
}

bool UsdAbc_AlembicDataReader::HasSpec(const SdfPath& path) const {
    return _impl->HasSpec(path);
}

SdfSpecType UsdAbc_AlembicDataReader::GetSpecType(const SdfPath& path) const {
...
bool UsdAbc_AlembicDataReader::HasValue(const SdfPath& path, Index index, VtValue* value) const {
    return _impl->HasValue(path, index, UsdAbc_AlembicDataAny(value));
}

void UsdAbc_AlembicDataReader::VisitSpecs(const SdfAbstractData& owner, SdfAbstractDataSpecVisitor* visitor) const {
    return _impl->VisitSpecs(owner, visitor);
}

TfTokenVector UsdAbc_AlembicDataReader::List(const SdfPath& path) const {
    return _impl->List(path);
}

const std::set<double>& UsdAbc_AlembicDataReader::ListAllTimeSamples() const {
