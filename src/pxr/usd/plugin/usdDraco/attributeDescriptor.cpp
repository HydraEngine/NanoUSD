...
const std::string UsdDracoAttributeDescriptor::METADATA_VALUES_TIME_KEY = "values_time";
...
UsdDracoAttributeDescriptor::UsdDracoAttributeDescriptor(Status status)
    : UsdDracoAttributeDescriptor(status,
...
UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::Create(draco::GeometryAttribute::Type attributeType,
    ...
    return UsdDracoAttributeDescriptor(VALID, attributeType, name, dataType, isPrimvar, numComponents, shape, isHalf,
...
const std::set<TfToken>& UsdDracoAttributeDescriptor::GetSupportedInterpolations() {
    static const std::set<TfToken> supportedInterpolations = {UsdGeomTokens->vertex, UsdGeomTokens->faceVarying};
...
UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForPositions(const UsdGeomMesh& mesh) {
    ...
    return FromUsdAttribute(attribute, draco::GeometryAttribute::POSITION, name, isPrimvar, indicesTime, interpolation);
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForTexCoords(const UsdGeomMesh& mesh) {
    return FromUsdMesh(mesh, draco::GeometryAttribute::TEX_COORD, GetTexCoordsName());
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForNormals(const UsdGeomMesh& mesh) {
    return FromUsdMesh(mesh, draco::GeometryAttribute::NORMAL, GetNormalsName());
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForPositions(const draco::Mesh& mesh) {
    const bool isPrimvar = false;
    return FromDracoMesh(mesh, draco::GeometryAttribute::POSITION, GetPositionsName(), isPrimvar);
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForTexCoords(const draco::Mesh& mesh) {
    const bool isPrimvar = true;
    return FromDracoMesh(mesh, draco::GeometryAttribute::TEX_COORD, GetTexCoordsName(), isPrimvar);
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForNormals(const draco::Mesh& mesh) {
    const bool isPrimvar = true;
    return FromDracoMesh(mesh, draco::GeometryAttribute::NORMAL, GetNormalsName(), isPrimvar);
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForHoleFaces() {
    return Create(draco::GeometryAttribute::GENERIC, GetHoleFacesName(), draco::DT_UINT8, false, 1, VECTOR, false,
                  GetDefaultTime(), GetDefaultTime(), TfToken());
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForAddedEdges() {
    return Create(draco::GeometryAttribute::GENERIC, GetAddedEdgesName(), draco::DT_UINT8, false, 1, VECTOR, false,
                  GetDefaultTime(), GetDefaultTime(), TfToken());
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::ForPosOrder() {
    return Create(draco::GeometryAttribute::GENERIC, GetPointOrderName(), draco::DT_UINT32, false, 1, VECTOR, false,
                  GetDefaultTime(), GetDefaultTime(), TfToken());
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::FromDracoMesh(const draco::Mesh& mesh,
                                                                       draco::GeometryAttribute::Type attributeType,
                                                                       TfToken name,
                                                                       bool isPrimvar) {
    // All attributes in meshes exported from USD have metadata with name.
    const int attributeId = mesh.GetAttributeIdByMetadataEntry(METADATA_NAME_KEY, name);
    if (attributeId == -1) return Absent();
    const draco::PointAttribute& attribute = *mesh.attribute(attributeId);

    // Get attribute metadata from Draco mesh.
    const draco::AttributeMetadata* metadata = mesh.GetAttributeMetadataByAttributeId(attributeId);

    // Create descriptor from Draco attribute and metadata.
    return FromDracoAttribute(attribute, *metadata, isPrimvar);
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::FromDracoAttribute(const draco::PointAttribute& attribute,
                                                                            const draco::AttributeMetadata& metadata,
                                                                            bool isPrimvar) {
    // Metadata must have a name.
    std::string name;
    if (!metadata.GetEntryString(METADATA_NAME_KEY, &name)) return Invalid();
    ...
    // Create a descriptor.
    return UsdDracoAttributeDescriptor::Create(attribute.attribute_type(), TfToken(name.c_str()), attribute.data_type(),
                                               isPrimvar, attribute.num_components(), shape, isHalf, valuesTime,
                                               indicesTime, interpolation);
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::FromUsdMesh(const UsdGeomMesh& mesh,
                                                                     draco::GeometryAttribute::Type attributeType,
                                                                     TfToken name) {
    ...
    if (!primvar) return Absent();

    // Create descriptor from primvar.
    return FromUsdPrimvar(primvar, attributeType);
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::FromUsdPrimvar(const UsdGeomPrimvar& primvar,
                                                                        draco::GeometryAttribute::Type attributeType) {
    // Skip primvars with unsupported interpolation.
    const std::set<TfToken>& supported = GetSupportedInterpolations();
    if (supported.find(primvar.GetInterpolation()) == supported.end()) return Invalid();

    // Primvar indices at a single time sample can be exported.
    UsdTimeCode indicesTime = GetDefaultTime();
    double indicesTimeDouble;
    if (!GetTimeFrom(primvar, &indicesTimeDouble)) return Invalid();
    indicesTime = UsdTimeCode(indicesTimeDouble);

    // Create descriptor from the underlying attribute and with a given name.
    const bool isPrimvar = true;
    return FromUsdAttribute(primvar.GetAttr(), attributeType, primvar.GetName(), isPrimvar, indicesTime,
                            primvar.GetInterpolation());
}

UsdDracoAttributeDescriptor UsdDracoAttributeDescriptor::FromUsdAttribute(const UsdAttribute& attribute,
                                                                          draco::GeometryAttribute::Type attributeType,
                                                                          const TfToken& name,
                                                                          bool isPrimvar,
    ...
    if (numDimensions > 2) return Invalid();
    ...
    if (numDimensions > 0) {
        ...
        if (numDimensions > 1) dimensionTwoSize = attribute.GetTypeName().GetDimensions().d[1];
    }
    if (dimensionOneSize < 1 || dimensionOneSize > 4 || dimensionTwoSize < 1 || dimensionTwoSize > 4) return Invalid();
    const size_t numComponents = dimensionOneSize * dimensionTwoSize;

    // Skip attributes with non-array data types.
    if (!attribute.GetTypeName().IsArray()) return Invalid();

    // Get Draco data type from attribute type.
    const std::type_info& typeInfo = attribute.GetTypeName().GetScalarType().GetType().GetTypeid();
    const draco::DataType dataType = UsdDracoAttributeFactory::GetDracoDataType(typeInfo);

    // Skip attributes with unsupported data types.
    if (dataType == draco::DT_INVALID) return Invalid();

    // Get data shape, whcih can be vector, quaternion, or matrix.
    const UsdDracoAttributeDescriptor::Shape shape = UsdDracoAttributeFactory::GetShape(typeInfo);
    ...
    UsdTimeCode valuesTime = GetDefaultTime();
    double valuesTimeDouble;
    if (!GetTimeFrom(attribute, &valuesTimeDouble)) return Invalid();
    valuesTime = UsdTimeCode(valuesTimeDouble);

    // Create a descriptor.
    return Create(attributeType, name, dataType, isPrimvar, numComponents, shape, isHalf, valuesTime, indicesTime,
                  interpolation);
}

std::unique_ptr<draco::AttributeMetadata> UsdDracoAttributeDescriptor::ToMetadata() const {
    // Create metadata.
    ...
    if (_shape != GetDefaultShape()) metadata->AddEntryString(METADATA_SHAPE_KEY.c_str(), GetShapeText());
    if (_isHalf != GetDefaultHalf()) metadata->AddEntryInt(METADATA_HALF_KEY.c_str(), static_cast<int>(_isHalf));
    if (_valuesTime != GetDefaultTime())
        metadata->AddEntryDouble(METADATA_VALUES_TIME_KEY.c_str(), _valuesTime.GetValue());
    if (_indicesTime != GetDefaultTime())
        metadata->AddEntryDouble(METADATA_INDICES_TIME_KEY.c_str(), _indicesTime.GetValue());
