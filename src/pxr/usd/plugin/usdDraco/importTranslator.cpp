...
void UsdDracoImportTranslator::_FindOriginalFaceEdges(draco::FaceIndex faceIndex,
    ...
    if (triangleVisited[faceIndex.value()]) return;
...
SdfLayerRefPtr UsdDracoImportTranslator::_Translate() {
    ...
    if (!_CheckDescriptors()) return layer;

    // Get data from Draco mesh.
    _PopulateValuesFromMesh();

    // Check validity of attributes.
    if (!_CheckData()) return layer;
...
void UsdDracoImportTranslator::_PopulateValuesFromMesh() {
    ...
    for (size_t i = 0; i < metadatas.size(); i++) {
        ...
        if (importAttribute->GetDescriptor().IsGeneric()) _genericAttributes.push_back(std::move(importAttribute));
    }
...
std::unique_ptr<UsdDracoImportAttributeInterface> UsdDracoImportTranslator::CreateAttributeFrom(
    ...
    const UsdDracoAttributeDescriptor descriptor =
            UsdDracoAttributeDescriptor::FromDracoAttribute(attribute, metadata, isPrimvar);
...
void UsdDracoImportTranslator::_PopulateIndicesFromMesh() {
    ...
    // Create corner table.
    std::unique_ptr<draco::CornerTable> cornerTable = draco::CreateCornerTableFromPositionAttribute(&_dracoMesh);
    ...
    for (size_t i = 0; i < numFaces; i++) {
        ...
        if (_addedEdges.HasPointAttribute()) {
            ...
            polygonEdges.clear();
            _FindOriginalFaceEdges(fi, cornerTable.get(), triangleVisited, polygonEdges);
        ...
        if (_holeFaces.GetMappedValue(face[0]) != 0) _holeIndices.push_back(faceIndex);
        faceIndex++;
    }
...
void UsdDracoImportTranslator::_SetAttributesToMesh(UsdGeomMesh* usdMesh) const {
    ...
    if (_holeFaces.HasPointAttribute()) usdMesh->GetHoleIndicesAttr().Set(_holeIndices);
    for (size_t i = 0; i < _genericAttributes.size(); i++) {
...
VtVec3fArray UsdDracoImportTranslator::_ComputeExtent() const {
    GfRange3f range;
    for (const GfVec3f& position : _positions.GetValues()) range.UnionWith(position);
...
bool UsdDracoImportTranslator::_SubdivisionRefersToPositions(const UsdGeomMesh& usdMesh) const {
    if (usdMesh.GetCreaseSharpnessesAttr().HasAuthoredValueOpinion()) return true;
    if (usdMesh.GetCornerSharpnessesAttr().HasAuthoredValueOpinion()) return true;
    return false;
}

bool UsdDracoImportTranslator::_SubdivisionRefersToFaces(const UsdGeomMesh& usdMesh) const {
    if (usdMesh.GetHoleIndicesAttr().HasAuthoredValueOpinion()) return true;
    return false;
}
