...
UsdSemanticsLabelsQuery::UsdSemanticsLabelsQuery(const TfToken& taxonomy, UsdTimeCode timeCode)
...
UsdSemanticsLabelsQuery::UsdSemanticsLabelsQuery(const TfToken& taxonomy, const GfInterval& interval)
...
bool UsdSemanticsLabelsQuery::_PopulateLabels(const UsdPrim& prim) const {
    ...
    auto labels = std::visit(
            [&schema](auto&& queryTime) -> _UnorderedTokenSet {
                ...
                if constexpr (std::is_same_v<TimeType, GfInterval>) {
                    ...
                    if (!labelsAttr.GetTimeSamplesInInterval(queryTime, &times)) {
                        TF_WARN("Failed to retrieve time samples at %s", UsdDescribe(labelsAttr).c_str());
                        return {};
                    }
                    if (!TF_VERIFY(std::all_of(std::cbegin(times), std::cend(times), [](const auto t) {
                    ...
                    const double earliest =
                            queryTime.IsMinFinite() ? queryTime.GetMin() : UsdTimeCode::EarliestTime().GetValue();
                    ...
                    for (const auto timeInInterval : times) {
                        ...
                        if (!labelsAttr.Get(&labelsAtTime, timeInInterval)) {
                            TF_WARN("Failed to read value at %s", UsdDescribe(labelsAttr).c_str());
                            return {};
                        }
                        uniqueLabels.insert(std::cbegin(labelsAtTime), std::cend(labelsAtTime));
                    }
                    return uniqueLabels;
                }
            },
    ...
    // results.
    const auto [iter, emplaced] = _cachedLabels.emplace(prim.GetPath(), _UnorderedTokenSet{});
    if (emplaced) {
...
bool UsdSemanticsLabelsQuery::_PopulateInheritedLabels(const UsdPrim& prim) const {
...
VtTokenArray UsdSemanticsLabelsQuery::ComputeUniqueDirectLabels(const UsdPrim& prim) const {
...
VtTokenArray UsdSemanticsLabelsQuery::ComputeUniqueInheritedLabels(const UsdPrim& prim) const {
...
bool UsdSemanticsLabelsQuery::HasDirectLabel(const UsdPrim& prim, const TfToken& label) const {
    // If the prim was not labeled, we can early exit without
    // locking.
    if (!_PopulateLabels(prim)) {
        return false;
    ...
    return (it != std::cend(_cachedLabels)) && (it->second.count(label));
}

bool UsdSemanticsLabelsQuery::HasInheritedLabel(const UsdPrim& prim, const TfToken& label) const {
    ...
    return std::any_of(std::cbegin(range), std::cend(range), [this, &label](const auto& path) {
        const auto it = _cachedLabels.find(path);
        return ((it != std::cend(_cachedLabels)) && (it->second.count(label)));
    });
}
