...
GfVec3f UsdPhysicsDiagonalize(const GfMatrix3f& m, GfQuatf& massFrame) {
    ...
    for (uint32_t i = 0; i < MAX_ITERS; i++) {
        ...
        if (d[a1][a2] == 0.0f || fabs(d[a1][a1] - d[a2][a2]) > 2e6 * fabs(2.0 * d[a1][a2])) break;
        ...
        GfQuatf r;
        if (absw > 1000)
            r = UsdPhysicsIndexedRotation(a, 1 / (4 * w),
                                          1.0f);  // h will be very close to 1, so use small angle approx instead
        else {
...
public:
    ...
    USDPHYSICS_API static UsdPhysicsMassProperties Sum(const UsdPhysicsMassProperties* props,
        ...
        if (combinedMass > 0.f) combinedCoM /= combinedMass;
    ...
    const GfMatrix3f& GetInertiaTensor() const { return _inertiaTensor; }

    /// Set inertia tensor
    /// \p inTensor New inertia tensor.
    void SetInertiaTensor(const GfMatrix3f& inTensor) { _inertiaTensor = inTensor; }

    /// Get center of mass
    /// \return Center of mass
    const GfVec3f& GetCenterOfMass() const { return _centerOfMass; }
    ...
    /// Set mass
    /// \p inMass New mass.
    void SetMass(float inMass) { _mass = inMass; }
