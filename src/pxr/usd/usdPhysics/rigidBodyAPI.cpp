...
UsdAttribute UsdPhysicsRigidBodyAPI::CreateRigidBodyEnabledAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdPhysicsTokens->physicsRigidBodyEnabled, SdfValueTypeNames->Bool,
...
UsdAttribute UsdPhysicsRigidBodyAPI::CreateKinematicEnabledAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdPhysicsTokens->physicsKinematicEnabled, SdfValueTypeNames->Bool,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}
...
UsdAttribute UsdPhysicsRigidBodyAPI::CreateStartsAsleepAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdPhysicsTokens->physicsStartsAsleep, SdfValueTypeNames->Bool,
...
UsdAttribute UsdPhysicsRigidBodyAPI::CreateVelocityAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdPhysicsTokens->physicsVelocity, SdfValueTypeNames->Vector3f,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}
...
UsdAttribute UsdPhysicsRigidBodyAPI::CreateAngularVelocityAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdPhysicsTokens->physicsAngularVelocity, SdfValueTypeNames->Vector3f,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}
...
const TfTokenVector& UsdPhysicsRigidBodyAPI::GetSchemaAttributeNames(bool includeInherited) {
    static TfTokenVector localNames = {
            UsdPhysicsTokens->physicsRigidBodyEnabled, UsdPhysicsTokens->physicsKinematicEnabled,
            UsdPhysicsTokens->physicsStartsAsleep,     UsdPhysicsTokens->physicsVelocity,
...
float UsdPhysicsRigidBodyAPI::ComputeMassProperties(GfVec3f* _diagonalInertia,
    ...
    if (accumulateMass || !rigidBodyMassInfo.hasInertia || !hasCoM) {
        ...
        std::vector<UsdShadeMaterial> physicsMaterials = UsdShadeMaterialBindingAPI::ComputeBoundMaterials(
        ...
        for (size_t i = 0; i < collisionPrims.size(); i++) {
            ...
            _MassApiData _MassApiData = _GetCollisionShapeMassAPIData(collisionAPI, rigidBodyMassInfo.density,
                                                                      &shapeDensity, physicsMaterials[i]);

            GfMatrix4f matrix;
            massProps.push_back(_ParseCollisionShapeForMass(collisionPrim, _MassApiData, shapeDensity, &matrix,
                                                            &xfCache, massInfoFn));
        ...
        if (!massProps.empty()) {
            ...
            GfQuatf accPa;
            const GfVec3f accInertia =
                    UsdPhysicsMassProperties::GetMassSpaceInertia(accumulatedMassProps.GetInertiaTensor(), accPa);
            ...
            if (!rigidBodyMassInfo.hasInertia) {
                ...
                if (outMass > 0.0f) {
                    ...
                    TF_DIAGNOSTIC_WARNING(
                            "The rigid body at %s has a possibly invalid inertia tensor of {1.0, 1.0, 1.0}, small "
                            "sphere approximated inertia was used. %s %s",
                            usdPrim.GetPrimPath().GetString().c_str(),
                            "Either specify correct values in the mass properties, or add collider(s) to any shape(s) "
                            "that you wish to automatically compute mass properties for.",
                            "If you do not want the objects to collide, add colliders regardless then disable the "
                            "'enable collision' property.");
                } else {
                    TF_DIAGNOSTIC_WARNING(
