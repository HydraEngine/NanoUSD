...
UsdAttribute UsdPhysicsCollisionGroup::GetMergeGroupNameAttr() const {
    return GetPrim().GetAttribute(UsdPhysicsTokens->physicsMergeGroup);
}

UsdAttribute UsdPhysicsCollisionGroup::CreateMergeGroupNameAttr(VtValue const& defaultValue, bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdPhysicsTokens->physicsMergeGroup, SdfValueTypeNames->String,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}

UsdAttribute UsdPhysicsCollisionGroup::GetInvertFilteredGroupsAttr() const {
    return GetPrim().GetAttribute(UsdPhysicsTokens->physicsInvertFilteredGroups);
}

UsdAttribute UsdPhysicsCollisionGroup::CreateInvertFilteredGroupsAttr(VtValue const& defaultValue,
                                                                      bool writeSparsely) const {
    return UsdSchemaBase::_CreateAttr(UsdPhysicsTokens->physicsInvertFilteredGroups, SdfValueTypeNames->Bool,
                                      /* custom = */ false, SdfVariabilityVarying, defaultValue, writeSparsely);
}
...
static unsigned int _GetCollisionTableIndex(const unsigned int idxA,
...
const SdfPathVector& UsdPhysicsCollisionGroup::CollisionGroupTable::GetCollisionGroups() const {
    return _groups;
}

bool UsdPhysicsCollisionGroup::CollisionGroupTable::IsCollisionEnabled(const unsigned int idxA,
...
bool UsdPhysicsCollisionGroup::CollisionGroupTable::IsCollisionEnabled(const SdfPath& primA,
                                                                       const SdfPath& primB) const {
    auto a = std::find(_groups.begin(), _groups.end(), primA);
    auto b = std::find(_groups.begin(), _groups.end(), primB);
    return IsCollisionEnabled(std::distance(_groups.begin(), a), std::distance(_groups.begin(), b));
}

UsdPhysicsCollisionGroup::CollisionGroupTable UsdPhysicsCollisionGroup::ComputeCollisionGroupTable(
    ...
    for (const UsdPhysicsCollisionGroup& groupA : allSceneGroups) {
        ...
        if (!invertedAttr.IsAuthored() || !invertedFilter) {
            ...
            std::set<unsigned int> requestedGroups(filteredGroupIndices.begin(), filteredGroupIndices.end());
    ...
    res._enabled.resize(((res._groups.size() + 1) * res._groups.size()) / 2, true);
    ...
    for (unsigned int iA = 0; iA < allSceneGroups.size(); iA++) {
        for (unsigned int iB = iA; iB < allSceneGroups.size(); iB++) {
            ...
            const unsigned int groupAId = primPathToIndex[allSceneGroups[iA].GetPath()];
            const unsigned int groupBId = primPathToIndex[allSceneGroups[iB].GetPath()];
            const unsigned int mergedTableIndex = _GetCollisionTableIndex(groupAId, groupBId, nextPrimId);
            ...
            const unsigned int tableIndex = _GetCollisionTableIndex(minGroup, maxGroup, allSceneGroups.size());
