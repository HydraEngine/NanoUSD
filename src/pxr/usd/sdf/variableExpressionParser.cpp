...
class StringNodeCreator : public NodeCreator {
public:
...
class VariableNodeCreator : public NodeCreator {
public:
    std::unique_ptr<Impl::Node> CreateNode(std::string* errMsg) override {
...
template <class Type>
class ConstantNodeCreator : public NodeCreator {
public:
    std::unique_ptr<Impl::Node> CreateNode(std::string* errMsg) override {
...
class NoneNodeCreator : public NodeCreator {
public:
    std::unique_ptr<Impl::Node> CreateNode(std::string* errMsg) override { return std::make_unique<Impl::NoneNode>(); }
};

class ListNodeCreator : public NodeCreator {
public:
...
using AvailableFunctions = std::tuple<Impl::If2Node,
                                      Impl::If3Node,
...
class FunctionNodeCreator : public NodeCreator {
public:
    FunctionNodeCreator(const std::string& functionName_) : functionName(functionName_) {};

    std::unique_ptr<Impl::Node> CreateNode(std::string* errMsg) final {
        bool matchedFunctionName = false;
        return _CreateNode(errMsg, &matchedFunctionName, (AvailableFunctions*)(nullptr));
    }
...
private:
    ...
    std::unique_ptr<Impl::Node> _CreateNode(std::string* errMsg,
                                            bool* matchedFunctionName,
                                            std::tuple<NodeType, Others...>*) {
        if (functionName == NodeType::GetFunctionName()) {
            *matchedFunctionName = true;

            std::unique_ptr<Impl::Node> node = _CreateNodeHelper<NodeType>(errMsg);
        ...
        return _CreateNode(errMsg, matchedFunctionName, (std::tuple<Others...>*)(nullptr));
    }

    std::unique_ptr<Impl::Node> _CreateNode(std::string* errMsg, bool* matchedFunctionName, std::tuple<>*) {
        if (*matchedFunctionName) {
            *errMsg = TfStringPrintf("Function '%s' does not take %zu arguments.", functionName.c_str(),
                                     functionArgs.size());
        } else {
            *errMsg = TfStringPrintf("Unknown function %s", functionName.c_str());
        }
        return nullptr;
    }
    ...
    std::enable_if_t<NodeType::IsVariadic, std::unique_ptr<Impl::Node>> _CreateNodeHelper(std::string* errMsg) {
        if (functionArgs.size() < NodeType::MinNumArgs) {
            *errMsg = TfStringPrintf("Function '%s' requires at least %zu arguments.", functionName.c_str(),
                                     NodeType::MinNumArgs);
            return nullptr;
        }
    ...
    std::enable_if_t<!NodeType::IsVariadic, std::unique_ptr<Impl::Node>> _CreateNodeHelper(std::string* errMsg) {
        ...
        return _CreateFunctionNode<NodeType>(errMsg, std::make_index_sequence<NodeType::NumArgs>());
    }

    template <class NodeType, size_t... I>
    std::unique_ptr<Impl::Node> _CreateFunctionNode(std::string* errMsg, std::index_sequence<I...>) {
...
public:
    ...
    CreatorType* GetExistingNodeCreator() {
        if (!_nodeStack.empty()) {
            if (CreatorType* existingCreator = dynamic_cast<CreatorType*>(_nodeStack.back().get())) {
...
template <class C>
struct VariableName : identifier {};

template <class C>
struct VariableImpl : if_must<VariableStart, VariableName<C>, VariableEnd> {
    using Name = VariableName<C>;
};

// Variable reference at the top level of an expression.
struct Variable : VariableImpl<Variable> {};

// ----------------------------------------

// Variable reference in a quoted string.
struct QuotedStringVariable : VariableImpl<QuotedStringVariable> {};

// Characters that must be escaped in a quoted string.
template <char QuoteChar>
struct QuotedStringEscapedChar : one<'`', '$', '\\', QuoteChar> {};
...
template <char QuoteChar>
struct QuotedStringStart : string<QuoteChar> {};

template <char QuoteChar>
struct QuotedStringEnd : string<QuoteChar> {};

template <char QuoteChar>
struct QuotedStringBody : star<sor<QuotedStringVariable, QuotedStringChars<QuoteChar>>> {};

template <char QuoteChar>
struct QuotedString : if_must<QuotedStringStart<QuoteChar>, QuotedStringBody<QuoteChar>, QuotedStringEnd<QuoteChar>> {
...
template <class Base>
struct FunctionArgumentWrapper : public Base {};
...
struct Function : if_must<seq<FunctionName, FunctionArgumentStart>, FunctionArguments, FunctionArgumentEnd> {};

// ----------------------------------------

struct ScalarExpression : sor<Variable, DoubleQuotedString, SingleQuotedString, Integer, Boolean, None, Function> {};

// ----------------------------------------

struct ListStart : one<'['> {};
...
struct Action<QuotedStringChars<QuoteChar>> {
    ...
    static void apply(const ActionInput& in, ParserContext& context) {
        context.GetNodeCreator<StringNodeCreator>()->parts.push_back({in.string(), /* isVar = */ false});
    }
};
...
struct Action<QuotedStringVariable::Name> {
    template <typename ActionInput>
    static void apply(const ActionInput& in, ParserContext& context) {
        context.GetNodeCreator<StringNodeCreator>()->parts.push_back({in.string(), /* isVar = */ true});
    }
};
...
struct Action<Integer> {
    ...
    static void apply(const ActionInput& in, ParserContext& context) {
        ...
        if (outOfRange) {
            _ThrowParseError(in, TfStringPrintf("Integer %s out of range.", in.string().c_str()));
        }
...
struct Action<ListElement> {
    ...
    static void apply(const ActionInput& in, ParserContext& context) {
        std::string errMsg;
        std::unique_ptr<Impl::Node> elemNode = context.CreateExpressionNode(&errMsg);
        if (!elemNode) {
            _ThrowParseError(in, errMsg);
        }

        ListNodeCreator* listCreator = context.GetExistingNodeCreator<ListNodeCreator>();
        if (!listCreator) {
...
struct Action<FunctionArgument> {
    ...
    static void apply(const ActionInput& in, ParserContext& context) {
        std::string errMsg;
        std::unique_ptr<Impl::Node> argNode = context.CreateExpressionNode(&errMsg);
        if (!argNode) {
            _ThrowParseError(in, errMsg);
        }

        FunctionNodeCreator* fnCreator = context.GetExistingNodeCreator<FunctionNodeCreator>();
        if (!fnCreator) {
            _ThrowParseError(in, "Unknown error");
        }
...
#define MATCH_ERROR(rule, msg) \
    template <>                \
    const std::string Errors<rule>::errorMsg = msg;
...
MATCH_ERROR(Variable::Name, "Variables must be a C identifier");
MATCH_ERROR(QuotedStringVariable::Name, "Variables must be a C identifier");
...
Sdf_VariableExpressionParserResult Sdf_ParseVariableExpression(const std::string& expr) {
    ...
    try {
        const bool parseSuccess = TfDebug::IsEnabled(SDF_VARIABLE_EXPRESSION_PARSING)
        ...
        return {nullptr,
                ...
                {TfStringPrintf("%s at character %zu", std::string(e.message()).c_str(),
                                e.positions().empty() ? expr.size() : e.positions().front().column - 1)}};
    }

    std::string errMsg;
    std::unique_ptr<Impl::Node> exprNode = context.CreateExpressionNode(&errMsg);
