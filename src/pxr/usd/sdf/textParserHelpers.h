...
void _RaiseErrorPEGTL(const Sdf_TextParserContext& context,
                      const Input& in,
                      const Position& pos,
    ...
    std::string inputAtError = std::string(in.at(pos), in.end_of_line(pos));
    std::string s = TfStringPrintf("%s at '%s' in <%s>\n", msg.c_str(), inputAtError.c_str(),
...
bool _IsGenericMetadataListOpType(const TfType& type, TfType* itemArrayType = nullptr);
...
template <class T>
bool _GeneralHasDuplicates(const std::vector<T>& v) {
...
template <class T>
inline bool _HasDuplicates(const std::vector<T>& v) {
    ...
    if (std::adjacent_find(v.begin(), v.end(), [](T const& l, T const& r) {
            return l >= r;
        }) == v.end()) {
        return false;
    }
...
template <class T>
bool _SetListOpItemsWithError(const TfToken& key,
                              SdfListOpType type,
                              const T& itemList,
                              Sdf_TextParserContext& context,
                              std::string& errorMessage) {
    ...
    if (_HasDuplicates(items)) {
        errorMessage =
                "Duplicate items exist for field '" + key.GetString() + "' at '" + context.path.GetAsString() + "'";
...
bool _SetItemsIfListOpWithError(const TfType& type, Sdf_TextParserContext& context, std::string& errorMessage) {
    ...
    return _SetListOpItemsWithError(context.genericMetadataKey, context.listOpType, vtArray, context, errorMessage);
}

template <class ListOpType>
std::pair<TfType, TfType> _GetListOpAndArrayTfTypes() {
    return {TfType::Find<ListOpType>(), TfType::Find<VtArray<typename ListOpType::value_type>>()};
}

template <class Input>
void _ReportParseError(Sdf_TextParserContext& context, const Input& in, const std::string& text) {
