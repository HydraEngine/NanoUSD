...
public:
    ...
    /// \c Type.
    static const Type& CanonicalizeType(const SdfSpecHandle&, const Type& x) { return x; }

    /// Canonicalize a key.  \c key_type must be convertible to the type of
    /// \p x.  The return value must be convertible to a \c key_type.
    static const key_type& CanonicalizeKey(const SdfSpecHandle&, const key_type& x) { return x; }
    ...
    static const mapped_type& CanonicalizeValue(const SdfSpecHandle&, const mapped_type& x) { return x; }

    /// Canonicalize a key/value pair.  \c value_type must be convertible
    /// to the type of \p x.  The return value must be convertible to a
    /// \c value_type.
    static const value_type& CanonicalizePair(const SdfSpecHandle&, const value_type& x) { return x; }
};
...
private:
    ...
    class _Iterator {
        class _PtrProxy {
        public:
            std::add_pointer_t<R> operator->() { return std::addressof(_result); }
    ...
    public:
        ...
        using value_type = R;
        using reference = R;
        using pointer = std::conditional_t<std::is_lvalue_reference<R>::value, std::add_pointer_t<R>, _PtrProxy>;
        ...
        _Iterator(const _Iterator<Owner2, I2, R2>& other) : _owner(other._owner), _data(other._data), _pos(other._pos) {
            // Do nothing
        }
...
public:
    ...
    iterator begin() { return _Validate() ? iterator(this, _Data(), _Data()->begin()) : iterator(); }
    iterator end() { return _Validate() ? iterator(this, _Data(), _Data()->end()) : iterator(); }
    ...
    size_type max_size() const { return _Validate() ? _ConstData()->max_size() : 0; }

    bool empty() const { return _Validate() ? _ConstData()->empty() : true; }

    std::pair<iterator, bool> insert(const value_type& value) { return _Insert(value); }

    iterator insert(iterator pos, const value_type& value) { return _Insert(value).first; }
    ...
    void insert(InputIterator first, InputIterator last) {
        if (_Validate()) {
            ...
            for (; first != last; ++first) {
                const value_type& v = ValuePolicy::CanonicalizePair(_Owner(), *first);
    ...
    iterator find(const key_type& key) {
        return _Validate() ? iterator(this, _Data(), _Data()->find(ValuePolicy::CanonicalizeKey(_Owner(), key)))
                           : iterator();
    }

    const_iterator find(const key_type& key) const {
        return _Validate() ? const_iterator(this, _ConstData(),
    ...
    iterator lower_bound(const key_type& key) {
        return _Validate() ? iterator(this, _Data(), _Data()->lower_bound(ValuePolicy::CanonicalizeKey(_Owner(), key)))
                           : iterator();
    }
    ...
    iterator upper_bound(const key_type& key) {
        return _Validate() ? iterator(this, _Data(), _Data()->upper_bound(ValuePolicy::CanonicalizeKey(_Owner(), key)))
                           : iterator();
    }
    ...
    std::pair<const_iterator, const_iterator> equal_range(const key_type& key) const {
        if (_Validate()) {
            std::pair<const_inner_iterator, const_inner_iterator> result =
                    _ConstData()->equal_range(ValuePolicy::CanonicalizeKey(_Owner(), key));
    ...
    bool operator==(const Type& other) const { return _Validate() ? _CompareEqual(other) : false; }

    bool operator!=(const Type& other) const { return !(*this == other); }

    friend bool operator==(const Type& lhs, const SdfMapEditProxy& rhs) { return rhs == lhs; }

    friend bool operator!=(const Type& lhs, const SdfMapEditProxy& rhs) { return rhs != lhs; }

    /// Invalid SdfMapEditProxy objects will compare less to an object
    /// of their map type
    bool operator<(const Type& other) const { return !_Validate() || _Compare(other) < 0; }

    bool operator>(const Type& other) const { return _Validate() ? _Compare(other) > 0 : false; }
...
private:
    ...
    SdfSpecHandle _Owner() const { return _editor ? _editor->GetOwner() : SdfSpecHandle(); }

    std::string _Location() const { return _editor ? _editor->GetLocation() : std::string(); }
            ...
            TF_FOR_ALL(it, other) {
                ...
                if (!canonicalOther.insert(canonicalValue).second) {
                    TF_CODING_ERROR(
                            ...
                            "exists in map.",
                            _Location().c_str(), TfStringify(canonicalValue.first).c_str());
                    return;
                }
            }
