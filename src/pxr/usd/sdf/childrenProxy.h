...
private:
    ...
    public:
        ...
        operator mapped_type() const { return *_pos; }
        ...
        bool operator==(const mapped_type& other) const { return *_pos == other; }
    ...
    public:
        ...
        static _PairProxy Dereference(This* owner, _inner_iterator i) { return _PairProxy(owner, i); }
    };
    ...
    private:
        _Value dereference() const { return _Traits::Dereference(_owner, _pos); }
...
public:
    ...
    operator mapped_vector_type() const { return _Validate() ? _view.values() : mapped_vector_type(); }

    map_type items() const { return _Validate() ? _view.template items_as<map_type>() : map_type(); }

    iterator begin() { return iterator(_GetThis(), _view.begin()); }
    iterator end() { return iterator(_GetThis(), _view.end()); }
    const_iterator begin() const { return const_iterator(_GetThis(), _view.begin()); }
    const_iterator end() const { return const_iterator(_GetThis(), _view.end()); }
    ...
    size_type size() const { return _Validate() ? _view.size() : 0; }

    size_type max_size() const { return _view.max_size(); }

    bool empty() const { return _Validate() ? _view.empty() : true; }
    ...
    iterator insert(iterator pos, const mapped_type& value) { return insert(value).first; }
    ...
    size_type erase(const key_type& key) { return _Erase(key) ? 1 : 0; }
    ...
    void clear() { _Copy(mapped_vector_type()); }

    iterator find(const key_type& key) { return _Validate() ? iterator(this, _view.find(key)) : iterator(); }

    const_iterator find(const key_type& key) const {
        return _Validate() ? const_iterator(this, _view.find(key)) : const_iterator();
    }

    size_type count(const key_type& key) const { return _Validate() ? _view.count(key) : 0; }

    bool operator==(const This& other) const { return _view == other._view; }
    ...
    explicit operator bool() const { return _view.IsValid(); }

private:
    const std::string& _GetType() const { return _type; }

    int _GetPermission() const { return _permission; }
    ...
    bool _Copy(const mapped_vector_type& values) { return _Validate(CanSet) ? _PrimCopy(values) : false; }

    bool _Insert(const mapped_type& value, size_t index) {
        return _Validate(CanInsert) ? _PrimInsert(value, index) : false;
    }

    bool _Erase(const key_type& key) { return _Validate(CanErase) ? _PrimErase(key) : false; }

    bool _PrimCopy(const mapped_vector_type& values) {
        typedef std::vector<typename ChildPolicy::ValueType> ChildrenValueVector;

        ChildrenValueVector v;
        for (size_t i = 0; i < values.size(); ++i) v.push_back(Adapter::Convert(values[i]));

        return _view.GetChildren().Copy(v, _type);
    }

    bool _PrimInsert(const mapped_type& value, size_t index) {
        return _view.GetChildren().Insert(Adapter::Convert(value), index, _type);
    }

    bool _PrimErase(const key_type& key) { return _view.GetChildren().Erase(key, _type); }
