...
static char const* Stringify(SdfPredicateFunctionResult r) {
    if (r) {
        return r.IsConstant() ? "constant true" : "varying true";
    } else {
        return r.IsConstant() ? "constant false" : "varying false";
    }
}

TF_CONDITIONALLY_COMPILE_TIME_ENABLED_DEBUG_CODES(false,  // Set to 'true' to compile-in debug output support.
                                                  SDF_PATH_EXPRESSION_EVAL);

#define DEBUG_MSG(...) TF_DEBUG_MSG(SDF_PATH_EXPRESSION_EVAL, __VA_ARGS__)

void Sdf_PathExpressionEvalBase::_PatternIncrSearchState::Pop(int newDepth) {
...
void Sdf_PathExpressionEvalBase::_PatternImplBase::_Init(
    ...
    auto closeAndAppendSegment = [this]() {
        _segments.push_back({_segments.empty() ? 0 : _segments.back().end, _components.size()});
    };
    ...
    for (auto iter = std::cbegin(pattern.GetComponents()), end = std::cend(pattern.GetComponents()); iter != end;
         ++iter) {
        ...
        // A glob pattern (we translate to regex).
            ...
            _components.push_back({Regex, static_cast<int>(_regexes.size() - 1), -1});
        }
    ...
    } else if (_stretchEnd || (!_components.empty() && _components.back().type == ExplicitName &&
    ...
    if (TfDebug::IsEnabled(SDF_PATH_EXPRESSION_EVAL)) {
        auto stringifyComponent = [this](_Component const& component) {
            std::string result = (component.type == ExplicitName)
                                         ? TfStringPrintf("'%s'", _explicitNames[component.patternIndex].c_str())
        ...
        DEBUG_MSG(
                ...
                "  stretchEnd   : %d\n"
                "  segments     : %s\n",
                pattern.GetText().c_str(), _prefix.GetAsString().c_str(), _stretchBegin, _stretchEnd,
                TfStringJoin(segmentStrs, ", ").c_str());
    }
}

SdfPredicateFunctionResult Sdf_PathExpressionEvalBase::_PatternImplBase::_Match(
        SdfPath const& path, TfFunctionRef<SdfPredicateFunctionResult(int, SdfPath const&)> runNthPredicate) const {
    ...
    if (!path.HasPrefix(_prefix)) {
        ...
        Result result = _prefix.HasPrefix(path) ? Result::MakeVarying(false) : Result::MakeConstant(false);
        DEBUG_MSG("<%s> lacks prefix <%s> -> %s\n", path.GetAsString().c_str(), _prefix.GetAsString().c_str(),
                  Stringify(result));
        return result;
    }
    ...
    if (_matchObjType == _MatchPropOnly && !isPrimPropertyPath) {
        DEBUG_MSG(
                "pattern demands a property; <%s> is a prim path -> "
                "varying false\n",
                path.GetAsString().c_str());
    ...
    if (_matchObjType == _MatchPrimOnly && isPrimPropertyPath) {
        DEBUG_MSG(
                "pattern demands a prim; <%s> is a property path -> "
                "constant false\n",
                path.GetAsString().c_str());
        return Result::MakeConstant(false);
    }
    ...
    if (_components.empty()) {
        ...
        if (_stretchBegin || _stretchEnd) {
            DEBUG_MSG(
                    "pattern accepts all descendant paths "
                    "-> constant true\n");
    ...
    // prefix, otherwise those components have nothing to match.
        ...
        DEBUG_MSG(
                "path matches prefix but pattern requires additional "
    ...
    auto checkMatch = [this, &runNthPredicate](_Segment const& seg, SdfPathVector::const_iterator pathIter) {
        for (auto iter = _components.cbegin() + seg.begin, end = _components.cbegin() + seg.end; iter != end;
             ++iter, ++pathIter) {
            switch (iter->type) {
                case ExplicitName: {
            ...
            if (iter->predicateIndex != -1) {
                Result predResult = runNthPredicate(iter->predicateIndex, *pathIter);
                if (!predResult) {
                    // The predicate's result's constancy is valid to
                    // propagate here.
                    DEBUG_MSG("Predicate fails '%s' -> %s\n", pathIter->GetAsString().c_str(), Stringify(predResult));
                    return predResult;
                }
            }
        }
    ...
    auto searchMatch = [&](_Segment const& seg, SdfPathVector::const_iterator pathBegin,
    ...
    for (_Segment const& segment : _segments) {
        ...
        // Final segment must match at the end.
            ...
            const Result result = checkMatch(segment, matchEnd - segment.GetSize());
    ...
    if (_stretchEnd) {
        DEBUG_MSG(
                "_Match(<%s>) succeeds with trailing stretch -> "
                "constant true\n",
                path.GetAsString().c_str());
        return Result::MakeConstant(true);
    }
...
SdfPredicateFunctionResult Sdf_PathExpressionEvalBase ::_PatternImplBase::_Next(
    ...
    if (_components.empty()) {
        if (_stretchBegin || _stretchEnd) {
            ...
            DEBUG_MSG(
                    "_Next(<%s>) must match prefix <%s> exactly -> "
                    "constant false\n",
                    path.GetAsString().c_str(), _prefix.GetAsString().c_str());
            search._constantDepth = prefixElemCount;
            search._constantValue = false;
    ...
    const size_t numMatchComponents =
            pathElemCount - (prevSegPtr ? search._segmentMatchDepths.back() : prefixElemCount);
    ...
    if (numMatchComponents < curSeg.GetSize()) {
        ...
        DEBUG_MSG(
                "_Next(<%s>) lacks enough matching components (%zu) for "
                "current segment (%zu) -> varying false\n",
                path.GetAsString().c_str(), numMatchComponents, curSeg.GetSize());
        return Result::MakeVarying(false);
    }
    ...
    if (!prevSegPtr && !_stretchBegin && numMatchComponents > curSeg.GetSize()) {
        ...
        search._constantValue = false;
        DEBUG_MSG(
                "_Next(<%s>) matching components (%zu) exceeds "
    ...
    const auto compEnd = make_reverse_iterator(_components.cbegin() + curSeg.begin);
    ...
    for (; compIter != compEnd; ++compIter, workingPath = workingPath.GetParentPath()) {
        if (compIter->type == _PatternImplBase::ExplicitName) {
            ...
            if (!name.empty() && name != workingPath.GetName()) {
                DEBUG_MSG(
                        "_Next(<%s>) component '%s' != '%s' -> "
                        "varying false\n",
                        path.GetAsString().c_str(), workingPath.GetName().c_str(), name.c_str());
                return Result::MakeVarying(false);
            }
            // Invoke predicate if this component has one.
            if (compIter->predicateIndex != -1) {
                SdfPredicateFunctionResult predResult = runNthPredicate(compIter->predicateIndex, workingPath);
                if (!predResult) {
                    ...
                    DEBUG_MSG(
                            "_Next(<%s>) failed predicate at <%s> -> "
                            "%s\n",
                            path.GetAsString().c_str(), workingPath.GetAsString().c_str(), Stringify(predResult));
                    return predResult;
                }
            }
        }
    ...
    for (; compIter != compEnd; ++compIter, workingPath = workingPath.GetParentPath()) {
        if (compIter->type == _PatternImplBase::Regex) {
            if (!_regexes[compIter->patternIndex].Match(workingPath.GetName())) {
                DEBUG_MSG(
                        "_Next(<%s>) component '%s' does not match wildcard "
                        "-> varying false\n",
                        path.GetAsString().c_str(), workingPath.GetName().c_str());
                return Result::MakeVarying(false);
            }
            // Invoke predicate if this component has one.
            if (compIter->predicateIndex != -1) {
    ...
    if (search._segmentMatchDepths.size() == _segments.size()) {
        if (_stretchEnd) {
            ...
            search._constantValue = true;
            DEBUG_MSG(
                    "_Next(<%s>) matches with trailing stretch -> "
                    "constant true\n",
                    path.GetAsString().c_str());
            return Result::MakeConstant(true);
        }
    ...
    DEBUG_MSG(
            "_Next(<%s>) partial yet incomplete match "
            "(%zd of %zd segments) -> varying false\n",
            path.GetAsString().c_str(), search._segmentMatchDepths.size(), _segments.size());
