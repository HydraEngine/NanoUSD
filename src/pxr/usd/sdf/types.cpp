...
constexpr size_t _Sdf_UnitMaxUnits = std::max({_SDF_FOR_EACH_UNITS(_SDF_UNIT_MAX_UNITS_OP, _SDF_UNITS)});
...
static void _AddToUnitsMaps(
        _UnitsInfo& info, const TfEnum& unit, const string& unitName, double scale, const string& category) {
...
#define _REGISTRY_FUNCTION(elem)                                                                        \
    TF_REGISTRY_FUNCTION_WITH_TAG(TfEnum, _SDF_UNITSLIST_CATEGORY(elem)) {                              \
        TF_PP_SEQ_FOR_EACH(_ADD_UNIT_ENUM, _SDF_UNITSLIST_CATEGORY(elem), _SDF_UNITSLIST_TUPLES(elem)); \
    }

_SDF_FOR_EACH_UNITS(_REGISTRY_FUNCTION, _SDF_UNITS)

#define _ADD_UNIT_TO_MAPS(category, elem)                                                             \
    _AddToUnitsMaps(*info, TF_PP_CAT(Sdf##category##Unit, _SDF_UNIT_TAG(elem)), _SDF_UNIT_NAME(elem), \
...
double SdfConvertUnit(const TfEnum& fromUnit, const TfEnum& toUnit) {
    ...
    if (!toUnit.IsA(fromUnit.GetType())) {
        TF_WARN("Can not convert from '%s' to '%s'.", TfEnum::GetFullName(fromUnit).c_str(),
...
static std::string _GetKeyPathText(std::vector<std::string> const& keyPath) {
    return keyPath.empty() ? std::string() : TfStringPrintf(" under key '%s'", TfStringJoin(keyPath, ":").c_str());
}
...
static void _AddInvalidTypeError(char const* prefix,
    ...
    errMsgs->push_back(
            TfStringPrintf("%s%s%s is not a valid scene description "
                           "datatype",
                           prefix, _GetDiagnosticStringForValue(value).c_str(), _GetKeyPathText(*keyPath).c_str()));
}

// Function pointer type for converting VtValue holding std::vector<VtValue> to
// VtArray of a specific type T.
using _ValueVectorToVtArrayFn = bool (*)(VtValue*, std::vector<std::string>*, std::vector<std::string> const*);
...
// vector<VtValue>.
template <class T>
bool _ValueVectorToVtArray(VtValue* value, std::vector<std::string>* errMsgs, std::vector<std::string> const* keyPath) {
        ...
        if (cast.IsEmpty()) {
            errMsgs->push_back(
                    TfStringPrintf("failed to cast array element "
                                   "%zu: %s%s to <%s>",
                                   std::distance(valVec.begin(), begin), _GetDiagnosticStringForValue(*begin).c_str(),
                                   _GetKeyPathText(*keyPath).c_str(), ArchGetDemangled<T>().c_str()));
            allValid = false;
        } else {
            cast.Swap(*e++);
        }
...
template <class T>
bool _PySeqToVtArray(VtValue* value, std::vector<std::string>* errMsgs, std::vector<std::string> const* keyPath) {
    using ElemType = T;
    bool allValid = true;
    TfPyLock lock;
    ...
    for (Py_ssize_t i = 0; i != len; ++i) {
        ...
        if (!h) {
            ...
            errMsgs->push_back(TfStringPrintf("failed to obtain element %s from sequence%s", TfStringify(i).c_str(),
                                              _GetKeyPathText(*keyPath).c_str()));
            allValid = false;
        }
        ...
        if (!e.check()) {
            errMsgs->push_back(
                    TfStringPrintf("failed to cast sequence element "
                                   "%s: %s%s to <%s>",
                                   TfStringify(i).c_str(),
                                   _GetDiagnosticStringForValue(pxr_boost::python::extract<VtValue>(h.get())).c_str(),
                                   _GetKeyPathText(*keyPath).c_str(), ArchGetDemangled<ElemType>().c_str()));
            allValid = false;
        } else {
            *elem++ = e();
        }
...
static bool _PyObjToAnyVtArray(VtValue* value, std::vector<std::string>* errMsgs, std::vector<std::string>* keyPath) {
    TfPyLock pyLock;
    TfPyObjWrapper obj = value->UncheckedGet<TfPyObjWrapper>();
