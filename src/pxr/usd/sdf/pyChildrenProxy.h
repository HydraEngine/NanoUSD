...
private:
    ...
    struct _ExtractKey {
        static pxr_boost::python::object Get(const _const_iterator& i) { return pxr_boost::python::object(i->first); }
    };

    struct _ExtractValue {
        static pxr_boost::python::object Get(const _const_iterator& i) { return pxr_boost::python::object(i->second); }
    };
    ...
    void _DelItemByIndex(int index) { _proxy._Erase(_GetView().key(_GetItemByIndex(index))); }

    void _Clear() { _proxy._Copy(mapped_vector_type()); }

    void _AppendItem(const mapped_type& value) { _proxy._Insert(value, _proxy.size()); }
    ...
    bool _HasKey(const key_type& key) const { return _GetView().find(key) != _GetView().end(); }

    bool _HasValue(const mapped_type& value) const { return _GetView().find(value) != _GetView().end(); }
