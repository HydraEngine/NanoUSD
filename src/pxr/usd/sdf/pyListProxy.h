...
private:
    ...
    static std::string _GetStr(const Type& x) { return TfPyRepr(static_cast<value_vector_type>(x)); }

    static value_type _GetItemIndex(const Type& x, int index) {
        return x[TfPyNormalizeIndex(index, x._GetSize(), true)];
    }

    static pxr_boost::python::list _GetItemSlice(const Type& x, const pxr_boost::python::slice& index) {
        using namespace pxr_boost::python;
    ...
    static void _SetItemSlice(Type& x, const pxr_boost::python::slice& index, const value_vector_type& values) {
        using namespace pxr_boost::python;
    ...
    static int _FindIndex(const Type& x, const value_type& value) {
        if (x._Validate()) {
            ...
            return index == Type::invalidIndex ? _GetInvalidIndex() : static_cast<int>(index);
        } else {
