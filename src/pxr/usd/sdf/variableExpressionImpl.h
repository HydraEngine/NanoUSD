...
enum class ValueType { Unknown, Boolean, Integer, String, List, None };
...
/// ValueType::Unknown.
ValueType GetValueType(const VtValue& val);

/// Returns a descriptive name for the value type held by \p val.
std::string GetValueTypeName(const VtValue& val);
...
/// empty VtValue.
VtValue CoerceIfUnsupportedValueType(const VtValue& val);
...
public:
    ...
    static EvalResult Error(std::vector<std::string>&& errors) { return {VtValue(), std::move(errors)}; }
...
public:
    ...
    std::unordered_set<std::string>& GetRequestedVariables() { return _requestedVariables; }
...
template <size_t N>
class FunctionWithNArgsNode : public Node {
public:
...
template <size_t N>
class FunctionWithAtLeastNArgsNode : public Node {
public:
...
protected:
    ...
    static EvalResult _Evaluate(EvalContext* ctx,
...
/// it evalutes to None.
class If2Node : public IfNode, public FunctionWithNArgsNode<2> {
public:
    If2Node(std::unique_ptr<Node>&& condition, std::unique_ptr<Node>&& ifValue);
...
class If3Node : public IfNode, public FunctionWithNArgsNode<3> {
public:
    If3Node(std::unique_ptr<Node>&& condition, std::unique_ptr<Node>&& ifValue, std::unique_ptr<Node>&& elseValue);
...
class ComparisonNode : public FunctionWithNArgsNode<2> {
public:
    static const char* GetFunctionName();
...
class LogicalNode : public FunctionWithAtLeastNArgsNode<2> {
public:
    static const char* GetFunctionName();
...
class LogicalNotNode : public FunctionWithNArgsNode<1> {
public:
    static const char* GetFunctionName();
...
class ContainsNode : public FunctionWithNArgsNode<2> {
public:
    static const char* GetFunctionName();

    ContainsNode(std::unique_ptr<Node>&& searchIn, std::unique_ptr<Node>&& searchFor);
...
class AtNode : public FunctionWithNArgsNode<2> {
public:
    static const char* GetFunctionName();

    AtNode(std::unique_ptr<Node>&& source, std::unique_ptr<Node>&& index);
...
class LenNode : public FunctionWithNArgsNode<1> {
public:
    static const char* GetFunctionName();
...
class DefinedNode : public FunctionWithAtLeastNArgsNode<1> {
public:
    static const char* GetFunctionName();
