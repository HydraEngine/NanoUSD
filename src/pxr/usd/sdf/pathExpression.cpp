...
SdfPathExpression::ExpressionReference const& SdfPathExpression::ExpressionReference::Weaker() {
    static ExpressionReference* theWeaker = new ExpressionReference{SdfPath(), "_"};
    return *theWeaker;
}

static bool ParsePathExpression(std::string const& inputStr,
                                std::string const& parseContext,
                                SdfPathExpression* pattern,
                                std::string* errMsg);

SdfPathExpression::SdfPathExpression(std::string const& inputStr, std::string const& parseContext) {
    std::string errMsg;
    if (!inputStr.empty() && !ParsePathExpression(inputStr, parseContext, this, &errMsg)) {
        *this = {};
        _parseError = errMsg;
...
SdfPathExpression const& SdfPathExpression::Everything() {
    static SdfPathExpression const* theEverything = new SdfPathExpression("//");
    return *theEverything;
}

SdfPathExpression const& SdfPathExpression::EveryDescendant() {
    static SdfPathExpression const* theEveryDescendant = new SdfPathExpression(".//");
    return *theEveryDescendant;
}

SdfPathExpression const& SdfPathExpression::Nothing() {
    static SdfPathExpression const* theNothing = new SdfPathExpression{};
    return *theNothing;
}

SdfPathExpression const& SdfPathExpression::WeakerRef() {
    static SdfPathExpression const* theWeaker =
            new SdfPathExpression{SdfPathExpression::MakeAtom(ExpressionReference::Weaker())};
    return *theWeaker;
}

SdfPathExpression SdfPathExpression::MakeComplement(SdfPathExpression&& right) {
...
SdfPathExpression SdfPathExpression::MakeOp(Op op, SdfPathExpression&& left, SdfPathExpression&& right) {
    SdfPathExpression ret;
    ...
    if (op == Difference &&
        (left == Nothing() || right == Nothing() || left == Everything() || right == Everything())) {
        op = Intersection;
    ...
    if (left == Nothing()) {
        ...
        ret._ops.insert(ret._ops.end(), left._ops.begin(), left._ops.end());
        ret._ops.push_back(op);

        // Move the left patterns & refs, then move-insert the same of the
        // right.
        ret._refs = std::move(left._refs);
        ret._refs.insert(ret._refs.end(), make_move_iterator(right._refs.begin()),
                         make_move_iterator(right._refs.end()));
        ret._patterns = std::move(left._patterns);
        ret._patterns.insert(ret._patterns.end(), make_move_iterator(right._patterns.begin()),
                             make_move_iterator(right._patterns.end()));
    }
    return ret;
}

SdfPathExpression SdfPathExpression::MakeAtom(ExpressionReference&& ref) {
...
SdfPathExpression SdfPathExpression::MakeAtom(PathPattern&& pattern) {
...
void SdfPathExpression::WalkWithOpStack(TfFunctionRef<void(std::vector<std::pair<Op, int>> const&)> logic,
    ...
    while (!stack.empty()) {
        ...
        if (stackOp == ExpressionRef) {
            ...
            logic(stack);
            operandIndex++;
            operandIndexEnd = (stackOp == Complement) ? 2 : 3;  // Complement is a unary op.
        }
...
void SdfPathExpression::Walk(TfFunctionRef<void(Op, int)> logic,
                             TfFunctionRef<void(ExpressionReference const&)> ref,
                             TfFunctionRef<void(PathPattern const&)> pattern) const {
...
SdfPathExpression SdfPathExpression::ReplacePrefix(SdfPath const& oldPrefix, SdfPath const& newPrefix) && {
    // We are an rvalue so we mutate & return ourselves.
    for (auto& ref : _refs) {
        ref.path = ref.path.ReplacePrefix(oldPrefix, newPrefix);
    }
    for (auto& pattern : _patterns) {
        pattern.SetPrefix(pattern.GetPrefix().ReplacePrefix(oldPrefix, newPrefix));
    }
    return std::move(*this);
}

bool SdfPathExpression::IsAbsolute() const {
    for (auto& ref : _refs) {
...
SdfPathExpression SdfPathExpression::MakeAbsolute(SdfPath const& anchor) && {
    // We are an rvalue so we mutate & return ourselves.
    for (auto& ref : _refs) {
...
bool SdfPathExpression::ContainsWeakerExpressionReference() const {
...
SdfPathExpression SdfPathExpression::ResolveReferences(
    ...
    auto logic = [&stack](Op op, int argIndex) {
        if (op == Complement) {
            ...
            if (argIndex == 2) {
                ...
                stack.pop_back();
                stack.back() = MakeOp(op, std::move(stack.back()), std::move(arg2));
            }
        }
    };
...
SdfPathExpression SdfPathExpression::ComposeOver(SdfPathExpression const& weaker) && {
    if (IsEmpty()) {
        return std::move(*this);
    }
...
std::string SdfPathExpression::GetText() const {
    std::string result;
    if (IsEmpty()) {
        return result;
    }
    ...
    auto printLogic = [&opName, &opStack, &result](std::vector<std::pair<Op, int>> const& stack) {
        ...
        if (stack.size() >= 2 /* has a parent op */) {
            ...
            parenthesize = parentOp < op || (parentOp == op && parentIndex == 2);
        }
        ...
        if (parenthesize && (op == Complement ? argIndex == 1 : argIndex == 2)) {
            result += ')';
        }
    };
...
std::ostream& operator<<(std::ostream& out, SdfPathExpression const& expr) {
...
struct Sdf_PathExprBuilder {
    ...
    void PushPattern(SdfPathPattern&& pattern) { _stacks.back().PushPattern(std::move(pattern)); }
...
private:
    struct _Stack {
        ...
        void PushExpressionRef(SdfPath&& path, std::string&& name) {
            exprStack.push_back(SdfPathExpression::MakeAtom({std::move(path), std::move(name)}));
        }

        void PushPattern(SdfPathPattern&& pattern) {
            exprStack.push_back(SdfPathExpression::MakeAtom(std::move(pattern)));
        }

        void PushExpr(SdfPathExpression&& expr) { exprStack.push_back(std::move(expr)); }
    ...
    private:
        void _Reduce() {
            ...
            if (op == SdfPathExpression::Complement) {
                ...
                exprStack.push_back(SdfPathExpression::MakeComplement(std::move(right)));
            } else {
                // All other ops are all binary.
                SdfPathExpression left = std::move(exprStack.back());
                exprStack.pop_back();
                exprStack.push_back(SdfPathExpression::MakeOp(op, std::move(left), std::move(right)));
            }
        }
...
struct ExpressionRefPathAndName : seq<list<identifier, one<'/'>>, ExpressionRefName> {};

struct AbsExpressionRefPath : seq<one<'/'>, ExpressionRefPathAndName> {};

struct RelExpressionRefPath : seq<opt<DotDots>, if_then_else<one<'/'>, ExpressionRefPathAndName, ExpressionRefName>> {};

struct ExpressionRefPath : sor<AbsExpressionRefPath, RelExpressionRefPath> {};

struct WeakerRef : seq<string<'%', '_'>, not_at<sor<identifier_other, one<':'>>>> {};

struct ExpressionReference : sor<WeakerRef, seq<one<'%'>, ExpressionRefPath>> {};
...
struct PathExprAtom : sor<ExpressionReference,
...
struct PathOperator
    : sor<OptSpaced<PathSetUnion>, OptSpaced<PathSetIntersection>, OptSpaced<PathSetDifference>, PathSetImpliedUnion> {
};
...
struct PathExprAction<ExpressionRefPath> {
    ...
    static void apply(Input const& in, Sdf_PathExprBuilder& builder) {
        ...
        builder.PushExpressionRef(pathPart.empty() ? SdfPath() : SdfPath(pathPart),
...
template <>
struct PathExprAction<PathSetComplement> : PathExprOpAction<SdfPathExpression::Complement> {};
template <>
struct PathExprAction<PathSetImpliedUnion> : PathExprOpAction<SdfPathExpression::ImpliedUnion> {};
template <>
struct PathExprAction<PathSetUnion> : PathExprOpAction<SdfPathExpression::Union> {};
template <>
struct PathExprAction<PathSetIntersection> : PathExprOpAction<SdfPathExpression::Intersection> {};
template <>
struct PathExprAction<PathSetDifference> : PathExprOpAction<SdfPathExpression::Difference> {};
...
struct PathExprAction<SdfPathPatternParser::PathPattern>
    : change_action_and_states<SdfPathPatternActions::PathPatternAction, SdfPathPatternActions::PatternBuilder> {
    template <class Input>
...
static bool ParsePathExpression(std::string const& inputStr,
    ...
    try {
        parse<must<pad<PathExpr, blank>, eolf>, PathExprAction>(
                string_input<>{inputStr, parseContext.empty() ? "<input>" : parseContext.c_str()}, builder);
        if (expr) {
            *expr = builder.Finish();
        }
