...
template <class T>
using IsSupportedScalarType =
        std::integral_constant<bool,
                               std::is_same<T, std::string>::value || std::is_same<T, bool>::value ||
                                       std::is_same<T, int64_t>::value>;

template <class T>
using IsSupportedListType =
        std::integral_constant<bool,
                               std::is_same<T, VtArray<std::string>>::value || std::is_same<T, VtArray<bool>>::value ||
                                       std::is_same<T, VtArray<int64_t>>::value>;

class _TypeVisitor {
public:
    ValueType operator()(const std::string&) const { return ValueType::String; }

    ValueType operator()(bool) const { return ValueType::Boolean; }

    ValueType operator()(int64_t) const { return ValueType::Integer; }
    ...
    template <class T>
    std::enable_if_t<IsSupportedListType<T>::value, ValueType> operator()(const T&) const {
        return ValueType::List;
    }
...
ValueType GetValueType(const VtValue& value) {
    return VtVisitValue(value, _TypeVisitor());
}

std::string GetValueTypeName(const VtValue& value) {
...
VtValue CoerceIfUnsupportedValueType(const VtValue& value) {
...
static bool _CollectErrors(std::vector<std::string>* errors, EvalResult* result) {
    if (result->errors.empty()) {
        return false;
    }

    errors->insert(errors->end(), std::make_move_iterator(result->errors.begin()),
                   std::make_move_iterator(result->errors.end()));
    return true;
}

template <class Result, class... Others>
static bool _CollectErrors(std::vector<std::string>* errors, Result* result, Others*... others) {
...
static std::vector<std::string> _CombineErrors(Results*... results) {
...
EvalContext::EvalContext(const VtDictionary* variables) : _variables(variables) {}

bool EvalContext::HasVariable(const std::string& var) {
    return static_cast<bool>(TfMapLookupPtr(*_variables, var));
}

std::pair<EvalResult, bool> EvalContext::GetVariable(const std::string& var) {
    // Check if we have circular variable substitutions.
    if (std::find(_variableStack.begin(), _variableStack.end(), var) != _variableStack.end()) {
        std::vector<std::string> formattedVars;
        for (const std::string& s : _variableStack) {
            formattedVars.push_back("'" + s + "'");
        }

        return {EvalResult::Error({TfStringPrintf(
                        "Encountered circular variable substitutions: [%s, '%s']",
                        TfStringJoin(formattedVars.begin(), formattedVars.end(), ", ").c_str(), var.c_str())}),
                true};
    }
    ...
    if (GetValueType(*value) == ValueType::Unknown) {
        return {EvalResult::Error({TfStringPrintf("Variable '%s' has unsupported type %s", var.c_str(),
                                                  GetValueTypeName(*value).c_str())}),
                true};
    }
    ...
    if (value->IsHolding<std::string>()) {
        ...
        if (Sdf_IsVariableExpression(strValue)) {
            Sdf_VariableExpressionParserResult subExpr = Sdf_ParseVariableExpression(strValue);
            if (subExpr.expression) {
                _variableStack.push_back(var);
                TfScoped<> popStack([this]() {
                ...
                for (std::string& err : subExpr.errors) {
                    err += TfStringPrintf(" (in variable '%s')", var.c_str());
                }
...
StringNode::StringNode(std::vector<Part>&& parts) : _parts(std::move(parts)) {
...
EvalResult StringNode::Evaluate(EvalContext* ctx) const {
    std::string result;
            ...
            if (!varHasValue) {
                ...
                if (varResult.value.IsHolding<std::string>()) {
                    ...
                    return EvalResult::Error(
                            {TfStringPrintf("String value required for substituting "
                                            "variable '%s', got %s.",
                                            variable.c_str(), GetValueTypeName(varResult.value).c_str())});
                }
...
VariableNode::VariableNode(std::string&& var) : _var(std::move(var)) {}

EvalResult VariableNode::Evaluate(EvalContext* ctx) const {
    std::pair<EvalResult, bool> varResult = ctx->GetVariable(_var);

    if (!varResult.second) {
        return EvalResult::Error({TfStringPrintf("No value for variable '%s'", _var.c_str())});
    }
...
template <class T>
ConstantNode<T>::ConstantNode(T value) : _value(value) {}

template <class T>
EvalResult ConstantNode<T>::Evaluate(EvalContext* ctx) const {
...
EvalResult NoneNode::Evaluate(EvalContext* ctx) const {
    return EvalResult::NoValue();
}

// ------------------------------------------------------------

ListNode::ListNode(std::vector<std::unique_ptr<Node>>&& elements) : _elements(std::move(elements)) {}

class _ListVisitor {
public:
    template <class T>
    typename std::enable_if_t<IsSupportedScalarType<T>::value, bool> operator()(T v) {
        if (list.IsEmpty()) {
            list = VtArray<T>(1, std::move(v));
            return true;
        } else if (list.IsHolding<VtArray<T>>()) {
            list.UncheckedMutate<VtArray<T>>([&v](VtArray<T>& arr) {
...
EvalResult ListNode::Evaluate(EvalContext* ctx) const {
    ...
    for (size_t i = 0; i < _elements.size(); ++i) {
        ...
        if (!VtVisitValue(r.value, visitor)) {
            errors.push_back(TfStringPrintf("Unexpected value of type %s in list at element %zu",
...
static std::string _FormatFunctionError(const std::string& err) {
    return TfStringPrintf("%s: %s", FunctionNode::GetFunctionName(), err.c_str());
}

// ------------------------------------------------------------

const char* IfNode::GetFunctionName() {
    return "if";
}

EvalResult IfNode::_Evaluate(EvalContext* ctx,
    ...
    if (!result.value.IsHolding<bool>()) {
        return EvalResult::Error({_FormatFunctionError<IfNode>("Condition must be a boolean value")});
    }
    ...
    const EvalResult elseValueResult = elseValue ? elseValue->Evaluate(ctx) : EvalResult::NoValue();

    if (elseValue) {
        const bool valuesHaveDifferentTypes = ifValueResult.value.GetType() != elseValueResult.value.GetType();
        const bool neitherValueIsNone = !ifValueResult.value.IsEmpty() && !elseValueResult.value.IsEmpty();

        if (valuesHaveDifferentTypes && neitherValueIsNone) {
            return EvalResult::Error(
                    {_FormatFunctionError<IfNode>("if-value and else-value must evaluate to the same type "
                                                  "or None.")});
        }
    }
...
If2Node::If2Node(std::unique_ptr<Node>&& condition, std::unique_ptr<Node>&& ifValue)
    : _condition(std::move(condition)), _ifValue(std::move(ifValue)) {}

EvalResult If2Node::Evaluate(EvalContext* ctx) const {
    return _Evaluate(ctx, _condition, _ifValue, nullptr);
}

If3Node::If3Node(std::unique_ptr<Node>&& condition, std::unique_ptr<Node>&& ifValue, std::unique_ptr<Node>&& elseValue)
    : _condition(std::move(condition)), _ifValue(std::move(ifValue)), _elseValue(std::move(elseValue)) {}

EvalResult If3Node::Evaluate(EvalContext* ctx) const {
...
EvalResult _Compare(const T& x, const T& y) {
    return EvalResult::Value(Comparator<T>()(x, y));
}

// Only allow equality comparisons for 'None' values.
template <template <typename> class Comparator>
EvalResult _Compare(const VtValue& x, const VtValue& y) {
    return EvalResult::Error(
            {_FormatFunctionError<ComparisonNode<Comparator>>("Comparison operation not supported for None")});
}

template <>
EvalResult _Compare<std::equal_to>(const VtValue& x, const VtValue& y) {
    return EvalResult::Value(true);
}

template <>
EvalResult _Compare<std::not_equal_to>(const VtValue& x, const VtValue& y) {
...
public:
    ...
    EvalResult operator()(const VtValue& x) const {
        TF_VERIFY(x.IsEmpty() && _y.IsEmpty());
        return _Compare<Comparator>(x, _y);
    }

    template <class T>
    std::enable_if_t<IsSupportedScalarType<T>::value, EvalResult> operator()(const T& x) const {
        return _Compare<T, Comparator>(x, _y.UncheckedGet<T>());
    }

    template <class T>
    std::enable_if_t<!IsSupportedScalarType<T>::value, EvalResult> operator()(const T& x) const {
        return EvalResult::Error({_FormatFunctionError<ComparisonNode<Comparator>>("Unsupported type for comparison")});
    }
...
template <template <typename> class Comparator>
ComparisonNode<Comparator>::ComparisonNode(std::unique_ptr<Node>&& x, std::unique_ptr<Node>&& y)
    : _x(std::move(x)), _y(std::move(y)) {}

template <template <typename> class Comparator>
EvalResult ComparisonNode<Comparator>::Evaluate(EvalContext* ctx) const {
    ...
    if (x.value.GetType() != y.value.GetType()) {
        return EvalResult::Error({_FormatFunctionError<ComparisonNode<Comparator>>(
                TfStringPrintf("Cannot compare values of type %s and %s", GetValueTypeName(x.value).c_str(),
...
#define DEFINE_COMPARISON_NODE(ComparatorType, FunctionName)        \
    ...
    template <>                                                     \
    const char* ComparisonNode<ComparatorType>::GetFunctionName() { \
        return FunctionName;                                        \
    }
...
LogicalNode<Operator>::LogicalNode(std::vector<std::unique_ptr<Node>>&& conditions)
    : _conditions(std::move(conditions)) {}

template <template <typename> class Operator>
EvalResult LogicalNode<Operator>::Evaluate(EvalContext* ctx) const {
    VtValue result;
    std::vector<std::string> errors;
    ...
    for (size_t i = 0; i < _conditions.size(); ++i) {
        ...
        if (_CollectErrors(&errors, &condition)) {
            ...
            errors.push_back(_FormatFunctionError<LogicalNode<Operator>>(
                    TfStringPrintf("Invalid type %s for argument %zu", GetValueTypeName(condition.value).c_str(), i)));
        } else {
            if (result.IsEmpty()) {
                result = condition.value.UncheckedGet<bool>();
            } else {
                result = Operator<bool>()(result.UncheckedGet<bool>(), condition.value.UncheckedGet<bool>());
            }
        }
    };

    return errors.empty() ? EvalResult::Value(std::move(result)) : EvalResult::Error(std::move(errors));
}

#define DEFINE_LOGICAL_NODE(LogicType, FunctionName)        \
    template class LogicalNode<LogicType>;                  \
    template <>                                             \
    const char* LogicalNode<LogicType>::GetFunctionName() { \
        return FunctionName;                                \
    }
...
const char* LogicalNotNode::GetFunctionName() {
    return "not";
}

LogicalNotNode::LogicalNotNode(std::unique_ptr<Node>&& condition) : _condition(std::move(condition)) {}

EvalResult LogicalNotNode::Evaluate(EvalContext* ctx) const {
    EvalResult condition = _condition->Evaluate(ctx);
    if (!condition.errors.empty()) {
        return EvalResult::Error(std::move(condition.errors));
    } else if (!condition.value.IsHolding<bool>()) {
        return EvalResult::Error({_FormatFunctionError<LogicalNotNode>(
                TfStringPrintf("Invalid type %s for argument", GetValueTypeName(condition.value).c_str()))});
    }
...
const char* ContainsNode::GetFunctionName() {
    return "contains";
}

ContainsNode::ContainsNode(std::unique_ptr<Node>&& searchIn, std::unique_ptr<Node>&& searchFor)
    : _searchIn(std::move(searchIn)), _searchFor(std::move(searchFor)) {}

class _ContainsVisitor {
public:
    _ContainsVisitor(const VtValue& searchFor) : _searchFor(searchFor) {}

    EvalResult operator()(const std::string& searchIn) const {
        if (!_searchFor.IsHolding<std::string>()) {
            return Error("Invalid search value");
        }

        return EvalResult::Value(TfStringContains(searchIn, _searchFor.UncheckedGet<std::string>()));
    }

    template <class ListType>
    std::enable_if_t<IsSupportedListType<ListType>::value, EvalResult> operator()(const ListType& searchIn) const {
        using ElemType = typename ListType::value_type;
        if (!_searchFor.IsHolding<ElemType>()) {
            return Error("Invalid search value");
        }

        return EvalResult::Value(std::find(searchIn.begin(), searchIn.end(), _searchFor.UncheckedGet<ElemType>()) !=
                                 searchIn.end());
    }

    template <class T>
    std::enable_if_t<!IsSupportedListType<T>::value, EvalResult> operator()(const T& searchIn) const {
        return Error("Value to search must be a list or string");
    }

private:
    static EvalResult Error(const std::string& err) {
...
EvalResult ContainsNode::Evaluate(EvalContext* ctx) const {
...
const char* AtNode::GetFunctionName() {
    return "at";
}

AtNode::AtNode(std::unique_ptr<Node>&& source, std::unique_ptr<Node>&& index)
    : _source(std::move(source)), _index(std::move(index)) {}

class _AtVisitor {
public:
    _AtVisitor(int64_t index) : _index(index) {}

    EvalResult operator()(const VtValue& v) const {
        if (v.IsHolding<SdfVariableExpression::EmptyList>()) {
    ...
    EvalResult operator()(const std::string& s) const {
    ...
    template <class ListType>
    std::enable_if_t<IsSupportedListType<ListType>::value, EvalResult> operator()(const ListType& l) const {
...
private:
    static EvalResult _Error(const std::string& err) { return EvalResult::Error({_FormatFunctionError<AtNode>(err)}); }

    // Normalize index to support negative indices. Lifted from
    // TfPyNormalizeIndex.
    std::pair<size_t, bool> _NormalizeIndex(size_t size) const {
...
EvalResult AtNode::Evaluate(EvalContext* ctx) const {
    ...
    if (!index.value.IsHolding<int64_t>()) {
        return EvalResult::Error({_FormatFunctionError<AtNode>("Index must be an integer")});
    }

    return VtVisitValue(source.value, _AtVisitor(index.value.UncheckedGet<int64_t>()));
}

// ------------------------------------------------------------

const char* LenNode::GetFunctionName() {
    return "len";
}

LenNode::LenNode(std::unique_ptr<Node>&& source) : _source(std::move(source)) {}
...
public:
    ...
    template <class T>
    using CanGetLength =
            std::integral_constant<bool, IsSupportedListType<T>::value || std::is_same<T, std::string>::value>;

    template <class T>
    std::enable_if_t<CanGetLength<T>::value, EvalResult> operator()(const T& v) const {
...
private:
    ...
    EvalResult _GetUnsupportedTypeError() const {
        return EvalResult::Error({_FormatFunctionError<LenNode>("Unsupported type")});
    }
};

EvalResult LenNode::Evaluate(EvalContext* ctx) const {
    EvalResult source = _source->Evaluate(ctx);
...
const char* DefinedNode::GetFunctionName() {
    return "defined";
}

DefinedNode::DefinedNode(std::vector<std::unique_ptr<Node>>&& nodes) : _nodes(std::move(nodes)) {}

EvalResult DefinedNode::Evaluate(EvalContext* ctx) const {
    VtValue result;
    std::vector<std::string> errors;
    ...
    for (size_t i = 0; i < _nodes.size(); ++i) {
        ...
        if (_CollectErrors(&errors, &varName)) {
            ...
            errors.push_back(_FormatFunctionError<DefinedNode>(
                    TfStringPrintf("Invalid type %s for argument %zu", GetValueTypeName(varName.value).c_str(), i)));
        } else {
            const bool isDefined = ctx->HasVariable(varName.value.UncheckedGet<std::string>());
            result = result.GetWithDefault<bool>(true) && isDefined;
        }
    }

    return errors.empty() ? EvalResult::Value(result) : EvalResult::Error(std::move(errors));
}
