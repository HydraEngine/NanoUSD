...
private:
    struct _Stack {
        ...
        void PushCall(SdfPredicateExpression::FnCall::Kind kind,
            ...
            exprStack.push_back(SdfPredicateExpression::MakeCall({kind, std::move(name), std::move(args)}));
        }

        void PushExpr(SdfPredicateExpression&& expr) { exprStack.push_back(std::move(expr)); }
...
struct ReservedWord : sor<NotKW, AndKW, OrKW, Inf, True, False> {};

struct Digits : plus<range<'0', '9'>> {};

struct Exp : seq<one<'e', 'E'>, opt<one<'-', '+'>>, must<Digits>> {};
struct Frac : if_must<one<'.'>, Digits> {};
struct PredArgFloat : seq<opt<one<'-'>>, sor<Inf, seq<Digits, if_then_else<Frac, opt<Exp>, Exp>>>> {};
...
struct PredParenCall : seq<PredFuncName, OptSpaced<one<'('>>, must<PredParenArgs, star<blank>, one<')'>>> {};
...
struct PredAtom
    : sor<PredColonCall, PredParenCall, PredBareCall, if_must<PredOpenGroup, OptSpaced<PredExpr>, PredCloseGroup>> {};
...
template <>
struct PredAction<ImpliedAnd> : PredOpAction<SdfPredicateExpression::ImpliedAnd> {};
...
struct PredAction<PredArgString> {
    ...
    static void apply(Input const& in, SdfPredicateExprBuilder& builder) {
        ...
        size_t trimAmount = 0;
        if (instr.size() >= 2 &&
            ((instr.front() == '"' && instr.back() == '"') || (instr.front() == '\'' && instr.back() == '\''))) {
            trimAmount = 1;
        }
