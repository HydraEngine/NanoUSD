...
TF_DEFINE_ENV_SETTING(SDF_LAYER_VALIDATE_AUTHORING,
                      false,
...
TF_DEFINE_ENV_SETTING(SDF_LAYER_INCLUDE_DETACHED,
                      "",
...
TF_DEFINE_ENV_SETTING(SDF_LAYER_EXCLUDE_DETACHED,
                      "",
...
TF_MAKE_STATIC_DATA(SdfLayer::DetachedLayerRules, _detachedLayerRules) {
    const std::vector<std::string> includePatterns = TfStringSplit(TfGetEnvSetting(SDF_LAYER_INCLUDE_DETACHED), ",");
    if (std::find(includePatterns.begin(), includePatterns.end(), "*") != includePatterns.end()) {
        _detachedLayerRules->IncludeAll();
    } else if (!includePatterns.empty()) {
        _detachedLayerRules->Include(includePatterns);
    }

    const std::vector<std::string> excludePatterns = TfStringSplit(TfGetEnvSetting(SDF_LAYER_EXCLUDE_DETACHED), ",");
...
static tbb::queuing_rw_mutex& _GetLayerRegistryMutex() {
    static tbb::queuing_rw_mutex mutex;
    return mutex;
}

static SdfAbstractDataRefPtr _CreateDataForFileFormat(const SdfFileFormatConstPtr& fileFormat,
                                                      const std::string& identifier,
                                                      const SdfLayer::FileFormatArguments& args) {
    return SdfLayer::IsIncludedByDetachedLayerRules(identifier) ? fileFormat->InitDetachedData(args)
                                                                : fileFormat->InitData(args);
}

SdfLayer::SdfLayer(const SdfFileFormatConstPtr& fileFormat,
    ...
    : _self(this),
      ...
      _validateAuthoring(validateAuthoring || TfGetEnvSetting<bool>(SDF_LAYER_VALIDATE_AUTHORING)),
    ...
    string layerIdentifier =
            Sdf_IsAnonLayerIdentifier(identifier) ? Sdf_ComputeAnonLayerIdentifier(identifier, this) : identifier;
...
const SdfFileFormatConstPtr& SdfLayer::GetFileFormat() const {
    return _fileFormat;
}

const SdfLayer::FileFormatArguments& SdfLayer::GetFileFormatArguments() const {
    return _fileFormatArgs;
}

SdfLayerRefPtr SdfLayer::_CreateNewWithFormat(const SdfFileFormatConstPtr& fileFormat,
                                              const string& identifier,
                                              const string& realPath,
                                              const ArAssetInfo& assetInfo,
                                              const FileFormatArguments& args) {
    ...
    return fileFormat->NewLayer(fileFormat, identifier, realPath, assetInfo, args);
}

void SdfLayer::_FinishInitialization(bool success) {
    _initializationWasSuccessful = success;
    _initializationComplete = true;  // unblock waiters.
}

bool SdfLayer::_WaitForInitializationAndCheckIfSuccessful() {
...
static bool _ModificationTimesEqual(const VtValue& v1, const VtValue& v2) {
...
static bool _ModificationTimesEqual(const VtDictionary& t1, const VtDictionary& t2) {
    if (t1.size() != t2.size()) {
        return false;
    }
    ...
    for (const auto& e1 : t1) {
        ...
        if (t2Iter == t2.end() || !_ModificationTimesEqual(e1.second, t2Iter->second)) {
            return false;
        }
    }
...
SdfLayerRefPtr SdfLayer::CreateAnonymous(const string& tag,
...
SdfLayerRefPtr SdfLayer::_CreateAnonymousWithFormat(const SdfFileFormatConstPtr& fileFormat,
                                                    const std::string& tag,
                                                    const FileFormatArguments& args) {
    if (fileFormat->IsPackage()) {
        TF_CODING_ERROR(
                "Cannot create anonymous layer: creating package %s "
    ...
    SdfLayerRefPtr layer =
            _CreateNewWithFormat(fileFormat, Sdf_GetAnonLayerIdentifierTemplate(tag), string(), ArAssetInfo(), args);
...
bool SdfLayer::IsAnonymousLayerIdentifier(const string& identifier) {
    return Sdf_IsAnonLayerIdentifier(identifier);
}

string SdfLayer::GetDisplayNameFromIdentifier(const string& identifier) {
    return Sdf_GetLayerDisplayName(identifier);
}

SdfLayerRefPtr SdfLayer::CreateNew(const string& identifier, const FileFormatArguments& args) {
    TF_DEBUG(SDF_LAYER).Msg("SdfLayer::CreateNew('%s', '%s')\n", identifier.c_str(), TfStringify(args).c_str());

    return _CreateNew(TfNullPtr, identifier, args);
}

SdfLayerRefPtr SdfLayer::CreateNew(const SdfFileFormatConstPtr& fileFormat,
                                   const string& identifier,
                                   const FileFormatArguments& args) {
    TF_DEBUG(SDF_LAYER).Msg("SdfLayer::CreateNew('%s', '%s', '%s')\n", fileFormat->GetFormatId().GetText(),
                            identifier.c_str(), TfStringify(args).c_str());

    return _CreateNew(fileFormat, identifier, args);
}

static SdfLayer::FileFormatArguments& _CanonicalizeFileFormatArguments(const std::string& filePath,
    ...
    SdfLayer::FileFormatArguments::iterator targetIt = args.find(SdfFileFormatTokens->TargetArg);
    ...
    // arguments specified.
    const SdfLayer::FileFormatArguments defaultArgs = fileFormat->GetDefaultFileFormatArguments();
...
SdfLayerRefPtr SdfLayer::_CreateNew(SdfFileFormatConstPtr fileFormat,
                                    const string& identifier,
                                    const FileFormatArguments& args,
                                    bool saveLayer) {
    string whyNot;
    ...
    if (localPath.empty()) {
        TF_CODING_ERROR("Cannot create new layer '%s': %s", absIdentifier.c_str(),
                        (whyNot.empty() ? "failed to compute path for new layer" : whyNot.c_str()));
        return TfNullPtr;
    }
    ...
    if (!fileFormat) {
        ...
        if (!TF_VERIFY(fileFormat)) return TfNullPtr;
    }
    ...
    if (Sdf_IsPackageOrPackagedLayer(fileFormat, identifier)) {
        TF_CODING_ERROR(
                "Cannot create new layer '%s': creating %s %s "
                "layer is not allowed through this API.",
                identifier.c_str(), fileFormat->IsPackage() ? "package" : "packaged",
                fileFormat->GetFormatId().GetText());
        return TfNullPtr;
    }
    ...
    if (targetArgIt != finalArgs.end()) {
        absIdentifier = Sdf_CreateIdentifier(absIdentifier, FileFormatArguments{*targetArgIt});
    }
    ...
    SdfLayerRefPtr layer;
    {
        ...
        if (_layerRegistry->Find(absIdentifier)) {
            TF_CODING_ERROR("A layer already exists with identifier '%s'", absIdentifier.c_str());
            return TfNullPtr;
        }

        layer = _CreateNewWithFormat(fileFormat, absIdentifier, localPath, ArAssetInfo(), finalArgs);
...
SdfLayerRefPtr SdfLayer::New(const SdfFileFormatConstPtr& fileFormat,
                             const string& identifier,
                             const FileFormatArguments& args) {
...
bool SdfLayer::_ComputeInfoToFindOrOpenLayer(const string& identifier,
                                             const SdfLayer::FileFormatArguments& args,
    ...
    if (!Sdf_SplitIdentifier(identifier, &layerPath, &layerArgs) || layerPath.empty()) {
        return false;
    }
    ...
    if (!isAnonymous) {
        ...
        resolvedLayerPath = Sdf_ResolvePath(layerPath, computeAssetInfo ? &assetInfo : nullptr);
    }
    ...
    info->fileFormat =
            SdfFileFormat::FindByExtension(resolvedLayerPath.empty() ? layerPath : resolvedLayerPath, layerArgs);
    info->fileFormatArgs.swap(_CanonicalizeFileFormatArguments(layerPath, info->fileFormat, layerArgs));

    info->isAnonymous = isAnonymous;
    info->layerPath.swap(layerPath);
    info->resolvedLayerPath = std::move(resolvedLayerPath);
    info->identifier = Sdf_CreateIdentifier(info->layerPath, info->fileFormatArgs);
    swap(info->assetInfo, assetInfo);
    return true;
}

template <class ScopedLock>
SdfLayerRefPtr SdfLayer::_TryToFindLayer(const string& identifier,
...
retry:
    ...
    if (!retryAsWriter) lock.release();

    return result;
}

/* static */
SdfLayerRefPtr SdfLayer::FindOrOpen(const string& identifier, const FileFormatArguments& args) {
    TRACE_FUNCTION();
    TF_DEBUG(SDF_LAYER).Msg("SdfLayer::FindOrOpen('%s', '%s')\n", identifier.c_str(), TfStringify(args).c_str());
    ...
    if (SdfLayerRefPtr layer =
                _TryToFindLayer(layerInfo.identifier, layerInfo.resolvedLayerPath, lock, /*retryAsWriter=*/true)) {
...
/* static */
SdfLayerRefPtr SdfLayer::FindOrOpenRelativeToLayer(const SdfLayerHandle& anchor,
                                                   const string& identifier,
                                                   const FileFormatArguments& args) {
    TRACE_FUNCTION();
    ...
    return FindOrOpen(SdfComputeAssetPathRelativeToLayer(anchor, identifier), args);
}

/* static */
SdfLayerRefPtr SdfLayer::OpenAsAnonymous(const std::string& layerPath, bool metadataOnly, const std::string& tag) {
    TF_PY_ALLOW_THREADS_IN_SCOPE();

    _FindOrOpenLayerInfo layerInfo;
    if (!_ComputeInfoToFindOrOpenLayer(layerPath, FileFormatArguments(), &layerInfo)) {
        return TfNullPtr;
    }
    ...
    if (!layerInfo.fileFormat) {
        TF_CODING_ERROR("Cannot determine file format for @%s@", layerInfo.identifier.c_str());
        return TfNullPtr;
    }
    ...
    {
        tbb::queuing_rw_mutex::scoped_lock lock(_GetLayerRegistryMutex());
        layer = _CreateNewWithFormat(layerInfo.fileFormat, Sdf_GetAnonLayerIdentifierTemplate(tag), string());
    ...
    try {
        if (!layer->_Read(layerInfo.identifier, layerInfo.resolvedLayerPath, metadataOnly)) {
...
const SdfSchemaBase& SdfLayer::GetSchema() const {
    return _schema;
}
...
SdfLayer::_ReloadResult SdfLayer::_Reload(bool force) {
    ...
    if (isAnonymous && GetFileFormat()->ShouldSkipAnonymousReload()) {
        ...
        VtDictionary externalAssetTimestamps = Sdf_ComputeExternalAssetModificationTimestamps(*this);
        ...
        if (resolvedPath.empty()) {
            TF_RUNTIME_ERROR("Cannot determine resolved path for '%s', skipping reload.", identifier.c_str());
            return _ReloadFailed;
        }
        ...
        // See if we can skip reloading.
        if (!force && !IsDirty() && (resolvedPath == oldResolvedPath) &&
            (_ModificationTimesEqual(timestamp, _assetModificationTime)) &&
            (_ModificationTimesEqual(externalAssetTimestamps, _externalAssetModificationTimes))) {
            return _ReloadSkipped;
        }
...
bool SdfLayer::Reload(bool force) {
    return _Reload(force) == _ReloadSucceeded;
}

bool SdfLayer::ReloadLayers(const set<SdfLayerHandle>& layers, bool force) {
    ...
    TF_FOR_ALL(layer, layers) {
        if (*layer) {
            if ((*layer)->_Reload(force) == _ReloadFailed) {
                status = false;
                TF_WARN("Unable to re-read @%s@", (*layer)->GetIdentifier().c_str());
            }
        }
    }
...
bool SdfLayer::Import(const string& layerPath) {
...
bool SdfLayer::ImportFromString(const std::string& s) {
    return GetFileFormat()->ReadFromString(this, s);
}

bool SdfLayer::_Read(const string& identifier, const ArResolvedPath& resolvedPathIn, bool metadataOnly) {
    TRACE_FUNCTION();
    ...
    TF_DEBUG(SDF_LAYER).Msg("SdfLayer::_Read('%s', '%s', metadataOnly=%s)\n", identifier.c_str(),
                            resolvedPathIn.GetPathString().c_str(), TfStringify(metadataOnly).c_str());

    SdfFileFormatConstPtr format = GetFileFormat();
    if (!format->SupportsReading()) {
        TF_CODING_ERROR(
                "Cannot read layer @%s@: %s file format does not"
                "support reading",
                identifier.c_str(), format->GetFormatId().GetText());
        return false;
    }
    return IsIncludedByDetachedLayerRules(identifier) ? format->ReadDetached(this, resolvedPath, metadataOnly)
                                                      : format->Read(this, resolvedPath, metadataOnly);
}

/*static*/
SdfLayerHandle SdfLayer::Find(const string& identifier, const FileFormatArguments& args) {
    TRACE_FUNCTION();
...
SdfLayerRefPtr SdfLayer::_Find(const string& identifier,
    ...
    if (SdfLayerRefPtr layer =
                _TryToFindLayer(layerInfo.identifier, layerInfo.resolvedLayerPath, lock, retryAsWriter)) {
...
/* static */
SdfLayerHandle SdfLayer::FindRelativeToLayer(const SdfLayerHandle& anchor,
                                             const string& identifier,
                                             const FileFormatArguments& args) {
    TRACE_FUNCTION();
    ...
    return Find(SdfComputeAssetPathRelativeToLayer(anchor, identifier), args);
}

std::set<double> SdfLayer::ListAllTimeSamples() const {
    return _data->ListAllTimeSamples();
}

std::set<double> SdfLayer::ListTimeSamplesForPath(const SdfPath& path) const {
    return _data->ListTimeSamplesForPath(path);
}

bool SdfLayer::GetBracketingTimeSamples(double time, double* tLower, double* tUpper) {
    return _data->GetBracketingTimeSamples(time, tLower, tUpper);
}

size_t SdfLayer::GetNumTimeSamplesForPath(const SdfPath& path) const {
    return _data->GetNumTimeSamplesForPath(path);
}

bool SdfLayer::GetBracketingTimeSamplesForPath(const SdfPath& path, double time, double* tLower, double* tUpper) {
    return _data->GetBracketingTimeSamplesForPath(path, time, tLower, tUpper);
}

bool SdfLayer::QueryTimeSample(const SdfPath& path, double time, VtValue* value) const {
    return _data->QueryTimeSample(path, time, value);
}

bool SdfLayer::QueryTimeSample(const SdfPath& path, double time, SdfAbstractDataValue* value) const {
    return _data->QueryTimeSample(path, time, value);
}

static TfType _GetExpectedTimeSampleValueType(const SdfLayer& layer, const SdfPath& path) {
    const SdfSpecType specType = layer.GetSpecType(path);
    if (specType == SdfSpecTypeUnknown) {
        TF_CODING_ERROR(
                "Cannot set time sample at <%s> since spec does "
                "not exist",
                path.GetText());
        return TfType();
    } else if (specType != SdfSpecTypeAttribute) {
        TF_CODING_ERROR(
                "Cannot set time sample at <%s> because spec "
                "is not an attribute",
                path.GetText());
        return TfType();
    }
    ...
    if (!valueType) {
        TF_CODING_ERROR("Cannot determine value type for <%s>", path.GetText());
    }

    return valueType;
}

void SdfLayer::SetTimeSample(const SdfPath& path, double time, const VtValue& value) {
    if (!PermissionToEdit()) {
        TF_CODING_ERROR(
                "Cannot set time sample on <%s>.  "
    ...
    if (value.GetType() == expectedType) {
        ...
        const VtValue castValue = VtValue::CastToTypeid(value, expectedType.GetTypeid());
        if (castValue.IsEmpty()) {
            TF_CODING_ERROR(
                    "Can't set time sample on <%s> to %s: "
                    "expected a value of type \"%s\"",
                    path.GetText(), TfStringify(value).c_str(), expectedType.GetTypeName().c_str());
            return;
        }
...
void SdfLayer::SetTimeSample(const SdfPath& path, double time, const SdfAbstractDataConstValue& value) {
    if (!PermissionToEdit()) {
        TF_CODING_ERROR(
                "Cannot set time sample on <%s>.  "
                "Layer @%s@ is not editable.",
    ...
    if (TfSafeTypeCompare(value.valueType, expectedType.GetTypeid())) {
        ...
        const VtValue castValue = VtValue::CastToTypeid(tmpValue, expectedType.GetTypeid());
        if (castValue.IsEmpty()) {
            TF_CODING_ERROR(
                    "Can't set time sample on <%s> to %s: "
                    "expected a value of type \"%s\"",
                    path.GetText(), TfStringify(tmpValue).c_str(), expectedType.GetTypeName().c_str());
            return;
        }
...
void SdfLayer::EraseTimeSample(const SdfPath& path, double time) {
    if (!PermissionToEdit()) {
        TF_CODING_ERROR(
                "Cannot set time sample on <%s>.  "
                "Layer @%s@ is not editable.",
    ...
    if (!HasSpec(path)) {
        TF_CODING_ERROR(
                "Cannot SetTimeSample at <%s> since spec does "
                "not exist",
                path.GetText());
        return;
    }
...
static const VtValue& _GetVtValue(const VtValue& v) {
    return v;
}

static VtValue _GetVtValue(const SdfAbstractDataConstValue& v) {
    VtValue value;
...
template <class T>
void SdfLayer::_PrimSetTimeSample(const SdfPath& path, double time, const T& value, bool useDelegate) {
...
template void SdfLayer::_PrimSetTimeSample(const SdfPath&, double, const VtValue&, bool);
template void SdfLayer::_PrimSetTimeSample(const SdfPath&, double, const SdfAbstractDataConstValue&, bool);

// ---
// End of SdfLayer static functions
// ---

void SdfLayer::_InitializeFromIdentifier(const string& identifier,
                                         const string& realPath,
    ...
    std::unique_ptr<Sdf_AssetInfo> newInfo(
            Sdf_ComputeAssetInfoFromIdentifier(identifier, realPath, assetInfo, fileVersion));
    if (!newInfo) return;
    ...
    if (oldIdentifier.empty()) {
        ...
        if (oldIdentifier != GetIdentifier()) {
            Sdf_ChangeManager::Get().DidChangeLayerIdentifier(_self, oldIdentifier);
        }
...
template <class T>
inline void SdfLayer::_SetValue(const TfToken& key, T value) {
    SetField(SdfPath::AbsoluteRootPath(), key, VtValue(value));
}

template <class T>
inline T SdfLayer::_GetValue(const TfToken& key) const {
    VtValue value;
...
SdfAssetPath SdfLayer::GetColorConfiguration() const {
    return _GetValue<SdfAssetPath>(SdfFieldKeys->ColorConfiguration);
}

void SdfLayer::SetColorConfiguration(const SdfAssetPath& colorConfiguration) {
    _SetValue(SdfFieldKeys->ColorConfiguration, colorConfiguration);
}

bool SdfLayer::HasColorConfiguration() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->ColorConfiguration);
}

void SdfLayer::ClearColorConfiguration() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->ColorConfiguration);
}

TfToken SdfLayer::GetColorManagementSystem() const {
    return _GetValue<TfToken>(SdfFieldKeys->ColorManagementSystem);
}

void SdfLayer::SetColorManagementSystem(const TfToken& cms) {
    _SetValue(SdfFieldKeys->ColorManagementSystem, cms);
}

bool SdfLayer::HasColorManagementSystem() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->ColorManagementSystem);
}

void SdfLayer::ClearColorManagementSystem() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->ColorManagementSystem);
}

void SdfLayer::SetComment(const string& newVal) {
    _SetValue(SdfFieldKeys->Comment, newVal);
}

string SdfLayer::GetComment() const {
    return _GetValue<string>(SdfFieldKeys->Comment);
}

void SdfLayer::SetDefaultPrim(const TfToken& name) {
    _SetValue(SdfFieldKeys->DefaultPrim, name);
}

TfToken SdfLayer::GetDefaultPrim() const {
    return _GetValue<TfToken>(SdfFieldKeys->DefaultPrim);
}

SdfPath SdfLayer::GetDefaultPrimAsPath() const {
    std::string pathString = _GetValue<TfToken>(SdfFieldKeys->DefaultPrim).GetString();
    SdfPath path = SdfPath::IsValidPathString(pathString) ? SdfPath(pathString) : SdfPath();
    return path.IsPrimPath() ? path.IsAbsolutePath() ? path : path.MakeAbsolutePath(SdfPath::AbsoluteRootPath())
                             : SdfPath();
}

void SdfLayer::ClearDefaultPrim() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->DefaultPrim);
}

bool SdfLayer::HasDefaultPrim() {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->DefaultPrim);
}

void SdfLayer::SetDocumentation(const string& newVal) {
    _SetValue(SdfFieldKeys->Documentation, newVal);
}

string SdfLayer::GetDocumentation() const {
    return _GetValue<string>(SdfFieldKeys->Documentation);
}

void SdfLayer::SetStartTimeCode(double newVal) {
    _SetValue(SdfFieldKeys->StartTimeCode, newVal);
}

double SdfLayer::GetStartTimeCode() const {
    return _GetValue<double>(SdfFieldKeys->StartTimeCode);
}

bool SdfLayer::HasStartTimeCode() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->StartTimeCode);
}

void SdfLayer::ClearStartTimeCode() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->StartTimeCode);
}

void SdfLayer::SetEndTimeCode(double newVal) {
    _SetValue(SdfFieldKeys->EndTimeCode, newVal);
}

double SdfLayer::GetEndTimeCode() const {
    return _GetValue<double>(SdfFieldKeys->EndTimeCode);
}

bool SdfLayer::HasEndTimeCode() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->EndTimeCode);
}

void SdfLayer::ClearEndTimeCode() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->EndTimeCode);
}

void SdfLayer::SetTimeCodesPerSecond(double newVal) {
    _SetValue(SdfFieldKeys->TimeCodesPerSecond, newVal);
}

double SdfLayer::GetTimeCodesPerSecond() const {
    // If there is an authored value for timeCodesPerSecond, return that.
    VtValue value;
    if (HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->TimeCodesPerSecond, &value)) {
...
bool SdfLayer::HasTimeCodesPerSecond() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->TimeCodesPerSecond);
}

void SdfLayer::ClearTimeCodesPerSecond() {
    return EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->TimeCodesPerSecond);
}

void SdfLayer::SetFramesPerSecond(double newVal) {
    _SetValue(SdfFieldKeys->FramesPerSecond, newVal);
}

double SdfLayer::GetFramesPerSecond() const {
    return _GetValue<double>(SdfFieldKeys->FramesPerSecond);
}

bool SdfLayer::HasFramesPerSecond() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->FramesPerSecond);
}

void SdfLayer::ClearFramesPerSecond() {
    return EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->FramesPerSecond);
}

void SdfLayer::SetFramePrecision(int newVal) {
    _SetValue(SdfFieldKeys->FramePrecision, newVal);
}

int SdfLayer::GetFramePrecision() const {
    return _GetValue<int>(SdfFieldKeys->FramePrecision);
}

bool SdfLayer::HasFramePrecision() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->FramePrecision);
}

void SdfLayer::ClearFramePrecision() {
    return EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->FramePrecision);
}

string SdfLayer::GetOwner() const {
    return _GetValue<string>(SdfFieldKeys->Owner);
}

void SdfLayer::SetOwner(const std::string& newVal) {
    _SetValue(SdfFieldKeys->Owner, newVal);
}

bool SdfLayer::HasOwner() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->Owner);
}

void SdfLayer::ClearOwner() {
    return EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->Owner);
}

string SdfLayer::GetSessionOwner() const {
    return _GetValue<string>(SdfFieldKeys->SessionOwner);
}

void SdfLayer::SetSessionOwner(const std::string& newVal) {
    _SetValue(SdfFieldKeys->SessionOwner, newVal);
}

bool SdfLayer::HasSessionOwner() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SessionOwner);
}

void SdfLayer::ClearSessionOwner() {
    return EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SessionOwner);
}

bool SdfLayer::GetHasOwnedSubLayers() const {
    return _GetValue<bool>(SdfFieldKeys->HasOwnedSubLayers);
}

void SdfLayer::SetHasOwnedSubLayers(bool newVal) {
    _SetValue(SdfFieldKeys->HasOwnedSubLayers, newVal);
}

VtDictionary SdfLayer::GetCustomLayerData() const {
    return _GetValue<VtDictionary>(SdfFieldKeys->CustomLayerData);
}

void SdfLayer::SetCustomLayerData(const VtDictionary& dict) {
    _SetValue(SdfFieldKeys->CustomLayerData, dict);
}

bool SdfLayer::HasCustomLayerData() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->CustomLayerData);
}

void SdfLayer::ClearCustomLayerData() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->CustomLayerData);
}

VtDictionary SdfLayer::GetExpressionVariables() const {
    return _GetValue<VtDictionary>(SdfFieldKeys->ExpressionVariables);
}

void SdfLayer::SetExpressionVariables(const VtDictionary& dict) {
    _SetValue(SdfFieldKeys->ExpressionVariables, dict);
}

bool SdfLayer::HasExpressionVariables() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->ExpressionVariables);
}

void SdfLayer::ClearExpressionVariables() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->ExpressionVariables);
}

SdfPrimSpecHandle SdfLayer::GetPseudoRoot() const {
    return SdfPrimSpecHandle(_idRegistry.Identify(SdfPath::AbsoluteRootPath()));
}

SdfLayer::RootPrimsView SdfLayer::GetRootPrims() const {
    return GetPseudoRoot()->GetNameChildren();
}

void SdfLayer::SetRootPrims(const SdfPrimSpecHandleVector& newComps) {
    return GetPseudoRoot()->SetNameChildren(newComps);
}

bool SdfLayer::InsertRootPrim(const SdfPrimSpecHandle& prim, int index) {
    return GetPseudoRoot()->InsertNameChild(prim, index);
}

void SdfLayer::RemoveRootPrim(const SdfPrimSpecHandle& prim) {
    GetPseudoRoot()->RemoveNameChild(prim);
}

SdfNameOrderProxy SdfLayer::GetRootPrimOrder() const {
    return GetPseudoRoot()->GetNameChildrenOrder();
}

void SdfLayer::SetRootPrimOrder(const vector<TfToken>& names) {
    GetPseudoRoot()->SetNameChildrenOrder(names);
}

void SdfLayer::InsertInRootPrimOrder(const TfToken& name, int index) {
    GetPseudoRoot()->InsertInNameChildrenOrder(name, index);
}

void SdfLayer::RemoveFromRootPrimOrder(const TfToken& name) {
    GetPseudoRoot()->RemoveFromNameChildrenOrder(name);
}

void SdfLayer::RemoveFromRootPrimOrderByIndex(int index) {
    GetPseudoRoot()->RemoveFromNameChildrenOrderByIndex(index);
}

void SdfLayer::ApplyRootPrimOrder(vector<TfToken>* vec) const {
    GetPseudoRoot()->ApplyNameChildrenOrder(vec);
}

SdfSubLayerProxy SdfLayer::GetSubLayerPaths() const {
    return SdfSubLayerProxy(std::make_unique<Sdf_SubLayerListEditor>(_self), SdfListOpTypeOrdered);
}

void SdfLayer::SetSubLayerPaths(const vector<string>& newPaths) {
    GetSubLayerPaths() = newPaths;
}

size_t SdfLayer::GetNumSubLayerPaths() const {
    return GetSubLayerPaths().size();
}

void SdfLayer::InsertSubLayerPath(const string& path, int index) {
...
void SdfLayer::RemoveSubLayerPath(int index) {
    GetSubLayerPaths().Erase(index);
}

SdfLayerOffsetVector SdfLayer::GetSubLayerOffsets() const {
    return GetFieldAs<SdfLayerOffsetVector>(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SubLayerOffsets);
}

SdfLayerOffset SdfLayer::GetSubLayerOffset(int index) const {
...
void SdfLayer::SetSubLayerOffset(const SdfLayerOffset& offset, int index) {
    SdfLayerOffsetVector offsets =
            GetFieldAs<SdfLayerOffsetVector>(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SubLayerOffsets);
    if (index < 0 || static_cast<size_t>(index) >= offsets.size()) {
        TF_CODING_ERROR("Invalid sublayer index");
        return;
    }
    ...
    SetField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SubLayerOffsets, VtValue(offsets));
}

SdfRelocates SdfLayer::GetRelocates() const {
    return GetFieldAs<SdfRelocates>(SdfPath::AbsoluteRootPath(), SdfFieldKeys->LayerRelocates);
}

void SdfLayer::SetRelocates(const SdfRelocates& relocates) {
    SetField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->LayerRelocates, relocates);
}

bool SdfLayer::HasRelocates() const {
    return HasField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->LayerRelocates);
}

void SdfLayer::ClearRelocates() {
    EraseField(SdfPath::AbsoluteRootPath(), SdfFieldKeys->LayerRelocates);
}

bool SdfLayer::_CanGetSpecAtPath(const SdfPath& path, SdfPath* canonicalPath, SdfSpecType* specType) const {
    if (path.IsEmpty()) {
        return false;
    }
...
template <class Spec>
SdfHandle<Spec> SdfLayer::_GetSpecAtPath(const SdfPath& path) {
    SdfPath canonicalPath;
    SdfSpecType specType;
    if (!_CanGetSpecAtPath(path, &canonicalPath, &specType) || !Sdf_SpecType::CanCast(specType, typeid(Spec))) {
        return TfNullPtr;
    }
...
SdfSpecHandle SdfLayer::GetObjectAtPath(const SdfPath& path) {
...
SdfPrimSpecHandle SdfLayer::GetPrimAtPath(const SdfPath& path) {
...
SdfPropertySpecHandle SdfLayer::GetPropertyAtPath(const SdfPath& path) {
    return _GetSpecAtPath<SdfPropertySpec>(path);
}

SdfAttributeSpecHandle SdfLayer::GetAttributeAtPath(const SdfPath& path) {
    return _GetSpecAtPath<SdfAttributeSpec>(path);
}

SdfRelationshipSpecHandle SdfLayer::GetRelationshipAtPath(const SdfPath& path) {
    return _GetSpecAtPath<SdfRelationshipSpec>(path);
}

bool SdfLayer::PermissionToEdit() const {
    return _permissionToEdit && !IsMuted();
}

bool SdfLayer::PermissionToSave() const {
    return _permissionToSave && !IsAnonymous() && !IsMuted() && Sdf_CanWriteLayerToPath(GetResolvedPath());
}

void SdfLayer::SetPermissionToEdit(bool allow) {
    _permissionToEdit = allow;
}

void SdfLayer::SetPermissionToSave(bool allow) {
    _permissionToSave = allow;
}

static bool _HasObjectAtPath(const SdfLayerHandle& layer, const SdfPath& path) {
    return layer->GetObjectAtPath(path);
}

static bool _CanEdit(const SdfLayerHandle& layer, const SdfNamespaceEdit& edit, std::string* detail) {
    if (edit.currentPath.IsPrimPath()) {
        if (edit.newPath.IsEmpty()) {
            // Remove prim child.
            return Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::CanRemoveChildForBatchNamespaceEdit(
                    layer, edit.currentPath.GetParentPath(), edit.currentPath.GetNameToken(), detail);
        } else {
            // Insert prim child.
            return Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::CanMoveChildForBatchNamespaceEdit(
                    layer, edit.newPath.GetParentPath(), layer->GetPrimAtPath(edit.currentPath),
                    edit.newPath.GetNameToken(), edit.index, detail);
        }
    } else {
        if (edit.newPath.IsEmpty()) {
            if (edit.currentPath.IsRelationalAttributePath()) {
                // Remove relational attribute.
                return Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::CanRemoveChildForBatchNamespaceEdit(
                        layer, edit.currentPath.GetParentPath(), edit.currentPath.GetNameToken(), detail);
            } else {
                // Remove prim property.
                return Sdf_ChildrenUtils<Sdf_PropertyChildPolicy>::CanRemoveChildForBatchNamespaceEdit(
                        layer, edit.currentPath.GetParentPath(), edit.currentPath.GetNameToken(), detail);
            }
        } else if (edit.newPath.IsRelationalAttributePath()) {
            if (SdfAttributeSpecHandle attr = layer->GetAttributeAtPath(edit.currentPath)) {
                // Move a prim or relational attribute to be a relational
                // attribute.
                return Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::CanMoveChildForBatchNamespaceEdit(
                        layer, edit.newPath.GetParentPath(), attr, edit.newPath.GetNameToken(), edit.index, detail);
            } else {
            ...
            // property
            return Sdf_ChildrenUtils<Sdf_PropertyChildPolicy>::CanMoveChildForBatchNamespaceEdit(
                    layer, edit.newPath.GetParentPath(), layer->GetPropertyAtPath(edit.currentPath),
                    edit.newPath.GetNameToken(), edit.index, detail);
        }
    }
}

static void _DoEdit(const SdfLayerHandle& layer, const SdfNamespaceEdit& edit) {
    if (edit.currentPath.IsPrimPath()) {
        if (edit.newPath.IsEmpty()) {
            // Remove prim child.
            Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::RemoveChildForBatchNamespaceEdit(
                    layer, edit.currentPath.GetParentPath(), edit.currentPath.GetNameToken());
        } else {
            // Insert prim child.
            Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::MoveChildForBatchNamespaceEdit(
                    layer, edit.newPath.GetParentPath(), layer->GetPrimAtPath(edit.currentPath),
                    edit.newPath.GetNameToken(), edit.index);
        }
    } else {
        if (edit.newPath.IsEmpty()) {
            if (edit.currentPath.IsRelationalAttributePath()) {
                // Remove relational attribute.
                Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::RemoveChildForBatchNamespaceEdit(
                        layer, edit.currentPath.GetParentPath(), edit.currentPath.GetNameToken());
            } else {
                // Remove prim property.
                Sdf_ChildrenUtils<Sdf_PropertyChildPolicy>::RemoveChildForBatchNamespaceEdit(
                        layer, edit.currentPath.GetParentPath(), edit.currentPath.GetNameToken());
            }
        } else {
            if (edit.newPath.IsRelationalAttributePath()) {
                // Move a prim or relational attribute to be a relational
                // attribute.
                Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::MoveChildForBatchNamespaceEdit(
                        layer, edit.newPath.GetParentPath(), layer->GetAttributeAtPath(edit.currentPath),
                        edit.newPath.GetNameToken(), edit.index);
            } else {
                // Move a prim property or relational attribute to be a prim
                // property
                Sdf_ChildrenUtils<Sdf_PropertyChildPolicy>::MoveChildForBatchNamespaceEdit(
                        layer, edit.newPath.GetParentPath(), layer->GetPropertyAtPath(edit.currentPath),
                        edit.newPath.GetNameToken(), edit.index);
            }
        }
...
SdfNamespaceEditDetail::Result SdfLayer::CanApply(const SdfBatchNamespaceEdit& edits,
    ...
    if (!edits.Process(NULL, std::bind(&_HasObjectAtPath, _self, ph::_1), std::bind(&_CanEdit, _self, ph::_1, ph::_2),
...
bool SdfLayer::Apply(const SdfBatchNamespaceEdit& edits) {
    if (!PermissionToEdit()) {
        return false;
    }
    ...
    if (!edits.Process(&final, std::bind(&_HasObjectAtPath, _self, ph::_1), std::bind(&_CanEdit, _self, ph::_1, ph::_2),
...
void SdfLayer::ScheduleRemoveIfInert(const SdfSpec& spec) {
    Sdf_ChangeManager::Get().RemoveSpecIfInert(spec);
}

void SdfLayer::_RemoveIfInert(const SdfSpec& spec) {
    if (!spec.IsDormant()) {
        SdfSpecHandle specHandle(spec);
        if (SdfPrimSpecHandle prim = TfDynamic_cast<SdfPrimSpecHandle>(specHandle)) {
        ...
        } else if (SdfPropertySpecHandle property = TfDynamic_cast<SdfPropertySpecHandle>(specHandle)) {
            RemovePropertyIfHasOnlyRequiredFields(property);
        }
    }
}

void SdfLayer::RemovePrimIfInert(SdfPrimSpecHandle prim) {
    if (prim && _RemoveInertDFS(prim)) _RemoveInertToRootmost(prim);
}

void SdfLayer::RemovePropertyIfHasOnlyRequiredFields(SdfPropertySpecHandle prop) {
    if (!(prop && prop->HasOnlyRequiredFields())) return;

    if (SdfPrimSpecHandle owner = TfDynamic_cast<SdfPrimSpecHandle>(prop->GetOwner())) {
        owner->RemoveProperty(prop);
        _RemoveInertToRootmost(owner);

    } else if (SdfAttributeSpecHandle attr = TfDynamic_cast<SdfAttributeSpecHandle>(prop)) {
        Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::RemoveChild(_self, attr->GetPath().GetParentPath(),
                                                                 attr->GetNameToken());
    } else if (SdfRelationshipSpecHandle rel = TfDynamic_cast<SdfRelationshipSpecHandle>(prop)) {
        Sdf_ChildrenUtils<Sdf_RelationshipChildPolicy>::RemoveChild(_self, rel->GetPath().GetParentPath(),
                                                                    rel->GetNameToken());
    }
...
void SdfLayer::RemoveInertSceneDescription() {
    SdfChangeBlock block;

    _RemoveInertDFS(GetPseudoRoot());
}

bool SdfLayer::_RemoveInertDFS(SdfPrimSpecHandle prim) {
    ...
    if (!inert) {
        ...
        TF_FOR_ALL(it, prim->GetNameChildren()) {
            ...
            if (_RemoveInertDFS(child) && !SdfIsDefiningSpecifier(child->GetSpecifier()))
...
void SdfLayer::_RemoveInertToRootmost(SdfPrimSpecHandle prim) {
    while (prim && !SdfIsDefiningSpecifier(prim->GetSpecifier()) && prim->IsInert()) {
...
bool SdfLayer::SplitIdentifier(const string& identifier, string* layerPath, FileFormatArguments* arguments) {
    return Sdf_SplitIdentifier(identifier, layerPath, arguments);
}

std::string SdfLayer::CreateIdentifier(const string& layerPath, const FileFormatArguments& arguments) {
    return Sdf_CreateIdentifier(layerPath, arguments);
}

const string& SdfLayer::GetIdentifier() const {
    return _assetInfo->identifier;
}

void SdfLayer::SetIdentifier(const string& identifier) {
    TRACE_FUNCTION();
    ...
    const string absIdentifier = Sdf_CreateIdentifier(newLayerPath, newArguments);
    const ArResolvedPath oldResolvedPath = GetResolvedPath();
...

    {
        ...
        SdfLayerRefPtr existingLayer = _Find(absIdentifier, FileFormatArguments(), lock, retryAsWriter);
        if (existingLayer) {
    ...
    if (oldResolvedPath != newResolvedPath) {
        const ArTimestamp timestamp = ArGetResolver().GetModificationTimestamp(newLayerPath, newResolvedPath);
        _assetModificationTime =
                (timestamp.IsValid() || Sdf_ResolvePath(newLayerPath)) ? VtValue(timestamp) : VtValue();
    }
}

void SdfLayer::UpdateAssetInfo() {
    TRACE_FUNCTION();
    ...
    SdfChangeBlock block;
    {
        ...
        if (!GetAssetName().empty()) {
            binder.reset(new ArResolverContextBinder(_assetInfo->resolverContext));
        }
...
string SdfLayer::GetDisplayName() const {
    return GetDisplayNameFromIdentifier(GetIdentifier());
}

const ArResolvedPath& SdfLayer::GetResolvedPath() const {
    return _assetInfo->resolvedPath;
}

const string& SdfLayer::GetRealPath() const {
    return _assetInfo->resolvedPath.GetPathString();
}

string SdfLayer::GetFileExtension() const {
    string ext = Sdf_GetExtension(GetRealPath());

    if (ext.empty()) ext = GetFileFormat()->GetPrimaryFileExtension();

    return ext;
}

const string& SdfLayer::GetRepositoryPath() const {
    return _assetInfo->assetInfo.repoPath;
}

const string& SdfLayer::GetVersion() const {
    return _assetInfo->assetInfo.version;
}

const VtValue& SdfLayer::GetAssetInfo() const {
    return _assetInfo->assetInfo.resolverInfo;
}

const string& SdfLayer::GetAssetName() const {
    return _assetInfo->assetInfo.assetName;
}

SdfLayerHints SdfLayer::GetHints() const {
...
SdfDataRefPtr SdfLayer::GetMetadata() const {
...
string SdfLayer::ComputeAbsolutePath(const string& assetPath) const {
    if (assetPath.empty() || Sdf_IsAnonLayerIdentifier(assetPath)) {
        return assetPath;
    }

    return SdfComputeAssetPathRelativeToLayer(SdfCreateNonConstHandle(this), assetPath);
}

SdfLayer::DetachedLayerRules& SdfLayer::DetachedLayerRules::Include(const std::vector<std::string>& patterns) {
...
SdfLayer::DetachedLayerRules& SdfLayer::DetachedLayerRules::Exclude(const std::vector<std::string>& patterns) {
    _exclude.insert(_exclude.end(), patterns.begin(), patterns.end());

    std::sort(_exclude.begin(), _exclude.end());
    _exclude.erase(std::unique(_exclude.begin(), _exclude.end()), _exclude.end());

    return *this;
}

bool SdfLayer::DetachedLayerRules::IsIncluded(const std::string& identifier) const {
    ...
    const bool included =
            _includeAll || std::find_if(_include.begin(), _include.end(), [&layerPath](const std::string& s) {
    ...
    const bool excluded = std::find_if(_exclude.begin(), _exclude.end(), [&layerPath](const std::string& s) {
                              return TfStringContains(layerPath, s);
                          }) != _exclude.end();
...
void SdfLayer::SetDetachedLayerRules(const DetachedLayerRules& rules) {
    ...
    *_detachedLayerRules = rules;
    ...
    for (const SdfLayerHandle& layer : GetLoadedLayers()) {
        const bool wasIncludedBefore = oldRules.IsIncluded(layer->GetIdentifier());
        const bool isIncludedNow = rules.IsIncluded(layer->GetIdentifier());
...
const SdfLayer::DetachedLayerRules& SdfLayer::GetDetachedLayerRules() {
    return *_detachedLayerRules;
}

bool SdfLayer::IsIncludedByDetachedLayerRules(const std::string& identifier) {
    return _detachedLayerRules->IsIncluded(identifier);
}

string SdfLayer::_GetMutedPath() const {
    return GetRepositoryPath().empty() ? GetIdentifier() : GetRepositoryPath();
}

set<string> SdfLayer::GetMutedLayers() {
    std::lock_guard<std::mutex> lock(*_mutedLayersMutex);
    return *_mutedLayers;
}

void SdfLayer::SetMuted(bool muted) {
    // XXX Racy...
...
/*static*/
bool SdfLayer::IsMuted(const string& path) {
    std::lock_guard<std::mutex> lock(*_mutedLayersMutex);
    return _mutedLayers->count(path);
}

/*static*/
void SdfLayer::AddToMutedLayers(const string& path) {
        ...
        if (SdfLayerHandle layer = Find(path)) {
            if (layer->IsDirty()) {
                ...
                if (layer->_data->StreamsData()) {
                    ...
                    {
                        std::lock_guard<std::mutex> lock(*_mutedLayersMutex);
                        TF_VERIFY((*_mutedLayerData).find(path) == (*_mutedLayerData).end());
...
/*static*/
void SdfLayer::RemoveFromMutedLayers(const string& path) {
    bool didChange = false;
    {
        // Racy...
        std::lock_guard<std::mutex> lock(*_mutedLayersMutex);
    ...
    if (didChange) {
        if (SdfLayerHandle layer = Find(path)) {
            if (layer->IsDirty()) {
                ...
                {
                    std::lock_guard<std::mutex> lock(*_mutedLayersMutex);
                    _MutedLayerDataMap::iterator i = _mutedLayerData->find(path);
...
bool SdfLayer::_ShouldNotify() const {
...
bool SdfLayer::_UpdateLastDirtinessState() const {
    // Did not change since last call...
    if (IsDirty() == _lastDirtyState) return false;
...
SdfLayerStateDelegateBasePtr SdfLayer::GetStateDelegate() const {
    return _stateDelegate;
}

void SdfLayer::SetStateDelegate(const SdfLayerStateDelegateBaseRefPtr& delegate) {
...
bool SdfLayer::IsEmpty() const {
    ...
    return GetRootPrims().empty() && GetRootPrimOrder().empty() && GetSubLayerPaths().empty() && GetRelocates().empty();
}

bool SdfLayer::StreamsData() const {
    return _GetData()->StreamsData();
}

bool SdfLayer::IsDetached() const {
    return _GetData()->IsDetached();
}

void SdfLayer::TransferContent(const SdfLayerHandle& layer) {
    if (!PermissionToEdit()) {
        TF_RUNTIME_ERROR("TransferContent of '%s': Permission denied.", GetDisplayName().c_str());
        return;
    }
...
static void _GatherPrimCompositionDependencies(const SdfPrimSpecHandle& prim, set<string>* assetReferences) {
    if (prim != prim->GetLayer()->GetPseudoRoot()) {
        // Prim references
        for (const SdfReference& ref : prim->GetReferenceList().GetAddedOrExplicitItems()) {
            assetReferences->insert(ref.GetAssetPath());
        }

        // Prim payloads
        for (const SdfPayload& payload : prim->GetPayloadList().GetAddedOrExplicitItems()) {
        ...
        for (const auto& varSetIt : variantSetMap) {
            ...
            for (const SdfVariantSpecHandle& varSpec : variants) {
                _GatherPrimCompositionDependencies(varSpec->GetPrimSpec(), assetReferences);
            }
        }
    }
...
set<string> SdfLayer::GetExternalReferences() const {
    return GetCompositionAssetDependencies();
}

bool SdfLayer::UpdateExternalReference(const string& oldLayerPath, const string& newLayerPath) {
    return UpdateCompositionAssetDependency(oldLayerPath, newLayerPath);
}

set<string> SdfLayer::GetCompositionAssetDependencies() const {
...
bool SdfLayer::UpdateCompositionAssetDependency(const string& oldLayerPath, const string& newLayerPath) {
    if (oldLayerPath.empty()) return false;
    ...
    _UpdatePrimCompositionDependencyPaths(GetPseudoRoot(), oldLayerPath, newLayerPath);

    return true;
}

std::set<std::string> SdfLayer::GetExternalAssetDependencies() const {
...
static std::optional<RefOrPayloadType> _UpdateRefOrPayloadPath(const string& oldLayerPath,
...
void SdfLayer::_UpdatePrimCompositionDependencyPaths(const SdfPrimSpecHandle& prim,
                                                     const string& oldLayerPath,
                                                     const string& newLayerPath) {
    TF_AXIOM(!oldLayerPath.empty());

    // Prim references
    prim->GetReferenceList().ModifyItemEdits(
            std::bind(&_UpdateRefOrPayloadPath<SdfReference>, oldLayerPath, newLayerPath, ph::_1));

    // Prim payloads
    prim->GetPayloadList().ModifyItemEdits(
            std::bind(&_UpdateRefOrPayloadPath<SdfPayload>, oldLayerPath, newLayerPath, ph::_1));
    ...
    for (const auto& setNameAndSpec : variantSetMap) {
        ...
        for (const auto& variantSpec : variants) {
            _UpdatePrimCompositionDependencyPaths(variantSpec->GetPrimSpec(), oldLayerPath, newLayerPath);
        }
    }

    // Recurse on nameChildren
    for (const auto& primSpec : prim->GetNameChildren()) {
        _UpdatePrimCompositionDependencyPaths(primSpec, oldLayerPath, newLayerPath);
    }
}

/*static*/
void SdfLayer::DumpLayerInfo() {
    TF_PY_ALLOW_THREADS_IN_SCOPE();
    tbb::queuing_rw_mutex::scoped_lock lock(_GetLayerRegistryMutex(), /*write=*/false);
    std::cerr << "Layer Registry Dump:" << std::endl << *_layerRegistry << std::endl;
}

bool SdfLayer::WriteDataFile(const string& filename) {
...
/*static*/
set<SdfLayerHandle> SdfLayer::GetLoadedLayers() {
    TF_PY_ALLOW_THREADS_IN_SCOPE();
    tbb::queuing_rw_mutex::scoped_lock lock(_GetLayerRegistryMutex(), /*write=*/false);
...
/* static */
template <class Lock>
SdfLayerRefPtr SdfLayer::_OpenLayerAndUnlockRegistry(Lock& lock, const _FindOrOpenLayerInfo& info, bool metadataOnly) {
    TfAutoMallocTag2 tag("Sdf", "SdfLayer::_OpenLayerAndUnlockRegistry " + info.identifier);
    ...
    TF_DEBUG(SDF_LAYER).Msg(
            ...
            info.fileFormat ? info.fileFormat->GetFormatId().GetText() : "unknown file format",
            TfStringify(info.fileFormatArgs).c_str(), metadataOnly ? "True" : "False");

    // XXX: Is this really a coding error? SdfLayer avoids issuing errors if
    //      given a non-existent file, for instance. Should we be following the
    //      same policy here?
    if (!info.fileFormat) {
        TF_CODING_ERROR("Cannot determine file format for @%s@", info.identifier.c_str());
        lock.release();
        return TfNullPtr;
    }

    // Create a new layer of the appropriate format.
    SdfLayerRefPtr layer = _CreateNewWithFormat(info.fileFormat, info.identifier, info.resolvedLayerPath,
    ...
    // other layers.
    TF_VERIFY(_layerRegistry->Find(layer->GetIdentifier(), layer->GetResolvedPath()) == layer, "Could not find %s",
    ...
    if (info.isAnonymous != layer->IsAnonymous()) {
        if (info.isAnonymous) {
            TF_CODING_ERROR(
                    "Opened anonymous layer ('%s' with format id '%s') "
                    "but resulting layer is not anonymous.",
                    info.identifier.c_str(), info.fileFormat->GetFormatId().GetText());
        } else {
            TF_CODING_ERROR(
                    "Opened layer without anonymous prefix "
    ...
    if (!layer->IsMuted()) {
        ...
        readSuccess = layer->_Read(info.identifier, info.resolvedLayerPath, metadataOnly);
        //     });
    ...
    if (!info.isAnonymous) {
        ...
        VtValue timestamp(ArGetResolver().GetModificationTimestamp(info.layerPath, info.resolvedLayerPath));
    ...
    layer->_externalAssetModificationTimes = Sdf_ComputeExternalAssetModificationTimestamps(*layer);
...
bool SdfLayer::HasSpec(const SdfPath& path) const {
    return _data->HasSpec(path);
}

SdfSpecType SdfLayer::GetSpecType(const SdfPath& path) const {
    return _data->GetSpecType(path);
}

vector<TfToken> SdfLayer::ListFields(const SdfPath& path) const {
    return _ListFields(_schema, *get_pointer(_data), path);
}

vector<TfToken> SdfLayer::_ListFields(SdfSchemaBase const& schema, SdfAbstractData const& data, const SdfPath& path) {
    ...
    for (size_t reqIdx = 0, reqSz = req.size(); reqIdx != reqSz; ++reqIdx) {
        ...
        if (iter == dataListEnd) {
            ...
            if (mightAlloc && dataList.size() == dataList.capacity()) {
                ...
                dataListEnd = dataList.data() + std::distance(dataListBegin, dataListEnd);
...
SdfSchema::FieldDefinition const* SdfLayer::_GetRequiredFieldDef(const SdfPath& path,
    ...
    if (ARCH_UNLIKELY(schema.IsRequiredFieldName(fieldName))) {
        ...
        if (SdfSchema::SpecDefinition const* specDef = schema.GetSpecDefinition(specType)) {
...
SdfSchema::FieldDefinition const* SdfLayer::_GetRequiredFieldDef(const SdfSchemaBase& schema,
                                                                 const TfToken& fieldName,
                                                                 SdfSpecType specType) {
    if (ARCH_UNLIKELY(schema.IsRequiredFieldName(fieldName))) {
        if (SdfSchema::SpecDefinition const* specDef = schema.GetSpecDefinition(specType)) {
...
bool SdfLayer::_HasField(const SdfSchemaBase& schema,
    ...
    if (SdfSchema::FieldDefinition const* def = _GetRequiredFieldDef(schema, fieldName, specType)) {
        if (value) *value = def->GetFallbackValue();
        return true;
    }
    return false;
}

bool SdfLayer::HasField(const SdfPath& path, const TfToken& fieldName, VtValue* value) const {
    SdfSpecType specType;
    ...
    // Otherwise if this is a required field, and the data has a spec here,
    // return the fallback value.
    if (SdfSchema::FieldDefinition const* def = _GetRequiredFieldDef(path, fieldName, specType)) {
        if (value) *value = def->GetFallbackValue();
        return true;
    }
    return false;
...
bool SdfLayer::HasField(const SdfPath& path, const TfToken& fieldName, SdfAbstractDataValue* value) const {
    SdfSpecType specType;
    if (_data->HasSpecAndField(path, fieldName, value, &specType)) {
        return true;
    }
    ...
    // return the fallback value.
    if (SdfSchema::FieldDefinition const* def = _GetRequiredFieldDef(path, fieldName, specType)) {
        if (value) return value->StoreValue(def->GetFallbackValue());
        return true;
    }
    return false;
}

bool SdfLayer::HasFieldDictKey(const SdfPath& path,
                               const TfToken& fieldName,
                               const TfToken& keyPath,
                               VtValue* value) const {
    if (_data->HasDictKey(path, fieldName, keyPath, value)) return true;
    // Otherwise if this is a required field, and the data has a spec here,
    // return the fallback value.
    if (SdfSchema::FieldDefinition const* def = _GetRequiredFieldDef(path, fieldName)) {
...
bool SdfLayer::HasFieldDictKey(const SdfPath& path,
    ...
    if (SdfSchema::FieldDefinition const* def = _GetRequiredFieldDef(path, fieldName)) {
        ...
        if (fallback.IsHolding<VtDictionary>()) {
            VtDictionary const& dict = fallback.UncheckedGet<VtDictionary>();
            if (VtValue const* v = dict.GetValueAtPath(keyPath)) {
                if (value) return value->StoreValue(*v);
                return true;
            }
        }
    }
...
VtValue SdfLayer::GetField(const SdfPath& path, const TfToken& fieldName) const {
    VtValue result;
    HasField(path, fieldName, &result);
    return result;
}

VtValue SdfLayer::_GetField(const SdfSchemaBase& schema,
                            const SdfAbstractData& data,
                            const SdfPath& path,
                            const TfToken& fieldName) {
    VtValue result;
...
VtValue SdfLayer::GetFieldDictValueByKey(const SdfPath& path, const TfToken& fieldName, const TfToken& keyPath) const {
    VtValue result;
    HasFieldDictKey(path, fieldName, keyPath, &result);
    return result;
}

static bool _IsValidFieldForLayer(const SdfLayer& layer, const SdfPath& path, const TfToken& fieldName) {
    return layer.GetSchema().IsValidFieldForSpec(fieldName, layer.GetSpecType(path));
}

void SdfLayer::SetField(const SdfPath& path, const TfToken& fieldName, const VtValue& value) {
    if (value.IsEmpty()) return EraseField(path, fieldName);

    if (ARCH_UNLIKELY(!PermissionToEdit())) {
        TF_CODING_ERROR("Cannot set %s on <%s>. Layer @%s@ is not editable.", fieldName.GetText(), path.GetText(),
                        GetIdentifier().c_str());
        return;
    }

    if (ARCH_UNLIKELY(_validateAuthoring) && !_IsValidFieldForLayer(*this, path, fieldName)) {
        TF_ERROR(SdfAuthoringErrorUnrecognizedFields, "Cannot set %s on <%s>. Field is not valid for layer @%s@.",
                 fieldName.GetText(), path.GetText(), GetIdentifier().c_str());
        return;
    }

    VtValue oldValue = GetField(path, fieldName);
    if (value != oldValue) _PrimSetField(path, fieldName, value, &oldValue);
}

void SdfLayer::SetField(const SdfPath& path, const TfToken& fieldName, const SdfAbstractDataConstValue& value) {
    if (value.IsEqual(VtValue())) return EraseField(path, fieldName);
    ...
    VtValue oldValue = GetField(path, fieldName);
    if (!value.IsEqual(oldValue)) _PrimSetField(path, fieldName, value, &oldValue);
}

void SdfLayer::SetFieldDictValueByKey(const SdfPath& path,
                                      const TfToken& fieldName,
                                      const TfToken& keyPath,
                                      const VtValue& value) {
    if (!PermissionToEdit()) {
        TF_CODING_ERROR("Cannot set %s:%s on <%s>. Layer @%s@ is not editable.", fieldName.GetText(), keyPath.GetText(),
                        path.GetText(), GetIdentifier().c_str());
        return;
    }

    if (ARCH_UNLIKELY(_validateAuthoring) && !_IsValidFieldForLayer(*this, path, fieldName)) {
        TF_ERROR(SdfAuthoringErrorUnrecognizedFields, "Cannot set %s:%s on <%s>. Field is not valid for layer @%s@.",
                 fieldName.GetText(), keyPath.GetText(), path.GetText(), GetIdentifier().c_str());
        return;
    }
...
void SdfLayer::EraseField(const SdfPath& path, const TfToken& fieldName) {
    if (ARCH_UNLIKELY(!PermissionToEdit())) {
        TF_CODING_ERROR("Cannot erase %s on <%s>. Layer @%s@ is not editable.", fieldName.GetText(), path.GetText(),
                        GetIdentifier().c_str());
        return;
    }
    ...
    // value.
    if (SdfSchema::FieldDefinition const* def = _GetRequiredFieldDef(path, fieldName)) {
        if (GetField(path, fieldName) == def->GetFallbackValue()) return;
    }
...
void SdfLayer::EraseFieldDictValueByKey(const SdfPath& path, const TfToken& fieldName, const TfToken& keyPath) {
    if (!PermissionToEdit()) {
        TF_CODING_ERROR("Cannot erase %s:%s on <%s>. Layer @%s@ is not editable.", fieldName.GetText(),
                        keyPath.GetText(), path.GetText(), GetIdentifier().c_str());
        return;
    }

    if (!_data->HasDictKey(path, fieldName, keyPath, static_cast<VtValue*>(NULL))) {
        return;
    }
...
SdfAbstractDataConstPtr SdfLayer::_GetData() const {
    return _data;
}

SdfAbstractDataRefPtr SdfLayer::_CreateData() const {
    return _CreateDataForFileFormat(GetFileFormat(), GetIdentifier(), GetFileFormatArguments());
}

void SdfLayer::_SwapData(SdfAbstractDataRefPtr& data) {
    _data.swap(data);
}

void SdfLayer::_AdoptData(const SdfAbstractDataRefPtr& newData) {
    SdfChangeBlock block;
    _data = newData;
    Sdf_ChangeManager::Get().DidReplaceLayerContent(_self);
}

void SdfLayer::_SetData(const SdfAbstractDataPtr& newData, const SdfSchemaBase* newDataSchema) {
    TRACE_FUNCTION();
    ...
    const auto deleteSpecFunc = [this](auto&&... args) {
        _PrimDeleteSpec(std::forward<decltype(args)>(args)...);
    };

    const auto createSpecFunc = [this](auto&&... args) {
        _PrimCreateSpec(std::forward<decltype(args)>(args)...);
    };

    const auto setFieldFunc = [this](auto&&... args) {
        _PrimSetField(std::forward<decltype(args)>(args)...);
    };

    const auto errorFunc = [this](SdfAuthoringError error, const std::vector<std::string>& descrs) {
        const char* errorType = error == SdfAuthoringErrorUnrecognizedSpecType ? "spec types" : "fields";

        TF_ERROR(error,
                 "Omitted unrecognized %s setting data on "
                 "@%s@: %s",
                 errorType, GetIdentifier().c_str(), TfStringJoin(descrs, "; ").c_str());
    };

    _ProcessIncomingData(newData, newDataSchema, /*processPropertyFields*/ true, deleteSpecFunc, createSpecFunc,
                         setFieldFunc, errorFunc);
...
SdfChangeList SdfLayer::CreateDiff(const SdfLayerHandle& layer, bool processPropertyFields) const {
    TRACE_FUNCTION();
    ...
    const auto deleteSpecFunc = [this](const SdfPath& path, bool inert) {
        Sdf_ChangeManager::Get().DidRemoveSpec(_self, path, inert);
    };

    const auto createSpecFunc = [this](const SdfPath& path, SdfSpecType specType, bool inert) {
        Sdf_ChangeManager::Get().DidAddSpec(_self, path, inert);
    };

    const auto setFieldFunc = [this](const SdfPath& path, const TfToken& fieldName, const VtValue& value,
                                     VtValue* oldValuePtr) {
        VtValue oldValue = oldValuePtr ? std::move(*oldValuePtr) : GetField(path, fieldName);

        Sdf_ChangeManager::Get().DidChangeField(_self, path, fieldName, std::move(oldValue), value);
    };

    const auto errorFunc = [](auto&&... args) {
        // Errors can be ignored because we are not doing any authoring.
    };

    _ProcessIncomingData(layer->_data, &layer->GetSchema(), processPropertyFields, deleteSpecFunc, createSpecFunc,
                         setFieldFunc, errorFunc);

    return Sdf_ChangeManager::Get().ExtractLocalChanges(_self);
}

template <typename DeleteSpecFunc, typename CreateSpecFunc, typename SetFieldFunc, typename ErrorFunc>
void SdfLayer::_ProcessIncomingData(const SdfAbstractDataPtr& newData,
    ...
    // Remove specs that no longer exist or whose required fields changed.
    {
        ...
        struct _SpecsToDelete : public SdfAbstractDataSpecVisitor {
            _SpecsToDelete(const SdfAbstractDataPtr& newData_) : newData(newData_) {}

            virtual bool VisitSpec(const SdfAbstractData& oldData, const SdfPath& path) {
                if (!newData->HasSpec(path) || (newData->GetSpecType(path) != oldData.GetSpecType(path))) {
                    paths.insert(path);
                }
                return true;
            }
        ...
        TF_REVERSE_FOR_ALL(i, specsToDelete.paths) {
            ...
            const bool processFields = path.IsPropertyPath() ? processPropertyFields : true;

            if (processFields) {
                std::vector<TfToken> fields = ListFields(path);

                SdfSpecType specType = _data->GetSpecType(path);
                const SdfSchema::SpecDefinition* specDefinition = GetSchema().GetSpecDefinition(specType);

                TF_FOR_ALL(field, fields) {
                    if (!specDefinition->IsRequiredField(*field)) {
                        setFieldFunc(path, *field, VtValue(), /* oldValue */ nullptr);
                    }
                }
            }
            ...
            // inertness
            deleteSpecFunc(path,
                           _IsInert(path, true /*ignoreChildren*/, true /* requiredFieldOnlyPropertiesAreInert */));
        }
    }
    ...
    // Create new specs.
    {
        ...
        struct _SpecsToCreate : public SdfAbstractDataSpecVisitor {
            _SpecsToCreate(const SdfAbstractData& oldData_) : oldData(oldData_) {}

            virtual bool VisitSpec(const SdfAbstractData& newData, const SdfPath& path) {
        ...
        TF_FOR_ALL(i, specsToCreate.paths) {
            ...
            if (path.IsPrimPath()) {
                ...
                // for these fields.
                inert = (newData->GetAs<SdfSpecifier>(path, SdfFieldKeys->Specifier, SdfSpecifierOver) ==
                         SdfSpecifierOver) &&
                        (newData->GetAs<TfToken>(path, SdfFieldKeys->TypeName, TfToken()).IsEmpty());
            } else if (path.IsPropertyPath()) {
                // Properties are considered inert if they are custom.
                inert = !newData->GetAs<bool>(path, SdfFieldKeys->Custom, false);
            }
        ...
        if (differentSchema) {
            ...
            for (int i = 0; i != SdfSpecTypeUnknown; ++i) {
                ...
                specDescrs.push_back(TfStringPrintf("'%s' first seen at <%s>",
    ...
    // Update spec fields.
    {
        struct _SpecUpdater : public SdfAbstractDataSpecVisitor {
            ...
            virtual bool VisitSpec(const SdfAbstractData& newData, const SdfPath& path) {
                ...
                const TfTokenVector newFields = _ListFields(newDataSchema, newData, path);
                ...
                for (TfToken const& field : oldFields) {
                    ...
                    if (std::find(newFields.begin(), newFields.end(), field) == newFields.end()) {
                        setFieldFunc(path, field, VtValue(), /* oldValue */ nullptr);
                    }
                }
                ...
                for (TfToken const& field : newFields) {
                    ...
                    if (oldValue != newValue) {
                        if (differentSchema && oldValue.IsEmpty() &&
                            !thisLayerSchema.IsValidFieldForSpec(field, layer->GetSpecType(path))) {
                            ...
                            setFieldFunc(path, field, newValue, &oldValue);
                        }
                    }
                }
        ...
        // this layer's schema.
        _SpecUpdater updater(this, newDataSchema ? *newDataSchema : GetSchema(), processPropertyFields, setFieldFunc);
        ...
        if (!updater.unrecognizedFields.empty()) {
            ...
            for (auto const& tokenPath : updater.unrecognizedFields) {
                fieldDescrs.push_back(TfStringPrintf("'%s' first seen at <%s>", tokenPath.first.GetText(),
...
template <class T>
void SdfLayer::_PrimSetField(
        const SdfPath& path, const TfToken& fieldName, const T& value, VtValue* oldValuePtr, bool useDelegate) {
    if (useDelegate && TF_VERIFY(_stateDelegate)) {
    ...
    Sdf_ChangeManager::Get().DidChangeField(_self, path, fieldName, std::move(oldValue), newValue);

    _data->Set(path, fieldName, value);
}

template void SdfLayer::_PrimSetField(const SdfPath&, const TfToken&, const VtValue&, VtValue*, bool);
template void SdfLayer::_PrimSetField(const SdfPath&, const TfToken&, const SdfAbstractDataConstValue&, VtValue*, bool);

template <class T>
void SdfLayer::_PrimPushChild(const SdfPath& parentPath, const TfToken& fieldName, const T& value, bool useDelegate) {
    if (!HasField(parentPath, fieldName)) {
        _PrimSetField(parentPath, fieldName, VtValue(std::vector<T>(1, value)));
        return;
    }
...
template void SdfLayer::_PrimPushChild(const SdfPath&, const TfToken&, const TfToken&, bool);
template void SdfLayer::_PrimPushChild(const SdfPath&, const TfToken&, const SdfPath&, bool);

template <class T>
void SdfLayer::_PrimPopChild(const SdfPath& parentPath, const TfToken& fieldName, bool useDelegate) {
    if (useDelegate && TF_VERIFY(_stateDelegate)) {
...
template void SdfLayer::_PrimPopChild<TfToken>(const SdfPath&, const TfToken&, bool);
template void SdfLayer::_PrimPopChild<SdfPath>(const SdfPath&, const TfToken&, bool);

template <class T>
void SdfLayer::_PrimSetFieldDictValueByKey(const SdfPath& path,
                                           const TfToken& fieldName,
                                           const TfToken& keyPath,
                                           const T& value,
                                           VtValue* oldValuePtr,
                                           bool useDelegate) {
    if (useDelegate && TF_VERIFY(_stateDelegate)) {
        _stateDelegate->SetFieldDictValueByKey(path, fieldName, keyPath, value, oldValuePtr);
        return;
    }
...
template void SdfLayer::_PrimSetFieldDictValueByKey(
        const SdfPath&, const TfToken&, const TfToken&, const VtValue&, VtValue*, bool);
template void SdfLayer::_PrimSetFieldDictValueByKey(
        const SdfPath&, const TfToken&, const TfToken&, const SdfAbstractDataConstValue&, VtValue*, bool);

bool SdfLayer::_MoveSpec(const SdfPath& oldPath, const SdfPath& newPath) {
    TRACE_FUNCTION();

    if (!PermissionToEdit()) {
        TF_CODING_ERROR("Cannot move <%s> to <%s>. Layer @%s@ is not editable.", oldPath.GetText(), newPath.GetText(),
                        GetIdentifier().c_str());
        return false;
    }
...
static void _MoveSpecInternal(SdfAbstractDataRefPtr data,
                              Sdf_IdentityRegistry* idReg,
                              const SdfPath& oldSpecPath,
                              const SdfPath& oldRootPath,
                              const SdfPath& newRootPath) {
    const SdfPath newSpecPath = oldSpecPath.ReplacePrefix(oldRootPath, newRootPath, /* fixTargets = */ false);

    data->MoveSpec(oldSpecPath, newSpecPath);
    idReg->MoveIdentity(oldSpecPath, newSpecPath);
}

void SdfLayer::_PrimMoveSpec(const SdfPath& oldPath, const SdfPath& newPath, bool useDelegate) {
    if (useDelegate && TF_VERIFY(_stateDelegate)) {
    ...
    Traverse(oldPath, std::bind(_MoveSpecInternal, _data, &_idRegistry, ph::_1, oldPath, newPath));
}

static bool _IsValidSpecForLayer(const SdfLayer& layer, SdfSpecType specType) {
    const SdfSchemaBase::SpecDefinition* specDef = layer.GetSchema().GetSpecDefinition(specType);
    return static_cast<bool>(specDef);
}

bool SdfLayer::_CreateSpec(const SdfPath& path, SdfSpecType specType, bool inert) {
    if (specType == SdfSpecTypeUnknown) {
        return false;
    }

    if (!PermissionToEdit()) {
        TF_CODING_ERROR("Cannot create spec at <%s>. Layer @%s@ is not editable.", path.GetText(),
                        GetIdentifier().c_str());
        return false;
    }
    ...
    if (_validateAuthoring && !_IsValidSpecForLayer(*this, specType)) {
        TF_ERROR(SdfAuthoringErrorUnrecognizedSpecType,
                 ...
                 "for layer @%s@",
                 path.GetText(), TfEnum::GetName(specType).c_str(), GetIdentifier().c_str());
        return false;
    }

    if (_data->HasSpec(path)) {
        TF_CODING_ERROR("Cannot create spec <%s> because it already exists in @%s@", path.GetText(),
                        GetIdentifier().c_str());
        return false;
    }
...
bool SdfLayer::_DeleteSpec(const SdfPath& path) {
    if (!PermissionToEdit()) {
        TF_CODING_ERROR("Cannot delete <%s>. Layer @%s@ is not editable", path.GetText(), GetIdentifier().c_str());
        return false;
    }
    ...
    if (_IsInertSubtree(path)) {
        ...
        SdfChangeBlock block;
        Traverse(path, [this, &cm = Sdf_ChangeManager::Get()](const SdfPath& specPath) {
...
void SdfLayer::_TraverseChildren(const SdfPath& path, const TraversalFunction& func) {
    std::vector<typename ChildPolicy::FieldType> children =
            GetFieldAs<std::vector<typename ChildPolicy::FieldType>>(path, ChildPolicy::GetChildrenToken(path));
...
void SdfLayer::Traverse(const SdfPath& path, const TraversalFunction& func) {
    std::vector<TfToken> fields = ListFields(path);
    TF_FOR_ALL(i, fields) {
...
static void _EraseSpecAtPath(SdfAbstractData* data, const SdfPath& path) {
    data->EraseSpec(path);
}

void SdfLayer::_PrimDeleteSpec(const SdfPath& path, bool inert, bool useDelegate) {
    if (useDelegate && TF_VERIFY(_stateDelegate)) {
    ...
    TraversalFunction eraseFunc = std::bind(&_EraseSpecAtPath, get_pointer(_data), ph::_1);
    Traverse(path, eraseFunc);
}

void SdfLayer::_PrimCreateSpec(const SdfPath& path, SdfSpecType specType, bool inert, bool useDelegate) {
    if (useDelegate && TF_VERIFY(_stateDelegate)) {
...
bool SdfLayer::_IsInert(const SdfPath& path, bool ignoreChildren, bool requiredFieldOnlyPropertiesAreInert) const {
    ...
    if (specType == SdfSpecTypePrim) {
        const SdfSpecifier specifier = GetFieldAs<SdfSpecifier>(path, SdfFieldKeys->Specifier, SdfSpecifierOver);
    ...
    if (specType == SdfSpecTypePrim || specType == SdfSpecTypeVariant || specType == SdfSpecTypeVariantSet ||
        specType == SdfSpecTypeAttribute || specType == SdfSpecTypeRelationship) {
        const SdfSchema::SpecDefinition* specDefinition = GetSchema().GetSpecDefinition(specType);
        ...
        TF_FOR_ALL(field, fields) {
            ...
            if (ignoreChildren && ((specType == SdfSpecTypePrim && (*field == SdfChildrenKeys->PrimChildren ||
                                                                    *field == SdfChildrenKeys->PropertyChildren ||
                                                                    *field == SdfChildrenKeys->VariantSetChildren)) ||
                                   (specType == SdfSpecTypeVariantSet && *field == SdfChildrenKeys->VariantChildren))) {
                continue;
            }
...
bool SdfLayer::_IsInertSubtree(const SdfPath& path) const {
    ...
    if (path.IsPrimVariantSelectionPath() && path.GetVariantSelection().second.empty()) {
        ...
        if (HasField(path, SdfChildrenKeys->VariantChildren, &variants)) {
            for (const TfToken& variant : variants) {
                if (!_IsInertSubtree(parentPath.AppendVariantSelection(vsetName, variant.GetString()))) {
                    return false;
                }
            }
        }
        ...
        for (auto const& childrenField : {SdfChildrenKeys->PrimChildren, SdfChildrenKeys->VariantSetChildren}) {
...
bool SdfLayer::ExportToString(std::string* result) const {
    TRACE_FUNCTION();
...
bool SdfLayer::_WriteToFile(const string& newFileName,
    ...
    TF_DESCRIBE_SCOPE("%s layer @%s@", isSave ? "Saving" : "Exporting", GetIdentifier().c_str());

    if (isSave && !PermissionToSave()) {
        TF_RUNTIME_ERROR("Cannot save layer @%s@, saving not allowed", newFileName.c_str());
        return false;
    }
    ...
    if (Sdf_IsPackageOrPackagedLayer(fileFormat, newFileName)) {
        TF_CODING_ERROR(
                "Cannot %s layer @%s@: writing %s %s layer "
                "is not allowed through this API.",
                isSave ? "save" : "export", newFileName.c_str(), fileFormat->IsPackage() ? "package" : "packaged",
                fileFormat->GetFormatId().GetText());
        return false;
    }

    if (!TF_VERIFY(fileFormat)) {
        TF_RUNTIME_ERROR("Unknown file format when attempting to write '%s'", newFileName.c_str());
        return false;
    }

    if (!fileFormat->SupportsWriting()) {
        TF_CODING_ERROR(
                "Cannot %s layer @%s@: %s file format does not"
                "support writing",
                isSave ? "save" : "export", newFileName.c_str(), fileFormat->GetFormatId().GetText());
        return false;
    }
    ...
    if (differentSchema) {
        SdfLayerRefPtr tmpLayer = CreateAnonymous("cross-schema-write-test", fileFormat, args);
        TfErrorMark m;
        tmpLayer->TransferContent(SdfLayerHandle(const_cast<SdfLayer*>(this)));
        if (!m.IsClean()) {
            TF_RUNTIME_ERROR(
                    "Failed attempting to write '%s' under a "
    ...
    bool ok = isSave ? fileFormat->SaveToFile(*this, newFileName, comment, args)
...
bool SdfLayer::Export(const string& newFileName, const string& comment, const FileFormatArguments& args) const {
    return _WriteToFile(newFileName, comment,
                        ...
                        GetFileFormat()->IsSupportedExtension(newFileName) ? GetFileFormat() : TfNullPtr, args);
}

bool SdfLayer::Save(bool force) const {
    return _Save(force);
}

bool SdfLayer::_Save(bool force) const {
    TRACE_FUNCTION();

    if (IsMuted()) {
        TF_CODING_ERROR("Cannot save muted layer @%s@", GetIdentifier().c_str());
        return false;
    }

    if (IsAnonymous()) {
        TF_CODING_ERROR("Cannot save anonymous layer @%s@", GetIdentifier().c_str());
        return false;
    }
    ...
    if (!force && !IsDirty() && TfPathExists(path)) return true;

    if (!_WriteToFile(path, std::string(), GetFileFormat(), GetFileFormatArguments())) return false;
