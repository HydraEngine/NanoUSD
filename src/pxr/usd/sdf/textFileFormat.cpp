...
SdfTextFileFormat::SdfTextFileFormat(const TfToken& formatId, const TfToken& versionString, const TfToken& target)
...
bool SdfTextFileFormat::_ReadFromAsset(SdfLayer* layer,
    ...
    if (!_CanReadImpl(asset, GetFileCookie())) {
        TF_RUNTIME_ERROR("<%s> is not a valid %s layer", resolvedPath.c_str(), GetFormatId().GetText());
        return false;
    }
    ...
    if (fileSizeWarning > 0 && asset->GetSize() > (fileSizeWarning * toMB)) {
        TF_WARN("Performance warning: reading %lu MB text-based layer <%s>.", asset->GetSize() / toMB,
                resolvedPath.c_str());
    }

    SdfLayerHints hints;
    SdfAbstractDataRefPtr data = InitData(layer->GetFileFormatArguments());
    if (!Sdf_ParseLayer(resolvedPath, asset, GetFormatId(), GetVersionString(), metadataOnly,
...
static bool _WriteLayer(const SdfLayer* l,
    ...
    for (TfTokenVector::const_iterator fieldIt = fields.begin(); fieldIt != metadataFieldsEnd; ++fieldIt) {
        ...
        if (field == SdfFieldKeys->Documentation) {
            ...
            for (size_t i = 0; i < c; i++) {
                ...
                _WriteLayerOffset(header, 0, false /* multiLine */, l->GetSubLayerOffset(static_cast<int>(i)));
    ...
    if (l->HasRelocates()) {
        Sdf_FileIOUtility::WriteRelocates(header, 1, true, l->GetRelocates());
    }
...
bool SdfTextFileFormat::WriteToFile(const SdfLayer& layer,
    ...
    std::shared_ptr<ArWritableAsset> asset =
            ArGetResolver().OpenAssetForWrite(ArResolvedPath(filePath), ArResolver::WriteMode::Replace);
    if (!asset) {
...
bool SdfTextFileFormat::ReadFromString(SdfLayer* layer, const std::string& str) const {
    ...
    if (!Sdf_ParseLayerFromString(trimmedStr, GetFormatId(), GetVersionString(), TfDynamic_cast<SdfDataRefPtr>(data),
                                  &hints)) {
        return false;
    }
...
bool SdfTextFileFormat::WriteToString(const SdfLayer& layer, std::string* str, const std::string& comment) const {
...
bool SdfTextFileFormat::WriteToStream(const SdfSpecHandle& spec, std::ostream& out, size_t indent) const {
