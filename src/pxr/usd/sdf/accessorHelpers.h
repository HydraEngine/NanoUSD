...
#define SDF_DEFINE_DICTIONARY_GET(name_, key_)                                             \
    SdfDictionaryProxy SDF_ACCESSOR_CLASS::name_() const {                                 \
        typedef Sdf_AccessorHelpers<SDF_ACCESSOR_CLASS> _Helper;                           \
...
template <typename T>
using Sdf_SetParameter = std::conditional<std::is_arithmetic<T>::value,
                                          std::add_const_t<T>,
...
#define SDF_DEFINE_GET_SET(name_, key_, type_) \
    SDF_DEFINE_TYPED_GET_SET(name_, key_, type_, Sdf_SetParameter<type_>::type)

#define SDF_DEFINE_GET_SET_HAS_CLEAR(name_, key_, type_) \
    SDF_DEFINE_TYPED_GET_SET_HAS_CLEAR(name_, key_, type_, Sdf_SetParameter<type_>::type)
...
template <class T, bool IsForSpec = std::is_base_of<SdfSpec, T>::value>
struct Sdf_AccessorHelpers;

template <class T>
struct Sdf_AccessorHelpers<T, true> {
    static const SdfSchemaBase& GetSchema(const T* spec) { return spec->GetSchema(); }

    static VtValue GetField(const T* spec, const TfToken& key) { return spec->GetField(key); }
    ...
    static bool HasField(const T* spec, const TfToken& key) { return spec->HasField(key); }

    static void ClearField(T* spec, const TfToken& key) { spec->ClearField(key); }

    static SdfSpecHandle GetSpecHandle(const T* spec) { return SdfCreateNonConstHandle(spec); }
};

template <class T>
struct Sdf_AccessorHelpers<T, false> {
    static const SdfSchemaBase& GetSchema(const T* spec) { return spec->_GetSpec().GetSchema(); }

    static VtValue GetField(const T* spec, const TfToken& key) { return spec->_GetSpec().GetField(key); }
    ...
    static bool HasField(const T* spec, const TfToken& key) { return spec->_GetSpec().HasField(key); }

    static void ClearField(T* spec, const TfToken& key) { spec->_GetSpec().ClearField(key); }

    static SdfSpecHandle GetSpecHandle(const T* spec) { return SdfCreateNonConstHandle(&(spec->_GetSpec())); }
};
