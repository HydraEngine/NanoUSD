...
public:
    ...
    static void Puts(Sdf_TextOutput& out, size_t indent, const std::string& str);

    // Printf-style formatted string output
    static void Write(Sdf_TextOutput& out, size_t indent, const char* fmt, ...);
    ...
    static void WriteAssetPath(Sdf_TextOutput& out, size_t indent, const std::string& str);

    static void WriteDefaultValue(Sdf_TextOutput& out, size_t indent, VtValue value);
    ...
    static void WriteSpline(Sdf_TextOutput& out, size_t indent, const TsSpline& spline);

    static bool WriteRelocates(Sdf_TextOutput& out, size_t indent, bool multiLine, const SdfRelocates& relocates);

    static bool WriteRelocates(Sdf_TextOutput& out, size_t indent, bool multiLine, const SdfRelocatesMap& reloMap);
    ...
    template <class T>
    static void WriteListOp(Sdf_TextOutput& out, size_t indent, const TfToken& fieldName, const SdfListOp<T>& listOp);

    static void WriteLayerOffset(Sdf_TextOutput& out, size_t indent, bool multiline, const SdfLayerOffset& offset);
...
private:
    ...
    typedef std::map<const std::string*, const VtValue*, _StringLessThan> _OrderedDictionary;

    static void _WriteDictionary(
            Sdf_TextOutput& out, size_t indent, bool multiLine, _OrderedDictionary& dictionary, bool stringValuesOnly);
};
...
struct Sdf_IsMetadataField {
    Sdf_IsMetadataField(const SdfSpecType specType) : _specDef(SdfSchema::GetInstance().GetSpecDefinition(specType)) {}
...
static bool Sdf_WritePrimPreamble(const SdfPrimSpec& prim, Sdf_TextOutput& out, size_t indent) {
    ...
    Sdf_FileIOUtility::Write(out, indent, "%s%s%s ", Sdf_FileIOUtility::Stringify(spec), !typeName.IsEmpty() ? " " : "",
...
static void Sdf_WriteSimpleField(Sdf_TextOutput& out, size_t indent, const SdfSpec& spec, const TfToken& field) {
    ...
    if (isUnregisteredValue) {
        ...
        if (boxedValue.IsHolding<SdfUnregisteredValueListOp>()) {
            Sdf_FileIOUtility::WriteListOp(out, indent, field, boxedValue.UncheckedGet<SdfUnregisteredValueListOp>());
        } else {
...
struct Sdf_IsPrimMetadataField : public Sdf_IsMetadataField {
    ...
    bool operator()(const TfToken& field) const {
        ...
        return (Sdf_IsMetadataField::operator()(field) || field == SdfFieldKeys->Payload ||
...
static bool Sdf_WritePrimMetadata(const SdfPrimSpec& prim, Sdf_TextOutput& out, size_t indent) {
    ...
    TfTokenVector::iterator metadataFieldsEnd = std::partition(fields.begin(), fields.end(), Sdf_IsPrimMetadataField());
    ...
    for (TfTokenVector::const_iterator fieldIt = fields.begin(); fieldIt != metadataFieldsEnd; ++fieldIt) {
        ...
        if (field == SdfFieldKeys->Documentation) {
            ...
            if (multiLine) {
                ...
                Sdf_FileIOUtility::Write(out, 0, "permission = %s", Sdf_FileIOUtility::Stringify(prim.GetPermission()));
            }
            ...
            Sdf_FileIOUtility::WriteDictionary(out, indent + 1, multiLine, prefixSubstitutions,
            ...
            Sdf_FileIOUtility::WriteDictionary(out, indent + 1, multiLine, suffixSubstitutions,
                                               /* stringValuesOnly = */ true);
...
static bool Sdf_WritePrimProperties(const SdfPrimSpec& prim, Sdf_TextOutput& out, size_t indent) {
    ...
    for (const SdfPropertySpecHandle& specHandle : properties) {
        ...
        if (specType == SdfSpecTypeAttribute) {
            ...
            Sdf_WriteRelationship(Sdf_CastAccess::CastSpec<SdfRelationshipSpec, SdfPropertySpec>(spec), out,
                                  indent + 1);
        }
    }
...
static bool Sdf_WritePrimChildren(const SdfPrimSpec& prim, Sdf_TextOutput& out, size_t indent) {
    bool newline = false;
...
static bool Sdf_WritePrimVariantSets(const SdfPrimSpec& prim, Sdf_TextOutput& out, size_t indent) {
...
static bool Sdf_WritePrimBody(const SdfPrimSpec& prim, Sdf_TextOutput& out, size_t indent) {
    Sdf_WritePrimNamespaceReorders(prim, out, indent);

    Sdf_WritePrimProperties(prim, out, indent);

    if (!prim.GetProperties().empty() && !prim.GetNameChildren().empty()) Sdf_FileIOUtility::Puts(out, 0, "\n");
...
static bool Sdf_WriteVariantSet(const SdfVariantSetSpec& spec, Sdf_TextOutput& out, size_t indent) {
    ...
    std::sort(variants.begin(), variants.end(), [](const SdfVariantSpecHandle& a, const SdfVariantSpecHandle& b) {
...
static bool Sdf_WriteConnectionStatement(Sdf_TextOutput& out,
                                         size_t indent,
                                         const SdfConnectionsProxy::ListProxy& connections,
                                         const std::string& opStr,
                                         const std::string& variabilityStr,
                                         const std::string& typeStr,
                                         const std::string& nameStr,
                                         const SdfAttributeSpec* attrOwner) {
    Sdf_FileIOUtility::Write(out, indent, "%s%s%s %s.connect = ", opStr.c_str(), variabilityStr.c_str(),
...
static bool Sdf_WriteConnectionList(Sdf_TextOutput& out,
                                    size_t indent,
                                    const SdfConnectionsProxy& connList,
                                    const std::string& variabilityStr,
                                    const std::string& typeStr,
                                    const std::string& nameStr,
                                    const SdfAttributeSpec* attrOwner) {
    if (connList.IsExplicit()) {
        ...
        if (!vec.empty()) {
            Sdf_WriteConnectionStatement(out, indent, vec, "prepend ", variabilityStr, typeStr, nameStr, attrOwner);
        }
...
struct Sdf_IsAttributeMetadataField : public Sdf_IsMetadataField {
    ...
    bool operator()(const TfToken& field) const {
        return (Sdf_IsMetadataField::operator()(field) || field == SdfFieldKeys->DisplayUnit);
    }
};

static inline bool Sdf_WriteAttribute(const SdfAttributeSpec& attr, Sdf_TextOutput& out, size_t indent) {
    std::string variabilityStr = Sdf_FileIOUtility::Stringify(attr.GetVariability());
    if (!variabilityStr.empty()) variabilityStr += ' ';
    ...
    const TsSpline spline = (splineVal.IsHolding<TsSpline>() ? splineVal.UncheckedGet<TsSpline>() : TsSpline());
    ...
    TfTokenVector::iterator metadataFieldsEnd =
            std::partition(fields.begin(), fields.end(), Sdf_IsAttributeMetadataField());
    ...
    if (hasInfo || hasDefault || hasCustomDeclaration || (!hasConnections && !hasTimeSamples && !hasSpline)) {
        VtValue value;

        if (hasDefault) value = attr.GetDefaultValue();

        Sdf_FileIOUtility::Write(out, indent, "%s%s%s %s", (hasCustomDeclaration ? "custom " : ""),
                                 variabilityStr.c_str(), typeName.c_str(), attr.GetName().c_str());
        ...
        for (TfTokenVector::const_iterator fieldIt = fields.begin(); fieldIt != metadataFieldsEnd; ++fieldIt) {
            ...
            if (field == SdfFieldKeys->Documentation) {
                ...
                Sdf_FileIOUtility::Write(out, multiLine ? indent + 1 : 0, "permission = %s%s",
                                         Sdf_FileIOUtility::Stringify(attr.GetPermission()), multiLine ? "\n" : "");
            } else if (field == SdfFieldKeys->SymmetryFunction) {
                Sdf_FileIOUtility::Write(out, multiLine ? indent + 1 : 0, "symmetryFunction = %s%s",
    ...
    if (hasConnections) {
        Sdf_WriteConnectionList(out, indent, attr.GetConnectionPathList(), variabilityStr, typeName, attr.GetName(),
                                &attr);
    }
...
static bool Sdf_WriteRelationshipTargetList(const SdfRelationshipSpec& rel,
...
struct Sdf_IsRelationshipMetadataField : public Sdf_IsMetadataField {
    Sdf_IsRelationshipMetadataField() : Sdf_IsMetadataField(SdfSpecTypeRelationship) {}

    bool operator()(const TfToken& field) const { return Sdf_IsMetadataField::operator()(field); }
};
...
static inline bool Sdf_WriteRelationship(const SdfRelationshipSpec& rel, Sdf_TextOutput& out, size_t indent) {
    ...
    TfTokenVector::iterator metadataFieldsEnd =
            std::partition(fields.begin(), fields.end(), Sdf_IsRelationshipMetadataField());
    ...
    if (hasDefaultValue) {
        ...
        if (!value.IsEmpty()) {
            Sdf_FileIOUtility::Write(out, indent, "%srel %s.default = ", varyingStr.c_str(), rel.GetName().c_str());
