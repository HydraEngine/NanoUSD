...
static int _ReadUTF8(unsigned char const*& cp, std::string* errMsg) {
...
// not.
static bool _ValidateAssetPathString(char const* path) {
    // Return true if 'code' is a C0 or C1 control code, false otherwise.
    auto isControlCode = [](int code) {
        return ((0x0 <= code && code <= 0x1f) || code == 0x7f || (0x80 <= code && code <= 0x9f));
    };
...
SdfAssetPath::SdfAssetPath(const std::string& path, const std::string& resolvedPath)
    : _assetPath(path), _resolvedPath(resolvedPath) {
...
bool SdfAssetPath::operator<(const SdfAssetPath& rhs) const {
    if (_assetPath < rhs._assetPath) return true;
    if (rhs._assetPath < _assetPath) return false;
