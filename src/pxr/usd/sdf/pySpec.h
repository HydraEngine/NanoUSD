...
public:
    ...
    void visit(CLS& c) const {
        ...
        if (PyObject_HasAttrString(c.ptr(), "__new__")) c.attr("__new__") = c.attr("__new__");
    ...
    void visit(CLS& c, char const* name, Options& options) const {
        ...
        c.def("__new__", CTOR::template __new__<CLS>,
              ...
              _doc.c_str(), options.keywords(), options.policies());
        c.staticmethod("__new__");
...
Sdf_PySpecDetail::NewVisitor<typename Sdf_PySpecDetail::NewCtor<T>> SdfMakePySpecConstructor(
        T* func, const std::string& doc = std::string()) {
...
struct _ConstHandleToPython {
    ...
    _ConstHandleToPython() { bp::to_python_converter<ConstHandle, _ConstHandleToPython<SpecType>>(); }
    static PyObject* convert(ConstHandle const& p) { return bp::incref(bp::object(TfConst_cast<Handle>(p)).ptr()); }
};
...
public:
    ...
    static PyObject* convert(const Handle& x) { return _CreateHolder(typeid(SpecType), x.GetSpec()); }
...
private:
    ...
    static bp::converter::to_python_function_t _RegisterConverter(bp::converter::to_python_function_t f) {
        ...
        bp::converter::registration* r =
                const_cast<bp::converter::registration*>(bp::converter::registry::query(bp::type_id<T>()));
        if (r) {
...
struct _HandleFromPython {
    ...
    _HandleFromPython() { bp::converter::registry::insert(&convertible, &construct, bp::type_id<Handle>()); }

private:
    static void* convertible(PyObject* p) {
        if (p == Py_None) return p;
...
struct SpecVisitor : bp::def_visitor<SpecVisitor<Abstract>> {
    ...
    public:
        ...
        static bool IsExpired(const HeldType& self) { return !self; }

        static bool IsValid(const HeldType& self) { return self; }

        static size_t __hash__(const HeldType& self) { return hash_value(self); }

        static bool __eq__(const HeldType& a, const HeldType& b) { return a == b; }

        static bool __ne__(const HeldType& a, const HeldType& b) { return a != b; }

        static bool __lt__(const HeldType& a, const HeldType& b) { return a < b; }

        static bool __le__(const HeldType& a, const HeldType& b) { return a <= b; }

        static bool __gt__(const HeldType& a, const HeldType& b) { return a > b; }

        static bool __ge__(const HeldType& a, const HeldType& b) { return a >= b; }
    };
...
public:
    ...
    void visit(CLS& c) const {
        ...
        static_assert(std::is_same<HeldType, SdfHandle<SpecType>>::value, "HeldType must be SdfHandle<SpecType>.");
...
inline Sdf_PySpecDetail::SpecVisitor<false> SdfPySpec() {
    return Sdf_PySpecDetail::SpecVisitor<false>();
}

inline Sdf_PySpecDetail::SpecVisitor<true> SdfPyAbstractSpec() {
    return Sdf_PySpecDetail::SpecVisitor<true>();
}

inline Sdf_PySpecDetail::SpecVisitor<false> SdfPySpecNoRepr() {
    return Sdf_PySpecDetail::SpecVisitor<false>(false);
}

inline Sdf_PySpecDetail::SpecVisitor<true> SdfPyAbstractSpecNoRepr() {
...
struct NewCtor<R(Args...)> : CtorBase<R(Args...)> {
    ...
    static bp::object __new__(bp::object& cls, Args... args) {
        ...
        if (TfPyConvertTfErrorsToPythonException(m)) bp::throw_error_already_set();
        bp::object result = TfPyObject(specHandle);
        if (TfPyIsNone(result)) TfPyThrowRuntimeError("could not construct " + ArchGetDemangled(typeid(HeldType)));
