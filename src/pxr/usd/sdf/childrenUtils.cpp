...
struct Sdf_IsValidPathComponent {
    ...
    bool operator()(const TfToken& newComponent) { return ChildPolicy::IsValidIdentifier(newComponent); }
    template <class T>
    bool operator()(const T& newComponent) {
        return false;
    }
};
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::CreateSpec(SdfLayer* layer,
    ...
    if (!layer->_CreateSpec(childPath, specType, inert)) {
        TF_CODING_ERROR("Failed to create spec of type \'%s\' at <%s>", TfEnum::GetName(specType).c_str(),
                        childPath.GetText());
        return false;
    }
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::SetChildren(const SdfLayerHandle& layer,
                                                 const SdfPath& path,
    ...
    TF_FOR_ALL(i, values) {
        ...
        if (path != (*i)->GetPath().GetParentPath() && path.HasPrefix((*i)->GetPath())) {
    ...
    TF_FOR_ALL(i, values) {
        ...
        std::vector<FieldType> oldSiblings = layer->GetFieldAs<std::vector<FieldType>>(oldParentPath, oldChildrenKey);

        typename std::vector<FieldType>::iterator oldNameIter = std::find(oldSiblings.begin(), oldSiblings.end(), key);
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::InsertChild(const SdfLayerHandle& layer,
                                                 const SdfPath& path,
    ...
    if ((size_t)index > childNames.size()) {
        TF_CODING_ERROR("Attempt to insert spec %s at an invalid index %d", newPath.GetText(), index);
        return false;
    }
    ...
    TF_FOR_ALL(i, childNames) {
        if (*i == key) {
            TF_CODING_ERROR("Attempt to insert duplicate spec %s", newPath.GetText());
            return false;
        }
    }
    ...
    std::vector<FieldType> oldSiblingNames = layer->GetFieldAs<std::vector<FieldType>>(oldParentPath, oldChildrenKey);

    typename std::vector<FieldType>::iterator oldNameIter =
            std::find(oldSiblingNames.begin(), oldSiblingNames.end(), key);
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::RemoveChild(const SdfLayerHandle& layer,
                                                 const SdfPath& path,
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::MoveChildForBatchNamespaceEdit(const SdfLayerHandle& layer,
                                                                    const SdfPath& path,
                                                                    const typename ChildPolicy::ValueType& value,
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::CanMoveChildForBatchNamespaceEdit(const SdfLayerHandle& layer,
                                                                       const SdfPath& path,
                                                                       const typename ChildPolicy::ValueType& value,
                                                                       const typename ChildPolicy::FieldType& newName,
                                                                       int index,
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::CanRemoveChildForBatchNamespaceEdit(const SdfLayerHandle& layer,
                                                                         const SdfPath& path,
...
SdfAllowed Sdf_ChildrenUtils<ChildPolicy>::CanRename(const SdfSpec& spec,
    ...
    if (!IsValidName(newName)) {
        return TfStringPrintf("Cannot rename %s to invalid name '%s'", spec.GetPath().GetText(), newName.GetText());
    }
...
template <class ChildPolicy>
bool Sdf_ChildrenUtils<ChildPolicy>::Rename(const SdfSpec& spec, const typename ChildPolicy::FieldType& newName) {
    SdfPath oldPath = spec.GetPath();

    if (!IsValidName(newName)) {
        TF_CODING_ERROR("Cannot rename %s to invalid name '%s'", oldPath.GetText(), newName.GetText());
        return false;
    }
...
template <>
SdfAllowed Sdf_ChildrenUtils<Sdf_AttributeConnectionChildPolicy>::CanRename(const SdfSpec& spec,
...
template <>
bool Sdf_ChildrenUtils<Sdf_AttributeConnectionChildPolicy>::Rename(const SdfSpec& spec, const SdfPath& newName) {
    TF_CODING_ERROR("Cannot rename attribute connections");
    return false;
}

template <>
SdfAllowed Sdf_ChildrenUtils<Sdf_RelationshipTargetChildPolicy>::CanRename(const SdfSpec& spec,
                                                                           const SdfPath& newName) {
...
template <>
bool Sdf_ChildrenUtils<Sdf_RelationshipTargetChildPolicy>::Rename(const SdfSpec& spec, const SdfPath& newName) {
    TF_CODING_ERROR("Cannot rename relationship targets");
    return false;
}
