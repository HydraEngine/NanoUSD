...
void Sdf_SubLayerListEditor::_OnEdit(SdfListOpType op,
    ...
    const SdfLayerOffsetVector oldLayerOffsets =
            _GetOwner()->GetFieldAs<SdfLayerOffsetVector>(SdfFieldKeys->SubLayerOffsets);
    ...
    for (size_t i = 0; i < newValues.size(); ++i) {
        ...
        std::vector<std::string>::const_iterator oldValuesIt = std::find(oldValues.begin(), oldValues.end(), newLayer);
