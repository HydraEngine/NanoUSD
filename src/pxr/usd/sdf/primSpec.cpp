...
TF_REGISTRY_FUNCTION(TfType) {
    TfType::Define<SdfPrimSpecHandleVector>().Alias(TfType::GetRoot(), "SdfPrimSpecHandleVector");
    TfType::Define<SdfVariantSetSpecHandleMap>().Alias(TfType::GetRoot(), "map<string, SdfVariantSetSpecHandle>");
}

////////////////////////////////////////////////////////////////////////

SdfPrimSpecHandle SdfPrimSpec::New(const SdfLayerHandle& parentLayer,
                                   const std::string& name,
                                   SdfSpecifier spec,
                                   const std::string& typeName) {
    TRACE_FUNCTION();

    return _New(parentLayer ? parentLayer->GetPseudoRoot() : TfNullPtr, TfToken(name), spec, TfToken(typeName));
}
...
SdfPrimSpecHandle SdfPrimSpec::_New(const SdfPrimSpecHandle& parentPrim,
                                    const TfToken& name,
                                    SdfSpecifier spec,
    ...
    if (!SdfPrimSpec::IsValidName(name)) {
        TF_RUNTIME_ERROR(
                ...
                "name",
                parentPrimPtr->GetPath().AppendChild(name).GetText(), name.GetText());
        return TfNullPtr;
    }
...
bool SdfPrimSpec::_ValidateEdit(const TfToken& key) const {
    if (_IsPseudoRoot()) {
...
const std::string& SdfPrimSpec::GetName() const {
    return GetPath().GetName();
}

TfToken SdfPrimSpec::GetNameToken() const {
    return GetPath().GetNameToken();
}

bool SdfPrimSpec::CanSetName(const std::string& newName, std::string* whyNot) const {
    if (_IsPseudoRoot()) {
        if (whyNot) {
...
bool SdfPrimSpec::SetName(const std::string& name, bool validate) {
    ...
    if (!Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::Rename(*this, newName)) {
        return false;
    }
...
bool SdfPrimSpec::IsValidName(const std::string& name) {
...
bool SdfPrimSpec::_IsPseudoRoot() const {
    return GetSpecType() == SdfSpecTypePseudoRoot;
}

SdfPrimSpecHandle SdfPrimSpec::GetNameRoot() const {
    return GetLayer()->GetPseudoRoot();
}

SdfPrimSpecHandle SdfPrimSpec::GetNameParent() const {
    return GetPath().IsRootPrimPath() ? SdfPrimSpecHandle() : GetLayer()->GetPrimAtPath(GetPath().GetParentPath());
}

SdfPrimSpecHandle SdfPrimSpec::GetRealNameParent() const {
    return GetLayer()->GetPrimAtPath(GetPath().GetParentPath());
}

SdfPrimSpec::NameChildrenView SdfPrimSpec::GetNameChildren() const {
    return NameChildrenView(GetLayer(), GetPath(), SdfChildrenKeys->PrimChildren);
}

void SdfPrimSpec::SetNameChildren(const SdfPrimSpecHandleVector& nameChildrenSpecs) {
    Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::SetChildren(GetLayer(), GetPath(), nameChildrenSpecs);
}

bool SdfPrimSpec::InsertNameChild(const SdfPrimSpecHandle& child, int index) {
    return Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::InsertChild(GetLayer(), GetPath(), child, index);
}

bool SdfPrimSpec::RemoveNameChild(const SdfPrimSpecHandle& child) {
    if (child->GetLayer() != GetLayer() || child->GetPath().GetParentPath() != GetPath()) {
        TF_CODING_ERROR(
                ...
                child->GetPath().GetText(), GetPath().GetText());
        return false;
    }
...
bool SdfPrimSpec::HasNameChildrenOrder() const {
    return !GetNameChildrenOrder().empty();
}

void SdfPrimSpec::SetNameChildrenOrder(const std::vector<TfToken>& names) {
    GetNameChildrenOrder() = names;
}

void SdfPrimSpec::InsertInNameChildrenOrder(const TfToken& name, int index) {
    GetNameChildrenOrder().Insert(index, name);
}

void SdfPrimSpec::RemoveFromNameChildrenOrder(const TfToken& name) {
    GetNameChildrenOrder().Remove(name);
}

void SdfPrimSpec::RemoveFromNameChildrenOrderByIndex(int index) {
    GetNameChildrenOrder().Erase(index);
}

void SdfPrimSpec::ApplyNameChildrenOrder(std::vector<TfToken>* vec) const {
...
SdfPrimSpec::PropertySpecView SdfPrimSpec::GetProperties() const {
    return PropertySpecView(GetLayer(), GetPath(), SdfChildrenKeys->PropertyChildren);
}

void SdfPrimSpec::SetProperties(const SdfPropertySpecHandleVector& propertySpecs) {
    if (!_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
        return;
    }

    Sdf_ChildrenUtils<Sdf_PropertyChildPolicy>::SetChildren(GetLayer(), GetPath(), propertySpecs);
}

bool SdfPrimSpec::InsertProperty(const SdfPropertySpecHandle& property, int index) {
    if (!_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
        return false;
    }
...
void SdfPrimSpec::RemoveProperty(const SdfPropertySpecHandle& property) {
    ...
    if (property->GetLayer() != GetLayer() || property->GetPath().GetParentPath() != GetPath()) {
        TF_CODING_ERROR(
                ...
                property->GetPath().GetText(), GetPath().GetText());
        return;
    }
...
SdfPrimSpec::AttributeSpecView SdfPrimSpec::GetAttributes() const {
    return AttributeSpecView(GetLayer(), GetPath(), SdfChildrenKeys->PropertyChildren);
}

SdfPrimSpec::RelationshipSpecView SdfPrimSpec::GetRelationships() const {
    return RelationshipSpecView(GetLayer(), GetPath(), SdfChildrenKeys->PropertyChildren);
}

SdfPropertyOrderProxy SdfPrimSpec::GetPropertyOrder() const {
    return SdfGetNameOrderProxy(SdfCreateHandle(this), SdfFieldKeys->PropertyOrder);
}

bool SdfPrimSpec::HasPropertyOrder() const {
    return !GetPropertyOrder().empty();
}

void SdfPrimSpec::SetPropertyOrder(const std::vector<TfToken>& names) {
    if (_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
        GetPropertyOrder() = names;
    }
}

void SdfPrimSpec::InsertInPropertyOrder(const TfToken& name, int index) {
    if (_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
        GetPropertyOrder().Insert(index, name);
    }
}

void SdfPrimSpec::RemoveFromPropertyOrder(const TfToken& name) {
    if (_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
        GetPropertyOrder().Remove(name);
    }
}

void SdfPrimSpec::RemoveFromPropertyOrderByIndex(int index) {
    if (_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
        GetPropertyOrder().Erase(index);
    }
}

void SdfPrimSpec::ApplyPropertyOrder(std::vector<TfToken>* vec) const {
    if (_ValidateEdit(SdfChildrenKeys->PropertyChildren)) {
...
SdfSpecHandle SdfPrimSpec::GetObjectAtPath(const SdfPath& path) const {
    if (path.IsEmpty()) {
...
SdfPrimSpecHandle SdfPrimSpec::GetPrimAtPath(const SdfPath& path) const {
    if (path.IsEmpty()) {
...
SdfPropertySpecHandle SdfPrimSpec::GetPropertyAtPath(const SdfPath& path) const {
    if (path.IsEmpty()) {
...
SdfAttributeSpecHandle SdfPrimSpec::GetAttributeAtPath(const SdfPath& path) const {
    if (path.IsEmpty()) {
...
SdfRelationshipSpecHandle SdfPrimSpec::GetRelationshipAtPath(const SdfPath& path) const {
    if (path.IsEmpty()) {
...
SDF_DEFINE_GET_SET(PrefixSubstitutions, SdfFieldKeys->PrefixSubstitutions, VtDictionary)
SDF_DEFINE_GET_SET(SuffixSubstitutions, SdfFieldKeys->SuffixSubstitutions, VtDictionary)
...
void SdfPrimSpec::SetTypeName(const std::string& value) {
...
SdfInheritsProxy SdfPrimSpec::GetInheritPathList() const {
    return SdfGetPathEditorProxy(SdfCreateHandle(this), SdfFieldKeys->InheritPaths);
}

bool SdfPrimSpec::HasInheritPaths() const {
    return GetInheritPathList().HasKeys();
}

void SdfPrimSpec::ClearInheritPathList() {
...
SdfSpecializesProxy SdfPrimSpec::GetSpecializesList() const {
    return SdfGetPathEditorProxy(SdfCreateHandle(this), SdfFieldKeys->Specializes);
}

bool SdfPrimSpec::HasSpecializes() const {
    return GetSpecializesList().HasKeys();
}

void SdfPrimSpec::ClearSpecializesList() {
...
SdfPayloadsProxy SdfPrimSpec::GetPayloadList() const {
    return SdfGetPayloadEditorProxy(SdfCreateHandle(this), SdfFieldKeys->Payload);
}

bool SdfPrimSpec::HasPayloads() const {
    return GetPayloadList().HasKeys();
}

void SdfPrimSpec::ClearPayloadList() {
...
SdfReferencesProxy SdfPrimSpec::GetReferenceList() const {
    return SdfGetReferenceEditorProxy(SdfCreateHandle(this), SdfFieldKeys->References);
}

bool SdfPrimSpec::HasReferences() const {
    return GetReferenceList().HasKeys();
}

void SdfPrimSpec::ClearReferenceList() {
...
SdfVariantSetNamesProxy SdfPrimSpec::GetVariantSetNameList() const {
    return SdfVariantSetNamesProxy(std::make_unique<Sdf_ListOpListEditor<SdfNameKeyPolicy>>(
            SdfCreateHandle(this), SdfFieldKeys->VariantSetNames));
}

bool SdfPrimSpec::HasVariantSetNames() const {
    return GetVariantSetNameList().HasKeys();
}

std::vector<std::string> SdfPrimSpec::GetVariantNames(const std::string& name) const {
    ...
    std::vector<TfToken> variantNameTokens =
            GetLayer()->GetFieldAs<std::vector<TfToken>>(variantSetPath, SdfChildrenKeys->VariantChildren);
...
SdfVariantSelectionProxy SdfPrimSpec::GetVariantSelections() const {
    if (!_IsPseudoRoot()) {
...
void SdfPrimSpec::BlockVariantSelection(const std::string& variantSetName) {
    if (_ValidateEdit(SdfFieldKeys->VariantSelection)) {
        SdfVariantSelectionProxy proxy = GetVariantSelections();
        if (proxy) {
            SdfChangeBlock block;
...
SdfRelocatesMapProxy SdfPrimSpec::GetRelocates() const {
    if (!_IsPseudoRoot()) {
...
void SdfPrimSpec::SetRelocates(const SdfRelocatesMap& newMap) {
    if (_ValidateEdit(SdfFieldKeys->Relocates)) {
        GetRelocates() = newMap;
    }
}

bool SdfPrimSpec::HasRelocates() const {
    return HasField(SdfFieldKeys->Relocates);
}

void SdfPrimSpec::ClearRelocates() {
    if (_ValidateEdit(SdfFieldKeys->Relocates)) {
...
static bool _FindOrCreateVariantSpec(SdfLayer* layer, const SdfPath& vsPath) {
    ...
    if (!TF_VERIFY(varSetSpec, "Failed to create variant set for '%s' in @%s@", vsPath.GetText(),
...
static bool _IsValidPath(const SdfPath& path) {
    // Can't use SdfCreatePrimInLayer with non-prim, non-variant paths.
    if (!path.IsAbsoluteRootOrPrimPath() && !path.IsPrimVariantSelectionPath()) {
        return false;
    }
...
struct _AbsPathHelper {
    ...
    explicit _AbsPathHelper(SdfPath const& inPath) : _inPath(inPath) {
    ...
    inline SdfPath const& GetAbsPath() const { return *_absPath; }
    inline SdfPath const& GetOriginalPath() const { return _inPath; }
...
bool Sdf_UncheckedCreatePrimInLayer(SdfLayer* layerPtr, const SdfPath& primPath) {
    ...
    while (!ancestors.empty()) {
        ...
        if (maybeVariantSelPaths && ancPath.IsPrimVariantSelectionPath()) {
            ...
            if (ARCH_UNLIKELY(!Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::CreateSpec(layerPtr, ancPath, SdfSpecTypePrim,
                                                                                  /*inert=*/true))) {
                TF_RUNTIME_ERROR(
                        "Failed to create prim at path '%s' in "
                        "layer @%s@",
                        ancPath.GetText(), layerPtr->GetIdentifier().c_str());
                return false;
            }
        }
    }
...
static inline bool Sdf_CanCreatePrimInLayer(SdfLayer* layer, _AbsPathHelper const& absPath) {
