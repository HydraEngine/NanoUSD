...
static Sdf_RegistryAliases _AssetInfoToAliases(const Sdf_AssetInfo& assetInfo) {
    std::string identifierSansArguments, arguments;
    TF_VERIFY(Sdf_SplitIdentifier(assetInfo.identifier, &identifierSansArguments, &arguments));
    // The identifier cannot be empty. If it is, GetLayers() will not function
    // correctly.
    TF_VERIFY(!assetInfo.identifier.empty());
    return {assetInfo.identifier,
            assetInfo.assetInfo.repoPath.empty() ? "" : Sdf_CreateIdentifier(assetInfo.assetInfo.repoPath, arguments),
            assetInfo.resolvedPath.empty() ? "" : Sdf_CreateIdentifier(assetInfo.resolvedPath, arguments)};
}
...
static bool _TryToRemove(const std::string& key,
                         const SdfLayerHandle& layer,
                         std::unordered_map<std::string, SdfLayerHandle, TfHash>* map) {
    if (!key.empty()) {
        if (const auto it = map->find(key); it != std::end(*map) && it->second == layer) {
            map->erase(it);
            return true;
        }
    }
...
void Sdf_LayerRegistry::_Layers::Update(const SdfLayerHandle& layer,
    ...
    if (oldAliases.realPath != newAliases.realPath) {
        ...
        if (!newAliases.realPath.empty()) {
            if (const auto insertion = _byRealPath.emplace(newAliases.realPath, layer); insertion.second) {
                ...
                TF_DEBUG(SDF_LAYER).Msg(
                        "Updated realPath '%s' would create "
    ...
    if (oldAliases.repositoryPath != newAliases.repositoryPath) {
        if (_TryToRemove(oldAliases.repositoryPath, layer, &_byRepositoryPath)) {
            TF_DEBUG(SDF_LAYER).Msg(
                    "Removed repositoryPath '%s' for "
                    "update.\n",
    ...
    if (oldAliases.identifier != newAliases.identifier) {
        if (_TryToRemove(oldAliases.identifier, layer, &_byIdentifier)) {
            TF_DEBUG(SDF_LAYER).Msg(
                    "Removed identifier '%s' for "
                    "update.\n",
...
std::pair<SdfLayerHandle, bool> Sdf_LayerRegistry::_Layers::Insert(const SdfLayerHandle& layer,
                                                                   const Sdf_AssetInfo& assetInfo) {
    const auto aliases = _AssetInfoToAliases(assetInfo);
    if (const auto it = _byRealPath.find(aliases.realPath); it != std::end(_byRealPath)) {
...
bool Sdf_LayerRegistry::_Layers::Erase(const SdfLayerHandle& layer, const Sdf_AssetInfo& assetInfo) {
    const auto aliases = _AssetInfoToAliases(assetInfo);
    ...
    if (_TryToRemove(aliases.repositoryPath, layer, &_byRepositoryPath)) {
        erased = true;
        TF_DEBUG(SDF_LAYER).Msg("Erased repositoryPath '%s' from registry.\n", aliases.repositoryPath.c_str());
    }
    if (_TryToRemove(aliases.identifier, layer, &_byIdentifier)) {
        erased = true;
        TF_DEBUG(SDF_LAYER).Msg("Erased identifier '%s' from registry.\n", aliases.repositoryPath.c_str());
    }
    return erased;
}

void Sdf_LayerRegistry::Insert(const SdfLayerHandle& layer, const Sdf_AssetInfo& assetInfo) {
    TRACE_FUNCTION();
    ...
    TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::Insert(%s)\n", Sdf_LayerDebugRepr(layer).c_str());
    ...
    if (!result.second) {
        ...
        TF_CODING_ERROR(
                ...
                layer->GetFileFormat()->GetFormatId().GetText(), Sdf_LayerDebugRepr(layer).c_str(),
                result.first->GetFileFormat()->GetFormatId().GetText(), Sdf_LayerDebugRepr(result.first).c_str());
    }
}

void Sdf_LayerRegistry::Update(const SdfLayerHandle& layer,
                               const Sdf_AssetInfo& oldInfo,
                               const Sdf_AssetInfo& newInfo) {
    TRACE_FUNCTION();
    ...
    TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::Update(%s)\n", Sdf_LayerDebugRepr(layer).c_str());

    _layers.Update(layer, oldInfo, newInfo);
}

void Sdf_LayerRegistry::Erase(const SdfLayerHandle& layer, const Sdf_AssetInfo& assetInfo) {
    bool erased = _layers.Erase(layer, assetInfo);

    TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::Erase(%s) => %s\n", Sdf_LayerDebugRepr(layer).c_str(),
...
SdfLayerHandle Sdf_LayerRegistry::_FindByIdentifier(const string& layerPath) const {
    TRACE_FUNCTION();
    ...
    TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::_FindByIdentifier('%s') => %s\n", layerPath.c_str(),
                            foundLayer ? "Found" : "Not Found");

    return foundLayer;
}

SdfLayerHandle Sdf_LayerRegistry::_FindByRepositoryPath(const string& layerPath) const {
    TRACE_FUNCTION();

    SdfLayerHandle foundLayer;

    if (layerPath.empty()) return foundLayer;
    ...
    TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::_FindByRepositoryPath('%s') => %s\n", layerPath.c_str(),
                            foundLayer ? "Found" : "Not Found");

    return foundLayer;
}

SdfLayerHandle Sdf_LayerRegistry::_FindByRealPath(const string& layerPath, const string& resolvedPath) const {
    TRACE_FUNCTION();
    ...
    if (!Sdf_SplitIdentifier(layerPath, &searchPath, &arguments)) return foundLayer;
    ...
    {
        TfErrorMark m;
        searchPath = !resolvedPath.empty() ? resolvedPath : Sdf_ComputeFilePath(searchPath);
    ...
    TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::_FindByRealPath('%s') => %s\n", searchPath.c_str(),
                            foundLayer ? "Found" : "Not Found");
    ...
    TF_FOR_ALL(i, layers) {
        if (SdfLayerHandle layer = *i) {
            ostr << TfStringPrintf(
                    ...
                    layer->GetFileFormat()->GetFormatId().GetText(), layer->GetIdentifier().c_str(),
                    layer->GetRepositoryPath().c_str(), layer->GetRealPath().c_str(), layer->GetVersion().c_str(),
                    TfStringify(layer->GetAssetInfo()).c_str(), (layer->IsMuted() ? "True" : "False"),
