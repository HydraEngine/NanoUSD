...
public:
    ...
    struct Hash {
        size_t operator()(const SdfLayerOffset& offset) const { return offset.GetHash(); }
    };

    friend inline size_t hash_value(const SdfLayerOffset& offset) { return offset.GetHash(); }
    ...
    bool operator!=(const SdfLayerOffset& rhs) const { return !(*this == rhs); }
    ...
    /// \sa SdfLayerOffset::operator<
    bool operator>(const SdfLayerOffset& rhs) const { return rhs < *this; }
