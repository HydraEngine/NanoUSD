...
private:
    ...
    void _UpdateListOp(const ListOpType& newListOp, const SdfListOpType* updatedListOpType = NULL) {
        ...
        std::pair<const SdfListOpType, bool> opTypesAndChanged[] = {
                {SdfListOpTypeExplicit, false}, {SdfListOpTypeAdded, false},     {SdfListOpTypeDeleted, false},
                {SdfListOpTypeOrdered, false},  {SdfListOpTypePrepended, false}, {SdfListOpTypeAppended, false}};
        ...
        for (auto& opTypeAndChanged : opTypesAndChanged) {
            ...
            if (opTypeAndChanged.second) {
                if (!_ValidateEdit(opTypeAndChanged.first, _listOp.GetItems(opTypeAndChanged.first),
        ...
        for (auto& opTypeAndChanged : opTypesAndChanged) {
            if (opTypeAndChanged.second) {
                this->_OnEdit(opTypeAndChanged.first, oldListOp.GetItems(opTypeAndChanged.first),
                              newListOp.GetItems(opTypeAndChanged.first));
            }
        }
    }
