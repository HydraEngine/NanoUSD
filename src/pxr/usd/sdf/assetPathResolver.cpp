...
VtDictionary Sdf_ComputeExternalAssetModificationTimestamps(const SdfLayer& layer) {
    ...
    for (const std::string& resolvedPath : externalAssetDependencies) {
        ...
        result[resolvedPath] = ArGetResolver().GetModificationTimestamp(resolvedPath, ArResolvedPath(resolvedPath));
    }
    return result;
}
...
Sdf_AssetInfo* Sdf_ComputeAssetInfoFromIdentifier(const string& identifier,
    ...
    TF_DEBUG(SDF_ASSET).Msg(
            ...
            "  assetInfo->version = '%s'\n",
            assetInfo->identifier.c_str(), assetInfo->resolvedPath.GetPathString().c_str(),
            resolveInfo.repoPath.c_str(), resolveInfo.assetName.c_str(), resolveInfo.version.c_str());
...
string Sdf_GetExtension(const string& identifier) {
    ...
    const string& assetPath =
            Sdf_StripIdentifierArgumentsIfPresent(identifier, &strippedPath) ? strippedPath : identifier;
