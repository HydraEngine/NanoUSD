...
Sdf_Children<ChildPolicy>::Sdf_Children() : _childNamesValid(false) {}
...
typename Sdf_Children<ChildPolicy>::ValueType Sdf_Children<ChildPolicy>::GetChild(size_t index) const {
    ...
    SdfPath childPath = ChildPolicy::GetChildPath(_parentPath, _childNames[index]);
...
template <class ChildPolicy>
bool Sdf_Children<ChildPolicy>::IsEqualTo(const Sdf_Children<ChildPolicy>& other) const {
...
template <class ChildPolicy>
bool Sdf_Children<ChildPolicy>::Copy(const std::vector<ValueType>& values, const std::string& type) {
    _childNamesValid = false;
    ...
    return Sdf_ChildrenUtils<ChildPolicy>::SetChildren(_layer, _parentPath, values);
}
...
bool Sdf_Children<ChildPolicy>::Insert(const ValueType& value, size_t index, const std::string& type) {
    ...
    return Sdf_ChildrenUtils<ChildPolicy>::InsertChild(_layer, _parentPath, value, index);
}

template <class ChildPolicy>
bool Sdf_Children<ChildPolicy>::Erase(const KeyType& key, const std::string& type) {
    _childNamesValid = false;
