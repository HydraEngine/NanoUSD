...
public:
    ...
    static SdfPathPattern Nothing() { return {}; }
    ...
    struct Component {
        bool IsStretch() const { return predicateIndex == -1 && text.empty(); }
        ...
        friend bool operator==(Component const& l, Component const& r) {
            return std::tie(l.text, l.predicateIndex, l.isLiteral) == std::tie(r.text, r.predicateIndex, r.isLiteral);
        }

        friend bool operator!=(Component const& l, Component const& r) { return !(l == r); }
    ...
    SdfPath const& GetPrefix() const& { return _prefix; }

    /// \overload
    SdfPath GetPrefix() && { return std::move(_prefix); }
    ...
    /// \overload
    SdfPathPattern& SetPrefix(SdfPath const& p) { return SetPrefix(SdfPath(p)); }
    ...
    /// path.
    std::vector<Component> const& GetComponents() const& { return _components; }

    /// \overload
    std::vector<Component> GetComponents() && { return std::move(_components); }
    ...
    std::vector<SdfPredicateExpression> const& GetPredicateExprs() const& { return _predExprs; }

    /// \overload
    std::vector<SdfPredicateExpression> GetPredicateExprs() && { return std::move(_predExprs); }

    /// Return true if this pattern identifies properties exclusively, false
    /// otherwise.
    bool IsProperty() const { return _isProperty; }

    /// Return true if this pattern is not empty, false if it is.
    explicit operator bool() const { return !_prefix.IsEmpty(); }
...
private:
    ...
    friend bool operator!=(SdfPathPattern const& l, SdfPathPattern const& r) { return !(l == r); }
