...
private:
    static bool _AreSpecsAtPathEqual(const SdfAbstractData& lhs, const SdfAbstractData& rhs, const SdfPath& path) {
        ...
        if (lhs.GetSpecType(path) != rhs.GetSpecType(path)) return false;
        if (lhsFieldSet != rhsFieldSet) return false;

        TF_FOR_ALL(field, lhsFields) {
            // Note: this comparison forces manufacturing of VtValues.
            if (lhs.Get(path, *field) != rhs.Get(path, *field)) return false;
        }
...
void SdfAbstractData::EraseDictValueByKey(const SdfPath& path, const TfToken& fieldName, const TfToken& keyPath) {
    VtValue dictVal = Get(path, fieldName);
