...
public:
    ...
    Sdf_IdentityRefPtr Identify(const SdfPath& path) {
        ...
        if (iter != _ids.end()) {
            rawId = iter->second;
            ++rawId->_refCount;
            return Sdf_IdentityRefPtr(TfDelegatedCountDoNotIncrementTag, rawId);
        }
    ...
    void MoveIdentity(const SdfPath& oldPath, const SdfPath& newPath) {
        ...
        std::pair<_IdMap::iterator, bool> newIdStatus = _ids.insert(std::make_pair(newPath, (Sdf_Identity*)nullptr));
...
void Sdf_Identity::_UnregisterOrDelete(Sdf_IdRegistryImpl* regImpl, Sdf_Identity* id) noexcept {
    if (regImpl) {
