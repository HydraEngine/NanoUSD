...
static TfTokenVector _GetFilteredFieldNames(const SdfLayerHandle& layer, const SdfPath& path, SchemaPred const& pred) {
...
static TfTokenVector _GetValueFieldNames(const SdfLayerHandle& layer, const SdfPath& path) {
    return _GetFilteredFieldNames(layer, path, [](const SdfSchemaBase& schema, const TfToken& fieldName) {
...
// the copy process. The returned lists are sorted using the
// TfTokenFastArbitraryLessThan comparator.
static TfTokenVector _GetChildrenFieldNames(const SdfLayerHandle& layer, const SdfPath& path) {
    return _GetFilteredFieldNames(layer, path, [](const SdfSchemaBase& schema, const TfToken& fieldName) {
        return schema.HoldsChildren(fieldName);
    });
}
...
static void _ProcessChildren(const TfToken& childrenField,
                             const VtValue& srcChildrenValue,
                             const VtValue& dstChildrenValue,
    ...
    const ChildrenVector& srcChildren =
            srcChildrenValue.IsEmpty() ? emptyChildren : srcChildrenValue.UncheckedGet<ChildrenVector>();
    const ChildrenVector& dstChildren =
            dstChildrenValue.IsEmpty() ? emptyChildren : dstChildrenValue.UncheckedGet<ChildrenVector>();
    ...
    // to copy.
    if (childrenInDst) {
        const VtValue oldDstChildrenValue = dstLayer->GetField(dstPath, childrenField);
        if (!TF_VERIFY(oldDstChildrenValue.IsHolding<ChildrenVector>())) {
            return;
        }

        for (const auto& oldDstChild : oldDstChildrenValue.UncheckedGet<ChildrenVector>()) {
            if (std::find(dstChildren.begin(), dstChildren.end(), oldDstChild) == dstChildren.end()) {
                const SdfPath oldDstChildPath = ChildPolicy::GetChildPath(dstPath, oldDstChild);
...
static void _ProcessChildField(const TfToken& childField,
                               const SdfLayerHandle& srcLayer,
                               const SdfPath& srcPath,
                               bool childrenInSrc,
                               const SdfLayerHandle& dstLayer,
                               const SdfPath& dstPath,
                               bool childrenInDst,
                               const SdfShouldCopyChildrenFn& shouldCopyChildren,
                               _CopyStack* copyStack) {
    std::optional<VtValue> srcChildrenToCopy, dstChildrenToCopy;
    if (!shouldCopyChildren(childField, srcLayer, srcPath, childrenInSrc, dstLayer, dstPath, childrenInDst,
    ...
    if (childField == SdfChildrenKeys->ConnectionChildren) {
        _ProcessChildren<Sdf_AttributeConnectionChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath,
                                                             childrenInSrc, dstLayer, dstPath, childrenInDst,
                                                             copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->MapperChildren) {
        _ProcessChildren<Sdf_MapperChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath, childrenInSrc,
                                                dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->MapperArgChildren) {
        _ProcessChildren<Sdf_MapperArgChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath,
                                                   childrenInSrc, dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->ExpressionChildren) {
        _ProcessChildren<Sdf_ExpressionChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath,
                                                    childrenInSrc, dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->RelationshipTargetChildren) {
        _ProcessChildren<Sdf_RelationshipTargetChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath,
                                                            childrenInSrc, dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->VariantChildren) {
        _ProcessChildren<Sdf_VariantChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath, childrenInSrc,
                                                 dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->VariantSetChildren) {
        _ProcessChildren<Sdf_VariantSetChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath,
                                                    childrenInSrc, dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->PropertyChildren) {
        _ProcessChildren<Sdf_PropertyChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath,
                                                  childrenInSrc, dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
    if (childField == SdfChildrenKeys->PrimChildren) {
        _ProcessChildren<Sdf_PrimChildPolicy>(childField, srcChildren, dstChildren, srcLayer, srcPath, childrenInSrc,
                                              dstLayer, dstPath, childrenInDst, copyStack);
        return;
    }
...
template <class ChildPolicy>
static void _DoAddNewSpec(const SdfLayerHandle& destLayer, const _SpecDataEntry& specData) {
    Sdf_ChildrenUtils<ChildPolicy>::CreateSpec(destLayer, specData.dstPath, specData.specType);
}

static void _DoAddNewPrimSpec(const SdfLayerHandle& destLayer, const _SpecDataEntry& specData) {
    ...
    Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::CreateSpec(destLayer, specData.dstPath, SdfSpecTypePrim,
                                                       /* inert = */ inert);
}

template <class ChildPolicy>
static void _DoAddNewPropertySpec(const SdfLayerHandle& destLayer, const _SpecDataEntry& specData) {
...
static void _AddNewSpecToLayer(const SdfLayerHandle& destLayer, const _SpecDataEntry& specData) {
...
static void _DoRemoveSpec(const SdfLayerHandle& dstLayer, const SdfPath& dstPath) {
    Sdf_ChildrenUtils<ChildPolicy>::RemoveChild(dstLayer, ChildPolicy::GetParentPath(dstPath),
...
static void _AddFieldValueToCopy(SdfSpecType specType,
                                 const TfToken& field,
                                 const SdfLayerHandle& srcLayer,
                                 const SdfPath& srcPath,
                                 bool fieldInSrc,
                                 ...
                                 const SdfPath& dstPath,
                                 bool fieldInDst,
                                 const SdfShouldCopyValueFn& shouldCopyValue,
                                 _FieldValueList* valueList) {
    std::optional<VtValue> value;
    if (shouldCopyValue(specType, field, srcLayer, srcPath, fieldInSrc, dstLayer, dstPath, fieldInDst, &value)) {
...
static void _ForEachField(const std::vector<TfToken>& srcFields,
...
bool SdfCopySpec(const SdfLayerHandle& srcLayer,
    ...
    if ((srcPath.IsAbsoluteRootOrPrimPath() || srcPath.IsPrimVariantSelectionPath()) !=
        ...
        srcPath.IsPropertyPath() != dstPath.IsPropertyPath() || srcPath.IsTargetPath() != dstPath.IsTargetPath() ||
        srcPath.IsMapperPath() != dstPath.IsMapperPath() || srcPath.IsMapperArgPath() != dstPath.IsMapperArgPath() ||
    ...
    if (srcLayer == dstLayer && (srcPath.HasPrefix(dstPath) || dstPath.HasPrefix(srcPath))) {
        ...
        SdfLayerRefPtr tmpSrcLayer = SdfLayer::CreateAnonymous("SdfCopySpec_tmp_src_layer", srcLayer->GetFileFormat(),
        ...
        if (!SdfCopySpec(srcLayer, srcPrimPath, tmpSrcLayer, srcPrimPath)) {
            TF_RUNTIME_ERROR(
                    "Failed to create temporary source for overlapped "
                    "SdfCopySpec <%s> -> <%s>",
                    srcPath.GetAsString().c_str(), dstPath.GetAsString().c_str());
            return false;
        }

        // Now copy the temporary src to the dst.
        return SdfCopySpec(tmpSrcLayer, srcPath, dstLayer, dstPath, shouldCopyValueFn, shouldCopyChildrenFn);
    }
    ...
    while (!copyStack.empty()) {
        ...
        if (specType == SdfSpecTypeUnknown) {
            TF_CODING_ERROR("Cannot copy unknown spec at <%s> from layer <%s>", toCopy.srcPath.GetText(),
        ...
        const TfTokenVector dstValueFields = _GetValueFieldNames(dstLayer, toCopy.dstPath);
        const TfTokenVector srcValueFields = _GetValueFieldNames(srcLayer, toCopy.srcPath);

        // From the list of value fields, retrieve all values that the copy
        // policy says we need to copy over to the destination.
        _ForEachField(srcValueFields, dstValueFields, [&](const TfToken& field, bool fieldInSrc, bool fieldInDst) {
            _AddFieldValueToCopy(specType, field, srcLayer, toCopy.srcPath, fieldInSrc, dstLayer, toCopy.dstPath,
                                 fieldInDst, shouldCopyValueFn, &copyEntry.dataToCopy);
        });
        ...
        const bool copyingPrimToVariant = specType == SdfSpecTypePrim && toCopy.dstPath.IsPrimVariantSelectionPath();
        ...
        if (copyingPrimToVariant || copyingVariantToPrim) {
            ...
            copyEntry.dataToCopy.erase(std::remove_if(copyEntry.dataToCopy.begin(), copyEntry.dataToCopy.end(),
            ...
            if (copyingPrimToVariant) {
                ...
                copyEntry.dataToCopy.push_back({SdfFieldKeys->Specifier, VtValue(SdfSpecifierOver)});
                ...
                for (const TfToken& field : {SdfFieldKeys->Specifier, SdfFieldKeys->TypeName}) {
                ...
                _ForEachField(srcFields, dstFields, [&](const TfToken& field, bool fieldInSrc, bool fieldInDst) {
                    _AddFieldValueToCopy(specType, field, srcLayer, srcPrimPath, fieldInSrc, dstLayer, toCopy.dstPath,
                                         fieldInDst, shouldCopyValueFn, &copyEntry.dataToCopy);
                });
        ...
        for (const _FieldValuePair& fieldValue : copyEntry.dataToCopy) {
            if (fieldValue.second.IsHolding<SdfCopySpecsValueEdit>()) {
                ...
                dstLayer->SetField(copyEntry.dstPath, fieldValue.first, fieldValue.second);
            }
        }
        ...
        // other fields.
        const TfTokenVector dstChildrenFields = _GetChildrenFieldNames(dstLayer, toCopy.dstPath);
        const TfTokenVector srcChildrenFields = _GetChildrenFieldNames(srcLayer, toCopy.srcPath);

        // Now add any children specs that need to be copied to our
        // copy stack.
        _ForEachField(srcChildrenFields, dstChildrenFields,
                      [&](const TfToken& field, bool fieldInSrc, bool fieldInDst) {
                          _ProcessChildField(field, srcLayer, toCopy.srcPath, fieldInSrc, dstLayer, toCopy.dstPath,
                                             fieldInDst, shouldCopyChildrenFn, &copyStack);
                      });
    }
...
bool SdfShouldCopyValue(const SdfPath& srcRootPath,
    ...
    if (fieldInSrc) {
        if (field == SdfFieldKeys->ConnectionPaths || field == SdfFieldKeys->TargetPaths ||
            ...
            if (srcLayer->HasField(srcPath, field, &srcListOp)) {
                const SdfPath& srcPrefix = srcRootPath.GetPrimPath().StripAllVariantSelections();
                const SdfPath& dstPrefix = dstRootPath.GetPrimPath().StripAllVariantSelections();

                srcListOp.ModifyOperations([&srcPrefix, &dstPrefix](const SdfPath& path) {
            ...
            if (srcLayer->HasField(srcPath, field, &refListOp)) {
                ...
                refListOp.ModifyOperations(std::bind(&_FixInternalSubrootPaths<SdfReference>, std::placeholders::_1,
                                                     std::cref(srcPrefix), std::cref(dstPrefix)));
            ...
            if (srcLayer->HasField(srcPath, field, &payloadListOp)) {
                ...
                payloadListOp.ModifyOperations(std::bind(&_FixInternalSubrootPaths<SdfPayload>, std::placeholders::_1,
                                                         std::cref(srcPrefix), std::cref(dstPrefix)));
...
bool SdfCopySpec(const SdfLayerHandle& srcLayer,
    ...
    return SdfCopySpec(srcLayer, srcPath, dstLayer, dstPath,
                       /* shouldCopyValueFn = */
                       std::bind(SdfShouldCopyValue, std::cref(srcPath), std::cref(dstPath), ph::_1, ph::_2, ph::_3,
                                 ph::_4, ph::_5, ph::_6, ph::_7, ph::_8, ph::_9),
                       /* shouldCopyChildrenFn = */
                       std::bind(SdfShouldCopyChildren, std::cref(srcPath), std::cref(dstPath), ph::_1, ph::_2, ph::_3,
                                 ph::_4, ph::_5, ph::_6, ph::_7, ph::_8, ph::_9));
}
