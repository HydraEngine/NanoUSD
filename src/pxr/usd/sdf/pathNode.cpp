...
struct Sdf_PathNodePrivateAccess {
    ...
    static inline std::atomic<unsigned int>& GetRefCount(Handle h) {
...
struct _ParentAndRef {
    const Sdf_PathNode* parent;
    T const& value;
    operator _ParentAnd<T>() const { return {parent, value}; }
};

// Allow void for 'expression' path case, which has no additional data.
template <>
struct _ParentAndRef<void> {
    const Sdf_PathNode* parent;
    operator _ParentAnd<void>() const { return {parent}; }
};
...
inline typename Table::NodeHandle _FindOrCreate(Table& table,
                                                ...
                                                const Sdf_PathNode* parent,
                                                const Args&... args) {
    std::pair<_ParentAndRef<Args...>, typename Table::PoolHandle> newItem{{parent, args...}, {}};
    ...
    if (iresult.second || (Table::NodeHandle::IsCounted && (Access::GetRefCount(iresult.first->second).fetch_add(1) &
...
inline void _Remove(const Sdf_PathNode* pathNode,
    ...
    if (iter != mapAndMutex.map.end() && iter->second.GetPtr() == reinterpret_cast<char const*>(pathNode)) {
...
void Sdf_PrimPartPathNode::operator delete(void* p) {
    ...
    Sdf_PathPrimPartPool::Free(PoolHandle::GetHandle(reinterpret_cast<char*>(p)));
}

void Sdf_PropPartPathNode::operator delete(void* p) {
...
Sdf_PathNode const* Sdf_PathNode::GetAbsoluteRootNode() {
    return *_absoluteRootNode;
}

Sdf_PathNode const* Sdf_PathNode::GetRelativeRootNode() {
...
Sdf_PathPrimNodeHandle Sdf_PathNode::FindOrCreatePrimVariantSelection(Sdf_PathNode const* parent,
                                                                      ...
                                                                      TfFunctionRef<bool()> isValid) {
    return _FindOrCreate<Sdf_PrimVariantSelectionNode>(*_primVarSelNodes, isValid, parent,
...
Sdf_PathPropNodeHandle Sdf_PathNode::FindOrCreateExpression(Sdf_PathNode const* parent, TfFunctionRef<bool()> isValid) {
    return _FindOrCreate<Sdf_ExpressionPathNode>(*_expressionNodes, isValid, parent);
}

Sdf_PathNode::Sdf_PathNode(bool isAbsolute)
    : _refCount(1), _elementCount(0), _nodeType(RootNode), _nodeFlags(isAbsolute ? IsAbsoluteFlag : 0) {}
...
using _PrimToPropTokenTables = tbb::concurrent_hash_map<Sdf_PathNode const*, _PropToTokenTable>;
...
const TfToken& Sdf_PathNode::GetPathToken(Sdf_PathNode const* primPart, Sdf_PathNode const* propPart) {
    ...
    _pathTokenTable->insert(primAccessor, std::make_pair(primPart, _PropToTokenTable()));
...
struct _StringBuffer {
    ...
    std::string GetString() const { return std::string(_str.crbegin(), _str.crend()); }
...
void Sdf_PathNode::_WriteTextToBuffer(Sdf_PathNode const* primPart, Sdf_PathNode const* propPart, Buffer& out) {
    ...
    Sdf_PathNode const* const root =
            (primPart->IsAbsolutePath() ? Sdf_PathNode::GetAbsoluteRootNode() : Sdf_PathNode::GetRelativeRootNode());
...
template <class Buffer>
void Sdf_PathNode::_WriteTextToBuffer(SdfPath const& path, Buffer& out) {
...
static inline bool _HasNamespaceDelimiter(const std::string& identifier) {
    return identifier.find(SdfPathTokens->namespaceDelimiter.GetString()[0]) != std::string::npos;
}

bool Sdf_PathNode::_IsNamespacedImpl() const {
...
void Sdf_PathNode::_WriteText(Buffer& out) const {
    switch (_nodeType) {
        ...
        case PrimPropertyNode:
            out.WriteText(SDF_PATH_PROPERTY_DELIMITER_STR, _Downcast<Sdf_PrimPropertyPathNode>()->_name.GetText());
            return;
...
Sdf_PrimPathNode::~Sdf_PrimPathNode() {
    _Remove(this, *_primNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()), _name);
}

Sdf_PrimPropertyPathNode::~Sdf_PrimPropertyPathNode() {
    _Remove(this, *_primPropertyNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()), _name);
}
...
template <class Buffer>
void Sdf_PrimVariantSelectionNode::_WriteTextImpl(Buffer& out) const {
...
Sdf_PrimVariantSelectionNode::~Sdf_PrimVariantSelectionNode() {
    _Remove(this, *_primVarSelNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()),
            *_variantSelection);
}
...
Sdf_TargetPathNode::~Sdf_TargetPathNode() {
    _Remove(this, *_targetNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()), _targetPath);
}

Sdf_RelationalAttributePathNode::~Sdf_RelationalAttributePathNode() {
    _Remove(this, *_relAttrNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()), _name);
}

template <class Buffer>
void Sdf_MapperPathNode::_WriteTextImpl(Buffer& out) const {
    out.WriteText(SDF_PATH_RELATIONSHIP_TARGET_END_STR);
    _WriteTextToBuffer(_targetPath, out);
    out.WriteText(SDF_PATH_RELATIONSHIP_TARGET_START_STR);
...
Sdf_MapperPathNode::~Sdf_MapperPathNode() {
    _Remove(this, *_mapperNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()), _targetPath);
}

template <class Buffer>
void Sdf_MapperArgPathNode::_WriteTextImpl(Buffer& out) const {
    out.WriteText(SDF_PATH_PROPERTY_DELIMITER_STR, _name.GetText());
}

Sdf_MapperArgPathNode::~Sdf_MapperArgPathNode() {
    _Remove(this, *_mapperArgNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()), _name);
}

template <class Buffer>
void Sdf_ExpressionPathNode::_WriteTextImpl(Buffer& out) const {
...
Sdf_ExpressionPathNode::~Sdf_ExpressionPathNode() {
    _Remove(this, *_expressionNodes, Sdf_PathNodeConstRefPtr(TfDelegatedCountIncrementTag, GetParentNode()));
}
...
static void _GatherChildrenFrom(Sdf_PathNode const* parent,
                                Table const& table,
    ...
    for (size_t outerIndex = 0; outerIndex != NumNodeMaps; ++outerIndex) {
        ...
        TF_FOR_ALL(i, mapAndMutex.map) {
            if (i->first.parent == parent)
                result->emplace_back(TfDelegatedCountIncrementTag,
                                     reinterpret_cast<Sdf_PathNode const*>(i->second.GetPtr()));
        }
    }
}

static vector<Sdf_PathNodeConstRefPtr> _GetChildren(Sdf_PathNode const* pathNode) {
...
static void _Visit(Sdf_PathNode const* path, Sdf_Stats* stats) {
    ...
    while (stats->lengthTable.size() <= len) stats->lengthTable.push_back(0);
    ...
    while (stats->numChildrenTable.size() <= numChildren) stats->numChildrenTable.push_back(0);
