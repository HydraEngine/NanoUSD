...
template <>
constexpr auto errorMessage<SingleQuote> = "Expected '";
template <>
constexpr auto errorMessage<DoubleQuote> = "Expected \"";
template <>
constexpr auto errorMessage<LeftParen> = "Expected (";
template <>
constexpr auto errorMessage<RightParen> = "Expected )";
template <>
constexpr auto errorMessage<LeftBracket> = "Expected [";
template <>
constexpr auto errorMessage<RightBracket> = "Expected ]";
template <>
constexpr auto errorMessage<LeftBrace> = "Expected {";
template <>
constexpr auto errorMessage<RightBrace> = "Expected }";
template <>
constexpr auto errorMessage<LeftAngleBracket> = "Expected <";
template <>
constexpr auto errorMessage<RightAngleBracket> = "Expected >";
template <>
constexpr auto errorMessage<At> = "Expected @";
template <>
constexpr auto errorMessage<Assignment> = "Expected =";
template <>
constexpr auto errorMessage<Digit> = "Expected number [0-9]";
template <>
constexpr auto errorMessage<Sdf_PathParser::Path> = "Expected Path";
template <>
constexpr auto errorMessage<ListOf<TupleValueItem>> =
        "Expected list of number, identifier, string, asset ref, or tuples "
        "separated by ,";
template <>
constexpr auto errorMessage<TupleValueItem> = "Expected number, identifier, string, asset ref, or tuple";
template <>
constexpr auto errorMessage<ListOf<ListValueItem>> =
        "Expected list of number, identifier, string, asset ref, list, or tuples "
        "separated by ,";
template <>
constexpr auto errorMessage<ListValueItem> = "Expected number, identifier, string, asset ref, list or tuple";
template <>
constexpr auto errorMessage<TokenSeparator> = "Expected spaces";
template <>
constexpr auto errorMessage<DictionaryKey> = "Expected string or identifier";
template <>
constexpr auto errorMessage<DictionaryValue> = "Expected dictionary";
template <>
constexpr auto errorMessage<DictionaryValueClose> = "Expected }";
template <>
constexpr auto errorMessage<ListOf<String>> = "Expected list of strings separated by ,";
template <>
constexpr auto errorMessage<String> = "Expected string";
template <>
constexpr auto errorMessage<StringDictionaryOpen> = "Expected {";
template <>
constexpr auto errorMessage<StringDictionaryClose> = "Expected }";
template <>
constexpr auto errorMessage<StringDictionaryItem> = "Expected string : string";
template <>
constexpr auto errorMessage<Identifier> = "Expected identifier";
template <>
constexpr auto errorMessage<MetadataClose> = "Expected )";
template <>
constexpr auto errorMessage<PathRef> = "Expected path reference";
template <>
constexpr auto errorMessage<TimeSampleMap> = "Expected dictionary of time samples (x : y)";
template <>
constexpr auto errorMessage<ConnectValue> = "Expected None, path ref, or list of path refs separated by ,";
template <>
constexpr auto errorMessage<RelationshipAssignmentClose> = "Expected ]";
template <>
constexpr auto errorMessage<RelationshipTargetClose> = "Expected ]";
template <>
constexpr auto errorMessage<RelocatesMapOpen> = "Expected {";
template <>
constexpr auto errorMessage<RelocatesMapClose> = "Expected }";
template <>
constexpr auto errorMessage<ReferenceList> = "Expected None, reference, or list of references separated by ,";
template <>
constexpr auto errorMessage<PayloadList> = "Expected None, payload, or list of payloads separated by ,";
template <>
constexpr auto errorMessage<InheritsOrSpecializesList> = "Expected None, path ref, or list of path refs separated by ,";
template <>
constexpr auto errorMessage<NameList> = "Expected string or list of strings enclosed in [] separated by ,";
template <>
constexpr auto errorMessage<VariantStatementOpen> = "Expected {";
template <>
constexpr auto errorMessage<VariantStatementClose> = "Expected }";
template <>
constexpr auto errorMessage<VariantStatementListOpen> = "Expected {";
template <>
constexpr auto errorMessage<VariantStatementListClose> = "Expected }";
template <>
constexpr auto errorMessage<VariantStatement> =
        "Expected sequence of child order, property order, prim, property, "
        "or variant sets enclosed in { }";
template <>
constexpr auto errorMessage<PrimContents> = "Expected child order, property order, prim, property, or variant set";
template <>
constexpr auto errorMessage<SublayerListClose> = "Expected ]";
template <>
constexpr auto errorMessage<NoneOrTypedListValue> = "Expected None or [";
...
struct TextParserAction<KeywordNone> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        Sdf_TextParserCurrentParsingContext parsingContext = context.parsingContext.back();
        if (parsingContext == Sdf_TextParserCurrentParsingContext::KeyValueMetadata ||
            ...
            _PopContext(context);
        } else if (parsingContext == Sdf_TextParserCurrentParsingContext::AttributeSpec) {
            _SetDefault(context.path, VtValue(SdfValueBlock()), context);
        } else if (parsingContext == Sdf_TextParserCurrentParsingContext::RelationshipSpec ||
                   parsingContext == Sdf_TextParserCurrentParsingContext::RelationshipAssignment) {
            context.relParsingTargetPaths = SdfPathVector();
        } else if (parsingContext == Sdf_TextParserCurrentParsingContext::TimeSamples) {
...
struct TextParserAction<KeywordCustomData> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::Metadata) {
            Sdf_TextParserCurrentParsingContext specContext = context.parsingContext.rbegin()[1];
...
struct TextParserAction<String> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        // are needed.
        if (const auto firstThree = inputStrView.substr(0, 3); firstThree == "\"\"\"" || firstThree == "'''") {
            evaluatedString = Sdf_EvalQuotedString(inputStrView.data(), inputStrView.length(), 3, nullptr);
        } else {
            evaluatedString = Sdf_EvalQuotedString(inputStrView.data(), inputStrView.length(), 1, nullptr);
        }

        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::DocMetadata) {
            context.data->Set(context.path, SdfFieldKeys->Documentation, VtValue::Take(evaluatedString));
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::Metadata) {
            context.data->Set(context.path, SdfFieldKeys->Comment, VtValue::Take(evaluatedString));
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::PrimSpec) {
            std::string errorMessage;
            if (!_CreatePrimSpec(evaluatedString, context, errorMessage)) {
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);

                throw PEGTL_NS::parse_error(errorMessage, in);
            }
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReorderRootPrims ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReorderNameChildren ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReorderProperties ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::VariantSetsMetadata) {
            context.nameVector.emplace_back(std::move(evaluatedString));
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::VariantSetStatement) {
            const SdfAllowed allow = SdfSchema::IsValidVariantIdentifier(evaluatedString);
            if (!allow) {
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), allow.GetWhyNot());
            ...
            context.path = context.path.AppendVariantSelection(context.currentVariantSetNames.back(), "");
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::VariantStatementList) {
            const SdfAllowed allow = SdfSchema::IsValidVariantIdentifier(evaluatedString);
            if (!allow) {
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), allow.GetWhyNot());
            ...
            context.path = context.path.GetParentPath().AppendVariantSelection(variantSetName, evaluatedString);

            context.data->CreateSpec(context.path, SdfSpecTypeVariant);
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::KindMetadata) {
            context.data->Set(context.path, SdfFieldKeys->Kind, VtValue(TfToken(evaluatedString)));
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::PrefixSubstitutionsMetadata ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SuffixSubstitutionsMetadata) {
            if (!context.seenStringDictionaryKey) {
                ...
                if (!context.values.SetupFactory(std::string("string"))) {
                    std::string errorMessage = "Unrecognized value typename 'string' for dictionary";
                    Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
                ...
                context.currentDictionaries[n - 2][context.stringDictionaryKey] = context.currentValue;
...
template <typename T>
struct _Bundler {
    void operator()(const double valueIn, VtValue* const valueOut) { *valueOut = VtValue(static_cast<T>(valueIn)); }
};

static VtValue _BundleSplineValue(Sdf_TextParserContext& context, const Sdf_ParserHelpers::Value& value) {
    VtValue result;
    TsDispatchToValueTypeTemplate<_Bundler>(context.spline.GetValueType(), value.Get<double>(), &result);
    return result;
}

static void _SetSplineTanWithWidth(Sdf_TextParserContext& context, const double width, const VtValue& slope) {
...
static void _SetSplineTanWithoutWidth(Sdf_TextParserContext& context, const VtValue& slope) {
    if (context.splineTanIsPre) {
        context.splineKnot.SetPreTanSlope(slope);
    } else {
        context.splineKnot.SetPostTanSlope(slope);
...
template <class Input>
std::pair<bool, Sdf_ParserHelpers::Value> _HelperGetNumericValueFromString(const Input& in,
                                                                           Sdf_TextParserContext& context) {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _GetNumericValueFromString(in.string_view());
    if (result.first) {
        TF_WARN("Integer literal '%s' on line %zu%s%s out of range, "
                ...
                in.string().c_str(), in.position().line, context.fileContext.empty() ? "" : " in file ",
                context.fileContext.empty() ? "" : context.fileContext.c_str());
    }
    return result;
}
...
struct TextParserAction<Number> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(in, context);
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::TimeSamples) {
            context.timeSampleTime = result.second.Get<double>();
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::LayerOffset ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::LayerScale) {
            if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::LayerOffset) {
...
struct TextParserAction<Identifier> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (parsingContext == Sdf_TextParserCurrentParsingContext::DictionaryTypeName) {
        ...
        } else if (parsingContext == Sdf_TextParserCurrentParsingContext::PermissionMetadata) {
            ...
            if (permissionStr == "public") {
                ...
                std::string errorMessage = "'" + permissionStr + "' is not a valid permission constant";
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);

                throw PEGTL_NS::parse_error(errorMessage, in);
            }

            context.data->Set(context.path, SdfFieldKeys->Permission, VtValue(permission));

            // this signals the end of the permission
            // metadata context
            _PopContext(context);
        } else if (parsingContext == Sdf_TextParserCurrentParsingContext::SymmetryFunctionMetadata) {
            context.symmetryFunctionName = in.string();
        } else if (parsingContext == Sdf_TextParserCurrentParsingContext::DisplayUnitMetadata) {
            const TfEnum& unit = SdfGetUnitFromName(in.string());
            if (unit == TfEnum()) {
                std::string errorMessage = "'" + in.string() + "' is not a valid display unit";
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);

                throw PEGTL_NS::parse_error(errorMessage, in);
            }
            context.data->Set(context.path, SdfFieldKeys->DisplayUnit, VtValue(unit));
        ...
        } else if (parsingContext == Sdf_TextParserCurrentParsingContext::PrimSpec) {
...
struct TextParserAction<NamespacedName> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::AttributeSpec) {
            std::string errorMessage;
        ...
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelationshipSpec) {
            std::string errorMessage;
...
struct TextParserAction<PathRef> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ConnectAttribute) {
            std::string pathStr = Sdf_EvalQuotedString(in.string().c_str(), in.string().length(), 1);
            ...
            if (!pathValid) {
                std::string errorMessage = "'" + pathStr + "' is not a valid prim or property scene path";
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
            ...
            if (absPath.ContainsPrimVariantSelection()) {
                TF_WARN("Connection path <%s> (in file @%s@, line %zu) has a "
                        ...
                        "fix this issue.",
                        absPath.GetText(), context.fileContext.c_str(), in.position().line,
        ...
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::InheritsListOpMetadata ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SpecializesListOpMetadata) {
            const std::string pathStr = Sdf_EvalQuotedString(in.string().c_str(), in.string().length(), 1);

            context.savedPath = SdfPath(pathStr);
            if (!context.savedPath.IsPrimPath()) {
                std::string errorMessage = "'" + pathStr + "' is not a valid prim path";
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
            ...
            if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::InheritsListOpMetadata) {
                context.inheritParsingTargetPaths.push_back(absPath);
            } else {
                context.specializesParsingTargetPaths.push_back(absPath);
            }
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelationshipAssignment ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelationshipTarget ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelationshipSpec) {
            std::string pathStr = Sdf_EvalQuotedString(in.string().c_str(), in.string().length(), 1);
        ...
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelationshipDefault) {
            std::string pathStr = Sdf_EvalQuotedString(in.string().c_str(), in.string().length(), 1);
            ...
            if (!context.relParsingNewTargetChildren.empty()) {
                ...
                children.insert(children.end(), context.relParsingNewTargetChildren.begin(),
                                context.relParsingNewTargetChildren.end());

                context.data->Set(context.path, SdfChildrenKeys->RelationshipTargetChildren, VtValue(children));
            }

            _PopContext(context);
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelocatesMetadata) {
            std::string pathStr = Sdf_EvalQuotedString(in.string().c_str(), in.string().length(), 1);
            ...
            if (!context.seenFirstRelocatesPath) {
                ...
                if (!SdfSchema::IsValidRelocatesSourcePath(context.relocatesKey)) {
                    const std::string errorMessage =
                            "'" + context.relocatesKey.GetAsString() + "' is not a valid relocates source path";
                    Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
                ...
                if ((path.IsEmpty() && !pathStr.empty()) || !SdfSchema::IsValidRelocatesTargetPath(path)) {
                    const std::string errorMessage =
                            "'" + context.relocatesKey.GetAsString() + "' is not a valid relocates target path";
                    Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
                ...
                const SdfPath srcPath = context.relocatesKey.MakeAbsolutePath(context.path);
                const SdfPath targetPath = path.MakeAbsolutePath(context.path);

                context.relocatesParsing.emplace_back(std::move(srcPath), std::move(targetPath));
                context.layerHints.mightHaveRelocates = true;

                context.relocatesKey = SdfPath();
                context.seenFirstRelocatesPath = false;
            }
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReferencesListOpMetadata ||
                   context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::PayloadListOpMetadata) {
            std::string pathStr = Sdf_EvalQuotedString(in.string().c_str(), in.string().length(), 1);
...
struct TextParserAction<AssetRef> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReferencesListOpMetadata ||
            context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::PayloadListOpMetadata) {
            const std::string_view inputStrView = in.string_view();
            const bool isTripleDelimited = (inputStrView.substr(0, 3) == "@@@");
            std::string evaluatedAssetPath =
                    Sdf_EvalAssetPath(inputStrView.data(), inputStrView.length(), isTripleDelimited);
            ...
            context.savedPath = SdfPath::EmptyPath();
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SubLayerMetadata) {
            const std::string_view inputStrView = in.string_view();
            const bool isTripleDelimited = (inputStrView.substr(0, 3) == "@@@");
            std::string evaluatedAssetPath =
                    Sdf_EvalAssetPath(inputStrView.data(), inputStrView.length(), isTripleDelimited);
...
struct TextParserAction<StringValue> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        std::string evaluatedString = Sdf_EvalQuotedString(inputStrView.data(), inputStrView.length(), numDelimeters);

        TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT).Msg("String value: " + evaluatedString + "\n");
...
struct TextParserAction<PathRefValue> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        std::string pathRef = Sdf_EvalQuotedString(inputString.c_str(), inputString.length(), 1);

        context.currentValue = pathRef.empty() ? SdfPath() : SdfPath(pathRef);
    }
};
...
struct TextParserAction<TypedValue> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::KeyValueMetadata) {
            _PopContext(context);
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::AttributeSpec) {
            _SetDefault(context.path, context.currentValue, context);
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::TimeSamples) {
...
struct TextParserAction<NameList> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReorderRootPrims) {
            context.data->Set(context.path, SdfFieldKeys->PrimOrder, VtValue(context.nameVector));

            context.nameVector.clear();

            _PopContext(context);
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::VariantSetsMetadata) {
            ...
            std::string errorMessage;
            if (!_SetListOpItemsWithError(SdfFieldKeys->VariantSetNames, context.listOpType, names, context,
                                          errorMessage)) {
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
            ...
            if (context.listOpType == SdfListOpTypeAdded || context.listOpType == SdfListOpTypeExplicit) {
                for (const auto& i : context.nameVector) {
                    context.data->CreateSpec(context.path.AppendVariantSelection(i, ""), SdfSpecTypeVariantSet);
                }

                context.data->Set(context.path, SdfChildrenKeys->VariantSetChildren, VtValue(context.nameVector));
            }
            ...
            if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ListOpMetadata) {
                _PopContext(context);
            }
        }
    }
...
struct TextParserAction<DictionaryType> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (!context.values.SetupFactory(context.dictionaryTypeName)) {
            std::string errorMessage =
                    "Unrecognized value typename '" + context.dictionaryTypeName + "' for dictionary";
            Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
...
struct TextParserAction<DictionaryKey> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (TfStringStartsWith(dictionaryKey, "\"") || TfStringStartsWith(dictionaryKey, "'")) {
            size_t numDelimeters = 1;
            if (TfStringStartsWith(dictionaryKey, "\"\"\"") || TfStringStartsWith(dictionaryKey, "'''")) {
                numDelimeters = 3;
            }

            dictionaryKey = Sdf_EvalQuotedString(dictionaryKey.c_str(), dictionaryKey.length(), numDelimeters);
        }
...
struct TextParserAction<DictionaryValueItem> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (context.expectDictionaryValue.back()) {
            ...
            size_t n = context.currentDictionaries.size();
            context.currentDictionaries[n - 2][context.currentDictionaryKey.back()] = context.currentValue;
        }
...
struct TextParserAction<SymmetryFunctionMetadata> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        // we set it as empty
        context.data->Set(context.path, SdfFieldKeys->SymmetryFunction,
...
struct TextParserAction<Sdf_PathParser::Dot> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::PrimSpec) {
...
struct TextParserAction<TimeSampleMap> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        context.data->Set(context.path, SdfFieldKeys->TimeSamples, VtValue(context.timeSamples));
...
struct TextParserAction<KeywordSpline> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        const TfType valueType = SdfGetTypeForValueTypeName(TfToken(context.values.valueTypeName));
        if (valueType == TfType::Find<SdfTimeCode>()) {
            ...
            if (context.splineValid) {
                ...
                std::string errorMessage = "Unsupported spline value type " + valueType.GetTypeName() +
                                           "for context value: " + context.values.valueTypeName +
        ...
        _PushContext(context, Sdf_TextParserCurrentParsingContext::SplineValues);
        // Assume we will get a SplineKnotItem, and if we get other SplineItem
        // we pop this there, by checking
        _PushContext(context, Sdf_TextParserCurrentParsingContext::SplineKnotItem);
    }
};
...
struct TextParserAction<KeywordNone_LC> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplinePostExtrapItem ||
            context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplinePreExtrapItem) {
            context.splineExtrap = TsExtrapolation(TsExtrapValueBlock);
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineInterpMode) {
...
struct TextParserAction<KeywordCurve> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineInterpMode) {
...
struct TextParserAction<KeywordSloped> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        _PushContext(context, Sdf_TextParserCurrentParsingContext::SplineExtrapSloped);
    }
};
...
struct TextParserAction<KeywordLoop> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKnotItem) {
        ...
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplinePostExtrapItem ||
            context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplinePreExtrapItem) {
            _PushContext(context, Sdf_TextParserCurrentParsingContext::SplineKeywordLoop);
        }
    }
};
...
struct TextParserAction<SplineLoopItem> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (std::trunc(numPreLoops) != numPreLoops || std::trunc(numPostLoops) != numPostLoops) {
            std::string errorMessage = "SplineLoopItem: Non-integer loop count";
            Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
            throw PEGTL_NS::parse_error(errorMessage, in);
        }
        TsLoopParams lp;
...
struct TextParserAction<KeywordRepeat> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKeywordLoop) {
...
struct TextParserAction<SplineKnotTime> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(in, context);
        context.splineKnot = TsKnot(context.spline.GetValueType(), context.spline.GetCurveType());
...
struct TextParserAction<SplineKnotItem> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        // to be popped also.
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKnotParam) {
            _PopContext(context);
        }
...
struct TextParserAction<SplineKnotValueWithoutPreValue> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        context.splineKnot.SetValue(_BundleSplineValue(context, context.splineKnotValue));
    }
};
...
struct TextParserAction<SplineKnotValueWithPreValue> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        context.splineKnot.SetPreValue(_BundleSplineValue(context, context.splineKnotPreValue));
        context.splineKnot.SetValue(_BundleSplineValue(context, context.splineKnotValue));
    }
};
...
struct TextParserAction<SplineKnotValues> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        _PushContext(context, Sdf_TextParserCurrentParsingContext::SplineKnotParam);
    }
};
...
struct TextParserAction<SplineTangentWithoutWidthValue> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        _SetSplineTanWithoutWidth(context, _BundleSplineValue(context, context.splineTangentValue));
    }
};
...
struct TextParserAction<SplineTangentWithWidthValue> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        _SetSplineTanWithWidth(context, context.splineTangentWidthValue.Get<double>(),
...
struct TextParserAction<KeywordPre> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineValues) {
            // We are still in spline values and it seems we will be getting a
            // SplinePreExtrapItem next, push that context.
            _PushContext(context, Sdf_TextParserCurrentParsingContext::SplinePreExtrapItem);
            return;
        }
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKnotParam) {
            ...
            _PushContext(context, Sdf_TextParserCurrentParsingContext::SplineTangent);
        }
    }
};
...
struct TextParserAction<KeywordPost> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineValues) {
            ...
            _PushContext(context, Sdf_TextParserCurrentParsingContext::SplinePostExtrapItem);
            return;
        }
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKnotParam) {
            context.splineTanIsPre = false;
            // We anticipate a SplineInterpMode here now.
            _PushContext(context, Sdf_TextParserCurrentParsingContext::SplineInterpMode);
        }
    }
};
...
struct TextParserAction<SplinePostShaping> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineTangent) {
            _PopContext(context);
        }
    }
};
...
struct TextParserAction<SplineValue> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.splineValid) {
            ...
            context.data->Set(context.path, SdfFieldKeys->Spline, VtValue(context.spline));
        }
...
struct TextParserAction<ConnectValue> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.connParsingTargetPaths.empty() && context.listOpType != SdfListOpTypeExplicit) {
            std::string errorMessage =
        ...
        for (const auto& path : context.connParsingTargetPaths) {
            const SdfAllowed allow = SdfSchema::IsValidAttributeConnectionPath(path);
            if (!allow) {
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), allow.GetWhyNot());
        ...
        if (context.listOpType == SdfListOpTypeAdded || context.listOpType == SdfListOpTypeExplicit) {
            for (const auto& pathIter : context.connParsingTargetPaths) {
                ...
                if (!context.data->HasSpec(path)) {
                    context.data->CreateSpec(path, SdfSpecTypeConnection);
                }
            }

            context.data->Set(context.path, SdfChildrenKeys->ConnectionChildren,
                              VtValue(context.connParsingTargetPaths));
        }

        std::string errorMessage;
        if (!_SetListOpItemsWithError(SdfFieldKeys->ConnectionPaths, context.listOpType, context.connParsingTargetPaths,
...
struct TextParserAction<KeywordDefault> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelationshipSpec) {
...
struct TextParserAction<RelationshipAssignmentOptional> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (!context.relParsingTargetPaths) {
            ...
            // seen the '='
            if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::RelationshipAssignment) {
                _PopContext(context);
            }
        ...
        if (context.relParsingTargetPaths->empty() && context.listOpType != SdfListOpTypeExplicit) {
            std::string errorMessage =
        ...
        if (context.listOpType == SdfListOpTypeAdded || context.listOpType == SdfListOpTypeExplicit) {
            ...
            for (const auto& pathIter : *context.relParsingTargetPaths) {
                ...
                if (!context.data->HasSpec(targetPath)) {
                    ...
                    context.data->CreateSpec(targetPath, SdfSpecTypeRelationshipTarget);
        ...
        if (!_SetListOpItemsWithError(SdfFieldKeys->TargetPaths, context.listOpType, *context.relParsingTargetPaths,
                                      context, errorMessage)) {
            Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
...
struct TextParserAction<RelationshipTargetClose> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        SdfPath path = context.path.AppendTarget(context.relParsingTargetPaths->back());
...
struct TextParserAction<PrimSpec> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (!context.nameChildrenStack.back().empty()) {
            context.data->Set(context.path, SdfChildrenKeys->PrimChildren, VtValue(context.nameChildrenStack.back()));
        }

        // store the names of our properties, if there are any
        if (!context.propertiesStack.back().empty()) {
            context.data->Set(context.path, SdfChildrenKeys->PropertyChildren, VtValue(context.propertiesStack.back()));
        }
        ...
        if (context.parsingContext.back() != Sdf_TextParserCurrentParsingContext::LayerSpec) {
            context.custom = false;
            context.variability = VtValue();
...
struct TextParserAction<RelocatesMapClose> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        // entity we want
        Sdf_TextParserCurrentParsingContext specContext = context.parsingContext.rbegin()[2];
        if (specContext == Sdf_TextParserCurrentParsingContext::PrimSpec) {
            SdfRelocatesMap relocatesParsingMap(std::make_move_iterator(context.relocatesParsing.begin()),
                                                std::make_move_iterator(context.relocatesParsing.end()));

            context.data->Set(context.path, SdfFieldKeys->Relocates, VtValue(relocatesParsingMap));
        } else if (specContext == Sdf_TextParserCurrentParsingContext::LayerSpec) {
            context.data->Set(context.path, SdfFieldKeys->LayerRelocates, VtValue(context.relocatesParsing));
        } else {
            Sdf_TextFileFormatParser_Err(context, in.input(), in.position(),
...
struct TextParserAction<PayloadList> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.payloadParsingRefs.empty() && context.listOpType != SdfListOpTypeExplicit) {
            std::string errorMessage =
        ...
        std::string errorMessage;
        if (!_SetListOpItemsWithError(SdfFieldKeys->Payload, context.listOpType, context.payloadParsingRefs, context,
                                      errorMessage)) {
            Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
...
struct TextParserAction<ReferenceListItem> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        SdfReference ref(context.layerRefPath, context.savedPath, context.layerRefOffset);
...
struct TextParserAction<ReferenceList> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.referenceParsingRefs.empty() && context.listOpType != SdfListOpTypeExplicit) {
            std::string errorMessage =
        ...
        std::string errorMessage;
        if (!_SetListOpItemsWithError(SdfFieldKeys->References, context.listOpType, context.referenceParsingRefs,
                                      context, errorMessage)) {
            Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
...
struct TextParserAction<InheritsOrSpecializesList> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::InheritsListOpMetadata) {
            if (context.inheritParsingTargetPaths.empty() && context.listOpType != SdfListOpTypeExplicit) {
                std::string errorMessage =
            ...
            std::string errorMessage;
            if (!_SetListOpItemsWithError(SdfFieldKeys->InheritPaths, context.listOpType,
                                          context.inheritParsingTargetPaths, context, errorMessage)) {
                Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);

                throw PEGTL_NS::parse_error(errorMessage, in);
            }
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SpecializesListOpMetadata) {
            if (context.specializesParsingTargetPaths.empty() && context.listOpType != SdfListOpTypeExplicit) {
                std::string errorMessage =
            ...
            std::string errorMessage;
            if (!_SetListOpItemsWithError(SdfFieldKeys->Specializes, context.listOpType,
...
struct TextParserAction<VariantsMetadata> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        VtValue oldVars;
        if (context.data->Has(context.path, SdfFieldKeys->VariantSelection, &oldVars)) {
        ...
        context.data->Set(context.path, SdfFieldKeys->VariantSelection, VtValue(refVars));
...
struct TextParserAction<VariantSetStatement> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        if (!context.data->HasSpec(variantSetPath)) {
            context.data->CreateSpec(variantSetPath, SdfSpecTypeVariantSet);

            // add the name of this variant set to the VariantSets field
            std::vector<TfToken> vec =
                    context.data->GetAs<std::vector<TfToken>>(context.path, SdfChildrenKeys->VariantSetChildren);

            vec.push_back(TfToken(context.currentVariantSetNames.back()));
            context.data->Set(context.path, SdfChildrenKeys->VariantSetChildren, VtValue(vec));
        }

        // Author the variant set's variants
        context.data->Set(variantSetPath, SdfChildrenKeys->VariantChildren,
...
struct TextParserAction<ChildOrPropertyOrderStatement> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReorderNameChildren) {
            context.data->Set(context.path, SdfFieldKeys->PrimOrder, VtValue(context.nameVector));

            _PopContext(context);
        } else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ReorderProperties) {
            context.data->Set(context.path, SdfFieldKeys->PropertyOrder, VtValue(context.nameVector));
...
struct TextParserAction<PrefixOrSuffixSubstitutionsMetadata> {
    template <class Input>
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::PrefixSubstitutionsMetadata) {
            context.data->Set(context.path, SdfFieldKeys->PrefixSubstitutions, VtValue(context.currentDictionaries[0]));
        } else {
            // suffix substitutions
            context.data->Set(context.path, SdfFieldKeys->SuffixSubstitutions, VtValue(context.currentDictionaries[0]));
        }
        ...
        if (TfStringStartsWith(cookie, expected)) {
            if (!context.versionString.empty() && !TfStringEndsWith(cookie, context.versionString)) {
                TF_WARN("File '%s' is not the latest %s version (found '%s', "
                        ...
                        "incorrect results.",
                        context.fileContext.c_str(), context.magicIdentifierToken.c_str(),
                        cookie.substr(expected.length()).c_str(), context.versionString.c_str());
            }
        } else {
            // throw error
            std::string errorMessage = TfStringPrintf("Magic Cookie '%s'.  Expected prefix of '%s'",
                                                      TfStringTrim(cookie).c_str(), expected.c_str());

            throw PEGTL_NS::parse_error(errorMessage, in);
        }

        context.nameChildrenStack.emplace_back();
        context.data->CreateSpec(SdfPath::AbsoluteRootPath(), SdfSpecTypePseudoRoot);
...
struct TextParserAction<LayerSpec> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        ...
        context.data->Set(SdfPath::AbsoluteRootPath(), SdfChildrenKeys->PrimChildren,
...
struct TextParserAction<SublayerListClose> {
    ...
    static void apply(const Input& in, Sdf_TextParserContext& context) {
        if (!context.subLayerPaths.empty()) {
            context.data->Set(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SubLayers, VtValue(context.subLayerPaths));

            context.data->Set(SdfPath::AbsoluteRootPath(), SdfFieldKeys->SubLayerOffsets,
...
bool Sdf_ParseLayer(const std::string& fileContext,
    ...
    Sdf_TextFileFormatParser::PEGTL_NS::string_input<> content{std::move(buffer), fileContext};
    context.values.errorReporter = [&context, capture0 = std::cref(content)](auto&& PH1) {
        return Sdf_TextFileFormatParser::_ReportParseError<Sdf_TextFileFormatParser::PEGTL_NS::string_input<>>(
        ...
        if (!metadataOnly) {
            if (TfDebug::IsEnabled(SDF_TEXT_FILE_FORMAT_PEGTL_TRACE)) {
                status = Sdf_TextFileFormatParser::PEGTL_NS::standard_trace<
                        Sdf_TextFileFormatParser::PEGTL_NS::must<Sdf_TextFileFormatParser::LayerSpec,
                                                                 Sdf_TextFileFormatParser::PEGTL_NS::internal::eof>,
                        Sdf_TextFileFormatParser::TextParserAction>(content, context);
                *hints = context.layerHints;
            } else {
                status = Sdf_TextFileFormatParser::PEGTL_NS::parse<
                        Sdf_TextFileFormatParser::PEGTL_NS::must<Sdf_TextFileFormatParser::LayerSpec,
                                                                 Sdf_TextFileFormatParser::PEGTL_NS::internal::eof>,
                        Sdf_TextFileFormatParser::TextParserAction, Sdf_TextFileFormatParser::TextParserControl>(
                        content, context);
                *hints = context.layerHints;
            }
        } else {
            ...
            status = Sdf_TextFileFormatParser::PEGTL_NS::parse<
                    Sdf_TextFileFormatParser::PEGTL_NS::must<Sdf_TextFileFormatParser::LayerMetadataOnly>,
                    Sdf_TextFileFormatParser::TextParserAction, Sdf_TextFileFormatParser::TextParserControl>(content,
                                                                                                             context);
            *hints = context.layerHints;
        }
    } catch (std::bad_variant_access const&) {
        TF_CODING_ERROR("Bad variant access in layer parser.");
        Sdf_TextFileFormatParser::Sdf_TextFileFormatParser_Err(context, content, content.position(),
                                                               "Internal layer parser error.");

        throw Sdf_TextFileFormatParser::PEGTL_NS::parse_error("Internal layer parser error", content);
    } catch (const Sdf_TextFileFormatParser::PEGTL_NS::parse_error& e) {
        Sdf_TextFileFormatParser::Sdf_TextFileFormatParser_Err(
                context, content, e.positions().empty() ? content.position() : e.positions()[0], e.what());
    }
...
bool Sdf_ParseLayerFromString(const std::string& layerString,
                              const std::string& magicId,
                              const std::string& versionString,
                              SdfDataRefPtr data,
                              SdfLayerHints* hints) {
    ...
    Sdf_TextFileFormatParser::PEGTL_NS::memory_input<> content{layerString.data(), layerString.size(), ""};

    context.values.errorReporter = [&context, capture0 = std::cref(content)](auto&& PH1) {
        return Sdf_TextFileFormatParser::_ReportParseError<Sdf_TextFileFormatParser::PEGTL_NS::memory_input<>>(
                context, capture0, std::forward<decltype(PH1)>(PH1));
    };
    bool status = false;
    try {
