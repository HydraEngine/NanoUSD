...
void SdfPredicateExpression::WalkWithOpStack(TfFunctionRef<void(std::vector<std::pair<Op, int>> const&)> logic,
...
void SdfPredicateExpression::Walk(TfFunctionRef<void(Op, int)> logic, TfFunctionRef<void(FnCall const&)> call) const {
    ...
    auto printCall = [&result](FnCall const& call) {
        ...
        switch (call.kind) {
            ...
            case FnCall::ParenCall: {
                std::vector<std::string> argStrs;
                for (auto const& arg : call.args) {
                    argStrs.push_back(TfStringPrintf("%s%s%s", arg.argName.empty() ? "" : arg.argName.c_str(),
                                                     arg.argName.empty() ? "" : "=",
...
SdfPredicateExpression::SdfPredicateExpression(std::string const& input, std::string const& context) {
