...
Sdf_ChangeManager::_Data::_Data() : outermostBlock(nullptr) {}

Sdf_ChangeManager::Sdf_ChangeManager() {}

Sdf_ChangeManager::~Sdf_ChangeManager() {}

void Sdf_ChangeManager::_SendNoticesForChangeList(const SdfLayerHandle& layer, const SdfChangeList& changeList) {
    ...
    const SdfChangeList::const_iterator layerEntry = changeList.FindEntry(SdfPath::AbsoluteRootPath());
...
SdfChangeList Sdf_ChangeManager::ExtractLocalChanges(const SdfLayerHandle& layer) {
    ...
    for (auto const& lc : changes) {
        ...
        if (TfDebug::IsEnabled(SDF_CHANGES)) {
            TF_DEBUG(SDF_CHANGES)
                    .Msg("Changes to layer %s:\n%s", lc.first->GetIdentifier().c_str(), TfStringify(lc.second).c_str());
        }
    }
...
void Sdf_ChangeManager::DidChangeLayerIdentifier(const SdfLayerHandle& layer, const std::string& oldIdentifier) {
    if (!layer->_ShouldNotify()) return;
...
static bool _IsOrderChangeOnly(const VtValue& oldVal, const VtValue& newVal) {
    ...
    if (!oldVal.IsEmpty() && !newVal.IsEmpty()) {
        ...
        if (oldNames.size() == newNames.size()) {
            ...
            std::set<TfToken, TfTokenFastArbitraryLessThan> oldNamesSet(oldNames.begin(), oldNames.end()),
...
void Sdf_ChangeManager::DidChangeField(const SdfLayerHandle& layer,
    ...
    if (field == FieldKeys->Default) {
        ...
        std::vector<std::string> addedLayers, removedLayers;
        {
            ...
            if (addedLayers.empty() && removedLayers.empty()) {
                removedLayers.insert(removedLayers.end(), oldSet.begin(), oldSet.end());
                addedLayers.insert(addedLayers.end(), newSet.begin(), newSet.end());
            }
        }
            ...
            if (TF_VERIFY(newOffsets.size() == subLayers.size())) {
                for (size_t i = 0; i < newOffsets.size(); ++i) {
                    if (oldOffsets[i] != newOffsets[i]) {
                        _GetListFor(changes, layer).DidChangeSublayerPaths(subLayers[i], SdfChangeList::SubLayerOffset);
                    }
                }
            }
        }
    ...
    } else if (field == FieldKeys->TimeCodesPerSecond && TF_VERIFY(path == SdfPath::AbsoluteRootPath())) {
        ...
        VtValue oldTcps = (!oldVal.IsEmpty() ? oldVal : layer->GetField(path, FieldKeys->FramesPerSecond));
        const VtValue newTcps = (!newVal.IsEmpty() ? newVal : layer->GetField(path, FieldKeys->FramesPerSecond));

        _GetListFor(changes, layer).DidChangeInfo(path, FieldKeys->TimeCodesPerSecond, std::move(oldTcps), newTcps);
    ...
    } else if (field == ChildrenKeys->ConnectionChildren || field == ChildrenKeys->ExpressionChildren ||
               field == ChildrenKeys->RelationshipTargetChildren || field == ChildrenKeys->VariantChildren ||
...
void Sdf_ChangeManager::DidChangeAttributeTimeSamples(const SdfLayerHandle& layer, const SdfPath& attrPath) {
    _GetListFor(_data.local().changes, layer).DidChangeAttributeTimeSamples(attrPath);
}

void Sdf_ChangeManager::DidMoveSpec(const SdfLayerHandle& layer, const SdfPath& oldPath, const SdfPath& newPath) {
    if (!layer->_ShouldNotify()) return;
...
SdfChangeList& Sdf_ChangeManager::_GetListFor(SdfLayerChangeListVec& theList, SdfLayerHandle const& layer) {
    auto iter = std::find_if(theList.begin(), theList.end(), [&layer](SdfLayerChangeListVec::value_type const& p) {
