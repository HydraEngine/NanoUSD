...
bool _ValueSetAtomic(Sdf_TextParserContext& context, std::string& errorMessage) {
...
bool _ValueSetTuple(Sdf_TextParserContext& context, std::string& errorMessage) {
    if (!context.values.IsRecordingString()) {
        if (context.values.valueIsShaped) {
            errorMessage = "Type name has [] for non-shaped value.\n";
            return false;
...
bool _ValueSetList(Sdf_TextParserContext& context, std::string& errorMessage) {
    if (!context.values.IsRecordingString()) {
        if (!context.values.valueIsShaped) {
...
bool _ValueSetShaped(Sdf_TextParserContext& context, std::string& errorMessage) {
    if (!context.values.IsRecordingString()) {
        if (!context.values.valueIsShaped) {
            errorMessage = "Type name missing [] for shaped value.";
            return false;
...
void _SetDefault(const SdfPath& path, VtValue val, Sdf_TextParserContext& context) {
    ...
    if (val.IsHolding<SdfPathExpression>()) {
        ...
        val.UncheckedMutate<VtArray<SdfPathExpression>>([&](VtArray<SdfPathExpression>& peArr) {
...
void _KeyValueMetadataStart(const std::string& key, SdfSpecType specType, Sdf_TextParserContext& context) {
    TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT)
            .Msg("Starting metadata for key: " + key + "(List op: " + _ListOpTypeToString(context.listOpType) + ")\n");
    ...
    if (specDef.IsMetadataField(context.genericMetadataKey)) {
        ...
        if (_IsGenericMetadataListOpType(fieldType, &itemArrayType)) {
            context.values.SetupFactory(schema.FindType(itemArrayType).GetAsToken().GetString());
        } else {
            context.values.SetupFactory(schema.FindType(fieldDef.GetFallbackValue()).GetAsToken().GetString());
        }
    } else {
...
bool _KeyValueMetadataEnd(SdfSpecType specType, Sdf_TextParserContext& context, std::string& errorMessage) {
    const SdfSchema& schema = SdfSchema::GetInstance();
    const SdfSchema::SpecDefinition& specDef = *schema.GetSpecDefinition(specType);
    if (specDef.IsMetadataField(context.genericMetadataKey)) {
        ...
        if (_IsGenericMetadataListOpType(fieldType)) {
            if (!fieldDef.IsValidListValue(context.currentValue)) {
                errorMessage = "invalid value for field \"" + context.genericMetadataKey.GetString() + "\"";
                return false;
            }
            ...
            context.data->Set(context.path, context.genericMetadataKey, VtValue(context.currentValue));
        }
    } else if (specDef.IsValidField(context.genericMetadataKey)) {
        // Prevent the user from overwriting fields that aren't metadata
        errorMessage = "\"" + context.genericMetadataKey.GetString() + "\" is registered as a non-metadata field";
        return false;
    } else {
        ...
        if (context.currentValue.IsHolding<VtDictionary>()) {
            ...
            auto getOldValue = [context]() {
                VtValue v;
                if (context.data->Has(context.path, context.genericMetadataKey, &v) &&
        ...
        if (!value.IsEmpty()) {
            context.data->Set(context.path, context.genericMetadataKey, VtValue(value));
        }
    }
...
bool _CreatePrimSpec(const std::string& primIdentifierString,
    ...
    TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT).Msg("Creating prim spec: " + primIdentifierString + "\n");
    ...
    context.data->Set(context.path, SdfFieldKeys->Specifier, VtValue(context.specifier));
    if (!context.primTypeName.empty()) {
        TfToken typeName = TfToken(context.primTypeName);
        context.data->Set(context.path, SdfFieldKeys->TypeName, VtValue(typeName));
    }
...
bool _CreateRelationshipSpec(const std::string& relationshipName,
    ...
    if (!SdfPath::IsValidNamespacedIdentifier(name)) {
        errorMessage = "'" + name.GetString() + " is not a valid relationship name";

        return false;
    }

    TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT)
            .Msg("Creating relationship spec for " + relationshipName +
    ...
    context.data->Set(context.path, SdfFieldKeys->Variability, VtValue(context.variability));

    if (context.custom) {
        context.data->Set(context.path, SdfFieldKeys->Custom, VtValue(true));
    }
...
bool _CreateAttributeSpec(const std::string& attributeName, Sdf_TextParserContext& context, std::string& errorMessage) {
    TfToken name(attributeName);
    if (!SdfPath::IsValidNamespacedIdentifier(name)) {
        errorMessage = "'" + name.GetString() + "' is not a valid attribute name";

        return false;
    }

    TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT)
            .Msg("Creating attribute spec for " + attributeName + ", current path is: " + context.path.GetAsString() +
                 "\n");
    ...
    if (!context.data->HasSpec(context.path)) {
        ...
        context.data->Set(context.path, SdfFieldKeys->Custom, VtValue(false));
    }
    ...
    if (context.data->Has(context.path, SdfFieldKeys->TypeName, &oldTypeValue)) {
        const TfToken& oldType = oldTypeValue.Get<TfToken>();
        if (newType != oldType) {
            errorMessage = "attribute '" + context.path.GetName() + "' already has type '" + oldType.GetString() +
                           "', cannot change to '" + newType.GetString() + "'";

            return false;
        }
    } else {
        context.data->Set(context.path, SdfFieldKeys->TypeName, VtValue(newType));
    }
    ...
    SdfVariability variability =
            context.variability.IsEmpty() ? SdfVariabilityVarying : context.variability.Get<SdfVariability>();
    VtValue oldVariability;
    if (context.data->Has(context.path, SdfFieldKeys->Variability, &oldVariability)) {
        if (variability != oldVariability.Get<SdfVariability>()) {
            errorMessage = "attribute '" + context.path.GetName() + "' already has variability '" +
                           TfEnum::GetName(oldVariability.Get<SdfVariability>()) + "', cannot change to '" +
                           TfEnum::GetName(variability) + "'";

            return false;
        }
    } else {
        context.data->Set(context.path, SdfFieldKeys->Variability, VtValue(variability));
    }

    return true;
}

std::pair<bool, Sdf_ParserHelpers::Value> _GetNumericValueFromString(const std::string_view in) {
    ...
    auto mustBeDouble = [](const std::string_view str) {
        return str.find('.') != str.npos || str.find('e') != str.npos || str.find('E') != str.npos;
    };
...
void _SetGenericMetadataListOpItems(const TfType& fieldType, Sdf_TextParserContext& context) {
...
bool _IsGenericMetadataListOpType(const TfType& type, TfType* itemArrayType) {
    static std::pair<TfType, TfType> listOpAndArrayTypes[] = {
            _GetListOpAndArrayTfTypes<SdfIntListOp>(),    _GetListOpAndArrayTfTypes<SdfInt64ListOp>(),
            _GetListOpAndArrayTfTypes<SdfUIntListOp>(),   _GetListOpAndArrayTfTypes<SdfUInt64ListOp>(),
            _GetListOpAndArrayTfTypes<SdfStringListOp>(), _GetListOpAndArrayTfTypes<SdfTokenListOp>(),
    };

    auto iter = std::find_if(std::begin(listOpAndArrayTypes), std::end(listOpAndArrayTypes), [&type](auto const& p) {
        return p.first == type;
    });
...
SdfSpecType _GetSpecTypeFromContext(Sdf_TextParserCurrentParsingContext parsingContext) {
    if (parsingContext == Sdf_TextParserCurrentParsingContext::AttributeSpec) {
        return SdfSpecTypeAttribute;
    }
    if (parsingContext == Sdf_TextParserCurrentParsingContext::RelationshipSpec) {
    ...
    if (parsingContext == Sdf_TextParserCurrentParsingContext::VariantStatementList) {
        return SdfSpecTypePrim;
    }
    return SdfSpecTypeUnknown;
}

void _PushContext(Sdf_TextParserContext& context, Sdf_TextParserCurrentParsingContext newParsingContext) {
    TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT).Msg("Pushing context: " + _ContextToString(newParsingContext) + "\n");
    if (!context.parsingContext.empty()) {
        TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT).Msg("Parent: " + _ContextToString(context.parsingContext.back()) + "\n");
    }

    TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT).Msg("Current path: " + context.path.GetAsString() + "\n\n");
...
void _PopContext(Sdf_TextParserContext& context) {
    TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT)
            .Msg("Popping context: " + _ContextToString(context.parsingContext.back()) + "\n");

    context.parsingContext.pop_back();

    if (!context.parsingContext.empty()) {
        TF_DEBUG(SDF_TEXT_FILE_FORMAT_CONTEXT)
                .Msg("Return to: " + _ContextToString(context.parsingContext.back()) + "\n");
    }
}
