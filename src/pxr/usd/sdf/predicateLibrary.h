...
public:
    ...
    explicit SdfPredicateFunctionResult(bool value) : SdfPredicateFunctionResult(value, MayVaryOverDescendants) {}

    /// Construct with \p value and \p constancy.
    SdfPredicateFunctionResult(bool value, Constancy constancy) : _value(value), _constancy(constancy) {}

    /// Create with \p value and 'ConstantOverDescendants'
    static SdfPredicateFunctionResult MakeConstant(bool value) { return {value, ConstantOverDescendants}; }

    /// Create with \p value and 'MayVaryOverDescendants'
    static SdfPredicateFunctionResult MakeVarying(bool value) { return {value, MayVaryOverDescendants}; }
    ...
    /// Return GetValue().
    operator UnspecifiedBoolType() const { return _value ? &SdfPredicateFunctionResult::_value : nullptr; }
...
private:
    ...
    friend bool operator!=(SdfPredicateFunctionResult lhs, SdfPredicateFunctionResult rhs) { return !(lhs == rhs); }

    friend bool operator==(SdfPredicateFunctionResult pfr, bool rhs) { return pfr._value == rhs; }
    friend bool operator==(bool lhs, SdfPredicateFunctionResult pfr) { return lhs == pfr._value; }
    friend bool operator!=(SdfPredicateFunctionResult pfr, bool rhs) { return pfr._value != rhs; }
    friend bool operator!=(bool lhs, SdfPredicateFunctionResult pfr) { return lhs != pfr._value; }
...
template <class DomainType>
class SdfPredicateLibrary {
    friend SdfPredicateProgram<DomainType> SdfLinkPredicateExpression<DomainType>(SdfPredicateExpression const& expr,
...
public:
    ...
    SdfPredicateLibrary& Define(std::string const& name, Fn&& fn, NamesAndDefaults const& namesAndDefaults) {
        ...
        if (auto obinder = _OverloadBinder<std::decay_t<Fn>>::TryCreate(std::forward<Fn>(fn), namesAndDefaults)) {
...
private:
    PredicateFunction _BindCall(std::string const& name, std::vector<SdfPredicateExpression::FnArg> const& args) const {
    ...
    static void _CheckOneNameAndDefault(bool& valid,
        ...
        if (!param.val.IsEmpty() && !param.val.CanCast<ParamType>()) {
            TF_CODING_ERROR(
                    ...
                    "type '%s' at index %zu",
                    param.name.c_str(), param.val.GetTypeName().c_str(), ArchGetDemangled<ParamType>().c_str(), index);
            valid = false;
        }
    }

    template <class ParamsTuple, size_t... I>
    static bool _CheckNamesAndDefaultsImpl(NamesAndDefaults const& namesAndDefaults, std::index_sequence<I...>) {
        ...
        int unused[] = {0, (_CheckOneNameAndDefault<std::tuple_element_t<N - I - 1, ParamsTuple>>(valid, N - I - 1, N,
                                                                                                  namesAndDefaults),
                            0)...};
        TF_UNUSED(unused);
        return valid;
    ...
    static bool _CheckNamesAndDefaultsWithSignature(NamesAndDefaults const& namesAndDefaults) {
        ...
        static_assert(std::is_convertible<DomainType, DomainArgType>::value, "");
        ...
        if (!params.empty()) {
            ...
            return _CheckNamesAndDefaultsImpl<ParamsTuple>(namesAndDefaults,
    ...
    static void _TryBindOne(size_t index,
        ...
        auto const* posArg = (index < args.size() && args[index].argName.empty()) ? &args[index] : nullptr;
    ...
    static bool _TryBindArgs(ParamsTuple& params,
        ...
        // _TryBindOne N times.
        int unused[] = {0, (_TryBindOne(I, std::tuple_size<ParamsTuple>::value, std::get<I>(params), bound, args,
                                        boundArgs, namesAndDefaults),
                            0)...};
        TF_UNUSED(unused);
        return bound;
    }
    ...
    static void _FillArbitraryArgs(std::true_type,
                                   ...
                                   Tuple& typedArgs) {
        std::vector<SdfPredicateExpression::FnArg>& rest = std::get<std::tuple_size<Tuple>::value - 1>(typedArgs);
    ...
    static PredicateFunction _TryToBindCall(Fn const& fn,
        ...
        static const bool TakesArbitraryArgs =
                _TakesArbitraryArgs<ParamsTuple>(std::integral_constant<bool, Traits::Arity >= 2>{});
        ...
        if (args.size() > maxArgs) {
            TF_RUNTIME_ERROR("Function takes at most %zu argument%s, %zu given", maxArgs, maxArgs == 1 ? "" : "s",
                             args.size());
            return {};
        }
        ...
        std::vector<bool> boundArgs;
        if (_TryBindArgs(typedArgs, args, namesAndDefaults, std::make_index_sequence<NumBindableArgs>{}, boundArgs)) {
            _FillArbitraryArgs(std::integral_constant<bool, TakesArbitraryArgs>{}, args, boundArgs, typedArgs);
            return [typedArgs, fn](DomainType const& obj) {
                return SdfPredicateFunctionResult{std::apply(fn, std::tuple_cat(std::make_tuple(obj), typedArgs))};
            };
        }
        return {};
    }
    ...
    struct _OverloadBinderBase {
        ...
        PredicateFunction Bind(std::vector<SdfPredicateExpression::FnArg> const& args) const { return _Bind(args); }
        virtual std::unique_ptr<_OverloadBinderBase> Clone() const = 0;

    protected:
        _OverloadBinderBase() = default;

        explicit _OverloadBinderBase(NamesAndDefaults const& namesAndDefaults) : _namesAndDefaults(namesAndDefaults) {}

        virtual PredicateFunction _Bind(std::vector<SdfPredicateExpression::FnArg> const& args) const = 0;
    ...
    pxr_tsl::robin_map<std::string, std::vector<_OverloadBinderBasePtr>> _binders;
};
